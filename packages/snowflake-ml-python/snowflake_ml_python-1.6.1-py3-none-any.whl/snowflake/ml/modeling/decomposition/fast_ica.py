...
class FastICA(BaseTransformer):
    ...
    def __init__(  # type: ignore[no-untyped-def]
        ...
        *,
        ...
        init_args = {'n_components':(n_components, None, False),
            ...
            'whiten':(whiten, "unit-variance", False),
            'fun':(fun, "logcosh", False),
            'fun_args':(fun_args, None, False),
            'max_iter':(max_iter, 200, False),
            'tol':(tol, 0.0001, False),
            'w_init':(w_init, None, False),
            'whiten_solver':(whiten_solver, "svd", False),
            'random_state':(random_state, None, False),}
        cleaned_up_init_args = validate_sklearn_args(
            args=init_args,
            klass=sklearn.decomposition.FastICA
        )
        self._sklearn_object: Any = sklearn.decomposition.FastICA(
        ...
        self._class_name=FastICA.__class__.__name__
    ...
    def set_input_cols(self, input_cols: Optional[Union[str, Iterable[str]]]) -> "FastICA":
        """
    ...
    def transform(self, dataset: Union[DataFrame, pd.DataFrame]) -> Union[DataFrame, pd.DataFrame]:
        ...
        For more details on this function, see [sklearn.decomposition.FastICA.transform]
        (https://scikit-learn.org/stable/modules/generated/sklearn.decomposition.FastICA.html#sklearn.decomposition.FastICA.transform)
