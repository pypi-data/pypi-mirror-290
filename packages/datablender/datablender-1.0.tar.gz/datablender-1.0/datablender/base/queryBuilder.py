...
class QueryBuilder:
    ...
    def __init__(
        ...
        self.config = config if config else File(
            path = os.path.join(
                os.path.dirname(__file__),
                '..',
                'configuration.json'
    ...
    def built(self) -> QueryBuilder:
        ...
        if self.definition == 'create':
            ...
            self.text_elements.append('a') if self.from_table else None
            self.text_elements.append('set')
            self.text_elements.append(','.join([
                ' = '.join([
                    self.checkVocabulary(column),
                    'b.{}'.format(self.checkVocabulary(value)) if self.from_table else self.setValues(value)
            ...
            if self.name:
                ...
                self.text_elements.append('from')
                self.text_elements.append('({})'.format(self.query))
        ...
        **kwargs
        ...
        if self.definition == 'select':
            ...
            if use_engine:
                ...
                self.resetAttributes()
                if is_transaction:
                    tr = self.connection.connection.transaction()
                    try:
                        await tr.start() 
                        values = await self.connection.connection.execute(
                            query
                        )
                    except Exception as error:
                        await tr.rollback()
    ...
    def where(
        ...
        if statement:
            ...
            self.where_statement=' and\n    '.join([
                ' = '.join([
                    '{}{}'.format(
                        'a.' if self.from_table else '',
                        self.checkVocabulary(column.get('name'))
                    ),
                    'b.{}'.format(
                        self.checkVocabulary(column.get('value'))
                    ) if self.from_table else self.setValues(column.get('value'))
