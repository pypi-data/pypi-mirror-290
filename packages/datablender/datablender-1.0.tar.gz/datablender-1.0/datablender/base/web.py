...
class Bot:
    ...
    def executeAction(self,action:BotAction) -> Union[list[WebElement],WebElement,None]:
        """Execute bot action.
    ...
    def executeActions(self,actions:list,past_values:list=[]):
        """Execute actions.
        ...
        if action.multi_mode=='replace':
            ...
            for value in action.getPossibleValues(past_values[-1] if past_values else None):
                ...
                self.executeMultiAction(
                    value,action,copy.deepcopy(actions),initial_url,copy.deepcopy(past_values)
                )
    ...
    def executeMultiAction(
        ...
        if action_.execute_previous_action:
            self.manageAction(self.previous_action,value)
