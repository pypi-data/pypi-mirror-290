...
from datablender.base import manageQuery, getFunction
...
class Data:
        ...
        **kwargs
        ...
        if (
            ...
            if geometry.get('saved_type') == 'text':
                series = series.apply(
                    lambda x: shapely.wkb.loads(str(x), hex=True)
                )
            ...
            if geometry.get('srid_from') is not None:
                ...
                if geometry.get('saved_type') in ['shapely','text']:

                
                    coords = shapely.get_coordinates(series)

                    transformer(coords[0, 0], coords[0, 1])
                    new_coords_z = transformer(coords[:, 0], coords[:, 1])
                    
                    series = pandas.Series(shapely.set_coordinates(
                        series.copy(),
                        numpy.array(new_coords_z).T
                    ))
            ...
            if export_format == 'geojson':   
                ...
                properties = self.frame[columns].astype(object)
        ...
        elif str(dtype) == 'object' and is_sample:
        ...
        return [
            None if not isinstance(a,list) and pandas.isna(a) else int(a) if isinstance(a,numpy.integer) else a
            for a in series
        ] if export_format == 'json' else series
    ...
    def exportGeojson(
        ...
        for index, row in enumerate(properties.values):
            ...
            yield self.exportGeometry(
                ...
                geometries[index],
                export_format,
                properties=dict(zip(columns,[None if pandas.isna(r) else r for r in row])),
                **geometry
            )
    ...
    def setType(
        ...
        **kwargs
        ...
        if geometry is not None:
            ...
            meta_geometry = {
                'type':re.search(r'\([a-z]+,',column_type.lower()).group()[1:-1],
    ...
    def export(
        ...
        #     return ParseDict(self.frame.to_dict('records'),)
        #     return geobuf.encode(self.frame.to_dict('records'))
    ...
    def select(
        ...
        self.frame = self.query_builder.select().sqlStatement(
    ...
    def write(
        ...
        **kwargs
        ...
        if column_name is not None:
            return 'http://localhost:8003/{}'.format(file_name)
