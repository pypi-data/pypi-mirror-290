...
class AsyncTable(AsyncSchemaElement):
    ...
    async def initiate(self) -> AsyncTable:
        ...
        if self.db_element:
            partitions = await self.query_builder.selectElements(
                'partition'
            ).asyncExecute()
    ...
    async def manage(
        ...
        if self.name in ['counts','locations']:
            print('-----------------------------')
            print('-----------------------------')
            print(self.name)
            print('status : ',self.status)
            print('database_name : ',self.connection.database_name)
            print('db_element name : ',self.db_element.get('name'))
            print('should_exists : ',self.should_exists)
            print('create : ',(not self.db_element and self.should_exists))
    ...
    async def structureData(
        ...
        if self.data is not None:
            ...
            if self.db_element:
                ...
                return {
                    'columns':copy.deepcopy(self.data.columns),
