...
class AdpIngest(object):
    ...
    def create_cloud_storage_link(self, storage_link_name : str, uri : str, credential_name :str, auth_user :str, auth_password : str, description : str = None) -> str:
        '''
            ...
            @param authUser (String): OCI user name
            @param authPassword (String): Auth Token
        ...
        payload = {'storage_link_name': storage_link_name,
                   'uri': uri,
                   'metadata': '{\"description\":\"' + description + '\",\"storageType\":\"ObjectStore\"}',
                   'credential_name': '\"' +  credential_name + '\"',
                   'credential_user': auth_user,
                   'credential_pwd': auth_password,
                   'edit': 'no'}
    ...
    def drop_cloud_storage_link(self, storage_link_name : str) -> str:
        ...
        return self.rest.delete(url, self.rest.stringify(payload))

    def get_cloud_buckets(self, owner : str = None) -> str:
        '''
            Get cloud buckets
    ...
    def _generate_metadata_source(self, manifest : dict) -> list:
        ...
        for column in manifest[0]['metadataSource']:
            column_metadata = {'columnName':column['columnName'], 'dataType': column['dataType'], 'columnId':column['columnId'], 'fieldName': column['fieldName'], 'skipColumn': False }
