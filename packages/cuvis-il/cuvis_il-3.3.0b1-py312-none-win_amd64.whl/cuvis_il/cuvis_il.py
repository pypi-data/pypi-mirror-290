...
def cuvis_acq_cont_get_pixel_format_swig(i_acqCont, i_id):
    return _cuvis_pyil.cuvis_acq_cont_get_pixel_format_swig(i_acqCont, i_id)

def cuvis_acq_cont_get_available_pixel_format_swig(i_acqCont, i_id, i_index):
    return _cuvis_pyil.cuvis_acq_cont_get_available_pixel_format_swig(i_acqCont, i_id, i_index)
...
class cuvis_imbuffer_t(object):
    ...
    If the 'imbuffer' is not a hyperspectral cube, the value will be nullptr
...
class cuvis_calibration_info_t(object):
    ...
    file_path = property(_cuvis_pyil.cuvis_calibration_info_t_file_path_get, _cuvis_pyil.cuvis_calibration_info_t_file_path_set, doc=r""" calibration unique ID""")
...
class cuvis_save_args_t(object):
    ...
    Only used if 'allow_session' is set.
    """)
    ...
    only used if 'allow_session=true' and _mode=Internal.
    """)
...
class cuvis_viewer_settings_t(object):
    ...
    pre_pan_sharpen_cube = property(_cuvis_pyil.cuvis_viewer_settings_t_pre_pan_sharpen_cube_get, _cuvis_pyil.cuvis_viewer_settings_t_pre_pan_sharpen_cube_set, doc=r""" pansharpen cube before user plugin""")
...
def cuvis_init(i_settings_path, i_global_loglevel):
    r"""
    ...
    """
    return _cuvis_pyil.cuvis_init(i_settings_path, i_global_loglevel)
...
def cuvis_async_capture_get(io_pAsyncResult, timeout_ms, o_pMesu):
    ...
    :type io_pAsyncResult: int, in/out
    :param io_pAsyncResult: the async handle obtained by calling 'capture_async'. If the call finished, the handle will be invalidated
    :type timeout_ms: int, in
    :param timeout_ms: the timeout in ms. Give 0 to wait for ever.
    :type o_pMesu: int, out
...
def cuvis_session_file_load(i_path, o_pSess):
    ...
    The session_info file is a cu3s file and consists of binary cu3 measurement data. Call 'cuvis_session_get_mesu'
...
def cuvis_session_file_get_mesu(i_sess, i_frameNo, i_type, o_pMesu):
    ...
    :param i_frameNo: the frame no. Counting from 0, must be below value of 'cuvis_session_get_size' of it's respective 'i_type'
    :type i_type: int, in
...
def cuvis_session_file_get_reference_mesu(i_sess, i_frameNo, i_type, o_pMesu):
    ...
    :param i_sess: the session_info file handle
    :type i_frameNo: int, in
    :param i_frameNo: the reference number. Counting from 0. If 'i_type' is not set, refers to the index of all references and must be below the value of 'cuvis_session_get_size' using type session_item_type_references. If 'i_type' is set, must be 0.
    :type i_type: int, in
...
def cuvis_session_file_get_thumbnail(i_sess, o_pThumbnail):
    ...
    :return: 'status_ok' if the buffer could be filled with the image element.
                 'status_unavailable' if the requested data was empty, or the key could not be found
    """
...
def cuvis_session_file_get_fps(i_sess, o_pFps):
    ...
    The session_info file meta-Information will be available only if the mode _session_get_operation_mode returns "Internal"
...
def cuvis_measurement_set_comment(i_mesu, i_comment):
    ...
    :param i_name: The new measurement's comment
    :rtype: int
    :return: 'status_ok', if the measurement's name was set successfully.
    """
...
def cuvis_acq_cont_get_component_info(i_acqCont, i_id, o_pCompInfo):
    ...
    :param i_acq_Cont: the acquisition context
    :type i_id: int, in
...
def cuvis_acq_cont_set_pixel_format(i_acqCont, i_id, i_pPixelFormat):
    r"""
    ...
    """
    return _cuvis_pyil.cuvis_acq_cont_set_pixel_format(i_acqCont, i_id, i_pPixelFormat)

def cuvis_acq_cont_get_available_pixel_format_count(i_acqCont, i_id, o_pCount):
    r"""
    ...
    """
    return _cuvis_pyil.cuvis_acq_cont_get_available_pixel_format_count(i_acqCont, i_id, o_pCount)
...
def cuvis_proc_cont_apply(i_procCont, i_mesu):
    ...
    Those get set via 'cuvis_proc_set_proc_args'
...
def cuvis_proc_cont_set_args(i_procCont, i_args):
    ...
    :type i_procCont: int, in
    :param i_procCont: The handle of the processing context
    :param i_processingMode: The processing mode to be checked
    :rtype: int
    :return: 'status_ok' if measurement was processed.
    """
...
def cuvis_measurement_get_capabilities(i_mesu, o_pCapabilities):
    ...
    :param i_calibration: The measurement
    :type o_pCapabilities: int, out
    :param o_pCapabilities: write the capabilites here. See CUVIS_MODE_CAPABILITIES_x flags.
    """
...
def cuvis_worker_get_can_skip_measurements(i_worker, o_pCanSkip):
    ...
    :param o_pCanSkipSupplementary: If 1, the worker is allowed to skip supplementary processing of measurements, if the supplementary queue is full
    """
...
def cuvis_acq_cont_binning_get(i_acqCont, o_pvalue):
    return _cuvis_pyil.cuvis_acq_cont_binning_get(i_acqCont, o_pvalue)

def cuvis_acq_cont_binning_set(i_acqCont, value):
    return _cuvis_pyil.cuvis_acq_cont_binning_set(i_acqCont, value)

def cuvis_acq_cont_binning_set_async(i_acqCont, o_pAsyncResult, value):
    return _cuvis_pyil.cuvis_acq_cont_binning_set_async(i_acqCont, o_pAsyncResult, value)
