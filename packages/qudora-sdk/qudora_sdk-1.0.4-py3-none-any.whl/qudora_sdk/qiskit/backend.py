...
def circuit_to_qir(circuit: QuantumCircuit) -> str:

    qasm3_string = qasm3.dumps(circuit)
    qir = qasm3_to_qir(qasm3_string, name="my-program")
    input_qir_bitcode = base64.b64encode(qir.bitcode).decode('ascii')

    return input_qir_bitcode
...
class QUDORABackend(Backend):
    ...
    def run(self, run_input: QuantumCircuit | List[QuantumCircuit], job_name: str = "Job from Qiskit-Provider", backend_settings : dict = None, **kwargs) -> QUDORAJob:
        ...
        # convert inputs to OpenQASM
        language = 'OpenQASM2'
        try:
            input_data = [circuit_to_openqasm(circ) for circ in run_input]

        except qasm2.exceptions.QASM2ExportError as exc:
            language = 'QIR_BITCODE'
            input_data = [circuit_to_qir(circ) for circ in run_input]
