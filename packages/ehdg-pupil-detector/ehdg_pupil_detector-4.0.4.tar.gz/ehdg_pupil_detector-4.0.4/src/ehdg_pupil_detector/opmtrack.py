...
def make_dir_with_error_handler(dir_input, dir_name_input):
    if not os.path.isdir(dir_input):
        try:
            os.mkdir(dir_input)
            print(f"{dir_name_input} is created.")
            print(f"{dir_name_input} : {dir_input}.")
        except FileNotFoundError:
            print(f"{dir_name_input} cannot be created.")
            print(f"{dir_name_input} : {dir_input}.")
            return
        except OSError:
            print(f"{dir_name_input} cannot be created.")
...
def split_video_in_half_with_ffmpeg(input_video_dir, left_video_dir=None, right_video_dir=None):
    ...
    if ffmpeg_exist:
        ...
        if right_video_dir is None:
            right_video_dir = str(input_video_dir).replace(input_file_name, f"right_split_{input_file_name}")
...
def main():
    ...
    if binary_threshold is not None:
        if "," in str(binary_threshold):
            try:
                min_th, max_th = str(binary_threshold).split(",")
                try:
                    min_th = int(min_th)
                    if 0 <= min_th <= 255:
                        config_dict["min_binary_threshold"] = min_th
                    else:
                        print(f"Invalid minimum threshold input: {min_th}")
                        print(f"Minimum threshold must be between 0 and 255.")
                        return
                    max_th = int(max_th)
                    if 0 <= max_th <= 255:
                        config_dict["max_binary_threshold"] = max_th
                    else:
                        print(f"Invalid maximum threshold input: {max_th}")
                        print(f"Maximum threshold must be between 0 and 255.")
                        return
                    if min_th >= max_th:
                        print("Minimum threshold must be less than maximum threshold.")
                        print(f"But it is : {str(binary_threshold)}")
                        return
                except Exception as e:
                    print(f"Error in retrieving binary threshold: {e}")
                    print("Invalid binary threshold input.")
                    print(f"It is : {str(binary_threshold)}.")
                    print("Example input:")
                    print("-bth 60,255")
                    return
            except Exception as e:
    ...
    if should_use_roi:
        if config_indicator == "pnm":
            ...
            if not os.path.isdir(l_out_dir):
                make_dir_with_error_handler(l_out_dir, "Left eye output directory")
            r_out_dir = str(output_directory).replace(output_folder_name, f"right_{output_folder_name}")
            if not os.path.isdir(r_out_dir):
                make_dir_with_error_handler(l_out_dir, "Right eye output directory")
            ...
            if not os.path.isdir(output_directory):
                make_dir_with_error_handler(output_directory, "Output directory")
