...
cpu = '{} 核'.format(_extInfo['cpu']['count'])
_gpu = _extInfo['gpu']['list'][0].split(' ')
gpu = _gpu[-1] if _gpu[-1] == 'Normal' else _gpu[-2] + ' ' + _gpu[-1]
_mem = float(_extInfo['memory']['free'].split(' ')[0]) if 'autodl' in host_name else float(_extInfo['memory']['total'].split(' ')[0])
mem = '{}G'.format(math.ceil(_mem / 1024))
...
def GetOssUrl(country, ext, region='', timeout=3):
