        ...
        while self.is_running or not self.task_queue.empty():
            try:
                ...
                try:
                    ...
                    is_ok, msg, result, is_counter = task.runTask(data, timeout, country, widget_name, _appendTask, gettask, ext, self.next_task, map)
                    if is_ok == False:
...
class MecordLongConnectThread(Thread):
    ...
    def __init__(self, country, executor, mtype=''):
        ...
        _extInfo = store.readDeviceInfo()
        extInfo = {}
        extInfo['C'] = '{} 核'.format(_extInfo['cpu']['count'])
        gpu = _extInfo['gpu']['list'][0].split(' ')
        extInfo['G'] = gpu[-1] if gpu[-1] == 'Normal' else gpu[-2] + ' ' + gpu[-1]
        host_name = utils.get_hostname() + ('(H)' if self.mtype == '-h' else '')
        _mem = float(_extInfo['memory']['free'].split(' ')[0]) if 'autodl' in host_name else float(_extInfo['memory']['total'].split(' ')[0])
        extInfo['M'] = '{}G'.format(math.ceil(_mem / 1024))
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            if limit >= 1 and len(self.widgets) >= 1:
                ...
                try:
                    datas, timeout = xy_pb.GetTask(self.service_country, self.widgets, limit=limit, keep_alive=False, reqid=traceid)
                    if len(datas) > 0:
