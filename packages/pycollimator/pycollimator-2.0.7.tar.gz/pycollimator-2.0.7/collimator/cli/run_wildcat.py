...
def run_batch_simulation(parameters: list[dict[str, Any]]):
    ...
    warned_about_changing_ports = False

    def scan_all_output_ports():
        all_output_ports = {}
        nodes_to_scan = [diagram]
        scanned = set()
        while nodes_to_scan:
            ...
            for port in node.output_ports:
                all_output_ports[f"{node.name_path_str}.{port.name}"] = id(port)
            if isinstance(node, collimator.framework.diagram.Diagram):
                nodes_to_scan.extend(node.nodes)
        return all_output_ports

    all_original_output_ports = scan_all_output_ports()

    def _run_sim(config, check_types=True):
        nonlocal warned_about_changing_ports
        ...
        if not warned_about_changing_ports:
            all_output_ports = scan_all_output_ports()
            if all_output_ports != all_original_output_ports:
                changed_ports = set()
                for port_path, port_id in all_output_ports.items():
                    if port_id != all_original_output_ports.get(port_path):
                        changed_ports.add(port_path)

                # FIXME: this warning is not user visible (DASH-1738)
                logger.warning(
                    "Some output ports have changed after setting model parameters. "
                    "This may lead to incorrect results in ensemble simulations.\n"
                    "This is likely not a problem with your model, but rather an "
                    "internal limitation of the current implementation.\n"
                    "Changed ports: %s",
                    ", ".join(changed_ports),
                )
                warned_about_changing_ports = True

                # FIXME: we can't do much here today:
                # 1. Too many blocks abuse configure_output_port() when they
                #    should not, so the above detection is too sensitive.
        ...
        # output ports may have changed (this is correct for numpy backend, but has no
        # effect on JAX -- we just end up lucky quite often).
    ...
    for i, config in enumerate(configs):
        check_types = i == 0  # only check types for the first run
        result = _run_sim(config, check_types=check_types)
