...
class AcausalSystem(LeafSystem):
    ...
    def __init__(
        ...
        sym_args = (time, self.ir.se_x, self.ir.se_y, *inputs, *params)

        # Continuous State
        self._configure_continuous_state(sym_args, leaf_backend)

        # Output Ports
        self._configure_outputs(sym_args, dp.outp_exprs, leaf_backend)

        # Zero Crossing
        self._configure_zcs(sym_args, dp.zcs, leaf_backend)
...
class AcausalCompiler:
    ...
    def generate_acausal_system(
        ...
        with build_recorder.paused():
            system = AcausalSystem(self.dp, self.ir, name, leaf_backend)
        build_recorder.compile_acausal_diagram(system)

        return system

    # execute compilation
    def __call__(self, name="acausal_system", leaf_backend="jax", return_ir=False):
        self.diagram_processing()
        self.index_reduction()
        system = self.generate_acausal_system(name=name, leaf_backend=leaf_backend)
        if return_ir:
            return system, self.ir
        return system
