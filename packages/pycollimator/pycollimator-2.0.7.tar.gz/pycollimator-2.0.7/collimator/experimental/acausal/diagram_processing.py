...
class DiagramProcessing:
    ...
    def identify_fluid_networks(self):
        ...
        fp_cmps = [c for c in self.diagram.comps if isinstance(c, FluidProperties)]
        if dbg_print:
        ...
        for fp, node_ids in self.fld_nws.items():
            ...
            # FIXME: if these fluid syms are true constants, then we only need to them once per
            # instance of any fluid class, not for each instacne of a fluid class.
                    ...
                    for port_tuple_i in port_set_mflow:
                        ...
                        for port_tuple_j in port_set_j:
                            ...
                            mflow_max_expr_j = sp.Piecewise(
                                (-mflow_j, -mflow_j > M_FLOW_EPS), (M_FLOW_EPS, True)
                            )
