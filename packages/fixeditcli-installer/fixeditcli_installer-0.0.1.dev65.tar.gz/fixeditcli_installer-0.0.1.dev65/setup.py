...
class install_wrapper(_install):
    ...
    def _install_fixeditcli(self):
        try:
            from src.installer_module.installer import install_fixeditcli, update_config, decode_token, read_version, get_fixedit_token
...
setup(
    ...
    entry_points={
        'console_scripts': [
            'fixeditcli-install=installer_module.installer:entry'
        ]
    },
    package_data={
        "installer_module": ["*.py"],
        "installer_module.data": ["*.txt"],
    },
    python_requires=">=3.8",
