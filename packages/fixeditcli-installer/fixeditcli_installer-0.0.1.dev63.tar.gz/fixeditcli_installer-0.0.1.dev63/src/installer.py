...
def read_version():
    ...
    if (version := os.getenv("FIXEDITCLI_VERSION")):
        return version

    # Get the relative path using Pathlib
    script_dir = Path(__file__).resolve().parent  # Directory of installer.py (src)
    version_file_path = script_dir.parent / "version.txt"  # One level up from src

    # Print contents of the installer.py directory (src) and parent directory
    logger.info(f"Installer script dir contents: {list(script_dir.iterdir())}")
    logger.info(f"Parent directory contents: {list(script_dir.parent.iterdir())}")
    ...
    try:
        ...
        raise ValueError(
            ...
            "same directory as the setup.py file or export the desired version environment variable "
            "(FIXEDITCLI_VERSION)."
        ) from e
    return version
