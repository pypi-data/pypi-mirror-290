...
def read_version_entry():
    ...
    try:
        version = pkg_resources.resource_string('fixeditcli_installer', "version.txt").decode("utf-8").strip()
    except (FileNotFoundError, IOError) as e:
        raise ValueError(
            "Could not read the version file. Make sure that the version file is present in the package."
        ) from e
    return version
