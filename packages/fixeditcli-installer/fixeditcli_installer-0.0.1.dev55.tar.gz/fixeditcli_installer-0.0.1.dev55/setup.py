...
class install_wrapper(_install):
    ...
    def _install_fixeditcli(self):
        try:
            from src.installer import install_fixeditcli, update_config, decode_token, read_version, get_fixedit_token
...
setup(
    ...
    entry_points={
        'console_scripts': [
            'fixeditcli-install=fixeditcli_installer.installer:entry'
        ]
    },
    package_data={"": ["src/*.py", "src/*.txt"]},
