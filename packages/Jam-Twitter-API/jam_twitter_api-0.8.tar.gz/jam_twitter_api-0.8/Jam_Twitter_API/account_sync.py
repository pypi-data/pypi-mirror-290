...
class TwitterAccountSync:
    ...
    def get_user_by_screen_name(self, screen_name: str, with_safety_mode_user_fields: bool = True) -> dict:
        variables = {"screen_name": screen_name, "withSafetyModeUserFields": with_safety_mode_user_fields}
        return self.gql("GET", Operation.UserByScreenName, variables)
    ...
    def user_tweets(self, username: str, limit: int = 40) -> list[dict[str, str | Any]]:
        user_id = self.get_user_id(username)
        variables = {"userId": user_id, "count": 20}
        tweets_data = []

        while len(tweets_data) < limit:
            json_data = self.gql("GET", Operation.UserTweets, variables)
    ...
    def get_relationships(self, source_username: str, target_username: str):
        headers = get_headers(self.session)
        headers["content-type"] = "application/x-www-form-urlencoded"
