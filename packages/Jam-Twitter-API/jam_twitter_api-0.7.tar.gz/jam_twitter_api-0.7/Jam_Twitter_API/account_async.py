...
from typing_extensions import Coroutine, List, Any, LiteralString
...
class TwitterAccountAsync:
    def __init__(self):
        self._session: requests.AsyncSession = requests.AsyncSession()
    ...
    @staticmethod
    def __get_session(proxy: str = None) -> requests.AsyncSession:
        session = requests.AsyncSession(timeout=30, verify=False)
        if proxy:
    ...
    async def v1(self, path: str, _params: dict) -> dict:
        headers = get_headers(self.session)
    ...
    def _verify_response(r: Response) -> dict:
        try:
            ...
            if rate_limit_remaining and int(rate_limit_remaining) in (0, 1):
                ...
                raise RateLimitError(reset_ts - int(time.time()))
    ...
    async def request_ct0(self) -> str:
        url = "https://twitter.com/i/api/2/oauth2/authorize"
        r = await self.session.get(url, allow_redirects=True)
    ...
    async def request_guest_token(
            self, session: AsyncSession, csrf_token: str = None
    ) -> str:
    ...
    async def __setup_session(self):
        ...
        guest_token = await self.request_guest_token(session, generated_csrf_token)
        ...
        csrf_token = await self.request_ct0()
    ...
    async def bind_account_v1(self, url: str) -> dict | Coroutine[Any, Any, dict[str, LiteralString | Any]]:
        """
        ...
        async def get_oauth_token() -> str:
            ...
            _response = await _session.get(url, allow_redirects=True)
        ...
        async def get_authenticity_token(_oauth_token: str) -> dict[str, LiteralString | Any] | str:
            _params = {
                "oauth_token": _oauth_token,
            }
            _response = await self.session.get(
        ...
        async def get_confirm_url(_oauth_token: str, _authenticity_token: str) -> str:
            _data = {
        ...
        async def process_confirm_url(_url: str) -> dict[str, LiteralString | Any]:
            response = await self.session.get(_url, allow_redirects=True)
            raise_for_status(response)
        ...
        authenticity_token = await get_authenticity_token(oauth_token)

        if isinstance(authenticity_token, dict):
            return authenticity_token

        confirm_url = await get_confirm_url(oauth_token, authenticity_token)
        return process_confirm_url(confirm_url)

    async def bind_account_v2(self, bind_params: dict[str, str]) -> str:
        """
        ...
        async def approve_auth_code(_auth_code: str) -> str:
            _params = {
                "approval": "true",
                "code": _auth_code,
            }
        ...
        auth_code = await get_auth_code()
        approved_code = await approve_auth_code(auth_code)
    ...
    async def create_poll(self, text: str, choices: list[str], poll_duration: int) -> dict:
        ...
        _data = await self.tweet(text, poll_params={"card_uri": card_uri})
        return _data
    ...
    async def email_phone_info(self) -> dict:
        """
    ...
    async def settings_info(self) -> dict:
        """
    ...
    async def screen_name(self) -> str:
        """
        :return: profile screen name
        """

        _data = await self.verify_credentials()
    ...
    async def registration_date(self) -> str:
        _data = await self.verify_credentials()
    ...
    async def dm(self, text: str, receivers: list[int], media: str = "") -> dict:
        ...
        if media:
            media_id = await self.upload_media(media, is_dm=True)
    ...
    async def custom_dm(self, text: str, receiver: int) -> dict:
        json_data = {
            "event": {
                "type": "message_create",
                "message_create": {
    ...
    async def delete_tweet(self, tweet_id: int | str) -> dict:
        variables = {"tweet_id": tweet_id, "dark_request": False}
        return await self.gql("POST", Operation.DeleteTweet, variables)

    async def get_user_by_screen_name(self, screen_name: str, with_safety_mode_user_fields: bool = True) -> dict:
        variables = {"screen_name": screen_name, "withSafetyModeUserFields": with_safety_mode_user_fields}
        return await self.gql("GET", Operation.UserByScreenName, variables)
        ...
        if media:
            for m in media:
                ...
                for tagged_user in m["tagged_users"]:
                    user_id = await self.get_user_id(tagged_user)
    ...
    async def quote(self, text: str, tweet_id: int, media: List[dict] = None) -> dict:
        """
    ...
    async def tweet_likes(
        ...
        while True:
            _data = await self.gql("GET", Operation.Favoriters, variables)
    ...
    async def tweet_retweeters(
        ...
        while True:
            _data = await self.gql("GET", Operation.Retweeters, variables)
    ...
    async def tweet_replies(
        ...
        while True:
            _data = await self.gql("GET", Operation.TweetDetail, variables)
    ...
    async def user_tweets(self, username: str, limit: int = 40) -> list[dict[str, str | Any]]:
        user_id = await self.get_user_id(username)
        variables = {"userId": user_id, "count": 20}
        tweets_data = []

        while len(tweets_data) < limit:
            json_data = await self.gql("GET", Operation.UserTweets, variables)
    ...
    async def get_relationships(self, source_username: str, target_username: str):
        headers = get_headers(self.session)
        headers["content-type"] = "application/x-www-form-urlencoded"

        r = await self.session.get(
            f"{self.v1_api}/friendships/show.json",
            headers=headers,
            params={"source_screen_name": source_username, "target_screen_name": target_username},
        )

        return self._verify_response(r)

    async def user_followers(self, username: str, limit: int = 200) -> list[str]:
        variables = {"screen_name": username, "count": 200}
        _users = []

        while True:
            r = await self.session.get(f"{self.v1_api}/followers/list.json", params=variables)
    ...
    async def user_followings(self, username: str, limit: int = 200) -> list[str]:
        variables = {"screen_name": username, "count": 200}
        _users = []

        while True:
            r = await self.session.get(f"{self.v1_api}/friends/list.json", params=variables)
            if r.status_code == 503:
    ...
    async def user_last_tweets(
        ...
        :param username:
        :return: list of last tweets
        """
        user_id = await self.get_user_id(username)
        json_data = await self.gql("GET", Operation.UserTweets, {"userId": user_id})
    ...
    async def update_profile_image(self, media: str) -> dict:
        media_id = await self.upload_media(media)
    ...
    async def update_profile_banner(self, media: str) -> dict:
        media_id = await self.upload_media(media)
        _params = {"media_id": media_id}
    ...
    async def update_profile_info(self, _params: dict) -> dict:
        headers = get_headers(self.session)
        r = await self.session.post(
    ...
    async def update_search_settings(self, settings: dict) -> dict:
        try:
    ...
    async def update_username(self, username: str):
        return await self.update_settings({"screen_name": username})
    ...
    async def home_timeline(self, limit: int) -> list[dict]:
        return await self._paginate(
            "POST", Operation.HomeTimeline, Operation.default_variables, limit
        )

    async def home_latest_timeline(self, limit: int) -> list[dict]:
        return await self._paginate(
            "POST",
    ...
    async def bookmarks(self, limit: int) -> list[dict]:
    ...
    async def _paginate(
        ...
        while (dups < DUP_LIMIT) and cursor:
            ...
            _data = await self.gql(method, operation, variables)
    ...
    async def upload_media(self, filename: str, is_dm: bool = False) -> int | None:
    ...
    async def dm_delete(self, conversation_id: str = None, message_id: str = None) -> dict:
        ...
        if conversation_id:
            results["conversation"] = await self.session.post(
        ...
        if message_id:
            ...
            results["message"] = await self.session.post(
    ...
    async def dm_search(self, query: str) -> dict:
        """
        ...
        res, cursor = await get()
        data_ = [res]

        while cursor:
            res, cursor = await get(cursor)
            data_.append(res)
    ...
    async def scheduled_tweets(self, ascending: bool = True) -> dict:
        ...
        return await self.gql("GET", Operation.FetchScheduledTweets, variables)

    async def delete_scheduled_tweet(self, tweet_id: int) -> dict:
        variables = {"scheduled_tweet_id": tweet_id}
        return await self.gql("POST", Operation.DeleteScheduledTweet, variables)

    async def clear_scheduled_tweets(self) -> None:
    ...
    async def draft_tweets(self, ascending: bool = True) -> dict:
        variables = {"ascending": ascending}
        return await self.gql("GET", Operation.FetchDraftTweets, variables)

    async def delete_draft_tweet(self, tweet_id: int) -> dict:
        variables = {"draft_tweet_id": tweet_id}
        return await self.gql("POST", Operation.DeleteDraftTweet, variables)

    async def clear_draft_tweets(self) -> None:
        user_id = int(re.findall('"u=(\d+)"', self.session.cookies.get("twid"))[0])
        drafts = await self.gql("GET", Operation.FetchDraftTweets, {"ascending": True})
        for _id in set(find_key(drafts, "rest_id")):
            if _id != user_id:
                await self.gql("POST", Operation.DeleteDraftTweet, {"draft_tweet_id": _id})

    async def notifications(self, _params: dict = None) -> dict:
        r = await self.session.get(
    ...
    async def recommendations(self, _params: dict = None) -> dict:
        r = await self.session.get(
    ...
    async def fleetline(self, _params: dict = None) -> dict:
        r = await self.session.get(
