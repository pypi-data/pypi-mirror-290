...
class RAG(object):
    ...
    def __init__(self,
                 chunk: list[str], 
                 embedding_model_id:str="BAAI/bge-base-en-v1.5") -> None:
        """
        :param chunk: The chunk to be stored in the vector database.        
        """
        docs = [Document(content=item, meta={"file_path":"document.md"}) for item in chunk]
        ...
        Question: {{question}}, using haystack component & pipeline interface
        Answer:
        """
    ...
    def query_response(self, question:str) -> str:
        response = self.basic_rag_pipeline.run({"text_embedder": {"text": question}, "prompt_builder": {"question": question}},debug=True)
        return response["llm"]["replies"][0]
    
    def generate_gradio_interface(self, demo_examples:list[str] = []):
        def yes_man(message, history):
            response = self.basic_rag_pipeline.run({"text_embedder": {"text": message}, "prompt_builder": {"question": message}})
            return response["llm"]["replies"][0]
        gr.ChatInterface(
            yes_man,
