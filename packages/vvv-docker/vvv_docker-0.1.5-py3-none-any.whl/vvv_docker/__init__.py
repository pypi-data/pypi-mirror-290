...
''').strip()
    ...
    def wrun_openresty(name, filepath_or_code, ports, log=True, type='auto', filename='./nginx.conf', extra='', remove=True):
        '''
        ports: ["80:80", "443:443"]
        '''
        ...
        if remove:
            ...
            if not wexist(filename):
                wupdate_by_type(filepath_or_code, filename, type)
        wstop_docker(name)
        _.wrun_cmd(f'docker run -d --name {name} {ports} -v {filename}:/usr/local/openresty/nginx/conf/nginx.conf {extra} openresty/openresty:latest')
        if log: _.wrun_cmd_loop(f'docker logs -f {name}')
    def wrun_openresty_ja3(name, filepath_or_code, ports, log=True, type='auto', filename='./nginx.conf', extra='', remove=True):
        '''
        ports: ["80:80", "443:443"]
        '''
        ports = ' '.join(['-p' + i for i in ports])
        if remove:
            ...
            if not wexist(filename):
                wupdate_by_type(filepath_or_code, filename, type)
        wstop_docker(name)
        _.wrun_cmd(f'docker run -d --name {name} {ports} -v {filename}:/usr/local/openresty/nginx/conf/nginx.conf {extra} cilame/openresty_ja3:latest')
        if log: _.wrun_cmd_loop(f'docker logs -f {name}')
    def wrun_nodejs(name, filepath_or_code, ports=[], log=True, type='auto', filename='./_nodejs.js', extra='', remove=True):
        '''
        ports: ["80:80", "443:443"]
        '''
        ports = ' '.join(['-p' + i for i in ports])
        if remove:
            ...
            if not wexist(filename):
                wupdate_by_type(filepath_or_code, filename, type)
        wstop_docker(name)
        _.wrun_cmd(f'docker run -d --name {name} {ports} -v {filename}:/1.js {extra} node:alpine node /1.js')
        if log: _.wrun_cmd_loop(f'docker logs -f {name}')
    def wrun_python(name, filepath_or_code, ports=[], log=True, type='auto', filename='./_python.py', extra='', remove=True):
        '''
        ports: ["80:80", "443:443"]
        '''
        ports = ' '.join(['-p' + i for i in ports])
        if remove:
            ...
            if not wexist(filename):
                wupdate_by_type(filepath_or_code, filename, type)
        wstop_docker(name)
        _.wrun_cmd(f'docker run -d --name {name} {ports} -v {filename}:/1.py {extra} cilame/py310:alpine python /1.py')
        if log: _.wrun_cmd_loop(f'docker logs -f {name}')
    def wrun_onnxruntime(name, filepath_or_code, ports=[], log=True, type='auto', filename='./_onnxruntime.py', files=[], extra='', remove=True, removefile=False):
        ...
        for fpath in files:
            ...
            if removefile:
                ...
                if not wexist(fpath):
                    wupdate_by_type(fpath, fname, 'file')
            vmap.append('-v ./' + fname + ':' + '/vvv/' + fname)
        vmap = ' '.join(vmap)
        wstop_docker(name)
        _.wrun_cmd(f'docker run -d --name {name} {ports} -v {filename}:/1.py {vmap} {extra} cilame/onnx:latest python /1.py')
        if log: _.wrun_cmd_loop(f'docker logs -f {name}')
    def wrun_gogs(name='gogs', ports=["10022:22", "10880:3000"], localdata='/var/gogs'):
        ...
        print('run gogs!')
    def wrun_chrome(name='default_chrome', ports=None):
        if not ports:
            ports = ["18999:18999"]
        ports = ' '.join(['-p' + i for i in ports])
        wstop_docker(name)
        wrun_cmd(f'docker run -d --name={name} {ports} chrome_slim:latest')
        print('run chrome!')
...
input_data = read_b64img(b64img, (40, 40))
...
''', files=["clz_net.claz", "clz_net.onnx"]) # localfile if set auto update and set in docker
    """
...
port must be "remoteport:18999", docker port is must be 18999.

commond:start:
    cmd> v_docker chrome start [name] [port]

commone:stop:
    cmd> v_docker chrome remove [name]
"""
...
def get_fpath(modulename):
    try:
        ...
        for i in os.listdir(path):
            if i.endswith('xz') or re.findall(r'\.xz\.\d+$', i):
...
def install(imagename):
    ...
    if imagename == "frp" or imagename == 'frps_0_59:alpine': 
        ...
        if not fpath: return
        update_a_docker(workspace, 'frps_0_59:alpine', fpath, 'frps_0_59.tar.xz', ziptype='xz')
    ...
    if imagename == "onnxruntime" or imagename == 'cilame/onnx:latest': 
        ...
        if not fpath: return
        update_a_docker(workspace, 'cilame/onnx:latest', fpath, 'onnxruntime.tar.xz', ziptype='xz')
    if imagename == "chrome" or imagename == 'chrome_slim:latest': 
        fpath = [get_fpath('vvv_docker_chrome_1'), get_fpath('vvv_docker_chrome_2')]
        if not fpath: return
        update_a_docker(workspace, 'chrome_slim:latest', fpath, 'chrome_slim.tar.xz', ziptype='xz')
...
def execute():
    filepath = os.path.join(os.path.expanduser("~"), 'vvv_dockerrc.json')
    argv = sys.argv
    tools = ['gogs','openresty','openresty_ja3','nodejs','python','frp','chrome','onnxruntime']
    ...
    if len(argv) == 1:
        ...
        print('[runcmd]:    v_docker cmd [...(server sh cmd)]')
        for t in tools:
            print('[tool]:', t)
        return
    if len(argv) > 1:
        if argv[1] == 'cmd':
            if len(argv) > 2:
                ...
                print('pls type in your commond...')
                return
        if argv[1] == 'chrome':
            if len(argv) > 2:
                ...
                if argv[2] == "start":
                    ...
                    ports = ["18999:18999"]
                    for k in argv[3:]:
                        if re.findall(r'^[a-zA-Z0-9][a-zA-Z0-9_]*$', k):
                            name = k
                        if re.findall(r'^[0-9]+:[0-9]+$', k):
                            ports = [k]
                    vvv.wrun_chrome(name, ports)
                    return
                if argv[2] == "remove":
                    name = 'default_chrome'
                    for k in argv[3:]:
                        if re.findall('^[a-zA-Z0-9][a-zA-Z0-9_]*$', k):
                            name = k
