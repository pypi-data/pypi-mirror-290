...
class ElnPage(windowobj.BasePageObject):
    ...
    def get_eln_table_entry_with_title(self, title: str, timeout_seconds: int = 10) -> Optional[ElnTableEntry]:
...
class NotebookExplorer(BasePage):
    ...
    def _maybe_select_first_location(self) -> None:
        ...
        dialog.body.find_element(By.XPATH, "./div[2]//td/div").click()
...
class ElnExperimentPage(BasePage):
    EXPERIMENT_XPATH: str = \
        "/html/body/div[2]/table/tbody/tr/td/div/div/div[2]/div[1]/div/div/div/div[3]/div[2]/div[1]/div/div/div"
    ...
    def tab_panel(self) -> ElnTabPanel:
        return ElnTabPanel(self, self.wait_for(
            lambda d: self.page_source.find_element(By.CSS_SELECTOR, ElnTabPanel.CSS_SELECTOR)))
    ...
    def get_eln_table_entry_with_title(self, title: str, timeout_seconds: int = 10) -> ElnTableEntry:
...
class FlowCytometryEntry(ElnEntry):
    ...
    def wait_for_qc(self) -> None:
        self.sapio_driver.wait_until_clickable(lambda d: self.east_tab_panel.main_element, timeout_seconds=90,
                                               stale_wait=True)
