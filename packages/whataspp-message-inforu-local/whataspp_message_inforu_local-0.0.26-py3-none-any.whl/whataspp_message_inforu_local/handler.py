...
#TODO Add calls to the logger

#TODO insertMessageRecievedToTheDatabase
def create_handler(request_parameters_dict: dict) -> list[int]:
...
"""
#TODO Move the inforu parts of this method to __extract_message_inforu_data(request_parameters_dict: dict) -> list[dict]:
def __extract_message_data(request_parameters_dict: dict) ->list[dict, dict]:
    ...
    for request_parameters_data_item in request_parameters_data:
        ...
        #TODO message_data["from_user_external_id"] = from_user_externl_id
...
def get_user_external_id_by_api_type_and_request_parameter(api_type_id: int, request_parameters_dict: dict) -> int:
    ...
    elif api_type_id == INFORU_WHATAPP_PUSH_API_TYPE_ID:
        user_external_generic_crud = GenericCRUD(
            default_schema_name="user_external", default_view_table_name="user_external_view")
        user_external_table = user_external_generic_crud.select_one_dict_by_column_and_value(
            select_clause_value="user_external_id, system_id, username, additional_fields_json",
            column_name="system_id", column_value=INFORU_WHATSAPP_SYSTEM_ID)
        user_external_id = user_external_table.get("user_external_id")
        if user_external_id is None:
            logger.exception(object={"user_external_id": user_external_id})
