...
class InMemoryExecutor(OpGraphExecutor):
    ...
    def _execute_aggregate_columns(
        ...
        return _SchemaAndBatches.from_dataframe(aggregate, metrics={})
    ...
    def _execute_correlate_columns(
        ...
        source_batches = self._execute(op.source, context)
        source_df = _as_df(source_batches)
        to_correlate = source_df[op.column_names]
        return _SchemaAndBatches.from_dataframe(
            to_correlate.corr(dtype="float32"), metrics={}
        )
    ...
    def _execute_histogram_column(
        ...
        source_batches = self._execute(op.source, context)
        source_df = _as_df(source_batches)
        to_histogram = source_df[op.column_name]
        histogram = to_histogram.hist(include_category=False).rename(
            {
                "breakpoint": op.breakpoint_column_name,
                "count": op.count_column_name,
            }
        )
        return _SchemaAndBatches.from_dataframe(histogram, metrics={})
