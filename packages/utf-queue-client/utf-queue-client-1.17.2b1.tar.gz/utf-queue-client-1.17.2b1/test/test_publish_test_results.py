...
def test_jobstatus_session_data(request, session_start_data, session_stop_data):
    ...
    pk_id = generate_uuid()
    args = ["--username", username, "--password", password, "--data_type", "SESSION_START"]
    args += ["--data", "PK_ID", pk_id]
    for k, v in session_start_data:
        args += ["--data", k, v]

    stop_args = ["--username", username, "--password", password, "--data_type", "SESSION_STOP"]
    ...
    def call_cli_script():
        ...
        process = subprocess.Popen(
            [
                sys.executable,
                os.path.join(
                    base_dir, "utf_queue_client", "scripts", "publish_test_results_cli.py"
                ),
            ]
            + args,
        )
...
def test_app_build_results_data(request, app_build_results_data):
    ...
    uuid_hex = uuid_date_time + uuid_hex[len(uuid_date_time):]
    suid = UUID(uuid_hex)
    args = ["--username", username, "--password", password, "--data_type", "BUILD_RESULT"]
