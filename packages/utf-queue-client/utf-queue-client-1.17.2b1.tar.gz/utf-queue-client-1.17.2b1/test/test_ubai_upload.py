...
from utf_queue_client.scripts.ubai_upload_cli import cli, MULTIPART_SIZE_THRESHOLD
from otel_extensions import inject_context_to_env, instrumented
import os
import subprocess
import sys
import tempfile
from ubai_client.apis import SearchApi, ArtifactApi
...
@pytest.mark.parametrize("use_queue", [True, False])
def test_ubai_upload_cli_large_file(request, metadata, set_environment_variable, use_queue):
...
@pytest.mark.parametrize("set_environment_variables", [[], [("OTEL_EXPORTER_OTLP_ENDPOINT", "https://otel-collector-http-wrong.silabs.net")]], indirect=True)
@instrumented
def test_ubai_upload_cli_script(request, metadata, use_queue, set_environment_variables):
    file = os.path.join(os.path.dirname(__file__), "test.hex")
