...
Ty_metadata = fire.Type("!fireducks.metadata")
...
Op_make_scalar_none = fire.Opcode("fireducks.make_scalar.none")
...
Op_make_scalar_time_point_ns = fire.Opcode(
    "fireducks.make_scalar.time_stamp_ns"
)

Op_make_invalid_scalar_f32 = fire.Opcode("fireducks.make_invalid_scalar.f32")
Op_make_invalid_scalar_f64 = fire.Opcode("fireducks.make_invalid_scalar.f64")
Op_make_invalid_scalar_time_point_ns = fire.Opcode(
    "fireducks.make_invalid_scalar.time_stamp_ns"
)
Op_make_invalid_scalar_time_delta_ns = fire.Opcode(
    "fireducks.make_invalid_scalar.time_delta_ns"
)
...
Op_abs = fire.Opcode("fireducks.abs")
...
def aggregate(table, func, axis):
    return _build_op(
        Op_aggregate,
        [TableType],
        [table, func, create_i32_attr("axis", axis)],
        chaining=True,
    ).outs[0]
...
def aggregate_column_scalar(table, func: str):
    # TODO: support chaining only on input
    return _build_op(
        Op_aggregate_column_scalar, [Ty_scalar], [table, func]
    ).outs[0]
...
def strftime(table, str_format):
    return _build_op(
        Op_strftime, [TableType], [table, str_format], chaining=True
    ).outs[0]
...
def drop_columns(table, labels):
    return _build_op(
        Op_drop_columns, [TableType], [table, labels], chaining=True
    ).outs[0]
...
def drop_duplicates(table, subset, keep, keep_org_index_when_no_dup):
    return _build_op(
        Op_drop_duplicates,
        [TableType],
        [
...
def dropna(table, subset, axis, is_any, thresh):
    return _build_op(
        Op_dropna,
        [TableType],
        [
...
def get_metadata(table):
    return _build_op(
        Op_get_metadata, [Ty_metadata], [table], chaining=True
    ).outs[0]
...
def groupby_transform(table, groupkey, aggmethod):
    return _build_op(
        Op_groupby_transform,
        [TableType],
        [table, groupkey, aggmethod],
        chaining=True,
    ).outs[0]


def groupby_select_transform(table, groupkey, columns, aggmethod):
    return _build_op(
        Op_groupby_select_transform,
        [TableType],
        [table, groupkey, columns, aggmethod],
        chaining=True,
    ).outs[0]
...
def str_slice(table, start, stop, end):
    return _build_op(
        Op_str_slice, [TableType], [table, start, stop, end], chaining=True
    ).outs[0]


def str_split(table, pat, n, regex, reverse):
    return _build_op(
        Op_str_split,
        [TableType],
        [
...
def take(table, indices, axis, check_negative, ignore_index):
    op = Op_take_rows if axis == 0 else Op_take_cols
    return _build_op(
        op,
        [TableType],
        [
...
def to_csv(table, sep, na_rep, header, index, quoting_style):
    return _build_op(
        Op_to_csv,
        [StringType],
        [
...
def write_csv(table, filename, sep, na_rep, header, index, quoting_style):
    return _build_op(
        Op_write_csv,
        [],
        [
...
def abs(table):
    return _build_op(Op_abs, [TableType], [table], chaining=True).outs[0]
...
def make_scalar_none():
    return _build_op(Op_make_scalar_none, [Ty_scalar], []).outs[0]
...
def make_scalar_time_point_ns(src: fire.Value):
    return _build_op(Op_make_scalar_time_point_ns, [Ty_scalar], [src]).outs[0]


def make_invalid_scalar_f32(src: float):
    src = fireducks.core.make_available_value(src, F32Type)
    return _build_op(Op_make_invalid_scalar_f32, [Ty_scalar], [src]).outs[0]


def make_invalid_scalar_f64(src: float):
    return _build_op(Op_make_invalid_scalar_f64, [Ty_scalar], [src]).outs[0]


def make_invalid_scalar_time_point_ns(src: fire.Value):
    return _build_op(
        Op_make_invalid_scalar_time_point_ns, [Ty_scalar], [src]
    ).outs[0]


def make_invalid_scalar_time_delta_ns(src: fire.Value):
    return _build_op(
        Op_make_invalid_scalar_time_delta_ns, [Ty_scalar], [src]
    ).outs[0]
...
def to_datetime(table, str_format):
    return _build_op(
        Op_to_datetime, [TableType], [table, str_format], chaining=True
    ).outs[0]
