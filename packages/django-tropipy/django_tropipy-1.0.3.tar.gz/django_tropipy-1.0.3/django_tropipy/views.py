...
from .common import config_exists, get_config, ORDER_KEYS, get_payment_payload, perform_payment
...
def create_order(purchase_data, **kwargs):
    for key in ORDER_KEYS:
        if purchase_data.get(key) is None:
            return None
    TropipayOrder.objects.create(uuid=purchase_data.get("uuid"), title=purchase_data.get("title"),
                                 name=purchase_data.get("name"), last_name=purchase_data.get("last_name"),
                                 email=purchase_data.get("email"), phone_number=purchase_data.get("phone_number"),
                                 description=purchase_data.get("description"), location=purchase_data.get("location"),
