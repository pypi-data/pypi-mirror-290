...
class ApiClient:
    ...
    def fetch_token(self, scopes=["client_paymentrequest"]) -> None:
        """Fetch an OAuth token using client credentials.

        Parameters
        ----------
        scopes : list of str, optional
            The list of OAuth scopes to request. Defaults to ["client_paymentrequest"].
    ...
    def initiate_wallet_transaction(self, input: InitiateTransactionInput) -> Transaction:
        ...
        try:
            ...
            if not response or response["__typename"] not in ["ApplePayTransaction", "SamsungPayTransaction", "GooglePayTransaction"]:
                raise Exception(f"Failed to initiate transaction\n{response}")
            return Transaction(
                id=response["id"],
                amount=Money(
                    quantity=response["amount"]["quantity"], currency=response["amount"]["currency"]),
                external_reference=response["externalReference"] if "externalReference" in response else None,
                nonce=response["nonce"],
                status=response["state"]["__typename"],
                status_reason=response["state"]["reason"] if "reason" in response["state"] else None
            )
        except Exception as error:
            raise Exception(str(error))

    def validate_apple_pay_merchant(self, input: ValidateApplePayMerchantInput) -> Session:
        ...
        try:
            ...
            if not response or response["__typename"] not in ["ValidateApplePayMerchantPayload"]:
    ...
    def verify_samsung_pay(self, input: VerifySamsungPayInput) -> Session:
        ...
        try:
            ...
            if not response or response["__typename"] not in ["VerifySamsungPayPayload"]:
                raise Exception(f"Failed to validate merchant.\n{response}")
