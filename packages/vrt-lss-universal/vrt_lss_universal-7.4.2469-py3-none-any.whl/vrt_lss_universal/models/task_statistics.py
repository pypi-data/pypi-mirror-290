...
class TaskStatistics(BaseModel):
    ...
    """ # noqa: E501
    planning_horizon: Optional[TimeWindow]
    __properties: ClassVar[List[str]] = ["planning_horizon"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of planning_horizon
        if self.planning_horizon:
            _dict['planning_horizon'] = self.planning_horizon.to_dict()
        # set to None if planning_horizon (nullable) is None
        # and model_fields_set contains the field
        if self.planning_horizon is None and "planning_horizon" in self.model_fields_set:
            _dict['planning_horizon'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "planning_horizon": TimeWindow.from_dict(obj["planning_horizon"]) if obj.get("planning_horizon") is not None else None
        })
        return _obj
