...
class TripAssumptions(BaseModel):
    ...
    """ # noqa: E501
    transport_speed_multipliers: Optional[Annotated[List[TransportSpeedMultiplier], Field(min_length=0, max_length=12)]] = Field(default=None, description="Transport speed change coefficients.")
    transport_capacity_multipliers: Optional[Annotated[List[TransportCapacityMultiplier], Field(min_length=0, max_length=12)]] = Field(default=None, description="Transport capacity change coefficients. Change the `capacity` parameters for all boxes and `max_capacity` for the transport. ")
