...
class PerformerLimits(BaseModel):
    ...
    """ # noqa: E501
    max_work_shifts: Optional[Annotated[int, Field(le=31, strict=True, ge=1)]] = Field(default=31, description="Limit of the number of performer's shifts in one planning.")
    demand_extra_durations: Optional[Annotated[List[DemandExtraDuration], Field(min_length=0, max_length=15001)]] = Field(default=None, description="A list of demands for which the performer will spend more time than indicated in the demand. Valid for the specified performer-order pair for all performer's shifts and all demands's events. ")
    __properties: ClassVar[List[str]] = ["max_work_shifts", "demand_extra_durations"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in demand_extra_durations (list)
        _items = []
        if self.demand_extra_durations:
            for _item in self.demand_extra_durations:
                if _item:
                    _items.append(_item.to_dict())
            _dict['demand_extra_durations'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "demand_extra_durations": [DemandExtraDuration.from_dict(_item) for _item in obj["demand_extra_durations"]] if obj.get("demand_extra_durations") is not None else None
        })
        return _obj
