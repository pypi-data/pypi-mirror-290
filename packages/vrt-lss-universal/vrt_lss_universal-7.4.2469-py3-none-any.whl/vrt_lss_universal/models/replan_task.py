...
class ReplanTask(BaseModel):
    ...
    transports: Annotated[List[Transport], Field(min_length=1, max_length=15001)] = Field(description="Available transports list.")
    ...
    external_routing: Optional[Annotated[List[RoutingTransportMatrix], Field(min_length=0, max_length=12)]] = Field(default=None, description="List of matrices of times and distances for each type of transport that are indicated in the data. The matrix should describe all locations for each type of transport from the data. When specifying an external routing matrix `external_routing`, the `geo_settings` parameters are not taken into account. ")
    ...
    __properties: ClassVar[List[str]] = ["locations", "orders", "performers", "transports", "hardlinks", "trips", "facts", "external_routing", "plan_settings", "replan_settings", "dataset_name"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ReplanTask from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ReplanTask from a dict"""
        if obj is None:
            return None
