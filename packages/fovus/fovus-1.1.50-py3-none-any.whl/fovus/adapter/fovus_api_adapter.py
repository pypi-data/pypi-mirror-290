...
class FovusApiAdapter:
    ...
    def _confirm_enable_preemptible_support(self, request):
        ...
        max_walltime_allowed_for_resumable_workload = 6
        ...
        if (
            ...
            or request[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS][ALLOW_PREEMPTIBLE] is True
        ):
            print(
                "Enabling preemptible resources will restrict the maximum task Walltime allowed to "
                ...
                + "individual task or job level. Are you sure to allow preemptible resources? (y/n): "
            )
            if input() == "y":
                request[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS][ALLOW_PREEMPTIBLE] = True
                ...
                if request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS] > max_walltime:
                    print(
                        f"Warning: When allowing preemptible resources, Walltime must be <= {max_walltime} hours "
                        + f"if resumable workload is {is_resumable_workload}. Overriding Walltime from "
                        + f"{request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS]} "
                        + f"to {max_walltime} hours."
                    )
                    request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS] = max_walltime
            else:
                request[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS][ALLOW_PREEMPTIBLE] = False
                request[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS][IS_RESUMABLE_WORKLOAD] = False
    ...
    def start_sync_file_request(
        workspace_id: str, job_id: str, paths: list[str], include_list: list[str], exclude_list: list[str]
    ):
        return {
