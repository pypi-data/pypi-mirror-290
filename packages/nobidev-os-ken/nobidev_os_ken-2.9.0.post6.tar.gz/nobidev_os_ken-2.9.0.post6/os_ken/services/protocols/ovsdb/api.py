...
def _get_bridge(attr_val, *, attr_name='name'):
    def _reader_(tables):
...
def get_bridge_by_uuid(bridge_uuid):
    return _get_bridge(bridge_uuid, attr_name='uuid')


def get_bridges(tables):
    return get_table('Bridge')(tables).rows.values()
...
def set_iface_external_id(iface_name, key, val):
    return set_external_id(key, val, lambda tables: _get_iface(tables, iface_name))
...
def set_iface_other_config(iface_name, key, val):
    return set_other_config(key, val, lambda tables: _get_iface(tables, iface_name))
...
def del_iface_external_id(iface_name, key):
    return del_external_id(key, lambda tables: _get_iface(tables, iface_name))
...
def del_iface_other_config(iface_name, key):
    return del_other_config(key, lambda tables: _get_iface(tables, iface_name))


def del_port(bridge_name, fn):
    def _modifier_(tables, *_):
        bridge = _get_bridge(bridge_name)(tables)
...
def del_port_by_uuid(bridge_name, port_uuid):
    return del_port(bridge_name, lambda tables: _get_port(tables, port_uuid, attr_name='uuid'))


def del_port_by_name(bridge_name, port_name):
    return del_port(bridge_name, lambda tables: _get_port(tables, port_name))
...
def create_port(bridge_name, port_info, iface_info=None):
    ...
    def _modifier_(tables, insert):
        ...
        iface = insert(tables['Interface'], iface_info['uuid'])
        for key, val in iface_info.items():
            if key in ['uuid']:
                continue
            setattr(iface, key, val)

        port = insert(tables['Port'], port_info['uuid'])
...
def del_bridge(fn):
    def _modifier_(tables, *_):
        ...
        if not delete_bridges:
            return
        elif isinstance(delete_bridges, list):
            delete_bridges = [delete_bridges]

        switch.bridges = list(filter(lambda item: item not in delete_bridges, switch.bridges))

    return _modifier_


def del_bridge_by_uuid(bridge_uuid):
    return del_bridge(_get_bridge(bridge_uuid, attr_name='uuid'))


def del_bridge_by_name(bridge_name):
    return del_bridge(_get_bridge(bridge_name))
...
def create_bridge(bridge_info, **kwargs):
    ...
    def _modifier_(tables, insert):
        ...
        bridge = insert(tables['Bridge'], bridge_info['uuid'])
...
def get_managers(tables):
    return tables['Manager'].rows.values()
