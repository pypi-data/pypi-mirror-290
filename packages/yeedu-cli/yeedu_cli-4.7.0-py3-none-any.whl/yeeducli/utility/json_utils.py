...
def remove_output(args, subcommand=None):
    try:
        ...
        for k, v in dict(json_payload).items():
            ...
            if str(k) in ["conf"] and subcommand in ['create-conf', 'edit-conf']:
    ...
    try:
        for k, v in dict(json_payload).items():
            ...
            engine_cluster_spark_config_keys = [
                'max_parallel_spark_job_execution_per_instance', 'standalone_workers_number']
...
def check_cloud_providers(clouds):
    try:
        check_non_empty_string(clouds)

        list_of_cloud = [cloud.strip() for cloud in clouds.split(',')]

        if not all(cloudProvider in CLOUD_PROVIDERS_LIST for cloudProvider in list_of_cloud):
            raise ArgumentTypeError(
                f"invalid choice: '{clouds}' (choose from {CLOUD_PROVIDERS_LIST})")
        return list_of_cloud
    except ValueError:
        raise ArgumentTypeError(
            f"invalid choice: '{clouds}' (choose from {CLOUD_PROVIDERS_LIST})")
