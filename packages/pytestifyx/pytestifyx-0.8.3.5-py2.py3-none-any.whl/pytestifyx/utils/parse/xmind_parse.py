class TestModule:
    def __init__(self, module_name, sub_module=None):
        ...
        for topic in topics:
            ...
            if "预期结果" in topic['title']:
                if len(new_path) >= 4:  # 确保路径长度足够以避免 ValueError
                    ...
                    test_case_name, test_step = new_path[-3]['title'], new_path[-2]['title']
                    module_name =  new_path[-(path_length)]['title']
                    sub_module = new_path[-(path_length-1)]['title']
                    ...
                    if module_name not in self.modules:
                        self.modules[module_name] = TestModule(module_name, sub_module)
