...
class EmailUtil:
    ...
    def send_email(self, subject, body, to=None, cc=None, bcc=None, attachments=None):
        ...
        msg['To'] = ', '.join(to) if to else ', '.join(self.email_settings['to'])
        log.info(msg['To'])
        ...
        if attachments:
            for file in attachments:
                log.info(f"Attaching file: {file}")
        ...
        recipients = to if to else self.email_settings['to']
        if cc:
        ...
        try:
            ...
            log.info("Logged in to SMTP server")
