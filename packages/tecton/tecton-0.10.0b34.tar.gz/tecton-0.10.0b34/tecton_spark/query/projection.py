    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        for column_name, unit in self.anchor_time_column_map.items():
            ...
            if interval_seconds == 0:
                ...
                spark_column = (
                    anchor_time_val - anchor_time_val % interval_seconds - self.aggregation_tile_interval
                ).alias(column_name)
...
class AdjustAnchorTimeToWindowEndSparkNode(SparkExecNode):
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        if self.aggregation_tile_interval == 0:
            return output_df
        for column in self.anchor_time_columns:
            output_df = output_df.withColumn(column, F.col(column) + self.aggregation_tile_interval)
        return output_df
...
@attrs.frozen
class ExplodeEventsAndSelectDistinctSparkNode(SparkExecNode):
    """
    ...
    def _select_distinct_for_explode_column(
        ...
        other_columns = [col for col in self.explode_columns if col != explode_column]
