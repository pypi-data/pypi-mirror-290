...
class KinesisOutputStream(OutputStream):
    ...
    @classmethod
    def from_proto(cls, proto: feature_view_pb2.OutputStream):
        if raw_options := proto.kinesis.options:
...
class KafkaOutputStream(OutputStream):
    ...
    @classmethod
    def from_proto(cls, proto: feature_view_pb2.OutputStream):
        if raw_options := proto.kafka.options:
            options = {option.key: option.value for option in raw_options}
        else:
            options = None
...
class FileConfig(BaseBatchConfig):
    ...
    This class is used as an input to a `BatchSource`'s parameter `batch_config`. Declaring this configuration class alone
...
class KafkaConfig(BaseStreamConfig):
    ...
    def __init__(
        ...
        - [User Guide](https://docs.tecton.ai/docs/defining-features/data-sources/creating-and-testing-a-streaming-data-source#write-the-stream-message-post-processor-function)
        - [Productionizing a Stream](https://docs.tecton.ai/docs/defining-features/feature-views/stream-feature-view#productionizing-a-stream)
...
class KinesisConfig(BaseStreamConfig):
    ...
    def __init__(
        ...
        - [User Guide](https://docs.tecton.ai/docs/defining-features/data-sources/creating-and-testing-a-streaming-data-source#write-the-stream-message-post-processor-function)
        - [Productionizing a Stream](https://docs.tecton.ai/docs/defining-features/feature-views/stream-feature-view#productionizing-a-stream)
        - [Spark Kinesis Documentation](https://spark.apache.org/docs/latest/streaming-kinesis-integration.html)
...
class PushConfig(BaseStreamConfig):
    ...
    def __init__(
        ...
        # Declare PushConfig instance object that can be used as argument in `StreamSource`
        ...
        _post_processor_function = function_serialization.to_proto(post_processor) if post_processor else None
...
class BigQueryConfig(BaseBatchConfig):
    ...
    def __init__(
        ...
        *,
        ...
        Example of a BigQueryConfig declaration:

        ```python
        from datetime import datetime, timedelta
        from tecton import BigQueryConfig, BatchSource
        ...
        bq_config = BigQueryConfig(
            ...
            # query="SELECT trip_id, duration_sec, start_station_name, end_station_name, start_time FROM bigquery-public-data.san_francisco_bikeshare.bikeshare_trips",
