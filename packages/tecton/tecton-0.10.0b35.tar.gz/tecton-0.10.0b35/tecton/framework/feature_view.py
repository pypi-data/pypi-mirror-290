...
from tecton.framework import data_source_filter
...
class FeatureView(base_tecton_object.BaseTectonObject):
    ...
    def _validate_feature_schema(self, name, features, aggregations, schema):
        ...
        if schema is not None:
            logger.warning(
                ...
                "https://docs.tecton.ai/docs/sdk-reference/features/migrate-to-features for information "
    ...
    def get_job(self, job_id: str) -> materialization_api.TectonJob:
        ...
        :return: :class:`JobData` object for the job.
        """
...
class MaterializedFeatureView(FeatureView):
    ...
    def run(
        ...
        **mock_inputs_kwargs: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        if (
            any(ds._data_source_type == DataSourceType.PUSH_NO_BATCH for ds in self.sources)
    ...
    def cache_config(self) -> Optional[configs.CacheConfig]:
        ...
        """
        if cache_config_proto := self._spec.cache_config:
...
def resolve_local_remote_model_artifact(
    ...
    for inference in inferences:
        try:
            model_artifact = _fetch_model(inference.model)
            model_artifact_map[model_artifact.name] = model_artifact
            if model_artifact.name in all_local_models:
                print(
                    f"There is both a remote and local model with name {model_artifact.name}, The remote model will be used."
                )
        except Exception:
            if inference.model not in all_local_models:
                msg = f"Model with name {inference.model} is not found remotely or locally."
                raise TectonValidationError(msg)
            else:
                model_artifact = all_local_models[inference.model]
                local_model_artifact = model_artifact_service_pb2.ModelArtifactInfo(
                    name=model_artifact.name,
                    model_file_path=model_artifact.model_file,
                    type=model_type_string_to_enum(model_artifact.model_type),
                    description=model_artifact.description,
                    tags=model_artifact.tags,
                    input_schema=type_utils.to_tecton_schema(model_artifact.input_schema),
                    output_schema=type_utils.to_tecton_schema([model_artifact.output_schema]),
                    environments=model_artifact.environments,
                )
                model_artifact_map[model_artifact.name] = local_model_artifact
                ...
                model_cache_dir, _ = _initialize_model_cache_dir(
                    inference.model, model_artifact.model_file, force_overwrite=True
                )

                artifact_files = model_artifact.artifact_files or []
                all_files = [model_artifact.model_file, *artifact_files]
...
class BatchFeatureView(MaterializedFeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        feature_view_function: Callable,
        sources: Sequence[Union[framework_data_source.BatchSource, data_source_filter.DataSourceFilter]],
        ...
        data_sources = tuple(
            source.source if isinstance(source, data_source_filter.DataSourceFilter) else source for source in sources
        )
...
def batch_feature_view(
    *,
    ...
    environment: Optional[str] = None,
    features: Optional[
        Union[Sequence[feature.Aggregate], Sequence[Union[feature.Attribute, feature.Embedding, feature.Inference]]]
    ] = None,
    context_parameter_name: Optional[str] = None,
):
    """Declare a Batch Feature View.

    :param mode: Either the compute mode for the Transformation function or else `pipeline` mode
    ...
    :param aggregations: A list of `Aggregate` Feature objects.
    ...
    :param schema: [Deprecated] The output schema of the Feature View transformation. If provided and `run_transformation_validations=True`,
    ...
    :param features: A list of Attribute, Aggregate, and Embedding feature values managed by this Feature View.
...
class StreamFeatureView(MaterializedFeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        source: Union[
            framework_data_source.StreamSource, framework_data_source.PushSource, data_source_filter.DataSourceFilter
        ],
        entities: Sequence[framework_entity.Entity],
        ...
        data_source = source.source if isinstance(source, data_source_filter.DataSourceFilter) else source
        ...
        if feature_view_function:
            ...
            pipeline_root = self._build_pipeline(
                name, feature_view_function, pipeline_function, [source], context_parameter_name
            )
        else:
        ...
        data_sources = (source.source if isinstance(source, data_source_filter.DataSourceFilter) else source,)
...
def stream_feature_view(
    *,
    ...
    environment: Optional[str] = None,
    features: Optional[Union[Sequence[feature.Aggregate], Sequence[feature.Attribute]]] = None,
    context_parameter_name: Optional[str] = None,
):
    ...
    :param aggregations: A list of `Aggregation` structs
    :param aggregation_secondary_key: Configures secondary key aggregates using the set column. Only valid when using aggregations.
    :param online: Whether the feature view should be materialized to the online feature store.
    :param offline: Whether the feature view should be materialized to the offline feature store.
    :param ttl: The TTL (or "look back window") for features defined by this feature view. This parameter determines how long features will live in the online store and how far to  "look back" relative to a training example's timestamp when generating offline training sets. Shorter TTLs improve performance and reduce costs.
    ...
    :param environment: The custom environment in which materialization jobs will be run. Defaults to `None`, which means
        jobs will execute in the default Tecton environment.
    :param features: A list of features this feature view manages.
    :param context_parameter_name: Name of the function parameter that Tecton injects MaterializationContext object to.
    ...
    def __init__(
        ...
        *,
        ...
        :param prevent_destroy: If True, this Tecton object will be blocked from being deleted or re-created (i.e. a
            ...
            such as inadvertently deleting a Feature Service used in production or recreating a Feature Table that
            triggers expensive rematerialization jobs. `prevent_destroy` also blocks changes to dependent Tecton objects
            that would trigger a recreation of the tagged object, e.g. if `prevent_destroy` is set on a Feature Service,
            that will also prevent deletions or re-creates of Feature Tables used in that service. `prevent_destroy` is
            only enforced in live (i.e. non-dev) workspaces.
        :param entities: A list of Entity objects, used to organize features.
        :param schema: A schema for the Feature Table. Supported types are: Int64, Float64, String, Bool and Array with Int64, Float32, Float64 and String typed elements. Additionally you must have exactly one Timestamp typed column for the feature timestamp.
        ...
        :param batch_compute: Configuration for batch materialization clusters. Should be one of:
            [`EMRClusterConfig`, `DatabricksClusterConfig`, `EMRJsonClusterConfig`, `DatabricksJsonClusterConfig`, `DataprocJsonClusterConfig`]
        :param online_serving_index: (Advanced) Defines the set of join keys that will be indexed and queryable during online serving.
        ...
        :param timestamp_field: The column name that refers to the timestamp for records that are produced by the
            Feature Table. This parameter is optional.
        """
...
class RealtimeFeatureView(_RealtimeOrPromptBase):
    ...
    def test_run(
        ...
        return run_api.run_mock_rtfv_pipeline(self._args.pipeline, transformation_specs, self.name, mock_inputs)
...
def show_odfv_dep_warning(func):
    @functools.wraps(func)
    def decorator(*args, **kwargs):
        logger.warning(
            "on_demand_feature_view has been renamed realtime_feature_view and will be removed in a future "
            "version. Please use realtime_feature_view.",
        )
...
# TODO (ajeya): Remove after `on_demand_feature_view` is fully removed
on_demand_feature_view = show_odfv_dep_warning(realtime_feature_view)


# The user's function just returns a string. Downstream components expect a dict for column names etc,
# so we wrap this in a dictionary {'prompt': <function_output>)
def prompt_wrapper(prompt_function):
    @wraps(prompt_function)
    def inner(*args, **kwargs):
        return {"prompt": prompt_function(*args, **kwargs)}
...
class Prompt(_RealtimeOrPromptBase):
    ...
    def __init__(
        ...
        *,
        ...
        pipeline_function = self._get_or_create_pipeline_function(
            ...
            owner=owner,
            tags=tags,
            feature_view_function=prompt_wrapper(feature_view_function),
        )
    ...
    @sdk_decorators.sdk_public_method
    def run_prompt(self, input_data: Dict[str, Any]) -> str:
...
def _source_to_pipeline_node(
    ...
    # TODO: elif isinstance(source, data_source_filter.UnfilteredSource):
    elif isinstance(source, data_source_filter.FilteredSource):
        node = pipeline_pb2.DataSourceNode(
...
def _fetch_model(model_name: str) -> model_artifact_service_pb2.ModelArtifactInfo:
    try:
        artifacts = (
        ...
        if len(artifacts) != 1:
            msg = f"No model artifact found with name: {model_name}"
