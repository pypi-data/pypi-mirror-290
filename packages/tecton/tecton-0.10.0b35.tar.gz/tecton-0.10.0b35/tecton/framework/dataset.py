...
from tecton_proto.metadataservice.metadata_service__client_pb2 import CreateSavedFeatureDataFrameRequest
...
class Dataset(ABC):
    """
    Abstract base class for Dataset, representing a dataset with various metadata
    and the ability to convert to a dataframe.
    """
...
class LegacyDataset(Dataset):
    ...
    def __init__(self, proto: SavedFeatureDataFrame, spark_df: pyspark.sql.DataFrame, pandas_df: pandas.DataFrame):
        self._tecton_df = TectonDataFrame(spark_df, pandas_df, snowflake_df=None)
        self._proto = proto
