...
def get_caller_identity() -> Union[ServiceAccountProfile, okta.UserProfile]:
    ...
    Throws:
      SystemError if the credentials are expired or not valid.
    """
...
def logout() -> None:
    """
    Log out of the current session that was previously authenticated using tecton.login().
...
def login(
    ...
    Example usages:
        - tecton.login(tecton_url="https://<example>.tecton.ai") to log in interactively as a user
        - tecton.login(tecton_url="https://<example>.tecton.ai", interactive=False) to log in non-interactively as a user
        - tecton.login(tecton_url="https://<example>.tecton.ai", tecton_api_key="<api_key>") to authenticate with a Tecton API key

    :param tecton_url: HTTP URL of the Tecton instance, e.g. https://<example>.tecton.ai
    :param interactive: Can only be set when authenticating as a user. When set to False, does not take the auth code
                        from standard input and requires completing login using `complete_login(<auth_code>)`.
                        By default it is True.
    :param tecton_api_key: Optional Tecton API key
    """
    # Validate inputs
    if tecton_api_key:
        if not interactive:
...
def complete_login(authorization_code: str) -> None:
    """
    Complete non-interactive authentication to a Tecton instance as a user.
