...
from tecton_core.query.nodes import ExplodeEventsAndSelectDistinctNode
...
def build_aggregated_time_range_validity_query(
    ...
    use_sawtooth_aggregation = fdw.compaction_enabled and fdw.get_min_batch_sawtooth_tile_size() is not None
    # For stream sawtooth aggregates, every event can result in a unique aggregation value like continuous mode.
    tile_interval = (
        pendulum.Duration(seconds=0) if use_sawtooth_aggregation else fdw.get_tile_interval_duration_for_offline_store
    )
    partial_aggs = build_get_features(
        ...
        from_source,
        feature_data_time_limits=feature_data_time_limits,
        aggregation_tile_interval_override=tile_interval,
    )
    if entities is not None:
...
def build_aggregated_time_range_ghf_query(
    ...
    qt = _filter_and_update_timestamp_columns(
        ...
        compute_mode,
        fdw,
        fdw.get_tile_interval_duration_for_offline_store,
...
def build_spine_join_querytree(
    ...
    if fdw.is_temporal or fdw.is_feature_table:
        ...
        if fdw.get_min_batch_sawtooth_tile_size():
            sawtooth_agg_data = compaction_utils.SawtoothAggregationData.from_aggregate_features(fdw)
...
def _add_sawtooth_anchor_time_and_partition_columns(
    ...
    timestamp_field: str,
    should_truncate_timestamps: bool,
    ...
    output_node = AddAnchorTimeColumnsForSawtoothIntervalsNode(
        ...
        data_delay_seconds=fdw.online_store_data_delay_seconds,
        feature_store_format_version=fdw.get_feature_store_format_version,
        aggregation_tile_interval=fdw.get_tile_interval_for_offline_store,
    ).as_ref()
...
def _build_sawtooth_aggregation_subtree(
    ...
    batch_partial_agg_subtree = _add_sawtooth_anchor_time_and_partition_columns(
        ...
        fdw=fdw,
        timestamp_field=anchor_time(),
        should_truncate_timestamps=False,
    ...
    if explode_partial_aggs_for_non_sawtooths:
        # If the fv contains < 2d aggregations with a stream size < 1hr, mimic sawtoothing behavior with the stream tile size as the sawtooth size.
    ...
    # Build spine for batch part full aggs.
    # TODO(samantha): Fix the spine creation for secondary key aggs.
    ...
    batch_spine_node = ExplodeEventsAndSelectDistinctNode(
        dialect=dialect,
        compute_mode=compute_mode,
        input_node=batch_spine_node,
    ...
    batch_part_subtree = AsofJoinFullAggNode(
        dialect=dialect,
        compute_mode=compute_mode,
    ...
    # The result of the full agg node has an anchor time that's the start of the day/hour.
