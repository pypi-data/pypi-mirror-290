...
from tecton.framework.data_source_filter import DataSourceFilter
from tecton.framework.data_source_filter import FilteredSource
from tecton.framework.data_source_filter import UnfilteredSource
...
class DataSource(base_tecton_object.BaseTectonObject):
    ...
    def filtered(self, start_time_offset: Optional[datetime.timedelta] = None) -> DataSourceFilter:
        """
        Returns this DataSource object wrapped as a FilteredSource. FilteredSources will automatically pre-filter
        sources in Feature View definitions and can reduce compute costs.

        :param start_time_offset: FilteredSource will pre-filter the data source to the time range [start_time +
        start_time_offset, end_time). Must be zero or negative. Defaults to zero.
        """
        return FilteredSource(self, start_time_offset)

    def unfiltered(self) -> DataSourceFilter:
        """
        Return an unfiltered DataSource. This scope will make an entire source available to a Feature View, but can
        increase compute costs as a result.
        """
        return UnfilteredSource(self)
...
class PushSource(StreamSource):
    """Deprecated. Use StreamSource with a PushConfig instead.
    ...
    def __init__(
        ...
        *,
        ...
        :param options: Additional options to configure the Source. Used for advanced use cases and beta features.
        """
        logger.warning("PushSource is deprecated. Please use StreamSource with a PushConfig instead")
