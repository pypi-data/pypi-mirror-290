...
class AsofJoinFullAggNode(QueryNode):
    ...
    def as_str(self) -> str:
        if self.sawtooth_aggregation_data:
            desc = "Events asof join partial aggregates, where partial aggregates are rolled up to compute full aggregates. The join conditions are: "
            anchor_time_to_partition_column = self.sawtooth_aggregation_data.get_anchor_time_to_partition_columns_map()
            for anchor_time, partition_column in anchor_time_to_partition_column.items():
                desc += f"partial_aggregate.{anchor_time} <= events.{anchor_time} and {partition_column} = True. "
            return desc
    ...
    def _get_time_range_for_aggregation(
        self, feature: feature_view_pb2.AggregateFeature
...
class AggregationSecondaryKeyExplodeNode(QueryNode):
    ...
    def for_feature_definition(
        ...
        return cls(
            ...
            earliest_anchor_time_from_window_start=fdw.earliest_anchor_time_from_window_start(fdw.earliest_window_start)
            if not fdw.has_lifetime_aggregate
            else None,
        ).as_ref()
...
class AddAnchorTimeColumnsForSawtoothIntervalsNode(QueryNode):
    ...
    Attributes:
        ...
        aggregation_tile_interval: The size of the partial agg tiles.
    ...
    feature_store_format_version: int
    aggregation_tile_interval: int

    @property
    def columns(self) -> Sequence[str]:
        columns = [column for column in self.input_node.columns if column != self.timestamp_field]
        return columns + list(self.anchor_time_column_map.keys())

    @property
    def output_schema(self) -> Optional[Schema]:
        schema = self.input_node.output_schema.to_dict()
        schema.pop(self.timestamp_field, None)
        schema.update({col: TimestampType() for col in self.anchor_time_column_map})
        return Schema.from_dict(schema)
    ...
    def as_str(self) -> str:
        ...
        return f"Add new columns {new_columns} which truncate {self.timestamp_field} column to the start time of most recent complete time units: {time_units}."
...
class DeriveValidityPeriodNode(QueryNode):
    ...
    def _is_time_window_series_feature(self, feature: feature_view_pb2.AggregateFeature) -> bool:
        time_window = create_time_window_spec_from_data_proto(feature.time_window)
        return isinstance(time_window, TimeWindowSeriesSpec)

    def _is_count_feature(self, feature: feature_view_pb2.AggregateFeature) -> bool:
        return (
...
@attrs.frozen
class ExplodeEventsAndSelectDistinctNode(QueryNode):
    ...
    @property
    def columns(self) -> Sequence[str]:
        return self.input_node.columns + list(self.explode_columns_to_boolean_columns.values())

    def as_str(self) -> str:
        if len(self.explode_columns) == 1:
            return f"Select distinct with columns {self.input_node.columns}. Appending column {self.explode_columns_to_boolean_columns[self.explode_columns[0]]}."
...
class AsofJoinSawtoothAggNode(QueryNode):
    ...
    @property
    def columns(self) -> Sequence[str]:
        return list(self.batch_input_node.columns)

    @property
    def output_schema(self) -> Optional[Schema]:
        return self.batch_input_node.output_schema
