...
def run_batch_materialization(
    ...
    if should_write_to_online_from_offline_store:
        ...
        # TODO(liangqigong): Convert the bulk_backfill_data to Json and write to the intermidiate store. Currently
        #  we just print the row number to verify the data.
        table = bulk_backfill_data.read_all()
        print(f"Generated {table.num_rows} rows for FV {fd.name} for bulk backfill")
