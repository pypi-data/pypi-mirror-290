...
def download_data_frame(path: str) -> pandas.DataFrame:
...
def get_features_params_from_task_params(
    ...
    if dataset_generation_params.WhichOneof("input") == "spine":
        ...
        return GetFeaturesForEventsParams(
            fco=fco,
            events=download_data_frame(spine.path),
        ...
        entities = (
            download_data_frame(datetime_range.entities_path) if datetime_range.HasField("entities_path") else None
        )
