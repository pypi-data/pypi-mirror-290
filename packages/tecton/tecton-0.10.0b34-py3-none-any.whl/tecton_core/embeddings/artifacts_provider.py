...
def _initialize_model_cache_dir(model_name: str, model_file_path: Optional[str] = None) -> Tuple[str, bool]:
    model_cache_dir = os.path.join(conf.get_or_raise("MODEL_CACHE_DIRECTORY"), model_name)
    if os.path.exists(os.path.join(model_cache_dir, METADATA_FN)) or (
        model_file_path and os.path.exists(os.path.join(model_cache_dir, model_file_path))
    ):
