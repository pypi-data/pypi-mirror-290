...
class SparkConversionMixin(object):
    ...
    def _convert_from_pandas(self, pdf, schema, timezone):
        ...
        if timezone is not None:
            ...
            if isinstance(schema, StructType):
                ...
                # iteritems() replaced by items() in Pandas 1.5 and above
                if pd.__version__ >= '1.5.0':
                    ii = pdf.items()
                else:
                    ii = pdf.iteritems()

                for column, series in ii:
    ...
    def _create_from_pandas_with_arrow(self, pdf, schema, timezone):
        ...
        # iteritems() replaced by items() in Pandas 1.5 and above
        pd_version_gt_one = pd.__version__ >= '1.5.0'
        # Create list of Arrow (columns, type) for serializer dump_stream
        arrow_data = [[(c, t) for (_, c), t in zip(pdf_slice.iteritems() if not pd_version_gt_one else pdf_slice.items(), arrow_types)]
