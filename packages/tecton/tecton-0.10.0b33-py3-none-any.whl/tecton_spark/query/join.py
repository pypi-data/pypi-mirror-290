...
class AsofJoinFullAggSparkNode(SparkExecNode):
    ...
    def _get_aggregations(self, join_spec: _AsofJoinerAndWindowAggregator) -> List[pyspark.sql.Column]:
        ...
        for feature in time_aggregation.features:
            ...
            sawtooth_agg_feature = (
                self.sawtooth_aggregation_data.get_sawtooth_aggregation_for_output_feature_name(
                    feature.output_feature_name
                )
                if self.sawtooth_aggregation_data is not None
                else None
            )
            ...
            elif sawtooth_agg_feature is not None and sawtooth_agg_feature.batch_sawtooth_tile_size is None:
    ...
    def _generate_aggregation_window_spec(
        ...
        feature: feature_view_pb2.AggregateFeature,
    ) -> spark_window.WindowSpec:
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        timestamp_cols = (
            self.sawtooth_aggregation_data.get_anchor_time_columns()
            if self.sawtooth_aggregation_data is not None
            else [anchor_time()]
        )
        join_spec = _AsofJoinerAndWindowAggregator.create(
...
class AsofJoinSawtoothAggSparkNode(SparkExecNode):
    ...
    def _add_output_feature_name_cols(self, stream_df: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
        # Match the schema of the batch input node which has a column for each output feature name.
        time_aggregation = self.fdw.trailing_time_window_aggregation()
        new_columns = []
        cols_to_drop = []
        for feature in time_aggregation.features:
            ...
            feature_names = aggregation_plan.materialized_column_names(feature.input_feature_name)
            new_columns.extend([F.col(name).alias(feature.output_feature_name) for name in feature_names])
            cols_to_drop += feature_names
    ...
    def _get_aggregations(self, join_spec: _AsofJoinerAndWindowAggregator) -> List[pyspark.sql.Column]:
        ...
        for feature in time_aggregation.features:
            ...
            if sawtooth_agg_feature.batch_sawtooth_tile_size is None:
                # Use range query for continuous aggregations < 2 days.
                start, end = self.fdw.time_range_for_relative_time_window(time_window)
                window_spec = join_spec.get_range_between_window_spec(
                    start * 2 - 1,
                    end * 2 - 1,
            ...
            agg = aggregation_plan.full_aggregation_transform([feature.output_feature_name], window_spec)
            aggregations.append(agg.alias(feature.output_feature_name))
        return aggregations
