...
class FeatureDefinitionWrapper:
    ...
    def has_explicit_view_schema(self) -> bool:
        ...
        elif isinstance(self.fv_spec, (specs.RealtimeFeatureViewSpec, specs.FeatureTableSpec)):
    ...
    def offline_store_config(self) -> OfflineFeatureStoreConfig:
        if isinstance(self.fv_spec, specs.RealtimeFeatureViewSpec) or self.fv_spec.offline_store is None:
    ...
    def time_range_policy(self) -> feature_view_pb2.MaterializationTimeRangePolicy:
        if isinstance(self.fv_spec, specs.RealtimeFeatureViewSpec) or self.fv_spec.time_range_policy is None:
    ...
    def get_tile_interval_duration_for_offline_store(self) -> pendulum.Duration:
        ...
        For most cases this will be the aggregation interval. For stream fvs with compaction, the offline store is always untiled, but we will still create tiles during retrieval for better performance depending on the type of aggregation windows in the fv.
        """
    ...
    def materialized_data_path(self) -> str:
        if isinstance(self.fv_spec, specs.RealtimeFeatureViewSpec) or self.fv_spec.materialized_data_path is None:
    ...
    def earliest_anchor_time_from_window_start(self, window_start: Optional[datetime.timedelta]) -> Optional[int]:
        if window_start is None:
            return None
        if self.is_continuous and not self.compaction_enabled:
