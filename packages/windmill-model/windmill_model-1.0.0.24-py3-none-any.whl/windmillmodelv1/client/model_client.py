...
from tritonv2.bls_parse import GaeaBLSParser
from windmillartifactv1.client.artifact_api_artifact import LocationStyle, get_location_path, parse_artifact_name, Alias
...
def write_apply_yaml(apply_list, sep, output_path):
    """
    write apply yaml file
    Args:
...
class ModelClient(BceInternalClient):
    ...
    def create_model(
        ...
        if artifact_uri != "":
            artifact_uri = ArtifactClient(self.config).create_location_with_uri(
                uri=artifact_uri, object_name=object_name, style=style
            )
    ...
    def dump_models(self, artifact_name: str, location_style: str, rename: str = None, output_uri: str = "."):
        """
        ...
        manifest_list = self.get_model_manifest(model_name.workspace_id,
        ...
        model_store_name = ModelStoreName(workspace_id=model_name.workspace_id,
    ...
    def build_graph(
        ...
        artifact_client.download_artifact(name=ensemble_name, output_uri=ensemble_output_uri)
        ...
        # 4. 解析bls节点
        bls_parse = GaeaBLSParser(artifact_client,
        ...
        for name, _ in ensemble_config.get_ensemble_steps().items():
            ...
            bcelogger.info(f"Downloading model:{object_name} version:{sub_models[name]} to {model_output_uri}")
            artifact_client.download_artifact(object_name=object_name,
                                              version=sub_models[name],
                                              output_uri=model_output_uri)
            bls_parse(model_name=name, model_uri=model_output_uri, ensemble_steps=ensemble_steps, sub_models=sub_models)
        ...
        for name, step in ensemble_steps.items():
            ...
            response = self.get_model(workspace_id=ensemble_model.workspace_id,
                                      model_store_name=ensemble_model.model_store_name,
                                      local_name=name)

            response = json.loads(response.raw_data)
            response["version"] = sub_models[name] if sub_models[name] != "latest" else \
                str(response["artifact"]["version"])
            response["category"] = response["category"]["category"]
            models[name] = response

        # 6. 构建graph
        graph = Graph(models=models)
        graph_content = graph.build(name=ensemble_name,
                                    local_name=ensemble_model.local_name,
                                    ensemble_steps=ensemble_steps)

        return graph_content

    def _dump_model(self, filesystem, model, location_style: str, rename: str = None, output_uri: str = "."):
        """
        dump model
        Args:
...
def update_model_metadata(model_metadata):
    ...
    """
    # 1. 调整 extraModels 为空
    if model_metadata["extraModels"] is not None and len(model_metadata["extraModels"]) != 0:
