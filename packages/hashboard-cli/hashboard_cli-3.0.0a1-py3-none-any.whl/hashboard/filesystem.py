    ...
    for fp_pattern in filepaths_or_patterns:
        ...
        if input_as_path.is_file():
            ...
            if validated_contents:
                ...
                if project_root_dir:
                    path = input_as_path.parent.absolute().relative_to(project_root_dir)
            ...
            for root, subdirs, filenames in os.walk(input_as_path):
                ...
                for filename in filenames:
                    ...
                    validated_contents = validate_file(filepath)
                    if validated_contents:
                        env_var_names |= extract_variable_names(validated_contents)
                        ...
                        path_suffix = f"/{input_as_path}" if input_as_path else ""
                        # If a project root is provided we reference relative to that path
                        if project_root_dir:
                            path_suffix = f"/{input_as_path.parent.absolute().relative_to(project_root_dir)}"

                        parent_directory = root.replace(
                            str(input_as_path), f"/tmp/repos/{project_id}{path_suffix}"
                        )
            ...
            expanded_files: List[str] = glob.glob(fp_pattern, recursive=True)
            files, env_vars = process_filepaths(expanded_files, project_id)
