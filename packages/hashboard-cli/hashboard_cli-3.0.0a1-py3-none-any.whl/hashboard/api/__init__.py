...
def create_build_from_local_files(
    ...
    build_spec = build_spec_from_local(
        paths, project_id, dbt_manifest_path, dbt_inclusion_list
    )

    return _create_async_build(
        session, project_id, build_spec, deploy, allow_dangerous_empty_build, partial
    )
...
def pull_resource(
    ...
    res = _graphql_query(
        s,
        """
        query PullResource($projectId: String!, $resourceType: String, $resourceId: String, $resourceAlias: String, $dataOpsOnly: Boolean) {
            pullResource(
                projectId: $projectId,
...
def _create_async_build(
    ...
    build_spec,
    deploy,
    allow_dangerous_empty_build=False,
    partial=True,
):
    return _graphql_query(
        session,
        """
        mutation CreateAsyncBuild($projectId: String!, $buildSpec: BuildSpecInput!, $deploy: Boolean!, $allowEmptyBuild: Boolean, $partial: Boolean) {
            createAsyncBuild( projectId: $projectId, buildSpec: $buildSpec, deploy: $deploy, allowEmptyBuild: $allowEmptyBuild, partial: $partial)
        }
        """,
        {
            "projectId": project_id,
