...
@click.option(
    ...
    required=False,
    help="Specifies the root of the dbt directory containing any Hashboard models to build when using the dbt integration. Will be used when running hb build create with the --dbt option to run dbt parse in the correct location.",
)
@click.pass_context
def init(ctx: click.Context, dbt_root: Optional[Path]):
    ...
    if dbt_root:
        ...
        write_hashboard_project_value(DBT_ROOT_KEY, str(dbt_root))
        click.echo()
        click.echo("✅ Successfully configured dbt integration.")
        click.echo(
            "When running hb build create with --dbt Hashboard will now build models specified in the provided dbt root directory."
        )
...
@cli.command("remove-from-code-control", cls=CommandWithTracking)
