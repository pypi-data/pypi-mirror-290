...
class Pesquisador:
                            ...
                            if content_wrapper:
                                try:
                                    ...
                                    if contents:
                                        for content in contents:
                                            try:
                                                ...
                                                try:
                                                    ...
                                                    if figure:
                                                        if figcaption:
                                                            self.dados.append({'figure':figure})
                                                            fig = figcaption.text
                                                            self.text += f'[{fig}: {img_src}]\n'
                                                            self.image += f'[{fig}: {img_src}]\n'
                                                except:
                                                    figcaption = 'null'
                                        ...
                                        self.dados.append({"title": title, "content": self.text, "image" : self.image})
                                        self.text = ''
                                        self.image = ''
                                    else:
    ...
    def get_images(self, query : str, list : list):
        data = self.get_response(query)
        if data:
            for image in data:
                imagem = image.get('figure')
                list.append(imagem)
