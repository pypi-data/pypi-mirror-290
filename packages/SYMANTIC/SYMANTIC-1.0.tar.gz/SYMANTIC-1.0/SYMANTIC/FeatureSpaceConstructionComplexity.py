...
from .Regressor_complexity import Regressor
...
class feature_space_construction:
  ...
  ##############################################################################################################
  '''
  def __init__(self,operators,df,no_of_operators=None,device='cpu',initial_screening=None,metrics=[0.06,0.995],disp=False,pareto=False,dimension=3,sis_features=20):
  ...
  def feature_space(self):
    ...
    if self.no_of_operators == None:
        ...
        rmse1, equation1,r21,r,c,n,intercepts,coeffs =  Regressor(self.df_feature_values,self.Target_column,self.columns,complexity,self.dimension,self.sis_features,self.device,metrics = self.metrics).regressor_fit()
        ...
        self.update_pareto_intercepts=torch.cat((self.update_pareto_intercepts,intercepts))
        
        self.update_pareto_intercepts = torch.cat((torch.tensor([0]),self.update_pareto_intercepts))
        
        additional_columns = torch.full((1, abs(self.update_pareto_coeff.shape[1])), float('nan'))
        
        additional_columns[:,0] = 0
        
        self.update_pareto_coeff = torch.cat((additional_columns,self.update_pareto_coeff))
        ...
        if rmse1 <= self.rmse_metric and r21 >= self.r2_metric: 
            ...
            data_final = {'Loss':rmse_final,'Complexity':complexity_final,'Equations':names_final,
                          'Intercepts':intercepts.tolist(),'Coefficients':coeffs.tolist()}
        ...
        while True:
            ...
            rmse, equation,r2,r,c,n,intercepts,coeffs =  Regressor(self.df_feature_values,self.Target_column,self.columns,complexity,self.dimension,self.sis_features,self.device,metrics = self.metrics).regressor_fit()
