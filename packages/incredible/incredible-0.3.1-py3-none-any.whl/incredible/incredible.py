...
def whist2d_ci(h, sigmas=np.array([1.0,2.0]), prob=None, plot=None,
    ...
    plot - a matplotlib device that we can plot to, if desired
    ...
    for p in prob:
        ...
        for j in range(j, len(o)):
            if np.sum(h['z'][np.unravel_index(o[range(j)], h['z'].shape)]) / np.sum(h['z']) >= p:
                ...
                break
    cont_gen = contour_generator(h['x'], h['y'], h['z'])
    contours = [cont_gen.lines(lev) for lev in level]
    res = {'mode':mode, 'density':level, 'level':prob, 'contours':contours}
    if plot is not None:
        ci2D_plot(res, plot, Line2D_kwargs={'color':contour_color})
        if mode_fmt is not None:
            plot.plot(mode[0], mode[1], mode_fmt)
    return res

def ci2D_plot(ci2d, plot, which=None, transpose=False, outline=True, fill=False,
                  Line2D_kwargs=None, fill_kwargs=None):
    '''
    `ci2d` should be a dictionary of the type returned by whist2d_ci

    `plot` is a matplotlib thingy on which to plot

    `which` is an iterable for the indices into ci2d (i.e. the levels) to plot

    Everything else is hopefully self explanatory
    '''
    if which is None:
        which = range(len(ci2d['contours']))
        ...
        for j in range(i):
            ...
            if not skip2D:
                for k in range(len(tri[i][j]['contours'])-1, -1, -1):
                    ...
                    ci2D_plot(tri[i][j], ax, which=[k], **ci2D_kwargs)
            if makefig:
                # Axis limits
