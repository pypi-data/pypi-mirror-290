...
class IncrementGitlabTags(Job):
    ...
    * rules: on_tagsg
    ...
    def render(self) -> Dict[str, Any]:
        command = f'python3 -m gcip.tools.increment_gitlab_tag --gitlab-projects "{self.gitlabProjects}" --increment-type {self.incrementType.value}'

        if self.gitlabHost:
            command += f' --gitlab-host "{self.gitlabHost}"'
        if self.privateTokenEnv:
            command += f' --private-token-env "{self.privateTokenEnv}"'
        if self.jobTokenEnv:
