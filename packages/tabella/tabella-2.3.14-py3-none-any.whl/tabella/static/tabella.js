    ...
    fetch(url, options)
        ...
        .then(data => {
            ...
            if (data["result"] !== undefined) {
                ...
                responseLabel.classList.remove("text-red-600", "dark:text-red-500");
                responseLabel.classList.add("text-green-600", "dark:text-green-500");
            } else {
                ...
                responseLabel.classList.remove("text-green-600", "dark:text-green-500");
                responseLabel.classList.add("text-red-600", "dark:text-red-500");
            }
...
function onInputTypeChange(schemaId, idx, numItems) {
    document.getElementById(`${schemaId}_${idx}-form`).classList.remove("hidden");
    for (let i = 0; i < numItems; i++) {
        if (i === idx) {
            continue;
        }
        try {
            document.getElementById(`${schemaId}_${i}-form`).classList.add("hidden");
        } catch (e) {
...
function removeItem(inputId, min, max) {
    const counter = document.getElementById(`item-${inputId}-counter`);
    const itemDiv = document.getElementById(`input-${inputId}_item${Number(counter.value)}`);
    itemDiv.remove();
    counter.value = Number(counter.value) - 1;
    determineItemButtonDisabled(inputId, min, max)
}
...
function toggleMethodCallDetails(count) {
    const methodCallDetails = document.getElementById(`method-call-details-${count}`);
