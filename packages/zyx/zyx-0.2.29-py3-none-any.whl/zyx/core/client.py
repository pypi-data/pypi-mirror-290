    ...
    **kwargs,
    ...
    if tools:
        if "web" in tools:
            ...
            usable_tools.append(WebsiteTools().read_url)
            usable_tools.append(DuckDuckGo().duckduckgo_search)
        elif "calculator" in tools:
            from .toolkits import Calculator

            usable_tools.append(Calculator())
        elif "shell" in tools:
            from .toolkits import ShellTools

            usable_tools.append(ShellTools())
        elif "python" in tools:
            from .toolkits import PythonTools

            usable_tools.append(PythonTools())
        for tool in tools:
            if isinstance(tool, Callable):
                usable_tools.append([tool])
    return client._run_completion(
        messages=messages, tools=usable_tools, response_model=response_model
    )
