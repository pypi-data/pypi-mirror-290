...
class Broker(ABC):
    ...
    def convert_to_event(self, order: Any):
    ...
    def underlying_config(self, underlying_symbol: str, expiry: str | None = None):
    ...
    def symbol_max_lots(self, exchange: ExchangeType, symbol: str):
        try:
    ...
    def reverse_transaction_type(self, transaction_type: Literal["BUY", "SELL"]):
        if transaction_type == "BUY":
            return "SELL"
        elif transaction_type == "SELL":
            return "BUY"
    ...
    def modify_orders_till_complete(
        ...
        while 1:
            ...
            orders = orders.filter(pl.col("order_id").is_in(orders_placed))
            orders = orders.filter(
