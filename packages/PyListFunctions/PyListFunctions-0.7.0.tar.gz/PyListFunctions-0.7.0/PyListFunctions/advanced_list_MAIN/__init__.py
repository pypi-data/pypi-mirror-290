import advanced_list_EXTRA
from typing import Any, SupportsIndex
__all__ = ['__advanced_list__']


class org_list_modification(advanced_list_EXTRA.main_extra):
    """
    ...
    def __setitem__(self, index, value):
        ...
        if index in self._lock_lst:
            ...
            if type(index) is int:
                if len(self) < index and self.auto_replenishment:
                    super().append(value)
                    if not self.no_prompt:
    ...
    def extend(self, iterable):
        ...
        if type(iterable) not in self.type_lst and self.type_lst != [Any]:
            if not self.ignore_error:
                ...
                print(f"The types of elements in this list is: {self.type_lst}, The value you want to change: {repr(iterable)}, type of value: {type(iterable)}, method: extend")
            elif self.ignore_error and self.no_prompt:
                pass
        else:
    ...
    def pop(self, __index: SupportsIndex = None):
        if self.lock_all:
            if self.ignore_error:
                if not self.no_prompt:
