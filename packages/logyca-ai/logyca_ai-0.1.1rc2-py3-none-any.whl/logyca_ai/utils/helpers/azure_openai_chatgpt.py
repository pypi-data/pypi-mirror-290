...
class AzureOpenAIChatGPT():
    ...
    def build_conversation_message_list(self,content:Content,image_recognition:bool=False,ocr_engine_path:str=None,output_temp_dir:str=None,cleanup_output_temp_dir_after_hours: int = 24)->list:
        """
            ...
            if assistant_message is not None:
                ...
                if user_message is None:
                    ...
                    if type_message in PDFMessage.get_default_types():
                        additional_content = message.get("additional_content",None)
                        messages.append({"role":str(ContentRole.USER),"content":
                            PDFMessage(**additional_content).build_message_content(image_recognition=image_recognition,ocr_engine_path=ocr_engine_path,output_temp_dir=output_temp_dir,cleanup_output_temp_dir_after_hours=cleanup_output_temp_dir_after_hours)
                        })
