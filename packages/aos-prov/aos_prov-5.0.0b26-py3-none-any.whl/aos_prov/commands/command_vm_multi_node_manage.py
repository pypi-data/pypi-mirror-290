        ...
        if info_out:
            for row in info_out.splitlines():
                if row.startswith('IP Address:'):
                    ...
                    return vm_ip
        time.sleep(10)
        print_error('Could not find IP address.')
    return ''
...
def new_vms(vm_name: str, disk_location: str, nodes_count=2, check_virtualbox=True) -> ():
    ...
    if (disk_location_path / IMAGE_WITHOUT_NODES_FILENAME).exists():
        # TODO: remove in case support of old version R3 is not required
        print_message('Found VM image (v3) without supporting Nodes functionality. Use it as one node VM.')
    ...
    elif (disk_location_path / NODE0_IMAGE_FILENAME).exists() and (disk_location_path / NODE1_IMAGE_FILENAME).exists():
                ...
                nodes.append(
                    {
                        ...
                        'uuid': str(uuid.uuid4()),
                        'disk_name': DYNAMIC_NODES[index],
                        'disk_location': Path(disk_location_path / DYNAMIC_NODES[index]),
                    },
                )
...
def add_node_to_unit_by_vm_group(
    ...
    detailed_list_vms = _get_detailed_vms_list(vm_name)
...
def _get_detailed_vms_list(vm_name: str) -> dict:
