...
class YamlWriter:
    ...
    def write_1(self, templates_path: str, output_path: str, device_name: str) -> None:
        env = Environment(
            loader=FileSystemLoader(templates_path),
            undefined=ChainableUndefined,
            lstrip_blocks=True,
        ...
        parent_dir = os.path.dirname(templates_path)
        ...
        for dir, _, files in os.walk(templates_path):
            ...
            if files:
                try:
                    ...
                    logger.error("Generate working file failed: {}".format(log_path))
                    logger.error("Error: {}".format(e))
                    exit()
            else:
                try:
                    ...
                    rel = rel.replace("\\", "/")
                    self.o_dir = self._fix_duplicate_path(
                        output_path, folder_name, device_name, rel
                    )
                    pathlib.Path(self.o_dir).mkdir(parents=True, exist_ok=True)
                    logger.info("Generate working directory successfully: {}"
                                .format(self.o_dir))
    ...
    def write_2(self, templates_path: str, output_path: str, device_name: str) -> None:
        env = Environment(
            loader=FileSystemLoader(templates_path),
            undefined=ChainableUndefined,
            lstrip_blocks=True,
        ...
        folder_name = os.path.basename(templates_path)
