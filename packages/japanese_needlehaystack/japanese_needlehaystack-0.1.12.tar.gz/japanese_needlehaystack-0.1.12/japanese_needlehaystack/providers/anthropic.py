...
class Anthropic(ModelProvider):
    DEFAULT_MODEL_KWARGS: dict = dict(max_tokens_to_sample=300, temperature=0)

    def __init__(
        self, model_name: str = "claude-2.1", model_kwargs: dict = DEFAULT_MODEL_KWARGS
    ):
        """
    ...
    async def evaluate_model(self, prompt: str) -> str:
        response = await self.model.completions.create(
            model=self.model_name, prompt=prompt, **self.model_kwargs
        )
    ...
    def generate_prompt(
        ...
        return self.prompt_structure.format(
            retrieval_question=retrieval_question, context=context
        )
