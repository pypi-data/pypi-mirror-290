...
class PackStatistics(BaseModel):
    ...
    """ # noqa: E501
    products_count: Annotated[int, Field(le=700001, strict=True, ge=0)] = Field(description="The total number of products (taking into account the number of goods in one group).")
    packages_count: Annotated[int, Field(le=700001, strict=True, ge=0)] = Field(description="The total number of packages.")
    total_mass: Union[Annotated[float, Field(le=1000000, strict=True, ge=0)], Annotated[int, Field(le=1000000, strict=True, ge=0)]] = Field(description="Total weight in kilograms.")
    total_volume: Union[Annotated[float, Field(le=1000000, strict=True, ge=0)], Annotated[int, Field(le=1000000, strict=True, ge=0)]] = Field(description="The total volume of all packed goods in cubic meters.")
    package_statistics: Annotated[List[PackageStatistics], Field(min_length=0, max_length=11)] = Field(description="Statistics on used packages.")
    __properties: ClassVar[List[str]] = ["products_count", "packages_count", "total_mass", "total_volume", "package_statistics"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PackStatistics from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in package_statistics (list)
        _items = []
        if self.package_statistics:
            for _item in self.package_statistics:
                if _item:
                    _items.append(_item.to_dict())
            _dict['package_statistics'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PackStatistics from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "products_count": obj.get("products_count"),
            "packages_count": obj.get("packages_count"),
            "total_mass": obj.get("total_mass"),
            "total_volume": obj.get("total_volume"),
            "package_statistics": [PackageStatistics.from_dict(_item) for _item in obj["package_statistics"]] if obj.get("package_statistics") is not None else None
        })
        return _obj
