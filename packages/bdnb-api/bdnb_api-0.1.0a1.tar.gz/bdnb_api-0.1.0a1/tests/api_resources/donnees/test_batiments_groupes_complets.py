...
from bdnb_api.types.donnees import BatimentsGroupesCompletListResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestBatimentsGroupesComplets:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_list(self, client: BdnbAPI) -> None:
        batiments_groupes_complet = client.donnees.batiments_groupes_complets.list()
        assert_matches_type(BatimentsGroupesCompletListResponse, batiments_groupes_complet, path=["response"])
    ...
    def test_raw_response_list(self, client: BdnbAPI) -> None:
        response = client.donnees.batiments_groupes_complets.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        batiments_groupes_complet = response.parse()
        assert_matches_type(BatimentsGroupesCompletListResponse, batiments_groupes_complet, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: BdnbAPI) -> None:
        with client.donnees.batiments_groupes_complets.with_streaming_response.list() as response:
...
class TestAsyncBatimentsGroupesComplets:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_list(self, async_client: AsyncBdnbAPI) -> None:
        batiments_groupes_complet = await async_client.donnees.batiments_groupes_complets.list()
        assert_matches_type(BatimentsGroupesCompletListResponse, batiments_groupes_complet, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncBdnbAPI) -> None:
        response = await async_client.donnees.batiments_groupes_complets.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        batiments_groupes_complet = await response.parse()
        assert_matches_type(BatimentsGroupesCompletListResponse, batiments_groupes_complet, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncBdnbAPI) -> None:
        async with async_client.donnees.batiments_groupes_complets.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
