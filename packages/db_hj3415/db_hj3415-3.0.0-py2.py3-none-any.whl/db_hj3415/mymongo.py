...
class Base:
    import cli
    mongo_client: MongoClient = connect_to_mongo(cli.load_mongo_addr())
...
class Corps(Base):
    ...
    @classmethod
    def _save_df(cls, code: str, page: str, df: pd.DataFrame, index_unique: bool, clear_table: bool):
        ...
        if clear_table:
            ...
            Base.mongo_client[code][page].delete_many({})
            import time
            time.sleep(.5)

        Base.mongo_client[code][page].insert_many(df.to_dict('records'))
...
class C101(Corps):
    ...
    def save(cls, code: str,  c101_data: dict) -> bool:
        ...
        if (set(c101_template) - set(c101_data.keys())) == set():
            ...
            Base.mongo_client[code]['c101'].create_index([('date', ASCENDING)], unique=True)
            try:
...
class C106(Corps):
    ...
    def save(cls, code: str, page: str, c106_df: pd.DataFrame):
        ...
        cls._save_df(code=code, page=page, df=c106_df, index_unique=True, clear_table=clear_table)
...
class C103(C1034):
    ...
    def save(cls, code: str, page: str, c103_df: pd.DataFrame):
        ...
        Base.mongo_client[code][page].create_index([('항목', TEXT)], unique=True)
        cls._save_df(code=code, page=page, df=c103_df, index_unique=False, clear_table=clear_table)
...
class C104(C1034):
    ...
    def save(cls, code: str, page: str, c104_df: pd.DataFrame):
        ...
        Base.mongo_client[code][page].create_index([('항목', TEXT)], unique=True)
        cls._save_df(code=code, page=page, df=c104_df, index_unique=True, clear_table=clear_table)
...
class C108(Corps):
    ...
    def save(cls, code: str, c108_data: pd.DataFrame):
        ...
        Base.mongo_client[code][page].create_index([('날짜', ASCENDING)])
        cls._save_df(code=code, page=page, df=c108_data, index_unique=False, clear_table=True)
...
class MI(Base):
    ...
    def get_recent(self) -> Tuple[str, float]:
        ...
        if self.validate_col():
            d = self.my_col.find({'date': {'$exists': True}}).sort('date', DESCENDING).next()
            del d['_id']
