...
class Base:
    ...
    mongo_client: pymongo.MongoClient = mymongo.connect_to_mongo(db_cli.load_mongo_addr())
...
class Corps(Base):
    ...
    def list_all_codes_names(cls) -> dict:
        def get_name(code_in: str) -> str:
            from pymongo import DESCENDING
            try:
                d = dict(cls.mongo_client[code_in]['c101'].find({'date': {'$exists': True}}, {"_id": 0})
                         .sort('date', DESCENDING).next())
