...
class plot:
    ...
    def get_RBF_class(self, cutoff:float=6.0, number_of_bins:int=100, 
        ...
        distance_matrix_dict = {a_class:{b_label:[] for b_label in self.uniqueAtomLabels} 
                                for a_class in unique_class_IDs}
        ...
        for ID_a, ID_neighbors_a in enumerate(ID_neighbors):
            ...
            for ID_b in ID_neighbors_a: 
                ...
                if ID_a != ID_b:  # Exclude self-interactions
                    distance_matrix_dict[class_a][label_b].append(np.linalg.norm(position_a - position_b))
        
        rbf = {a_class:{} for a_class in unique_class_IDs}
        for a_class in unique_class_IDs:
            for b_label in self.uniqueAtomLabels:
                distances = np.array(distance_matrix_dict[a_class][b_label])
                distances = distances[distances > 0.1]
                rbf_a, bin_edges = np.histogram(distances, bins=number_of_bins, range=(0, cutoff))
                bin_centers = (bin_edges[1:] + bin_edges[:-1]) / 2
                ...
                if density_normalize:
                    rbf_a /= len(positions) / self.get_volume()
                rbf[a_class][b_label] = [bin_centers, rbf_a]
