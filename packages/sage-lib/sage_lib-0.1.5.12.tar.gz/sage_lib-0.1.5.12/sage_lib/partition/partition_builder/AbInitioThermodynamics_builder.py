...
class AbInitioThermodynamics_builder(PartitionManager):
    ...
    def plot_RBF_cluster(self, atoms_cluster:list=None, cluster_labels:list=None, cutoff:float=3.0, number_of_bins:int=100, output_path:str='./'):
        directory = os.path.dirname(output_path)
        if not os.path.exists(directory):
            os.makedirs(directory, exist_ok=True)

        rbf = {cluster_class_name_idx:{ atomlabel_class_name:np.zeros((2,number_of_bins)) for atomlabel_class_name in self.uniqueAtomLabels} for cluster_class_name_idx, cluster_class_name in enumerate(cluster_labels) }
        ...
        for cluster_class_idx, cluster_class_name in enumerate(cluster_labels):
            fig, ax = plt.subplots()
            ...
            for atomlabel_class_idx, atomlabel_class_name in enumerate(self.uniqueAtomLabels):
                bin_centers, rbf_a = rbf[cluster_class_idx][atomlabel_class_name]
    ...
    def handleABITAnalysis(self, values: Dict[str, Dict], file_location: str = None) -> None:
        ...
        for abit, params in values.items():
            ...
            if abit.upper() == 'COMPOSITION_ANALYSIS':
                ...
                r_cut, n_max, l_max, sigma = 3.5, 4, 4, 0.5
                ...
                for c_i, c in enumerate(self.containers):
                    c.AtomPositionManager.class_ID = np.array( [ nc for nc in structure_labels[c_i]] )
                ...
                self.plot_RBF_cluster(atoms_cluster=structure_labels, cluster_labels=class_labels, cutoff=r_cut, number_of_bins=100, output_path='rbf/')
