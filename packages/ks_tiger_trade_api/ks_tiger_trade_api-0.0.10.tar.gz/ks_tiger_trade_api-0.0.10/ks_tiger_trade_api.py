...
def extract_my_symbol(contract: Contract):
...
class KsTigerTradeApi(BaseTradeApi):
    ...
    # 获取账号信息
    def query_account(self, currency: KsCurrency = KsCurrency.USD) -> tuple[RetCode, Union[MyAccountData, ErrorData]]:
        try:
            data: PortfolioAccount = self.trd_ctx.get_prime_assets(self.account_id)
        except Exception as e:
    ...
    def query_position(self, vt_symbols=[], directions: list[KsDirection] = []):
        try:
            data = self.trd_ctx.get_positions(symbol=None)
        except Exception as e:
