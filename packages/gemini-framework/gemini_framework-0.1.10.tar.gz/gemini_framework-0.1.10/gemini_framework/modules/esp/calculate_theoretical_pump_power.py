...
class CalculateTheoreticalPumpPower(UnitModuleAbstract):
    model = None
    ...
    def __init__(self, unit):
        ...
        self.link_output('calculated', 'esp_pump_power')

    def step(self, loop):
        self.loop = loop
        self.loop.start_time = self.get_output_last_data_time('esp_pump_power')
        ...
        for ii in range(1, self.loop.n_step):
            try:
                ...
                esp_pump_power.append(y['pump_power'])
            except Exception as e:
                self.logger.warn("ERROR:" + repr(e))
                esp_pump_power.append(None)

        if esp_pump_power:
            self.write_output_data('esp_pump_power', time, esp_pump_power)
