...
def retry_operation(tries: int, delay: int, backoff: int):
    """
        ...
        def wrapper_retry(*args, **kwargs) -> Any:
            ...
            while attempts < tries:
                try:
                    ...
                    print(
                        f"Attempt {attempts + 1} failed with error: {e}. Retrying in {current_delay} seconds..."
                    )
                    time.sleep(current_delay)
                    attempts += 1
                    current_delay *= backoff
            raise Exception(f"Operation failed after {tries} attempts")
...
class SparkManager:
    ...
    @retry_operation(tries=4, delay=2, backoff=2)
    def read(
        self,
        env: Env,
