...
from nicegui_extentions.markdown import (
    MarkdownPreViewer,
...
from src.nicegui_extentions.reconnect_box import set_reconnect_box
...
async def show_in_dialog():
    ...
    with ui.dialog() as dialog, ui.card().classes("w-full").style("max-width: none"):
        # test use with other component
        md = MarkdownPreViewer()
    dialog.open()
    md.render(markdown, anchor=1)


async def show_in_dialog_and_update():
    """click to update"""
    with ui.dialog() as dialog, ui.card().classes("w-full").style("max-width: none"):
        # test use with other component
        md = MarkdownPreViewer()
...
async def show():
    """click to update"""
    md = MarkdownPreViewer().classes("w-96")
    md.render(markdown, anchor=1)


async def show_and_update():
    """click to update"""
    md = MarkdownPreViewer()
    md.render(markdown, anchor=0)
    await asyncio.sleep(2)
    md.render("# 测试第二次发送")


initMarkdownPreViewerDependence()
button = ui.button("show in dialog", on_click=show_in_dialog)
button = ui.button("show", on_click=show)
button = ui.button("show in dialog and update", on_click=show_in_dialog_and_update)
button = ui.button("show and update", on_click=show_and_update)
app.on_connect(lambda: set_reconnect_box())

ui.run(reload=False, dark=None)
