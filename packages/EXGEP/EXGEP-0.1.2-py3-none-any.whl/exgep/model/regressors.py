...
def regressor_selector(regressor_names, random_state = None):
    ...
    regressors = ['dummy', 'lightgbm', 'xgboost', 'catboost', 'bayesianridge', 'lassolars', 'adaboost', 'gradientboost', 'histgradientboost',
                  'knn', 'sgd', 'bagging', 'svr', 'elasticnet', 'randomforest', 'gbdt']
    ...
    regressor_dict['randomforest'] = randomforest_loader(random_state=random_state) if 'randomforest' in selected_regressors else None
    regressor_dict['gbdt'] = gbdt_loader(random_state=random_state) if 'gbdt' in selected_regressors else None
...
def randomforest_loader(random_state):
    ...
    def randomForestHParams(trial):
        param_dict = {}
        param_dict['n_estimators'] = trial.suggest_int('n_estimators', 10, 2000)
...
def gbdt_loader(random_state):
    from sklearn.ensemble import GradientBoostingRegressor

    def gbdtHParams(trial):
        param_dict = {}
        param_dict['n_estimators'] = trial.suggest_int('n_estimators', 50, 2000)
        param_dict['learning_rate'] = trial.suggest_float('learning_rate', 0.001, 0.1)
        param_dict['max_depth'] = trial.suggest_int('max_depth', 2, 25)
        param_dict['min_samples_split'] = trial.suggest_int('min_samples_split', 2, 20)
        param_dict['min_samples_leaf'] = trial.suggest_int('min_samples_leaf', 1, 20)
    ...
    return (GradientBoostingRegressor, gbdtHParams)