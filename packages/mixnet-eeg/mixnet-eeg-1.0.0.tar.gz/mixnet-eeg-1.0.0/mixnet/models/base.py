...
class BaseModel(Trainer):
    ...
    def evaluate(self, X_test, y_test):
        ...
        if model.name.startswith('MIN2Net'):
            if 'without_decoder' not in model.name:
                try:
            ...
            elif 'without_decoder' in model.name:
                zs, y_pred_clf = test_pred[0], test_pred[1]
                y_pred_argm = np.argmax(y_pred_clf, axis=1)
                Y = {'y_true': y_test, 'y_pred': y_pred_argm, 'y_pred_clf':y_pred_clf, 'latent': zs}
        elif '_contrastive'  in model.name:
            zs, y_pred_clf = test_pred[0], test_pred[1]
            y_pred_argm = np.argmax(y_pred_clf, axis=1)
            Y = {'y_true': y_test, 'y_pred': y_pred_argm, 'y_pred_clf':y_pred_clf, 'latent': zs}
        elif model.name == 'FBMSNet':
            center, y_pred_clf = test_pred[0], test_pred[1]
            y_pred_argm = np.argmax(y_pred_clf, axis=1)
            Y = {'y_true': y_test, 'y_pred': y_pred_argm, 'y_pred_clf':y_pred_clf, 'center': center}
        else:
    ...
    def predict(self, X_test, y_test):
        ...
        if model.name.startswith('MIN2Net'):
            ...
            zs, y_pred_clf = test_pred[0], test_pred[1]
            y_pred_argm = np.argmax(y_pred_clf, axis=1)
            Y = {'y_true': y_test, 'y_pred': y_pred_argm, 'latent': zs}
        elif model.name == 'FBMSNet':
            center, y_pred_clf = test_pred[0], test_pred[1]
            y_pred_argm = np.argmax(y_pred_clf, axis=1)
            Y = {'y_true': y_test, 'y_pred': y_pred_argm, 'center': center}
        else:
            y_pred_argm = np.argmax(test_pred, axis=1)
            Y = {'y_true': y_test, 'y_pred': y_pred_argm, 'y_pred_clf':test_pred}
    ...
    def evaluate_seizure(self, X_test, y_test):
        # if X_test.ndim != 4:
        #     raise Exception('ValueError: `X_test` is incompatible: expected ndim=4, found ndim='+str(X_test.ndim))
        ...
        if model.name.startswith('MIN2Net'):
            ...
            y_pred_argm = np.argmax(test_pred, axis=1)
            Y = {'y_pred': y_pred_argm, 'y_pred_clf':test_pred}
            
        print(classification_report(y_test, y_pred_argm))
        sensitivity, specificity, f1_binary, f1_macro, f1_weighted = self.sen_spec(y_test, y_pred_argm)
        evaluation.update({ 'sensitivity':sensitivity, 'specificity': specificity, 'f1-score-binary': f1_binary, 'f1-score-macro': f1_macro, 'f1-score-weighted': f1_weighted})
        evaluation.update(dict(zip(['w_'+name+'_loss' for name in self.loss_names], self.best_loss_weights.numpy())))   
        return Y, evaluation
