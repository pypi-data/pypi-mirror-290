...
def test_dev_help() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert "Run a ReadyAPI app in development mode." in result.output
    assert (
        "This is equivalent to readyapi run but with reload enabled and listening on the 127.0.0.1 address."
        in result.output
    )
    assert (
        "Otherwise, it uses the first ReadyAPI app found in the imported module or package."
        in result.output
    )
...
def test_run_help() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert "Run a ReadyAPI app in production mode." in result.output
    assert (
        "This is equivalent to readyapi dev but with reload disabled and listening on the 0.0.0.0 address."
        in result.output
    )
    assert (
        "Otherwise, it uses the first ReadyAPI app found in the imported module or package."
...
def test_callback_help() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert "ReadyAPI CLI - The readyapi command line app." in result.output
...
def test_version() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert "ReadyAPI CLI version:" in result.output


def test_script() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "coverage", "run", "-m", "readyapi_cli", "--help"],
        capture_output=True,
        encoding="utf-8",
    )
