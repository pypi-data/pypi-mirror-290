...
def find_papers(query="", doi=[], number_of_papers=100, start_year=2010, end_year=datetime.now().year+1, sort="published", order="desc"):
    cr = Crossref()
    if query!="":
        ...
        papers = cr.works(ids=doi, limit=number_of_papers, filter = {'from-pub-date': str(start_year), 'until-pub-date': str(end_year)}, sort=sort)
    else:
