...
from tgqSim.utils.visualization import to_text_diagram
...
class QuantumCircuit:
    ...
    def __str__(self):
        return to_text_diagram(self.gate_list, self.width)
    ...
    # todo: define add measure gate method
    def add_measure_gate(self, gate_pos: Union[list, int], *gate_info):
        if isinstance(gate_pos, list):
            min_pos, max_pos = min(gate_pos), max(gate_pos)
        else:
    ...
    def append(self, gate: CommonGate):
        ...
        if gate.name in MEASURE:
            ...
            # todo:
            # self.add_measure_gate(gate['qbit'], gate['name'])
            self.add_measure_gate(gate.qbit, gate.name)
        ...
        # elif gate['name'] in BASE_SINGLE_GATE:
            ...
            if gate.theta is None:
                ...
                #     self.add_single_gate(gate['qbit'],
                #                          gate['name'],
                #                          *gate['theta'])
                # else:
