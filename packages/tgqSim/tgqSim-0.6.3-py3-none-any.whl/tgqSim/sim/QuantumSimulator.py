...
class QuantumSimulator:
    ...
    def _run_with_npu_device(self, circuit: QuantumCircuit):
        ...
        for (gate_pos, gate_info) in circuit.gate_list:
            ...
            if gate_type in BASE_SINGLE_GATE_MAP.keys():
    ...
    def run_statevector(self, circuit: QuantumCircuit, device: str):
        """
        根据线路的门序列计算末态的量子态
        :return:
        """
    ...
    def execute(self, circuit: QuantumCircuit, shots: int = 1000, device:str='cpu') -> dict:
        ...
        for gate in circuit.gate_list:
            gate_dict.update({gate[1][0]: gate[0]})
        # print(gate_dict)
        if 'measure' in gate_dict.keys():
            measure_bits_list = sorted(gate_dict['measure'], reverse=True)
        if measure_bits_list is None or measure_bits_list == []:  # 若为空默认测量所有可能
            # bugfix, comment this line
            # distribution = count
            measure_bits_list = sorted([_ for _ in range(circuit.width)], reverse=True)
