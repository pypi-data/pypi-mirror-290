...
from ....common.types import CrawlerContent, CrawlerDemoUser, DatapoolContentType, StudyUrlTask, CrawlerBackTask
...
class WikipediaPlugin(BasePlugin):
        ...
        async with async_playwright() as playwright:
            ...
            async for yielded in self.parse_links(page):
                if isinstance(yielded, CrawlerBackTask) and self.is_page_with_content(yielded.url):
                    ...
                    logger.info(f"no content: {yielded.url}")
    ...
    def is_page_with_content(url: str):
        ...
        return not any(has_sub(sub) for sub in ["/wiki/Special:", "/wiki/Wikipedia:", "/w/index.php?title=Special:"])
