...
class CrawlerScheduler(Stoppable):
    ...
    async def stop(self):
        ...
        for session_id in self.todo_queues:
            await self.todo_queues[session_id].stop()
        logger.info("todo_queues stopped")
    ...
    async def _maybe_hard_stop(self, session: Session, meta: dict):
        ...
        if await session.is_alive():
            num_since_last_tagged = meta["since_last_tagged"]  # await session.get_since_last_tagged()
            ...
            if need_hard_stop:
                ...
                await self._set_hint_url_status(meta.get("hint_id"), CrawlerHintURLStatus.Canceled, session, meta)
            else:
        ...
        if urls_done:
            ...
            if content_done:
                ...
                if url_state is not None:
                    ...
                    await self._set_hint_url_status(meta["hint_id"], url_state.status, session, meta)
                else:
    ...
    async def _set_hint_url_status(self, hint_id, status: CrawlerHintURLStatus, session: Session, meta=None):
        if meta is None:
            meta = await session.get_meta()
        ...
        if status in (
            ...
            if session.id in self.todo_queues:  # may be false if scheduler with that session is killed
    ...
    async def _try_running_session_queue(self, session_id) -> GenericQueue:
        if session_id not in self.todo_queues:
            queue_name = f"{self.cfg.WORKER_TASKS_QUEUE_NAME}_{session_id}"
        ...
        return self.todo_queues[session_id]

    async def _enqueue_worker_task(
        self, url: AnyUrl | str, session_id, status: Optional[CrawlerHintURLStatus] = None  # priority: int,
    ):
        logger.info(f"_enqueue_worker_task {url=} {type(url)=} {status=}")
        task = WorkerTask(url=str(url), status=status)

        queue = await self._try_running_session_queue(session_id)
        await queue.push(
            QueueMessage(
                session_id=session_id, message_type=QueueMessageType.Task, data=task.to_dict()  # , priority=priority
            )
        )
