...
class Construction:
    def __init__(self):
        self.phantoms = {} #{'_1': Var, '_2': Point, '_3': [Point, float, '_2]}
        self.vars = []
        self.elements = [
    ...
    def update(self, name, data, log = None):
        ...
        if obj is None:
            ...
            if isinstance(data, (Point, Line, Angle, Polygon, Circle, Vector)):
                ...
                print(f"Construction.update({name}) ERROR: element data type update has no realization")
        else:
    ...
    def rebuild(self, debug = False, full = False):
        log = {}
        if full:
            for command in self.commands: self.apply(command, debug)
        else:
            state = []
            for name in self.state:
                level = self.state[name]['level']
            ...
            for level in range(len(state)):
                for name in state[level]:
                    #print(f'BEFORE: {name} built {self.state[name]["built"]}')
