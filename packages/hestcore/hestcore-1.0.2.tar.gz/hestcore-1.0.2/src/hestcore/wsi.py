...
class NumpyWSI(WSI):
    ...
    def create_patcher(
        ...
        return NumpyWSIPatcher(self, patch_size, src_pixel_size, dst_pixel_size, overlap, mask, coords_only, custom_coords, threshold, pil)
...
class OpenSlideWSI(WSI):
    ...
    def create_patcher(
        ...
        return OpenSlideWSIPatcher(self, patch_size, src_pixel_size, dst_pixel_size, overlap, mask, coords_only, custom_coords, threshold, pil)
...
class CuImageWSI(WSI):
    ...
    def create_patcher(
        ...
        return CuImageWSIPatcher(self, patch_size, src_pixel_size, dst_pixel_size, overlap, mask, coords_only, custom_coords, threshold, pil)
...
class WSIPatcher:
    ...
    def save_visualization(self, path, vis_width=1000, dpi=150):
        mask_plot = self.wsi.get_tissue_vis(
            self.mask,
            fill_color=(0, 255, 0),
...
def contours_to_img(
    ...
    for _, group in groups:
        ...
        for _, row in group.iterrows():
            ...
            draw_cont_fill(image=img, contours=[cont], color=fill_color)
