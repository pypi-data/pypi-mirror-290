...
def send_email_with_attachment(subject, to, contents, attachments):
    ...
    if to[0] == "":
        yag.send(
            to=[default_to], subject=subject, contents=contents, attachments=attachments
        )
    else:
...
def main():
    USAGE = "usage MAIL [email address] -s [subject] -a [attachments]"
    if len(sys.argv) < 2:
        print(USAGE)
        return
    to = sys.argv[1]
