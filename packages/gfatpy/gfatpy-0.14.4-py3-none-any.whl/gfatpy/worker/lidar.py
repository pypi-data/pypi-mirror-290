...
from gfatpy.lidar.nc_convert.converter import measurements_to_nc
...
from gfatpy.lidar.nc_convert.types import Measurement
...
def nc_convert(lidar_name: str, target_date: str | date | None = None) -> str:
    ...
    if not raw_dir.exists():
        return f"{raw_dir} not found in {RAW_DIR}."

    measurements = to_measurements(raw_dir.glob("RS*"))
    measurements_to_nc(
        measurements,
        lidar_name=lidar,
        raw_dir=RAW_DIR,
...
def quicklook(
    lidar_name: str, channel: str, target_date: str | date | None = None
) -> str:
    ...
    quicklook_dir = PRODUCTS_DIR / lidar_name / "quicklooks"
    quicklook_dir.mkdir(parents=True, exist_ok=True)

    figures = []
    for file_ in files:
        logger.info(f"{file_.name} measurements found in {data_dir}.")

        quicklook_from_file(
            filepath=file_, channels=[channel], output_dir=quicklook_dir
        )
        ...
        figures.append(
            ...
            / f"{lidar_name}"
            / "quicklooks"
            / f"quicklook_{lidarnick}_{channel}_{datestr}.png"
        )
    return f"{figures[0]} created."
...
def send_to_scc(
    ...
    **kwargs,
    ...
    info_scc_dir = Path(
        kwargs.get("info_scc_dir", (GFATPY_DIR / "env_files").absolute().as_posix())
    )
    config_path = info_scc_dir / info_scc_config_name
    ...
    for file_ in files:
        ...
        if measurement_exists:
            logger.info(
                f"Measurement {measurement_id} exists in SCC: {measurement_exists}."
            )
            return f"{measurement_id} already exists in SCC."
...
def plot_scc(
    lidar_name: str, scc_id: int, target_date: str | date | None = None
) -> list[str] | str:
