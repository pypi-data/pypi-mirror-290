...
def Shopify_get_image_url_from_gid(shop, access_token, api_version, gid):
    """
...
def Shopify_publish_blog_post(shop="", access_token="", api_version="2024-01", blog_id="", title="", content="", author="", tags=[], published_at=None, image_path=None):
    """
    ...
    if image_path:
        ...
        upload_response = Shopify_upload_file(shop=shop, access_token=access_token, api_version=api_version, image_path=image_path, file_name=file_name, title=title)  
       
        if upload_response.status_code == 200:
            gid = upload_response.data['data']['fileCreate']['files'][0]['id']
            image_url = Shopify_get_image_url_from_gid(shop, access_token, api_version, gid)
            if not image_url:
    ...
    if response.status_code == 201:
        return CustomResponse(data=response.json(), status_code=201)
    else:
...
def main():
    ...
    shop = os.getenv("SHOPIFY_VINZO_SHOP")
    access_token=os.getenv("SHOPIFY_VINZO_TOKEN")
    ...
    if option == '1':
        metaobject_handle="vinzo-lust-banner"
    ...
    if option == '2':   
        ...
        custom_response = Shopify_publish_blog_post(shop=shop, access_token=access_token, blog_id=blog_id, title=title, content=content, author=author, tags=tags, published_at=published_at, image_path=image_path )
        print(custom_response)
    if option == '3':   
        # Example Usage
        shop = shop
        ...
        upload_response = Shopify_upload_file(shop, access_token, api_version, file_path)
