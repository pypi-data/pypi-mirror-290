...
class MediaKillerApp(AbstractApp):
    ...
    def run(self):
        ...
        if self.args.pretend_mode:
            ...
            for m in env.progress.track(
                missions, task_id=self.global_task, description="正在假装运行…"
            ):
                cmd = ScriptWriter.compile_cmd(m)
                env.print(f"[grey]假装执行：[/grey] {cmd}")
            return
        ...
        if self.args.script_output:
            ...
            for m in missions:
                ...
                env.print(
                    f"[yellow]{job_counter}[/yellow] [cyan]{mission.source.name}[/cyan] 转换完成"
                )
