...
class Transcoder:
    ...
    def __enter__(self):
        self.task = env.progress.add_task(
            description=self.mission.source.name, total=None
        )
    ...
    async def transcode(self):
        ...
        env.progress.update(self.task, description="检查目标目录可用性…", completed=0.0)
        ...
        env.current_ffmpeg = self.ffmpeg
        ...
        def on_progress(progress: Progress):
            desc = f"{self.mission.source.name} [yellow]x{progress.speed:.2f}[/yellow]"
    ...
    def run(self):
        try:
            ...
            env.error(f"FFMPEG 执行出错：[red]{err.message}[/red]")
