...
class ALICE(tkinter.Frame):
    ...
    def open_tiepoint_popup(self):
        ...
        def callback(*_: Any):
            try:
                _x = float(ref_depth_sv.get()) + float(profile_depth_sv.get())
                create_tiepointButton.configure(state="normal")
            except ValueError:
                create_tiepointButton.configure(state="disabled")

        ref_depth_sv.trace_add("write", callback)
        profile_depth_sv.trace_add("write", callback)
        ...
        create_tiepointButton = tkinter.Button(
            master=top, text="Create tiepoint", command=add_tiepoint
        )
        create_tiepointButton.configure(state="disabled")
        create_tiepointButton.pack(side=tkinter.BOTTOM)
