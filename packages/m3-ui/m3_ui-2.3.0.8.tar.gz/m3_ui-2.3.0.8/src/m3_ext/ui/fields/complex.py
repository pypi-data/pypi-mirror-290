...
class ExtDictSelectField(BaseExtTriggerField):
    ...
    class ExtTrigger(BaseExtComponent):
        ...
        def render(self):
            res = f'iconCls: "{self.icon_cls if self.icon_cls else ""}"'
            res += f',handler: {self.handler}' if self.handler else ''
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Тип поля
        self.type = ExtDictSelectField.INT
    ...
    def set_value_from_model(self, obj):
        ...
        self.set_empty_record_value()
        for attr in attr_set:
            value = getattr(obj, attr, None)
            self.__record_value[attr] = value() if callable(value) else value

    def set_empty_record_value(self):
        """
        Установка пустого record_value.
        """
        self.__record_value = {}
