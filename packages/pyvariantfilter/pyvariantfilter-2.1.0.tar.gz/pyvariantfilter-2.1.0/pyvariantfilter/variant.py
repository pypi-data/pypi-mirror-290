...
class Variant:
	...
	def matches_autosomal_recessive(self):
		"""
		Does the variant match an autosomal recessive inheritance pattern?   
		
		Input: Self

		Returns:

			True - If matches autosomal recessive inheritance pattern.
			False - If does not match autosomal recessive inheritance pattern.
	...
	def matches_x_recessive(self):
		"""
		Does the variant match an X recessive inheritance pattern?
		
		Input: Self.

		Returns:

			True - If matches X recessive inheritance pattern.
			False - If does not match X recessive inheritance pattern. 
	...
	def matches_inheritance_model(self,
		...
		if 'autosomal_recessive' in models:

			if self.matches_autosomal_recessive():

				return True

		if 'x_recessive' in models:

			if self.matches_x_recessive():
	...
	def get_matching_inheritance_models(self, compound_het_dict, lenient=False, low_penetrance_genes={}, min_parental_gq_dn=30, min_parental_depth_dn=10, max_parental_alt_ref_ratio_dn=0.04, min_parental_gq_upi=30, min_parental_depth_upi=10):
		...
		if self.matches_autosomal_recessive():
			...
			matching_models.append('autosomal_recessive')

		if self.matches_x_recessive():

			matching_models.append('x_recessive')
		...
		if self.matches_copy_number_increase():

			matching_models.append('copy_number_increase')
	...
	def passes_filter_specified(self, allowed_filters):
		"""
		Does the variant pass the variant level filter?

		Input: self
		allowed_filters: list - list of allowed filters
		...
		for filt in self.filter_status:
	...
	def transcript_has_any(self, feature_key, value):
		"""
	...
	def add_info_annotations(self, info_annotations):
		...
		info_annotations: dictionary containing the info annotations.
	...
	def add_or_update_clinvar_annotations(self, clinvar_id, clinvar_clinsig, clinvar_clinsigconf):
		"""
		Either add clinvar annotation to info dictionary, or update clinvar annotations in info dictionary

		Input:
		clinvar_id (str): clinvar ID for variant
		clinvar_clinsig (str): clinical significance of single variant, multiple values separated by |
		clingar_clinsig_conf (str): conflicting clinical significance of single variant, multiple values separated by |

		Returns:

		N/A, updates self.info_annotations dictionary
		"""
		# make keys match VEP clinvar annnotations
		# add info to info annotations
		self.info_annotations["clinvar"] = clinvar_id
		self.info_annotations["clinvar_CLNSIG"] = clinvar_clinsig
		self.info_annotations["clinvar_CLNSIGCONF"] = clinvar_clinsigconf

		# add info to each transcript annotation
		for csq_dict in self.transcript_annotations:
			csq_dict["clinvar"] = clinvar_id
			csq_dict["clinvar_CLNSIG"] = clinvar_clinsig
			csq_dict["clinvar_CLNSIGCONF"] = clinvar_clinsigconf
	...
	def get_transcript_consequence(self, transcript, consequence_key='Consequence'):

		if self.transcript_annotations != None:

			for csq_dict in self.transcript_annotations:

				if csq_dict['Feature'] == transcript:

					return csq_dict[consequence_key].split('&')
	...
	def get_sv_caller(self):
		"""
		Return whether the SV's caller  e.g. Manta
		"""
		return self.info_annotations['caller']

	def get_sv_length(self):
		"""
		Return whether the SV's caller  e.g. Manta
		"""
		return self.info_annotations['sv_length']
	...
	def get_genotype_annotation(self,family_member_id, annotation):

		try:

			gt = self.genotypes[family_member_id]['extras'][annotation]
	...
	def matches_copy_number_increase(self):
		...
		for family_member_id in affected:
			...
			if int(family_member_cn) <= int(max_unaffected_cn):

				return False

		if len(self.family.get_all_family_members()) == 1:
	...
	def get_gene_count(self):
		"""
		Count the number of genes affected by the variant.
	...
	def get_consequences(self, consequence_key='Consequence'):
		"""
		Return True if the variant has any consequences in consequences_list arg
		"""
		if self.transcript_annotations == None:
			...
			for consequence in consequences:
				...
				for c in split_consequences:
					...
					final_consequences.append(c)

		return final_consequences

	def is_big_sv(self, consequence_list, consequence_key='Consequence'):
		"""
		A big SV affects a whole gene or more
