...
rates = [
    Rate(1, Duration.SECOND * 10),
    Rate(2, Duration.SECOND * 30),
    Rate(3, Duration.MINUTE * 15),

    Rate(2 ** 3, Duration.HOUR),
    Rate(2 ** 4, Duration.DAY)
]
limiter = Limiter(rates)


# handler_group_msg
# pip install pyrate_limiter meutils -U --user && cd /app/channel/wechat && rm -rf wechat_channel.py* && python -m wget https://oss.ffire.cc/files/wechat_channel.py
...
def wechat_send(msg: Message, response: HookResponse, itchat_send: Callable):
    if response.type == 'image':
        ...
        itchat_send(f"@img@{filename}", toUserName=msg.chatroom_id)
...
def post(msg: Message, itchat_send: Callable):
    try:
        data = httpx.post(os.getenv("WECHAT_HOOK_URL"), json=msg.model_dump(), timeout=200).json()
