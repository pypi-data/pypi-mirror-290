...
from libpulse.libpulse import (
    ...
    struct_ctypes, Pa_cvolume, Pa_channel_map,

    PA_OPERATION_CANCELLED, PA_OPERATION_DONE, PA_OPERATION_RUNNING,
...
OPERTION_STATES = dict((eval(state), state) for state in
...
def operation_success(result, c_context):
    print(f'result = {OPERTION_STATES[result]!r}')
...
class PactlSubCommands:
    ...
    async def cmd_exit(self, args):
        ...
        lp = self.lib_pulse
        operation = await lp.pa_context_exit_daemon()
        operation_success(operation, lp.c_context)
    ...
    async def cmd_play_sample(self, args):
        ...
        """
        lp = self.lib_pulse
        operation = await lp.pa_context_play_sample(args.name,
    ...
    async def cmd_remove_sample(self, args):
        ...
        lp = self.lib_pulse
        operation = await lp.pa_context_remove_sample(args.name)
        operation_success(operation, lp.c_context)
    ...
    async def cmd_unload_module(self, args):
        ...
        try:
            ...
            for module in await lp.pa_context_get_module_info_list():
                if module.name == name:
                    op = await lp.pa_context_unload_module(module.index)
                    operation_success(op, lp.c_context)
                    count += 1
            if count:
                operation_success(PA_OPERATION_DONE, lp.c_context)
            return
        operation = await lp.pa_context_unload_module(index)
        operation_success(operation, lp.c_context)
    ...
    async def cmd_move_sink_input(self, args):
        ...
        """
        lp = self.lib_pulse
        operation = await lp.pa_context_move_sink_input_by_name(
                                                        args.idx, args.sink)
        operation_success(operation, lp.c_context)

    async def cmd_move_source_output(self, args):
        lp = self.lib_pulse
        operation = await lp.pa_context_move_source_output_by_name(
                                                        args.idx, args.source)
        operation_success(operation, lp.c_context)
    ...
    async def cmd_suspend_sink(self, args):
        ...
        lp = self.lib_pulse
        if args.sink:
            op = await lp.pa_context_suspend_sink_by_name(
                                                        args.sink, args.bool)
        else:
            op = await lp.pa_context_suspend_sink_by_index(
    ...
    async def cmd_suspend_source(self, args):
        lp = self.lib_pulse
        if args.source:
            op = await lp.pa_context_suspend_source_by_name(
                                                    args.source, args.bool)
        else:
            op = await lp.pa_context_suspend_source_by_index(
                                                PA_INVALID_INDEX, args.bool)
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_card_profile(self, args):
        ...
        """
        lp = self.lib_pulse
        op = await lp.pa_context_set_card_profile_by_name(
                                                    args.card, args.profile)
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_default_sink(self, args):
        ...
        """
        lp = self.lib_pulse
        op = await lp.pa_context_set_default_sink(args.sink)
        operation_success(op, lp.c_context)

    async def cmd_set_default_source(self, args):
        lp = self.lib_pulse
        op = await lp.pa_context_set_default_source(args.source)
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_sink_port(self, args):
        ...
        """
        lp = self.lib_pulse
        op = await lp.pa_context_set_sink_port_by_name(args.sink, args.port)
        operation_success(op, lp.c_context)

    async def cmd_set_source_port(self, args):
        lp = self.lib_pulse
        op = await lp.pa_context_set_source_port_by_name(
    ...
    async def cmd_get_source_mute(self, args):
        ...
        source = await lp.pa_context_get_source_info_by_name(args.source)
        if source:
            print(f"Mute: {'yes' if source.mute else 'no'}")
        else:
            self.exit_no_info('source')
    ...
    async def cmd_set_sink_volume(self, args):
        ...
        op = await lp.pa_context_set_sink_volume_by_name(
    ...
    async def cmd_set_source_volume(self, args):
        ...
        op = await lp.pa_context_set_source_volume_by_name(
    ...
    async def cmd_set_sink_input_volume(self, args):
        ...
        op = await lp.pa_context_set_sink_input_volume(
    ...
    async def cmd_set_source_output_volume(self, args):
        ...
        op = await lp.pa_context_set_source_output_volume(
                                                args.idx, c_volume.byref())
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_sink_mute(self, args):
        ...
        if args.mute == 'toggle':
            ...
            op = await lp.pa_context_set_sink_mute_by_name(args.sink, mute)
        else:
            op = await lp.pa_context_set_sink_mute_by_name(args.sink,
                                                           args.mute)
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_source_mute(self, args):
        ...
        if args.mute == 'toggle':
            ...
            op = await lp.pa_context_set_source_mute_by_name(args.source, mute)
        else:
            op = await lp.pa_context_set_source_mute_by_name(args.source,
                                                                args.mute)
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_sink_input_mute(self, args):
        ...
        if args.mute == 'toggle':
            ...
            op = await lp.pa_context_set_sink_input_mute(args.idx, mute)
        else:
            op = await lp.pa_context_set_sink_input_mute(args.idx, args.mute)
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_source_output_mute(self, args):
        ...
        if args.mute == 'toggle':
            ...
            op = await lp.pa_context_set_source_output_mute(args.idx, mute)
        else:
            op = await lp.pa_context_set_source_output_mute(args.idx,
                                                            args.mute)
        operation_success(op, lp.c_context)
    ...
    async def cmd_set_sink_formats(self, args):
        ...
        try:
            ...
            op = await lp.pa_ext_device_restore_save_formats(
