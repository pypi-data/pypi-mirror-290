...
def _run(
    *,
    ...
    # Do the magic
    try:
        # Remove repeated repo trims if they exist
        exclude = tuple(set(exclude))
        keep = tuple(set(keep))
        bleeding_edge = tuple(set(bleeding_edge))
...
@click.option(
    ...
    type=RepoType(),
    default=(),
    help="Exclude specific repo(s) by the repo url trim - use '*' as a wildcard",
)
@click.option(
    "-k",
    "--keep",
    ...
    type=RepoType(),
    default=(),
    help="Keep the version of specific repo(s) by the repo url trim (still checks for the new versions) - use '*' as a wildcard",
)
@click.option(
    "-b",
    "--bleeding-edge",
    ...
    type=RepoType(),
    default=(),
    help="Get the latest version or commit of specific repo(s) by the repo url trim - use '*' as a wildcard",
)
@click.pass_context
