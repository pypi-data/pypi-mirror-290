...
class File(io.IOBase):
    __slots__ = ("__source__", "__source_scheme__", "__source_name__", "__fileobj__", "__fetcher__")
    ...
    def __init__(
        ...
        object.__setattr__(self, "__source_name__", source_name)
    ...
    def __getattr__(self, name: str) -> Any:
        if name in ("__source__", "__source_scheme__", "__source_name__", "__fileobj__", "__fetcher__"):
    ...
    @classmethod 
    def validate(cls, value: ValidatorFunctionWrapHandler, info: Optional[Union[Dict, ValidationInfo]] = None) -> "File":
        ...
        if parsed_url.scheme == "data":
            if cls.__max_file_size__ is not None:
                ...
                if estimated_size > cls.__max_file_size__:
                    raise ValueError(f"File size exceeds the maximum allowed size of {cls.__max_file_size__} bytes.")
            # ? File type restrictions
            mimetype = value.split(":", 1)[1].split(";", 1)[0]
            extension = mimetypes.guess_extension(mimetype)
            if extension is None:
                extension = f".{mimetype.split('/', 1)[1]}"
        ...
        if parsed_url.scheme == "http" or parsed_url.scheme == "https":
            ...
            return File(
                value, 
                source_scheme="url", 
                source_name=parsed_url.path.split("/")[-1],
    ...
    @classmethod
    def serialize(cls, value: Any, info: Optional[Union[Dict, SerializationInfo]] = None) -> str:
        ...
        if s3_key is None:
            ...
            if value.__source_name__ is not None:
                s3_key = f"{s3_path}/{value.__source_name__}"
