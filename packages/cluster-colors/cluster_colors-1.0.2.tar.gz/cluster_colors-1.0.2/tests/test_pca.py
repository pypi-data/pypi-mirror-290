...
from cluster_colors import clusters
import numpy as np
import math
...
class TestPCA:
    def test_linear(self):
        ...
        cluster_ = clusters.Cluster.from_stacked_vectors(line)
        x, y = cluster_._direction_of_highest_variance
    ...
    def test_square(self):
        ...
        cluster_ = clusters.Cluster.from_stacked_vectors(square)
        x, y = cluster_._direction_of_highest_variance
        assert x == 0 or y == 0
        assert x == 1 or y == 1
    ...
    def test_wide_rect(self):
        ...
        cluster_ = clusters.Cluster.from_stacked_vectors(wide_rect)
        np.testing.assert_allclose(cluster_._direction_of_highest_variance, (1, 0))

    def test_tall_rect(self):
        """When all points form a tall rectangle, axis points up."""
        tall_rect = np.array([[1, 1, 1], [2, 1, 1], [2, 5, 1], [1, 5, 1]])
        cluster_ = clusters.Cluster.from_stacked_vectors(tall_rect)
