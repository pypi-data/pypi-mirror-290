...
def match_semver_regex(version_str: str) -> Optional[packaging.version.Version]:
    ...
    Returns:
        Version:
            A version object that describes the version string passed in,
            or None if the string is not in the correct format.
    ...
    Example:
        .. code-block:: python
            ...
            # version yields Version('2.7.1')

    """
    comparable = re.match(SEMVER_REGEX, version_str)
    if comparable:
        return packaging.version.parse(comparable.group())
    return None


def is_dev_version(version_str: str) -> bool:
    """Takes in a version string and returns True if that version string is a dev version.

    Example:
    2.7.1 is not a dev version
    2.7.1rc.0+1.g51dbcd6b8e is a dev version
    """
    try:
...
def client_ahead_of_server_message(
    client_version: packaging.version.Version, server_version: packaging.version.Version
) -> str:
...
class Client:
    ...
    def _check_version(self) -> str:
        ...
        try:
            ...
            if server_version is not None and client_version is not None:
                server_is_dev_version = is_dev_version(server_version_str)
    ...
    def start_file_scan(
        ...
        api = swagger_client.ModelTestingApi(self._api_client)
        with RESTErrorHandler():
            response = api.start_file_scan(body=req)
    ...
    def get_file_scan_result(
        ...
        try:
            response = api.get_file_scan_result(
        ...
        while True:
            with RESTErrorHandler():
                if page_token == "":
                    ...
                    res = api.list_file_scan_results(
                        **params,
                    )
                else:
                    res = api.list_file_scan_results(
    ...
    def delete_file_scan_result(
        ...
        api = swagger_client.FileScanningApi(self._api_client)
        try:
            api.delete_file_scan_result(file_scan_id_uuid=file_scan_id)
