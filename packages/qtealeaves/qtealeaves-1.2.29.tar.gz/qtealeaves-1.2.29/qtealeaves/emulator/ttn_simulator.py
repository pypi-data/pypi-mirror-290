...
from qtealeaves.abstracttns import _AbstractTN, _projector_for_rho_i
...
class TTNLayer(list):
    ...
    def from_tensorlist(
        ...
        device: str, optional
            Device where the computations will take place
    ...
    def initialize_layer(
        ...
        if self.is_ttn_toplayer and self.layer_idx == 0:
            ...
            if tensor.shape[-1] != 1:
                raise Exception("TTN is not a pure state.")
    ...
    def dot(self, other):
        ...
        for ii, ket in enumerate(self):
            ...
            ket.convert(dtype=bra.dtype, device=bra.device)
...
class TTN(_AbstractTN):
        ...
        if is_root:
            ...
            for layer in tensor_list:
                ...
                for elem in layer:
                    ...
                    _ = elem.mpi_bcast(
                        elem, comm, tensor_backend.dtype, cls.tn_mpi_types(), root=root
                    )
            ...
            for _ in range(num_layers):
                ...
                for _ in range(num_tensors):
                    elem = tensor_cls.mpi_bcast(
                        None, comm, tensor_backend.dtype, cls.tn_mpi_types(), root=root
                    )
    ...
    def pre_timeevo_checks(self, raise_error=False):
        ...
        if is_product_state and raise_error:
            raise Exception("TTN time evolution with product state will fail.")
    ...
    def get_rho_i(self, idx):
        ...
        if list(self.iso_center) != [layer_idx, tensor_idx]:
            self.move_pos(
                (layer_idx, tensor_idx),
        ...
        if abs(1 - trace) > 10 * rho.dtype_eps:
            logger.warning("Renormalizing reduced density matrix.")
            rho /= trace
    ...
    def iso_towards(
        ...
        for elem in complete_path:
            ...
            src, dst, singvals_cut, singvals = self.shift_iso_to(
                src, dst, src_link, dst_link, trunc=trunc_shift, conv_params=conv_params
            )
    ...
    def _initialize_tree(self, method="empty", isometry=True, sectors={}):
        ...
        if method == "vacuum":
            matrix = np.zeros(
                (self.num_sites, self.local_dim[0]), dtype=self._tensor_backend.dtype
            )
            matrix[:, 0] = 1
            ...
            for idx in range(self.num_layers)[::-1]:
                links = [
                    ...
                    for l_child, r_child in zip(links[0::2], links[1::2])
                ]
    ...
    def product_state_from_local_states(
        ...
        prod_ttn : :py:class:`TTN`
            Corresponding product state TTN.
        """
        for ii in range(0, mat.shape[0]):
            mat_tmp = tensor_backend.tensor_cls.from_elem_array(mat[ii])
            norm = mat_tmp.norm()
            if abs(norm - 1) > 10e-5:
                raise ValueError(
        ...
        for ii in range(num_sites_tree // 2):
            ...
            if i1 < num_sites:
                ...
                mat1 = tensor_backend.tensor_cls.from_elem_array(mat[i1])
            else:
                local_dim1 = 1
                mat1 = tensor_backend([1, 1], ctrl="O").reshape([1, 1])
            if i2 < num_sites:
                local_dim2 = local_dim[i2]
                mat2 = tensor_backend.tensor_cls.from_elem_array(mat[i2])
            else:
                local_dim2 = 1
    ...
    def _set_cache(self, key, value):
        ...
        if key in self._cache_get_children_prob:
            raise Exception("Cannot overwrite tensor: " + str(key))
    ...
    def _get_cache(self, key):
        ...
        # Key exists ...
        if key[2] is None:
            raise Exception("State not provided correctly")
    ...
    def set_cache_clearing_strategy_sampling(self, strategy):
        ...
        **Arguments**
        ...
        if strategy not in ["state", "num_qubits"]:
            raise Exception("Unknown cache clearing strategy %s." % (strategy))
    ...
    def _get_children_prob(self, tensor, site_idx, curr_state, do_clear_cache):
        ...
        norm_err = abs(1 - probabilities.sum())
    ...
    def build_effective_operators(self, measurement_mode=False):
        ...
        if list(self.iso_center) != self.default_iso_pos:
            self.isometrize_all()
        ...
        for layer in self.layers[::-1]:
            ...
            self.move_pos(
                (lidx, len(layer) - 1), device=self._tensor_backend.computational_device
            )

            for tidx in range(len(layer) - 1, -1, -1):
                if self.is_masked_pos((lidx, tidx)):
                    continue
    ...
    def meas_bond_entropy(self):
        ...
        if center_svals is None:
            if self.iso_center is None:
                raise Exception("There is no iso-center when trying to measure.")
        ...
        for layer_idx, layer in enumerate(self):
            for idx, singvals in enumerate(layer.singvals):
                ...
                if singvals is None:
                    ...
                    singvals = np.array(singvals.flatten())
