    ...
    **Arguments**
    ...
    def __init__(self, set_names="default", mapping_func=None):
        ...
        if mapping_func is None:
            ...
            self._mapping_func = default_mapping
        else:
    ...
    def keys(self):
        ...
        if not self.one_unique:
            raise Exception("Only works for one unique operator set.")
        default_key = self.set_names[0]
    ...
    def write_operator(
        ...
        **Arguments**
        ...
        """
        if not self.one_unique:
            raise Exception("Fortran does not support different Hilbert spaces.")
