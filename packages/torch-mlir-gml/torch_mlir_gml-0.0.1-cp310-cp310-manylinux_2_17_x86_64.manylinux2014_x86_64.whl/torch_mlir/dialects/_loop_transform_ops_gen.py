...
def apply_patterns_scf_for_loop_canonicalization(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyForLoopCanonicalizationPatternsOp(loc=loc, ip=ip))
...
def apply_conversion_patterns_scf_structural_conversions(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplySCFStructuralConversionPatternsOp(loc=loc, ip=ip))
...
def loop_hoist_loop_invariant_subsets(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(HoistLoopInvariantSubsetsOp(target=target, loc=loc, ip=ip))
...
def loop_coalesce(transformed, target, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(LoopCoalesceOp(transformed=transformed, target=target, loc=loc, ip=ip))
...
def loop_fuse_sibling(fused_loop, target, source, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(LoopFuseSiblingOp(fused_loop=fused_loop, target=target, source=source, loc=loc, ip=ip))
...
def loop_outline(function, call, target, func_name, *, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return _get_op_result_or_op_results(LoopOutlineOp(function=function, call=call, target=target, func_name=func_name, loc=loc, ip=ip))
...
def loop_peel(peeled_loop, remainder_loop, target, *, peel_front=None, fail_if_already_divisible=None, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return _get_op_result_or_op_results(LoopPeelOp(peeled_loop=peeled_loop, remainder_loop=remainder_loop, target=target, peel_front=peel_front, fail_if_already_divisible=fail_if_already_divisible, loc=loc, ip=ip))
...
def loop_pipeline(transformed, target, *, iteration_interval=None, read_latency=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(LoopPipelineOp(transformed=transformed, target=target, iteration_interval=iteration_interval, read_latency=read_latency, loc=loc, ip=ip))
...
def loop_promote_if_one_iteration(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(LoopPromoteIfOneIterationOp(target=target, loc=loc, ip=ip))
...
def loop_unroll_and_jam(target, factor, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(LoopUnrollAndJamOp(target=target, factor=factor, loc=loc, ip=ip))
...
def loop_unroll(target, factor, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(LoopUnrollOp(target=target, factor=factor, loc=loc, ip=ip))
...
def scf_take_assumed_branch(target, *, take_else_branch=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(TakeAssumedBranchOp(target=target, take_else_branch=take_else_branch, loc=loc, ip=ip))
