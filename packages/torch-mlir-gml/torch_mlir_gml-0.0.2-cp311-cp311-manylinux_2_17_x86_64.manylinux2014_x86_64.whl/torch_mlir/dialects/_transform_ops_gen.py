...
def annotate(target, name, *, param=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(AnnotateOp(target=target, name=name, param=param, loc=loc, ip=ip))
...
def apply_patterns_canonicalization(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyCanonicalizationPatternsOp(loc=loc, ip=ip))
...
def apply_cse(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyCommonSubexpressionEliminationOp(target=target, loc=loc, ip=ip))
...
def apply_dce(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyDeadCodeEliminationOp(target=target, loc=loc, ip=ip))
...
def apply_licm(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyLoopInvariantCodeMotionOp(target=target, loc=loc, ip=ip))
...
def apply_patterns(target, *, apply_cse=None, max_iterations=None, max_num_rewrites=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyPatternsOp(target=target, apply_cse=apply_cse, max_iterations=max_iterations, max_num_rewrites=max_num_rewrites, loc=loc, ip=ip))
...
def apply_registered_pass(result, target, pass_name, *, options=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(ApplyRegisteredPassOp(result=result, target=target, pass_name=pass_name, options=options, loc=loc, ip=ip))
...
def apply_conversion_patterns_dialect_to_llvm(dialect_name, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyToLLVMConversionPatternsOp(dialect_name=dialect_name, loc=loc, ip=ip))
...
class ForeachOp(_ods_ir.OpView):
  ...
  def __init__(self, results_, targets, *, zip_shortest=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    _ods_context = _ods_get_default_loc_context(loc)
    if bool(zip_shortest): attributes["zip_shortest"] = _ods_ir.UnitAttr.get(
      _ods_get_default_loc_context(loc))
    results.extend(results_)
    _ods_successors = None
    super().__init__(self.build_generic(attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip))
  ...
  @builtins.property
  def zip_shortest(self):
    return "zip_shortest" in self.operation.attributes

  @zip_shortest.setter
  def zip_shortest(self, value):
    if bool(value):
      self.operation.attributes["zip_shortest"] = _ods_ir.UnitAttr.get()
    elif "zip_shortest" in self.operation.attributes:
      del self.operation.attributes["zip_shortest"]

  @zip_shortest.deleter
  def zip_shortest(self):
    del self.operation.attributes["zip_shortest"]
...
def foreach(results_, targets, *, zip_shortest=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(ForeachOp(results_=results_, targets=targets, zip_shortest=zip_shortest, loc=loc, ip=ip))
...
def get_consumers_of_result(consumers, target, result_number, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(GetConsumersOfResult(consumers=consumers, target=target, result_number=result_number, loc=loc, ip=ip))
...
def get_defining_op(result, target, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(GetDefiningOp(result=result, target=target, loc=loc, ip=ip))
...
def get_operand(result, target, raw_position_list, *, is_inverted=None, is_all=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(GetOperandOp(result=result, target=target, raw_position_list=raw_position_list, is_inverted=is_inverted, is_all=is_all, loc=loc, ip=ip))
...
def get_parent_op(parent, target, *, isolated_from_above=None, allow_empty_results=None, op_name=None, deduplicate=None, nth_parent=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(GetParentOp(parent=parent, target=target, isolated_from_above=isolated_from_above, allow_empty_results=allow_empty_results, op_name=op_name, deduplicate=deduplicate, nth_parent=nth_parent, loc=loc, ip=ip))
...
def get_producer_of_operand(producer, target, operand_number, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(GetProducerOfOperand(producer=producer, target=target, operand_number=operand_number, loc=loc, ip=ip))
...
def get_result(result, target, raw_position_list, *, is_inverted=None, is_all=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(GetResultOp(result=result, target=target, raw_position_list=raw_position_list, is_inverted=is_inverted, is_all=is_all, loc=loc, ip=ip))
...
def get_type(type_param, value, *, elemental=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(GetTypeOp(type_param=type_param, value=value, elemental=elemental, loc=loc, ip=ip))
...
def include(results_, target, failure_propagation_mode, operands_, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(IncludeOp(results_=results_, target=target, failure_propagation_mode=failure_propagation_mode, operands_=operands_, loc=loc, ip=ip))
...
def match_operation_empty(operand_handle, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(MatchOperationEmptyOp(operand_handle=operand_handle, loc=loc, ip=ip))
...
def match_operation_name(operand_handle, op_names, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(MatchOperationNameOp(operand_handle=operand_handle, op_names=op_names, loc=loc, ip=ip))
...
def match_param_cmpi(param, reference, predicate, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(MatchParamCmpIOp(param=param, reference=reference, predicate=predicate, loc=loc, ip=ip))
...
def merge_handles(handles, *, deduplicate=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(MergeHandlesOp(handles=handles, deduplicate=deduplicate, loc=loc, ip=ip))
...
class NamedSequenceOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_470')) else
      _ods_ir.AttrBuilder.get('anonymous_470')(function_type, context=_ods_context))
...
def named_sequence(sym_name, function_type, *, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(NamedSequenceOp(sym_name=sym_name, function_type=function_type, sym_visibility=sym_visibility, arg_attrs=arg_attrs, res_attrs=res_attrs, loc=loc, ip=ip))
...
def num_associations(num, handle, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(NumAssociationsOp(num=num, handle=handle, loc=loc, ip=ip))
...
def param_constant(param, value, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(ParamConstantOp(param=param, value=value, loc=loc, ip=ip))
...
def print_(*, target=None, name=None, assume_verified=None, use_local_scope=None, skip_regions=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(PrintOp(target=target, name=name, assume_verified=assume_verified, use_local_scope=use_local_scope, skip_regions=skip_regions, loc=loc, ip=ip))
...
def select(result, target, op_name, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(SelectOp(result=result, target=target, op_name=op_name, loc=loc, ip=ip))
...
def verify_(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(VerifyOp(target=target, loc=loc, ip=ip))
