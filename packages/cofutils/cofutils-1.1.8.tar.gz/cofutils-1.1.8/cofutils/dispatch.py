...
def auto_exec():
    if GLOBAL_ARGS.test:
        ...
        return
    if not os.path.exists(GLOBAL_ARGS.coflog_path):
        os.mkdir(GLOBAL_ARGS.coflog_path)
    if GLOBAL_ARGS.output_dir is not None:
        ...
        for filename in os.listdir(GLOBAL_ARGS.coflog_path):
            # Construct full file path
            file_path = os.path.join(GLOBAL_ARGS.coflog_path, filename)
...
def auto_gen():
    ...
    nsys_cmd = f'pkill nsys; nsys start --stop-on-exit=false --stats=true -c cudaProfilerApi -f true -o {GLOBAL_ARGS.coflog_path}/target ;nsys launch --wait=primary --trace=cuda,nvtx '
    ...
    bash /tmp/{GLOBAL_ARGS.target_file_name}.sh \n\
second_return_code=$?\n\
...
def task_setup():
    global GLOBAL_ARGS
    GLOBAL_ARGS.coflog_path='/tmp/coflog/'
