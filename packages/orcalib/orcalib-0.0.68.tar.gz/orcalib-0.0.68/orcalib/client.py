...
class OrcaClient:
    ...
    @staticmethod
    def check_version_compatibility() -> bool | None:
        """
        ...
        if OrcaClient.SERVER_VERSION == "0.0.0":
            logger.info(f"OrcaDB Instance version is '{OrcaClient.SERVER_VERSION}'. Skipping version check.")
        elif OrcaClient.CLIENT_VERSION == "0.0.0":
            logger.info(f"OrcaLib version is '{OrcaClient.CLIENT_VERSION}'. Skipping version check.")
        elif OrcaClient.SERVER_VERSION != OrcaClient.CLIENT_VERSION:
            logger.warning(
                f"OrcaLib version {OrcaClient.CLIENT_VERSION} does not match OrcaDB instance version {OrcaClient.SERVER_VERSION}. Please ensure you use version {OrcaClient.SERVER_VERSION} of OrcaLib."
            )
            return False
        else:
            return True
