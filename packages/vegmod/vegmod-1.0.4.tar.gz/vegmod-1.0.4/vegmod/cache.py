...
class Cache:
    def __init__(self, file_path: str):
        ...
        self.cache = {k: v for k, v in dirty_cache.items() if time.time() - v["_cached_at"] < 3600}
    ...
    def manage(self):
        ...
        for k, v in self.items():
            if time.time() - v["_cached_at"] > 3600:
                del self[k]
                entries_removed += 1
        cache_size = len(self)
        logger.info(f"Cache Management: removed={entries_removed} cache_size={cache_size}")
