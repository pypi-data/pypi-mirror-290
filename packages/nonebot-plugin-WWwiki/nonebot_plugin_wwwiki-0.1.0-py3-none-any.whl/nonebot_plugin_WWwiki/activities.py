...
alc = Alconna("鸣潮活动提醒", Option("-o|--开启"), Option("-c|--关闭"), Option("-a|--添加群聊", Args["group_id?", str]), Option("-r|--移除群聊", Args["group_id?", str]))
...
# @reminder.assign("a")
# async def add(target: MsgTarget, group_id: Optional[str] = None):
#     async with aio_open(group_data, 'r', encoding='utf-8') as f:
#         groupid = await f.read()
#         default_config = {"opened_groups": []}
...
#                 await reminder.finish("开启成功")
#         else:
#             await reminder.finish("请输入群号")
#     else:
#         return


# @reminder.assign("r")
# async def remove(target: MsgTarget, group_id: Optional[str] = None):
#     async with aio_open(group_data, 'r', encoding='utf-8') as f:
#         groupid = await f.read()
#         default_config = {"opened_groups": []}
