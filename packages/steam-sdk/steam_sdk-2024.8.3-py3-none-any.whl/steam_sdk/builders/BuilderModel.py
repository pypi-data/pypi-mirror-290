...
class BuilderModel:
    ...
    def buildTFM(self, output_path: str,  local_library_path: str,  TFM_inputs = None, magnet_data = None, verbose: bool = True):
        """
            Building a TFM model
        """
        builder_ledet = BuilderLEDET(path_input_file=self.path_input_file, input_model_data=self.model_data,
                                     input_map2d=self.path_map2d, flag_build=True, input_roxie_data=self.roxie_data, smic_write_path='skip', verbose=verbose)
        self.builderTFM = BuilderTFM(builder_LEDET=builder_ledet, output_path=output_path, local_library_path=local_library_path,
                                    TFM_inputs=TFM_inputs, magnet_data=magnet_data, verbose= verbose)
        ...
        if self.circuit_data.TFM != None:
            if not self.circuit_data.TFM.skip_TFM:
                ...
                for key, value in TFM.__dict__.items():
                    if key == 'magnets_TFM' or value is None: continue
                    if key == 'temperature':
                        if value is not None:
                            T = value
                        else:
                ...
                for key, m_TFM in magnets_TFM.items():
                    ...
                    BM_TFM.buildTFM(output_path=output_path_TFM, local_library_path=local_library_path, TFM_inputs=TFM_general, magnet_data=m_TFM, verbose=verbose)
                    ...
                    # Add the corresponding C_ground of each magnet to the global parameters in the format 'C_ground_x_Magnet_1' = value
                    # for magn_attr, magn_value in m_TFM.__dict__.items():
                    #     if magn_value is None : continue
                    #     # Add for each magnet the corresponding C_ground to the netlist parameters in the format 'C_ground' = 'C_ground_x_Magnet_1'
                    #     self.circuit_data.GlobalParameters.global_parameters[f'{magn_attr}_{key}'] = magn_value
                    #     self.circuit_data.Netlist[key].parameters[f'{magn_attr}'] = f'{magn_attr}_{key}'
