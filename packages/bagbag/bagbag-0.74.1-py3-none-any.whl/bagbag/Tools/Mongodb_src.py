...
class MongoDB():
    ...
    def _conn(self) -> pymongo.MongoClient:
        """
        This function establishes a connection to a MongoDB database using the provided credentials and
        configuration.
        :return: The `_conn` method returns a `pymongo.MongoClient` object from the `self.conns`
        dictionary based on the `_id` of the current object. If the MongoClient object for the given
        `_id` does not exist in the `self.conns` dictionary, a new MongoClient object is created based
    ...
    def _id(self) -> str:
        ...
        current process.
        :return: The `_id` method is returning a string that concatenates the name of the current thread
    ...
    def _query(self, clean:bool=False):
        """
        The function `_query` initializes a dictionary in the `query` attribute of an object based on a
        given ID if it does not already exist or if the `clean` parameter is set to `True`.
        
        :param clean: The `clean` parameter in the `_query` method is a boolean parameter with a default
        ...
        :return: The function `_query` is returning the query dictionary associated with the `mid` key.
    ...
    def Database(self, name:str) -> MongoDB:
        """
    ...
    def Collection(self, name:str) -> MongoDB:
        """
        The function `Collection` sets the collection name for a MongoDB object and returns the object
        itself.
        
        :param name: The `name` parameter in the `Collection` method is a string that represents the
        name of the MongoDB collection that you want to work with
        :type name: str
        :return: The method `Collection` is returning the instance of the class itself (`self`) after
        setting the `collection` attribute to the provided `name` parameter.
        """
        self.collection = name 
        return self

    def Where(self, key:str, opera:str, value:str) -> MongoDB:
        """
        ...
        if opera == "=":
            self._query()['opera'][key] = value
        elif opera == '>':
            self._query()['opera'][key] = {"$gt": value}
        elif opera == '>=' or opera == '=>':
            self._query()['opera'][key] = {"$gte": value}
        elif opera == '<':
            self._query()['opera'][key] = {"$lt": value}
        elif opera == '<=' or opera == '=<':
            self._query()['opera'][key] = {"$lte": value}

        return self
    
    def OrderBy(self, key:str, order:str="asc") -> MongoDB:
        """
        ...
        """
        if order == "asc":
            self._query()['order'] = [key, pymongo.ASCENDING]
        elif order == "desc":
            self._query()['order'] = [key, pymongo.DESCENDING]
        else:
    ...
    def First(self) -> dict | None:
        ...
        """
        co = self._conn()[self.database][self.collection]
        query = self._query()
        self._query(clean=True)

        co = co.find(query['opera'])
    ...
    def Limit(self, number:int) -> MongoDB:
        """
        ...
        method chaining.
        """
        self._query()['limit'] = number 

        return self
    
    def Data(self, data:dict|list) -> MongoDB:
        """
        ...
        method checks the type of `data` and if it is a dictionary, it converts the `_id` field to an
    ...
    def Insert(self) -> str | list:
        ...
        if type(self.data[mid]) == dict:
            co = self._conn()[self.database][self.collection]
            oid = str(co.insert_one(self.data[mid]))
            del(self.data[mid])
            return oid 
        elif type(self.data[mid]) == list:
            co = self._conn()[self.database][self.collection]
            oids = co.insert_many(self.data[mid])
            del(self.data[mid])
    ...
    def Update(self):
        ...
        """
        co = self._conn()[self.database][self.collection]
        opera = self._query()['opera']
        self._query(clean=True)

        mid = self._id()

        co.update_many(opera, {'$set': self.data[mid]})
    ...
    def Delete(self) -> int:
        ...
        return co.delete_many(opera).deleted_count
