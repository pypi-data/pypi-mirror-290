...
class _pixstats_utils:
    ...
    def initialise_pixstats(self, mode="tracks"):
        ...
        channel_names = pixstats_data["channel"].unique()
        channels = [chan.lower() for chan in channel_names]
        ...
        if calculate_fret:
            if set(["donor", "acceptor"]).issubset(channels):
                ...
                show_info(f"FRET calculation requires Donor+Acceptor or DD+DA channels")
                show_info(f"Found channels: {channels}")
                show_info("Copy the tracks to other channels in the Tracks Edit tab")
        ...
        for metric in data.columns:
            try:
                if metric in self.moltrack_metrics.values():
                    ...
                    donor_data = donor_data - donor_bg
                    acceptor_data = acceptor_data - acceptor_bg
