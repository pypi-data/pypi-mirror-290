from DashAI.back.core.schema_fields import BaseSchema, schema_field, int_field, float_field, list_field
...
class MLPImageClassifierSchema(BaseSchema):
    ...
    hidden_dims: schema_field(
        list_field(item_type=int_field(gt=0), min_items=1),
        placeholder=[128],
        description=(
            "Hidden dims (one for each inner layer)"
        ),
    )  # type: ignore
...
class MLPImageClassifier(BaseModel, ConfigObject):
    ...
    def predict(self, x_pred) -> list:
        ...
        """
        image_dataset = ImageDataset(x_pred)
