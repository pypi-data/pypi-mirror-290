...
def random_dataloader(dataset: Dataset, max_steps: int, batch_size: int, collate_fn=None):
    sampler = RandomSampler(dataset, max_steps=max_steps, batch_size=batch_size)
    dataloader = DataLoader(dataset, batch_size=None, sampler=sampler, collate_fn=collate_fn)
    return dataloader
