...
class KytosEline(Backend):
    ...
    def get_link_name(self, span):
        ...
        for link in self.get_links():
            if (
                link["endpoint_a"] == span["endpoint_a"]
                and link["endpoint_b"] == span["endpoint_b"]
            ) or (
                link["endpoint_a"] == span["endpoint_b"]
                and link["endpoint_b"] == span["endpoint_a"]
            ):
    ...
    def get_current_config(self, kytos_evc, evc):
        ...
        if int(datetime.datetime.utcnow().timestamp()) > evc.decommissioning_time:
