...
from ..core import get_default_logger, snake_to_camel
...
class HLClient(object):
    ...
    def execute(self, request_str: str, variable_values=None) -> Dict[str, dict]:
        ...
        for i in range(self.EXECUTION_RETRYS):
            try:
                ...
                LOGGER.info(f"ClientConnectionError: {e} -- " f"Retrying {i+1}/{self.EXECUTION_RETRYS}")
                error = e
                time.sleep(1)
    ...
    def _execute(self, request_str: str, variable_values=None):
        LOGGER.debug(f"_execute: {request_str}")
