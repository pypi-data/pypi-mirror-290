...
from ..core import DEPRECATED_CAPABILITY_IMPLEMENTATION_FILE, HLBaseModel
...
def create_artefact(ctx, id, artefact_yaml):
    ...
    class _TrainingRunArtefactType(HLBaseModel):
        id: str

    class CreateTrainingRunPayload(HLBaseModel):
        errors: List[str]
    ...
    artefact_result = client.createTrainingRunArtefact(
        ...
        inferenceConfig=artefact.inference_config,
        trainingConfig=artefact.training_config,
    )
    ...
    create_capability_for_artefact(
        ...
        training_run_id=id,
        inference_config=artefact.inference_config,
    )
...
def create_metrics_result(
    ...
    class CreateExperimentResultPayload(HLBaseModel):
        errors: List[str]
    ...
    result = client.createExperimentResult(
        ...
        researchPlanMetricId=metric.research_plan_metric_id,
        result=metric.result,
        occuredAt=metric.occured_at.isoformat(),
    ).gql_dict()
