...
@click.option(
    ...
    required=False,
    default=None,
    help="File to dump the resulting assessments to (yaml format)",
)
@click.pass_context
def create(
    ctx,
...
def create_from_dataset(
    ...
    class Image(HLBaseModel):
        id: int
        original_source_url: str

    class ImageConnection(HLBaseModel):
        page_info: PageInfo
        nodes: List[Image]
...
@click.pass_context
def create_from_avro(ctx, avro_file, file_id, workflow_id):
    client = ctx.obj["client"]
    # Check Avro file against schema
    with open(avro_file, "rb") as f:
        list(avro.reader(f, reader_schema=ENTITY_AVRO_SCHEMA))
    file_info = upload_file_to_s3(client, avro_file, mimetype="application/octet-stream")
...
@click.pass_context
def read(ctx, query, yaml_file, output, count, fields):
    client = ctx.obj["client"]
