...
from ..client import CaseType, TaskStatusEnum, TaskType, TrainingRunType
from ..core import HLBaseModel


class UpdateTaskPayload(HLBaseModel):
    errors: List[str]
...
def create(ctx, workflow_order_id, file_ids, output):
    ...
    class AddFilesToProjectOrderPayload(HLBaseModel):
        errors: List[str]
        tasks: List[TaskType]
...
def lease_from_steps(
    ...
    class LeaseTaskPayload(HLBaseModel):
        errors: List[str]
...
def delete(ctx, id, output):
    ...
    class DeleteTaskPayload(HLBaseModel):
        errors: List[str]
        task: Optional[TaskType] = None
