...
def install(package_name, options, executable, quiet_install=True):
    ...
    if quiet_install:
        subprocess.run([executable, "-m"] + install_param + pip_install_common_options, check=True, cwd=PACKAGE_ROOT)
    else:
        subprocess.run([executable, "-m"] + install_param, check=True, cwd=PACKAGE_ROOT)
