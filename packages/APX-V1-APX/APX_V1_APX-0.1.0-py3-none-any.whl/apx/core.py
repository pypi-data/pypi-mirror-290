...
def status(cluster_names: Optional[Union[str, List[str]]] = None,
    ...
    .. code-block:: python

        {
            ...
            'last_use': (str) the last command/entrypoint that affected this
              cluster,
            'status': (apx.ClusterStatus) cluster status,
    ...
    - ``INIT``: The cluster may be live or down. It can happen in the following
      ...
      - Ongoing provisioning or runtime setup. (A ``apx.launch()`` has started
        but has not completed.)
      - Or, the cluster is in an abnormal state, e.g., some cluster nodes are
        down, or the APX runtime is unhealthy. (To recover the cluster,
        try ``apx launch`` again on it.)

    - ``UP``: Provisioning and runtime setup have succeeded and the cluster is
      live.  (The most recent ``apx.launch()`` has completed successfully.)

    - ``STOPPED``: The cluster is stopped and the storage is persisted. Use
      ``apx.start()`` to restart the cluster.
    ...
    - In normal cases where clusters are entirely managed by APX (i.e., no
    ...
    - In cases where the clusters are changed outside of APX (e.g., manual
...
def _start(
    ...
    if not force and cluster_status == status_lib.ClusterStatus.UP:
        apx_logging.print(f'Cluster {cluster_name!r} is already up.')
        return handle
...
def start(
    ...
    Args:
        ...
        idle_minutes_to_autostop: automatically stop the cluster after this
            ...
            ``apx.autostop(idle_minutes=<minutes>)``. If not set, the
        ...
        force: whether to force start the cluster even if it is already up.
            Useful for upgrading APX runtime.
    ...
    Raises:
        ValueError: argument values are invalid: (1) the specified cluster does
          ...
          autostop settings).
        apx.exceptions.NotSupportedError: if the cluster to restart was
          launched using a non-default backend that does not support this
          operation.
        apx.exceptions.ClusterOwnerIdentitiesMismatchError: if the cluster to
...
def stop(cluster_name: str, purge: bool = False) -> None:
    ...
    Args:
        ...
        purge: (Advanced) Forcefully mark the cluster as stopped in APX's
    ...
    Raises:
        ...
        apx.exceptions.NotSupportedError: if the specified cluster is a spot
          cluster, or a TPU VM Pod cluster, or the managed jobs controller.
    """
    if controller_utils.Controllers.from_name(cluster_name) is not None:
        raise exceptions.NotSupportedError(
            f'Stopping APX controller {cluster_name!r} '
            f'is not supported.')
    ...
    if isinstance(backend, backends.CloudVmRayBackend):
        ...
        try:
            ...
            raise exceptions.NotSupportedError(
                ...
                f'{colorama.Style.BRIGHT}apx down {cluster_name}') from e
...
def down(cluster_name: str, purge: bool = False) -> None:
    ...
    Args:
        ...
        purge: (Advanced) Forcefully remove the cluster from APX's cluster
    ...
    Raises:
        ...
        apx.exceptions.NotSupportedError: the specified cluster is the managed
          jobs controller.
    """
    handle = global_user_state.get_handle_from_cluster_name(cluster_name)
    if handle is None:
...
def autostop(
    ...
    - A job is submitted (``apx.launch()`` or ``apx.exec()``).
    ...
    Raises:
        ...
        apx.exceptions.ClusterNotUpError: if the cluster is not UP.
        apx.exceptions.NotSupportedError: if the cluster is not based on
          CloudVmRayBackend or the cluster is TPU VM Pod.
        apx.exceptions.ClusterOwnerIdentityMismatchError: if the current user is
          not the same as the user who created the cluster.
        apx.exceptions.CloudUserIdentityError: if we fail to get the current
          user identity.
    """
    ...
    if controller_utils.Controllers.from_name(cluster_name) is not None:
        raise exceptions.NotSupportedError(
            f'{operation} APX controller {cluster_name!r} '
            f'is not supported.')
...
def queue(cluster_name: str,
    ...
    Please refer to the apx.cli.queue for the document.
...
def cancel(
    ...
    Please refer to the apx.cli.cancel for the document.
    ...
    if all:
        apx_logging.print(f'{colorama.Fore.YELLOW}'
...
def tail_logs(cluster_name: str,
    ...
    Please refer to the apx.cli.tail_logs for the document.
        ...
        [
            {
                ...
                'store': List[apx.StoreType],
                'last_use': int timestamp of last use,
                'status': apx.StorageStatus,
            }
        ]
    """
    storages = global_user_state.get_storage()
    for storage in storages:
        storage['store'] = list(storage.pop('handle').apx_stores.keys())
    return storages
