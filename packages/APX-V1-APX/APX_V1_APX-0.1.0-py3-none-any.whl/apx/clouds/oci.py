...
class OCI(clouds.Cloud):
    ...
    def make_deploy_resources_variables(
        ...
        if _tenancy_prefix is None:
            try:
                ...
                # valid, the 'apx check' would fail (OCI disabled).
    ...
    def check_credentials(cls) -> Tuple[bool, Optional[str]]:
        ...
        short_credential_help_str = (
            ...
            'https://apxdeploy.readthedocs.io/en/latest/getting-started/installation.html#oracle-cloud-infrastructure-oci'
        )
        credential_help_str = (
    ...
    def get_credential_file_mounts(self) -> Dict[str, str]:
        ...
        apx_cfg_file = oci_utils.oci_config.get_apx_user_config_file()

        # OCI config and API key file are mandatory
        credential_files = [oci_cfg_file, api_key_file]

        # Apx config file is optional
        if os.path.exists(apx_cfg_file):
            credential_files.append(apx_cfg_file)
    ...
    def get_current_user_identity(cls) -> Optional[List[str]]:
        # NOTE: used for very advanced APX functionality
        # Can implement later if desired
        # If the user switches the compartment_ocid, the existing clusters
        # might be leaked, as `apx status -r` will fail to find the original
    ...
    def query_status(cls, name: str, tag_filters: Dict[str, str],
        ...
        for node in vms:
            ...
            if vm_status in status_map:
                apx_status = status_map[vm_status]
                if apx_status is not None:
                    status_list.append(apx_status)
