...
# file to any remote nodes (any cloud). Instead, a APX IAM role is
# assigned to both AWS head and workers.
# TODO(apxdeploy): This also means we leave open a bug for AWS SSO users that
...
# See https://github.com/apxdeploy-org/apxdeploy/pull/742.
# Generate the name of the security group we're looking for.
# (username, last 4 chars of hash of hostname): for uniquefying
# users on shared-account scenarios.
DEFAULT_SECURITY_GROUP_NAME = f'apx-sg-{common_utils.user_and_hostname_hash()}'
# Security group to use when user specified ports in their resources.
USER_PORTS_SECURITY_GROUP_NAME = 'apx-sg-{}'
...
class AWS(clouds.Cloud):
    ...
    PROVISIONER_VERSION = clouds.ProvisionerVersion.APXDEPLOY
    STATUS_VERSION = clouds.StatusVersion.APXDEPLOY
    ...
    def _get_default_ami(cls, region_name: str, instance_type: str) -> str:
        ...
        image_id = service_catalog.get_image_id_from_tag(
            'apxdeploy:gpu-ubuntu-2004', region_name, clouds='aws')
        if acc is not None:
            assert len(acc) == 1, acc
            acc_name = list(acc.keys())[0]
            if acc_name == 'K80':
                image_id = service_catalog.get_image_id_from_tag(
                    'apxdeploy:k80-ubuntu-2004', region_name, clouds='aws')
    ...
    def _get_image_id(
        ...
        if image_id_str.startswith('apxdeploy:'):
    ...
    def get_image_size(cls, image_id: str, region: Optional[str]) -> float:
        if image_id.startswith('apxdeploy:'):
    ...
    def get_zone_shell_cmd(cls) -> Optional[str]:
        ...
        command_str = (
            ...
            f' | {constants.APX_PYTHON_CMD} -u -c "import sys, json; '
    ...
    def make_deploy_resources_variables(
        ...
        disk_encrypted = apxdeploy_config.get_nested(('aws', 'disk_encrypted'),
                                                    False)
        user_security_group_config = apxdeploy_config.get_nested(
        ...
        if security_group is None:
            ...
            with ux_utils.print_exception_no_traceback():
                logger.warning(
                    ...
                    'as `aws.security_group_name` in `~/.apx/config.yaml` is specified as '
        ...
        return {
            ...
            'security_group_managed_by_apxdeploy':
    ...
    def check_credentials(cls) -> Tuple[bool, Optional[str]]:
        ...
        dependency_installation_hints = (
            ...
            'Run the following commands:'
            f'\n{cls._INDENT_PREFIX}  $ pip install apxdeploy[aws]'
        ...
        if identity_type == AWSIdentityType.SSO:
            ...
            if static_credential_exists:
                hints += (
                    ' To ensure multiple clouds work correctly, please use APX '
            ...
            # assigned the IAM role: apxdeploy-v1.
        ...
        from apx.clouds.service_catalog import aws_catalog
        ...
        try:
            ...
            return False, (
                ...
                'https://apxdeploy.readthedocs.io/en/latest/cloud-setup/cloud-permissions/aws.html'  # pylint: disable=
    ...
    def get_credential_file_mounts(self) -> Dict[str, str]:
        ...
        #
        # TODO(apxdeploy): ~/.aws/credentials is required for users using
    ...
    def create_image_from_cluster(cls,
        ...
        image_name = f'apxdeploy-{cluster_name.display_name}-{int(time.time())}'
    ...
    def maybe_move_image(cls, image_id: str, source_region: str,
        ...
        if source_region == target_region:
            return image_id
        image_name = f'apxdeploy-cloned-from-{source_region}-{image_id}'
