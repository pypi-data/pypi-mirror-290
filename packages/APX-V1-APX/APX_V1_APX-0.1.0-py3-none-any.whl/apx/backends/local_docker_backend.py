...
from apx.adaptors import docker
from apx.backends import backend_utils
from apx.backends import docker_utils
...
_DOCKER_DEFAULT_LABELS = {'app': 'apx'}
_DOCKER_LABEL_PREFIX = 'apxmeta_'
_DOCKER_HANDLE_PREFIX = 'apxdocker-'
...
class LocalDockerBackend(backends.Backend['LocalDockerResourceHandle']):
    ...
    Here's a correspondence map to help understand how apx concepts map to
    Docker concepts:

    * Apx cluster is a Docker container.
    ...
    * Apx tasks are executed in the container (cluster) through docker exec.
      I.e., apx.execute calls docker.container.exec

    * Apx stop is equivalent to apx down - both terminate the container.
    ...
    * There's no notion of resources - this can be constrained if required,
    ...
    def _sync_workdir(self, handle: LocalDockerResourceHandle,
        ...
        logger.info('Since the workdir is synced at build time, sync_workdir is'
                    ' a NoOp. If you are running apx exec, your workdir has not'
                    ' been updated.')
    ...
    def _setup(self, handle: LocalDockerResourceHandle, task: 'task_lib.Task',
        ...
        to it to handle apx exec commands.
        """
    ...
    def _update_state(self):
        ...
        labels prefixed by apxmeta_ (e.g. apxmeta_workdir).
        """
        search_filter = {
    ...
    def _execute_task_one_node(self, handle: LocalDockerResourceHandle,
        ...
        with tempfile.NamedTemporaryFile(mode='w') as temp_file:
            ...
            cmd = f'chmod +x {script_path} && docker cp {script_path} ' \
                  f'{container.name}:/apx/{docker_utils.APX_DOCKER_RUN_SCRIPT}'
            subprocess.run(cmd, shell=True, check=True)

        _, exec_log = container.exec_run(
            cmd=f'/bin/bash -c "./apx/{docker_utils.APX_DOCKER_RUN_SCRIPT}"',
            stdout=True,
            stderr=True,
            stdin=True,
            tty=True,
        ...
        try:
            ...
            _, kill_log = container.exec_run(
                cmd='/bin/bash -c "pgrep apx_run.sh | xargs kill"',
                stdout=True,
                stderr=True,
                stream=True,
                privileged=True)
