...
Reasons:
 - With our patch, the ray up will always reuse the existing head node with the same cluster
   ...
   APX code will guarantee that reusing the existing head node is safe, as the ray yaml
   is generated by APX (not modified by the user), i.e. the case where the launch hash
   ...
   disable_launch_config_check to the generated ray yaml (apx.templates.<cloud>-ray.yml.j2)
...
def monkey_patch_hash_launch_conf(node_conf, auth):
    ...
    full_auth = auth.copy()
    full_auth.pop('ssh_proxy_command', None)  # NOTE: apxdeploy changes.
...
def monkey_patch_should_create_new_head(
    ...
    # APX: We don't need to check if the head node has the same launch hash, as
    # the upper-level code of APX will guarantee that the head node is always
    # up-to-date.
    return False
