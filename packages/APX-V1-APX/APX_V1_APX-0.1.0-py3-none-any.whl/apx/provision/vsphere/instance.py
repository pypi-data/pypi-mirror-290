...
from apx.provision.vsphere import vsphere_utils
from apx.provision.vsphere.common import custom_script as custom_script_lib
from apx.provision.vsphere.common import metadata_utils
from apx.provision.vsphere.common.vim_utils import poweroff_vm
...
from apx.provision.vsphere.vsphere_utils import VsphereClient
...
def run_instances(region: str, cluster_name: str,
    ...
    # example: UP --> apx stop --> powerOn worker node manually
    ...
    if config.resume_stopped_nodes and to_start_num > 0 and stopped_instances:
        ...
        # example: UP --> apx stop --> delete head node or remove
        # head tag manually
    ...
    head_tag = [{'Key': TAG_APXDEPLOY_HEAD_NODE, 'Value': HEAD_NODE_VALUE}]
    ...
    if not gpu_instance:
        ...
        if len(image_instance_mapping_df) == 0:
            raise Exception(f"""There is no image can match instance type named
                ...
                Or tag 'APXDEPLOY-$manufacturer_short_name$' like
                APXDEPLOY-NVIDIA.""")
...
def _choose_vsphere_cluster_name(config: common.ProvisionConfig, region: str,
    ...
    apxdeploy framework-optimized availability_zones"""
    vsphere_cluster_name = None
...
def _get_cluster_name_filter(cluster_name_on_cloud):
    return [{'Key': TAG_APXDEPLOY_CLUSTER_NAME, 'Value': cluster_name_on_cloud}]
