...
APX_METADATA_VERSION = 'v1'
APX_METADATA_PATH = (pathlib.Path.home() / '.apx' / 'metadata' /
                     APX_METADATA_VERSION)
APX_CLUSTER_METADATA_PATH = APX_METADATA_PATH / 'clusters'
logger = apx_logging.init_logger(__name__)


def _get_cluster_metadata_dir(cluster_name: str) -> pathlib.Path:
    dirname = APX_CLUSTER_METADATA_PATH / cluster_name
...
def _get_instance_metadata_dir(cluster_name: str,
    ...
    dirname = (APX_CLUSTER_METADATA_PATH / cluster_name / 'instances' /
               instance_id)
    if instance_id != '*':
        dirname.mkdir(parents=True, exist_ok=True)
    return dirname.resolve()
