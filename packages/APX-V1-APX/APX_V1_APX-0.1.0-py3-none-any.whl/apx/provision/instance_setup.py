...
from apx import provision
from apx import apx_logging
from apx.provision import common
from apx.provision import docker_utils
from apx.provision import logging as provision_logging
...
_DUMP_RAY_PORTS = (
    f'{constants.APX_PYTHON_CMD} -c \'import json, os; '
    f'json.dump({constants.APX_REMOTE_RAY_PORT_DICT_STR}, '
    f'open(os.path.expanduser("{constants.APX_REMOTE_RAY_PORT_FILE}"), "w", '
    'encoding="utf-8"))\';')

_RAY_PORT_COMMAND = (
    f'RAY_PORT=$({constants.APX_PYTHON_CMD} -c '
    '"from apx.apxlet import job_lib; print(job_lib.get_ray_port())" '
    '2> /dev/null || echo 6379);'
    f'{constants.APX_PYTHON_CMD} -c "from apx.utils import common_utils; '
    'print(common_utils.encode_payload({\'ray_port\': $RAY_PORT}))"')

# Command that calls `ray status` with APX's Ray port set.
RAY_STATUS_WITH_APX_RAY_PORT_COMMAND = (
    f'{_RAY_PORT_COMMAND}; '
    f'RAY_ADDRESS=127.0.0.1:$RAY_PORT {constants.APX_RAY_CMD} status')

# Command that waits for the ray status to be initialized. Otherwise, a later
# `apx status -r` may fail due to the ray cluster not being ready.
...
# Restart apxlet when the version does not match to keep the apxlet up-to-date.
# We need to activate the python environment to make sure autostop in apxlet
# can find the cloud SDK/CLI in PATH.
MAYBE_APXLET_RESTART_CMD = (f'{constants.ACTIVATE_APX_REMOTE_PYTHON_ENV}; '
                            f'{constants.APX_PYTHON_CMD} -m '
                            'apx.apxlet.attempt_apxlet;')
...
def start_ray_on_head_node(cluster_name: str, custom_resource: Optional[str],
    ...
    ray_options = (
        ...
        f'--port={constants.APX_REMOTE_RAY_PORT} '
        f'--dashboard-port={constants.APX_REMOTE_RAY_DASHBOARD_PORT} '
        f'--object-manager-port=8076 '
        f'--temp-dir={constants.APX_REMOTE_RAY_TEMPDIR}')
    if custom_resource:
    ...
    # from environment variables set by user. APX's ray cluster should use
    ...
    # available nodes.
    # Reference: https://github.com/apxdeploy-org/apxdeploy/issues/2441
    cmd = (f'{constants.APX_RAY_CMD} stop; '
           'unset AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY; '
           'RAY_SCHEDULER_EVENTS=0 RAY_DEDUP_LOGS=0 '
           f'{constants.APX_RAY_CMD} start --head {ray_options} || exit 1;' +
...
def start_ray_on_worker_nodes(cluster_name: str, no_restart: bool,
    ...
    ray_options = (f'--address={head_ip}:{constants.APX_REMOTE_RAY_PORT} '
                   f'--object-manager-port=8076')
    ...
    cmd = (
        ...
        'RAY_SCHEDULER_EVENTS=0 RAY_DEDUP_LOGS=0 '
        f'{constants.APX_RAY_CMD} start --disable-usage-stats {ray_options} || '
        'exit 1;' + _RAY_PRLIMIT)
    if no_restart:
        ...
        # will return 0, i.e., we don't know apxdeploy's ray cluster is running.
        ...
        cmd = f'{constants.APX_RAY_CMD} stop; ' + cmd
...
@common.log_function_start_end
@_auto_retry()
def start_apxlet_on_head_node(cluster_name: str,
                              cluster_info: common.ClusterInfo,
                              ssh_credentials: Dict[str, Any]) -> None:
    """Start apxlet on the head node."""
    del cluster_name
    runners = provision.get_command_runners(cluster_info.provider_name,
                                            cluster_info, **ssh_credentials)
    head_runner = runners[0]
    ...
    log_path_abs = str(provision_logging.get_log_path())
    logger.info(f'Running command on head node: {MAYBE_APXLET_RESTART_CMD}')
    # We need to source bashrc for apxlet to make sure the autostop event can
    # access the path to the cloud CLIs.
    returncode, stdout, stderr = head_runner.run(MAYBE_APXLET_RESTART_CMD,
                                                 stream_logs=False,
                                                 require_outputs=True,
                                                 log_path=log_path_abs,
                                                 source_bashrc=True)
    if returncode:
        raise RuntimeError('Failed to start apxlet on the head node '
                           f'(exit code {returncode}). Error: '
                           f'===== stdout ===== \n{stdout}\n'
                           f'===== stderr ====={stderr}')
