...
def bootstrap_instances(
    ...
    if (requested_service_account ==
        ...
        # in ~/.apx/config.yaml), we assume they have already set up the
        # necessary roles and role bindings.
        # If not, set up the roles and bindings for apxdeploy-service-account
        # here.
        ...
        _configure_autoscaler_cluster_role_binding(namespace,
                                                   config.provider_config)
        # APX system namespace is required for FUSE mounting. Here we just
        ...
        _configure_apxdeploy_system_namespace(config.provider_config)
...
def _configure_ssh_jump(namespace, config: common.ProvisionConfig):
    ...
    ssh_jump_name = pod_cfg['metadata']['labels']['apxdeploy-ssh-jump']
...
def _configure_apxdeploy_system_namespace(
        provider_config: Dict[str, Any]) -> None:
    """Creates the namespace for apxdeploy-system mounting if it does not exist.

    Also patches the APX service account to have the necessary permissions
    to manage resources in the namespace.
    """
    svc_account_namespace = provider_config['namespace']
    apxdeploy_system_namespace = provider_config['apxdeploy_system_namespace']
    kubernetes_utils.create_namespace(apxdeploy_system_namespace)

    # Note - this must be run only after the service account has been
    # created in the cluster (in bootstrap_instances).
    # Create the role in the apxdeploy-system namespace if it does not exist.
    _configure_autoscaler_role(apxdeploy_system_namespace,
                               provider_config,
                               role_field='autoscaler_apxdeploy_system_role')
    # We must create a unique role binding per-namespace that APX is
    ...
    # different APX instances may be running in different namespaces.
    override_name = provider_config['autoscaler_apxdeploy_system_role_binding'][
        'metadata']['name'] + '-' + svc_account_namespace

    # Create the role binding in the apxdeploy-system namespace, and have
    # the subject namespace be the namespace that the APX service
    # account is created in.
    _configure_autoscaler_role_binding(
        apxdeploy_system_namespace,
        provider_config,
        binding_field='autoscaler_apxdeploy_system_role_binding',
...
def _configure_fuse_mounting(provider_config: Dict[str, Any]) -> None:
    ...
    APX pod requests this resource to mount the FUSE filesystem.

    We create this daemonset in the apxdeploy_system_namespace, which is
    ...
    'apxdeploy-system' (populated in clouds.Kubernetes).
    """

    logger.info('_configure_fuse_mounting: Setting up FUSE device manager.')

    fuse_device_manager_namespace = provider_config['apxdeploy_system_namespace']
