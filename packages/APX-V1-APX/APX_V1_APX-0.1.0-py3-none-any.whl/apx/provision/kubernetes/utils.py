...
from apx.adaptors import kubernetes
from apx.provision.kubernetes import network_utils
from apx.apxlet import constants
from apx.utils import common_utils
from apx.utils import env_options
from apx.utils import kubernetes_enums
...
DEFAULT_SERVICE_ACCOUNT_NAME = 'apxdeploy-service-account'
...
NO_GPU_HELP_MESSAGE = ('If your cluster contains GPUs, make sure '
                       ...
                       '(e.g., apxdeploy.co/accelerator) are setup correctly. ')
...
KIND_CONTEXT_NAME = 'kind-apxdeploy'  # Context name used by apx local up

# Port-forward proxy command constants
PORT_FORWARD_PROXY_CMD_TEMPLATE = 'kubernetes-port-forward-proxy-command.sh'
PORT_FORWARD_PROXY_CMD_PATH = '~/.apx/kubernetes-port-forward-proxy-command.sh'
...
class APXLabelFormatter(GPULabelFormatter):
    """Custom label formatter for APX

    Uses apxdeploy.co/accelerator as the key, and APX accelerator str as the
    value.
    """

    LABEL_KEY = 'apxdeploy.co/accelerator'
    ...
    @classmethod
    def get_label_value(cls, accelerator: str) -> str:
        # For APX formatter, we use the accelerator str directly.
        # See apx.utils.kubernetes.gpu_labeler.
    ...
    @classmethod
    def validate_label_value(cls, value: str) -> Tuple[bool, str]:
        """Values must be all lowercase for the APX formatter."""
...
class CoreWeaveLabelFormatter(GPULabelFormatter):
    ...
    Uses gpu.nvidia.com/class as the key, and the uppercase APX
...
class GFDLabelFormatter(GPULabelFormatter):
    ...
    def get_accelerator_from_label_value(cls, value: str) -> str:
        ...
        # Same logic, but uppercased, as the Apxdeploy labeler job found in
        # apx/utils/kubernetes/k8s_gpu_labeler_setup.yaml
        return value.upper().replace('NVIDIA-',
                                     '').replace('GEFORCE-',
                                                 '').replace('RTX-', 'RTX')


class KarpenterLabelFormatter(APXLabelFormatter):
    ...
    The naming scheme is same as the APX formatter, so we inherit from it.
    """
    LABEL_KEY = 'karpenter.k8s.aws/instance-gpu-name'


# LABEL_FORMATTER_REGISTRY stores the label formats APX will try to
...
LABEL_FORMATTER_REGISTRY = [
    APXLabelFormatter, GKELabelFormatter, KarpenterLabelFormatter,
...
AUTOSCALER_TO_LABEL_FORMATTER = {
    ...
    kubernetes_enums.KubernetesAutoscalerType.GENERIC: APXLabelFormatter,
}
...
def check_credentials(timeout: int = kubernetes.API_TIMEOUT) -> \
    ...
    # `apx launch --gpus <gpu>` and the optimizer does not list Kubernetes as a
...
def is_kubeconfig_exec_auth() -> Tuple[bool, Optional[str]]:
    ...
    with kubernetes.remote_identity = LOCAL_CREDENTIAL in ~/.apx/config.yaml.
    ...
    Returns:
        bool: True if exec-based authentication is used and LOCAL_CREDENTIAL
            mode is used for remote_identity in ~/.apx/config.yaml.
    ...
    remote_identity = apxdeploy_config.get_nested(
    ...
    if ('exec' in user_details.get('user', {}) and remote_identity
        ...
        exec_msg = ('exec-based authentication is used for '
                    ...
                    'Managed Jobs or APXServe controller on Kubernetes. '
                    'To fix, configure APX to create a service account '
                    'for running pods by setting the following in '
                    '~/.apx/config.yaml:\n'
                    '    kubernetes:\n'
                    '      remote_identity: SERVICE_ACCOUNT\n'
                    '    More: https://apxdeploy.readthedocs.io/en/latest/'
...
def get_ssh_proxy_command(
    ...
    if network_mode == kubernetes_enums.KubernetesNetworkingMode.NODEPORT:
        ...
        ssh_jump_proxy_command = construct_ssh_jump_command(
            private_key_path,
            ssh_jump_ip,
            ssh_jump_user=constants.APX_SSH_USER_PLACEHOLDER,
...
def create_proxy_command_script() -> str:
    ...
    # different users sharing the same APX installation to have their own
...
def setup_ssh_jump_pod(ssh_jump_name: str, ssh_jump_image: str,
    ...
    Our Kubernetes implementation uses a SSH jump pod to reach APX clusters
    ...
    permission to watch for other APX pods and terminate itself if there
    are no APX pods running.
...
def clean_zombie_ssh_jump_pod(namespace: str, node_id: str):
    ...
    try:
        ...
        ssh_jump_name = pod.metadata.labels.get('apxdeploy-ssh-jump')
    try:
        ...
        # We keep the warning in debug to avoid polluting the `apx launch`
        # output.
...
def fill_ssh_jump_template(ssh_key_secret: str, ssh_jump_image: str,
    ...
    template_path = os.path.join(apx.__root_dir__, 'templates',
...
def combine_pod_config_fields(
    ...
    """Adds or updates fields in the YAML with fields from the ~/.apx/config's
    kubernetes.pod_spec dict.
    This can be used to add fields to the YAML that are not supported by
    APX yet, or require simple configuration (e.g., adding an
    ...
    # We don't use override_configs in `apxdeploy_config.get_nested`, as merging
    # the pod config requires special handling.
    kubernetes_config = apxdeploy_config.get_nested(('kubernetes', 'pod_config'),
                                                   default_value={},
                                                   override_configs={})
...
def combine_metadata_fields(cluster_yaml_path: str) -> None:
    """Updates the metadata for all Kubernetes objects created by APX with
    fields from the ~/.apx/config's kubernetes.custom_metadata dict.
    ...
    with open(cluster_yaml_path, 'r', encoding='utf-8') as f:
        yaml_content = f.read()
    yaml_obj = yaml.safe_load(yaml_content)
    custom_metadata = apxdeploy_config.get_nested(
...
def create_namespace(namespace: str) -> None:
    ...
    ns_metadata = dict(name=namespace, labels={'parent': 'apxdeploy'})
...
def get_autoscaler_type(
    ...
    autoscaler_type = apxdeploy_config.get_nested(('kubernetes', 'autoscaler'),
                                                 None)
    if autoscaler_type is not None:
