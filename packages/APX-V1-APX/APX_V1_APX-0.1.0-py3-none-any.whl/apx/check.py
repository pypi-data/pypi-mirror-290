...
from apx import clouds as apx_clouds
...
def check(
    ...
    echo('Checking credentials to enable clouds for APX.')
    enabled_clouds = []
    disabled_clouds = []

    def check_one_cloud(
            cloud_tuple: Tuple[str, Union[apx_clouds.Cloud,
                                          ModuleType]]) -> None:
    ...
    def get_cloud_tuple(
            cloud_name: str) -> Tuple[str, Union[apx_clouds.Cloud, ModuleType]]:
        ...
        if cloud_name.lower().startswith('cloudflare'):
            return cloudflare.APX_CHECK_NAME, cloudflare
        else:
            cloud_obj = apx_clouds.CLOUD_REGISTRY.from_str(cloud_name)
            assert cloud_obj is not None, f'Cloud {cloud_name!r} not found'
            return repr(cloud_obj), cloud_obj

    def get_all_clouds():
        return tuple([repr(c) for c in apx_clouds.CLOUD_REGISTRY.values()] +
                     [cloudflare.APX_CHECK_NAME])
    ...
    config_allowed_cloud_names = [
        get_cloud_tuple(c)[0] for c in apxdeploy_config.get_nested((
    ...
    # Cloudflare is not a real cloud in apx_clouds.CLOUD_REGISTRY, and should
    # not be inserted into the DB (otherwise `apx launch` and other code would
    ...
    if disallowed_cloud_names:
        disallowed_clouds_hint = (
            ...
            'included in allowed_clouds in ~/.apx/config.yaml: '
            f'{", ".join([c for c in disallowed_cloud_names])}')
    if len(all_enabled_clouds) == 0:
        echo(
            click.style(
                'No cloud is enabled. APX will not be able to run any '
                'task. Run `apx check` for more info.',
                fg='red',
                bold=True))
        ...
        echo(
            click.style(
                ...
                dim=True) + click.style(f'apx check{clouds_arg}', bold=True) +
            '\n' + click.style(
...
def get_cached_enabled_clouds_or_refresh(
        raise_if_no_cloud_access: bool = False) -> List[apx_clouds.Cloud]:
    ...
    if raise_if_no_cloud_access and not cached_enabled_clouds:
        with ux_utils.print_exception_no_traceback():
            raise exceptions.NoCloudAccessError(
                ...
                f'{colorama.Style.BRIGHT}apx check{colorama.Style.RESET_ALL}')
    return cached_enabled_clouds


def get_cloud_credential_file_mounts(
        excluded_clouds: Optional[Iterable[apx_clouds.Cloud]]
) -> Dict[str, str]:
    ...
    for cloud in enabled_clouds:
        if (excluded_clouds is not None and
                apx_clouds.cloud_in_iterable(cloud, excluded_clouds)):
            continue
    ...
    # only clouds with computing instances are marked as enabled by apxdeploy.
