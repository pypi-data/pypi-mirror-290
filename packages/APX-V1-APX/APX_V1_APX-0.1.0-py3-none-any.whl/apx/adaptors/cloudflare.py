...
_IMPORT_ERROR_MESSAGE = ('Failed to import dependencies for Cloudflare.'
                         'Try pip install "apxdeploy[cloudflare]"')
...
_INDENT_PREFIX = '    '
NAME = 'Cloudflare'
APX_CHECK_NAME = 'Cloudflare (for R2 object store)'
        ...
        if cloudflare_credentials is None:
            with ux_utils.print_exception_no_traceback():
                raise ValueError('Cloudflare credentials not found. Run '
                                 '`apx check` to verify credentials are '
                                 'correctly set up.')
        else:
...
def check_credentials() -> Tuple[bool, Optional[str]]:
    ...
    if hints:
        ...
        hints += 'https://apxdeploy.readthedocs.io/en/latest/getting-started/installation.html#cloudflare-r2'  # pylint: disable=line-too-long
