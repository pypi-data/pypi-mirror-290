...
APX is a framework for easily running machine learning* workloads on any
...
APX will automatically execute it on AWS, Google Cloud Platform or
Microsoft Azure.

*: APX is primarily targeted at machine learning workloads, but it can
...
discussion](https://github.com/apxdeploy-org/apxdeploy/discussions/1016)
"""
...
INIT_FILE_PATH = os.path.join(ROOT_DIR, 'apx', '__init__.py')
_COMMIT_FAILURE_MESSAGE = (
    'WARNING: APX fail to {verb} the commit hash in '
    f'{INIT_FILE_PATH!r} (APX can still be normally used): '
    '{error}')
...
def get_commit_hash():
    with open(INIT_FILE_PATH, 'r', encoding='utf-8') as fp:
        commit_match = re.search(r'^_APXDEPLOY_COMMIT_SHA = [\'"]([^\'"]*)[\'"]',
                                 fp.read(), re.M)
        if commit_match:
    ...
    if 'APXDEPLOY_COMMIT_SHA' not in commit_hash:
        return commit_hash
    try:
        # Getting the commit hash from git, and check if there are any
        # uncommitted changes.
        # TODO: keep this in sync with apx/__init__.py
...
def replace_commit_hash():
    ...
    try:
        with open(INIT_FILE_PATH, 'r', encoding='utf-8') as fp:
            ...
            content = re.sub(r'^_APXDEPLOY_COMMIT_SHA = [\'"]([^\'"]*)[\'"]',
                             f'_APXDEPLOY_COMMIT_SHA = \'{get_commit_hash()}\'',
                             content,
                             flags=re.M)
...
remote = [
    ...
    # APX: != 1.48.0 is required to avoid the error where ray dashboard fails to start when
...
# When apx/backends/wheel_utils.py builds wheels, it will not contain the
...
setuptools.setup(
    ...
    license='Apache 2.0',
    readme='README.md',
    description='APX: An intercloud broker for the clouds',
    ...
    entry_points={
        'console_scripts': ['apx = apx.cli:cli'],
    },
    ...
    project_urls={
        'Homepage': 'https://github.com/apxdeploy-org/apxdeploy',
        'Issues': 'https://github.com/apxdeploy-org/apxdeploy/issues',
        'Discussion': 'https://github.com/apxdeploy-org/apxdeploy/discussions',
        'Documentation': 'https://apxdeploy.readthedocs.io/en/latest/',
    },
)
