...
_DB_PATH = pathlib.Path(constants.APXSERVE_METADATA_DIR) / 'services.db'
...
class ReplicaStatus(enum.Enum):
    ...
    # The `apx.launch` is pending due to max number of simultaneous launches.
    PENDING = 'PENDING'

    # The replica VM is being provisioned. i.e., the `apx.launch` is still
    # running.
    ...
    # The replica VM is being shut down. i.e., the `apx down` is still running.
...
def get_spec(service_name: str,
             version: int) -> Optional['service_spec.ApxServiceSpec']:
