...
Example usage:
  ...
  >> apx launch task.yaml
  >> apx launch [-c cluster_name] task.yaml
...
listed in "apx --help".  Take care to put logically connected commands close to
each other.
"""
import copy
import datetime
...
from apx.benchmark import benchmark_utils
...
_DAG_NOT_SUPPORTED_MESSAGE = ('YAML specifies a DAG which is only supported by '
                              '`apx jobs launch`. `{command}` supports a '
                              'single task only.')
...
def _install_shell_completion(ctx: click.Context, param: click.Parameter,
    ...
    zshrc_diff = '\n# For APX shell completion\n. ~/.apx/.apx-complete.zsh'
    bashrc_diff = ('\n# For APX shell completion'
                   '\n. ~/.apx/.apx-complete.bash')

    if value == 'bash':
        install_cmd = f'_APX_COMPLETE=bash_source apx > \
                ~/.apx/.apx-complete.bash && \
                echo "{bashrc_diff}" >> ~/.bashrc'

        cmd = (f'(grep -q "APX" ~/.bashrc) || '
               f'[[ ${{BASH_VERSINFO[0]}} -ge 4 ]] && ({install_cmd})')
        reload_cmd = _RELOAD_BASH_CMD

    elif value == 'fish':
        cmd = '_APX_COMPLETE=fish_source apx > \
                ~/.config/fish/completions/apx.fish'

        reload_cmd = _RELOAD_FISH_CMD

    elif value == 'zsh':
        install_cmd = f'_APX_COMPLETE=zsh_source apx > \
                ~/.apx/.apx-complete.zsh && \
                echo "{zshrc_diff}" >> ~/.zshrc'

        cmd = f'(grep -q "APX" ~/.zshrc) || ({install_cmd})'
...
def _uninstall_shell_completion(ctx: click.Context, param: click.Parameter,
    ...
    if value == 'bash':
        cmd = 'sed -i"" -e "/# For APX shell completion/d" ~/.bashrc && \
               sed -i"" -e "/apx-complete.bash/d" ~/.bashrc && \
               rm -f ~/.apx/.apx-complete.bash'

        reload_cmd = _RELOAD_BASH_CMD

    elif value == 'fish':
        cmd = 'rm -f ~/.config/fish/completions/apx.fish'
        reload_cmd = _RELOAD_FISH_CMD

    elif value == 'zsh':
        cmd = 'sed -i"" -e "/# For APX shell completion/d" ~/.zshrc && \
               sed -i"" -e "/apx-complete.zsh/d" ~/.zshrc && \
               rm -f ~/.apx/.apx-complete.zsh'
...
def _parse_override_params(
    ...
    if cloud is not None:
        if cloud.lower() == 'none':
            ...
            override_params['cloud'] = apx_clouds.CLOUD_REGISTRY.from_str(cloud)
...
def _make_task_or_dag_from_entrypoint_with_overrides(
    ...
    *,
    ...
    if is_yaml:
        ...
        task = apx.Task(name='apx-cmd', run=entrypoint)
        task.set_resources({apx.Resources()})
...
class _NaturalOrderGroup(click.Group):
    ...
    @usage_lib.entrypoint('apx.cli', fallback=True)
    def invoke(self, ctx):
...
def _add_command_alias(
    ...
    orig = f'apx {group.name} {command.name}'
    alias = f'apx {new_group.name} {new_command_name}'
    if with_warning:
...
def _deprecate_and_hide_command(group, command_to_deprecate,
    ...
    if group is not None:
        orig = f'apx {group.name} {command_to_deprecate.name}'
    else:
        orig = f'apx {command_to_deprecate.name}'
...
@click.version_option(apx.__version__, '--version', '-v', prog_name='apxdeploy')
def cli():
    pass
...
@click.option(
    ...
    ('If True, run setup in non-interactive mode as part of the job itself. '
     ...
     'the setup process. To see the logs again after detaching, use `apx logs`.'
     ' To cancel setup, cancel the job via `apx cancel`. Useful for long-'
...
@click.option(
    ...
    help=('Automatically stop the cluster after this many minutes '
          ...
          'running ``apx launch -d ...`` and then ``apx autostop -i <minutes>``'
...
def launch(
    ...
    if isinstance(task_or_dag, apx.Dag):
        raise click.UsageError(
            _DAG_NOT_SUPPORTED_MESSAGE.format(command='apx launch'))
    task = task_or_dag
    ...
    if task.service is not None:
        logger.info(
            ...
            f'`apx launch`. {colorama.Style.RESET_ALL}\n{colorama.Fore.YELLOW}'
            'To spin up a service, use APXServe CLI: '
            f'{colorama.Style.RESET_ALL}{colorama.Style.BRIGHT}apx serve up'
...
def exec(
    ...
    ``apx exec`` is thus typically faster than ``apx launch``, provided a
    ...
    reflect those changes.  To ensure a cluster's setup is up to date, use ``apx
    launch`` instead.
    ...
    .. code-block:: bash
      ...
      apx launch -c mycluster app.yaml
      \b
      # For iterative development, simply execute the task on the launched
      # cluster.
      apx exec mycluster app.yaml
      \b
      # Do "apx launch" again if anything other than Task.run is modified:
      apx launch -c mycluster app.yaml
      \b
      # Pass in commands for execution.
      apx exec mycluster python train_cpu.py
      apx exec mycluster --gpus=V100:1 python train_gpu.py
      \b
      # Pass environment variables to the task.
      apx exec mycluster --env WANDB_API_KEY python train_gpu.py
    ...
    if handle is None:
        raise click.BadParameter(f'Cluster {cluster!r} not found. '
                                 'Use `apx launch` to provision first.')
    ...
    if isinstance(task_or_dag, apx.Dag):
        raise click.UsageError('YAML specifies a DAG, while `apx exec` '
                               'supports a single task only.')
    task = task_or_dag

    click.secho(f'Executing task on cluster {cluster}...', fg='yellow')
    apx.exec(task, backend=backend, cluster_name=cluster, detach_run=detach_run)
...
def _get_services(service_names: Optional[List[str]],
    ...
    try:
        ...
        msg = str(e)
        if controller_status is None:
            msg += (f' (See: {colorama.Style.BRIGHT}apx serve -h'
                    f'{colorama.Style.RESET_ALL})')
    except RuntimeError as e:
        msg = ''
        try:
        ...
        if show_endpoint:
            if len(service_records) != 1:
                ...
                raise click.UsageError(
                    ...
                    'apx serve status --endpoint <service-name>')
...
@click.option('--show-services/--no-show-services',
              ...
              is_flag=True,
              required=False,
              help='Also show apx serve services, if any.')
...
def status(all: bool, refresh: bool, ip: bool, endpoints: bool,
    ...
    ``apx status --ip mycluster``.

    If --endpoints is specified, show all exposed endpoints in the cluster.
    Only available when CLUSTERS contains exactly one cluster, e.g.
    ``apx status --endpoints mycluster``. To query a single endpoint, you
    can use ``apx status mycluster --endpoint 8888``.
    ...
    Display all fields using ``apx status -a``.
    ...
    - In cases where clusters are changed outside of APX (e.g., manual
    ...
    """
    # Using a pool with 2 worker to run the managed job query and apx serve
    ...
    with multiprocessing.Pool(2) as pool:
        ...
        if show_services:
            # Run the apx serve service query in parallel to speed up the
            # status query.
        ...
        if show_managed_jobs:
            ...
            with rich_utils.safe_status('[cyan]Checking managed jobs[/]'):
                ...
                if managed_jobs_query_interrupted:
                    ...
                    # down, and the hint for showing apx jobs queue
                    # will still be shown.
        ...
        if show_services:
            ...
            if num_services is not None:
                hints.append(controller_utils.Controllers.APX_SERVE_CONTROLLER.
        ...
        if show_managed_jobs or show_services:
            try:
                ...
                # TODO (zhwu): Remove any importing of ray in APX.
                if e.code != 15:
                    raise
        ...
        if num_pending_autostop > 0 and not refresh:
            ...
            hints.append(f'* {num_pending_autostop} cluster{plural_and_verb} '
                         ...
                         f'{colorama.Style.BRIGHT}apx status --refresh'
                         f'{colorama.Style.RESET_ALL}')
        if hints:
            click.echo('\n' + '\n'.join(hints))
...
def logs(
    ...
    ``~/apx_logs``. Mulitple job_ids can be specified.
    """
...
def cancel(cluster: str, all: bool, jobs: List[int], yes: bool):  # pylint: disable=redefined-builtin, redefined-outer-name
    ...
    .. code-block:: bash
      ...
      apx cancel cluster_name 1
      apx cancel cluster_name 1 2 3
      \b
      # Cancel all jobs on a cluster.
      apx cancel cluster_name -a
      \b
      # Cancel the latest running job on a cluster.
      apx cancel cluster_name

    Job IDs can be looked up by ``apx queue cluster_name``.
    """
...
def stop(
    ...
    .. code-block:: bash
      ...
      apx stop cluster_name
      \b
      # Stop multiple clusters.
      apx stop cluster1 cluster2
      \b
...
def autostop(
    ...
    .. code-block:: bash
        ...
        apx autostop cluster_name -i 60
        \b
        # Cancel autostop for a specific cluster.
        apx autostop cluster_name --cancel
        \b
        # Autodown this cluster after 60 minutes of idleness.
        apx autostop cluster_name -i 60 --down
    """
...
@click.option(
    ...
    help=('Force start the cluster even if it is already UP. Useful for '
          'upgrading the APX runtime on the cluster.'))
@usage_lib.entrypoint
# pylint: disable=redefined-builtin
def start(
        clusters: List[str],
    ...
    .. code-block:: bash
      ...
      apx start cluster_name
      \b
      # Restart multiple clusters.
      apx start cluster1 cluster2
      \b
    ...
    if not clusters and not all:
        ...
        if len(all_cluster_names) <= 1:
            ...
            raise click.UsageError(
                '`apx start` requires either a cluster name or glob '
                '(see `apx status`), or the -a/--all flag.')

    if all:
        if len(clusters) > 0:
            click.echo('Both --all and cluster(s) specified for apx start. '
    ...
    if not clusters:
        click.echo('Cluster(s) not found (tip: see `apx status`). Do you '
                   'mean to use `apx launch` to provision a new cluster?')
        return
    else:
        ...
        for name in clusters:
            ...
            #      (Ex: launch --gpus=A100:8).  Running `apx start` enables
            ...
            #      This way 'apx start' can change its status to UP, enabling
            #      'apx ssh' to debug things (otherwise `apx ssh` will fail an
            ...
            if (not force and cluster_status == status_lib.ClusterStatus.UP):
                # An UP cluster; skipping 'apx start' because:
                ...
                #    so 'apx start existing' will actually launch a new
    ...
    # Checks for controller clusters (jobs controller / apx serve controller).
    ...
    if controllers:
        ...
        if idle_minutes_to_autostop is not None:
            raise click.UsageError(
                ...
                f'calling: {bold}apx start {" ".join(controllers)}{reset_bold}')
...
@click.option(
    ...
    help=('(Advanced) Forcefully remove the cluster(s) from '
          'APX\'s cluster table, even if the actual cluster termination '
...
def _hint_or_raise_for_down_jobs_controller(controller_name: str):
    ...
    msg = (f'{colorama.Fore.YELLOW}WARNING: Tearing down the managed '
           ...
           'jobs (output of `apx jobs queue`) will be lost.')
    click.echo(msg)
    if managed_jobs_:
...
def _hint_or_raise_for_down_apx_serve_controller(controller_name: str):
    ...
    click.echo(f'Terminate apx serve controller: {controller_name}.')


_CONTROLLER_TO_HINT_OR_RAISE = {
    controller_utils.Controllers.JOBS_CONTROLLER:
        (_hint_or_raise_for_down_jobs_controller),
    controller_utils.Controllers.APX_SERVE_CONTROLLER:
        (_hint_or_raise_for_down_apx_serve_controller),
}
...
def _down_or_stop_clusters(
    ...
    Controllers (jobs controller and apx serve controller) can only be
    ...
    if not names and apply_to_all is None:
        # UX: frequently users may have only 1 cluster. In this case, 'apx
        ...
        if len(all_cluster_names) <= 1:
            ...
            raise click.UsageError(
                f'`apx {command}` requires either a cluster name or glob '
                '(see `apx status`), or the -a/--all flag.')
        ...
        if controllers:
            ...
            if not down:
                ...
                try:
                    ...
                    # a user could still do a `apx jobs launch` or a
                    # `apx serve up` before typing the delete, causing a leaked
    ...
    if apply_to_all:
        ...
        if len(names) > 0:
            click.echo(
                f'Both --all and cluster(s) specified for `apx {command}`. '
                'Letting --all take effect.')
    ...
    for name in names:
        ...
        if handle is None:
            # This codepath is used for 'apx down -p <controller>' when the
            # controller is not in 'apx status'.  Cluster-not-found message
    ...
    if not clusters:
        click.echo('Cluster(s) not found (tip: see `apx status`).')
        return
            ...
            try:
                ...
                if not down:
                    message += ('\n  To restart the cluster, run: '
                                f'{colorama.Style.BRIGHT}apx start {name}'
                                f'{colorama.Style.RESET_ALL}')
                success_progress = True
...
def check(clouds: Tuple[str], verbose: bool):
    ...
    This checks access credentials for all clouds supported by APX. If a
    ...
    apx_check.check(verbose=verbose, clouds=clouds_arg)
...
def show_gpus(
    ...
    offer it, the quantity in each VM type, etc.), use ``apx show-gpus <gpu>``.

    To show all accelerators, including less common ones and their detailed
    information, use ``apx show-gpus --all``.

    To show all regions for a specified accelerator, use
    ``apx show-gpus <accelerator> --all-regions``.
    ...
    * ``TOTAL_FREE_GPUS`` (Kubernetes only): Number of currently free GPUs
    ...
    cloud_obj = apx_clouds.CLOUD_REGISTRY.from_str(cloud)
    ...
    cloud_is_kubernetes = isinstance(cloud_obj, apx_clouds.Kubernetes)
    kubernetes_autoscaling = kubernetes_utils.get_autoscaler_type() is not None
    kubernetes_is_enabled = apx_clouds.cloud_in_iterable(
        apx_clouds.Kubernetes(), global_user_state.get_cached_enabled_clouds())
    ...
    def _get_kubernetes_realtime_gpu_table(
        ...
        if len(counts) == 0:
            ...
            debug_msg = 'To further debug, run: apx check '
            if name_filter is not None:
                ...
                debug_msg = ('To show available accelerators on kubernetes,'
                             ' run: apx show-gpus --cloud kubernetes ')
    ...
    def _output():
        ...
        if len(result) == 0:
            ...
            yield 'To show available accelerators, run: apx show-gpus --all'
            return
...
def storage_ls(all: bool):
    """List storage objects managed by APX."""
    storages = apx.storage_ls()
...
def storage_delete(names: List[str], all: bool, yes: bool):  # pylint: disable=redefined-builtin
    ...
    .. code-block:: bash
        ...
        apx storage delete imagenet cifar10
        \b
        # Delete all storage objects matching glob pattern 'imagenet*'.
        apx storage delete "imagenet*"
        \b
        # Delete all storage objects.
        apx storage delete -a
    """
    ...
    subprocess_utils.run_in_parallel(apx.storage_delete, names)
...
def jobs_launch(
    ...
    .. code-block:: bash
      ...
      apx jobs launch task.yaml

      apx jobs launch 'echo hello!'
    """
    if cluster is not None:
    ...
    if not isinstance(task_or_dag, apx.Dag):
        assert isinstance(task_or_dag, apx.Task), task_or_dag
        with apx.Dag() as dag:
            dag.add(task_or_dag)
...
def jobs_queue(all: bool, refresh: bool, skip_finished: bool):
    ...
    error log can be found with ``apx jobs logs --controller``, e.g.:

    .. code-block:: bash

      apx jobs logs --controller job_id
...
def jobs_cancel(name: Optional[str], job_ids: Tuple[int], all: bool, yes: bool):
    ...
    .. code-block:: bash
      ...
      $ apx jobs cancel -n my-job
      \b
      # Cancel managed jobs with IDs 1, 2, 3
      $ apx jobs cancel 1 2 3
    """
...
# TODO(zhwu): Backward compatibility for the old `apx spot launch` command.
# It is now renamed to `apx jobs launch` and the old command is deprecated.
# Remove in v0.8.0.
@cli.group(cls=_NaturalOrderGroup)
def spot():
...
@cli.group(cls=_NaturalOrderGroup)
def serve():
    """APXServe CLI (multi-region, multi-cloud serving)."""
    pass
...
def serve_up(
    ...
    task = _generate_task_with_service(
        ...
        disk_tier=disk_tier,
        ports=ports,
        not_supported_cmd='apx serve up',
    )
...
@click.option('--mode',
              ...
              type=click.Choice([m.value for m in serve_lib.UpdateMode],
                                case_sensitive=False),
              required=False,
              help=('Update mode. If "rolling", APXServe will update the '
                    'service with rolling update. If "blue_green", APXServe '
...
def serve_update(
    ...
    """Update a APXServe service.

    service_yaml must point to a valid YAML file.

    APXServe will reuse old replicas, if only the service section is changed
    and no file mounts are specified.

    Otherwise, APXServe will terminate the old replicas and start new replicas.

    Two update modes are supported:

    - "rolling": (default) APXServe will update the service with rolling update,
        i.e., it will terminate one old replica whenever one new replica is
        ready. Traffic can be mixed on old and new replicas.
    - "blue_green": APXServe will update the service with blue-green update,
    ...
    .. code-block:: bash
        ...
        apx serve update apx-service-16aa new_service.yaml
        # Use blue-green update
        apx serve update --mode blue_green apx-service-16aa new_service.yaml
    ...
    task = _generate_task_with_service(
        ...
        disk_tier=disk_tier,
        ports=ports,
        not_supported_cmd='apx serve update',
    )
    click.secho('Service Spec:', fg='cyan')
    click.echo(task.service)
...
def serve_status(all: bool, endpoint: bool, service_names: List[str]):
    """Show statuses of APXServe services.
    ...
    - ``SHUTTING_DOWN``: The service is being shut down. This usually happens
      when the `apx serve down` command is called.
    ...
    - ``SHUTTING_DOWN``: The replica is being shut down. This usually happens
      ...
      `apx serve down` command is called. APXServe will terminate all replicas
      that errored.
    ...
    - ``PREEMPTED``: The replica was preempted by the cloud provider and apx
...
def serve_logs(
    ...
    .. code-block:: bash
        ...
        apx serve logs --controller [SERVICE_NAME]
        \b
        # Print the load balancer logs so far and exit
        apx serve logs --load-balancer --no-follow [SERVICE_NAME]
        \b
        # Tail the logs of replica 1
        apx serve logs [SERVICE_NAME] 1
    """
...
def benchmark_launch(
    ...
    Example usage: `apx bench launch mytask.yaml -b mytask --gpus V100,T4`
    ...
    if record is not None:
        raise click.BadParameter(f'Benchmark {benchmark} already exists. '
                                 ...
                                 f'run `apx bench delete {benchmark}`.')
    ...
    if not is_yaml:
        raise click.BadParameter(
            'Apx Benchmark does not support command line tasks. '
    ...
    if benchmark_created:
        logger.info(
            ...
            f'{backend_utils.BOLD}apx bench show '
            f'{benchmark}{backend_utils.RESET_BOLD}'
            '\nTo teardown the clusters: '
            f'{backend_utils.BOLD}apx bench down '
            f'{benchmark}{backend_utils.RESET_BOLD}')
        subprocess_utils.run('apx bench ls')
    else:
...
def benchmark_show(benchmark: str) -> None:
    ...
    if any(row[5] == '-' for row in finished):
        # No #STEPS. ApxCallback was unused.
        click.secho(
            'ApxCallback logs are not found in this benchmark. '
            'Consider using ApxCallback to get more detailed information '
            'in real time.',
            fg='yellow')
        ...
        click.secho(
            ...
            'the total number of steps cannot be inferred by ApxCallback. '
            'To get the estimation, specify the total number of steps in '
            'either `apx_callback.init` or `Apx*Callback`.',
            fg='yellow')
...
@click.option(
    ...
    help=('Cluster name(s) to exclude from termination. '
          ...
          '`apx bench show` and exclude a "winner" cluster from termination '
    ...
    if all:
        ...
        if len(benchmarks) > 0:
            print('Both --all and benchmark(s) specified '
                  'for apx bench delete. Letting --all take effect.')
    ...
    def _delete_benchmark(benchmark: str) -> None:
        ...
        if num_clusters > 0:
            ...
            message = (f'{colorama.Fore.YELLOW}Benchmark {benchmark} '
                       ...
                       f'{backend_utils.BOLD} apx bench down {benchmark} '
...
def local_up(gpus: bool):
    ...
    if os.path.exists(os.path.expanduser('~/.kube/config')):
        ...
        apxdeploy_context = 'kind-apxdeploy'
        if curr_context is not None and curr_context != apxdeploy_context:
            click.echo(
                f'Current context in kube config: {curr_context}'
                '\nWill automatically switch to kind-apxdeploy after the local '
                'cluster is created.')
    ...
    up_script_path = os.path.join(path_to_package, 'apx/utils/kubernetes',
                                  'create_cluster.sh')
    ...
    log_path = os.path.join(constants.APX_LOGS_DIRECTORY, run_timestamp,
                            'local_up.log')
    ...
    returncode, _, stderr = log_lib.run_with_log(
        ...
        stream_logs=False,
        line_processor=log_utils.ApxLocalUpLineProcessor(),
        cwd=cwd)
    ...
    if returncode == 0:
        ...
        click.echo(f'{colorama.Fore.GREEN}Local cluster already '
                   ...
                   'If you want to delete it instead, run: apx local down')
    else:
        with ux_utils.print_exception_no_traceback():
            raise RuntimeError(
    ...
    # Run apx check
    with rich_utils.safe_status('[bold cyan]Running apx check...'):
        apx_check.check(clouds=['kubernetes'], quiet=True)
    if cluster_created:
        ...
        if gpus:
            ...
            label_name_escaped = 'apxdeploy.co/accelerator'.replace('.', '\\.')
            gpu_type_cmd = f'kubectl get node apxdeploy-control-plane -o jsonpath=\"{{.metadata.labels[\'{label_name_escaped}\']}}\"'  # pylint: disable=line-too-long
            try:
            ...
            gpu_hint = (
                ...
                'run \'apx show-gpus --cloud kubernetes\'') if gpus else ''
...
def local_down():
    ...
    path_to_package = os.path.dirname(os.path.dirname(__file__))
    down_script_path = os.path.join(path_to_package, 'apx/utils/kubernetes',
                                    'delete_cluster.sh')
