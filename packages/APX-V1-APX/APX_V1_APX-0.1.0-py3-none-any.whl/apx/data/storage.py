"""Storage and Store Classes for Apx Data."""
import enum
import os
import re
import shlex
...
from apx.adaptors import aws
from apx.adaptors import azure
from apx.adaptors import cloudflare
from apx.adaptors import gcp
from apx.adaptors import ibm
from apx.data import data_transfer
from apx.data import data_utils
from apx.data import mounting_utils
from apx.data import storage_utils
...
def get_cached_enabled_storage_clouds_or_refresh(
    ...
    # This is a temporary solution until https://github.com/apxdeploy-org/apxdeploy/issues/1943 # pylint: disable=line-too-long
    # is resolved by implementing separate 'enabled_storage_clouds'
    enabled_clouds = apx_check.get_cached_enabled_clouds_or_refresh()
...
class AbstractStore:
    ...
    class StoreMetadata:
        ...
        def __init__(self,
                     *,
                     ...
                     is_apx_managed: Optional[bool] = None):
            self.name = name
            self.source = source
            self.region = region
            self.is_apx_managed = is_apx_managed
        ...
        def __repr__(self):
            return (f'StoreMetadata('
                    ...
                    f'\n\tsource={self.source},'
                    f'\n\tregion={self.region},'
                    f'\n\tis_apx_managed={self.is_apx_managed})')
    ...
    def __init__(self,
        ...
        Args:
            ...
            is_apx_managed: Whether the store is managed by Apx. If None, it
            ...
            sync_on_reconstruction: bool; Whether to sync data if the storage
              ...
              for direct use, e.g. for 'apx storage delete', or the storage is
              being re-used, e.g., for `apx start` on a stopped cluster.
        ...
        # Whether apx is responsible for the lifecycle of the Store.
        self._validate()
        self.initialize()
    ...
    def from_metadata(cls, metadata: StoreMetadata, **override_args):
        ...
        return cls(name=override_args.get('name', metadata.name),
                   ...
                   is_apx_managed=override_args.get('is_apx_managed',
                                                    metadata.is_apx_managed),
            ...
            if handle is None:
                ...
                with ux_utils.print_exception_no_traceback():
                    raise exceptions.StorageSpecError(
                        'Attempted to mount a non-apx managed bucket '
...
class Storage(object):
    """Storage objects handle persistent and large volume storage in the apx.
    ...
    class StorageMetadata(object):
        ...
        - (optional) Set of stores managed by apx added to the Storage object
        """
        ...
        def __init__(
            ...
            *,
            ...
            apx_stores: Optional[Dict[StoreType,
                                      AbstractStore.StoreMetadata]] = None):
            assert storage_name is not None or source is not None
            self.storage_name = storage_name
            self.source = source
            self.mode = mode
            # Only stores managed by apx are stored here in the
            # global_user_state
            self.apx_stores = {} if apx_stores is None else apx_stores
        ...
        def __repr__(self):
            return (f'StorageMetadata('
                    ...
                    f'\n\tsource={self.source},'
                    f'\n\tmode={self.mode},'
                    f'\n\tstores={self.apx_stores})')

        def add_store(self, store: AbstractStore) -> None:
            storetype = StoreType.from_store(store)
            self.apx_stores[storetype] = store.get_metadata()

        def remove_store(self, store: AbstractStore) -> None:
            storetype = StoreType.from_store(store)
            if storetype in self.apx_stores:
                del self.apx_stores[storetype]
    ...
    def __init__(self,
        ...
        Args:
          ...
          persistent: bool; Whether to persist across apx launches.
        ...
        # for spot. Once apx storage supports forced management for external
        ...
        # Apx optimizer either adds a storage object instance or selects
        # from existing ones
        ...
        if handle is not None:
            ...
            self._add_store_from_metadata(self.handle.apx_stores)
            ...
            if prefix in ['s3', 'gs', 'https', 'r2', 'cos']:
                with ux_utils.print_exception_no_traceback():
                    raise exceptions.StorageNameError(
                        ...
                        'bucket created outside of APX, please specify it '
    ...
    def _add_store_from_metadata(
            self, apx_stores: Dict[StoreType,
                                   AbstractStore.StoreMetadata]) -> None:
        """Reconstructs Storage.stores from apx_stores.

        Reconstruct AbstractStore objects from apx_store's metadata and
        adds them into Storage.stores
        """
        for s_type, s_metadata in apx_stores.items():
    ...
    def add_store(self,
        ...
        try:
            ...
            # Creation failed, so this must be apx managed store. Add failure
            # to state.
            ...
            # Bucket get failed, so this is not apx managed. Do not update state
    ...
    def _add_store(self, store: AbstractStore, is_reconstructed: bool = False):
        ...
        # If store initialized and is apx managed, add to state
        if store.is_apx_managed:
    ...
    def delete(self, store_type: Optional[StoreType] = None) -> None:
        """Deletes data for all apx-managed storage objects.

        If a storage is not managed by apx, it is not deleted from the cloud.
        User must manually delete any object stores created outside of apx.
        ...
        if store_type:
            ...
            is_apx_managed = store.is_apx_managed
            # We delete a store from the cloud if it's apx managed. Else just
            # remove handle and return
            if is_apx_managed:
                self.handle.remove_store(store)
                store.delete()
                # Check remaining stores - if none is apx managed, remove
                # the storage from global_user_state.
                delete = all(
                    s.is_apx_managed is False for s in self.stores.values())
                if delete:
                    global_user_state.remove_storage(self.name)
                else:
...
class S3Store(AbstractStore):
    ...
    def __init__(self,
        ...
        super().__init__(name, source, region, is_apx_managed,
        ...
        if not _is_storage_cloud_enabled(str(clouds.AWS())):
            with ux_utils.print_exception_no_traceback():
                raise exceptions.ResourcesUnavailableError(
                    ...
                    'AWS by running `apx check`. More info: '\
                    'https://apxdeploy.readthedocs.io/en/latest/getting-started/installation.html.' # pylint: disable=line-too-long
                    )
    ...
    def initialize(self):
        ...
        if self.is_apx_managed is None:
            # If is_apx_managed is not specified, then this is a new storage
            # object (i.e., did not exist in global_user_state) and we should
            # set the is_apx_managed property.
            # If is_apx_managed is specified, then we take no action.
            self.is_apx_managed = is_new_bucket
    ...
    def delete(self) -> None:
        deleted_by_apxdeploy = self._delete_s3_bucket(self.name)
        if deleted_by_apxdeploy:
    ...
    def _get_bucket(self) -> Tuple[StorageHandle, bool]:
        ...
        Raises:
            ...
            StorageExternalDeletionError: If externally deleted storage is
                ...
                'apx storage delete' or 'apx start'
        """
        s3 = aws.resource('s3')
        ...
        # the bucket is to be created by Apx. However, creation is skipped if
        # Store object is being reconstructed for deletion or re-mount with
        # apx start, and error is raised instead.
        if self.sync_on_reconstruction:
            bucket = self._create_s3_bucket(self.name, self.region)
            return bucket, True
        else:
            # Raised when Storage object is reconstructed for apx storage
            # delete or to re-mount Storages with apx start but the storage
...
class GcsStore(AbstractStore):
        ...
        if not _is_storage_cloud_enabled(str(clouds.GCP())):
            with ux_utils.print_exception_no_traceback():
                raise exceptions.ResourcesUnavailableError(
                    ...
                    'GCP by running `apx check`. '
                    'More info: https://apxdeploy.readthedocs.io/en/latest/getting-started/installation.html.')  # pylint: disable=line-too-long
    ...
    def delete(self) -> None:
        deleted_by_apxdeploy = self._delete_gcs_bucket(self.name)
        if deleted_by_apxdeploy:
    ...
    def _get_bucket(self) -> Tuple[StorageHandle, bool]:
        ...
        try:
            ...
            if isinstance(self.source, str) and self.source.startswith('gs://'):
                ...
                # created by Apx. However, creation is skipped if Store object
                ...
                if self.sync_on_reconstruction:
                    ...
                    # apx storage delete or to re-mount Storages with apx start
...
class AzureBlobStore(AbstractStore):
    ...
    _ACCESS_DENIED_MESSAGE = 'Access Denied'
    DEFAULT_STORAGE_ACCOUNT_NAME = 'apx{region}{user_hash}'
    DEFAULT_RESOURCE_GROUP_NAME = 'apx{user_hash}'
        ...
        if not _is_storage_cloud_enabled(str(clouds.Azure())):
            with ux_utils.print_exception_no_traceback():
                raise exceptions.ResourcesUnavailableError(
                    ...
                    'Azure by running `apx check`. More info: '
                    'https://apxdeploy.readthedocs.io/en/latest/getting-started/installation.html.'  # pylint: disable=line-too-long
                )
    ...
    def initialize(self):
        ...
        # resource_group_name is set to None when using non-apx-managed
    ...
    def _get_storage_account_and_resource_group(
        ...
        state.db to reuse apx managed storage.

        2) Users provide externally created non-apx managed storage endpoint
        ...
        apx managed storage, through config.yaml. Then, resource group is
        inferred from it.
    ...
    def delete(self) -> None:
        """Deletes the storage."""
        deleted_by_apxdeploy = self._delete_az_bucket(self.name)
        if deleted_by_apxdeploy:
    ...
    def _get_bucket(self) -> Tuple[str, bool]:
        ...
        Returns:
            ...
            bool: represents either or not the bucket is managed by apxdeploy
        ...
        # the container is to be created by Apx. However, creation is skipped
...
class R2Store(AbstractStore):
        ...
        if not _is_storage_cloud_enabled(cloudflare.NAME):
            with ux_utils.print_exception_no_traceback():
                raise exceptions.ResourcesUnavailableError(
                    ...
                    'enable Cloudflare R2 by running `apx check`. '\
                    'More info: https://apxdeploy.readthedocs.io/en/latest/getting-started/installation.html.'  # pylint: disable=line-too-long
                    )
    ...
    def delete(self) -> None:
        deleted_by_apxdeploy = self._delete_r2_bucket(self.name)
        if deleted_by_apxdeploy:
