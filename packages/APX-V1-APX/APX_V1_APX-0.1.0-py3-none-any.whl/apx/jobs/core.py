...
def launch(
    ...
    Please refer to apx.cli.job_launch for documentation.

    Args:
        task: apx.Task, or apx.Dag (experimental; 1-task only) to launch as a
          managed job.
    ...
    Raises:
        ValueError: cluster does not exist. Or, the entrypoint is not a valid
            chain dag.
        apx.exceptions.NotSupportedError: the feature is not supported.
    """
    entrypoint = task
    ...
    with tempfile.NamedTemporaryFile(prefix=f'managed-dag-{dag.name}-',
        ...
        apx_logging.print('Launching jobs controller...')
        apx.launch(task=controller_task,
                   stream_logs=stream_logs,
                   cluster_name=controller_name,
                   detach_run=detach_run,
                   idle_minutes_to_autostop=apxlet_constants.
...
def queue(refresh: bool, skip_finished: bool = False) -> List[Dict[str, Any]]:
    ...
    Please refer to apx.cli.job_queue for documentation.
        ...
        [
            {
                ...
                'status': (apx.jobs.ManagedJobStatus) of the job,
                'cluster_resources': (str) resources of the cluster,
                'region': (str) region of the cluster,
            }
        ]
    Raises:
        apx.exceptions.ClusterNotUpError: the jobs controller is not up or
            does not exist.
    ...
    if refresh and handle is None:
        ...
        rich_utils.force_update_status(
            ...
            'controller[/]')
        handle = apx.start(jobs_controller_type.value.cluster_name)
...
def cancel(name: Optional[str] = None,
    ...
    Please refer to apx.cli.job_cancel for documentation.

    Raises:
        apx.exceptions.ClusterNotUpError: the jobs controller is not up.
...
def tail_logs(name: Optional[str], job_id: Optional[int], follow: bool,
    ...
    Please refer to apx.cli.job_logs for documentation.
    ...
    handle = backend_utils.is_controller_accessible(
        ...
        stopped_message=(
            ...
            f'`apx start {jobs_controller_type.value.cluster_name}`.'))
