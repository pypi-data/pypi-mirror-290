...
def safe_status(msg: str) -> Union['rich_console.Status', _NoOpConsoleStatus]:
    ...
    from apx import apx_logging  # pylint: disable=import-outside-toplevel
    if (threading.current_thread() is threading.main_thread() and
            not apx_logging.is_silent()):
        global _status
        if _status is None:
...
def force_update_status(msg: str):
    """Update the status message even if apx_logging.is_silent() is true."""
    if (threading.current_thread() is threading.main_thread() and
            _status is not None):
        _status.update(msg)
