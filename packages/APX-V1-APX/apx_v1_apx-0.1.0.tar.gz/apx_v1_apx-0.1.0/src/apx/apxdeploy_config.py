...
(default: ~/.apx/config.yaml). Caller can then use

  >> apxdeploy_config.loaded()

to check if the config is successfully loaded.

To read a nested-key config:

  >> apxdeploy_config.get_nested(('auth', 'some_auth_config'), default_value)
...
OVERRIDEABLE_CONFIG_KEYS, override_configs must be provided (can be empty):
  ...
  >> apxdeploy_config.get_nested(('docker', 'run_options'), default_value
                        override_configs={'docker': {'run_options': 'value'}})

To set a value in the nested-key config:

  >> config_dict = apxdeploy_config.set_nested(('auth', 'some_key'), value)
...
then:
    ...
    # Assuming ~/.apx/config.yaml exists and can be loaded:
    apxdeploy_config.loaded()  # ==> True

    apxdeploy_config.get_nested(('a', 'nested'), None)    # ==> 1
    apxdeploy_config.get_nested(('a', 'nonexist'), None)  # ==> None
    apxdeploy_config.get_nested(('a',), None)             # ==> {'nested': 1}

    # If ~/.apx/config.yaml doesn't exist or failed to be loaded:
    apxdeploy_config.loaded()  # ==> False
    apxdeploy_config.get_nested(('a', 'nested'), None)    # ==> None
    apxdeploy_config.get_nested(('a', 'nonexist'), None)  # ==> None
    apxdeploy_config.get_nested(('a',), None)             # ==> None
"""
import copy
import os
import pprint
...
#
# (1) (Used internally) If env var {ENV_VAR_APXDEPLOY_CONFIG} exists, use its
#     path;
...
ENV_VAR_APXDEPLOY_CONFIG = 'APXDEPLOY_CONFIG'

# Path to the local config file.
CONFIG_PATH = '~/.apx/config.yaml'
...
def _try_load_config() -> None:
    ...
    config_path_via_env_var = os.environ.get(ENV_VAR_APXDEPLOY_CONFIG)
        ...
        if not os.path.exists(config_path):
            with ux_utils.print_exception_no_traceback():
                raise FileNotFoundError(
                    ...
                    f'{ENV_VAR_APXDEPLOY_CONFIG} ({config_path!r}) does not '
                    'exist. Please double check the path or unset the env var: '
                    f'unset {ENV_VAR_APXDEPLOY_CONFIG}')
    else:
    ...
    if os.path.exists(config_path):
        ...
        if _dict is not None:
            common_utils.validate_schema(
                ...
                'https://apxdeploy.readthedocs.io/en/latest/reference/config.html. '  # pylint: disable=line-too-long
                'Error: ',
                skip_none=False)
