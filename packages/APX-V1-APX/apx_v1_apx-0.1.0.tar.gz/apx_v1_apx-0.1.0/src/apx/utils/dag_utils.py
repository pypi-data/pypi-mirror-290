...
# Message thrown when APIs apx.{exec,launch,spot.launch}() received a string
# instead of a Dag.  CLI (cli.py) is implemented by us so should not trigger
# this.
_ENTRYPOINT_STRING_AS_DAG_MESSAGE = """\
Expected a apx.Task or apx.Dag but received a string.

If you meant to run a command, make it a Task's run command:

    task = apx.Task(run=command)

The command can then be run as:

  apx.exec(task, cluster_name=..., ...)
  # Or use {'V100': 1}, 'V100:0.5', etc.
  task.set_resources(apx.Resources(accelerators='V100:1'))
  apx.exec(task, cluster_name=..., ...)

  apx.launch(task, ...)

  apx.spot.launch(task, ...)
""".strip()


def convert_entrypoint_to_dag(entrypoint: Any) -> 'dag_lib.Dag':
    """Convert the entrypoint to a apx.Dag.

    Raises TypeError if 'entrypoint' is not a 'apx.Task' or 'apx.Dag'.
    """
    ...
    if isinstance(entrypoint, str):
        ...
        with ux_utils.print_exception_no_traceback():
            raise TypeError(
                'Expected a apx.Task or apx.Dag but received argument of type: '
                f'{type(entrypoint)}')
