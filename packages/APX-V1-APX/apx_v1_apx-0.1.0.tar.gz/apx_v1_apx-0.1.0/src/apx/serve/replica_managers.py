...
from apx.serve import serve_state
from apx.serve import serve_utils
from apx.serve import service
...
if typing.TYPE_CHECKING:
    from apx.serve import service_spec
...
# Since apx.launch is very resource demanding, we limit the number of
# concurrent apx.launch process to avoid overloading the machine.
_MAX_NUM_LAUNCH = psutil.cpu_count() * 2


# TODO(tian): Combine this with
# apx/spot/recovery_strategy.py::StrategyExecutor::launch
def launch_cluster(replica_id: int,
                   task_yaml_path: str,
                   cluster_name: str,
                   resources_override: Optional[Dict[str, Any]] = None,
                   max_retry: int = 3) -> None:
    """Launch a apx serve replica cluster.
    ...
    try:
        ...
        task = apx.Task.from_yaml_config(config)
        ...
        raise RuntimeError(
            f'Failed to launch the apx serve replica cluster {cluster_name} '
            'due to failing to initialize apx.Task from yaml file.') from e
    retry_cnt = 0
    ...
    while True:
        ...
        try:
            ...
            apx.launch(task,
                       ...
                       detach_run=True,
                       retry_until_up=True,
                       _is_launched_by_apx_serve_controller=True)
            ...
            raise RuntimeError('Failed to launch the apx serve replica '
            ...
            if not any(
                ...
                raise RuntimeError('Failed to launch the apx serve replica '
                                   f'cluster {cluster_name}.') from e
            logger.info('Failed to launch the apx serve replica cluster with '
                        f'error: {common_utils.format_exception(e)})')
        except Exception as e:  # pylint: disable=broad-except
            logger.info('Failed to launch the apx serve replica cluster with '
                        f'error: {common_utils.format_exception(e)})')
        ...
        if retry_cnt >= max_retry:
            raise RuntimeError('Failed to launch the apx serve replica cluster '
                               f'{cluster_name} after {max_retry} retries.')
        gap_seconds = backoff.current_backoff()
        logger.info('Retrying to launch the apx serve replica cluster '
                    f'in {gap_seconds:.1f} seconds.')
        time.sleep(gap_seconds)


# TODO(tian): Combine this with
# apx/spot/recovery_strategy.py::terminate_cluster
def terminate_cluster(cluster_name: str,
                      replica_drain_delay_seconds: int = 0,
                      max_retry: int = 3) -> None:
    """Terminate the apx serve replica cluster."""
    ...
    while True:
        ...
        try:
            ...
            if retry_cnt >= max_retry:
                raise RuntimeError('Failed to terminate the apx serve replica '
                                   f'cluster {cluster_name}.') from e
            gap_seconds = backoff.current_backoff()
            logger.error(
                'Failed to terminate the apx serve replica cluster '
...
def _get_resources_ports(task_yaml: str) -> str:
    ...
    task = apx.Task.from_yaml(task_yaml)
    # Already checked all ports are the same in apx.serve.core.up
...
class ReplicaStatusProperty:
    ...
    Attributes:
        apx_launch_status: Process status of apx.launch.
        ...
        apx_down_status: Process status of apx.down.
    """
    # None means apx.launch is not called yet.
    apx_launch_status: Optional[ProcessStatus] = None
    ...
    # None means apx.down is not called yet.
    apx_down_status: Optional[ProcessStatus] = None
    ...
    def unrecoverable_failure(self) -> bool:
        ...
        if self.first_ready_time is not None:
            if self.first_ready_time >= 0:
                ...
                # For replica with a failed apx.launch, it is likely due to some
                # misconfigured resources, so we don't want to auto restart it.
                # For replica with a failed apx.down, we cannot restart it since
    ...
    def should_track_service_status(self) -> bool:
        ...
        This includes:
            ...
            (2) Readiness probe.
        """
        if self.apx_launch_status != ProcessStatus.SUCCEEDED:
            return False
        if self.apx_down_status is not None:
            return False
        if self.user_app_failed:
            return False
        if self.preempted:
    ...
    def to_replica_status(self) -> serve_state.ReplicaStatus:
        ...
        if self.apx_launch_status is None:
            # Pending to launch
            return serve_state.ReplicaStatus.PENDING
        if self.apx_launch_status == ProcessStatus.RUNNING:
            if self.apx_down_status == ProcessStatus.FAILED:
                return serve_state.ReplicaStatus.FAILED_CLEANUP
            if self.apx_down_status == ProcessStatus.SUCCEEDED:
        ...
        if self.apx_launch_status == ProcessStatus.INTERRUPTED:
            # apx.down is running and a scale down interrupted apx.launch
            return serve_state.ReplicaStatus.SHUTTING_DOWN
        if self.apx_down_status is not None:
            if self.preempted:
                # Replica (spot) is preempted
                return serve_state.ReplicaStatus.PREEMPTED
            if self.apx_down_status == ProcessStatus.RUNNING:
                # apx.down is running
                return serve_state.ReplicaStatus.SHUTTING_DOWN
            if self.apx_launch_status == ProcessStatus.INTERRUPTED:
                return serve_state.ReplicaStatus.SHUTTING_DOWN
            ...
            if self.apx_launch_status == ProcessStatus.FAILED:
                # apx.launch failed
        ...
        if self.apx_launch_status == ProcessStatus.FAILED:
            ...
            # due to the `if self.apx_down_status is not None`` check above.
            # However, it should have been started by _refresh_process_pool.
            # If not started, this means some bug prevent apx.down from
        ...
        if self.first_ready_time is not None and self.first_ready_time >= 0.0:
            ...
            # No readiness probe passed and apx.launch finished
...
class ReplicaManager:
    ...
    def __init__(self, service_name: str,
                 spec: 'service_spec.ApxServiceSpec') -> None:
    ...
    def update_version(self, version: int, spec: 'service_spec.ApxServiceSpec',
...
class APXReplicaManager(ReplicaManager):
    """Replica Manager for APX clusters.
    ...
    def __init__(self, service_name: str, spec: 'service_spec.ApxServiceSpec',
    ...
    def _launch_replica(
        ...
        # to avoid too many apx.launch running at the same time.
    ...
    def _terminate_replica(self,
        ...
        if replica_id in self._launch_process_pool:
            ...
            info.status_property.apx_launch_status = ProcessStatus.INTERRUPTED
        ...
        def _download_and_stream_logs(info: ReplicaInfo):
            ...
            if handle is None:
                ...
                return
            assert isinstance(handle, backends.CloudVmRayResourceHandle)
            replica_job_logs_dir = os.path.join(constants.APX_LOGS_DIRECTORY,
                                                'replica_jobs')
            job_log_file_name = (
        ...
        info.status_property.apx_down_status = ProcessStatus.RUNNING
    ...
    def _refresh_process_pool(self) -> None:
        ...
        This function will checks all apx.launch and apx.down process on
        ...
        for replica_id, p in list(self._launch_process_pool.items()):
            if not p.is_alive():
                ...
                error_in_apx_launch = False
                if info.status == serve_state.ReplicaStatus.PENDING:
                    # apx.launch not started yet
                    if (serve_state.total_number_provisioning_replicas() <
                            _MAX_NUM_LAUNCH):
                        p.start()
                        info.status_property.apx_launch_status = (
                    ...
                    if p.exitcode != 0:
                        ...
                        error_in_apx_launch = True
                    else:
                        info.status_property.apx_launch_status = (
                            ProcessStatus.SUCCEEDED)
                serve_state.add_or_update_replica(self._service_name,
                                                  replica_id, info)
                if error_in_apx_launch:
            ...
            if not p.is_alive():
                ...
                if p.exitcode != 0:
                    logger.error(f'Down process for replica {replica_id} '
                                 f'exited abnormally with code {p.exitcode}.')
                    info.status_property.apx_down_status = (
                        ProcessStatus.FAILED)
                else:
                    info.status_property.apx_down_status = (
                        ProcessStatus.SUCCEEDED)
                ...
                # later when we support `apx serve update`.
                removal_reason = None
    ...
    ###########################################
    # APXServe Update and replica versioning. #
    ###########################################
    ...
    def _get_version_spec(self, version: int) -> 'service_spec.ApxServiceSpec':
