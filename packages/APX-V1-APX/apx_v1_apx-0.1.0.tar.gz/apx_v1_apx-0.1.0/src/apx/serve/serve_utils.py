"""User interface with the APXServe."""
import base64
import collections
import enum
import os
...
from apx.serve import constants
from apx.serve import serve_state
from apx.apxlet import constants as apxlet_constants
...
if typing.TYPE_CHECKING:
    ...
    from apx.serve import replica_managers

APX_SERVE_CONTROLLER_NAME: str = (
    f'apx-serve-controller-{common_utils.get_user_hash()}')
...
_APXDEPLOY_PROVISION_LOG_PATTERN = r'.*tail -n100 -f (.*provision\.log).*'
_APXDEPLOY_LOG_PATTERN = r'.*tail -n100 -f (.*\.log).*'
# TODO(tian): Find all existing replica id and print here.
_FAILED_TO_FIND_REPLICA_MSG = (
    f'{colorama.Fore.RED}Failed to find replica '
    '{replica_id}. Please use `apx serve status [SERVICE_NAME]`'
    f' to check all valid replica id.{colorama.Style.RESET_ALL}')
# Max number of replicas to show in `apx serve status` by default.
# If user wants to see all replicas, use `apx serve status --all`.
...
def generate_service_name():
    return f'apx-service-{uuid.uuid4().hex[:4]}'


def generate_remote_service_dir_name(service_name: str) -> str:
    service_name = service_name.replace('-', '_')
    return os.path.join(constants.APXSERVE_METADATA_DIR, service_name)
...
def set_service_status_and_active_versions_from_replica(
    ...
    if record is None:
        raise ValueError('The service is up-ed in an old version and does not '
                         'support update. Please `apx serve down` '
...
def _terminate_failed_services(
    ...
    return (f'{colorama.Fore.YELLOW}terminate service {service_name!r} with '
            ...
            'leak. Please check the following APX clusters on the '
        ...
        if (service_status is None or service_status['status']
            ...
            if purge:
                ...
                messages.append(
                    ...
                    f'(Use `apx serve down {service_name} --purge` to '
...
def wait_service_registration(service_name: str, job_id: int) -> str:
    """Util function to call at the end of `apx.serve.up()`.
    ...
    This function will:
        (1) Check the name duplication by job id of the controller. If
            ...
            apx/serve/api.py::up for more details.
            ...
            if job_id != record['controller_job_id']:
                with ux_utils.print_exception_no_traceback():
                    raise ValueError(
                        ...
                        'To update an existing service, run: `apx serve down` '
                        'and then `apx serve up` again (in-place update will '
    ...
    while True:
        ...
        if tmp is not None and tmp != '':
            ...
            if '\n' in line or '\r' in line:
                # Tailing detailed progress for user. All logs in apxdeploy is
                # of format `To view detailed progress: tail -n100 -f *.log`.
                x = re.match(_APXDEPLOY_PROVISION_LOG_PATTERN, line)
                if x is not None:
                    log_file = os.path.expanduser(x.group(1))
                elif re.match(_APXDEPLOY_LOG_PATTERN, line) is None:
...
# ================== Table Formatter for `apx serve status` ==================
...
class ServeCodeGen:
    ...
    def _build(cls, code: List[str]) -> str:
        ...
        return (f'{apxlet_constants.APX_PYTHON_CMD} '
    ...
    def update_service(cls, service_name: str, version: int, mode: str) -> str:
        code = [
            ...
            # machine has an old APX version before that, we need to avoid
