...
from apx import task as task_mod
...
APX_DOCKER_SETUP_SCRIPT = 'apx_setup.sh'
APX_DOCKER_RUN_SCRIPT = 'apx_run.sh'
APX_DOCKER_WORKDIR = 'apx_workdir'
...
def build_dockerimage(task: task_mod.Task,
    ...
    """
    # Get tempdir
    temp_dir = tempfile.mkdtemp(prefix='apx_local_')
    ...
    _, img_metadata = create_dockerfile(base_image=task.docker_image,
                                        ...
                                        copy_path=f'{APX_DOCKER_WORKDIR}/',
                                        run_command=task.run,
                                        build_dir=temp_dir)

    dst = os.path.join(temp_dir, APX_DOCKER_WORKDIR)
