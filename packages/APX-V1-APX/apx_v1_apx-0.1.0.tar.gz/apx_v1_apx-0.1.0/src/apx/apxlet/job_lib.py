...
_JOB_STATUS_LOCK = '~/.apx/locks/.job_{}.lock'
...
_DB_PATH = os.path.expanduser('~/.apx/jobs.db')
...
class JobStatus(enum.Enum):
    ...
    # not started yet. apxlet can transit the state from INIT to FAILED
...
def make_ray_job_id(apx_job_id: int) -> str:
    return f'{apx_job_id}-{getpass.getuser()}'
...
def get_ray_port():
    """Get the port Apxdeploy-internal Ray cluster uses.

    If the port file does not exist, the cluster was launched before #1790,
    return the default port.
    """
    port_path = os.path.expanduser(constants.APX_REMOTE_RAY_PORT_FILE)
...
def get_job_submission_port():
    """Get the dashboard port Apxdeploy-internal Ray cluster uses.
...
def update_status() -> None:
    # This will be called periodically by the apxlet to update the status
...
def dump_job_queue(username: Optional[str], all_jobs: bool) -> str:
    ...
    for job in jobs:
        ...
        job['log_path'] = os.path.join(constants.APX_LOGS_DIRECTORY,
...
class JobLibCodeGen:
    ...
    _PREFIX = [
        'import os',
        'import getpass',
        'from apx.apxlet import job_lib, log_lib, constants',
        # Backward compatibility for old apxlet lib version on the remote
        # machine. The `job_owner` argument was removed in #3037, and if the
        # remote machine has an old APX version before that, we need to
        # pass the `job_owner` argument to the job_lib functions.
        # TODO(zhwu): Remove this in 0.7.0 release.
        'job_owner_kwargs = {} '
        'if getattr(constants, "APXLET_LIB_VERSION", 0) >= 1 '
    ...
    def tail_logs(cls,
        ...
        code = [
            ...
            f'log_dir = None if run_timestamp is None else os.path.join({constants.APX_LOGS_DIRECTORY!r}, run_timestamp)',
    ...
    def _build(cls, code: List[str]) -> str:
        ...
        code = ';'.join(code)
        return f'{constants.APX_PYTHON_CMD} -u -c {shlex.quote(code)}'
