...
_APX_LOG_WAITING_GAP_SECONDS = 1
_APX_LOG_WAITING_MAX_RETRY = 5
_APX_LOG_TAILING_GAP_SECONDS = 0.2
        ...
        with line_processor:
            while True:
                ...
                if (args.end_streaming_at is not None and
                    ...
                    # E.g., this is used for `apx bench` to hide the
                    # redundant messages of `apx launch` while
    ...
    **kwargs,
    ...
    with subprocess.Popen(cmd,
        ...
        try:
            ...
            if not hasattr(constants, 'APX_GET_PYTHON_PATH_CMD'):
                ...
                python_path = subprocess.check_output(
                    constants.APX_GET_PYTHON_PATH_CMD,
                    shell=True,
...
def make_task_bash_script(codegen: str,
    ...
    # DEACTIVATE_APX_REMOTE_PYTHON_ENV: Deactivate the APX runtime env, as
    ...
    script = [
        textwrap.dedent(f"""\
            ...
            set +a
            {constants.DEACTIVATE_APX_REMOTE_PYTHON_ENV}
            export PYTHONUNBUFFERED=1
            cd {constants.APX_REMOTE_WORKDIR}"""),
    ]
...
def run_bash_command_with_log(bash_command: str,
                              ...
                              stream_logs: bool = False,
                              with_ray: bool = False):
    with tempfile.NamedTemporaryFile('w', prefix='apx_app_',
                                     delete=False) as fp:
        ...
        if tmp is not None and tmp != '':
            ...
            if status not in [
                ...
                if wait_last_logs:
                    ...
                    time.sleep(1 + _APX_LOG_TAILING_GAP_SECONDS)
            ...
            time.sleep(_APX_LOG_TAILING_GAP_SECONDS)
            status = job_lib.get_status_no_lock(job_id)
...
def tail_logs(job_id: Optional[int],
    ...
    if job_id is None:
        ...
        # apx.apxlet.job_lib.JobLibCodeGen.tail_logs for more details.
    ...
    if log_dir is None:
        print(f'{job_str.capitalize()} not found (see `apx queue`).',
              file=sys.stderr)
        return
    ...
    while status is not None and not status.is_terminal():
        ...
        if os.path.exists(log_path) and status != job_lib.JobStatus.INIT:
            break
        if retry_cnt >= _APX_LOG_WAITING_MAX_RETRY:
            print(
            ...
            return
        print(f'INFO: Waiting {_APX_LOG_WAITING_GAP_SECONDS}s for the logs '
              'to be written...')
        time.sleep(_APX_LOG_WAITING_GAP_SECONDS)
        status = job_lib.update_job_status([job_id], silent=True)[0]
