...
from apx.adaptors import common as adaptors_common
from apx.adaptors import oci as oci_adaptor
from apx.clouds.utils import oci_utils
from apx.apxlet.providers.oci import utils
...
class oci_query_helper:
    ...
    def find_compartment(cls, region) -> str:
        ...
        apxdeploy_compartment = oci_utils.oci_config.get_compartment(region)
        if apxdeploy_compartment is not None:
            return apxdeploy_compartment

        # If not specified, we try to find the one apxdeploy-compartment
    ...
    def find_create_vcn_subnet(cls, region) -> Optional[str]:
        ...
        if len(vcns) > 0:
            ...
            # Create the apxdeploy_vcn and related resources
            return cls.create_vcn_subnet(net_client, apxdeploy_compartment)

    @classmethod
    @utils.debug_enabled(logger=logger)
    def create_vcn_subnet(cls, net_client,
                          apxdeploy_compartment) -> Optional[str]:
        try:
            ...
            update_route_table_response = net_client.update_route_table(
                ...
                UpdateRouteTableDetails(route_rules=[
                    oci_adaptor.oci.core.models.RouteRule(
                        ...
                        description='Route table for APX VCN',
                        route_type='STATIC')
                ]))
            ...
            cls.delete_vcn(net_client, apxdeploy_vcn, subnet, ig, sg)
            subnet = None
    ...
    @classmethod
    @utils.debug_enabled(logger=logger)
    def delete_vcn(cls, net_client, apxdeploy_vcn, apxdeploy_subnet,
                   internet_gateway, service_gateway):
        if apxdeploy_vcn is None:
        ...
        try:
            ...
            if apxdeploy_subnet is not None:
                # Delete subnet
                delete_subnet_response = net_client.delete_subnet(
                    subnet_id=apxdeploy_subnet)
                logger.debug(f'Deleted subnet {apxdeploy_subnet}'
            ...
            while retry_count < oci_utils.oci_config.MAX_RETRY_COUNT:
                try:
                    delete_vcn_response = net_client.delete_vcn(
                        vcn_id=apxdeploy_vcn)
                    logger.debug(
                        f'Deleted vcn {apxdeploy_vcn}-{delete_vcn_response.data}'
                    )
                    break
                except oci_adaptor.service_exception() as e:
