...
from apx.jobs import recovery_strategy
...
# `apx.jobs.controller` namespace when executed directly, so as
# to inherit the setup from the `apx` logger.
logger = apx_logging.init_logger('apx.jobs.controller')


def _get_dag_and_name(dag_yaml: str) -> Tuple['apx.Dag', str]:
...
class JobsController:
    ...
    def __init__(self, job_id: int, dag_yaml: str,
        ...
        #   Example value: apx-2022-10-04-22-46-52-467694_my-spot-name_spot_id-17-0
        job_id_env_vars = []
    ...
    def _download_log_and_stream(
        ...
        """
        managed_job_logs_dir = os.path.join(constants.APX_LOGS_DIRECTORY,
                                            'managed_jobs')
        controller_utils.download_and_stream_latest_job_log(
            self._backend, handle, managed_job_logs_dir)
        logger.info(f'\n== End of logs (ID: {self._job_id}) ==')

    def _run_one_task(self, task_id: int, task: 'apx.Task') -> bool:
        ...
        Raises:
            exceptions.ProvisionPrechecksError: This will be raised when the
                underlying `apx.launch` fails due to precheck errors only.
            ...
            if cluster_status != status_lib.ClusterStatus.UP:
                ...
                if job_status is not None and not job_status.is_terminal():
                    ...
                    failure_reason = (
                        ...
                        f'apx jobs logs --controller {self._job_id}')
        ...
        if not job_status.is_terminal():
            ...
            managed_job_state.set_failed(
                ...
                failure_reason=('Unexpected error occurred. For details, '
                                f'run: apx jobs logs --controller {job_id}'))
