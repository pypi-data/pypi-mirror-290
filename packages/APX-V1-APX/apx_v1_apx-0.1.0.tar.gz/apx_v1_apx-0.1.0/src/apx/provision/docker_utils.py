...
def docker_start_cmds(
    ...
    docker_run = [
        docker_cmd,
        'run',
        # APX: Remove --rm flag to keep the container after `ray stop`
        # is executed.
        ...
        user_options_str,
        '--net=host',
        # APX: Add following options to enable fuse.
        '--cap-add=SYS_ADMIN',
        '--device=/dev/fuse',
...
# APX: New class to initialize docker containers on a remote node.
...
class DockerInitializer:
    ...
    def _run(self,
        ...
        if run_env == 'docker':
            ...
            # APX: We do not include `-it` flag here, as that will cause
    ...
    def initialize(self) -> str:
        ...
        # APX: Check if the container is exited but not removed.
        ...
        # APX: Docker login if user specified a private docker registry.
    ...
    # APX: New function to check whether a container is exited
    # (but not removed). This is due to previous `apx stop` command,
