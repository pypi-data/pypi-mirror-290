...
from apx.provision.gcp import constants
from apx.provision.gcp import instance_utils
...
def _apxdeploy_log_error_and_exit_for_failover(error_code: str,
                                              error_msg: str) -> None:
    ...
    prefix = 'APXDEPLOY_ERROR_NO_NODES_LAUNCHED: '
...
def _is_permission_satisfied(service_account, crm, iam, required_permissions,
    ...
    # apxdeploy-v1 service account level, which can be checked with
...
def _configure_iam_role(config: common.ProvisionConfig, crm, iam) -> dict:
    ...
    email = constants.APXDEPLOY_SERVICE_ACCOUNT_EMAIL_TEMPLATE.format(
        account_id=constants.APXDEPLOY_SERVICE_ACCOUNT_ID,
    ...
    if not satisfied:
        # APX: Fallback to the old ray service account name for
        ...
        if ray_satisfied:
            ...
            logger.info('_configure_iam_role: '
                        'Creating new service account {}'.format(
                            constants.APXDEPLOY_SERVICE_ACCOUNT_ID))
            # APX: a GCP user without the permission to create a service
            # account will fail here.
            service_account = _create_service_account(
                constants.APXDEPLOY_SERVICE_ACCOUNT_ID,
                constants.APXDEPLOY_SERVICE_ACCOUNT_CONFIG,
                project_id,
                iam,
            )
    ...
    if (instance_utils.get_node_type(
            config.node_config) == instance_utils.GCPNodeType.TPU):
        # APX: The API for TPU VM is slightly different from normal compute
        # instances.
...
def get_usable_vpc_and_subnet(
    ...
    if specific_vpc_to_use is not None:
        ...
        if len(vpcnets_all) == 1:
            ...
            if not subnets:
                _apxdeploy_log_error_and_exit_for_failover(
            ...
            # VPC with this name not found. Error out and let APX failover.
            _apxdeploy_log_error_and_exit_for_failover(
                'VPC_NOT_FOUND',
    ...
    logger.info(
        f'Creating a default VPC network, {constants.APXDEPLOY_VPC_NAME}...')

    # Create a APX VPC network if it doesn't exist
    vpc_list = _list_vpcnets(project_id,
                             compute,
                             filter=f'name={constants.APXDEPLOY_VPC_NAME}')
    if len(vpc_list) == 0:
        body = constants.VPC_TEMPLATE.copy()
        body['name'] = body['name'].format(VPC_NAME=constants.APXDEPLOY_VPC_NAME)
        body['selfLink'] = body['selfLink'].format(
            PROJ_ID=project_id, VPC_NAME=constants.APXDEPLOY_VPC_NAME)
        _create_vpcnet(project_id, compute, body)

    _create_rules(project_id, compute, constants.FIREWALL_RULES_TEMPLATE,
                  constants.APXDEPLOY_VPC_NAME)

    usable_vpc_name = constants.APXDEPLOY_VPC_NAME
    subnets = _list_subnets(project_id,
                            region,
                            compute,
    ...
    logger.info(f'A VPC network {constants.APXDEPLOY_VPC_NAME} created.')
...
def _configure_subnet(region: str, cluster_name: str,
    ...
    # APX: make sure there's a usable VPC
...
def _get_service_account(account: str, project_id: str, iam):
    ...
    try:
        ...
        if e.resp.status not in [403, 404]:
            # APX: added 403, which means the service account doesn't
