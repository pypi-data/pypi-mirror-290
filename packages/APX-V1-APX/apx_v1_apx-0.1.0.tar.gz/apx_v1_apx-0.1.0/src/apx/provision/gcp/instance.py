...
from apx.provision import constants as provision_constants
...
def _get_head_instance_id(instances: List) -> Optional[str]:
    ...
    for inst in instances:
        ...
        if (labels.get(provision_constants.TAG_RAY_NODE_KIND) == 'head' or
                labels.get(provision_constants.TAG_APXDEPLOY_HEAD_NODE) == '1'):
...
def _run_instances(region: str, cluster_name_on_cloud: str,
    ...
    # APX: 'TERMINATED' for compute VM, 'STOPPED' for TPU VM
    ...
    # APX: We try to use the instances with the same matching launch_config
