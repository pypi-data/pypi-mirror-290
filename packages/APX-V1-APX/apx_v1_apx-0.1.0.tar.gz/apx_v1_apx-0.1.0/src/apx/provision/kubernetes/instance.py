...
from apx.provision import constants
from apx.provision import docker_utils
from apx.provision.kubernetes import config as config_lib
...
TAG_POD_INITIALIZED = 'apxdeploy-initialized'
...
def _raise_pod_scheduling_errors(namespace, new_nodes):
    ...
    def _lack_resource_msg(resource: str,
        ...
        msg = (
            ...
            'were not found in a single node. Other APX tasks or pods may '
            ...
            if pod_status == 'Pending':
                ...
                if 'Insufficient smarter-devices/fuse' in event_message:
                    raise config_lib.KubernetesError(
                        ...
                        '`kubectl delete pods -n apxdeploy-system -l name=smarter-device-manager`.'  # pylint: disable=line-too-long
...
def _set_env_vars_in_pods(namespace: str, new_pods: List):
    ...
    See https://github.com/apxdeploy-org/apxdeploy/issues/2287 for
    more details.
...
def _check_user_privilege(namespace: str, new_nodes: List) -> None:
    ...
    check_k8s_user_sudo_cmd = (
        'if [ $(id -u) -eq 0 ]; then'
        # If user is root, create an alias for sudo used in apxdeploy setup
...
def _create_pods(region: str, cluster_name_on_cloud: str,
    ...
    pod_spec['metadata']['labels'].update(
        {TAG_APXDEPLOY_CLUSTER_NAME: cluster_name_on_cloud})
    ...
    if to_start_count < 0:
        raise RuntimeError(
            ...
            'Use "apx down" to terminate the cluster.')
    ...
    try:
        ...
        logger.warning('run_instances: Error occurred while checking for '
                       ...
                       'override runtimeClassName in ~/.apx/config.yaml. '
                       'For more details, refer to https://apxdeploy.readthedocs.io/en/latest/reference/config.html')  # pylint: disable=line-too-long
    ...
    for _ in range(to_start_count):
        if head_pod_name is None:
            ...
            # pods on larger nodes which may be able to fit multiple APX
            # "nodes".
                ...
                'podAntiAffinity': {
                    ...
                    'preferredDuringSchedulingIgnoredDuringExecution': [{
                        ...
                        'podAffinityTerm': {
                            'labelSelector': {
                                'matchExpressions': [{
                                    'key': TAG_APXDEPLOY_CLUSTER_NAME,
                                    'operator': 'In',
    ...
    if networking_mode == kubernetes_enums.KubernetesNetworkingMode.NODEPORT:
        ...
        ssh_jump_pod_name = pod_spec['metadata']['labels']['apxdeploy-ssh-jump']
...
def query_instances(
    ...
    # Get all the pods with the label apxdeploy-cluster: <cluster_name>
    try:
        pods = kubernetes.core_api().list_namespaced_pod(
            namespace,
            label_selector=f'apxdeploy-cluster={cluster_name_on_cloud}',
