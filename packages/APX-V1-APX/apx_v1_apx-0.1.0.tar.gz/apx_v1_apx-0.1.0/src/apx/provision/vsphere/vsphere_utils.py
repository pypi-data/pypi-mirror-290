...
from apx.adaptors import vsphere as vsphere_adaptor
from apx.clouds.service_catalog import vsphere_catalog
from apx.clouds.service_catalog.common import get_catalog_path
from apx.clouds.service_catalog.data_fetchers.fetch_vsphere import (
...
from apx.clouds.service_catalog.data_fetchers.fetch_vsphere import (
    initialize_vms_csv)
from apx.provision.vsphere.common import vim_utils
from apx.provision.vsphere.common.cls_api_client import ClsApiClient
from apx.provision.vsphere.common.cls_api_helper import ClsApiHelper
from apx.provision.vsphere.common.id_generator import generate_random_uuid
from apx.provision.vsphere.common.service_manager_factory import (
    ServiceManagerFactory)
from apx.provision.vsphere.common.vim_utils import create_spec_with_script
from apx.provision.vsphere.common.vim_utils import poweron_vm
from apx.provision.vsphere.common.vim_utils import wait_for_tasks
from apx.provision.vsphere.common.vim_utils import wait_internal_ip_ready

logger = apx_logging.init_logger(__name__)

CREDENTIALS_PATH = '~/.vsphere/credential.yaml'
VM_PREFIX = 'apxdeploy-vm-'
...
class VsphereClient:
    ...
    def create_instances(
        ...
        if service_manager:
            ...
            profile_id = self.get_apxdeploy_profile_id()
            if profile_id is None:
                logger.error('The VM storage policy \'apxdeploy_policy\''
                             ' is not available.')
                raise exceptions.ResourcesUnavailableError(
                    'The VM storage policy \'apxdeploy_policy\''
                    ' is not available.')
    ...
    def get_apxdeploy_profile_id(self):
