...
APXDEPLOY = 'apxdeploy'
DEFAULT_APXDEPLOY_INSTANCE_PROFILE = APXDEPLOY + '-v1'
DEFAULT_APXDEPLOY_IAM_ROLE = APXDEPLOY + '-v1'

# Suppress excessive connection dropped logs from boto
logging.getLogger('botocore').setLevel(logging.WARNING)


def _apxdeploy_log_error_and_exit_for_failover(error: str) -> None:
...
def bootstrap_instances(
    ...
    assert 'NetworkInterfaces' not in node_cfg, (
        'APX: NetworkInterfaces is not supported in '
        'node config')
    assert subnet_ids is None, 'APX: SubnetIds is not supported.'
    ...
    # NOTE(apxdeploy): apxdeploy uses the default AMIs in apx/clouds/aws.py.
...
def _configure_iam_role(iam) -> Dict[str, Any]:
    ...
    instance_profile_name = DEFAULT_APXDEPLOY_INSTANCE_PROFILE
    ...
    if not profile.roles:
        role_name = DEFAULT_APXDEPLOY_IAM_ROLE
        ...
        if role is None:
            ...
            role.Policy('APXPassRolePolicy').put(
                PolicyDocument=json.dumps(apxdeploy_pass_role_policy_doc))
