...
from apx.clouds import aws as aws_cloud
from apx.provision import common
from apx.provision import constants
from apx.provision.aws import utils
...
def _default_ec2_resource(region: str) -> Any:
    if not hasattr(aws, 'version'):
        ...
        # on the controller due to a new `apx jobs launch or `apx start`.
        #
        # First, controller consists of an outer process (apx.jobs.controller's
        # main) and an inner process running the controller logic (started as a
        # multiprocessing.Process in apx.jobs.controller). `apx.provision.aws`
        ...
        # {old apx.provision.aws, old apx.adaptors.aws}, and outer process has
        # loaded {old apx.adaptors.aws}.
        #
        # In controller.py's start(), the inner process may exit due to managed
        # job exits or `apx jobs cancel`, entering outer process'
        # `finally: ... _cleanup()` path. Inside _cleanup(), we eventually call
        # into `apx.provision.aws` which loads this module for the first time
        # for the outer process. At this point, outer process has loaded
        # {old apx.adaptors.aws, new apx.provision.aws}.
        #
        # This version mismatch becomes a "backward compatibility" problem if
        # `new apx.provision.aws` depends on `new apx.adaptors.aws` (assuming
        # API changes in apx.adaptors.aws). Therefore, here we use a hack to
        # reload apx.adaptors.aws to go from old to new.
        #
...
def _create_instances(ec2_fail_fast, cluster_name: str,
    ...
    tags = {
        ...
        constants.TAG_APXDEPLOY_CLUSTER_NAME: cluster_name,
        **tags
    }
...
def run_instances(region: str, cluster_name_on_cloud: str,
    ...
    def _create_node_tag(target_instance, is_head: bool = True) -> str:
        ...
        if is_head:
            node_tag.append({
                'Key': 'Name',
                'Value': f'apx-{cluster_name_on_cloud}-head'
            })
        else:
            node_tag.append({
                'Key': 'Name',
                'Value': f'apx-{cluster_name_on_cloud}-worker'
            })
...
def terminate_instances(
    ...
    managed_by_apxdeploy = provider_config['security_group'].get(
        'ManagedByAPX', True)
    ec2 = _default_ec2_resource(region)
    filters = [
        {
        ...
        if 'FromPort' in existing_rule and 'ToPort' in existing_rule:
            ...
            for group_pairs in existing_rule['UserIdGroupPairs']:
                if group_pairs['GroupId'] != sg.id:
                    ...
                    # The security group created by APX allows all traffic
