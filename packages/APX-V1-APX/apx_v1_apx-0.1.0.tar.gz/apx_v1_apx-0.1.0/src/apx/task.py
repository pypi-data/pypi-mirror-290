...
class Task:
    ...
    def __init__(
        ...
        *,
        ...
        is assumed (the same as ``apx launch``).
        ...
        Example:
            .. code-block:: python
                ...
                apx.Task(setup='pip install requirements.txt',
                         run='python train.py',
                         workdir='.')

                # An empty Task for provisioning a cluster.
                task = apx.Task(num_nodes=n).set_resources(...)

                # Chaining setters.
                apx.Task().set_resources(...).set_file_mounts(...)
        ...
        # Default to CPU VM
        self.resources: Union[List[apx.Resources],
                              Set[apx.Resources]] = {apx.Resources()}
        self._service: Optional[service_spec.ApxServiceSpec] = None
        # Resources that this task cannot run on.
        self.blocked_resources = blocked_resources

        self.time_estimator_func: Optional[Callable[['apx.Resources'],
                                                    int]] = None
        self.file_mounts: Optional[Dict[str, str]] = None

        # Only set when 'self' is a jobs controller task: 'self.managed_job_dag'
        # is the underlying managed job dag (apx.Dag object).
        self.managed_job_dag: Optional['apx.Dag'] = None

        # Only set when 'self' is a apx serve controller task.
    ...
    def from_yaml_config(
        ...
        if experimnetal_configs is not None:
            ...
            logger.debug('Overriding apxdeploy config with task-level config: '
        ...
        task.set_resources(apx.Resources.from_yaml_config(resources_config))

        service = config.pop('service', None)
        if service is not None:
            service = service_spec.ApxServiceSpec.from_yaml_config(service)
    ...
    def from_yaml(yaml_path: str) -> 'Task':
        ...
        Example:
            .. code-block:: python
                ...
                task = apx.Task.from_yaml('/path/to/task.yaml')
    ...
    def set_resources(
        ...
        Args:
          resources: either a apx.Resources, a set of them, or a list of them.
        ...
        """
        if isinstance(resources, apx.Resources):
            resources = {resources}
    ...
    @property
    def service(self) -> Optional[service_spec.ApxServiceSpec]:
        return self._service

    def set_service(self,
                    service: Optional[service_spec.ApxServiceSpec]) -> 'Task':
        """Sets the service spec for this task.

        Args:
          service: a ApxServiceSpec object.
    ...
    def set_time_estimator(self, func: Callable[['apx.Resources'],
                                                int]) -> 'Task':
        ...
        for target, source in file_mounts.items():
            ...
            if not data_utils.is_cloud_store_url(source):
                if (not os.path.exists(
                        ...
                        not source.startswith('apxdeploy:')):
                    with ux_utils.print_exception_no_traceback():
                        raise ValueError(
                            f'File mount source {source!r} does not exist '
                            'locally. To fix: check if it exists, and correct '
                            'the path.')
            # TODO(zhwu): /home/username/apx_workdir as the target path need
            # to be filtered out as well.
            if (target == constants.APX_REMOTE_WORKDIR and
                    self.workdir is not None):
                with ux_utils.print_exception_no_traceback():
                    raise ValueError(
                        f'Cannot use {constants.APX_REMOTE_WORKDIR!r} as a '
    ...
    def set_storage_mounts(
        ...
        Storage mounts are a dictionary: ``{mount_path: apx.Storage object}``,
        each of which mounts a apx.Storage object (a cloud object store bucket)
        to a path inside the remote cluster.

        A apx.Storage object can be created by uploading from a local directory
        ...
        Example:
            .. code-block:: python
                ...
                task.set_storage_mounts({
                    '/remote/imagenet/': apx.Storage(name='my-bucket',
                                                     source='/local/imagenet'),
                })

        Args:
          storage_mounts: an optional dict of ``{mount_path: apx.Storage
    ...
    def __rshift__(self, b):
        apx.dag.get_current_dag().add_edge(self, b)
