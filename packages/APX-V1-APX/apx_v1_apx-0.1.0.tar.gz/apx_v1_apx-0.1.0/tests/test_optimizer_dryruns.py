...
def _test_parse_task_yaml(spec: str, test_fn: Optional[Callable] = None):
    ...
    with tempfile.NamedTemporaryFile('w') as f:
        ...
        f.flush()
        with apx.Dag():
            task = apx.Task.from_yaml(f.name)
...
def _make_resources(
    ...
    **resources_kwargs,
    ...
    return apx.Resources(*resources_args, **resources_kwargs)
...
def _test_resources_launch(monkeypatch,
    ...
    with apx.Dag() as dag:
        task = apx.Task('test_task')
        task.set_resources({resources})
    apx.launch(dag, dryrun=True, cluster_name=cluster_name)
    assert True


def test_resources_aws(monkeypatch):
    _test_resources_launch(monkeypatch, apx.AWS(), 'p3.2xlarge')


def test_resources_azure(monkeypatch):
    _test_resources_launch(monkeypatch, apx.Azure(), 'Standard_NC24s_v3')


def test_resources_gcp(monkeypatch):
    _test_resources_launch(monkeypatch, apx.GCP(), 'n1-standard-16')
...
def test_partial_v100(monkeypatch):
    _test_resources_launch(monkeypatch, apx.AWS(), accelerators='V100')
    _test_resources_launch(monkeypatch,
                           apx.AWS(),
                           accelerators='V100',
                           use_spot=True)
    _test_resources_launch(monkeypatch, apx.AWS(), accelerators={'V100': 8})
...
def test_invalid_instance_type(monkeypatch):
    for cloud in [apx.AWS(), apx.Azure(), apx.GCP(), None]:
        with pytest.raises(ValueError) as e:
...
def test_infer_cloud_from_instance_type(monkeypatch):
    ...
    _test_resources(monkeypatch,
                    ...
                    expected_cloud=apx.GCP())
    #Azure instances
    _test_resources(monkeypatch,
                    instance_type='Standard_NC12s_v3',
                    expected_cloud=apx.Azure())
...
def test_invalid_region(monkeypatch):
    for cloud in [apx.AWS(), apx.Azure(), apx.GCP()]:
        with pytest.raises(ValueError) as e:
...
def test_invalid_zone(monkeypatch):
    ...
    with pytest.raises(ValueError) as e:
        _test_resources(monkeypatch, apx.GCP(), zone=input_zone)
    assert 'Invalid zone' in str(e.value)
...
def test_invalid_image(monkeypatch):
    ...
    with pytest.raises(ValueError) as e:
        _test_resources(monkeypatch, cloud=apx.Lambda(), image_id='some-image')
...
def test_invalid_accelerators_regions(enable_all_clouds, monkeypatch):
    ...
    with pytest.raises(exceptions.ResourcesUnavailableError) as e:
        apx.launch(task, cluster_name='should-fail', dryrun=True)
        assert 'No launchable resource found for' in str(e.value), str(e.value)


def _test_optimize_speed(resources: apx.Resources):
    with apx.Dag() as dag:
        task = apx.Task(run='echo hi')
        task.set_resources(resources)
    start = time.time()
...
def test_optimize_speed(enable_all_clouds, monkeypatch):
    _test_optimize_speed(apx.Resources(cpus=4))
    for cloud in clouds.CLOUD_REGISTRY.values():
        _test_optimize_speed(apx.Resources(cloud, cpus='4+'))
    _test_optimize_speed(apx.Resources(cpus='4+', memory='4+'))
    _test_optimize_speed(
        apx.Resources(cpus='4+', memory='4+', accelerators='V100:1'))
    _test_optimize_speed(
        apx.Resources(cpus='4+', memory='4+', accelerators='A100-80GB:8'))
    _test_optimize_speed(
        apx.Resources(cpus='4+', memory='4+', accelerators='tpu-v3-32'))
...
def test_infer_cloud_from_region_or_zone(monkeypatch):
    ...
    _test_resources_launch(monkeypatch, region='us-east-1', cloud=apx.AWS())
    _test_resources_launch(monkeypatch, region='us-east-1', cloud=apx.Lambda())
    ...
    # Typo, fuzzy hint.
    with pytest.raises(ValueError) as e:
        _test_resources_launch(monkeypatch, zone='us-west-2-a', cloud=apx.AWS())
...
def test_ordered_resources(enable_all_clouds, monkeypatch):
    ...
    with apx.Dag() as dag:
        task = apx.Task('test_task')
        task.set_resources([
            apx.Resources(accelerators={'V100': 1}),
            apx.Resources(accelerators={'T4': 1}),
            apx.Resources(accelerators={'K80': 1}),
            apx.Resources(accelerators={'T4': 4}),
        ])
    dag = apx.optimize(dag)
...
def test_optimize_disk_tier(enable_all_clouds):
    ...
    best_tier_resources = apx.Resources(disk_tier=resources_utils.DiskTier.BEST)
    ...
    low_tier_resources = apx.Resources(disk_tier=resources_utils.DiskTier.LOW)
    ...
    high_tier_resources = apx.Resources(disk_tier=resources_utils.DiskTier.HIGH)
