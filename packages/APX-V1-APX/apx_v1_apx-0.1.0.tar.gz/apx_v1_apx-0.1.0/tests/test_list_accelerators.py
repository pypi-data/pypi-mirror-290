...
def test_list_accelerators_all():
    result = apx.list_accelerators(gpus_only=False)
    assert 'V100' in result, result
    assert 'tpu-v3' in result, result
...
def test_list_accelerators_name_filter():
    ...
    result = apx.list_accelerators(gpus_only=False, name_filter='V100')
...
def test_list_accelerators_name_quantity_filter():
    ...
    result = apx.list_accelerators(name_filter='V100', quantity_filter=4)
    all_accelerators = []
    for res in result.values():
        for instance in res:
...
def test_list_accelerators_positive_quantity_filter():
    ...
    result = apx.list_accelerators(quantity_filter=4)
    all_accelerators = []
    for res in result.values():
        for instance in res:
...
def test_list_accelerators_name_clouds_filter():
    ...
    for cloud in CLOUDS_TO_TEST:
        result = apx.list_accelerators(clouds=cloud.lower(), name_filter='V100')
        all_accelerators = []
        for res in result.values():
            for instance in res:
                all_accelerators.append(
...
def test_list_accelerators_name_quantity_clouds_filter():
    ...
    for cloud in CLOUDS_TO_TEST:
        result = apx.list_accelerators(clouds=cloud.lower(),
                                       name_filter='A100',
                                       quantity_filter=4)
        all_accelerators = []
        for res in result.values():
