...
class TestExecutionOnExistingClusters:
    ...
    def _mock_cluster_state(self, _mock_db_conn, enable_all_clouds):
        ...
        handle = backends.CloudVmRayResourceHandle(
            ...
            launched_nodes=1,
            launched_resources=apx.Resources(
                apx.AWS(),
    ...
    def test_launch_exec(self, _mock_cluster_state, monkeypatch):
        ...
        task.set_resources(apx.Resources(accelerators='A100:8'))
        apx.launch(task, cluster_name='test-cluster1', dryrun=True)
        apx.exec(task, cluster_name='test-cluster1', dryrun=True)
        task.set_resources(apx.Resources(accelerators='A100:3'))
        apx.launch(task, cluster_name='test-cluster1', dryrun=True)
        apx.exec(task, cluster_name='test-cluster1', dryrun=True)
        task.set_resources(
            apx.Resources(
        ...
        task = apx.Task(run='echo hi')
        task.set_resources(apx.Resources(accelerators='V100:4'))
        apx.launch(task, cluster_name='test-cluster2', dryrun=True)
        apx.exec(task, cluster_name='test-cluster2', dryrun=True)
        task.set_resources(
            apx.Resources(apx.GCP(), accelerators='V100:3', region='us-west1'))
        apx.launch(task, cluster_name='test-cluster2', dryrun=True)
        apx.exec(task, cluster_name='test-cluster2', dryrun=True)

        task = apx.Task(run='echo hi')
        apx.exec(task, cluster_name='test-cluster3', dryrun=True)

        task = apx.Task(run='echo hi')
        task.set_resources(
            apx.Resources(disk_tier=resources_utils.DiskTier.BEST))
        apx.launch(task, cluster_name='test-disk-tier1', dryrun=True)
        apx.exec(task, cluster_name='test-disk-tier1', dryrun=True)
        apx.launch(task, cluster_name='test-disk-tier2', dryrun=True)
        apx.exec(task, cluster_name='test-disk-tier2', dryrun=True)
        task.set_resources(
            apx.Resources(disk_tier=resources_utils.DiskTier.LOW))
        apx.launch(task, cluster_name='test-disk-tier1', dryrun=True)
        apx.exec(task, cluster_name='test-disk-tier1', dryrun=True)
        apx.launch(task, cluster_name='test-disk-tier2', dryrun=True)
        apx.exec(task, cluster_name='test-disk-tier2', dryrun=True)
        task.set_resources(
            apx.Resources(disk_tier=resources_utils.DiskTier.HIGH))
        apx.launch(task, cluster_name='test-disk-tier1', dryrun=True)
        apx.exec(task, cluster_name='test-disk-tier1', dryrun=True)
    ...
    def _run_launch_exec_with_error(self, task, cluster_name):
        with pytest.raises(exceptions.ResourcesMismatchError) as e:
            ...
            assert 'do not match the existing cluster.' in str(e.value), str(
                e.value)
        with pytest.raises(exceptions.ResourcesMismatchError) as e:
            apx.exec(task, cluster_name=cluster_name, dryrun=True)
            assert 'do not match the existing cluster.' in str(e.value), str(
                e.value)
    ...
    def test_launch_exec_mismatch(self, _mock_cluster_state, monkeypatch):
        ...
        task.set_resources(apx.Resources(accelerators='V100:8'))
        ...
        # Cloud mismatch
        task.set_resources(apx.Resources(apx.AWS(), accelerators='V100'))
        self._run_launch_exec_with_error(task, 'test-cluster2')

        task.set_resources(apx.Resources(apx.GCP()))
        self._run_launch_exec_with_error(task, 'test-cluster1')
