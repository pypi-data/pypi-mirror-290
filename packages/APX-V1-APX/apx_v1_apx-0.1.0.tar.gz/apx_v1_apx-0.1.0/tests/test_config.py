...
def _reload_config() -> None:
    apxdeploy_config._dict = None
    apxdeploy_config._try_load_config()


def _check_empty_config() -> None:
    """Check that the config is empty."""
    assert not apxdeploy_config.loaded()
    assert apxdeploy_config.get_nested(
        ('aws', 'ssh_proxy_command'), None) is None
    assert apxdeploy_config.get_nested(('aws', 'ssh_proxy_command'),
                                      'default') == 'default'
    with pytest.raises(RuntimeError):
        apxdeploy_config.set_nested(('aws', 'ssh_proxy_command'), 'value')
...
def test_no_config(monkeypatch) -> None:
    ...
    monkeypatch.setattr(apxdeploy_config, 'CONFIG_PATH', '/tmp/does_not_exist')
    _reload_config()
    _check_empty_config()
...
def test_empty_config(monkeypatch, tmp_path) -> None:
    ...
    with open(tmp_path / 'empty.yaml', 'w', encoding='utf-8') as f:
        f.write('')
    monkeypatch.setattr(apxdeploy_config, 'CONFIG_PATH', tmp_path / 'empty.yaml')
    _reload_config()
    _check_empty_config()
        ...
        spot:
            controller:
                resources:
                    disk_size: 256
        """)
    monkeypatch.setattr(apxdeploy_config, 'CONFIG_PATH', tmp_path / 'valid.yaml')
    _reload_config()
    proxy_config = apxdeploy_config.get_nested(
...
def test_invalid_field_config(monkeypatch, tmp_path) -> None:
    ...
    config_path.open('w', encoding='utf-8').write(
        ...
        aws:
            ...
            not_a_field: 123
        """))
    monkeypatch.setattr(apxdeploy_config, 'CONFIG_PATH',
...
def test_config_get_set_nested(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(apxdeploy_config, 'CONFIG_PATH', config_path)
    _reload_config()
    # Check that the config is loaded with the expected values
    assert apxdeploy_config.loaded()
    assert apxdeploy_config.get_nested(('aws', 'vpc_name'), None) == VPC_NAME
    assert apxdeploy_config.get_nested(('aws', 'disk_encrypted'), None)
    assert apxdeploy_config.get_nested(('aws', 'use_internal_ips'), None)
    assert apxdeploy_config.get_nested(('aws', 'ssh_proxy_command'),
                                      None) == PROXY_COMMAND
    assert apxdeploy_config.get_nested(('gcp', 'vpc_name'), None) == VPC_NAME
    assert apxdeploy_config.get_nested(('kubernetes', 'networking'),
                                      None) == NODEPORT_MODE_NAME
    # Check set_nested() will copy the config dict and return a new dict
    new_config = apxdeploy_config.set_nested(('aws', 'ssh_proxy_command'),
                                            'new_value')
    assert new_config['aws']['ssh_proxy_command'] == 'new_value'
    assert apxdeploy_config.get_nested(('aws', 'ssh_proxy_command'),
                                      None) == PROXY_COMMAND
    new_config = apxdeploy_config.set_nested(('kubernetes', 'networking'),
                                            PORT_FORWARD_MODE_NAME)
    assert apxdeploy_config.get_nested(('kubernetes', 'networking'),
                                      None) == NODEPORT_MODE_NAME
    # Check that dumping the config to a file with the new None can be reloaded
    new_config2 = apxdeploy_config.set_nested(('aws', 'ssh_proxy_command'), None)
    new_config_path = tmp_path / 'new_config.yaml'
    common_utils.dump_yaml(new_config_path, new_config2)
    monkeypatch.setattr(apxdeploy_config, 'CONFIG_PATH', new_config_path)
    _reload_config()
    assert apxdeploy_config.get_nested(('aws', 'vpc_name'), None) == VPC_NAME
    assert apxdeploy_config.get_nested(('aws', 'use_internal_ips'), None)
    assert apxdeploy_config.get_nested(
        ('aws', 'ssh_proxy_command'), None) is None
    assert apxdeploy_config.get_nested(('gcp', 'vpc_name'), None) == VPC_NAME
    assert apxdeploy_config.get_nested(('gcp', 'use_internal_ips'), None)
    ...
    _reload_config()
    assert apxdeploy_config.get_nested(('aws', 'vpc_name'), None) == VPC_NAME
    assert apxdeploy_config.get_nested(('aws', 'use_internal_ips'), None) is None
    assert apxdeploy_config.get_nested(
        ('aws', 'ssh_proxy_command'), None) is None
    # set_nested() should still work
    new_config4 = apxdeploy_config.set_nested(('aws', 'ssh_proxy_command'),
                                             'new_value')
...
def test_config_with_env(monkeypatch, tmp_path) -> None:
    ...
    config_path = tmp_path / 'config.yaml'
    _create_config_file(config_path)
    monkeypatch.setenv(apxdeploy_config.ENV_VAR_APXDEPLOY_CONFIG, config_path)
    monkeypatch.setattr(apxdeploy_config, 'CONFIG_PATH',
...
def test_k8s_config_with_override(monkeypatch, tmp_path,
    ...
    task = apx.Task.from_yaml(task_path)

    # Test Kubernetes overrides
    # Get cluster YAML
    cluster_name = 'test-kubernetes-config-with-override'
    task.set_resources_override({'cloud': apx.Kubernetes()})
    apx.launch(task, cluster_name=cluster_name, dryrun=True)
    cluster_yaml = pathlib.Path(
        f'~/.apx/generated/{cluster_name}.yml.tmp').expanduser().rename(
    ...
    assert cluster_pod_config['metadata']['labels']['parent'] == 'apxdeploy'
...
def test_gcp_config_with_override(monkeypatch, tmp_path,
    ...
    task.set_resources_override({'cloud': apx.GCP(), 'accelerators': 'L4'})
    apx.launch(task, cluster_name=cluster_name, dryrun=True)
    cluster_yaml = pathlib.Path(
        f'~/.apx/generated/{cluster_name}.yml.tmp').expanduser().rename(
            tmp_path / (cluster_name + '.yml'))
