...
ALL_INSTANCE_TYPE_INFOS = sum(
    apx.list_accelerators(gpus_only=True).values(), [])

DUMMY_NODES = [
    apx.optimizer._DUMMY_SOURCE_NAME,
    apx.optimizer._DUMMY_SINK_NAME,
]
...
def generate_random_dag(
    ...
    """Generates a random Apx DAG to test Apx optimizer."""
    random.seed(seed)
    single_node_task_ids = random.choices(list(range(num_tasks)),
                                          k=num_tasks // 2)
    with apx.Dag() as dag:
        for i in range(num_tasks):
            op = apx.Task(name=f'task{i}')
            ...
            if num_parents == 0:
                ...
                for parent in parents:
                    parent >> op
                # NOTE: Apx only takes single input data source
                src_cloud = parents[0]
                # Apx uses the parent's output data size
                src_volume = None
...
def find_min_objective(
        dag: apx.Dag,
        minimize_cost: bool) -> Tuple[float, Dict[apx.Task, apx.Resources]]:
    ...
    def _optimize_by_brute_force(tasks, plan):
        """Optimizes a Apx DAG in a brute-force manner."""
        # NOTE: Here we assume that the Apx DAG is topologically sorted.
        ...
        for resources in task.resources:
            ...
            if len(tasks) == 1:
                if minimize_cost:
                    objective = apx.Optimizer._compute_total_cost(
                        graph, topo_order, plan)
                else:
                    objective = apx.Optimizer._compute_total_time(
                        graph, topo_order, plan)
...
def compare_optimization_results(dag: apx.Dag, minimize_cost: bool):
    copy_dag = copy.deepcopy(dag)
    if minimize_cost:
        optimizer_plan = apx.Optimizer._optimize_dag(dag,
                                                     apx.OptimizeTarget.COST)
    else:
        optimizer_plan = apx.Optimizer._optimize_dag(dag,
                                                     apx.OptimizeTarget.TIME)
    if minimize_cost:
        objective = apx.Optimizer._compute_total_cost(dag.get_graph(),
                                                      dag.tasks, optimizer_plan)
    else:
        objective = apx.Optimizer._compute_total_time(dag.get_graph(),
                                                      dag.tasks, optimizer_plan)
...
def test_optimizer(enable_all_clouds):
    for seed in range(3):
        ...
        apx.Optimizer._add_dummy_source_sink_nodes(dag)
