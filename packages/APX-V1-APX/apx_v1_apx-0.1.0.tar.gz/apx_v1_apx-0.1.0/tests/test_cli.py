...
def test_show_gpus():
    """Tests `apx show-gpus` can be invoked (but not correctness).

    Tests below correspond to the following terminal commands, in order:

    -> apx show-gpus
    -> apx show-gpus --all
    -> apx show-gpus V100:4
    -> apx show-gpus :4
    -> apx show-gpus V100:0
    -> apx show-gpus V100:-2
    -> apx show-gpus --cloud aws --region us-west-1
    -> apx show-gpus --cloud lambda
    -> apx show-gpus --cloud lambda --all
    -> apx show-gpus V100:4 --cloud lambda
    -> apx show-gpus V100:4 --cloud lambda --all
    """
    cli_runner = cli_testing.CliRunner()
