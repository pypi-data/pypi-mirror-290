...
from apx.utils import db_utils
...
def _mock_cluster_state(_mock_db_conn):
    ...
    handle = backends.CloudVmRayResourceHandle(
        ...
        launched_nodes=2,
        launched_resources=apx.Resources(apx.AWS(),
    ...
    handle = backends.CloudVmRayResourceHandle(
        ...
        launched_nodes=1,
        launched_resources=apx.Resources(apx.GCP(),
    ...
    handle = backends.CloudVmRayResourceHandle(
        ...
        launched_nodes=4,
        launched_resources=apx.Resources(apx.Azure(),
...
def _mock_serve_controller(_mock_db_conn):
    handle = backends.CloudVmRayResourceHandle(
        cluster_name=serve.APX_SERVE_CONTROLLER_NAME,
        cluster_name_on_cloud=serve.APX_SERVE_CONTROLLER_NAME,
    ...
    global_user_state.add_or_update_cluster(
        serve.APX_SERVE_CONTROLLER_NAME,
        handle,
        requested_resources={handle.launched_resources},
        ready=True)
...
class TestJobsOperations:
    ...
    def test_down_jobs_controller(self, _mock_cluster_state,
        ...
        monkeypatch.setattr(
            'apx.backends.backend_utils.is_controller_accessible',
            mock_is_controller_accessible)

        monkeypatch.setattr(
            'apx.backends.cloud_vm_ray_backend.CloudVmRayBackend.run_on_head',
        ...
        result = cli_runner.invoke(cli.down, ['apx-jobs-con*'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output

        result = cli_runner.invoke(cli.down, ['apx-jobs-con*', '-p'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output
    ...
    def test_stop_jobs_controller(self, _mock_cluster_state,
        ...
        result = cli_runner.invoke(cli.stop, ['apx-jobs-con*'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output
    ...
    def test_autostop_jobs_controller(self, _mock_cluster_state,
        ...
        result = cli_runner.invoke(cli.autostop, ['apx-jobs-con*'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output
...
class TestServeOperations:
    ...
    def test_down_serve_controller(self, _mock_cluster_state,
        ...
        def mock_get_services_one_service(
            ...
            service = {
                ...
                'policy': None,
                'requested_resources': apx.Resources(),
        ...
        cli_runner = cli_testing.CliRunner()
        result = cli_runner.invoke(cli.down, [serve.APX_SERVE_CONTROLLER_NAME],
                                   input='n')
        assert 'Terminate apx serve controller:' in result.output, (
            result.exception, result.output, result.exc_info)
        assert isinstance(result.exception,
                          SystemExit), (result.exception, result.output)
        ...
        result = cli_runner.invoke(cli.down, ['apx-serve-con*'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output

        result = cli_runner.invoke(cli.down, ['apx-serve-con*', '-p'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output
    ...
    def test_stop_serve_controller(self, _mock_cluster_state,
        ...
        cli_runner = cli_testing.CliRunner()
        result = cli_runner.invoke(cli.stop, [serve.APX_SERVE_CONTROLLER_NAME])
        assert result.exit_code == click.UsageError.exit_code
        assert (
            f'Stopping controller(s) \'{serve.APX_SERVE_CONTROLLER_NAME}\' is '
            'currently not supported' in result.output)

        result = cli_runner.invoke(cli.stop, ['apx-serve-con*'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output
    ...
    def test_autostop_serve_controller(self, _mock_cluster_state,
        ...
        assert (
            ...
            f'\'{serve.APX_SERVE_CONTROLLER_NAME}\' is currently not supported'
            in result.output)

        result = cli_runner.invoke(cli.autostop, ['apx-serve-con*'])
        assert not result.exception
        assert 'Cluster(s) not found' in result.output
    ...
    def test_cancel_on_serve_controller(self, _mock_cluster_state,
        ...
        result = cli_runner.invoke(cli.cancel,
                                   [serve.APX_SERVE_CONTROLLER_NAME, '-a'])
        assert result.exit_code == click.UsageError.exit_code
        assert 'Cancelling the apx serve controller\'s jobs is not allowed.' in str(
            result.output)
    ...
    def test_down(self, _mock_db_conn):
        ...
        assert result.exit_code == 1
        assert (controller_utils.Controllers.APX_SERVE_CONTROLLER.value.
    ...
    def test_logs(self, _mock_db_conn):
        ...
        assert controller_utils.Controllers.APX_SERVE_CONTROLLER.value.default_hint_if_non_existent in str(
            result.exception), (result.exception, result.output,
                                result.exc_info)
