class Board:
    ...
    def __minimax(self, board, maximizing_player=True, alpha=float('-inf'), beta=float('inf')) -> tuple[float | int, str]:
        if board.terminalState(self):
        ...
        if maximizing_player:
            ...
            best_move = None
            for move in board.legalMoves(self, board.X):
                board.move(self, move)
                value, _ = board.__minimax(self, board, False, alpha, beta)
                board.undoMove(self)
                if value > best_value:
                    best_value = value
                    best_move = move
            ...
            best_value = float('inf')
            best_move = None
            for move in board.legalMoves(self, board.O):
                board.move(self, move)
                value, _ = board.__minimax(self, board, True, alpha, beta)
                board.undoMove(self)
                if value < best_value:
                    best_value = value
                    best_move = move
    ...
    def calculatePosition(self) -> tuple[float | int, str]:
        return Board.__minimax(self, Board, Board.mover)
