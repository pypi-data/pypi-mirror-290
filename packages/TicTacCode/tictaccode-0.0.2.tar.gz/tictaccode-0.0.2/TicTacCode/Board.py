...
class Board:
    ...
    def saveGame(self, promptOption: bool = True, saveFolder: str = 'Saved Games') -> None:
        if not Board.terminalState(self) and promptOption:
            prompt = input('Do you want to save this unfinished game? (Y/n)\n')
            if prompt.upper() != 'Y':
                return None
        now = datetime.datetime.now()
        with open(f'{saveFolder}/Saved Game at {now.year}.{now.month}.{now.day} {now.hour}.{now.minute}.{now.second}',
                  'w') as pgn:
            pgn.write(f'[Event "Play Against Self"]\n')
            pgn.write(f'[Date "{now.year}.{now.month}.{now.day}"]\n')
            match Board.winner(self):
                case 'X':
                    result = '1-0'
                case 'O':
                ...
                case None:
                    result = 'Game Ongoing'
            for name, info in [('X', 'xPlayer'), ('O', 'oPlayer'), ('Result', result)]:
                pgn.write(f'[{name} "{info}"]\n')
            match result:
                case '1-0':
                    pgn.write('[Termination "X Won By Connecting 3"]\n')
                case '0-1':
                    pgn.write('[Termination "O Won By Connecting 3"]\n')
                case '1/2-1/2':
                    pgn.write('[Termination "Game Tied Because No One Connected 3"]\n')
                case "Game Ongoing":
                    pgn.write('[Termination "Game Ongoing"]\n')
            for item in Board.moveStack:
                pgn.write(item + " ")
