...
def get_event_callback(ipython: InteractiveShell, event_name: str, callback_name: str) -> Callable | None:
...
def stop_listen():
    ipython = get_ipython()  # type: ignore
    if ipython:
        pre_execute_callback = get_event_callback(ipython, "pre_execute", "_capture_graph")
        if pre_execute_callback:
            ipython.events.unregister("pre_execute", pre_execute_callback)  # type: ignore

        post_execute_callback = get_event_callback(ipython, "post_execute", "_notebook_cell_content_update")
        if post_execute_callback:
            ipython.events.unregister("post_execute", post_execute_callback)  # type: ignore
...
class Autolog:
    ...
    def _log_vectice_asset(self, asset: TVecticeObjects) -> None:
        ...
        if isinstance(asset_to_log, Dataset):
            ...
            _logger.info(
                f"{asset_type} {object_name!r} named {asset_name_variable!r} as a variable {link} to iteration {self._iteration.index!r}."
            )
        else:
