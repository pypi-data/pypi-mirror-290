...
def get_connection() -> Connection:
    ...
    phase = LOGIN["phase"]
    if phase:
        from vectice.autolog.autolog_class import stop_listen

        stop_listen()
        _logger.warning(
            "Autolog has been disabled. You are now using the core Vectice API. Please run autolog.config() to use autolog again."
        )
