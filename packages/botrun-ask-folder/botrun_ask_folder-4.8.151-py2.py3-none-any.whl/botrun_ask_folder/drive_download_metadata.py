...
from botrun_ask_folder.models.collection_status import (
    CollectionStatus,
    CollectionStatusEnum,
)
from botrun_ask_folder.models.collection_item import CollectionItem
...
def init_collection_status(dic_result: dict, output_folder: str):
    """
    Initialize a CollectionStatus object and save it as a JSON file.
    """
    folder_id = output_folder.split("/")[-1]

    # Create CollectionItems from dic_result['items']
    collection_items = [CollectionItem(**item) for item in dic_result["items"]]

    # Create CollectionStatus
    collection_status = CollectionStatus(
        id=folder_id, status=CollectionStatusEnum.PROCESSING, items=collection_items
    )

    # Save CollectionStatus as JSON
    file_path = os.path.join(output_folder, f"{folder_id}-collection-status.json")
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(collection_status.model_dump_json(indent=4))
