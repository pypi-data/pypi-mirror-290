...
def query_qdrant_knowledge_base(
    ...
    open("./users/botrun_ask_folder/str_knowledge_base.txt", "w").write(
        str_knowledge_base
    )
    ...
    try:
        ...
        try:
            ...
            generate_ref = generate_ref_section(
                lst_refs, generated_page_name, include_ref_page
            )
            custom_log("INFO", f"Generated generate_ref: {generate_ref}")
            yield ModelResponse(
                stream=True,
                choices=[{"delta": {"content": generate_ref}, "finish_reason": None}],
            )
...
def generate_ref_section(
    lst_refs: List[Dict], generated_page_name: str, include_ref_page: bool
) -> str:
    if len(lst_refs) == 0:
        return ""
    ref_files = {}
