...
from botrun_ask_folder.status_tracker import LogStatus, capture_stdout, generate_log_viewer_html
...
def botrun_ask_folder(google_drive_folder_id: str,
    ...
    generate_log_viewer_html(output_path=f'./data/botrun_ask_folder/status_tracker/status.html', 
    ...
    print(f"Starting botrun_ask_folder for folder ID: {log_status.google_drive_folder_id}")
    # 把程式執行輸出資訊輸出到 log 檔案
    capture_stdout_botrun_ask_folder(google_drive_folder_id=google_drive_folder_id, 
                                     force=force, 
...
def capture_stdout_botrun_ask_folder(google_drive_folder_id: str,
                      force=False,
                      gen_page_imgs=False
                      ) -> None:
    """
    ...
    handle_downloaded_files_and_save_to_qdrant(google_drive_folder_id, force, gen_page_imgs, new_items)
...
def handle_downloaded_files_and_save_to_qdrant(
        google_drive_folder_id: str, force=False,
        gen_page_imgs=False, new_items: list = []):
    run_split_txts(
