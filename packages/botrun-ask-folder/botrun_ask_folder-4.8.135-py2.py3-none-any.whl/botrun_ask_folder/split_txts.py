...
from .drive_download_metadata import update_download_metadata_after_process_file, get_drive_download_metadata
...
def process_powerpoint(max_chars_per_file: int,
                       file_name: str,
                       file_type: str,
                       output_folder: str,
                       source_file: str,
    ...
    for slide_number, slide in enumerate(prs.slides):
        ...
        for paragraph in paragraphs:
            if current_char_count + len(paragraph) > max_chars_per_file and current_chunk:
                # 储存当前块并开始新的块
                try:
                    lst_rag_metadata.append(RagMetadata(
                        name=get_save_chunk_file_name(file_name, f"{get_page_number_prefix()}{slide_number + 1}",
                                                      f"{chunk_number:03d}"),
                        ori_file_name=f'{file_name}{file_type}',
                        gen_page_imgs=gen_page_imgs,
                        page_number=slide_number + 1,
                    ))
                    save_chunk(current_chunk, output_folder, file_name, f"{get_page_number_prefix()}{slide_number + 1}",
                               f"{chunk_number:03d}")
                except Exception as e:
                    print(f"Error saving chunk for slide {slide_number + 1}, chunk {chunk_number}: {e}")
...
def process_excel(original_file_path: str, output_folder: str, file_name: str, file_type: str,
                  max_chars_per_file: int = 5000) -> List[RagMetadata]:
    ...
    for sheet_name in xls.sheet_names:
        ...
        for row in content_list:
            if current_char_count + len(row) > max_chars_per_file and current_chunk:
                # 儲存當前塊並開始新的塊
                lst_rag_metadata.append(RagMetadata(
                    name=get_save_chunk_file_name(file_name,
                ...
                save_chunk(current_chunk, output_folder, file_name, f"{sheet_name}_{get_page_number_prefix()}_1_",
                           file_counter)
                current_chunk = []
                current_char_count = 0
                file_counter += 1
        ...
        if current_chunk:
            lst_rag_metadata.append(RagMetadata(
                name=f"{file_name}_{sheet_name}.{get_page_number_prefix()}{file_counter}.txt",
                ori_file_name=f'{file_name}{file_type}',
                gen_page_imgs=False,
                page_number=PAGE_NUMBER_NA,
                sheet_name=sheet_name,
            ))
            save_chunk(current_chunk, output_folder, file_name, sheet_name, file_counter)
    return lst_rag_metadata
...
def process_single_file(source_file: str,
    ...
    if file_type in ['.ppt', '.pptx']:
        lst_rag_metadata = process_powerpoint(chars_per_page, file_name, file_type,
                                              output_folder, source_file, gen_page_imgs=False, )
    if file_type in ['.xls', '.xlsx']:
        lst_rag_metadata = process_excel(source_file, output_folder, file_name, file_type)
    if file_type == '.ods':
        converted_file_path = convert_office_file(source_file, '.xlsx')
        lst_rag_metadata = process_excel(converted_file_path, output_folder, file_name, '.xlsx')
    ...
    if file_type == '.txt' or is_text_file(source_file):
        lst_rag_metadata = process_txt(chars_per_page, file_name, file_type, output_folder, source_file)
    final_files_count = len(list(Path(output_folder).glob('*')))
    stats['generated'] += (final_files_count - initial_files_count)
    return lst_rag_metadata


def process_txt(max_chars_per_file: int,
                file_name: str,
                file_type: str,
                output_folder: str,
                source_file: str) -> List[RagMetadata]:
    ...
    def save_current_chunk():
        ...
        if current_chunk:
            content = '\n'.join(current_chunk)
            lst_rag_metadata.append(RagMetadata(
                name=get_save_chunk_file_name(file_name, f"{get_page_number_prefix()}_1_", f"{file_counter:03d}"),
                ori_file_name=f'{file_name}{file_type}',
                gen_page_imgs=False,
                page_number=PAGE_NUMBER_NA,
                sheet_name="",
            ...
            save_chunk([content], output_folder, file_name, f"{get_page_number_prefix()}_1_", f"{file_counter:03d}")
            current_chunk = []
            current_char_count = 0
            file_counter += 1
...
def process_pdf(max_chars_per_file: int,
                file_name: str,
                file_type: str,
                output_folder: str,
                source_file: str,
    ...
    for page_number in range(len(doc)):
        ...
        for paragraph in paragraphs:
            if current_char_count + len(paragraph) > max_chars_per_file and current_chunk:
                # 儲存當前塊並開始新的塊
                lst_rag_metadata.append(RagMetadata(
                    name=get_save_chunk_file_name(file_name, f"{get_page_number_prefix()}{page_number + 1}",
                                                  f"{chunk_number:03d}"),
                    ori_file_name=f'{file_name}{file_type}',
                    gen_page_imgs=gen_page_imgs,
                    page_number=page_number + 1,
                ))
                save_chunk(current_chunk, output_folder, file_name, f"{get_page_number_prefix()}{page_number + 1}",
                           f"{chunk_number:03d}")
                current_chunk = []
                current_char_count = 0
                chunk_number += 1
...
def split_txts_no_threads(source_files: list,
    ...
    if new_items is not None:
        new_file_names = [truncate_filename(append_export_extension_to_path(item['name'], item['mimeType'])) for item in new_items]
    for index, (source_file, output_folder) in enumerate(zip(source_files, output_folders)):
        ...
        try:
            ...
            if metadata_dir and len(lst_rag_metadata) > 0:
                update_download_metadata_after_process_file(metadata_dir, lst_rag_metadata)
