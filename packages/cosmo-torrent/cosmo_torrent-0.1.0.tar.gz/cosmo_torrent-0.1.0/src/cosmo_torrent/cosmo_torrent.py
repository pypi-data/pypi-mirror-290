...
def download(url, target):
    try:
        ...
        with tqdm.tqdm(
            ...
            disable=True if total_size <=  10 * block_size else None,
        ) as progress_bar:
...
def _download_and_check_data(identifier, cache_folder, url):
    with tempfile.TemporaryDirectory() as download_folder:
        ...
        download(url.rstrip("/") + "/" + archive, downloaded_archive)

        md5 = identifier + ".md5"
        downloaded_md5 = os.path.join(download_folder, md5)
        download(url.rstrip("/") + "/" + md5, downloaded_md5)
...
def _download_data_archive(identifier, cache_folder):
    url = COSMO_TORRENT_BASE_URL.rstrip("/") + "/" + identifier + ".zip"
    urlretrieve(url, cache_folder)


if __name__ == "__main__":
    # prepare_upload(".", "ucat.test")
