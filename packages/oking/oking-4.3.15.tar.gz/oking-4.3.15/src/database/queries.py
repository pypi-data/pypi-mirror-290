    		...
    		USING (
    			SELECT
    				...
    				from RDB$DATABASE
                     ...
                     tmp.identificador2 = sem.identificador2)
...
def get_protocol_semaphore_id2_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        return 'update openk_semaforo.semaforo set data_sincronizacao = now(), mensagem = %s where identificador = %s and identificador2 = %s'
    elif connection_type.lower() == 'oracle':
        return 'update openk_semaforo.semaforo set data_sincronizacao = sysdate, mensagem = :1 where identificador = :2 and identificador2 = :3'
    elif connection_type.lower() == 'sql':
        return 'update openk_semaforo.semaforo set data_sincronizacao = getdate(), mensagem = ? where identificador = ? and identificador2 = ?'
    elif connection_type.lower() == 'firebird':
        return 'update semaforo set data_sincronizacao = CURRENT_TIMESTAMP, mensagem = ? where identificador = ? and identificador2 = ?'
...
def get_semaphore_command_data_sincronizacao(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return '''
                ...
                WHEN NOT MATCHED THEN
                INSERT (IDENTIFICADOR, IDENTIFICADOR2, TIPO_ID, DATA_ALTERACAO, DATA_SINCRONIZACAO, MENSAGEM)
                VALUES (tmp.identificador, tmp.identificador2, tmp.tipo_id, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, tmp.mensagem)
                '''
...
(
    ...
    primary key (identificador, identificador2),
  ...
  ACCOUNT UNLOCK'''
		...
		use openk_semaforo;
ALTER AUTHORIZATION ON DATABASE::[openk_semaforo] TO [openk];
...
insert into tipo ( nome) values ( 'PRODUTO');
insert into tipo ( nome) values ( 'ESTOQUE');
insert into tipo ( nome) values ( 'PRECO');
insert into tipo ( nome) values ( 'FOTO');
insert into tipo ( nome) values ( 'CLIENTE');
insert into tipo ( nome) values ( 'PEDIDO');
insert into tipo ( nome) values ( 'NOTA_FISCAL');
insert into tipo ( nome) values ( 'CONDICAO_PAGAMENTO');
insert into tipo ( nome) values ( 'REPRESENTANTE');
insert into tipo ( nome) values ( 'IMPOSTO');
insert into tipo ( nome) values ( 'LISTA_PRECO');
insert into tipo ( nome) values ('LISTA_PRECO_PRODUTO');
insert into tipo ( nome) values ('PLANO_PAGAMENTO_CLIENTE');
insert into tipo ( nome) values ('COLETA_DADOS_CLIENTE');
insert into tipo ( nome) values ('COLETA_DADOS_VENDA');
insert into tipo ( nome) values ('PRODUTO_RELACIONADO');
insert into tipo ( nome) values ('VENDA_SUGERIDA');
insert into tipo ( nome) values ('PRODUTO_CROSSELLING');
insert into tipo ( nome) values ('PRODUTO_LANCAMENTO');
insert into tipo ( nome) values ('PRODUTO_VITRINE');
insert into tipo ( nome) values ('COLETA_DADOS_COMPRA');
insert into tipo ( nome) values ('PEDIDO_PARA_OKVENDAS');
insert into tipo ( nome) values ('PONTO_FIDELIDADE');
...
(
    ...
    constraint fk_semaforo_tipo foreign key (tipo_id) references openk_semaforo.tipo (id)
        ...
        firebirdsql.connect(user=src.client_data['user'], password=src.client_data['password'],
...
def get_query_client_erp(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return 'SELECT ID FROM OPENK_SEMAFORO.CLIENTE WHERE CODIGO_REFERENCIA_ERP = :cliente_erp '
    elif connection_type.lower() == 'sql':
...
def get_query_cliente_cpfcnpj(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return 'SELECT ID FROM OPENK_SEMAFORO.CLIENTE WHERE CPF_CNPJ = :cliente_erp '
    elif connection_type.lower() == 'sql':
...
def get_insert_client_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        return '''insert into openk_semaforo.cliente 
                  ...
                  , end_principal_codigo_ibge, direcao )
        ...
        return '''insert into openk_semaforo.cliente 
                 (nome, razao_social, tipo_pessoa, cpf_cnpj, sexo, email, rg, orgao
                  ...
                  , end_principal_referencia_ent, end_principal_codigo_ibge, direcao )
            values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ) '''
    elif connection_type.lower() == 'firebird':
...
def get_update_client_sql(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        if src.client_data['operacao'].lower().__contains__('mplace'):
            newsql = newsql + 'WHERE cpf_cnpj = %s'
        else:
            newsql = newsql + 'WHERE codigo_referencia_erp = %s'
        return newsql
        ...
        return '''UPDATE OPENK_SEMAFORO.CLIENTE
                     SET nome = :1
                      ...
                      , rg = :7
                      , orgao = :8
                      , data_nascimento_constituicao = TO_DATE(:9, 'YYYY-MM-DD HH24:MI:SS')
                      , telefone_residencial = :10
                      , telefone_celular = :11
                      , inscricao_estadual = :12
                      , inscricao_municipal = :13
                      , codigo_referencia_erp = :14
                      , codigo_representante = :15
                      , end_principal_cep = :6
                      , end_principal_tipologradouro = :17
                      , end_principal_logradouro = :18
                      , end_principal_numero = :19
                      , end_principal_complemento = :20
                      , end_principal_bairro = :21
                      , end_principal_cidade = :22
                      , end_principal_estado = :23
                     , END_PRINCIPAL_REFERENCIA_ENT = :24
                     , end_principal_codigo_ibge = :25
                 WHERE CODIGO_REFERENCIA_ERP = :26 '''
    elif connection_type.lower() == 'sql':
        return '''update openk_semaforo.cliente
                    SET nome = ?
                      , razao_social = ?
...
def get_insert_order_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        return '''insert into openk_semaforo.pedido 
                         (pedido_oking_id, pedido_venda_id, pedido_canal, data_pedido, status, cliente_id, valor, valor_desconto
...
def get_query_order(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return 'SELECT ID FROM OPENK_SEMAFORO.PEDIDO WHERE PEDIDO_OKING_ID = :order_id'
    elif connection_type.lower() == 'sql':
...
def get_insert_order_items_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return '''insert into semaforo_itens_pedido (pedido_id, sku, codigo_erp, quantidade, ean, valor, valor_desconto, valor_frete) 
                        values (?, ?, ?, ?, ?, ?, ?, ?) '''
...
def get_query_non_integrated_order(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return 'SELECT ID FROM OPENK_SEMAFORO.PEDIDO WHERE PEDIDO_OKING_ID = :order_id AND DATA_SINCRONIZACAO IS NULL'
    elif connection_type.lower() == 'sql':
    ...
    if connection_type.lower() == 'mysql':
        return '''insert into openk_semaforo.pedido 
                 (pedido_oking_id, pedido_venda_id, pedido_canal, numero_pedido_externo, data_pedido, status, cliente_id
                  ...
                  , data_previsao_entrega, transportadora, opcao_forma_entrega, modo_envio, canal_id, loja_id, codigo_representante
                  ...
                  , descritor_pre_definido_2, descritor_pre_definido_3, descritor_pre_definido_4, descritor_pre_definido_5
        ...
        return '''insert into semaforo_pedido 
                          ...
                          , opcao_pagamento_erp, codigo_rastreio, data_previsao_entrega, transportadora, opcao_forma_entrega, modo_envio
...
def get_insert_b2b_order_items_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        return '''insert into openk_semaforo.itens_pedido 
                    ...
                    , filial_faturamento, cnpj_filial_venda) 
...
def get_order_protocol_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return '''UPDATE OPENK_SEMAFORO.PEDIDO 
                   ...
                   WHERE PEDIDO_OKING_ID = :order_id'''
    elif connection_type.lower() == 'sql':
        return '''update openk_semaforo.pedido 
...
def update_foto_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return '''UPDATE OPENK_SEMAFORO.PRODUTO 
                   ...
                   WHERE CODIGO_ERP_SKU = :codigo_erp_sku'''
    elif connection_type.lower() == 'sql':
        return '''update openk_semaforo.produto
...
#     elif connection_type.lower() == 'oracle':
#         return "UPDATE OPENK_SEMAFORO.PRODUTO SET data_sincronizacao = SYSDATE WHERE CODIGO_ERP_SKU = :codigo_erp_sku"
#     elif connection_type.lower() == 'sql':
...
#     if connection_type.lower() == 'mysql':
#         return '''insert into openk_semaforo.produto (codigo_erp, codigo_erp_sku, data_atualizacao, data_sincronizacao, data_envio_foto)
# 					values (%s, %s, %s, %s, %s) '''
#     elif connection_type.lower() == 'oracle':
#         return '''INSERT INTO OPENK_SEMAFORO.produto (PEDIDO_ID, SKU, CODIGO_ERP, QUANTIDADE, EAN, VALOR, VALOR_DESCONTO, VALOR_FRETE, CNPJ_FILIAL_VENDA, CODIGO_FILIAL_ERP, CODIGO_FILIAL_EXPEDICAO_ERP, CODIGO_FILIAL_FATURAMENT_ERP)
# 					VALUES (:1, :2, :3, :4, :5)'''
#     elif connection_type.lower() == 'sql':
#         return '''insert into openk_semaforo.produto (codigo_erp, codigo_erp_sku, data_atualizacao, data_sincronizacao, data_envio_foto)
# 					values (?, ?, ?, ?, ?) '''
...
def get_product_protocol_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        return 'update openk_semaforo.produto set data_sincronizacao = now(), data_envio_foto = now where codigo_erp = %s and codigo_erp_sku = %s'
    elif connection_type.lower() == 'oracle':
        return 'update openk_semaforo.produto set data_sincronizacao = SYSDATE, data_envio_foto = SYSDATE where codigo_erp = :codigo_erp and codigo_erp_sku = :codigo_erp_sku'
    elif connection_type.lower() == 'sql':
        return 'update openk_semaforo.produto set data_sincronizacao = getdate(), data_envio_foto = getdate() where codigo_erp = ? and codigo_erp_sku = ?'
    elif connection_type.lower() == 'firebird':
        return 'update semaforo_produto set data_sincronizacao = CURRENT_TIMESTAMP, data_envio_foto = CURRENT_TIMESTAMP where codigo_erp = ? and codigo_erp_sku = ?'
...
def get_protocol_semaphore_id3_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return 'update semaforo set data_sincronizacao = CURRENT_TIMESTAMP where identificador = ? and tipo_id = ?'
    ...
    if connection_type.lower() == 'mysql':
        ...
        return '''
            insert into semaforo_cliente (nome, razao_social, cpf, cnpj, email, telefone_residencial, telefone_celular,
                                cep, tipo_logradouro, logradouro, numero, complemento, bairro, cidade, estado, referencia,
                                cliente_erp, direcao, data_sincronizacao, data_alteracao, codigo_ibge, inscricaoestadual)
            value (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, null, %s, %s, null, %s, %s)
            on duplicate key update
                data_alteracao = values(data_alteracao),
