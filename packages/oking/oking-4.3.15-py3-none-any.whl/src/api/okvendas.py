...
from src.entities.response import InvoiceResponse, CatalogoResponse, OkvendasResponse, ListaPrecoResponse, \
    ProductTaxResponse, RepresentativeResponse, ClientResponse, OkvendasEstoqueResponse
...
def post_prices(price: List):
    try:
        ...
        send_log('envia_preco_job', False, False, f'== Preço Enviado', 'info', 'PRECO', '')
        return result
...
def get_product_by_code(code) -> (ResponseProductByCode, str):
    try:
        ...
        if response.status_code == 404:
            ...
            return None, f"{result['message']} código sku: {code}" if 'message' in result else f'Não foi encontrado o produto: {code} na api okvendas'
...
def put_price_lists_products(price_list_code: str, body: str) -> bool:
    try:
        url = f'{src.client_data.get("url_api_principal")}/catalogo/listapreco/preco/lote?price_list_code={price_list_code}'
        headers = {'Content-type': 'application/json',
                   'Accept': 'application/json',
                   'access-token': src.client_data.get('token_api_integracao')}
        json_body = jsonpickle.encode(body, unpicklable=False)
    ...
    try:
        ...
        if response.ok:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     ...
                     f'não configurado no painel do OKVENDAS.', 'warning',
                     'envia_pedido_to_okvendas_job', body["numero_pedido_externo"])
            # return [OkvendasResponse(**r) for r in response.json()]
            return result
        else:
            logger.error(
    ...
    try:
        ...
        if response.ok:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     ...
                     + response_data, 'error', 'envia_pontos_to_okvendas_job', 'PONTOS_PARA_OKVENDAS')
            return False

    except Exception as e:
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True
                 , f'Erro {str(e)}', 'error', 'envia_pontos_to_okvendas_job', 'PONTOS_PARA_OKVENDAS')
        return False
