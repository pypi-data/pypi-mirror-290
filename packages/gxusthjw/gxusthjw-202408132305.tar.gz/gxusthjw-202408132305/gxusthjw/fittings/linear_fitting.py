...
# 基于statsmodels的线性拟合 ------------------------------------------
class LinearFittingMethodOfSm(Enum):
    """
    枚举类`LinearFittingMethodOfSm`表征“statsmodels模块提供的线性拟合方法”。
    """
...
def linear_fitting_sm(y: npt.ArrayLike,
                      ...
                      method: Union[str, LinearFittingMethodOfSm] = "ols",
                      **kwargs):
    """
    ...
    if isinstance(method, LinearFittingMethodOfSm):
        if method == LinearFittingMethodOfSm.OLS:
            fitting_res = sm.OLS(y_var, x_var_ff).fit()
        elif method == LinearFittingMethodOfSm.RLM:
...
def interactive_linear_fitting_sm(y: npt.ArrayLike,
                                  ...
                                  length: Optional[int] = None):
    """
    交互式地基于statsmodels提供的线性拟合方法（OLS或RLM）对指定的数据（y，x）执行拟合。

    :param y: 因变量。
    :param x: 自变量。
    :param start: 要拟合数据的起始位置索引（初始值）。
    :param length: 要拟合数据的索引长度（初始值）。
    :return: 拟合结果，
    """
    ys = np.asarray(y)
    ys_len = ys.shape[0]
    ...
    def update():
        ...
        fitting_res, x_var, y_var = _linear_fitting_sm(ys, xs,
                                                       start_selected,
                                                       length_selected,
                                                       method_selected)
