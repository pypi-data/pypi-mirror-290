...
from ._rotary_emb import precompute_freqs_cis, apply_rotary_emb, apply_rotary_emb_self


class EmbeddingLayer(nn.Module):
	
	def __init__(self, embed_dim: int, vocab_size: Optional[int] = None, embedding=None, padding_idx: Optional[int] = None):
		"""
...
class CNNEmbedding(nn.Module):
	...
	def __init__(self, embed_dim: int, seq_length: int = 16, out_channels: int = None, kernel_sizes=(2, 3, 4),
	             activation=None, embedding=None, vocab_size: int = None, batch_norm=False, bias=False,
...
class RotaryEmbedding(nn.Module):
	...
	def forward(self, inputs: torch.Tensor, start_pos: int = 0):
		...
		self.freqs_cis = self.freqs_cis.to(inputs.device)
		freqs_cis = self.freqs_cis[start_pos: start_pos + seq_length]
		query = self.qw(inputs)
		key = self.kw(inputs)
...
class SelfRotaryEmbedding(nn.Module):
	...
	def forward(self, inputs: torch.Tensor, start_pos: int = 0):
		...
		self.freqs_cis = self.freqs_cis.to(inputs.device)
		freqs_cis = self.freqs_cis[start_pos: start_pos + seq_length]
		return apply_rotary_emb_self(inputs, freqs_cis=freqs_cis)
