...
def _silent(func: callable) -> callable:
    ...
    """
    def wrapper(self, *args, **kwargs): # pylint: disable=inconsistent-return-statements
        ...
        try:
            ...
            if self._errors is None:
                self._errors = {}
            self._errors['init'] = deepcopy(error) # pylint: disable=protected-access
...
class MetaClass(type):
    ...
    def __call__(cls, *args: tuple, **kwargs: dict) -> Self:
        ...
        try:
            obj._is_frozen = obj._config.frozen # pylint: disable=attribute-defined-outside-init
        except AttributeError:
            pass
    ...
    def __new__(mcs, name: str, bases: tuple, attrs: dict) -> Self:
        ...
        Example:
            ...
            >>> obj1 = MyClass()
            >>> obj2 = MyClass()
            >>> MyClass._config.value, obj1._config.value, obj2._config.value
            (1, 1, 1)

            >>> obj1._config.value = 3
            >>> MyClass._config.value, obj1._config.value, obj2._config.value
            (3, 3, 3)
        ...
        for attr_name, attr_value in attrs.items():
            ...
            elif not attr_name.startswith('__') and \
...
class BaseObject(_BaseObject):
    ...
    def to_dict(self, add_class_path: bool = False, recursion: bool = False) -> dict:
        ...
        for key, value in self.__dict__.items():
            if key not in self._config.exclude_keys:
                key = self._config.key_mapping.get(key, key)
        ...
        if add_class_path:
            dct['__class_path__'] = self.get_full_doted_class_path()
...
class BaseDict(BaseObject):
    ...
    def __setitem__(self, key: str, item: Any, caller: str = None) -> None:
        ...
        if key in self._config.keys_blacklist:
    ...
    def __setstate__(self, state: dict) -> None:
        ...
        """
        if not hasattr(self, '__data__'): # NOTE: coverage not passing here since `self` will always have __data__ attribute
            self.__data__ = {} # pylint: disable=attribute-defined-outside-init

        # Do not use self.update because eu need to create all attributes too
    ...
    def is_valid_key(self, key: str) -> bool:
        ...
        are not in the self._config.keys_blacklist.
        ...
        """
        return not key.startswith('_') and key not in self._config.keys_blacklist
