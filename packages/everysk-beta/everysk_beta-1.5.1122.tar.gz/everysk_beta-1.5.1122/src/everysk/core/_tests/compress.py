...
class CompressZlibJsonTestCase(TestCase):
    ...
    def test_date_format(self):
        obj = Date.today()
        with mock.patch.dict('os.environ', {'SERIALIZE_DATE_FORMAT':'%Y%m%d'}):
            obj_compressed = compress(obj, protocol='zlib', serialize='json')
            self.assertEqual(decompress(obj_compressed, protocol='zlib', serialize='json'), obj.strftime('%Y%m%d'))

    def test_datetime_format(self):
        obj = DateTime.now()
        with mock.patch.dict('os.environ', {'SERIALIZE_DATETIME_FORMAT':'%Y%m%d %HH:%M:%S'}):
            obj_compressed = compress(obj, protocol='zlib', serialize='json')
            self.assertEqual(decompress(obj_compressed, protocol='zlib', serialize='json'), obj.strftime('%Y%m%d %HH:%M:%S'))
...
class CompressZlibPickleTestCase(TestCase):
    ...
    def test_compress(self):
        string = 'aa aa aa aa aa aa aa aa'
        self.assertEqual(compress(string, protocol='zlib', serialize='pickle'), self.str_repr)

    def test_decompress(self):
        self.assertEqual(decompress(self.str_repr, protocol='zlib', serialize='pickle'), 'aa aa aa aa aa aa aa aa')
...
class FileHandlingTestCase(TestCase):
    ...
    def test_date_format(self):
        ...
        with mock.patch.dict('os.environ', {'SERIALIZE_DATE_FORMAT':'%Y%m%d'}):
            obj_compressed = compress(obj, protocol='zlib', serialize='pickle')
            self.assertEqual(decompress(obj_compressed, protocol='zlib', serialize='pickle'), obj)
...
class CompressGzipJsonTestCase(TestCase):
    ...
    def test_date_format(self):
        ...
        with mock.patch.dict('os.environ', {'SERIALIZE_DATE_FORMAT':'%Y%m%d'}):
            obj_compressed = compress(obj, protocol='gzip', serialize='json')
            self.assertEqual(decompress(obj_compressed, protocol='gzip', serialize='json'), obj.strftime('%Y%m%d'))

    def test_datetime_format(self):
        obj = DateTime.now()
        with mock.patch.dict('os.environ', {'SERIALIZE_DATETIME_FORMAT':'%Y%m%d %HH:%M:%S'}):
            obj_compressed = compress(obj, protocol='gzip', serialize='json')
            self.assertEqual(decompress(obj_compressed, protocol='gzip', serialize='json'), obj.strftime('%Y%m%d %HH:%M:%S'))
...
class CompressGzipPickleTestCase(TestCase):
    ...
    def test_date_format(self):
        ...
        with mock.patch.dict('os.environ', {'SERIALIZE_DATE_FORMAT':'%Y%m%d'}):
            obj_compressed = compress(obj, protocol='gzip', serialize='pickle')
            self.assertEqual(decompress(obj_compressed, protocol='gzip', serialize='pickle'), obj)
