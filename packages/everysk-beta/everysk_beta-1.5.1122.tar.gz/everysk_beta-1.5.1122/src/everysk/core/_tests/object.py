...
from everysk.core.object import BaseField, BaseDict, BaseObject, BaseDictConfig, _required, _validate, MetaClass
...
class BaseObjectTestCase(TestCase):
    ...
    def test_copy_config(self):
        obj1 = BaseObject()
        obj2 = copy(obj1)
        self.assertEqual(obj1._config, obj2._config)
    ...
    def test_get_state(self):
        ...
        self.assertEqual(the_pickle.__dict__, {})
    ...
    def test_pickle_with_data(self):
        ...
        for key, value in obj.__get_attributes__().items():
    ...
    def test_to_dict_class(self):
        obj = BaseObject(_private=1, public=2, obj=BaseObject(attr=1))
        self.assertDictEqual(obj.to_dict(add_class_path=True), {'__class_path__': 'everysk.core.object.BaseObject', '_private': 1, 'public': 2, 'obj': obj.obj})
...
class MetaClassConfigTestCase(TestCase):
    ...
    def setUp(self) -> None:
        # pylint: disable=no-member
        BaseObjectConfig._config.value = 1
        TestBaseDictConfig._config.value = 1

    def test_object_config_attribute(self):
        # pylint: disable=no-member
        self.assertEqual(BaseObjectConfig._config.value, 1)
        obj = BaseObjectConfig()
        self.assertEqual(obj._config.value, BaseObjectConfig._config.value)

    def test_object_config_singleton(self):
        # pylint: disable=no-member
        obj = BaseObjectConfig()
        obj._config.value = 3
        self.assertEqual(BaseObjectConfig._config.value, 3)
        self.assertEqual(obj._config.value, BaseObjectConfig._config.value)
...
class MetaClassAttributesTestCase(TestCase):
    ...
    def test_class_attributes(self):
        ...
        self.assertDictEqual(
            attrs,
            {
                '_config': BaseObject._config.__class__,
                '_errors': dict,
                '_is_frozen': bool,
                '_silent': bool,
                'p01': BaseField(attr_type=str, default='Parent field'),
