...
class ExpressionTestCase(TestCase):
    ...
    def test_get_tokens(self):
        expected_data = compress({'class_name': 'Expression', 'method_name': 'get_tokens', 'self_obj': {'__class_path__': 'everysk.sdk.engines.expression.Expression'}, 'params': {'expression': 'a + b', 'data_types': ['cpp_var', 'str_var']}}, protocol='gzip', serialize='json')
        engine = Expression()
    ...
    def test_solve_method(self):
        expected_data = compress({'class_name': 'Expression', 'method_name': 'solve', 'self_obj': {'__class_path__': 'everysk.sdk.engines.expression.Expression'}, 'params': {'expression': 'a + b', 'user_args': {'a': 1, 'b': 2}}}, protocol='gzip', serialize='json')
        engine = Expression()
