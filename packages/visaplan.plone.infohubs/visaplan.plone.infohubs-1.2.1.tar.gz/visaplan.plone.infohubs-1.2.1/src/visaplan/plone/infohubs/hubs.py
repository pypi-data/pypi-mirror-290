...
try:
    pkg_version('visaplan.plone.tools')
except PackageNotFoundError:
    HAS_VISAPLAN_TOOLS = False
    # Logging / Debugging:
    from logging import getLogger
    logger = getLogger('visaplan.plone.infohubs:hubs')
    debug_active = DevelopmentMode
else:
...
try:
    pkg_version('visaplan.zope.reldb')
except PackageNotFoundError:
    HAS_VISAPLAN_RELDB = False
    try:
        pkg_version('visaplan.plone.sqlwrapper')
    except PackageNotFoundError:
...
NAMED_ADAPTERS = {  # abbreviations for getToolByName ...
    # from tomcom.adapter:
    'acl':      'acl_users',
    ...
    # from visaplan.plone.adapters:
    'puc':      'portal_user_catalog',
    }
KNOWN_TOOLS.update(NAMED_ADAPTERS.values())  # noqa
looksLikeATool = make_toolDetector(known=KNOWN_TOOLS,
                                   blacklist=['plone_portal_state',
                                              'plone_context_state',
                                              ],
...
if HAS_VISAPLAN_TOOLS:
    NAMED_ADAPTERS.update({
        ...
        # (tool, method) tuples not supported:
        # we can get away quite well with our information reuse chain
        # (for portal, use info['portal_url'] or info['portal_object'])
...
def make_hubs(context, debug=False, ordered=False):
    """
    ...
    # might be patched, so we must not import this too early:
    # Zope:
    from Products.CMFCore.utils import getToolByName

    parent_class = (OrderedDict if ordered
                    else dict)

    class ToolsHub(parent_class):
        """
        ...
        def __getitem__(self, key):
            try:
                return parent_class.__getitem__(self, key)
            except KeyError:
                contextonly = 0
                if 0 and key == 'plone_portal_state':
                    pp(key=key)
                    set_trace()
                if looksLikeATool(key):
                    method = getToolByName
                elif key in NAMED_ADAPTERS:
                ...
                if not contextonly:
                    args.append(key)
                try:
                    # possible problem with patched getToolByName ...
                    val = method(*args)
                except AttributeError as e:
                    logger.error('Error getting hub[%(key)r]!', locals())
                    logger.error('method is %(method)r', locals())
                    logger.exception(e)
                    if debug_active:
                        pp(args=args)
                        set_trace()
                ...
                parent_class.__setitem__(self, key, val)
                return parent_class.__getitem__(self, key)
    ...
    def detect_group_title():
        ...
        if not info['gid']:
            return None
        return groupinfo_factory(context, pretty=1, forlist=1
                                 )(info['gid']
                                   )['group_title']
    ...
    def detect_portal_url():
        return info['portal_object'].absolute_url()

    def detect_portal_object():  # ... sic:
        return hub['portal_url'].getPortalObject()
    ...
    def detect_current_language():
        return (info['request'].get('LANGUAGE')
                or hub['portal_languages'].getDefaultLanguage()
                )
    ...
    FUNCMAP = {  # Objektinformationen:
               ...
               'group_title': detect_group_title,          # requires v.p.tools
               'managed_group_title': managed_group_title, # requires v.p.tools
               ...
               'is_member_of': get_is_member_of,  # requires v.p.tools
