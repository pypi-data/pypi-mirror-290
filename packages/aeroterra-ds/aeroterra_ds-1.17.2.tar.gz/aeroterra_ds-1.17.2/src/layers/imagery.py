...
def get_imagery_layer(link=None, layer_id=None, gis=None):
    if link is not None:
        return ImageryLayer(link)
    elif layer_id is not None:
        if gis is None:
            raise Exception("If A Layer Id Is Provided, You Must Include a Gis")
...
def clip_imagery(layer, bounds, crs, folder, size=None, delete_clips=False, quality=1, join=True, bands=None, file="joined.tif", complete_borders=False):
    """
