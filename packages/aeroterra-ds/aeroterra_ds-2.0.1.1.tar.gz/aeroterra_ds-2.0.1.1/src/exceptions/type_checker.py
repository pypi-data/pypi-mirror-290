            ...
            if origin in (list, tuple, set):
                ...
                for sub_type in possible_types:
                    try:
                        ...
                        continue
                if not matches:
                    raise TypeException(f"[{param_name}] Expected type {expected_type.__name__} or None, but got {type(param_value).__name__}.")
            else:
...
# @type_checker
# def example_function(a: int, b: Optional[List[int]], c: Optional[int] = None):
#     print(a, b)
