    ...
    def unwrap_optional(expected_type):
        ...
        if hasattr(expected_type, '__origin__') and expected_type.__origin__ is Union:
            ...
            if type(None) in args:
                # Remove type(None) from the union and return the rest
        ...
        if isinstance(expected_type, type):
            ...
            if origin in (list, tuple, set):
                ...
                for sub_type in possible_types:
                    try:
                        check_type(param_value, sub_type)
                        matches = True
                        break
                    except TypeException:
                        continue
                if not matches:
                    raise  TypeException(f"Expected type {expected_type.__name__} or None, but got {type(param_value).__name__}.")
            else:
...
# @type_checker
# def example_function(a: int, b: Optional[List[int]]):
#     print(a, b)
