...
class MultiConv3D(nn.Module):
    ...
    It includes a residual connection that adds the input directly to the output
    of the convolutional blocks if their dimensions match.
    ...
    normalization : str, optional
        Type of normalization to use. Options include 'batch', 'layer', or 'instance'.
        Default is 'layer'.
    ...
    forward(x)
        Defines the computation performed at every call. Applies the convolution
        blocks to the input tensor, adds the result to the output of the skip
        connection, and returns the final output.
    """

    def __init__(self, in_channels, out_channels, num_convs=2, normalization='layer'):
    ...
    def forward(self, x):
        """
        Applies the module's operations to the input tensor and returns the output.
        ...
        -------
        torch.Tensor
            The output tensor after applying the convolution blocks and the residual connection.
        """
