...
def create_app(path: Path) -> MyFastAPI:
    ...
    def predict(
        ...
        # result = await runner.predict(id, pred.predict, request_body)
        result = pred.predict(**request_body["input"])
        end_time = datetime.now()
        time_taken = (end_time - start_time).total_seconds()
        final_result = upload_files(
            result, upload_file=lambda fh: upload_file(fh, "")  # type: ignore
        )
        return JSONResponse(
    ...
    def predic_with_id(
        ...
        time_taken = (end_time - start_time).total_seconds()
        final_result = upload_files(
            result, upload_file=lambda fh: upload_file(fh, "")
        )
        logger.debug(result)
