...
@app.command()
def init():
    download_file('https://rockai-web-resouce-bucket.s3.ap-northeast-1.amazonaws.com/rock.yaml',Path.cwd() / 'rock.yaml')
    download_file('https://rockai-web-resouce-bucket.s3.ap-northeast-1.amazonaws.com/predictor.py',Path.cwd() / 'predictor.py')
    download_file('https://rockai-web-resouce-bucket.s3.ap-northeast-1.amazonaws.com/dockerignore',Path.cwd() / '.dockerignore')
...
def login_to_docker(api_token: Annotated[str, typer.Argument(help="Your API token")],debug: Annotated[bool, typer.Option(help="enable debug mode")] = False):
