...
@click.option('--workspace-id', required=False, help='当前TAPD项目空间id')
def config(tester, base_url, workspace_id):
    """配置测试人员、API Base Url"""
    if tester:
    ...
    if workspace_id:
        ...
        click.echo(click.style(f'当前配置的 workspace id：{workspace_id}', fg='bright_cyan', bold=True))
    if tester or base_url or workspace_id:
        ...
        click.echo(click.style(f'workspace_id={CONFIG_DATA.get("workspace_id") or "未配置"}', fg='bright_cyan', bold=True))
        click.echo(
            click.style(f'请输入 unicase config --tester <name> --base-url <base url> --workspace-id <workspace_id> 进行配置', fg='bright_cyan',
                        bold=True))


def request_(method: str, url, **kwargs):
    if method.lower() == 'get':
        if kwargs.get('params'):
            kwargs['params']['workspace_id'] = CONFIG_DATA.get('workspace_id')
    elif method.lower() == 'post':
        if kwargs.get('data'):
            kwargs['data']['workspace_id'] = CONFIG_DATA.get('workspace_id')
...
def create(ctx, name):
    ...
    if choice:
        ...
        if target_file_path.exists():
            target_file_path = Path(str(target_file_path).rsplit('.')[0] + str(int(time.time())) + '.xlsm')
        iteration_id = iterations[iteration_name]
        tapd.set_iteration_id(iteration_id)

        excel = utils.Excel(utils.TEMPLATE_XLS, tapd)
        # Excel Sheet 表面最长只能 31 个字符，超出会自动截断，并提示内容丢失, sheet 表名不能含 / 字符
        excel.set_sheet_name(utils.secure_filename(iteration_name[:25] + (iteration_name[:25] and '...')))
