...
def get_(url, params=None, **kwargs):
    if params:
        params['workspace_id'] = CONFIG_DATA.get('workspace_id')
    return temp_get(url, params, **kwargs)

def post_(url, data=None, json=None, **kwargs):
    if data:
        data['workspace_id'] = CONFIG_DATA.get('workspace_id')
    return temp_post(url, data, json, **kwargs)
...
class Tapd:
    ...
    def get_tapd_test_plans(self):
        """custom_field_2 为迭代 ID, custom_field_3 需求ID，以需求ID为key, 测试计划ID为value"""
        url = urljoin(self.base_url, '/tapd/origin/test_plans')
        response = requests.get(url, params={'status': 'open', 'custom_field_2': self.iteration_id,
                                             'fields': 'id,custom_field_3'})
        data = [(item['TestPlan']['custom_field_3'], item['TestPlan']['id']) for item in
                response.json().get('data', []) if item['TestPlan']['custom_field_3']]
        return OrderedDict(data)
    ...
    def post_tapd_task(self, **kwargs):
        ...
        kwargs.setdefault('workspace_id', '20426489')
            ...
            for row_index in bar:
                ...
                if self.is_valid_case(custom_field_1_cell, title_cell):
                    ...
                    if case_id:
                        ...
                        relation_story_id = stories_map.get(self.case_ws[story_cell].value)
                        test_plan_id = test_plans_map.get(relation_story_id)
