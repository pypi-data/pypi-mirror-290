...
def _traverse(
    ...
    if "[" in current_segment and "]" in current_segment:
        ...
        if isinstance(obj, Mapping):
            matching_keys = [k for k in obj.keys() if re.fullmatch(current_segment, str(k))]
            if debug:
                print(f"Matching keys for pattern '{current_segment}': {matching_keys}")
            if len(matching_keys) == 1:
                obj = obj[matching_keys[0]]
                current_path = f"{current_path}.{matching_keys[0]}" if current_path else matching_keys[0]
            elif len(matching_keys) > 1:
                return [_traverse(obj[k], remaining_segments, default, debug, f"{current_path}.{k}", return_paths) for k in matching_keys]
            else:
                if debug:
...
#     if isinstance(obj, dict):
#         return {k: filter_nulls(v) for k, v in obj.items() if v}
#     elif isinstance(obj, list):
#         return [filter_nulls(v) for v in obj if v]
#     else:
#         return obj
def filter_nulls(obj):
    if isinstance(obj, dict):
        return {k: v for k, v in ((k, filter_nulls(v)) for k, v in obj.items()) if v not in (None, {}, [])}
    elif isinstance(obj, list):
