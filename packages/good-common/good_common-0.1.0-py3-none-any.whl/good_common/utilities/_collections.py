...
def to_dict(obj: typing.Any) -> dict:
    ...
    if hasattr(obj, "model_export"):
        ...
        if isinstance(obj, type):
            ...
            return {field.name: getattr(field, 'default', None) for field in dataclasses.fields(obj)}
        else:
...
def recursive_convert_lists(obj: dict | list | typing.Any) -> dict | list | typing.Any:
    ...
    if isinstance(obj, dict):
        if all(isinstance(k, str) and k.isdigit() for k in obj.keys()):
            return [recursive_convert_lists(v) for _, v in sorted(obj.items(), key=lambda x: int(x[0]))]
...
#     Args:
#         obj (dict): Object to index.
#         keys_as_tuples (bool, optional): Whether to use tuples as keys. Defaults to False.

#     Returns:
#         dict: Indexed representation of the object.
#     """
...
#     return {
#         path_tuple_to_string(path) if not keys_as_tuples else path: value
#         for path, value in _index_object(obj)
#     }
...
# def index_object(obj: typing.Any, prefix: str = '') -> dict[str, typing.Any]:
#     if isinstance(obj, dict):
#         return {f"{prefix}{key}": index_object(value, f"{prefix}{key}.") 
#                 for key, value in obj.items() if value is not None}
#     elif isinstance(obj, list):
#         return {f"{prefix}[{i}]": index_object(value, f"{prefix}[{i}].") 
#                 for i, value in enumerate(obj) if value is not None}
#     else:
#         return obj
...
#     Args:
#         obj (dict[str, TVal]): Indexed object to convert.

#     Returns:
#         dict[str, typing.Any]: Nested form of the object.
#     """
#     # return _deindex_object(
#     #     {path_string_to_tuple(key): value for key, value in obj.items()}
#     # )
#     return _deindex_object(
#         {path_string_to_tuple(key): value for key, value in obj.items()}
#     )
...
def path_string_to_tuple(path: str) -> tuple:
    ...
    return tuple(int(i[1:-1]) if i.startswith("[") and i.endswith("]") else i for i in path.split("."))


type TKey = str | int
type TVal = str | int | float | bool | None | dict[typing.Any, typing.Any] | list[typing.Any]
type TReturn = dict[TKey, typing.Any] | list[typing.Any] | str | int | float | bool | None
...
#     Args:
#         obj (dict[tuple[TKey, ...], TVal]): Indexed object to convert.

#     Returns:
#         TReturn: Nested form of the object, which can be:
...
#         _set_nested_dict(result, key, value)
#     return _numeric_dicts_to_list(result)
...
def _set_nested_dict(d: dict[TKey, typing.Any], keys: tuple[TKey, ...], value: typing.Any) -> None:
    """
    ...
    if isinstance(d, list):
        if not isinstance(keys[-1], int):
            raise TypeError(f"List indices must be integers, not {type(keys[-1]).__name__}")
        while len(d) <= keys[-1]:
            d.append({})
        d[keys[-1]] = value
    else:
...
class DotDict(MutableMapping):
    ...
    def update(self, other: DictLike | None = None, **kwargs: typing.Any) -> None:
...
#     This class extends the standard python dictionary to allow accessing
#     and setting items using dot notation (e.g., `d.key` instead of `d['key']`).

#     Args:
#         init_dict (DictLike | None, optional): Initial dictionary to populate the DotDict. Defaults to None.
#         **kwargs: Additional key-value pairs to add to the DotDict.
...
#     def __repr__(self) -> str:
#         if len(self) > 0:
#             return f"<{type(self).__name__}: {', '.join(sorted(repr(k) for k in self.keys()))}>"
#         else:
...
#             default (typing.Any, optional): Default value to return if the key is not found. Defaults to None.

#         Returns:
#             typing.Any: The value associated with the key, or the default value if not found.
#         """
...
#         Returns:
#             DotDict: A new DotDict instance with copied key-value pairs.
#         """
...
#         Returns:
#             dict: A regular dictionary representation of the DotDict.
#         """
#         return typing.cast(dict, as_nested_dict(self, dct_class=dict))
