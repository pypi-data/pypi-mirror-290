...
def init_config():
    ...
    # Copy template files from the package resources
    template_files = ["template-commit-message.txt", "template-pull-request.txt"]
    for template_file in template_files:
        with pkg_resources.path(flowai, template_file) as template_path:
            shutil.copy(template_path, prompts_dir / template_file)
...
def main(
    ...
    if not config_manager.config_exists():
        console.print("[bold yellow]It looks like this is your first time running FlowAI. Let's set it up![/bold yellow]")
        init_config()
        return
    ...
    if version:
        console.print(f"FlowAI version: {__version__}")
        raise typer.Exit()
    ...
    if not (list_models or prompt or select_prompt_file):
    ...
    if list_models:
        ...
        for provider, provider_models in models.items():
            console.print(f"\n[bold blue]{provider.capitalize()}:[/bold blue]")
            if provider_models and provider_models[0] not in ["No API key set", "Error fetching models"]:
                for model in provider_models:
                    console.print(f"  {model}")
        
        # Print helpful links for providers with missing API keys or errors
        console.print("\n[bold yellow]Providers with missing API keys or errors:[/bold yellow]")
        for provider, url in provider_urls.items():
            if provider not in models or models[provider][0] in [f"{provider}:No API key set", "Error fetching models"]:
                console.print(f"[yellow]{provider.capitalize()}: {url}[/yellow]")
        return

    if model:
        # Fetch the latest models from the providers
        available_models = get_available_models(config)
        ...
        if model not in all_models:
            ...
            all_models_lower = [m.lower() for m in all_models]
            
            # Find close matches
            close_matches = get_close_matches(model_lower, all_models_lower)
            
            # Find partial matches
            partial_matches = [m for m in all_models if model_lower in m.lower()]
            
            # Combine and deduplicate matches
            combined_matches = list(set(close_matches + partial_matches))
            
            if combined_matches:
                console.print(f"[bold red]Model '{model}' not found. Did you mean:[/bold red]")
                for match in combined_matches:
                    console.print(f"  [yellow]{match}[/yellow]")
            else:
                console.print(f"[bold red]Model '{model}' not found and no close matches available.[/bold red]")
            raise typer.Exit(code=1)
    else:
        model = config.get('DEFAULT', 'default_model')
        if not model or ':' not in model:
            console.print("[bold red]Error: No valid model set. Please run 'flowai --init' or use --model to set a model.[/bold red]")
            raise typer.Exit(code=1)
    ...
    if flow:
        ...
        if stream:
            ...
            result = task_manager.compile_results(template, sections, full_prompt, quiet=debug, debug=debug)
    ...
    print(f"[#555555]Model used: {llm_connector.model} | Input tokens: {llm_connector.input_tokens} | Output tokens: {llm_connector.output_tokens} | Elapsed time: {elapsed_time:.3f}s[/#555555]", file=sys.stderr)
