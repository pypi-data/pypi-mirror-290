...
def force_center_text( frame, overtext , posy = 400, fg_rgb = (255,255,255) , fosize = 2*32):
    """
    ...
    frame = iprint( frame, drtext=str(overtext), font=FONT, position=(midl,posy), color_rgb=fg_rgb)
    return frame
...
class widget:
    ...
    """
    def __init__(self,connector=1, typ = "signal", pos = (580,60), timeout=1, adaptive_position=True):
        ...
        if self.connector == 1: self.inputi = config.CONFIG['mminput1']
            ...
            print("X... nonexisting widget connector")
            sys.exit(1)
        ...
        #
        mmapwr.mmcreate( self.inputi  )
        #print("******************  mm created",config.CONFIG["mminput1"])
    ...
    def refresh(self):
        ...
        value;min;max;
        """
        input1,input1ext = mmread_n_clear( self.inputi )
        input1 = input1.strip()
        ...
        if (input1 is not None) and (type(input1) == str) and (input1 in ['signal', 'dial', 'box', 'sub', 'tacho']):
            ...
            if len(res)>3: # timeout
                ...
                if timo>0.8 and timo<3600: # between 1 sec and 1 hour
                    self.timeout = timo
                    print("X... changing  timeout (1-3600sec)", timo)
                else:
                    print("X... uneligible timeout (1-3600sec)", timo)
    ...
    def display(self, frame ):
        ...
        #if self.is_active():
        if self.typ == "signal":
            frame = signal_strength( frame, position=cposition , percentage= self.value0100 , value=self.value, color=dcolor, title=self.title)
        if self.typ == "dial":
            frame = dial( frame, position=cposition , percentage= self.value0100 , value=self.value , color=dcolor, title=self.title)
        if self.typ == "tacho":
            frame = tacho( frame, position=cposition , percentage= self.value0100 , value=self.value , color=dcolor, title=self.title)
        if self.typ == "box":
            frame = text_box( frame, position=cposition , title = self.title  , values = str(self.value).replace(",","\n") , color=dcolor)
        if self.typ == "sub":
            #print("D... STARTING SUB PRINT")
            frame = iprint( frame, self.value , MSG_FONT,
...
class Camera(BaseCamera):
    ...
    def acquire_one_frame(cap):
        ...
        if not ret:
            ...
            frame = force_center_text( frame, overtext, posy = 200)
    ...
    def camera_or_image( cap, vidnum):
        ...
        if not (vidnum is None) and (vidnum==-1):
            ...
            if cap is None and vidnum is None:
                ...
                frame = force_center_text(frame, overtext , posy = 35, fg_rgb = (0,0,0) ) # blue
                txt_img = "xxxxxxxxxxx"
    ...
    def frames( ):
        ...
        #
        mywidgets = []
        mywidgets.append( widget(1,pos=(585,50) , typ = "signal") )
            ...
            if ret: #********************************************************* operations
                ...
                if senh.add_frame(frame):  # it is a proper image....
                    ...
                    if overtext is not None:
                        frame = force_center_text( frame, overtext, posy = 400, fg_rgb= (0,255, 0) )
                        # DIAL --------
                        post = (590,430)
                        ...
                        #                 color_bgra= (0,255, 0, 255-int(overtextalpha))
                        #                )
                    ...
                    if baduser is not None:
                        frame = force_center_text( frame, f"error! => {baduser}", posy = 440, fg_rgb= (0,0,255 ), fosize = 32 )
                    ...
                    for i in mywidgets:
                        i.refresh()
                        #print("C...   widget #", i, end=" " )
                        if i.is_active():
                            frame = i.display(frame)
                        #print()

                    # input1,input1ext = mmread_n_clear( config.CONFIG['mminput1'] )
                    # if input1 != "xxxxxx":
                    #     print(">",input1, "Exteded==", input1ext)
                    #     res = input1ext.split(";")
                    #     ff = float(res[0])
                    #     print(ff)
                    #     frame = signal_strength( frame, position=(580,60) , percentage= ff  )
                    # #else:
                    # #    print("...")
