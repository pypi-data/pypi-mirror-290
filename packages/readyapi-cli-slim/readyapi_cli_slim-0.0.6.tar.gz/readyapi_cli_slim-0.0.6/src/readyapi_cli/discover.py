...
def get_app_name(*, mod_data: ModuleData, app_name: Union[str, None] = None) -> str:
    ...
    if app_name:
        ...
        if not isinstance(app, ReadyAPI):
            raise ReadyAPICLIException(
                f"The app name {app_name} in {mod_data.module_import_str} doesn't seem to be a ReadyAPI app"
            )
        return app_name
    for preferred_name in ["app", "api"]:
        if preferred_name in object_names_set:
            obj = getattr(mod, preferred_name)
            if isinstance(obj, ReadyAPI):
                return preferred_name
    for name in object_names:
        obj = getattr(mod, name)
        if isinstance(obj, ReadyAPI):
            return name
    raise ReadyAPICLIException("Could not find ReadyAPI app in module, try using --app")
...
def get_import_string(
    *, path: Union[Path, None] = None, app_name: Union[str, None] = None
    ...
    if not path.exists():
        raise ReadyAPICLIException(f"Path does not exist {path}")
    ...
    import_panel = Padding(
        Panel(
            import_example,
            title="[b green]Importable ReadyAPI app[/b green]",
            expand=False,
            padding=(1, 2),
        ),
        1,
    )
    logger.info("Found importable ReadyAPI app")
    print(import_panel)
