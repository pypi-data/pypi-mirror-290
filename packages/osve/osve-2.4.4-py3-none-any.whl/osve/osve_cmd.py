...
def parse_options():
    ...
    - kernels_path: [OPTIONAL] Provide the location of the SPICE
        kernels path (So the value of KERNELS variable at PATH_SYMBOLS 
        in the SPICE Meta-Kernel)
    ...
    parser.add_argument("-k", "--KernelsPath",
                        help="[OPTIONAL] Provide the location of the SPICE  kernels path (So the value of KERNELS variable at PATH_SYMBOLS in the SPICE Meta-Kernel)",
                        required=False)
...
def main():
    ...
    kernels_path = ""
    if args.KernelsPath:
        if not os.path.exists(args.KernelsPath):
            logging.error('KernelsPath "{}" does not exist'.format(args.KernelsPath))
            sys.exit(0)
        else:
            kernels_path = args.KernelsPath

    here = os.path.abspath(os.path.dirname(__file__))

    # Get library
    the_osve = osve()
    the_osve.execute(args.RootPath, args.SessionFile, kernels_path)
