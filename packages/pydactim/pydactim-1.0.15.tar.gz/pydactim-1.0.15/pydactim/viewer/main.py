...
from ..perfusion import SignalDSC
...
class NiftiViewer(QMainWindow):
    ...
    def init_tabs(self):
        ...
        other_submenu = transform_menu.addMenu("Misc")
        ...
        action = QAction("Histogram matching", self)
        action.triggered.connect(lambda x: self.transforms("Histogram matching"))
        other_submenu.addAction(action)
        ...
        action = QAction("Normalize", self)
        action.triggered.connect(lambda x: self.transforms("Normalize"))
        other_submenu.addAction(action)

        perf_submenu = transform_menu.addMenu("Perfusion")

        action = QAction("Perfusion DSCk", self)
        action.triggered.connect(lambda x: self.transforms("Perfusion DSCk"))
        perf_submenu.addAction(action)
    ...
    def run_transform(self, *args):
        if args[0] == "Sort DICOM":
            ...
            new_path = normalize(args[1], force=args[2])
            if new_path not in self.nifti_found:
            ...
            if new_path not in self.nifti_found:
                ...
                self.nifti_found.append(new_path)
                self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
        elif args[0] == "Histogram matching":
            new_path = histogram_matching(args[1], args[2], force=args[3])
            if new_path not in self.nifti_found:
                self.load_sequence(new_path)
                self.nifti_found.append(new_path)
                self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
            ...
            if new_path2 not in self.nifti_found:
                ...
                self.nifti_found.append(new_path2)
                self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
        elif args[0] == "Perfusion DSC":
            dsc_path, aif_path, brain_path, mode, te, baseline, offset, cbv, cbf, mtt, ttp, tmax, psr = args
            baseline = int(baseline) if baseline is not None else 0
            offset = int(offset) if offset is not None else 0
            te = float(te) if te is not None else 0.0
            print(f"{dsc_path =}, {brain_path =}, {aif_path =}, {mode =}")
            print(f"{baseline =}, {offset =}, {te =}")
            print(f"{cbv =}, {cbf =}, {mtt =}, {ttp =}, {tmax =}, {psr =}")
            signal = SignalDSC(dsc_path, brain_path, aif_path, baseline=baseline, offset=offset, TE=te)
            signal.signal_to_concentration(clip=True)
            signal.solve_deconvolution(mode=mode)
            if cbv: signal.solve_CBV(clip=True)
            if cbf: signal.solve_CBF()
            if mtt: signal.solve_MTT()
            if ttp: signal.solve_TTP()
            if tmax: signal.solve_Tmax()
            if psr: signal.solve_PSR()
...
class TransformsGUI(QWidget):
    def __init__(self, path, selected_path, transform, run_transform, conf):
        ...
        if transform == "Sort DICOM":
            ...
            title = QLabel("Path for the image to normalize")
        ...
        elif transform == "Histogram matching":
            title = QLabel("Path for the reference image")
            combo_box = QComboBox(self)
            combo_box.addItems(path)
            combo_box.setCurrentText(selected_path)
            layout.addWidget(title)
            ...
            title = QLabel("Path for image to correct")
            combo_box_2 = QComboBox(self)
            combo_box_2.addItems(path)
            combo_box_2.setCurrentText(selected_path)
            layout.addWidget(title)
        ...
        elif transform == "Perfusion DSC":
            title_1 = QLabel("Path for the DSC time series images (must be 4D)")
            combo_box_1 = QComboBox(self)
            # combo_box_1.addItems(path)
            combo_box_1.setCurrentText(selected_path)
            layout.addWidget(title_1)
            layout.addWidget(combo_box_1)

            title_2 = QLabel("Path for the AIF image (must be 3D)")
            combo_box_2 = QComboBox(self)
            layout.addWidget(title_2)
            layout.addWidget(combo_box_2)

            title_3 = QLabel("Path for the brain mask image (must be 3D)")
            combo_box_3 = QComboBox(self)
            layout.addWidget(title_3)
            layout.addWidget(combo_box_3)

            title_7 = QLabel("Deconvolution mode")
            svd = QRadioButton("SVD", self)
            svd.setChecked(True)
            svd.toggled.connect(self.deconv_radio)
            tsvd = QRadioButton("tSVD", self)
            tsvd.toggled.connect(self.deconv_radio)
            osvd = QRadioButton("oSVD", self)
            osvd.toggled.connect(self.deconv_radio)
            tikhonov = QRadioButton("Tikhonov", self)
            tikhonov.toggled.connect(self.deconv_radio)
            self.deconv = QLabel('SVD', self)

            layout.addWidget(title_7)
            layout.addWidget(svd)

            hbox = QHBoxLayout(layout)
            title_5 = QLabel("TE")
            te = QComboBox(self)
            te.setCurrentText("0.03")
            hbox.addWidget(title_5)
            hbox.addWidget(te)
            title_4 = QLabel("Baseline")
            baseline = QComboBox(self)
            baseline.setCurrentText("8")
            hbox.addWidget(title_4)
            hbox.addWidget(baseline)
            title_6 = QLabel("Offset")
            offset = QComboBox(self)
            offset.setCurrentText("0")
            hbox.addWidget(title_6)
            hbox.addWidget(offset)
            ...
            button = QPushButton("Run Function", self)
            button.clicked.connect(lambda: self.launch_transform(transform, combo_box_1.currentText(), combo_box_2.currentText(), combo_box_3.currentText(), self.deconv.currentText(), te.currentText(), baseline.currentText(), offset.currentText(), cbv.isChecked(), cbf.isChecked(), mtt.isChecked(), ttp.isChecked(), tmax.isChecked(), psr.isChecked()))
            layout.addWidget(button, alignment=Qt.AlignmentFlag.AlignHCenter)
    ...
    def deconv_radio(self):
        rb = self.sender()
        if rb.isChecked():
            self.deconv.setText(rb.text())
