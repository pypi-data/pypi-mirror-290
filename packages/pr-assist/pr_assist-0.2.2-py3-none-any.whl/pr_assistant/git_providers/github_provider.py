...
class GithubProvider(GitProvider):
    ...
    def get_repo_settings(self):
        try:
            # contents = self.repo_obj.get_contents(".pr_assistant.toml", ref=self.pr.head.sha).decoded_content

            # more logical to take 'pr_assistant.toml' from the default branch
            contents = self.repo_obj.get_contents(".pr_assistant.toml").decoded_content
            return contents
        except Exception:
            return ""
        ...
        if deployment_type == 'user':
            try:
                ...
                raise ValueError(
                    ...
                    "https://github.com/Khulnasoft/pr-assistant#method-2-run-from-source") from e
