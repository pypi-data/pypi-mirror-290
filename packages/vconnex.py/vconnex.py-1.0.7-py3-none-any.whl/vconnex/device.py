...
class VconnexDeviceManager:
    ...
    def initialize(self) -> bool:
        ...
        with self.lock:
            ...
            try:
                ...
                if access_config is None or "mqConfig" not in access_config:
                    _LOGGER.error(
    ...
    def get_device(self, device_id: str):
        """Get device info by device code."""
            ...
            for device in self.mgr.device_map.values():
                if device.data is not None:
                    for name, msg_dict in list(device.data.items()):
                        if "created_ts" in msg_dict and msg_dict["created_ts"] <= expires_ts:
