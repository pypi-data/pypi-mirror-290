...
from papertlab.args_formatter import (
    DotEnvFormatter,
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        description="papertlab is GPT powered coding in your browser and terminal",
        add_config_file_help=True,
        default_config_files=default_config_files,
        auto_env_var_prefix="PAPERTLAB_",
    )
    ...
    group.add_argument(
        ...
        default=".papertlab.model.settings.yml",
        help="Specify a file with papertlab model settings for unknown models",
    )
    group.add_argument(
        "--model-metadata-file",
        metavar="MODEL_METADATA_FILE",
        default=".papertlab.model.metadata.json",
    ...
    default_input_history_file = (
        os.path.join(git_root, ".papertlab.input.history") if git_root else ".papertlab.input.history"
    )
    default_chat_history_file = (
        os.path.join(git_root, ".papertlab.chat.history.md") if git_root else ".papertlab.chat.history.md"
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .papertlab.llm.history)",
    )
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .papertlab* to .gitignore (default: True)",
    )
    default_papertlabignore_file = (
        os.path.join(git_root, ".papertlabignore") if git_root else ".papertlabignore"
    )
    group.add_argument(
        "--papertlabignore",
        metavar="PAPERTLABIGNORE",
        default=default_papertlabignore_file,
        help="Specify the papertlab ignore file (default: .papertlabignore in git root)",
    )
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Attribute papertlab code changes in the git author name (default: True)",
    )
    group.add_argument(
        "--attribute-committer",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Attribute papertlab commits in the git committer name (default: True)",
    )
    group.add_argument(
        "--attribute-commit-message",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'papertlab: ' (default: False)",
    )
    group.add_argument(
        "--commit",
        action="store_true",
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help="Check for new papertlab versions on launch",
        default=True,
    )
    group.add_argument(
        "--apply",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .papertlab.conf.yml in git root, cwd"
            " or home directory)"
        ),
    )
    group.add_argument(
        ...
        "--browser",
        action="store_true",
        help="Run papertlab in your browser",
        default=False,
    )
