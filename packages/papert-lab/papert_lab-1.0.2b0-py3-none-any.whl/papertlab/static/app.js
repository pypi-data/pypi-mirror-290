const Terminal = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setOutput(prevOutput => [{command, result: data.result}, ...prevOutput]);
        } catch (error) {
            console.error('Error executing command:', error);
            setOutput(prevOutput => [{command, result: `Error: ${error.message}`}, ...prevOutput]);
        }
    };

    return (
        <div className="h-full flex flex-col bg-black text-green-400 font-mono">
            <form onSubmit={handleSubmit} className="flex p-2 bg-black border-b border-green-700">
                <span className="text-green-400 mr-2">$</span>
                <input
                    type="text"
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    className="flex-grow bg-black text-green-400 focus:outline-none"
                    placeholder="Enter command"
                />
            </form>
            <div ref={outputRef} className="flex-grow overflow-y-auto p-2 text-sm whitespace-pre-wrap">
...
const MessageContent = ({ content, role }) => {
    ...
    React.useEffect(() => {
        ...
        while ((match = codeBlockRegex.exec(content)) !== null) {
            ...
            newParts.push({ type: 'code', content: match[1].trim(), id: `code-${newParts.length}` });
        ...
        try {
            if (input.startsWith('/')) {
                ...
                const response = await fetch('/api/chat', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ message: userMessage, selectedFiles: selectedFilesList, model: selectedModel })
                });
                        ...
                        return (
                            <li key={newPath} className="flex items-center">
                                <input
                                    ...
                                    className="mr-2"
                                />
                                <span className="cursor-pointer hover:text-green-300" onClick={() => handleFileClick(fullPaths[key])}>
            ...
            {isInitializing ? (
                ...
                <>
                    ...
                    <div className="flex-grow flex flex-col sm:flex-row overflow-hidden">
                        ...
                        {showTerminalPanel && (
                            <div className="w-full sm:w-1/3 overflow-y-auto border-b sm:border-b-0 sm:border-r border-green-700 p-2">
                                <h2 className="text-lg sm:text-xl font-bold mb-2">Terminal</h2>
                                <Terminal />
                            </div>
                        )}
                        <div className={`flex-grow flex flex-col ${showFilePanel || showTerminalPanel ? 'sm:w-1/2' : 'w-full'}`}>
                    ...
                    {selectedFile && (
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
                            <div className="bg-gray-800 p-5 rounded-lg w-full max-w-4xl h-5/6 flex flex-col">
                                ...
                                <div className="flex-grow overflow-hidden bg-gray-900 rounded">
                                    <textarea 
                                        ref={editorRef} 
                                        defaultValue={fileContent}  
                                        className="w-full h-full bg-gray-900 text-green-400 font-mono resize-none focus:outline-none p-2"
                                    />
                                </div>
                            </div>
                        </div>
