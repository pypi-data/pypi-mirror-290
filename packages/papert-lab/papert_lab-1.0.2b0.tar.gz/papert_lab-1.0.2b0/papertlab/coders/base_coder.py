...
class Coder:
    abs_fnames = None
    repo = None
    last_papertlab_commit_hash = None
    papertlab_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import papertlab.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                papertlab_commit_hashes=from_coder.papertlab_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"papertlab v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .papertlabignore"
                )
    ...
    def __init__(
        ...
        self.papertlab_commit_hashes = set()
        ...
        if papertlab_commit_hashes:
            self.papertlab_commit_hashes = papertlab_commit_hashes
        else:
            self.papertlab_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_error(f"Skipping {fname} that matches papertlabignore spec.")
                continue
    ...
    def send_message(self, inp):
        self.papertlab_edited_files = None
        ...
        if edited:
            self.papertlab_edited_files = edited
    ...
    def lint_edited(self, fnames):
        ...
        if self.repo and self.auto_commits and not self.dry_run:
            commit_res = self.repo.commit(
                fnames=fnames, context="The linter made edits to these files", papertlab_edits=True
            )
            if commit_res:
    ...
    def calculate_and_show_tokens_and_cost(self, messages, completion=None):
        ...
        if self.main_model.info.get("input_cost_per_token"):
            ...
            self.usage_report += (
                f"Cost: ${format_cost(cost)}, Total Cost (session): ${format_cost(self.total_cost)}.\n\n"
            )
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_papertlab_commit_hash = commit_hash
        self.papertlab_commit_hashes.add(commit_hash)
        self.last_papertlab_commit_message = commit_message
        if self.show_diffs:
            self.commands.cmd_diff()

        self.git_commit_id = f"You can use /undo to revert and discard commit {commit_hash}."
    ...
    def get_chat_history(self, num_messages=5):
        all_messages = self.done_messages + self.cur_messages
