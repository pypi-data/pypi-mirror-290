...
class Commands:
    ...
    def cmd_chat_mode(self, args):
        ...
        show_formats = OrderedDict(
            [
                ("help", "Get help about using papertlab (usage, config, troubleshoot)."),
    ...
    def cmd_undo(self, args):
        "Undo the last git commit if it was done by papertlab"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return
        ...
        if last_commit_hash not in self.coder.papertlab_commit_hashes:
            self.io.tool_error("The last commit was not made by papertlab in this chat session.")
            self.io.tool_error(
    ...
    def cmd_diff(self, args=""):
        "Display the diff of the last papertlab commit"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return
        ...
        if last_commit_hash not in self.coder.papertlab_commit_hashes:
            self.io.tool_error(f"Last commit {last_commit_hash} was not an papertlab commit.")
    ...
    def basic_help(self):
        ...
        self.io.tool_output()
        self.io.tool_output("Use `/help <question>` to ask questions about how to use papertlab.")

    def cmd_help(self, args):
        "Ask questions about papertlab"
        ...
        user_msg += """
# Announcement lines from when this session of papertlab was launched:
...
def get_help_md():
    from papertlab.coders import Coder
    from papertlab.models import Model
