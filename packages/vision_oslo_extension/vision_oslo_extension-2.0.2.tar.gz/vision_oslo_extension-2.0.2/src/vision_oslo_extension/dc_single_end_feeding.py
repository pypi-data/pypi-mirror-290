    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            if sheet_name == 'Start':
                ...
                #RefRatingList = start_df.iloc[:, 4].tolist()
                #RatingList = start_df.iloc[:, 5].tolist()
...
def sef_branch_data_process(simname, option_select,time_start, time_end, time_step, text_input, time_increment,branch_list,results_df):
    ...
    if not os.path.isfile(filename):
        print("List file not yet exist, regenerating list file, will take some time")
        oslo_extraction.list_extract(simname,time_start,time_end,"2")
    else:
        print("Existing List file found, make sure List file is correct.")
    ...
    if lst_file_size < 10000: # a random size (bytes) to check if lst should be redone (10000 bytes = 10 kb)
        print("List smaller than threshold, regenerating list file, will take some time")
        oslo_extraction.list_extract(simname,time_start,time_end,"2")
    #list file not regenerated if file size is bigger than 10000 bytes
...
def branch_list_RMS_process(simname,branch_df,second,time_increment):
    ...
    for index, node in enumerate(node_list):
        print('Processing {} out of {} nodes..'.format(index+1,total))
