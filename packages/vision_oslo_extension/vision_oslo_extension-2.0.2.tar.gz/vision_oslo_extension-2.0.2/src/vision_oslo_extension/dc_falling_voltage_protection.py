...
def branch_reading_process(simname,branch_list):
    ...
    for index, branch in enumerate(branch_list):
        print("Processing {} ...".format(branch))
...
def branch_FVP_rating(branchstep_d4_df,branch_process_df_r):
    ...
    for key, df in branchstep_d4_df.items():
        ...
        print("Comparing step values to FVP curve in branch {} ...".format(key))
...
def FVP_step_excel(simname,excel_file,text_input,results_df,sub_sum_df,dict_sub_step,dict_substation_name,rating_data,dict_substation_stock,rolling_stock_df):
    ...
    with pd.ExcelWriter(excel_file2, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        ...
        for key, branch_step in dict_sub_step.items():
            ...
            print("Adding A sheet for substation {} ...".format(key))
        ...
        if not  plot_scatter(writer, dict_sub_step,dict_substation_stock,rolling_stock_df):
            return False
...
def plot_scatter(writer, dict_sub_step,dict_substation_stock,rolling_stock_df):
    ...
    for key, branch_step in dict_sub_step.items():
        ...
        print("Adding scatter plot for substaion {} ...".format(key))
        ...
        print("Adding rolling stock information for substaion {} ...".format(key))
        ...
        chart = ScatterChart()
        chart.title = f"Falling Voltage Protection Plot, {sheet_name_variable}"
        chart.x_axis.title = "Current (kA)"
        chart.y_axis.title = "Voltage (V)"

        # Set the y-axis limits
        chart.y_axis.scaling.min = 0  # Replace with your desired min y value
        chart.y_axis.scaling.max = 900  # Maximum Voltage to Plot
        # Set the x-axis limits
        chart.x_axis.scaling.min = 0  # Replace with your desired min x value
        chart.x_axis.scaling.max = 10  # Maximum Current (kA) to Plot
        ...
        series2.graphicalProperties.line.solidFill = ColorChoice(prstClr="yellow")
        ...
        series3.graphicalProperties.line.solidFill = ColorChoice(prstClr="orange")
...
if __name__ == "__main__":
    ...
    text_input = "FVPinput"  # Adjust as needed
