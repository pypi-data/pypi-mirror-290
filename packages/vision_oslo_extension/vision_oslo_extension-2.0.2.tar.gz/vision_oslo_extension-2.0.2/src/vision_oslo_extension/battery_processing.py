...
def main(simname, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    for bhtp in traction_list:
        if not SharedMethods.check_bhtpbank(bhtp):
            return False
    ...
    if option == "4":
        ...
        for train in train_list:
            filename = simname + "_" + train + ".osop.ds1"
    ...
    data_write_save(simname, excel_file,start,total,space,start_df,result_df,train_list,route_list,ds1dataframe,trainnumber)
...
def one_stop_train_extraction(simname, time_start, time_end,train_list):
    ...
    for items in train_list:
        #print(items)
        oslo_extraction.train_step_one(simname,items,time_start,time_end)

    return True

# write data to excel
def data_write_save(simname, excel_file,start,total,space,start_df,result_df,train_list,route_list,ds1dataframe,trainnumber):
    # write data to excel
    with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a',if_sheet_exists='overlay') as writer:
        ...
        for index, train in enumerate(train_list):
            ...
            dflist.to_excel(writer, sheet_name=train, index=False, startrow = 2)
        
        data_row = 4 # first data row number

        print("Add Information Supporting Plotting....")
        addtional_info(wb,writer,route_list,ds1dataframe,result_df,data_row)
        
        print("Plot the Battery Information....")
        jounery_plot(wb,route_list,ds1dataframe,result_df,data_row)
...
def addtional_info(wb,writer,route_list,ds1dataframe,result_df,data_row):
    
    for route in route_list:
        # insert empty tab
        empty_df = pd.DataFrame()
        route_name = result_df.loc[result_df['Train ID'] == route[0], 'Train Journey']
        route_name = route_name.values[0]
...
def jounery_plot(wb,route_list,ds1dataframe,result_df,data_row):
    ...
    for index, route in enumerate(route_list):
        ...
        for train in route:
            xvalues = Reference(wb[train], min_col=33, min_row=data_row, max_row=data_row+ds1dataframe[train].shape[0]-1)
            yvalues = Reference(wb[train], min_col=36, min_row=data_row, max_row=data_row+ds1dataframe[train].shape[0]-1)
        ...
        for train in route:
            xvalues = Reference(wb[train], min_col=34, min_row=data_row, max_row=data_row+ds1dataframe[train].shape[0]-1)
            yvalues = Reference(wb[train], min_col=36, min_row=data_row, max_row=data_row+ds1dataframe[train].shape[0]-1)
            head = result_df.loc[result_df['Train ID'] == train, 'Section'].values[0]
            ...
            if sheet_name == 'Start':
                ...
                if table_list is not False:
                    for line in table_list:
                        link = line[5]
                        if not isinstance(link, (int, float)): # if the linkage is not a number
...
def bhtpbank_analysis_process(traction_list):
    ...
    for bhtp in traction_list:
        ...
        bhtpname = SharedMethods.check_bhtpbank(bhtp)
...
def train_reading_process(simname, train_list,time_increment,start_df,result):
    ...
    for train in train_list:
        ...
        print("Processing train No. {} using {} ...".format(train,bhtp))
        filename = simname + "_" + train +".osop.ds1"
        delimiter = "\s+"
...
def interpolate_cal_value(voltage, speed, data_df):
    ...
    # Interpolate # Result is an array format
...
def get_result_range(start,total,space,trainnumber):
    ...
    # 5 percentage result
    add_range_3 = [f"J{start + 2}:K{start + total + 1}",f"P{start + 2}:R{start + total + 1}"]
...
if __name__ == "__main__":
    ...
    simname = "007_Baseline"  # Provide a simulation name or adjust as needed
