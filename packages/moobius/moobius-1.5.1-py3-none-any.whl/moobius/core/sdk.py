...
from moobius.types import MessageContent, MessageBody, Button, ButtonClick, InputComponent, Payload, MenuItemClick, UpdateBody, UpdateItem, CopyBody, Character, ChannelInfo, CanvasItem, StyleItem, MenuItem, ButtonClickArgument
...
class Moobius:
    ...
    def __init__(self, config_path, db_config_path=None, service_mode=True, **kwargs):
        ...
        Parameters:
          ...
          **kwargs: Logging and other config parameters go here. Most commonly "log_settings".
    ...
    async def start(self):
        ...
        self.scheduler.add_job(self.on_checkin, 'interval', seconds=self.checkin_interval) # This check-in must be after on_start()
    ...
    async def send_message(self, message, channel_id=None, sender=None, recipients=None, subtype=None, len_limit=None, file_display_name=None):
        ...
        async def _get_file_message_content(file_path, file_display_name=None, subtype=None):
            """Converts a file_path into a MessageContent object, uploading files if need be."""
            if type(file_path) is not str:
                file_path = file_path.as_posix() # For pathlib.paths.
            file_uri = await self.http_api.convert_to_url(file_path)
            size = None
            if file_uri != file_path:
        ...
        if message.get('recipients'):
            ...
            if self.service_mode:
                ...
                for ky in ['sender', 'timestamp', 'context']: # Message up messages have no sender, for some reason.
                    if ky in message:
                        del message[ky]
    ...
    async def do_member_sync(self, channel_id, member_id):
        """Syncs a member. Accepts a channel_id and member_id. Returs None. This is the most common way to send buttons, etc."""
        pass
    ...
    async def handle_received_payload(self, payload):
        ...
        if 'type' in payload_data:
            ...
            if payload.type == types.MESSAGE_DOWN:
                payload_body = types._recv_tmp_convert('on_message_down', payload_body)
                ...
                await self.on_refresh(payload_data['body']['channel_id'], payload_data['body']['sender'])
            else:
    ...
    async def on_action(self, action_data):
        ...
        if subtype == types.JOIN_CHANNEL:
            await self.on_join(action_data['channel_id'], action_data['sender'])
        elif subtype == types.LEAVE_CHANNEL:
            await self.on_leave(action_data['channel_id'], action_data['sender'])
        elif subtype == types.BUTTON_CLICK:
            if action_data.get('arguments'):
                action_data['arguments'] = [ButtonClickArgument(**a) for a in action_data['arguments']]
            ...
            await self.on_refresh(action_data['channel_id'], action_data['sender'])
        else:
    ...
    async def on_refresh(self, channel_id, member_id):
        """
        This callback accepts a "Copy" request from the user. Returns None."""
        await self.do_member_sync(channel_id, member_id)

    async def on_join(self, channel_id, member_id):
        """
    ...
    async def on_leave(self, channel_id, member_id):
        """
