...
class KPP(SuperPage):
    ...
    def trying_start_new_arrival_while_current(self, text, tags):
        threading.Thread(target=self.kpp_arriving_attention_thread,
                         kwargs={"text": text, "tags": tags}).start()
    ...
    def external_button_pressed(self):
        if self.status["lift_up_in_progress"]:
    ...
    def set_status(self, status):
        if not status:
            return
        self.status = status
        while self.operator.road_anim_info["kpp_barrier_arrow"]["busy"]:
            pass
        if status["barrier_open"]:
            self.operator.road_anim_info["kpp_barrier_arrow"]['pos'] = 85
        else:
            self.operator.road_anim_info["kpp_barrier_arrow"]['pos'] = 0
