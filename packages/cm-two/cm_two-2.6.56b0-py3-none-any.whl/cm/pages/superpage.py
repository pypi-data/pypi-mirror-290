...
class SuperPage:
    ...
    def create_buttons(self, buttons):
        ...
        for obj in buttons:
            ...
            self.can.create_window(obj[1], obj[2], window=button,
                                   tag="button")
    ...
    def checking_thread(self):
        ...
        while True:
            ...
            if ((
                self.operator.current == 'MainPage' or self.operator.current == 'ManualGateControl') and not
    ...
    def get_nav_bar_btns(self):
        gravity = self.operator.gravity_mode
        kpp = self.operator.kpp_mode
        all_btns = []
        if gravity:
    ...
    def open_entry_gate_operation_start(self):
        threading.Thread(target=self.rotate_gate_arrow, args=(
            self.settings.entry_gate, 'open', 'OUT', 1)).start()

    def open_exit_gate_operation_start(self):
        threading.Thread(target=self.rotate_gate_arrow, args=(
            self.settings.exit_gate, 'open', 'OUT', 1)).start()
    ...
    def rotate_gate_arrow(self, arrow_attr, pos, course, step=1, endpos=85,
                          sleeptime=0.001):
        arrow_info = self.operator.road_anim_info[arrow_attr]
        #init_page = self.operator.currentPage
        ...
        while arrow_info['pos'] != endpos:
            ...
            #if init_page == self.operator.currentPage:
    ...
    def draw_barrier_arrow(self, barrier_name):
        """ НЕ РАБОТАЕТ """
        if self.operator.currentPage.blockImgDrawn:
            return
        if barrier_name not in self.barriers:
            print(f"Шлагбаум {barrier_name} не найден на странице {self.name}")
            return
        barrier_info = self.barriers[barrier_name]
        start = 10
        end = self.gate_arrow_img.height - 4
        obj = self.getAttrByName(barrier_info["settings_object"])
        tag = str(uuid.uuid4())
        tags = ['maincanv' + tag]
        barrier_info["tags"].append(tag)
        tkimage = ImageTk.PhotoImage(
            self.gate_arrow_img.rotate(barrier_info['pos'], expand=True,
                                       center=(start, end)))
        self.can.delete(barrier_info["tags"][:-1])
        barrier_info["tags"] = barrier_info["tags"][:5]
        self.can.create_image(obj[1], obj[2], image=tkimage, tags=tags)
        barrier_info['img'] = tkimage
        barrier_info['img_obg'] = self.gate_arrow_img
    ...
    def draw_set_arrow(self, arrow_attr):
        ...
        tag = str(uuid.uuid4())
        tags = ['maincanv'] + [tag]
        if not arrow_attr in self.kpp_barrier_arrows_tag:
            self.kpp_barrier_arrows_tag[arrow_attr] = []
        self.kpp_barrier_arrows_tag[arrow_attr].append(tag)
        # print('Установка стрел', self.operator.road_anim_info)
        tkimage = ImageTk.PhotoImage(
            self.gate_arrow_img.rotate(arrow_info['pos'], expand=True,
                                       center=(start, end)))
        try:
            self.can.delete(self.kpp_barrier_arrows_tag[arrow_attr][:-1])
            self.kpp_barrier_arrows_tag[arrow_attr] = \
            self.kpp_barrier_arrows_tag[arrow_attr][:5]
        except IndexError:
            pass
        self.can.create_image(obj[1], obj[2], image=tkimage, tags=tags)
        self.operator.road_anim_info[arrow_attr]['img'] = tkimage
        self.operator.road_anim_info[arrow_attr][
            'img_obg'] = self.gate_arrow_img
