...
class Operator:
    ...
    def __init__(self, root, settings, scale, can, deffaultScreenSize,
                 ...
                 test_mode=False, gravity=True, kpp_mode_enable=False,
                 rfid=False):
        ...
        if gravity:
            ...
            threading.Thread(
                target=self.first_page_after_auth.checking_thread).start()
        ...
        threading.Thread(target=self.wr.scale_reciever,
                         args=(scale_server_ip,), daemon=False).start()
    ...
    def turn_cams(self, on=True, cam_type=None):
        threading.Thread(
            ...
            kwargs={"on": on, "cam_type": cam_type}).start()
    ...
    def turn_cams_thread(self, on=True, cam_type=None):
        ...
        for cam in self.cameras_info:
            ...
            while cam["enable"] and not "video_worker_inst" in cam.keys():
                # print("T112. Waiting inst")
                pass
            if cam_type:
                if cam['type'] == cam_type:
                    # while not "video_worker_inst" in cam.keys() and self.test_mode:
                    #    pass
                    # print("TT113 CAN SHOW")
        ...
        cam_meta = {"kpp_cam_internal":
                    ...
                    "cad_gross":
                        {"xpos": self.gross_x, "ypos": self.gross_y,
                         ...
                         "zoomed_y": 513, "zoomed_x": 1038.5},
                    "main":
                        {"xpos": 417, "ypos": 280, "v_width": 420,
                         "v_height": 400, "zoomed_y": 513, "zoomed_x": 1038.5},
                    }
        if self.settings.kpp_mirrored:
            cam_meta["kpp_cam_internal"]["xpos"] = 457
            cam_meta["kpp_cam_external"]["xpos"] = 1620
        inst = start_video_stream(self.root, self.first_page_after_auth.can,
                                  xpos=cam_meta[cam["type"]]["xpos"],
                                  ypos=cam_meta[cam["type"]]["ypos"],
                                  v_width=cam_meta[cam["type"]]["v_width"],
                                  v_height=cam_meta[cam["type"]]["v_height"],
                                  cam_port=port,
                                  cam_login=cam['login'],
                                  cam_pass=cam['pw'],
                                  cam_ip=cam_ip,
                                  ...
                                  zoomed_y=cam_meta[cam["type"]]["zoomed_y"],
                                  zoomed_x=cam_meta[cam["type"]]["zoomed_x"],
                                  cam_type=cam['type'],
    ...
    def close_app_thread(self):
        ...
        threading.Thread(target=self.ar_qdk.capture_cm_terminated).start()
        threading.Thread(target=self.ar_qdk.restart_unit).start()
        self.root.destroy()
        sys.exit(0)
    ...
    def cad_work(self, info):
        ...
        if info['cam_type'] == 'cad_gross':
            ...
            self.mainPage.can.create_text(x + 350, y - 16, text=text,
            ...
            self.mainPage.can.create_text(x + 65, y - 16, text=text,
                                          tag=(tag, "page_elements"),
                                          fill="#F5B85E",
                                          font=fonts.cad_work_font)
        threading.Thread(
            target=self.tag_timeout_deleter, args=(tag,)).start()
                    ...
                    if not auto_pass:
                        self.kpp_page.draw_manual_pass_entry(
                            ...
                            alert_text=f"Автоматический доступ для машины "
                                       ...
                                       f"но это будет отражено в журнале.")
    ...
    def operate_kpp_car_detected(self, *args, **kwargs):
    ...
    def open_new_page(self, page):
        try:
            ...
            # self.currentPage.lower("win_widgets")
        except:
            print(traceback.format_exc())
    ...
    def set_cam_info(self, info):
        ...
        threading.Thread(target=self.create_video_streams).start()
    ...
    def operate_new_plate_recognition_trying(self, info):
        ...
        side = info["side"]
        self.currentPage.operate_new_plate_recognition_trying(
            count, max_count, side)
        # if self.current == "KPP":
        #    self.currentPage.set_recognition_count(count, max_count)
