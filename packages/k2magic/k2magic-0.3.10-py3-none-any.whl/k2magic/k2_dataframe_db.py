...
class K2DataFrameDB(DataFrameDB):
    ...
    def get_repo_data(self, table_name: str, start_time: Union[str, int, datetime], end_time: Union[str, int, datetime],
        ...
        """
        查询Repo数据，参数说明可参考K2Assets在线文档的开发者手册。
        :param table_name:
        :param start_time:
        :param end_time:
        :param devices:
                ...
                if self.behind_repo:
                    if self.engine.name == "postgresql":
                        ...
                        query = traverse(obj=query, opts={},
                                         visitors={"binary": _pg_modify_case, "textclause": _pg_modify_case,
                                                   "column": _pg_modify_case, "table": _pg_modify_case})
...
def _disclose_db_url(repo_db_url: URL, repo_meta: dict) -> URL:
    ...
    if repo_db_url.drivername != 'k2assets+repo':
        raise ValueError("Not a valid url (k2assets+repo://) to disclose")
...
def _build_engine_meta(repo_meta) -> MetaData:
    """
    用Repo里定义的数据结构，构造engine的metadata对象。实现当Repo与底层数据库不一致时，以Repo数据结构为准的效果。
