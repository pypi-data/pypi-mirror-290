...
def render_link(linkdef, mdclass=""):
    t = Template(
        """<a href="{{ link.url }}" name="{{ link.name }}" title="{{ link.title }}"
...
def extra_static_content(caller, st_type, user):
    ...
    """
    tpl = template.Template("{% for sc in static_content %}{{ sc|safe }}{% endfor %}")
    static_content = core_signals.extra_static_content.send(
        sender="extra_static_content", caller=caller, st_type=st_type, user=user
    )
    static_content = [result[1] for result in static_content]
    return tpl.render(template.Context({"static_content": static_content}))
