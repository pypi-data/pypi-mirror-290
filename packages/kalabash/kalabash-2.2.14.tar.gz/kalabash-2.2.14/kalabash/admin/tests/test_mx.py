...
from kalabash.core import models as core_models, factories as core_factories
...
class MXTestCase(KbashTestCase):
    ...
    def setUpTestData(cls):  # NOQA:N802
        ...
        cls.domain = factories.DomainFactory(name="alphamonak.com")
        # should not exist
    ...
    def test_management_command(
        ...
        management.call_command("kbash", "check_mx", "--no-dnsbl", "--ttl=0")
        ...
        id_ = qs[0].id
        management.call_command("kbash", "check_mx", "--no-dnsbl", "--ttl=7200")
        qs = models.MXRecord.objects.filter(domain=self.domain)
        ...
        # ttl
        management.call_command("kbash", "check_mx", "--no-dnsbl")
        qs = models.MXRecord.objects.filter(domain=self.domain)
    ...
    def test_single_domain_update(
        ...
        mock_getaddrinfo.side_effect = utils.mock_ip_query_result
        mock_g_gethostbyname.return_value = "1.2.3.4"
        management.call_command("kbash", "check_mx", "--domain", self.domain.name)
        self.assertTrue(models.MXRecord.objects.filter(domain=self.domain).exists())
        self.assertFalse(
            models.MXRecord.objects.filter(domain=self.bad_domain).exists()
        )

        management.call_command("kbash", "check_mx", "--domain", str(self.bad_domain.pk))
        self.assertFalse(
            models.MXRecord.objects.filter(domain=self.bad_domain).exists()
        )
        self.assertEqual(len(mail.outbox), 1)

        management.call_command("kbash", "check_mx", "--domain", "toto.com")
    ...
    def test_invalid_mx(self, mock_query, mock_getaddrinfo, mock_g_gethostbyname):
        ...
        management.call_command(
            "kbash", "check_mx", "--domain", domain.name, "--no-dnsbl"
        )
    ...
    def test_get_domain_mx_list_logging(self, mock_query, mock_ip_address):
        ...
        with LogCapture("kalabash.admin") as log:
            get_domain_mx_list("does-not-exist.example.com")
        ...
        log.check(
            ...
            ("kalabash.admin", "ERROR", _("No MX record for %s") % "no-mx.example.com"),
            ("kalabash.admin", "ERROR", _("No working name servers found")),
            (
                "kalabash.admin",
                "WARNING",
    ...
    def test_ipv6_logging(self, mock_query, mock_ip_address):
        ...
        log.check(
            ("kalabash.admin", "ERROR", _("No AAAA record for %s") % "mx3.example.com")
        )
        localconfig = core_models.LocalConfig.objects.first()
        localconfig.parameters.set_value("enable_ipv6_mx_checks", False, "admin")
        localconfig.save()
        with LogCapture("kalabash.admin") as log2:
            get_domain_mx_list("test3.com")
        log2.check()


@override_settings(DNSBL_PROVIDERS=["zen.spamhaus.org"])
class DNSBLTestCase(KbashTestCase):
    """TestCase for DNSBL related features."""
    ...
    def setUpTestData(cls):  # NOQA:N802
        ...
        cls.domain3 = factories.DomainFactory(
            name="kalabash.com", enable_dns_checks=False
        )
    ...
    def test_management_command(
        ...
        management.call_command("kbash", "check_mx")
    ...
    def test_notifications(self, mock_query, mock_getaddrinfo, mock_g_gethostbyname):
        ...
        management.call_command("kbash", "check_mx", "--email", "user@example.test")
...
class DNSChecksTestCase(KbashTestCase):
