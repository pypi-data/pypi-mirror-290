...
from kalabash.core.tests import test_ldap
from kalabash.lib.tests import NO_LDAP, KbashTestCase
...
class AccountTestCase(KbashTestCase):
    ...
    def test_create_disabled_account(self):
        ...
        alias = models.Alias.objects.get(address=account.email)
        self.assertEqual(alias.enabled, False)
...
class LDAPAccountTestCase(test_ldap.LDAPTestCaseMixin, KbashTestCase):
...
class PermissionsTestCase(KbashTestCase):
    ...
    def test_domain_admins(self):
        ...
        self.values["role"] = "StandardUsers"
        self.ajax_post(
            reverse("admin:account_change", args=[self.user.id]), self.values
        )
