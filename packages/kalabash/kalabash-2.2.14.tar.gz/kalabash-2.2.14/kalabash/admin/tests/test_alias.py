...
class AliasTestCase(KbashTestCase):
    ...
    def test_upper_case_alias(self):
        ...
        self.assertEqual(
            AliasRecipient.objects.get(
                alias__internal=False, r_mailbox=mb
            ).alias.address,
            "toto@test.com",
        )
        ...
        self.ajax_post(reverse("admin:alias_add"), values)
        self.assertTrue(Alias.objects.filter(address="titi@test.com").exists())
    ...
    def test_dlist(self):
        ...
        self.ajax_post(reverse("admin:alias_change", args=[dlist.id]), values)
        self.assertEqual(dlist.recipients_count, 2)

        self.ajax_delete(
            "{}?selection={}".format(reverse("admin:alias_delete"), dlist.id)
        )
        self.assertRaises(Alias.DoesNotExist, Alias.objects.get, address="all@test.com")
