...
def core_menu(selection, user):
    ...
    if user.is_superuser:
        entries += [
            {"name": "settings", "label": _("Kalabash"), "url": reverse("core:index")}
        ]
    if not len(entries):
        return ""
...
@register.simple_tag
def get_version():
    return pkg_resources.get_distribution("kalabash").version
...
@register.simple_tag
def get_kalabash_logo():
    try:
        logo = settings.KALABASH_CUSTOM_LOGO
    except AttributeError:
        logo = None
    if logo is None:
        return os.path.join(settings.STATIC_URL, "css/kalabash-white.png")
    return logo
