...
from kalabash.lib.exceptions import (
    BadRequest,
    Conflict,
    InternalError,
    PermDeniedException,
...
try:
    from kalabash.lib.ldap_utils import LDAPAuthBackend
...
class User(AbstractUser):
    ...
    def _crypt_password(self, raw_value):
        ...
        if scheme is None:
            from kalabash.core.apps import load_core_settings
    ...
    def role(self, role):
        ...
        if role == "SuperAdmins":
            ...
            if self.is_superuser or role == "StandardUsers":
                ObjectAccess.objects.filter(user=self).delete()
            self.is_superuser = False
            try:
                self.groups.add(Group.objects.get(name=role))
            except Group.DoesNotExist:
                self.groups.add(Group.objects.get(name="StandardUsers"))
            if role != "StandardUsers" and not self.can_access(self):
                from kalabash.lib.permissions import grant_access_to_object
...
def populate_callback(user, group="StandardUsers"):
    """Populate callback
    ...
    Kalabash. This function is in charge of creating the mailbox
