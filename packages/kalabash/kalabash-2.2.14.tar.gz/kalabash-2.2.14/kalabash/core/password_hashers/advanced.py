...
class SHA256CRYPTHasher(PasswordHasher):
    ...
    def _encrypt(self, clearvalue, salt=None):
        ...
        return sha256_crypt.using(rounds=rounds).hash(clearvalue)
...
class SHA512CRYPTHasher(PasswordHasher):
    ...
    def _encrypt(self, clearvalue, salt=None):
        rounds = param_tools.get_global_parameter("rounds_number")
        return sha512_crypt.using(rounds=rounds).hash(clearvalue)
...
if argon2_hasher is not None:
    ...
    class ARGON2IDHasher(PasswordHasher):
        ...
        It is the strongest scheme provided by kalabash
        ...
        def __init__(
            ...
            if hasattr(settings, "KALABASH_ARGON2_TIME_COST"):
                parameters["time_cost"] = settings.KALABASH_ARGON2_TIME_COST

            if hasattr(settings, "KALABASH_ARGON2_MEMORY_COST"):
                parameters["memory_cost"] = settings.KALABASH_ARGON2_MEMORY_COST

            if hasattr(settings, "KALABASH_ARGON2_PARALLELISM"):
                parameters["parallelism"] = settings.KALABASH_ARGON2_PARALLELISM
