...
from kalabash.lib import tests as lib_tests
from .. import utils


class DomainLimitsTestCase(lib_tests.KbashTestCase):
    ...
    def test_domain_aliases_limit(self):
        ...
        values = {
            ...
            "domain_admins_limit": 2,
            "aliases": "alias1.com",
            "aliases_1": "alias2.com",
        }
        self.ajax_post(reverse("admin:domain_change", args=[domain.id]), values)
        self.assertTrue(limit.is_exceeded())
        values["aliases_2"] = "alias3.com"
        self.ajax_post(reverse("admin:domain_change", args=[domain.id]), values, 403)
    ...
    def test_domain_admins_limit(self):
        ...
        self.ajax_post(reverse("admin:account_change", args=[user.id]), values, 400)
    ...
    def test_mailbox_aliases_limit(self):
        ...
        self.ajax_post(reverse("admin:account_change", args=[user.id]), values, 403)

        limit.max_value = 5
        limit.save()
        values = {
            "address": "forward2@test.com",
