...
from kalabash.admin import constants, models
from kalabash.dnstools import models as dns_models
...
class CheckMXRecords(BaseCommand):
    ...
    def check_valid_mx(self, domain, mx_list, **options):
        ...
        if not alerts:
            return
        subject = _("[kalabash] MX issue(s) for domain {}").format(domain.name)
    ...
    def check_domain(self, domain, timeout=3, ttl=7200, **options):
        ...
        if not alerts:
            return
        subject = _("[kalabash] DNSBL issue(s) for domain {}").format(domain.name)
