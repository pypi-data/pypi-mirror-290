...
class Email(object):
    ...
    def __init__(self, mailid, mformat="plain", dformat="plain", links=False):
        ...
        if isinstance(links, bool):
            ...
            # TODO: kalabash-webmail currently sets links == 0 or 1, it needs
        ...
        # used by kalabash_webmail.lib.imapemail.ImapEmail
        self.attachments = {}
    ...
    def headers(self):
        if self._headers is None:
            ...
            # don't break kalabash_webmail.lib.imapemail.ImapEmail, it
    ...
    def get_header(self, msg, header):
        ...
        # kalabash_webmail.lib.imapemail.ImapEmail
        if header in msg:
            decoded_values = []
...
def set_email_headers(msg, subject, sender, rcpt):
    ...
    msg["User-Agent"] = "Kalabash %s" % (
        pkg_resources.get_distribution("kalabash").version
    )
