...
from kalabash.core import constants
from kalabash.core.password_hashers import get_password_hasher
from kalabash.core.password_hashers.utils import get_dovecot_schemes
from kalabash.lib.tests import NO_SMTP, KbashTestCase
...
class AuthenticationTestCase(KbashTestCase):
    ...
    def test_password_argon2_parameter_change(self):
        ...
        self.client.logout()
        with self.settings(
            KALABASH_ARGON2_TIME_COST=4,
            KALABASH_ARGON2_MEMORY_COST=10000,
            KALABASH_ARGON2_PARALLELISM=4,
        ):
            self.client.post(reverse("core:login"), data)
        user.refresh_from_db()
        self.assertTrue(user.password.startswith("{ARGON2ID}"))
        ...
        self.client.logout()
        with self.settings(
            KALABASH_ARGON2_TIME_COST=3,
            KALABASH_ARGON2_MEMORY_COST=1000,
            KALABASH_ARGON2_PARALLELISM=2,
        ):
            self.client.post(reverse("core:login"), data)
        user.refresh_from_db()
        self.assertTrue(user.password.startswith("{ARGON2ID}"))
    ...
    @mock.patch("kalabash.lib.sysutils.exec_cmd")
...
class PasswordResetTestCase(KbashTestCase):
    ...
    def setUpTestData(cls):  # NOQA:N802
        ...
        cls.account_ko = factories.UserFactory(
            username="user2@test.com", groups=("StandardUsers",)
        )
...
class SMTPAuthenticationTestCase(KbashTestCase):
