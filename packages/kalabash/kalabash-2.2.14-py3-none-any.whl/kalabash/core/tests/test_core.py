...
from kalabash.lib.tests import KbashTestCase, SimpleKbashTestCase
from .. import factories, mocks, models


class AuthenticationTestCase(KbashTestCase):
    ...
    def setUpTestData(cls):  # NOQA:N802
        ...
        cls.account = factories.UserFactory(
            username="user@test.com", groups=("StandardUsers",)
        )
...
class ManagementCommandsTestCase(SimpleKbashTestCase):
    """Test management commands."""

    def test_change_default_admin(self):
        """Use dedicated option."""
        management.call_command("load_initial_data", "--admin-username", "kbashadmin")
        self.assertTrue(self.client.login(username="kbashadmin", password="password"))
    ...
    def test_non_duplicate_client_creation(self):
        ...
        self.assertEqual(1, app_model.objects.filter(name="kalabash_frontend").count())
        management.call_command("load_initial_data")
        self.assertEqual(1, app_model.objects.filter(name="kalabash_frontend").count())
    ...
    def test_clean_inactive_accounts(self):
        ...
        account = factories.UserFactory(
            username="user1@domain.test", groups=("StandardUsers",), last_login=last_login
        )
...
class ProfileTestCase(KbashTestCase):
    ...
    def test_update_password_url(self):
        ...
        non_local_user = factories.UserFactory(
            username="user@external.com", groups=("StandardUsers",), is_local=False
        )
...
class APIAccessFormTestCase(KbashTestCase):
    """Check form access."""
...
class APICommunicationTestCase(KbashTestCase):
    """Check communication with the API."""

    def test_management_command(self):
        """Check command."""
        with httmock.HTTMock(
            mocks.kbash_api_instance_search,
            mocks.kbash_api_instance_create,
            mocks.kbash_api_instance_update,
            mocks.kbash_api_versions,
        ):
        ...
        with httmock.HTTMock(
            ...
            mocks.kbash_api_instance_create,
            mocks.kbash_api_instance_update,
            mocks.kbash_api_versions_no_update,
        ):
            management.call_command("communicate_with_public_api")
        self.assertEqual(len(mail.outbox), 1)
