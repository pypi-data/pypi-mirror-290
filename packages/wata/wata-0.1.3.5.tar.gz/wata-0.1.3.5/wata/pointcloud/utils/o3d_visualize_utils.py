...
def show_pcd_from_points_by_open3d(points, point_size=1, background_color=None, colors=None, create_coordinate=True,
    ...
    if isinstance(colors, int):
        ...
        # print(points.shape)
        points_pcd = open3d.geometry.PointCloud()
        points_pcd.points = open3d.utility.Vector3dVector(points[:,:3])
        colors = points[:, colors]
        # colors[colors < 50] = 1
        colors_normalized = np.minimum(1, colors / 255)
        # Colors = plt.get_cmap('RdBu')
        # colors_normalized = Colors(colors / 50)
        ...
        points_pcd.colors = open3d.utility.Vector3dVector(colors_rgb)
