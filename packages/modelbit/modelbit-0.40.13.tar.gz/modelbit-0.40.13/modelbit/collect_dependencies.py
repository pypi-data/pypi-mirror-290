...
from .utils import convertLambdaToDef, unindent, dumpJson
...
def getRuntimePythonProps(func: Optional[Callable[..., Any]],
  ...
  if job is not None:
    ...
    props.job = jobWrapperToProps(job)
...
def annotationsToTypeStr(annotations: Dict[str, Any]):
  ...
  for name, tClass in annotations.items():
    if tClass == Any:
      annoStrs[name] = "Any"
    elif tClass.__module__ == "typing":
      try:
        annoStrs[name] = tClass.__origin__.__name__  # returns list for List, dict for Dict, tuple for Tuple
      except:
        pass
    else:
      try:
...
def jobWrapperToProps(jr: 'ModelbitJobWrapper'):
...
def _collectArgTypesAndImports(func: Callable[..., Any], collection: NamespaceCollection):
  try:
    import ast
    ...
    for d in sigAst.decorator_list:  # type: ignore
      collectObj(d)
    for b in sigAst.body:  # type: ignore
      collectObj(b)
...
def _unwrapFunction(func: Callable[..., Any]) -> Callable[..., Any]:
...
def _isMainModuleWrappedFunc(func: Callable[..., Any]):
  while _isWrappedFunc(func):
    func = _unwrapFunction(func)
...
def maybeCollectLocalModule(moduleName: str, collection: NamespaceCollection):
...
def collectSklearnEstimatorContainer(obj: Any, name: str, collection: NamespaceCollection):
...
def _collectKeras(obj: Any, name: str, collection: NamespaceCollection):
...
def recursivelyCollectModules(obj: Any, collection: NamespaceCollection):
  if type(obj) is list:
...
def collectVar(obj: Any, name: str, collection: NamespaceCollection):
