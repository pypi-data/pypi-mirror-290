...
from typing import Any, Callable, Dict, Optional, Tuple, Union, cast, List, TextIO
...
def getSerializerDesc(serializer: Optional[str] = None):
  if serializer is None:
    return None
...
@contextmanager
def tempFilePath(suffix: Optional[str] = None):
...
@contextmanager
def storeFileOnSuccess(finalPath: str):
...
def getStatusOutputStream(fileSize: int) -> TextIO:
  visible = showStatusThresholdBytes < fileSize
...
def progressBar(inputSize: int, desc: str):
