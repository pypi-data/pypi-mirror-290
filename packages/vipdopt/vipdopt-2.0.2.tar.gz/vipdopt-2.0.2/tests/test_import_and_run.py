...
if __name__ == '__main__':
    ...
    design_region_import_x = np.linspace(
        0, img.shape[0] - 1, project.device.field_shape[0]
    )
    design_region_import_y = np.linspace(
        0, img.shape[1] - 1, project.device.field_shape[1]
    )
    design_region_import = np.array(
        np.meshgrid(design_region_import_x, design_region_import_y, indexing='ij')
    ).transpose((1, 2, 0))
    ...
    if not debug_post_run:
        plt.imshow(
            np.abs(project.device.get_design_variable()[..., 0]),
    ...
    for idx, num in enumerate([0, 2, -1]):
        plt.plot(
            project.config['lambda_values_um'],
            quad_trans[num],
            color=quad_colors[num],
            label=quad_labels[num],
        )
    ...
    plt.hlines(
        0.225,
        project.config['lambda_values_um'][0],
        project.config['lambda_values_um'][-1],
        color='black',
        linestyle='--',
        label='22.5%',
    )
