...
class CellSegmentationInference:
    ...
    def process_wsi(
        ...
        with torch.no_grad():
            ...
            for batch in wsi_inference_dataloader:
                ...
                # Handle last batch
                if len(gpu_ids) > 1 and patches.shape[0] < len(gpu_ids):
                ...
                for idx, (patch_instance_types, patch_metadata) in enumerate(
                    ...
                    wsi_scaling_factor = wsi.metadata['target_patch_mpp'] / wsi.metadata['base_mpp']
                    ...
                    for cell in patch_instance_types.values():
                        ...
                        cell_dict = {
                            ...
                            "offset_global": (offset_global * wsi_scaling_factor).tolist()
                        }
                        cell_detection = {
                            "bbox": bbox_global.tolist(),
                            "centroid": centroid_global.tolist(),
                        ...
                        # get the cell token
                        bb_index = cell["bbox"] / self.module.patch_size
        ...
        if geojson:
            ...
            geojson_list = self.convert_geojson(cell_dict_wsi["cells"], True, wsi_scaling_factor=wsi_scaling_factor)
    ...
    def get_cell_predictions_with_tokens(
        ...
        (
            _,
            instance_types,
        ) = self.module.calculate_instance_map(predictions, magnification=magnification)
    ...
    def convert_geojson(
        self, cell_list: list[dict], polygons: bool = False, wsi_scaling_factor=1.0
    ) -> List[dict]:
...
class CellPostProcessor:
    def __init__(self, cell_list: List[dict], logger: logging.Logger) -> None:
        ...
        xy_np = self.cell_df['patch_coordinates'].values
        self.cell_df["patch_row"] = [xy[0] for xy in xy_np]
        self.cell_df["patch_col"] = [xy[1] for xy in xy_np]
        self.cell_df["patch_coordinates"] = self.cell_df["patch_row"].astype(str) + '_' + self.cell_df["patch_col"].astype(str)
        
        #self.cell_df = self.cell_df.parallel_apply(convert_coordinates, axis=1)
        self.logger.info("Finished converting coordinates")
    ...
    def _remove_overlap(self, cleaned_edge_cells: pd.DataFrame) -> pd.DataFrame:
        ...
        for iteration in range(20):
            ...
            for i in range(len(poly_list)):
                query_poly = poly_list[i]
                query_uid = uid_list[i]
                if query_uid not in iterated_cells:
                    ...
                    if (
                        ...
                        submergers_uid = []
                        for inter_poly_idx in intersected_polygons:
                            inter_poly = poly_list[inter_poly_idx]
                            inter_poly_uid = uid_list[inter_poly_idx]
                            if (
                                inter_poly_uid != query_uid
                                and inter_poly_uid not in iterated_cells
                            ):
                                if (
                                    ...
                                    submergers_uid.append(inter_poly_uid)
                                    iterated_cells.add(inter_poly_uid)
                        ...
                        if len(submergers) == 0:
                            ...
                            selected_poly_uid = submergers_uid[selected_poly_index]
...
class InferenceWSIParser:
    ...
    def __init__(self) -> None:
        ...
        parser.add_argument('--gpu_ids', nargs='+', help='<Required> Set flag', default=['0'])
        parser.add_argument(
            "--multigpu",
            action='store_true',
            help="Use all GPUs. Default: False"
        )
        parser.add_argument(
            "--magnification",
            type=float,
    ...
    def parse_arguments(self) -> dict:
        ...
        var = vars(opt)
        var['gpu_ids'] = [int(i) for i in var['gpu_ids']]
        return var
