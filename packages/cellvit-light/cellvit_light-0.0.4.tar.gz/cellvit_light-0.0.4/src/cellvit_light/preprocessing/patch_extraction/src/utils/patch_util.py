...
def target_mpp_to_downsample(
    base_mpp: float, target_mpp: float, patch_size, overlap, tolerance: float = 0.1
) -> Tuple[int, float]:
    ...
    closest_int = round(overlap * scale)
    scale = closest_int / overlap # this is the real scale, because we rescale such that the overlap is an int

    assert round(scale * patch_size) - 0.0001 < scale * patch_size and scale * patch_size < round(scale * patch_size) + 0.0001
    assert round(scale * overlap) - 0.0001 < scale * overlap and scale * overlap < round(scale * overlap) + 0.0001
