...
class SilicaComponent {
    ...
    showLoaders(action, isInitiating) {
        ...
        this.querySelectorAll(loadingSelectors.join(", ")).forEach(el => {
            if (this.shouldElementBeProcessed(el, target_from_action)) {
     ...
     * @param targetAction The target action name, if any.
     * @returns {boolean} True if the element should be processed, false otherwise.
     */
    shouldElementBeProcessed(el, targetAction) {
        if (!el.hasAttribute("silica:target")) return true;
        const silicaTarget = el.getAttribute("silica:target");
        const targetWithoutParentheses = silicaTarget.split("(")[0];
        return [silicaTarget, targetWithoutParentheses].includes(targetAction);
    }
    ...
    showErrorModal(message) {
        ...
        // Write content to iframe and adjust height
        ...
        //
        // // Adjust height when iframe content changes
