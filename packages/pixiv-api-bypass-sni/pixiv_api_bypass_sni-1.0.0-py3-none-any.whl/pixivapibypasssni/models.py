...
class Illustration:
    ...
    def __init__(
        ...
        self.id = id
        self.image_urls = {
            **{Size(s): (u.replace("i.pximg.net", alt_image_site, 1) if use_alt_image_site else u) for s, u in image_urls.items()},
            Size.ORIGINAL: meta_single_page.get("original_image_url", None),
        }
        if use_alt_image_site:
            self.image_urls[Size.ORIGINAL] = self.image_urls[Size.ORIGINAL].replace("i.pximg.net", alt_image_site, 1)
        self.is_bookmarked = is_bookmarked
        self.is_muted = is_muted
        self.meta_pages = [
            {Size(s): (u.replace("i.pximg.net", alt_image_site, 1) if use_alt_image_site else u) for s, u in p["image_urls"].items()} for p in meta_pages
        ]
