...
class DefinePeriods(Extract):
    ...
    def period_zero(self,col):
        ...
        if frequency_ == 'W':   
            df_ = df_[(df_['Date']>self.df_Prep['timestamp'].max()) & (df_['Date']>=self.actual_date) & (df_['Date']<= (self.actual_date + relativedelta(weeks=1)))]
    ...
    def period_all(self,col):
        ...
        if frequency_ == 'M': 
            ...
            df_extract_forecast = df_extract_forecast.groupby(self.columns_group, as_index=False).agg({self.column_forecast:sum}) 
            df_extract_forecast.rename(columns={self.column_forecast:'Next'}, inplace=True)
        ...
        if frequency_ == 'W': 
            df_ = df_[(df_['Date']>(self.actual_date+relativedelta(weeks=1))) & (df_['Date']<= (self.actual_date + relativedelta(weeks=(self.period+1))))]
