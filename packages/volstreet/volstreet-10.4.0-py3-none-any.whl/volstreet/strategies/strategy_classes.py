...
class BaseStrategy(ABC):
    ...
    def generate_position_details_file_path(self, underlying: Index | Stock) -> Path:
...
class TrendV2(BaseStrategy):
    ...
    def logic(
        ...
        position_details_file = self.generate_position_details_file_path(underlying)
...
class ReentryStraddle(BaseStrategy):
    ...
    def logic(
        ...
        recording_task = partial(
            record_position_status, reentry_position, position_details_file
        )
        recording_task = timed_executor(55)(recording_task)
        while current_time().time() < time(*exit_time):
            if all(
            ...
            for option in (
                reentry_position.all_calls[:1] + reentry_position.all_puts[:1]
...
class ThetaXDelta(BaseStrategy):
    ...
    def logic(
        ...
        # Identifying the strangle
        strangle = identify_strangle(underlying, equality_constraint=True)
        ...
        while (
            current_time().time() < time(*exit_time)
            and not position_manager.exit_triggered()
        ):
            ...
            if position_manager.current_state != "neutral":
                ...
                theta_profit_target = (
                    position_manager.aggregate_greeks().theta <= exposure * 0.0002
                )
                profit_pct_target = position_manager.mtm() / exposure >= 0.0007
            ...
            if position_manager.current_state == "neutral":
                ...
                if (
                    ...
                    notifier(
                        f"{underlying.name} {self.strategy_tag} both stop-losses hit. "
                ...
                if actionable_direction != "":
                    ...
                    actionable_direction = ""
                    notifier(
                        f"{underlying.name} {self.strategy_tag} state changed to {position_manager.current_state}",
                        self.webhook_url,
                        "INFO",
                    )
                    continue
            ...
            # If we are long, check for call reentry
                ...
                if anchor_call.fetch_ltp() <= anchor_call.avg_price:
                    ...
                    notifier(
                        f"{underlying.name} {self.strategy_tag} state changed to {_state}",
                        self.webhook_url,
                        "INFO",
                    )
                    continue
        ...
        notifier(
            f"{underlying.name} {self.strategy_tag} exiting. Reasons: {position_manager.exit_triggers}",
            self.webhook_url,
            "INFO",
        )
    ...
    def logic(
        ...
        save_json_data(
            ...
            Path(
                ...
                f"{underlying.name}_overnight_positions.json",
            ),
...
class BiweeklyStraddle(BaseStrategy):
    ...
    def logic(
        ...
        save_json_data(
            ...
            Path(
                f"{ActiveSession.obj.userId}",
                f"{underlying.name}_biweekly_position.json",
            ),
