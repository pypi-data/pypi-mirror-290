...
LastEditTime: 2024-08-15 00:16:58
FilePath: \python\multithreaded_downloader000\downloader\downloader.py
...
class DownloadThread(threading.Thread):
    def __init__(self, bytes_queue: Queue, url, proxies=None):
        super().__init__(daemon=True)
        self.bytes_queue = bytes_queue
        self.url = url
    ...
    def run(self):
        while not self.bytes_queue.empty():
            ...
            try:
                ...
                with open(f"{temp_dir}/{bytes_range[0]}.tmp", "wb") as f:
                    f.write(response.content)
            except Exception as e:
                print(f"Failed to download range {bytes_range[1]}: {e}")
            finally:
                self.bytes_queue.task_done()

def get_file_size(url, proxies=None) -> int:
...
def get_thread_download(file_length, copies_count) -> Queue:
    bytes_queue = Queue()
    chunk_size = file_length // copies_count
    for i in range(copies_count):
        start_bytes = i * chunk_size
        end_bytes = (file_length - 1) if i == copies_count - 1 else (start_bytes + chunk_size - 1)
        bytes_length = f"{start_bytes}-{end_bytes}"
        bytes_queue.put([i, bytes_length])
    return bytes_queue

class MultiThreadedDownloader:
    def __init__(self, url, filename, thread_count=8, copies_count=10, proxies=None):
        self.url = url
        self.filename = filename
        self.thread_count = thread_count
        self.copies_count = copies_count
    ...
    def create_threads(self, bytes_queue):
        thread_list = []
        for _ in range(self.thread_count):
            thread = DownloadThread(bytes_queue, self.url, self.proxies)
            thread.start()
            thread_list.append(thread)
        for thread in thread_list:
            thread.join()
    ...
    def download(self):
        if not os.path.exists('files'):
            os.makedirs('files')
        file_length = get_file_size(self.url, self.proxies)
        copies_queue = get_thread_download(file_length, self.copies_count)
        self.create_threads(copies_queue)
        copies_queue.join()  # Ensure all threads are completed
        return self.composite_file()
