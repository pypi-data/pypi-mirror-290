...
class СтильДруку(enum.Enum):
    ...
    def отримати(стиль):
        if стиль is Жоден:
            обраний_стиль = СтильДруку.замовчання()
...
def є_екземпляром(змінна, тип_данних):  # real signature unknown; restored from __doc__
    """
    Повертає "істина" якщо "змінна" є екземляром классу або підкласу вказаному у "тип_данних"
    """
    return isinstance(змінна, тип_данних)

def отримати_атрибут(обєкт, імя, за_замовчанням=Жоден):  # known special case of getattr
    """
    ...
    Коли задан параметра "за_замовчанням" то функція повертає його коли атрибута
    не існує в іншому випадку ініціюється виключна ситуація
    """
    return getattr(обєкт, імя, за_замовчанням)
