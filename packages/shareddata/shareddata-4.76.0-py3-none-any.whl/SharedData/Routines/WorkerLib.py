...
def run_routine(command, routine):
    ...
    if installed:
        ...
        routine['subprocess'] = subprocess.Popen(cmd, env=env, stderr=subprocess.PIPE)
            ...
            try:
                if 'subprocess' in routine:
                    ...
                    if exit_code is not None and exit_code != 0:
                        stderr_output = routine['subprocess'].stderr.read().decode('utf-8')
                        Logger.log.error(f"Routine finished with error: {stderr_output}")
