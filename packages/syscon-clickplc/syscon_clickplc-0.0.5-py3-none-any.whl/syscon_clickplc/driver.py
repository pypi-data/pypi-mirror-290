...
from syscon_clickplc.util import AsyncioModbusClient
...
class ClickPLC(AsyncioModbusClient):
    ...
    data_types: ClassVar[dict] = {
        ...
        'dd': 'int32'    # (D)ouble (D)ata register
    }
    ...
    async def _get_dd(self, start: int, end: int) -> dict:
        """Read DD registers. Called by `get`.

        DD entries start at Modbus address 416384 (416385 in the Click software's
        1-indexed notation). Each DD entry takes 32 bits, which is 2 16bit registers.
        """
        if start < 1 or start > 1000:
            raise ValueError('CTD must be in [1, 1000]')
        if end is not None and (end < 1 or end > 1000):
            raise ValueError('CTD end must be in [1, 1000]')
    ...
    async def _set_ctd(self, start: int, data: list[int] | int):
        """Set CTD registers. Called by `set`.

        See _get_ctd for more information.
        """
        if start < 1 or start > 250:
            raise ValueError('CTD must be in [1, 250]')
        ...
        for data in parsed.values():
            ...
            if not data['type']:
                raise TypeError(
                    ...
                    "github issue at numat/syscon_clickplc to get it added."
                )
