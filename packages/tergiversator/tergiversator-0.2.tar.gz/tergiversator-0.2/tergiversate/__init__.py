...
def unmount_if_mounted(mount_path, my_env):
    ...
    if os.path.ismount(mount_path):
        output = subprocess.run(["umount", mount_path], env=my_env, \
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=True).stdout.decode()

    return output

def mount_remote(mount_path, user, hostname, pathname, stricthostkey, port):
    ...
    brk = 0
    try:
        output += subprocess.run(["sshfs", user + "@" + hostname + ":" + pathname, \
            mount_path, "-o", f"StrictHostKeyChecking={stricthostkey}", \
                "-p", f"{port}"], stdout=subprocess.PIPE, \
                stderr=subprocess.STDOUT, check=True).stdout.decode()
    except subprocess.CalledProcessError as error:
        output += f"{error}\n"
        summary += f"{hostname}:{pathname} - Unavailable\n"
        brk = 1
