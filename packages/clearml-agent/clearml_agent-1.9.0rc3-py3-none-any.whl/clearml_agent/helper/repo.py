...
def _is_script_patched(lines):
    return len(lines or []) >= 2 and lines[0].strip().startswith('from clearml ') and 'Task.init' in lines[1]


def _is_script_patched_new(lines):
    return len(lines or []) >= 3 and lines[0].strip().startswith('try: from allegroai ') and 'Task.init' in lines[2]
...
def fix_package_import_diff_patch(entry_script_file):
    ...
    is_patched = _is_script_patched(lines)
    is_patched_new = _is_script_patched_new(lines)
    if not (is_patched or is_patched_new):
        return

    patch_len = 3 if is_patched_new else 2  # how many lines is the patch

    original_lines = lines
    # skip over the first two or three lines, these are ours
    # then skip over empty or comment lines
    lines = [(i, line.split('#', 1)[0].rstrip()) for i, line in enumerate(lines)
             if i >= patch_len and line.strip('\r\n\t ') and not line.strip().startswith('#')]
    ...
    # now we need to move back the patched lines
    entry_line = lines[found_index][0]
    new_lines = original_lines[patch_len:entry_line + 1] + original_lines[0:patch_len] + original_lines[entry_line + 1:]
    # noinspection PyBroadException
    try:
...
def patch_add_task_init_call(local_filename):
    ...
    if _is_script_patched(lines[idx_a:]) or _is_script_patched_new(lines[idx_a:]):
