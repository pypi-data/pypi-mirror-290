...
class IdeManager:
    ...
    def __post_init__(self):
        ...
        if 'globals' in excluded:
            raise BuildError("It's not possible to use `SANS='globals`, because it would break "
                             "pyodide runner itself.")
    ...
    def __str__(self):
        return f"{self.__class__.__name__}('{self.py_name}', ...), in file {self.env.file_location()}"
    ...
    def _validation_outcome(self, msg:Optional[str], config_opt:Optional[str]=None):
        ...
        if config_opt:
            msg += (
                ...
                f"{config_opt} to false in `mkdocs.yml`."
            )
    ...
    def id_to_hash(self, clear:str):
        ...
        if not self.env.is_unique_then_register(html_id):
            raise BuildError(
