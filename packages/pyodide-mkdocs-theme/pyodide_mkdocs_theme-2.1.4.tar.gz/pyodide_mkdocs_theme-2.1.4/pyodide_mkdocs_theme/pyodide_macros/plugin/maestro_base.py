...
class BaseMaestro(BasePlugin[PyodideMacrosConfig]):
    ...
    args_IDE_MIN_SIZE: int = ConfigExtractor('args.IDE', prop='MIN_SIZE')
    ...
    #   - others: from PyodideMacrosPlugin.on_config
    ...
    def on_config(self, config:MkDocsConfig):       # pylint: disable=missing-function-docstring
        ...
        ArgConfig.update_lang_defaults_with_current_lang(self)
    ...
    def dump_to_js_config(self, base_url):
        ...
        with BaseMaestro._dumpable_state(self, base_url):
...
{ "".join(dumping) }
  ...
  CONFIG.pythonLibs = new Set(CONFIG.pythonLibs)
    ...
    @staticmethod
    @contextmanager
    def _dumpable_state(maestro: Optional['BaseMaestro'], base_url:str):
        """
