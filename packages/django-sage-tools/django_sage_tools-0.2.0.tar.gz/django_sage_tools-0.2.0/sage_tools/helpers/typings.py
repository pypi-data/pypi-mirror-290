...
where specific data types need to be more clearly represented. These types
are created using the `NewType` helper function from the `typing` module,
which creates distinct types based on existing ones, but they are still
treated as their base types by the Python interpreter.
used to emphasize its role in representing time in seconds.
