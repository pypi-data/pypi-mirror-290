...
class OracleTools:
    ...
    @staticmethod
    def copy_data(src_db, dst_db, src_onwer, src_table, dst_onwer, dst_table, commit_rownum=3000):
        ...
        if src_onwer == '' or src_onwer is None:
            select_sql = 'select * from ' + src_table
        else:
            select_sql = 'select * from ' + src_onwer + '.' + src_table
        ...
        if dst_onwer == '' or dst_onwer is None:
            insert_sql = 'insert into ' + dst_table + ' values(' + s + ')'
        else:
            insert_sql = 'insert into ' + dst_onwer + '.' + dst_table + ' values(' + s + ')'
            
        params = []
        num = 0
        while rs.next():
            if len(params) == commit_rownum:
                num = num + dst_db.run(insert_sql, params)
                params.clear()
            else:
                params.append(rs.get_row())
