        ...
        if cloud_provider == CloudProvider.GCP:
            ...
            if existing_environment is not None:
                ...
                if prompt:
                    ...
                    # us-west-2, us-west-1, af-south-1, ap-east-1, ...
                    all_regions = session.get_available_regions("s3")
                    region_base = "-".join(region.split("-")[:-1])  # e.g. "us-west"
                    region_options = [region]
                    other_regions = []
                    for r in all_regions:
                        if r.startswith(region_base) and r != region:
                            region_options.append(r)
                        elif r != region:
                            other_regions.append(r)
                    sorted_regions = region_options + other_regions
