...
class ReleaseServicePlan(ServicePlan):
    ...
    aws_environment_config: Optional[AWSEnvironmentConfig]
    deployment_id: str
    docker_step: Literal["build", "promote"]
    ...
    async def execute_plan(
        ...
        if isinstance(self.service, CloudRun):
            service_url = await release_docker_image_to_cloud_run(
                docker_image=docker_image_result.docker_image,
    ...
    async def execute_plan(
        ...
        async with self._lock as lock_info:
            ...
            try:
                ...
                build_service_result: BuildServiceResult = results[0]  # type: ignore
...
class PromoteServicePlan(ServicePlan):
    ...
    def print_plan(
        ...
        left_padding_str = " " * left_padding
        console.print(
            f"{left_padding_str}{ServiceRef(self.service)} will be [{OP_COLOR}]promoted[/{OP_COLOR}] from {EnvironmentRef(self.from_service_manager)} to {EnvironmentRef(self.to_service_manager)} with the following workflow:"
        )
