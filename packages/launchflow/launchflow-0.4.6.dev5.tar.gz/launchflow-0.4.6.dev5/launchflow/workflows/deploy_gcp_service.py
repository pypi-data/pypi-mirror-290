...
async def release_docker_image_to_cloud_run(
    ...
    gcp_region = gcp_environment_config.default_region
    if cloud_run_service.region:
        # TODO: we should probably grab the region from outputs instead of inputs in case the user skips a create step
        gcp_region = cloud_run_service.region

    client = run_v2.ServicesAsyncClient()
    service = await client.get_service(
        name=f"projects/{gcp_environment_config.project_id}/locations/{gcp_region}/services/{cloud_run_service._cloud_run_service_container.resource_id}"
    )
...
async def poll_mig_updating(
    ...
    loop = asyncio.get_event_loop()
    mig = await loop.run_in_executor(None, get_mig)
...
async def release_docker_image_to_compute_engine(
            ...
            metadata=compute.Metadata(
                items=[
                    ...
                    compute.Items(
                        ...
                        value=json.dumps(
                            {
                                "spec": {"containers": [{"image": docker_image}]},
                                "volumes": [],
    ...
    loop = asyncio.get_event_loop()
    template_op = await loop.run_in_executor(None, insert_template)
    ...
    update_op = await loop.run_in_executor(None, update_instances)
