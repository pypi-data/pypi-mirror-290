...
class SpyAgent(BasicAgent):
    ...
    def __init__(self, name):
        ...
        # 拼接目标文件的完整路径
        path = os.path.join(current_folder_path, 'meta/simple_spy_chinese/human_records.json')
        self.human_record = HumanRecord(filename=path)
        super().__init__(name)
    ...
    def interact(self, req=AgentReq) -> AgentResp:
        ...
        if req.status == STATUS_ROUND:
            #if self.memory.load_variable('undercoverProbability') > 50:
            logger.info('----------自己正常发言-----------')
            logger.info('---------其他人的发言为:{}-------------'.format(othersSpeck))
            logger.info('---------异常概率为:{}-------------'.format(str(self.memory.load_variable("undercoverProbability"))))
            ...
            logger.info("-------自己发言的prompt:" + prompt)
            # # 去重检验，根据memery中其他人的发言，进行去重检测，若重复，则增加特殊标记
            result = check_and_update_result(result, othersSpeck)
            myWord = self.memory.load_variable("word")
            logger.info("-------大模型输出的结果:{}----".format(result))
            logger.info("-------我拿到的卧底词为:{}----".format(str(myWord)))
            if myWord in result:
                result.replace(result, " ")
                logger.info("---------出现了相应的特殊词,进行替换---------")
            # 如果大模型返回为空,选择别人发言回答
