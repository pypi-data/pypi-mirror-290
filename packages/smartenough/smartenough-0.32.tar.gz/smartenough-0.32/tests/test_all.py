...
def test_url_validation_others():
    def generate_url(model_provider):
        result = get_smart_answer("Give me a list of some interesting news sites, return only valid urls please", model_provider, validation="url")
        assert isinstance(result, list)
        assert all(url.startswith("http") for url in result)
