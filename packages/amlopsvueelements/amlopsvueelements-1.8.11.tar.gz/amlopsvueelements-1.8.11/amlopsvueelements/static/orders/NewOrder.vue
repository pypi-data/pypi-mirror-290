        ...
        <div class="content-wrap flex flex-col">
          ...
          <div class="w-full flex">
            <MultiselectField class="w-11/12" v-model="airGroupModel" group-values="data" group-label="group"
              :group-select="true" label-text="Aircraft" placeholder="Please select aircraft" :show-no-result="false"
              :disabled="!meta && !isAdmin || !formModel.client" :errors="validationInfo?.aircraft?.$errors"
              :is-validation-dirty="validationInfo?.$dirty" label="full_repr" :options="airGroups" track-by="full_repr">
            </MultiselectField>
...
const arrivalDateTime = ref({
  date: new Date(new Date().getTime()).toLocaleDateString('en-CA'),
  time: new Date(new Date().getTime() + (1 * 60 * 60 * 1000)).toTimeString(),
  timezone: 'Local'
})
...
} = useFetch<IAircraftType[], (id: number) => Promise<IAircraftTypeEntity[]>>(async (id: number) => {
  return await OrderReferences.fetchAircraftTypes(id as number)
})
const {
  ...
  return await OrderReferences.fetchAircrafts(id as number)
})
const {
...
const updateFuelBeforeAfter = (val: string) => {
  ...
  console.log(formModel.value.fuel_order!.fueling_on, val)
}
...
watch(() => airGroupModel.value, (value) => {
  if (!value) return
  if (value.type === 'AircraftType') {
    formModel.value.aircraft = null;
    formModel.value.aircraft_type = value;
