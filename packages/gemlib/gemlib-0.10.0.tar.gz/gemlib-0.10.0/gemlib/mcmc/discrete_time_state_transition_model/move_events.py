...
from gemlib.distributions import UniformInteger, UniformKCategorical
...
def _make_event_count_selector(
    src_dest_ids, count_bounding_fn, count_max, name
):
...
def discrete_move_events_proposal(
    ...
    def proposal():
        ...
        count_selector = _make_event_count_selector(
            src_dest_ids, count_bounding_fn, count_max, "event_count"
        )
...
first event item only.",
    ...
    def bootstrap_results(self, init_state):
        with tf.name_scope("uncalibrated_event_times_rw/bootstrap_results"):
            ...
            proposal_example = self._proposal(init_state[0]).sample(seed=[0, 0])
