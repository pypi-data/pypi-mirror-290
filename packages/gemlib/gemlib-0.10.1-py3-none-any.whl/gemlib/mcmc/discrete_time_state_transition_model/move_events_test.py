...
def test_discrete_move_events_proposal(sir_metapop_example):
    num_units = 2
    ...
    sample = proposal.sample(seed=[0, 1])
    lp = proposal.log_prob(sample)
...
# )
def test_uncalibrated_event_time_update(sir_metapop_example):
    def tlp(_):
    ...
    pkr = kernel.bootstrap_results(sir_metapop_example["events"])
    next_events, results = kernel.one_step(
    ...
    events, results = tf.scan(
        one_step, elems=seeds, initializer=(sir_metapop_example["events"], pkr)
    )
