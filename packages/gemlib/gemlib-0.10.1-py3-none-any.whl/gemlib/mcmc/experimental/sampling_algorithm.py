...
ChainAndKernelState = Tuple[ChainState, KernelState]
...
class KernelInitMonad:
    ...
    def then(self, next_kernel: KernelInitMonad):
...
class SamplingAlgorithm:
    ...
    def __init__(
        ...
        if isinstance(init_fn, KernelInitMonad) and isinstance(
            ...
            self.__init: KernelInitMonad = init_fn
            self.__step: KernelStepMonad = step_fn
        else:
            self.__init: KernelInitMonad = KernelInitMonad(init_fn)
            self.__step: KernelStepMonad = KernelStepMonad(step_fn)
    ...
    def step(self, *args, **kwargs):
        ...
        return self.__step(*args, **kwargs)

    def then(self, next_kernel: SamplingAlgorithm):
        """Sequential combinator"""
        return SamplingAlgorithm(
            init_fn=(self.__init >> next_kernel.__init),
            step_fn=(self.__step >> next_kernel.__step),
        )
