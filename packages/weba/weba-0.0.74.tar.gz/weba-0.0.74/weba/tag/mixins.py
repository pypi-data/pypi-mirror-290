...
class TagMixins(Tag):
    ...
    def find_comment(self, string: str) -> "TagContextManager | None":
        return self._wrap_result(  # type: ignore
            self.find(string=lambda text: isinstance(text, Comment) and text.strip().startswith(string)).find_next()  # type: ignore
        )

    def select_comment(self, string: str) -> "TagContextManager | None":
        return self.find_comment(string)
    ...
    def find_comments(self, string: str) -> list["TagContextManager"]:
        ...
        return [self._wrap_result(comment.find_next()) for comment in comments]  # type: ignore

    def select_comments(self, string: str) -> list["TagContextManager"]:
        return self.find_comments(string)
