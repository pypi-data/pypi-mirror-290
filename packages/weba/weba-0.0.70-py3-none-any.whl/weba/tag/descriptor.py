    ...
    def __get__(self, instance: Union[T, None], owner: Type[T]) -> Tag:
        ...
        if tag is None:
            if self._selector or self._comment:
                if not self._selector and self._comment:
                    if found_tag := instance.find_comment(self._comment):
                        found_tag = found_tag.find_next()
                elif self._selector:
                    found_tag = instance.select_one(self._selector)

                    if self._comment:
                        found_tag = instance.find_comment(self._comment)
                        found_tag = found_tag.find_next()  # type: ignore

                if not self._strict:
                    return found_tag  # type: ignore

                if found_tag is None or (not self._selector and not self._comment):
                    class_path = f"{instance.__class__.__module__}.{instance.__class__.__qualname__}"
                ...
                if self._extract:
                    ...
                    if self._selector:
                        ...
                        for comment in instance.find_comments(self._comment):
                            if comment_tag := comment.find_next():
                                if not isinstance(comment_tag, NavigableString):
                                    comment_tag.decompose()

                if not isinstance(found_tag, Tag):
                    found_tag = Tag(found_tag, instance._html)  # type: ignore (we need to access private property)
