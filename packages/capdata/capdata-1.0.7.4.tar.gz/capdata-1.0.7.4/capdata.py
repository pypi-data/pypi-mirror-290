...
def get_bond_yield_curve(curve, start, end, freq='1d', window=None):
    return data.curve.get_bond_yield_curve(curve, start, end, freq, window)
...
def get_bond_spread_curve(curve, start, end, freq='1d', window=None):
    return data.curve.get_bond_spread_curve(curve, start, end, freq, window)
...
def get_ir_yield_curve(curve, start, end, freq='1d', window=None):
    return data.curve.get_ir_yield_curve(curve, start, end, freq, window)
...
def get_bond_yield_curve_list(ccy: List[str], ir_index: List[str]):
    return data.refer.get_bond_yield_curve_list(ccy, ir_index)
...
def get_bond_credit_curve_list(ccy: List[str], ir_index: List[str]):
    return data.refer.get_bond_credit_curve_list(ccy, ir_index)
...
参数:
  ...
  ccy -- 货币列表 ['CNY']
  bond_mkt_type -- 市场类型列表 ['TREASURY_BOND']
...
def get_bond_list(market: List[str], ccy: List[str], bond_mkt_type: List[str], bond_inst_type: List[str],
    ...
    return data.refer.get_bond_list(market, ccy, bond_mkt_type, bond_inst_type, credit_rating)
