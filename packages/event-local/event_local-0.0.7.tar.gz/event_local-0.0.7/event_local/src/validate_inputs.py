...
class ValidateInputs(GenericCRUD):
    ...
    def validate_location_id(self, location_id: int) -> None:
        if location_id is None or not isinstance(location_id, int) or location_id < 0:
            raise ValueError(f"Invalid location_id: {location_id} (type: {type(location_id)})")
        _location_id = self.select_one_value_by_column_and_value(
            schema_name="location", view_table_name="location_view", select_clause_value="location_id",
            column_name="location_id", column_value=location_id)
        if _location_id is None:
            raise ValueError(f"Invalid location_id: {location_id} (not found in location.location_view)")

    def validate_organizers_profile_id(self, organizers_profile_id: int) -> None:
        if not organizers_profile_id or not isinstance(organizers_profile_id, int) or organizers_profile_id <= 0:
            raise ValueError(
        ...
        _organizers_profile_id = self.select_one_value_by_column_and_value(
            schema_name="profile", view_table_name="profile_view", select_clause_value="profile_id",
            column_name="profile_id", column_value=organizers_profile_id)
    ...
    def validate_input_dict(self, input_dict: dict) -> None:
        ...
        if facebook_event_url and not validate_url(facebook_event_url):
            raise ValueError(f"Invalid input: facebook_event_url - {facebook_event_url}")
    ...
    def validate_id(self, entity_id: int, schema: str = "event") -> None:
        ...
        value = self.select_one_value_by_column_and_value(schema_name=schema, view_table_name=schema + "_view",
                                                          select_clause_value=schema + "_id",
                                                          column_name=schema + "_id",
                                                          column_value=entity_id)
        if value is None:
            raise ValueError(f"Invalid {schema}_id: {entity_id} (not found in {schema}.{schema}_view)")
