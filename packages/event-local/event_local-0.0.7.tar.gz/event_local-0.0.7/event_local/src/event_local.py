from database_mysql_local.generic_crud import GenericCRUD, DEFAULT_SQL_SELECT_LIMIT
...
class EventsLocal(GenericCRUD, metaclass=MetaLogger, object=EVENTS_LOCAL_CODE_LOGGER_OBJECT):
    def __init__(self, is_test_data: bool = False) -> None:
        super().__init__(default_schema_name="event", default_column_name="event_id",
                         default_table_name="event_table", default_view_table_name="event_view",
                         is_test_data=is_test_data)
        self.validate_inputs = ValidateInputs(is_test_data=is_test_data)

    # TODO: test
    def select_all_events(self, lang_code_str: str, limit: int = None) -> list:
        ...
        sql_query = f"""
            ...
            LIMIT {limit}"""
        self.cursor.execute(sql_query, (lang_code_str,))
        columns = ("event_id, item_type_id, location_id, organizers_profile_id, website_url, "
    ...
    def select_events_by_title(self, title: str, limit: int = None) -> list:
        ...
        events_list = self.select_multi_dict_by_column_and_value(
            view_table_name="event_ml_view", column_name="title", column_value=title, limit=limit)
        return events_list
    ...
    def select_events_by_profile_id(self, profile_id: int, limit: int = None) -> list:
        ...
        columns = ("event.event_id, eventlocation_id, eventorganizers_profile_id, eventwebsite_url, "
    ...
    def select_by_event_id(self, event_id: int) -> dict:
        ...
        event = self.select_one_dict_by_column_and_value(column_value=event_id, select_clause_value=select_clause)
        return event
    ...
    def update_by_event_id(self, event_id: int, event_data: dict) -> int:
        ...
        self.validate_inputs.validate_id(event_id)
        self.validate_inputs.validate_input_dict(input_dict=event_data)
        amount_of_events_updated = self.update_by_column_and_value(column_value=event_id, data_dict=event_data)
        return amount_of_events_updated

    def delete_by_event_id(self, event_id: int) -> int:
        """Deletes the event row with the given event_id"""
        self.validate_inputs.validate_id(event_id)
        amount_of_events_deleted = self.delete_by_column_and_value(column_value=event_id)
    ...
    def get_test_event_id(self) -> int:
        ...
        test_event_id = super().get_test_entity_id(
            entity_name="event", insert_function=self.insert,
            view_name="event_view", insert_kwargs={'location_id': location_id})
    ...
    def get_number_of_invitations_for_event(self, event_id: int) -> int:
        sql_statement = f"""SELECT COUNT(*)
        FROM event_profile.event_profile_view
        WHERE event_id={event_id} AND is_invited=TRUE"""
        self.cursor.execute(sql_statement=sql_statement)
        number_of_invitations_for_event = self.cursor.fetchone()[0]
        return number_of_invitations_for_event
    
    def get_number_of_registrations_for_event(self, event_id: int) -> int:
        sql_statement = f"""SELECT COUNT(*)
        FROM event_profile.event_profile_view
        WHERE event_id={event_id} AND is_user_filled_registration=TRUE"""
        self.cursor.execute(sql_statement=sql_statement)
        number_of_registrations_for_event = self.cursor.fetchone()[0]
        return number_of_registrations_for_event
    
    def get_number_of_approved_by_admin_for_event(self, event_id: int) -> int:
        sql_statement = f"""SELECT COUNT(*)
        FROM event_profile.event_profile_view
        WHERE event_id={event_id} AND is_approved_by_admin=TRUE"""
        self.cursor.execute(sql_statement=sql_statement)
        number_of_approved_by_admin = self.cursor.fetchone()[0]
        return number_of_approved_by_admin
    
    def get_number_of_participants_for_event(self, event_id: int) -> int:
        sql_statement = f"""SELECT COUNT(*)
        FROM event_profile.event_profile_view
        WHERE event_id={event_id} AND is_will_arrive=TRUE"""
        self.cursor.execute(sql_statement=sql_statement)
        number_of_participants = self.cursor.fetchone()[0]
