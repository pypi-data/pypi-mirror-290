...
from .constants_event_local import EVENT_CODE_LOGGER_OBJECT


# TODO: test
class HolidayLocal(GenericCRUD, metaclass=MetaLogger, object=EVENT_CODE_LOGGER_OBJECT):
    """HolidayLocal"""

    def __init__(self, is_test_data: bool = False) -> None:
        super().__init__(default_schema_name="event",
                         default_table_name="event_table",
                         default_view_table_name="event_view",
                         default_column_name="event_id",
                         is_test_data=is_test_data)

    def insert_holiday(self, *, date: str, holiday: str,
                       country: str, state: str = None) -> int:
        ...
        location_id = 3  # TODO
        if self.holiday_exists(date=date, holiday=holiday, country=country, state=state):
            event_table_data = {
    ...
    def update_holiday(self, *, holiday_id: int, holiday_data: Dict[str, any]) -> None:
        ...
        super().update_by_column_and_value(column_value=holiday_id, data_dict=event_table_data)
    ...
    def holiday_exists(self, *, date: str, holiday: str, country: str, state: str = None) -> bool:
