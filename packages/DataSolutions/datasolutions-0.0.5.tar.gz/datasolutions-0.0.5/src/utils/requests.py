...
from src.utils.exceptions import BadRequest, DataSolutionsAPIException
...
def retry_if_not_bad_request(exception):
    if isinstance(exception, BadRequest):
        return False  # Do not retry if it's a BadRequestException
    return True  # Retry for other exceptions


@retry(
    wait=wait_exponential(multiplier=1, min=4, max=10),
    retry=retry_if_exception(retry_if_not_bad_request),
