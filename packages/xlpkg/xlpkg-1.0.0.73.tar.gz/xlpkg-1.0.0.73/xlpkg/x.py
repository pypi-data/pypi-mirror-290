import re,os,ast,sys,time,json,string,logging,pymysql,webbrowser, random, inspect, urllib3, binascii, hashlib, datetime,glob,uuid
import subprocess as subp,importlib,pandas as pd
...
#from PyQt6.QtGui import QIcon,QDesktopServices as QDS,QAction
...
def f_assert(r,n=0):
    match n:
        case 0:
            if r.json()['success']==True and r.reason=='OK' and r.status_code==200:
                return(True)
        case 1:
...
def f_str2dt(dt_str,n=0):
    ...
    match n:
        case 0:
            fmt='%Y-%m-%d %H:%M:%S'
            ret=datetime.strptime(dt_str,fmt)
        case 1:
            fmt = '%Y-%m-%d'
            ret = datetime.strptime(dt_str, fmt)
        case 2:
...
#日期时间转时间戳
def f_dt2ts(dt,n=0):
    ts=datetime.datetime.fromtimestmp(dt)
    ret=''
    match n:
        case 0:
...
import os, sys, time, json, string, logging, pymysql, webbrowser
import random
from selenium import webdriver
from collections.abc import Iterable, Iterator, ItemsView
...
def f_conn(n):
    ...
    match n:
        ...
        case 'ptkc_pre'|'prod_prod':    #跑腿快车生产环境
        ...
        case 'zt'|'zentao':             #禅道
            ...
            pass
        case 'dj' | 'daijia':  # 禅道
            conn = pymysql.connect(host='t-mysql.driving.gxptkc.com', user='chauffeur', password='X4bPAmkWKAbo', database='kcdx')
            pass
    return(conn)
    pass
...
#日期时间
def f_dt(n=0,fmt=0):
    curr=datetime.datetime.now()+datetime.timedelta(days=n)
    match fmt:
        case 0:
