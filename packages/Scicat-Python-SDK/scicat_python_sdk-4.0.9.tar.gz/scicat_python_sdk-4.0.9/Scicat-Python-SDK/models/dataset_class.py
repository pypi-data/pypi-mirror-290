...
class DatasetClass(BaseModel):
    ...
    end_time: Optional[datetime] = Field(default=None, description="End time of data acquisition for this dataset, format according to chapter 5.6 internet date/time format in RFC 3339. Local times without timezone/offset info are automatically transformed to UTC using the timezone of the API server.", alias="endTime")
    creation_location: StrictStr = Field(description="Unique location identifier where data was taken, usually in the form /Site-name/facility-name/instrumentOrBeamline-name. This field is required if the dataset is a Raw dataset.", alias="creationLocation")
    ...
    sample_id: Optional[StrictStr] = Field(default=None, description="ID of the sample used when collecting the data.", alias="sampleId")
    instrument_id: Optional[StrictStr] = Field(default=None, description="ID of the instrument where the data was created.", alias="instrumentId")
    ...
    input_datasets: Optional[List[StrictStr]] = Field(default=None, description="Array of input dataset identifiers used in producing the derived dataset. Ideally these are the global identifier to existing datasets inside this or federated data catalogs. This field is required if the dataset is a Derived dataset.", alias="inputDatasets")
    used_software: Optional[List[StrictStr]] = Field(default=None, description="A list of links to software repositories which uniquely identifies the pieces of software, including versions, used for yielding the derived data. This field is required if the dataset is a Derived dataset.", alias="usedSoftware")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.origdatablocks:
            for _item_origdatablocks in self.origdatablocks:
                if _item_origdatablocks:
                    _items.append(_item_origdatablocks.to_dict())
            _dict['origdatablocks'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in datablocks (list)
        _items = []
        if self.datablocks:
            for _item_datablocks in self.datablocks:
                if _item_datablocks:
                    _items.append(_item_datablocks.to_dict())
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "origdatablocks": [OrigDatablock.from_dict(_item) for _item in obj["origdatablocks"]] if obj.get("origdatablocks") is not None else None,
            "datablocks": [Datablock.from_dict(_item) for _item in obj["datablocks"]] if obj.get("datablocks") is not None else None,
