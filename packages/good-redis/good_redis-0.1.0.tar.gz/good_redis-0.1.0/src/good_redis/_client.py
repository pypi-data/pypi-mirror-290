...
class Redis(StrictRedis):
    ...
    def psdelete(self, name: str):
        self.delete(f"{name}:t", f"{name}:s", f"{name}:exc")
    ...
    def psexpire(self):
        ...
        for key in self.smembers(":ps:keys"):
            ...
            for ekey in expired_keys:
                ...
                # self.sadd(f'{key}:exc', ekey)
            # logger.info(key)
            # for key in self.zrange(f'{key}:s', 0, -1, withscores=True):
            #     if key[1] > ts:
            #         logger.info(key)
            #         self.hdel(f'{key}:t', key[0])
            #         self.zrem(f'{key}:s', key[0])
            #         self.sadd(f'{key}:exc', key[0])
...
class AsyncRedis(AsyncRedisBase):
    ...
    async def psdelete(self, name: str):
        async with self.pipeline() as pipe:
            await pipe.delete(f"{name}:t", f"{name}:s", f"{name}:exc")
            await pipe.rem(":ps:keys", name)
