...
from qecore.utility import get_application, get_center
...
START_OPTIONS = ' | via "{start_via}" | via command "{command}" | via command | in "{session}"'


@step('{m_btn} click "{name}" "{role_name}"' + DESC + ATTR + ROOT + SIZE_POS_DEC)
...
@step('Mouse over "{name}" "{role_name}"' + DESC + ATTR + ROOT + SIZE_POS_DEC)
def mouse_over(context, retry=True, expect_positive=True, **kwargs) -> None:
    ...
    with GetNode(context, **kwargs, retry=retry, expect_positive=expect_positive) as (_, node):
        try:
            # Dogtail has a mistake in implementation for some tree nodes.
            node.point()
        except NotImplementedError:
            # In case of that mistake, use a workaround.
            absoluteMotion(*(get_center(node)), window_id=node.window_id)


@step('Make an action "{action}" for "{name}" "{role_name}"' + DESC + ATTR + ROOT + SIZE_POS_DEC)
...
@step('Item "{name}" "{role_name}" | found |' + DESC + ATTR + ROOT + SIZE_POS_DEC)
...
@step('Item "{name}" "{role_name}" | was not found |' + DESC + NOT_ATTR + ROOT + SIZE_POS_DEC)
...
@step('Item "{name}" "{role_name}"' + DESC + TEXT + ROOT + SIZE_POS_DEC)
...
@step('Item "{name}" "{role_name}"' + DESC + NOT_TEXT + ROOT + SIZE_POS_DEC)
...
@step('Item "{name}" "{role_name}"' + NOT_DESC + ATTR + ROOT + SIZE_POS_DEC)
...
@step('Wait until "{name}" "{role_name}"' + DESC + ATTR + ROOT + SIZE_POS_DEC)
...
@step('Wait until "{name}" "{role_name}" | appears' + DESC + ROOT + SIZE_POS_DEC)
...
@step('Wait until "{name}" "{role_name}" | disappears' + DESC + NOT_ATTR + ROOT + SIZE_POS_DEC)
...
@step('Type text: "{text}"')
def type_text(context, text) -> None:  # pylint: disable=unused-argument
    """
...
def press_key(context, key_name) -> None:  # pylint: disable=unused-argument
    """
...
def key_combo(context, combo_name) -> None:  # pylint: disable=unused-argument
    """
...
def absolute_motion(context, position_x, position_y) -> None:  # pylint: disable=unused-argument
    """
