...
def get_center(node) -> Tuple:
    ...
    return Tuple(
        (node.position[0] + node.size[0] / 2, node.position[1] + node.size[1] / 2)
    )
...
def patch_scenario(scenario, max_attempts=3, stability=False):
    ...
    def scenario_run_with_retries(scenario_run, *args, **kwargs):
        ...
        if stability:
            ...
            LOGGING.debug(f"The scenario is stable after '{attempt}' attempts.")
            return False

        # If stability is not in effect it should end much earlier.
        LOGGING.debug(f"Auto retry scenario failed after '{attempt}' attempts.")
        return True
