...
def load_settings(custom_path=None,old=None):
    ...
    if 'JGT_SETTINGS_SYSTEM' in os.environ:
        ...
        env_settings=json.loads(os.environ['JGT_SETTINGS_SYSTEM'])
        update_settings(_settings, env_settings)
    ...
    if 'JGT_SETTINGS' in os.environ:
        ...
        env_settings=json.loads(os.environ['JGT_SETTINGS'])
        _settings.update(env_settings)
    ...
    if 'JGT_SETTINGS_PROCESS' in os.environ:
        ...
        env_settings=json.loads(os.environ['JGT_SETTINGS_PROCESS'])
        update_settings(_settings,env_settings )
...
def add_patterns_arguments(parser:argparse.ArgumentParser=None,load_from_settings=True)->argparse.ArgumentParser:
  ...
  patterns_list_value=load_arg_default_from_settings("patterns",None) if load_from_settings else None
  pn_group.add_argument("-pls", "--list-patterns", help="List Patterns", action="store_true",default=patterns_list_value)
