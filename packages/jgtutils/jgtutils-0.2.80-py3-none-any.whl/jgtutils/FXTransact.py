...
class FXTransactDataHelper:
    ...
    @staticmethod
    def load_fxtrade_from_fxtransact(fxtransact:FXTransactWrapper,str_trade_id:int):
        for trade in fxtransact.trades.trades:
            if trade.trade_id == str_trade_id:
                return trade
        return None
    
    @staticmethod
    def load_fxorder_from_fxtransact(fxtransact:FXTransactWrapper,str_order_id:int):
        for order in fxtransact.orders.orders:
            if order.order_id == str_order_id:
                return order
        return None
