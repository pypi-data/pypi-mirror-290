...
class PulseSeqFile:
    ...
    def _format_gradients(grad_table) -> str:
        return_string = ("# Format of arbitrary gradients:\n#   time_shape_id of 0 means default" 
                        ...
                        f"# {'id':<3} {'amplitude':<13} {'amp_shape_id':<12} {'time_shape_id':<13} {'delay':<7}\n" 
    ...
    def _format_adc(adc_table) -> str:
        ...
        for id_, adcdef in adc_table.items():
            ...
            dwell = np.round(adcdef['dwell'].m_as("ns"), decimals=0)
    ...
    def to_cmrseq(self, system_specs: SystemSpec, block_indices: Iterable[int] = None) -> List[Sequence]:
            ...
            if block_def[dir_idx] != 0:
                ...
                if g_def is not None:
                    ...
                    gradient_object = bausteine.TrapezoidalGradient(system_specs,
                        ...
                        name=f"trapezoidal_id{block_def[dir_idx]}", snap_to_raster=False)
                else:
                    ...
                    gradient_object = bausteine.ArbitraryGradient(system_specs=system_specs,
                        ...
                        name=f"shape_gradient_id{block_def[dir_idx]}", snap_to_raster=False)
    ...
    def from_sequence(self, sequence: Sequence):
        ...
        current_block_entry = {"RF": None, "GX": None, "GY": None, "GZ": None,
                               "ADC": None, "EXT": None}
        progress_bar = tqdm(range(sum([len(q) for q in all_queues]) - 5),
                            total=sum([len(q) for q in all_queues]) - 5,
                            desc="Converting to Pulseq blocks")
        current_start = 0
        current_end = 0
        while any([len(_) > 1 for _ in all_queues.values()]):
            progress_bar.set_postfix_str(str([rf_que[0].tmin.m_as("ms"), gx_que[0][1][0], gy_que[0][1][0],
                           gz_que[0][1][0], adc_que[0].tmin.m_as("ms")]))
            min_idx = min(range(len(key_list)),
                          key=lambda x:
                          [rf_que[0].tmin.m_as("ms"), gx_que[0][1][0], gy_que[0][1][0],
                           gz_que[0][1][0], adc_que[0].tmin.m_as("ms")][x])
            blocktype = key_list[min_idx]
            block_def = all_queues[blocktype].popleft()

            if blocktype in ["RF", "ADC"]:
                new_block_min = block_def.tmin.m_as("ms")
                new_block_max = block_def.tmax.m_as("ms")
            else:
                new_block_min = block_def[1][0]
                new_block_max = block_def[1][-1]

            if current_block_entry[blocktype] is None:
                current_block_entry[blocktype] = block_def
                current_end = max(current_end, new_block_max)
            # if new block min is smaller than the max time of any gradient channel,
            # split the gradient of the previous block and combine the waveform in the new sequence block
            elif new_block_min < current_end:
                # Assumptions for this block collision:
                # 1. RF and ADCs have absolute priority for definining block borders, they are
                # always fully contained in a block. This is ensured by sequence validation.
                ...
                all_queues[blocktype].appendleft(block_def)
                for (previous_block, grad_dir) in [(current_block_entry[k], k) for k in ("GX", "GY", "GZ")]:
                    if previous_block is not None and previous_block[1][-1] > new_block_min:
                        # If point is already contained in the definition, use it as index
                        if np.any(is_close := np.isclose(previous_block[1], new_block_min)):
                            insertion_index = np.squeeze(np.argwhere(is_close))
                            insertion_val = previous_block[2][insertion_index]
                            tmp_wf = previous_block[2]
                            tmp_t = previous_block[1]
                        # Else interpolate the waveform, insert the point and split at this location
                        else:
                            insertion_index = np.searchsorted(previous_block[1], new_block_min)
                            insertion_val = np.interp(new_block_min, previous_block[1], previous_block[2])
                            tmp_wf = np.insert(previous_block[2], insertion_index, insertion_val)
                            tmp_t = np.insert(previous_block[1], insertion_index, new_block_min)
                        current_block_entry[grad_dir] = ("arbitrary", tmp_t[:insertion_index + 1],
                                                          tmp_wf[:insertion_index + 1])
                        all_queues[grad_dir].appendleft(("arbitrary", tmp_t[insertion_index:],
                                                          tmp_wf[insertion_index:]))
                current_end = new_block_min
            else:
                progress_bar.update(1)
                all_queues[blocktype].appendleft(block_def)
                self._register_block(current_block_entry, current_start,
                                     current_end - current_start, sequence._system_specs)
                current_block_entry = {"RF": None, "GX": None, "GY": None, "GZ": None,
                                       "ADC": None, "EXT": None}
                current_start = current_end

        # Finalize the last block definition
        if not all(_ is None for _ in current_block_entry.items()):
            self._register_block(current_block_entry, current_start,
                                 current_end - current_start, sequence._system_specs)
    ...
    def _register_block(self, curr_block: dict, previous_break_point: float,
        ...
        """
        rf_id, gx_id, gy_id, gz_id, adc_id, ext_id = 0, 0, 0, 0, 0, 0
        ...
        for idx, block in sorted(grad_blocks, key=lambda x: x[1][1][0]):
            gradient_ids[idx] = self._register_gradient_event(block, previous_break_point,
    ...
    def _register_rf_event(self, block: cmrseq.bausteine.RFPulse,
        ...
        rf_def = (peak_amp.m_as("uT") / gamma.m_as("Hz/uT"),
                  ...
                  block.frequency_offset.m_as("Hz"), block.phase_offset.m_as("rad"))
        rf_id = self.check_add_def(rf_def, self.rf_hash_table)
        self.rf_table[rf_id] = dict(phase_offset=block.phase_offset,
    ...
    def _register_adc_event(self, block: cmrseq.bausteine.ADC, previous_break_point: float) -> int:
        ...
        dwell = np.around((timings[1] - timings[0]).m_as("ns"), decimals=8)
        num_samples = timings.shape[0]
        delay = np.round(block.tmin.m_as("ms") - previous_break_point, decimals=8) * 1000
    ...
    def _register_gradient_event(self, block: tuple[str, tuple[np.ndarray, np.ndarray]],
        ...
        if block[0] == "trapezoid":
            ...
            self.traps_table[gradient_id] = dict(amplitude=Quantity(amplitude, "mT/m") * gamma.to("Hz/mT"),
            ...
            if len(unique_dt) == 1 and np.isclose(unique_dt[0], raster_time, atol=1e-6):
                ...
                # in the 'delay' field
                time_shape = np.round((np.array(block[1]) - block[1][0]) / raster_time).astype(int)
                time_id = self.check_add_shape(time_shape)

            gwf_id = self.check_add_shape(np.array(normed_wf))
            ...
            self.grads_table[gradient_id] = dict(amplitude=Quantity(amplitude, "mT/m") * gamma.to("Hz/mT"),
