...
class ManageDataApi:
    ...
    def delete_data(
        self,
        accession: Annotated[Optional[List[StrictStr]], Field(description="Accessions files to be deleted.")] = None,
        ...
        The objects will be marked for deletion. In case the selected object has more than one version, the endpoint deletes all of them. The following object types can be deleted by the endpoint:  * `STUDY`  * `SAMPLE_GROUP`   * `LIBRARY_GROUP`   * `PREPARATION_GROUP`   * `TABULAR_DATA`  * `GENE_VARIANT`  * `FLOW_CYTOMETRY`   Moreover, if the selected object is linked to any other data, the endpoint will also delete the linked data associated with it according to the specified rules:  * If you delete a `STUDY`, the entire study with all the listed group types, will be removed.  * Deleting a `SAMPLE_GROUP` will result in the removal of all samples associated with this sample group, along with any linked `TABULAR_DATA`/`GENE_VARIANT`/`FLOW_CYTOMETRY`/`LIBRARY_GROUP`/`PREPARATION_GROUP`. The linked `TABULAR_DATA`/`GENE_VARIANT`/`FLOW_CYTOMETRY` will be deleted, even if it is linked to another metadata group.  * If you delete `TABULAR_DATA`/`GENE_VARIANT`/`FLOW_CYTOMETRY`, all associated data, such as links and runs will be removed from ODM.

        :param accession: Accessions files to be deleted.
