...
def extract_numeric_fields_from_raw(ar_front_data, third_part_text, name_extracted):
    ...
    try:
        ...
        if len(occupation.split(' ')) > 2:
            raise Exception("Forcing to go inside except block")
...
def qatar_front_id_extraction(client, image_data, front_id_text, front_id_text_description):
    ...
    numeric_fields = extract_numeric_fields_from_raw(front_id_text_description, third_part_text, front_data.get('name', ''))
