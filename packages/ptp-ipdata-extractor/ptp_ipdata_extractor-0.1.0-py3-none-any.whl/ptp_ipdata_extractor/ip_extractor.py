...
def get_available_ips(command):
    """
    Retrieves a list of all available IPs from the database.

    This function queries a database view to get all available IPs and returns them as a list.
    ...
    """
    try:
        all_available_ips = query_ibi(service='sql', command=command)
        ip_list = all_available_ips['IP'].tolist()
        return ip_list
    except Exception as error:
        print(f"Couldn't find all available IPs, exceptions is- {error}")


def get_release_per_ip(ip):
    """
    Retrieves a list of distinct releases available for a specific IP from the database.

    This function queries a database view specific to the provided IP and retrieves all distinct
    releases associated with that IP.

    Args:
        ip (str): The IP identifier to query releases for. This is used to construct the table/view
                  name in the SQL query.

    Returns:
        dict: A dictionary where the key is the IP and the value is a list of releases available
              for that IP. Returns an empty dictionary if no releases are found.

    Raises:
        Exception: If there is an issue with querying the database or if no releases are found
                   for the given IP.
    """
    if ip:
        try:
            ip_release_dict = {}
            command = f"select distinct release from V_PNP_IP_{ip}"
            release = query_ibi(service='sql', command=command)
            if release.empty is False:
                ip_release_dict[ip] = release['release'].tolist()
                return dict
        except Exception as error:
            print(f"Couldn't provide available releases for this ip- {ip}, exception is: {error}")
    else:
        print("No valid IP was provided")
