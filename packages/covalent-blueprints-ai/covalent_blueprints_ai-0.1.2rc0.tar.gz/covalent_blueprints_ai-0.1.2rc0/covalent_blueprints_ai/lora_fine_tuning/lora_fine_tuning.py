...
"""Blueprint for fine-tuning a model using the PEFT-LoRA method and deploying
it as a text generation service."""
    ...
    *,
    ...
    Args:
        ...
        data: The identifier passed to `load_dataset`, or the path to
            a local dataset, or a `Dataset` object. Defaults to "imdb".
            Please note that a data argument that refers to a local
            dataset cannot be overridden at runtime (i.e. when calling
            `.run()` on the blueprint returned by this function).
        dataset_map_func: An arbitrary function that accepts a dataset
            as its only argument and returns a dataset. Defaults to None.
        split: Split to use for the fine-tuning data. Defaults to "all".
    ...
    Args for BitAndBytesConfig:
        use_quantization: Whether to use quantization. Defaults to False.
        load_in_4bit: Whether to load the model in 4-bit. Defaults to True.
        bnb_4bit_quant_type: The quantization type for 4-bit. Defaults to
            "nf4".
        bnb_4bit_compute_dtype: The compute dtype for 4-bit. Defaults to
            "float16".
    ...
    Args for TrainingArguments:
        ...
        learning_rate: The learning rate for the fine-tuning. Defaults
            to 2e-3.
        num_train_epochs: The number of training epochs. Defaults to 5.
        save_total_limit: The total number of checkpoints to save.
            Defaults to 1.
        save_strategy: The strategy for saving checkpoints. Defaults to
            "epoch".
        per_device_train_batch_size: The training batch size per device.
            Defaults to 2.
        ...
        max_steps: The maximum number of training steps. Defaults to -1.
        warmup_ratio: The warmup ratio for the learning rate scheduler.
            Defaults to 0.03.
        group_by_length: Whether to group the data by length. Defaults
            to True.
        lr_scheduler_type: The type of learning rate scheduler. Defaults
            to "cosine".
    ...
    Args for LoraConfig:
        ...
        target_modules: The target modules for LoRA. Passing None relies
            on the model being recognized. Passing the string "linear"
            targets all linear modules. An explicit list of target
            modules is also accepted. Defaults to "linear".
    ...
    Args for SFTTrainer:
        ...
        dataset_batch_size: The batch size for the dataset. Defaults to
            10.
    ...
    The service endpoints accept the following keyword-only parameters:
    ...
    bp = get_blueprint(
        f"{PREFIX}/lora_fine_tuning",
        _cls=ServiceWorkflowBlueprint
    )
    bp.example = get_usage_example(lora_fine_tuning)
    ...
    if (
        ...
        if split == "all":
            warnings.warn(
                "split='all' is not supported for local datasets. Will use split='train'."
            )
            split = "train"
