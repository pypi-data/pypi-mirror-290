...
class OperationTarget(BaseModel):
    ...
    target_key: Annotated[str, Field(min_length=1, strict=True, max_length=1024)] = Field(description="Target key.")
    target_type: ObjectType
    target_detail_key: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=1024)]] = Field(default=None, description="The key of the object part, if it exists for this operation - [more](#section/Description/Project). ")
    crop_type: CropType
    humidity: Annotated[int, Field(le=999, strict=True, ge=1)] = Field(description="Humidity crop, in ppm (‰).")
    mass: Union[Annotated[float, Field(le=10000000.1, strict=True, ge=0)], Annotated[int, Field(le=10000000, strict=True, ge=0)]] = Field(description="Weight, in tn.")
    ...
    __properties: ClassVar[List[str]] = ["date", "target_key", "target_type", "target_detail_key", "crop_type", "humidity", "mass", "attributes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.target_detail_key is None and "target_detail_key" in self.model_fields_set:
