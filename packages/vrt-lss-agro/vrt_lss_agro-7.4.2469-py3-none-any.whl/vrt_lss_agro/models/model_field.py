...
class ModelField(BaseModel):
    ...
    crop_key: Annotated[str, Field(min_length=1, strict=True, max_length=1024)] = Field(description="Crop key.")
    amount: Annotated[int, Field(le=10000000, strict=True, ge=0)] = Field(description="The total amount of grain (in tons) to be harvested from the field.")
    ...
    humidity_forecast: Optional[Annotated[List[HumidityForecastElement], Field(min_length=0, max_length=3653)]] = Field(default=None, description="Forecast on grain humidity in the field. If the humidity for a specific day is not specified, then the base crop humidity is used for this. ")
    ...
    __properties: ClassVar[List[str]] = ["key", "crop_key", "amount", "capacity_forecast", "humidity_forecast", "attributes"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "humidity_forecast": [HumidityForecastElement.from_dict(_item) for _item in obj["humidity_forecast"]] if obj.get("humidity_forecast") is not None else None,
