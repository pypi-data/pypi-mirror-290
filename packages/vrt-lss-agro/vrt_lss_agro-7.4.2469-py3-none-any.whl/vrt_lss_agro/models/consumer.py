...
class Consumer(BaseModel):
    ...
    consumption_forecast: Annotated[List[CapacityForecastElement], Field(min_length=0, max_length=3653)] = Field(description="Capacity forecast.")
    stock_forecast: Annotated[List[StockForecastElement], Field(min_length=0, max_length=3653)] = Field(description="Guaranteed balance requirement forecast.")
    __properties: ClassVar[List[str]] = ["key", "crop_key", "consumption_forecast", "stock_forecast"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "consumption_forecast": [CapacityForecastElement.from_dict(_item) for _item in obj["consumption_forecast"]] if obj.get("consumption_forecast") is not None else None,
            "stock_forecast": [StockForecastElement.from_dict(_item) for _item in obj["stock_forecast"]] if obj.get("stock_forecast") is not None else None
        })
        return _obj
