...
class ManufacturingOperation(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["type", "crop_key", "source", "destination", "measurements"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "source": OperationTarget.from_dict(obj["source"]) if obj.get("source") is not None else None,
            "destination": OperationTarget.from_dict(obj["destination"]) if obj.get("destination") is not None else None,
            "measurements": OperationMeasurements.from_dict(obj["measurements"]) if obj.get("measurements") is not None else None
        })
        return _obj
