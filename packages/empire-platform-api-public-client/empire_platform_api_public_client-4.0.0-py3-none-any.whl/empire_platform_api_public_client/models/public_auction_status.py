# coding: utf-8

"""
    Platform API

    _OpenAPI specification for the **Platform API** of **Empire**, the allocation and nomination platform of BritNed_  ---  Additional documentation available in the API's [GitHub repository](https://github.com/britned/empire-platform-api) 

    The version of the OpenAPI document: 4.0.0
    Contact: britned.info@britned.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import json
import pprint
import re  # noqa: F401
from aenum import Enum, no_arg





class PublicAuctionStatus(str, Enum):
    """
    PublicAuctionStatus
    """

    """
    allowed enum values
    """
    PRELIMINARY_SPEC_PUBLISHED = 'PRELIMINARY_SPEC_PUBLISHED'
    FINAL_SPEC_PUBLISHED = 'FINAL_SPEC_PUBLISHED'
    BIDDING_OPEN = 'BIDDING_OPEN'
    BIDDING_CLOSED = 'BIDDING_CLOSED'
    PROVISIONAL_RESULTS_PUBLISHED = 'PROVISIONAL_RESULTS_PUBLISHED'
    FINAL_RESULTS_PUBLISHED = 'FINAL_RESULTS_PUBLISHED'
    CANCELLED = 'CANCELLED'

    @classmethod
    def from_json(cls, json_str: str) -> PublicAuctionStatus:
        """Create an instance of PublicAuctionStatus from a JSON string"""
        return PublicAuctionStatus(json.loads(json_str))


