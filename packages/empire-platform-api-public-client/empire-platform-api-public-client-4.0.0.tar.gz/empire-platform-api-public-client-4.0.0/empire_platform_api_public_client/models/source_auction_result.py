...
class SourceAuctionResult(BaseModel):
    ...
    SourceAuctionResult
    """
    available_allocated_capacity: StrictInt = Field(..., alias="availableAllocatedCapacity", description="Capacity value in kilowatts (kW) - the required system precision allows for handling capacity values as integers")
    clearing_price: Union[confloat(multiple_of=0.01, strict=True), conint(strict=True)] = Field(..., alias="clearingPrice", description="Currency value in Euros (€), accepted with a precision of maximum 2 decimal places")
