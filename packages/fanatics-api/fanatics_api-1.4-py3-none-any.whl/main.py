...
from proxy_url import get_proxy

def fetch_single_data(serialNumber, url, headers, failed_file, time_sleep, password):
    proxy = get_proxy(password, password)
    i = 0
    while proxy:
        payload = {
        ...
        try:
            ...
            i+=1
            print(f"Retry {i} times, sleep 1 second")
            time.sleep(1)
            # Get a new proxy and retry
            if i > 10:
                print(f"Retry {i} times, force to get a new proxy")
                proxy = get_proxy(password, password, force=True)
            else:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i in range(begin, end):
            ...
            if os.path.exists(f"data/{serialNumber}.json"):
                continue
            futures.append(executor.submit(fetch_single_data, serialNumber, url, headers, failed_file, time_sleep, password))
...
def retry_single_data(serialNumber, url, headers, failed_file, time_sleep):
    proxy = get_proxy()
...
def retry_failed(failed_file, time_sleep, max_workers, password):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(retry_single_data, line.split(" ")[0], url, headers, failed_file, time_sleep): line for line in lines}
