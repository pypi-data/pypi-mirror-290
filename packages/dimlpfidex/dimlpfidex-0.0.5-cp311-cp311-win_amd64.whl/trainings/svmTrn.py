...
def get_and_check_parameters(init_args):
    ...
    parser.add_argument("--degree", type=lambda x: int_type(x, min=0), metavar="<int [0, inf[>", help="Degree of the polynomial kernel function", default=3, action=TaggableAction, tag="SVM")
    parser.add_argument("--gamma", type=lambda x: enum_type(x, "scale", "auto", _=dict(func=float_type, min=0)), metavar="<{scale, auto, float [0,inf[}>", help="Gamma value (kernel coefficient for ‘rbf’, ‘poly’ and ‘sigmoid’)", default="scale", action=TaggableAction, tag="SVM")
    parser.add_argument("--coef0", type=float_type, metavar="<float>", help="Independant term in the kernel function", default=0.0, action=TaggableAction, tag="SVM")
