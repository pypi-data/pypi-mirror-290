...
impl UseDirective {
    ...
    pub(crate) fn for_path(path: &str, target: &str) -> Self {
        UseDirective {
            paths: HashMap::from([(target.to_string(), path.to_string())]),
            reexport: None,
            content: vec![],
    ...
    pub(crate) fn for_use_paths(paths: HashMap<String, String>) -> Self {
        UseDirective {
            paths,
            reexport: None,
            content: vec![],
        ...
        'path_loop: for path in self.paths.values_mut() {
            for module in modules {
                if path.starts_with(&format!("{}::", module.ident)) {
                    path.insert_str(
                        0,
                        &format!("{}::", &module.name[0..module.name.rfind("::").unwrap()]),
                    );
                    continue 'path_loop;
                }
            }
