...
def first(*thelist, **options):
    ...
    @Usage:
        thelist = [None, "OK"]
        the_first_non_none_value = first(*thelist, default="EMPTY")
        assert  the_first_non_none_value == "OK"

        thelist = [None, None]
        the_first_non_none_value = first(*thelist, default="EMPTY")
        assert  the_first_non_none_value == "EMPTY"
        ...
        def empty_check(x):
            if x:
                ...
                return None
        thelist = [None, 0, {}, [], {}, False, ""]
        the_first_non_none_value = first(*thelist, check=empty_check, default="EMPTY")
        assert  the_first_non_none_value == "EMPTY"
    """
...
def int_list_to_bytes(*args, **kwargs):
    ...
    案例：
        listutils.int_list_to_bytes([1,2,3,4]) == b'\x01\x02\x03\x04'

    备注：
        listutils.int_list_to_bytes 实际调用的是 numericutils.ints2bytes
    """
