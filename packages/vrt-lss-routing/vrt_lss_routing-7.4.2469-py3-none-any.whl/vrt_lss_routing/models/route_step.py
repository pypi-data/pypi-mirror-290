...
class RouteStep(BaseModel):
    ...
    polyline: Annotated[List[Trackpoint], Field(min_length=0, max_length=1000000)] = Field(description="An array of geographic points with time reference between which a path is built.")
    __properties: ClassVar[List[str]] = ["transport_type", "polyline"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in polyline (list)
        _items = []
        if self.polyline:
            for _item in self.polyline:
                if _item:
                    _items.append(_item.to_dict())
            _dict['polyline'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "polyline": [Trackpoint.from_dict(_item) for _item in obj["polyline"]] if obj.get("polyline") is not None else None
        })
        return _obj
