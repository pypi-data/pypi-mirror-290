...
from mdata.core.extensions import metadata, segments
...
from ...core.raw import RawMetadataFeatureSpec, RawBaseFeatureSpec
from ...core.v2 import raw_v2
from ...core.v2.shared_defs import SegmentDefinitionType
        ...
        with use_string_io(target, expected_file_ext=as_ext(HeaderFileFormats.CSV), mode='w',
            ...
            if 'segment_specs' in header:
                ...
                for concept in header['segment_specs']:
                    rows += [[SegmentDefinitionType.Segments, concept if type(concept) is str else next(iter(concept))]]
            if 'segment_data_specs' in header:
                header: raw_v2.RawHeaderSpecV2
                for typ, spec in header['segment_data_specs'].items():
                    handle_datapoint_spec(SegmentDefinitionType.SegmentData, typ, spec)
