...
from typing import List, Optional, Union, TypedDict, Generator
...
from skapp.utils import Fqdn, merge, Prefixed, LazyString
...
class BasePodController(NamespacedResource):
    ...
    def _env(self):
        def _var(args) -> V1EnvVar:
            ...
            if isinstance(v, str):
                ...
                if "secret" in v:
                    ...
                    name = _name if isinstance(secret, dict) and secret.get('external', False) else Prefixed(_name)
        ...
        env = (
            ...
            V1EnvVar(
                name="RELEASE",
                value=LazyString("{{ helm('.Release.Name') }}")
            )
        ]
    ...
    def _container_extras(self):
        ...
        if self.probes:
            for kind, check in self.probes.items():
                ...
                extras[f"{kind}_probe"] = client.V1Probe(
                        **check
                    )
...
class Ingress(NamespacedResource):
                ...
                "servers": [
                    {
                        ...
                        "tls": {
                            "mode": "SIMPLE",
                            "credential_name": Prefixed(f"{self.metadata.name}-cert"),
                        },
                    }
                    if ssl
                    else {
    ...
    def generate_nginx(self, namespace):
        ...
        if self.websockets:
            ...
            metadata.annotations["nginx.ingress.kubernetes.io/proxy-read-timeout"] = '3600'
            metadata.annotations["nginx.ingress.kubernetes.io/proxy-send-timeout"] = '3600'
        ...
        yield client.V1Ingress(
            ...
            kind="Ingress",
            metadata=metadata,
            spec=client.V1IngressSpec(rules=rules, tls=tls),
        ).to_dict()
            ...
            merge(
                defaults,
                {
                    "annotations": {
                        ...
                        "nginx.ingress.kubernetes.io/ssl-redirect": "true" if self.https_redirect else "false"
                    }
                 },
            )
        )
