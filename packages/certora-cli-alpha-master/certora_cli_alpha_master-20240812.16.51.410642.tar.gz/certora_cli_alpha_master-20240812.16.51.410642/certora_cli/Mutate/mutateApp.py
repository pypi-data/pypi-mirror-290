...
from Mutate import mutateAttribute as Attrs
from EVMVerifier.certoraContextAttributes import ContextAttributes
from EVMVerifier.certoraContextClass import CertoraContext


DEFAULT_NUM_MUTANTS = 5
ENV_FREE_CHECK = 'envfreeFuncsStaticCheck'
PATCH_SUFFIXES = ['.sol', '.patch']
...
class WebUtils:
    ...
    def config_server(args: SimpleNamespace) -> str:
        ...
        prover_context = getattr(args, 'prover_context', {})
        if prover_context:
        ...
        if args.orig_run is not None:
            if MConstants.STAGING_DOTCOM in args.orig_run:
                return MConstants.STAGING
            elif MConstants.PROVER_DOTCOM in args.orig_run:
                return default
            elif MConstants.DEV_DOTCOM in args.orig_run:
...
class MutateApp:
    ...
    def submit(self) -> None:
        ...
        if not self.orig_run:
            ...
            success, certora_run_result = self.run_certora_prover(self.conf, mutation_test_id, msg=MConstants.ORIGINAL,
                                                                  is_mutant=True)
            ...
            if certora_run_result:
                ...
                if local:
                    assert certora_run_result.link, "submit: no certora_run_result.link"
                    result_link = certora_run_result.link
                    src = Path(certora_run_result.link) / MConstants.REPORTS / MConstants.SPLIT_STATS_DATA
                    if src.is_file():
                        try:
                            shutil.copy(src, Path.cwd())
                            self.with_split_stats_data = True
                        except Exception as e:
                            logging.debug(f"did not manage to copy {MConstants.SPLIT_STATS_DATA} to current directory: "
                                          f"{e}")
                    else:
                        logging.debug(f"{MConstants.SPLIT_STATS_DATA} is not in {src.parent}")
                else:
                    assert certora_run_result.rule_report_link, "submit: no certora_run_result.rule_report_link"
            ...
            if not success or not certora_run_result or not result_link or not self.sources_dir:
                try:
            ...
            if local:
                try:
                    if not Path(result_link).is_dir():
                        raise Util.CertoraUserInputError(f"Unexpected path for a local job {result_link}")
                except TypeError:
            ...
            assert self.orig_run_dir, "submit: orig_run_dir"
        ...
        assert self.collect_file, "submit: no collect_file"
    ...
    def run_mutant(self, mutant: Mutant, trg_dir: Path, orig_conf: Path,
    ...
    def generate_gambit_conf_file(self) -> None:
        ...
        assert self.gambit, "submit: no gambit"
    ...
    @staticmethod
    def add_file_to_mutants(manual_mutants: List[Mutant], mutation_file: Path, file_to_mutate: Path) -> None:
        if mutation_file.suffix in PATCH_SUFFIXES:
            manual_mutant_obj = generate_manual_mutant_object(mutation_file, file_to_mutate)
            manual_mutants.append(manual_mutant_obj)
        else:
            raise Util.CertoraUserInputError(f"Illegal mutant file, {mutation_file.name},"
                                             " extension must be .sol or .patch")

    @staticmethod
    def add_dir_to_mutants(manual_mutants: List[Mutant], mutation_dir: Path, file_to_mutate: Path) -> None:
        mutation_dir_path = Path(mutation_dir).resolve()
        manual_mutants_files = [p for p in mutation_dir_path.glob('*') if p.is_file() and p.suffix in PATCH_SUFFIXES]
        ...
        for file in manual_mutants_files:
            MutateApp.add_file_to_mutants(manual_mutants, file, file_to_mutate)
    ...
    def parse_manual_mutations(self) -> List[Mutant]:
        ...
        ret_mutants: List[Mutant] = []
        assert self.manual_mutants, "parse_manual_mutations: self.manual_mutants"
        for mutant in self.manual_mutants:
            ...
            mutants_location = Path(mutant[MConstants.MUTANTS_LOCATION]).resolve()
            if mutants_location.is_dir():
                MutateApp.add_dir_to_mutants(ret_mutants, mutants_location, file_to_mutate)
            elif mutants_location.is_file():
                MutateApp.add_file_to_mutants(ret_mutants, mutants_location, file_to_mutate)
            else:
        ...
        if original_results:
            ...
            if not self.sync:
                ...
                if not self.dump_csv:
                    Console().print(f"\n\n{Util.print_rich_link(test_results.original)}\n")
                    raise MutUtil.EmptyMutationReport("Cannot create mutation report if original report failed")
        ...
        if orig_collect_success and mutant_collect_success:
            ...
            if self.ui_out:
                if mutants_results:
                    ...
                    raise Util.ImplementationError("collect: Unexpected empty mutants_results")
            else:
    ...
    def get_results(self, link: Optional[str], fetcher: ReportFetcher) -> Optional[List[RuleResult]]:
        ...
        if isinstance(fetcher, FileFetcher):
            ...
            if MConstants.RULES not in output_json:
                mutation_logger.debug(f"Bad format for {MConstants.OUTPUTJSON}")
                return list()
    ...
    def poll_collect(self) -> None:
        ...
        while duration < poll_timeout_seconds:
            ...
            if not ready:
                ...
                assert self.ui_out, "poll_collect: self.ui_out"
                final_report = self.read_ui_file()
                web_utils = WebUtils(SimpleNamespace(**vars(self)))
    ...
    def check_prover_context(self) -> None:
        prover_attrs = [attr.get_conf_key() for attr in ContextAttributes.attribute_list]
    ...
    def get_args(self, args_list: List[str]) -> None:
        attr_dict = Attrs.get_args(args_list)
    ...
    def read_conf_file(self) -> None:
        ...
        mutation_attrs = [attr.get_conf_key() for attr in Attrs.MutateAttributes.attribute_list]
...
class FileFetcher(ReportFetcher):
    ...
    def get_output_json(self, link: str) -> Optional[Dict[str, Any]]:
        output_path = Path(link) / MConstants.REPORTS / MConstants.OUTPUTJSON
        if not output_path.is_file():
            mutation_logger.info(f"Got no {MConstants.OUTPUTJSON} file")
            return None
    ...
    def get_treeview_json(self, link: str) -> Optional[Dict[str, Any]]:
        ...
        treeview_path = Path(link) / MConstants.REPORTS / "treeView"
        ...
        return {MConstants.VERIFICATION_PROGRESS: json.dumps(treeview_str)}
