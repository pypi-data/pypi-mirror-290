...
from typing import Sequence, TextIO, cast, Any, List
...
from relationalai.clients.util import get_execution_environment
...
class Spinner:
    ...
    def __init__(self, message="", finished_message="", failed_message=None, delay=None):
        ...
        if delay and (isinstance(delay, int) or isinstance(delay, float)):
            self.delay = delay
        environment = get_execution_environment()
        self.is_snowflake_notebook = environment == 'snowflake_notebook'
        self.is_jupyter = environment == 'jupyter'
        if delay is None:
            self.delay = 0.5 if self.is_snowflake_notebook else 0.1
        self.last_message = ""
        self.display = None

    def get_message(self):
        return f"{next(self.spinner_generator)} {self.message}"
    
    def update(self, message:str|None=None, color:str|None=None, file:TextIO|None=None):
        if message is None: 
        ...
        if self.is_jupyter:
            ...
            self.reset_cursor()
            diff = len(self.last_message.encode('utf-8')) - len(rich_string.encode('utf-8'))
            rich.print(rich_message + (" " * diff), file=file, end="")
