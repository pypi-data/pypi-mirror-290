...
from relationalai.errors import RAITypeError
from .. import dsl
import gravis as gv
...
def between(param:str,value, min, max,  type_=Number):
    if (not isinstance(value, type_)) or (value < min or value > max):
        exception = RAITypeError(param, f"must be a {type_.__name__.lower()} between {min} and {max}, got {value!r}")
        raise exception

def positive(param:str, value, type_=Number):
    if (not isinstance(value, type_)) or (value <= 0):
        exception = RAITypeError(param, f"must be a positive {type_.__name__.lower()}, got {value!r}")
        raise exception

def nonnegative(param:str, value, type_=Number):
    if (not isinstance(value, type_)) or (value < 0):
        exception = RAITypeError(param, f"must be a non-negative {type_.__name__.lower()}, got {value!r}")
        raise exception
...
class Compute:
    ...
    def local_clustering_coefficient(self, node):
        if not self._graph.undirected:
            exception = RAITypeError("graph", "must be undirected")
            raise exception
