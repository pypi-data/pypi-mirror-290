...
__all__=["db","ckd","dbdata","storidata","stdata","检查匹配"]
...
class dot(object):
    def __init__(self,arg):
        ...
        f=open(arg["file"],"wt",encoding="utf8")
        f.write(jinja2.Template(模板).render(dbdata))
        f.close()
        if "cmd" in arg:
            print(arg["cmd"])
            os.system(arg["cmd"])
...
class render(object):
    ...
    def rendertable(self,t):
        ...
        nt=nt+"\n"
        f=open(stdata["file"],encoding=stdata.get("coding","") or "utf8")
        ft=f.readlines()
        newline=f.newlines
        f.close()
        k=False
        ...
        if ot=="":
            ls=len(ft)
        f=open(stdata["file"],"wt",encoding=stdata.get("coding","utf8"))
        f.newline=newline
        for i in range(len(ft)):
            if i==ls:
                f.write(nt)
...
def 检查匹配(类型,名称):
    ...
    排除标志=True
    加减="+"
    for i in stdata.get(类型.lower(),"+ .*").split():
        if i in ["+","-"]:
            加减=i
        else:
