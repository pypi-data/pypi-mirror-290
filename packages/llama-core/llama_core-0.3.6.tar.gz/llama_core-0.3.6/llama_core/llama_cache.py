...
class BaseLlamaCache(ABC):
    ...
    @abstractmethod
    def __getitem__(self, key: Sequence[int]) -> "llama_core.llama.LlamaState":
        raise NotImplementedError
...
class LlamaRAMCache(BaseLlamaCache):
    ...
    def __init__(self, capacity_bytes: int = (2 << 30)):
        ...
        self.capacity_bytes = capacity_bytes
        self.cache_state: OrderedDict[Tuple[int, ...], "llama_core.llama.LlamaState"] = (
            OrderedDict()
        )
    ...
    def __setitem__(self, key: Sequence[int], value: "llama_core.llama.LlamaState"):
        key = tuple(key)
...
class LlamaDiskCache(BaseLlamaCache):
    ...
    def _find_longest_prefix_key(
        ...
        for k in self.cache.iterkeys():  # type: ignore
            prefix_len = llama_core.llama.Llama.longest_token_prefix(k, key)
            if prefix_len > min_len:
                min_len = prefix_len
    ...
    def __getitem__(self, key: Sequence[int]) -> "llama_core.llama.LlamaState":
        ...
        if _key is None:
            raise KeyError("Key not found")
        value: "llama_core.llama.LlamaState" = self.cache.pop(_key)  # type: ignore
