...
class Session(Generic[InteractiveItemTypeT, PageTypeT, BrowserTypeT]):
    ...
    async def query_data(
        ...
        """
        response, _, _ = await self._query(
            query,
            timeout,
            lazy_load_pages_count,
    ...
    async def query(
        ...
        response, driver, node = await self._query(
            query,
            timeout,
            lazy_load_pages_count,
            wait_for_network_idle,
    ...
    async def _query(
        ...
        warn(
            ...
            See `agentql.ext.playwright.async_api.Page.{method_name}`
            ...
            Example:
                ...
                reponse: dict = await page.{method_name}(QUERY)
            """,
            DeprecationWarning,
            stacklevel=3,
        )
        ...
        if DebugManager.debug_mode_enabled:
            await self._web_driver.take_screenshot(
