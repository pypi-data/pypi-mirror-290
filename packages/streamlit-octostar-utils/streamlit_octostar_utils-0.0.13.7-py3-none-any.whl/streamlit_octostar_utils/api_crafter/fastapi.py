...
class DefaultErrorRoute(Route):
    ...
    def format_error(request, body, exc, debug=False):
    ...
    async def handle_error(request: Request, exc: Exception, debug: bool):
        ...
        return DefaultErrorRoute.format_error(request, body, exc, debug=debug)
    
    def add_default_exceptions_handler(fs_app, debug=False):
        fs_app.add_exception_handler(RequestValidationError, lambda r, e: DefaultErrorRoute.handle_error(r, e, debug))
        fs_app.add_exception_handler(StarletteHTTPException, lambda r, e: DefaultErrorRoute.handle_error(r, e, debug))
        fs_app.add_exception_handler(Exception, lambda r, e: DefaultErrorRoute.handle_error(r, e, debug))
                ...
                if any(self.queues[priority][route_name] for route_name in self.queues[priority].keys()):
                    ...
                    for _ in range(len(route_names)):
                        ...
                        if len(self.queues[priority][route_names[idx]]) > 0:
                            ...
                            logging.debug(f"Selected next task from route: {route_names[idx]} with priority: {priority}")
                            return next_task
            return None
    ...
    async def worker(self, i):
        ...
        while not self.kill_event.is_set():
            ...
            try:
                ...
                task.future.set_exception(asyncio.TimeoutError(f"Task {task} exceeded timeout of {task.timeout} seconds"))
