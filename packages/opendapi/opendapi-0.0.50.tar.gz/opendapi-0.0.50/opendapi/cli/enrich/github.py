...
class GithubEnricher(Enricher):
    ...
    def use_portal_based_devx(self) -> bool:
        ...
        repo_full_name = (self.trigger_event.repo_full_name or "").lower()
        return (
            "woven" in repo_full_name
            or "dbt" in repo_full_name
            or "server" in repo_full_name
        )
    ...
    def create_or_update_summary_comment_metadata_unsynced(
        ...
        # tiered rollout, dbt repos first
        if self.use_portal_based_devx:
    ...
    def portal_metadata_update_link(self) -> str:
        ...
        portal_url = (
            str(self.validate_response.server_meta.portal_url)
            or "https://app.wovencollab.com"
        )
        return urllib.parse.urljoin(
            portal_url, f"/github/pull-requests/{repo_path}/{number}"
        )
        ...
        try:
            ...
            self.print_markdown_and_text(
                (
                    ...
                    "metadata updated experience for this PR, please visit "
                    f"{self.portal_metadata_update_link}"
                ),
                color="green",
            )
