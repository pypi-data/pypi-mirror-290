...
quant_colors = [c1[0], c1[2], c1[4], c1[6], c1[8], c1[10], c1[12], c1[14], c1[16], c1[18], \
	...
	def get_data(self):
		...
		if self.n_dataset_columns > 0:
			...
			if dataset is None or len(dataset[0]) == 0:
				...
				agg_error = False
				for g in groups:
					agg_data[g] = [None for _ in range(self.n_dataset_columns)]
				agg_by = self.aggreg_var.get()
				if agg_by == "Count":
					for g in groups:
						grp_subset = subsets[g]
					...
					for g in groups:
						...
						for colidx in range(len(grp_subset)):
							vals = no_none(grp_subset[colidx])
							agg_data[g][colidx] = min(vals) if len(vals) > 0 else None
				elif agg_by == "Maximum":
					for g in groups:
						grp_subset = subsets[g]
						for colidx in range(len(grp_subset)):
							vals = no_none(grp_subset[colidx])
							agg_data[g][colidx] = max(vals) if len(vals) > 0 else None
				elif agg_by == "Median":
					for g in groups:
						grp_subset = subsets[g]
						for colidx in range(len(grp_subset)):
							vals = no_none(grp_subset[colidx])
							agg_data[g][colidx] = statistics.median(vals) if len(vals) > 0 else None
				elif agg_by == "Arithmetic mean":
					for g in groups:
						grp_subset = subsets[g]
						for colidx in range(len(grp_subset)):
							vals = no_none(grp_subset[colidx])
							agg_data[g][colidx] = statistics.mean(vals) if len(vals) > 0 else None
					...
					for g in groups:
						...
						for colidx in range(len(grp_subset)):
							vals = no_none(grp_subset[colidx])
							try:
								agg_data[g][colidx] = spstats.gmean(vals) if len(vals) > 0 else None
							except:
								agg_data[g][colidx] = None
								agg_error = True
					if agg_error:
					...
					for g in groups:
						...
						for colidx in range(len(grp_subset)):
							vals = no_none(grp_subset[colidx])
							try:
								agg_data[g][colidx] = spstats.hmean(vals) if len(vals) > 0 else None
							except:
								agg_data[g][colidx] = None
								agg_error = True
					if agg_error:
					...
					for g in groups:
						...
						for colidx in range(len(grp_subset)):
							vals = no_none(grp_subset[colidx])
							agg_data[g][colidx] = sum(vals) if len(vals) > 0 else None
				self.agg_data = list(map(list, zip(*[([k]+agg_data[k]) for k in agg_data.keys()])))
...
class PlotDialog(Dialog):
	...
	def set_xlabel(self, *args):
		dlg = OneEntryDialog(self.dlg, "X-Axis Label", "Enter a title for the un-rotated X-axis label:")
		xlabel = dlg.show()
		if xlabel is not None:
			self.xlabel = xlabel
			if not self.rotated:
				...
				self.plot.axes.set_ylabel(xlabel)
			self.plot.draw()
	def set_ylabel(self, *args):
		dlg = OneEntryDialog(self.dlg, "Y-Axis Label", "Enter a title for the un-rotated Y-axis label:")
		ylabel = dlg.show()
		if ylabel is not None:
			self.ylabel = ylabel
			if not self.rotated:
				...
				self.plot.axes.set_xlabel(ylabel)
			self.plot.draw()
...
class VariableDistDialog(Dialog):
	def __init__(self, parent, column_specs):
		...
		self.type_sel.bind("<<ComboboxSelected>>", self.q_redraw)
	...
	def set_title(self, *args):
		...
		if title is not None:
			self.plot_title = title
			self.varplot.axes.set_title(title)
			self.varplot.axes.title.set_visible(True)
			self.varplot.draw()
	def set_xlabel(self, *args):
		...
		if xlabel is not None:
			self.xlabel = xlabel
			self.varplot.axes.set_xlabel(xlabel)
	...
	def set_ylabel(self, *args):
		...
		if ylabel is not None:
			self.ylabel = ylabel
			self.varplot.axes.set_ylabel(ylabel)
			self.varplot.draw()
	def set_rotated(self, *args):
		self.rotated = not self.rotated
		self.q_redraw()
...
class PctStackedAreaDialog(Dialog):
	def __init__(self, parent, column_specs):
		...
		self.x_sel = ttk.Combobox(pt_frame, state="readonly", textvariable=self.x_var, width=25, height=15, values=self.categ_columns)
	...
	def get_data(self):
		...
		if dataset is not None:
			...
			if len(dataset[0]) > 1:
				...
				groups = sorted_numstrs(list(set(dataset[0])))
				agg_data = {}
				agg_error = False
				for g in groups:
					agg_data[g] = [None for _ in range(1,self.n_dataset_columns)]
				agg_by = self.aggreg_var.get()
				if agg_by == "Arithmetic mean":
					for g in groups:
						grp_subset = subsets[g]
						for colidx in range(len(grp_subset)):
							agg_data[g][colidx] = statistics.mean(grp_subset[colidx])
				elif agg_by == "Geometric mean":
					for g in groups:
						grp_subset = subsets[g]
						try:
							agg_data[g] = list(spstats.gmean(grp_subset, axis=1))
						except:
							agg_error = True
							warning("Cannot calculate a geometric mean with negative values", {"parent": self.dlg})
				elif agg_by == "Harmonic mean":
					for g in groups:
						grp_subset = subsets[g]
						try:
							agg_data[g] = list(spstats.hmean(grp_subset, axis=1))
						except:
							agg_error = True
							warning("Cannot calculate a harmonic mean with negative values", {"parent": self.dlg})
				else:	# Sum
					for g in groups:
						grp_subset = subsets[g]
						for colidx in range(len(grp_subset)):
							agg_data[g][colidx] = sum(grp_subset[colidx])
				if not agg_error:
					# Check for NaN
					agg_error = any([any(np.isnan(x)) for x in agg_data.values()])
					if agg_error:
						warning("Aggregated data has NaN values; plot cannot be produced.", {"parent": self.dlg})
					else:
						# Convert aggregated data back into a table for display and export
						self.plot_data = list(map(list, zip(*[([k]+agg_data[k]) for k in agg_data.keys()])))
						self.plot_data_labels = self.data_labels
						self.plot_data_btn["state"] = tk.NORMAL
						# Reject negative values
						neg_err = False
						for col in self.plot_data[1:]:
							if any([x < 0.0 for x in col]):
								neg_err = True
								break
						if neg_err:
							self.plot_data = None
			...
			if len(dataset[0]) > 1:
				...
				if agg_by == "Arithmetic mean":
					...
					for g in groups:
						grp_subset = subsets[g]
						try:
							agg_data[g] = list(np.min(grp_subset, axis=1))
						except:
							agg_error = True
							warning("Cannot calculate a minimum value", {"parent": self.dlg})
							break
				elif agg_by == "Maximum":
					for g in groups:
						grp_subset = subsets[g]
						try:
							agg_data[g] = list(np.max(grp_subset, axis=1))
						except:
							agg_error = True
							warning("Cannot calculate a maximum value", {"parent": self.dlg})
							break
				else:	# Sum
					for g in groups:
						grp_subset = subsets[g]
		...
		if self.n_dataset_columns > 0:
			...
			if n_groups < 2:
				...
				if not agg_error:
					...
					agg_error = any([any(np.isnan(x)) for x in agg_data.values()])
					if agg_error:
						warning("Aggregated data has NaN values", {"parent": self.dlg})
					else:
						# Convert aggregated data back into a table for display and export
						self.agg_data = list(map(list, zip(*[(agg_data[k]+[k]) for k in agg_data.keys()])))
						self.agg_data_groups = groups
...
class OneEntryDialog(Dialog):
	def __init__(self, parent, title, prompt, msgwraplength=80, obscure=False):
