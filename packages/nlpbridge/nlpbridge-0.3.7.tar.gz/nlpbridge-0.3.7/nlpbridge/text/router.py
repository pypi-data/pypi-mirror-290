...
class Node(Callable[[dict], TypedDict]):
    ...
    def rand_prompt(self):
        # start_sys_template + user_prompt + end_sys_template
        ...
        if self.user_templates:
            if self.user_template_index >= 0 and len(self.user_templates) > self.user_template_index:
                ...
                # chat_limit - len(user_templates), 随机选择一个user_template
...
class RouterManager:
    ...
    async def get_graph(self, chat_id: str, llm, router_id: int, condition_cls: Condition = Condition,
                        node_cls: Node = Node, recursion_limit: int = 30, **kwargs) -> Tuple[CompiledGraph, dict]:
        router = await asyncio.to_thread(RouterManager.get_router, router_id)
        ...
        async def fetch_nodes_and_edges():
            with ThreadPoolExecutor() as executor:
                future_nodes = asyncio.to_thread(NodeManager.get_nodes, int_node_ids)
                future_edges = asyncio.to_thread(EdgeManager.get_edges, int_edge_ids)
                nodes, edges = await asyncio.gather(future_nodes, future_edges)
                return nodes, edges

        nodes, edges = await fetch_nodes_and_edges()

        # Fetch templates
        template_ids = self.get_template_ids(nodes)
        templates = await asyncio.to_thread(TemplateManager.get_templates, template_ids)
    ...
    def get_graph0(self, chat_id: str, llm, router_id: int, condition_cls: Condition = Condition,
                   node_cls: Node = Node, recursion_limit: int = 30, **kwargs) -> Tuple[CompiledGraph, dict]:
        ...
        template_ids = self.get_template_ids(nodes)
        templates = TemplateManager.get_templates(template_ids)
        ...
        graph = self._create_graph(runnable_nodes, edges, condition_cls)
        cfg = {"configurable": {"thread_id": router_id}, "recursion_limit": recursion_limit}
        graph.update_state(config=cfg, values={"nodes": nodes})
        return graph, cfg
