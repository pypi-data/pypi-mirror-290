...
#
# <filename.mimul var=... var=...>
#
def convert_includes(domain, paragraph):
    while match := search(r"<(.*?\.mimul)(.*?)>", paragraph):
        a = match.start(0)
        b = match.end(0)
        with Path(match.group(1)).open("r") as f:
            html = mimul(domain, f.read())
...
def mimul(domain, mimul_content, css=""):
    output = ""
    inul = False
    if "----" in mimul_content:
        beginning, width_spec, middle, end = mimul_content.split("----")
        ...
        NAVIGATION = mimul(domain, rest)
        CONTENT = mimul(domain, middle)
        FOOTER = mimul(domain, end)
        ...
        paragraphs = mimul_content.split("\n\n")
