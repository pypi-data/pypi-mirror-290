...
{Fore.grey_50}InList - {color_2}{Style.bold}15{Style.reset}'''
                        ...
                        ci=''
                        result=session.query(Entry).filter(or_(Entry.Barcode==code,Entry.Code==code,Entry.ALT_Barcode==code)).first()
                        results=session.query(Entry).filter(or_(Entry.Barcode==code,Entry.Code==code,Entry.ALT_Barcode==code)).all()
                        ...
                        if results_ct > 0:
                            select=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Do you wish to select an alternative to the first?",helpText="yes or no, default=no",data="boolean")
                            if select in [False,'d']:
                                ...
                                for num,i in enumerate(results):
                                    ...
                                    print(msg)
                                which=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Which number?",helpText="number in yellow",data="integer")
                                if which in [None,]:
                                    continue
                                elif which in ['d',]:
                                    result=results[0]
                        ...
                        if resultsName_ct > 0:
                            ...
                            if results_ct < 1:
                                warn=''
                            select=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"Do you wish to select an alternative to the first {warn}",helpText="yes or no, default=no",data="boolean")
                            if select in [False,'d']:
                                pass
                            elif select in [True,]:
                        ...
                        ptext=f'''{Fore.grey_70}[{Fore.light_steel_blue}ListMode Entry Info{Fore.grey_70}]
