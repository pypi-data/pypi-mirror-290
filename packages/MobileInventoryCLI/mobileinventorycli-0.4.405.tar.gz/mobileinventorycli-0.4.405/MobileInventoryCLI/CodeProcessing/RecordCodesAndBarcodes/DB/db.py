...
class Entry(BASE,EntryExtras):
    ...
    WD_DSPLY=Column(Integer)
    CHKSTND_SPLY=Column(Integer)
...
{Fore.light_yellow}{self.Name} [{Fore.misty_rose_3}EID={Fore.pale_green_1b}{self.EntryId}{Fore.light_yellow}] ({Fore.light_magenta}{self.Barcode}[{Fore.spring_green_3a}UPC{Style.reset}{Fore.light_magenta}]:{Fore.light_red}{self.Code}[{Fore.orange_red_1}SHELF/TAG/CIC]{Fore.light_red}){Style.reset}'''
        return msg
    ...
    def __repr__(self):
        ...
        {Style.bold+Style.underline+Fore.orange_red_1}Name{Style.reset}='{self.Name}',
        {Fore.light_steel_blue}{Style.underline}Description{Style.reset} = '{Fore.cyan}{self.Description}{Style.reset}',
    ...
    def scan_set(self,code=None,field=None,value=None,loop=False):
        ...
        if code and field and value:
            ...
            while True:
                ...
                h=f'{Prompt.header.format(Fore=Fore,mode=mode,fieldname=fieldname,Style=Style)}'
                fieldsHelp=f''''''
                fieldsList=[i.name for i in Entry.__table__.columns]
                ...
                field=Prompt.__init2__(None,func=self.mkText,ptext=f"{h}{Fore.green_yellow}Field(see help|?)",helpText=fieldsMapped2,data=None)
                if field in [None,]:
                    if loop == False:
                        return
                    else:
...
from MobileInventoryCLI.CodeProcessing.RecordCodesAndBarcodes.DB.RandomStringUtil import RandomString
