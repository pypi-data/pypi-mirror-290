...
class LdapManager(Interface):
    idlType = "auth.LdapManager:3.0.0"
...
class RadiusManager(Interface):
    idlType = "auth.RadiusManager:3.0.0"
...
class TacPlusManager(Interface):
    idlType = "auth.TacPlusManager:1.0.0"
    ...
    class _getTacPlusServers(Interface.Method):
        ...
        @staticmethod
        def decode(rsp, agent):
            _ret_ = [raritan.rpc.tacplus.ServerSettings.decode(x0, agent) for x0 in rsp['_ret_']]
            for x0 in _ret_:
                typecheck.is_struct(x0, raritan.rpc.tacplus.ServerSettings, DecodeException)
            return _ret_
    ...
    class _setTacPlusServers(Interface.Method):
        ...
        def encode(serverList):
            for x0 in serverList:
                typecheck.is_struct(x0, raritan.rpc.tacplus.ServerSettings, AssertionError)
            args = {}
            args['serverList'] = [raritan.rpc.tacplus.ServerSettings.encode(x0) for x0 in serverList]
            return args
    ...
    class _testTacPlusServer(Interface.Method):
        ...
        def encode(username, password, settings):
            typecheck.is_string(username, AssertionError)
            typecheck.is_string(password, AssertionError)
            typecheck.is_struct(settings, raritan.rpc.tacplus.ServerSettings, AssertionError)
            args = {}
            args['username'] = username
            args['password'] = password
            args['settings'] = raritan.rpc.tacplus.ServerSettings.encode(settings)
            return args
