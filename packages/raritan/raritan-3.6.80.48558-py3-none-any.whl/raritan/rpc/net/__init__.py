...
class InterfaceInfo(Structure):
    idlType = "net.InterfaceInfo:2.0.0"
    elements = ["name", "label", "type", "enabled", "masterIfName", "state", "macAddr", "mtu", "ipv4", "ipv6"]

    def __init__(self, name, label, type, enabled, masterIfName, state, macAddr, mtu, ipv4, ipv6):
...
class EthSettings(Structure):
    idlType = "net.EthSettings:3.0.0"
    elements = ["linkMode", "mtu", "authType", "eap"]

    def __init__(self, linkMode, mtu, authType, eap):
...
class InterfaceInfoChangedEvent(raritan.rpc.idl.Event):
    idlType = "net.InterfaceInfoChangedEvent:1.0.1"
