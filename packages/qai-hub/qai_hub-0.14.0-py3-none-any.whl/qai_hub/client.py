...
def _assert_is_valid_zipped_model(filepath: str) -> None:
    ...
    def __get_model_dir(
        ...
        model_subdirs = [dname for dname in zippedNames if dname.endswith(suffix)]
...
def _raise_internal_api_failure(message: str, url: Optional[str]):
    long_message = message

    if url is not None:
        request_url = urlparse(url)
        long_message += f" Please visit {request_url.scheme}://{request_url.netloc} to check the service's status."
...
def _api_call(api_func, *args, **kwargs) -> Any:
    ...
    def __get_exception_string(exc) -> str:
        # for some 403 exceptions the actual message is embedded in the exception
        message = str(exc)
        try:
    ...
    try:
        ...
        url = (
            e.request.url
            if e.request is not None and e.request.url is not None
            else None
        )
        _raise_internal_api_failure("Could not connect to Hub.", url)
        ...
        _raise_internal_api_failure(
            "Timeout occurred while communicating with Hub.", url
        )
        ...
        elif e.status_code == api_status_codes.HTTP_503_SERVICE_UNAVAILABLE:
            _raise_internal_api_failure(
                "Qualcomm AI Hub is unavailable right now.", e.url
            )
        else:
...
EXTENSIONS_BY_MODEL_TYPE = {
    MODEL_TYPES_BY_EXTENSION[ext]: ext + ".zip"
    if MODEL_TYPES_BY_EXTENSION[ext] in ZIPPABLE_MODEL_TYPES
    else ext
...
class Model:
    ...
    def download(self, filename: str | None = None) -> SourceModel | str:
        ...
        filename:
            ...
            Note for Windows: If the model is a Core ML model, `filename` must
            be specified, because Core ML currently cannot load mlmodel
            in-memory on Windows.
        ...
        if filename is not None:
            ...
            if self.model_type == SourceModelType.UNRECOGNIZED_MODEL_TYPE:
                ...
                raise UserError(
                    "Cannot load a Core ML model in memory on Windows "
...
class JobStatus:
    ...
    ----------
    code: str
        Status code for the job. Valid codes are "SUCCESS", "FAILED", "UNKNOWN", "CREATED",
        OPTIMIZING_MODEL", "PROVISIONING_DEVICE", and "MEASURING_PERFORMANCE"
    message: str
...
class Job(ABC):
    ...
    :py:func:`qai_hub.submit_inference_job`, :py:func:`qai_hub.get_job`, or
    ...
    def wait(self, timeout: int | None = None) -> JobStatus:
        ...
        # populate some possibly unitialized members
        if isinstance(self, CompileJob):
            if status.success:
                if self._target_model is None:
                    compile_job_pb = _api_call(
                        api.get_job, self._owner.config, job_id=self.job_id
                    ).compile_job
                    self.target_shapes: InputSpecs = dict(
                        api.utils.tensor_type_list_pb_to_list_shapes(
                            compile_job_pb.target_tensor_type_list
                        )
                    )
                    self._target_model: Model | None = self._owner._make_target_model(
                        compile_job_pb, self
                    )
        elif isinstance(self, ProfileJob):
            if not self.shapes:
                profile_job_pb = _api_call(
                    api.get_job, self._owner.config, job_id=self.job_id
                ).profile_job
                self.shapes: InputSpecs = dict(
                    api.utils.tensor_type_list_pb_to_list_shapes(
                        profile_job_pb.tensor_type_list
                    )
                )
        elif isinstance(self, InferenceJob):
            pass
    ...
    def get_status(self) -> JobStatus:
        ...
        if job_type == "compile_job":
            state = job_pb.compile_job.job_state
            failure_reason = job_pb.compile_job.failure_reason
            ...
            state = job_pb.inference_job.job_state
            failure_reason = job_pb.inference_job.failure_reason
        else:
...
class CompileJob(Job):
    ...
    def download_target_model(
        ...
        filename:
            ...
            A filename must be provided in the following cases:
              - Target model is a MLModel and user is on Windows OS.
              - Target model is a compiled MLModel (.mlmodelc)
...
class ProfileJob(Job):
    ...
    through :py:func:`qai_hub.submit_profile_job`, :py:func:`qai_hub.get_job`, or
    :py:func:`qai_hub.get_jobs`.
...
class InferenceJob(Job):
    ...
    through :py:func:`qai_hub.submit_inference_job`, :py:func:`qai_hub.get_job`, or
    :py:func:`qai_hub.get_jobs`.
...
class Client:
    ...
    def _make_target_model(
        self, job_pb: api_pb.CompileJob, producer: Job
    ) -> Model | None:
    ...
    def _upload_model(
        self,
        model: Model
        | Any  # Any instead of SourceModel to keep mypy happy
        | str
        | Path,
        model_type: SourceModelType,
        name: str | None = None,
    ...
    def get_job(self, job_id: str) -> Job:
        ...
        Examples
        --------
        Get job and print its status (granted you provide a valid ID)::
    ...
    def submit_compile_job(
        ...
        device:
            Devices for which to compile the input model for.
        ...
        input_specs:
            ...
            model (e.g., Core ML model) created from this profile job, and may
    ...
    def submit_profile_job(
        ...
        for dev in devices:
            ...
            if self.verbose:
                msg = (
                    f"Scheduled profiling job ({job.job_id}) successfully. To see "
                    "the status and results:\n"
                    f"    {job.url}\n"
                )
                print(msg)
    ...
    def submit_compile_and_profile_jobs(
        ...
        calibration_data: Dataset | DatasetEntries | str | None = None,
        retry: bool = True,
    ) -> Tuple[CompileJob | None, ProfileJob | None] | List[
