...
def get_refcounts(outs):
    def get_refcount(value):
        ...
        try:
            ...
            fn = object.__getattribute__(
                value.get_bound_obj(), "_fire_getrefcount"
            )
            logger.debug("call _fire_getrefcount for %s", value)
            cnt = fn()

            # -1: remove reference from __self__ of fn
            assert cnt > 0
            return cnt - 1
        ...
        # -1: remove reference from the argument of sys.getrefcount
        return sys.getrefcount(value.get_bound_obj()) - 1
    ...
    return {v: get_refcount(v) for v, cnt in counts.items() if cnt > 1}
...
def filter_outputs_by_frame(user_frame, refcounts):
    ...
    if not _is_interactive():
        ...
        #     logger.debug("%s %s", inst, inst.opcode in dis.hasjabs)
        ...
        logger.debug(
            "later_uses: %s has_jabs: %s", list(set(later_uses)), has_jabs
        )
...
def pickup_outputs(outs, package) -> List[fire.Value]:
    ...
    # Workaround for memory problem. GT #2664
    #
    # An objects with refcount is 1 is referenced only by Value.__bound_to.
    # The Value bounded to such object will be unbind here to enable gc to
    # collect the object even though the Value sill referenced by others such
    # as a cache stored in a user frame created by frame.f_locals.
    #
    # Since such object should be never used by a user program any more, and
    # **in the current implementation** the Value referenced from the object
    # should be never used, link from the Value to the object can be removed
    # safely.
    #
    # Be careful that such Value will be never used.
    orphans = [val for val, cnt in refcounts.items() if cnt == 1]
    for val in orphans:
        logger.debug("unbind %s to reduce reference counts", val)
        val.unbind()

    # If count == 1, an object bound to a value is referred only from a value.
    # That means this object is never referred afterward.
