...
appBasePath = os.path.abspath(os.getcwd())
logConfigPath = os.path.join(appBasePath, 'config', 'logging.yaml')
loggingBasePath = os.path.join(appBasePath, 'logs')
modulesPath = os.path.join(pathlib.Path(__file__).resolve().absolute().parents[2], 'airports_pipeline_modules')
...
class LoggingConf(object):
    ...
    def __init__(self, logConfigPath = logConfigPath):
        ...
        try:
            ...
            print("logging.yaml not found. A new logging.yaml will be created!")

    def create_log_directories(self):

        # Create base logging directory if it does not exists
        os.mkdir(loggingBasePath) if not os.path.isdir(loggingBasePath) else print("Log directory exists.")
    ...
    def create_logging_conf(self, dynamicPartDict):
        ...
        _staticItems = ["__main__", "airports_web", "airports_cli"]
        ...
        for _item in _items:
            if _item not in _staticItems:
                if _item == "csv_parser":
                    _key = f"airports_pipeline_modules.{_item}.parser"
                else:
                    _key = f"airports_pipeline_modules.{_item}.{_item}"
            else:
                _key = _item
