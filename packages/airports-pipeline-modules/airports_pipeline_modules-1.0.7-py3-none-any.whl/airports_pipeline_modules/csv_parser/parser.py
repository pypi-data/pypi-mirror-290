...
class Parser(object):
    ...
    def connect_database(self, host = None, port = None,
                username = None, password = None, defaultDbname = None,
                dbname = None, schema = None):
        
        try:
            self.database = Postgres(host = host, port = port,
                username = username, password = password, defaultDbname = defaultDbname,
                dbname = dbname, schema = schema)
        except Exception as err:
            logger.error(f"Could not create database connections! Exiting...")
            sys.exit()

    def insert_rows(self, tableName = None, filePath = filePath, commitSize = 1000):
        ...
        with open(filePath, "r", newline="", encoding="utf-8") as csvfile:
            ...
            logger.info(f"Created the reader for file {filePath}")

            self.database.create_table(tableName = tableName, fieldNames = _reader.fieldnames)
            ...
            for _row in _reader:               
                ...
                if _counter == 1:
                    ...
                    self.database.insert(tableName = tableName, values = _values)
                    _values = []
