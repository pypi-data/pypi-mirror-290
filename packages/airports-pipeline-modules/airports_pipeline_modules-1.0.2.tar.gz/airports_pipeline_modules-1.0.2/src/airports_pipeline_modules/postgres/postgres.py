...
import psycopg, re, sys, logging, os, pathlib
...
logger.info("Postgresql module is loading...")
...
class Postgres(object):
    ...
    def create_table(self, engine = None, schema = None, tableName = None, fieldNames = None):
        ...
        if not tableName or not fieldNames:
            logger.error("Error: Table name and/or field names for the table were not supplied. Quiting...")
    ...
    def insert(self, tableName = None, table = None, values = None):
        ...
        if not tableName:
            logger.info(f"{tableName} is not provided! Skipping...")
