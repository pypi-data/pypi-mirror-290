...
def tokenize_and_align(out_path: str, dataset: Iterator[Tuple[str, str]], old_tokenizer, new_tokenizer):
    """Utility function to align a dataset using the Moses format. The Moses format is a simple 
    format where each line contains the source and target sentence, separated by ' ||| '.

    out_path: str
        The path to save the aligned corpus to.
    dataset: Iterator[Tuple[str, str]]
        The dataset iterator, expects a tuple of source and target sentences.
    old_tokenizer: transformers.AutoTokenizer
        The tokenizer of the source language.
    new_tokenizer: transformers.AutoTokenizer
        The tokenizer of the target language.
    """
    ...
    if os.path.exists(
        ...
        if os.path.exists(out_path):
            ...
            for corpus in corpus_list:
                ...
                tokenize_and_align(out_path, dataset, old_tokenizer, new_tokenizer)
