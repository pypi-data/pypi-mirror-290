        ...
        def _internal():
            ...
            for i, repo in enumerate(self.__index["repos"]):
                ...
                if path.startswith(reponame):
                    return os.path.join(self.__cache_path, str(i), path[len(reponame) + 1:])
                if path.startswith(str(i)):
                    return os.path.join(self.__cache_path, str(i), path[len(str(i)) + 1:])

        var = _internal()

        return os.path.abspath(var) if var else None
