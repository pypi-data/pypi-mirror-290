import pytest
from zuu.core.drill import get_deep, set_deep, del_deep, set_default_deep
...
class TestComplexDataStructures:
            ...
            "products": [
                {
                    ...
                    "specs": {
                        "brand": "TechCo",
                        "components": [
                            {"type": "CPU", "details": {"model": "i7", "speed": "3.2GHz"}},
                            {"type": "RAM", "details": {"size": "16GB", "type": "DDR4"}}
                        ]
                    }
                }
            ],
    ...
    def test_get_deep_complex(self, complex_data):
        ...
        assert get_deep(complex_data, "products", 0, "specs", "components", 1, "details", "size") == "16GB"
    ...
    def test_set_deep_complex(self, complex_data):
        ...
        set_deep(complex_data, "settings", "theme", "colors", "tertiary", value="#28a745")
    ...
    def test_set_default_deep_complex(self, complex_data):
        set_default_deep(complex_data, "users", 2, value={"name": "Charlie"}, fillpadding=True)
    ...
    def test_complex_operations(self, complex_data):
        ...
        set_default_deep(complex_data, "settings", "theme", "animations", value={"enabled": True, "duration": "0.3s"})

        assert get_deep(complex_data, "users", 0, "details", "hobbies", 2) == "painting"
        with pytest.raises(KeyError):
            get_deep(complex_data, "products", 0, "specs", "brand")
        assert get_deep(complex_data, "settings", "theme", "animations", "duration") == "0.3s"
