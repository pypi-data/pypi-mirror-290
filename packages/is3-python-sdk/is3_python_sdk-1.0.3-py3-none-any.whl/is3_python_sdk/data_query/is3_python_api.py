...
from is3_python_sdk.domain.task_data import TaskDataDef
from is3_python_sdk.utils import Logging, get_header, get_property
from is3_python_sdk.utils.fileUtil import FileUtil
from is3_python_sdk.utils.is3_request_util import RequestUtil
...
class iS3PythonApi:
    ...
    def uploadFilesByPath(self, path, filePaths):
    ...
    '''
            预处理 JSON 数据并将其保存到指定的输出文件夹中。
    
            :param jsonData: 要处理的 JSON 数据，应该是字典或列表。
            :param outputFolderPath: 输出文件夹路径，用于保存处理后的 JSON 文件。
    '''
...
if __name__ == '__main__':
    filePath = r'E:\python_workspace\pycharm_workspace\is3-python-kafka-sdk\is3_python_sdk\config\config.ini'
    is3Api = iS3PythonApi(filePath, '1821553930207760386')
    file_paths = (
        r'E:\python_workspace\pycharm_workspace\is3-python-kafka-sdk\is3_python_sdk\domain')
    response = is3Api.uploadFilesByPath('sxing/demo', file_paths)
    print(response)