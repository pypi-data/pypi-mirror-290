...
class RefreshableBotoSession:
    ...
    def refresh_session_periodically(self):
        with self.refresh_lock:
            try:
                self.autorefresh_session.get_frozen_credentials()
                # print(f"RefreshableBotoSession: Automatic session refresh completed.")
            except Exception as err1:
                print(f"[WARNING] RefreshableBotoSession: Exception refreshing session credentials {err1}")
                try:
                    ...
                    print(f"[ERROR] RefreshableBotoSession: Exception refreshing credentials (2nd try) {err2}")
                else:
                    self.start_refresh_timer()
            else:
                self.start_refresh_timer()        # Start a new refresh timer after each refresh
