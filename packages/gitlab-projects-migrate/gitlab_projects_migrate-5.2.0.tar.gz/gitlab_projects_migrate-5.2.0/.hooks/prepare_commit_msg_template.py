...
def parse_commit_input(filepath: str) -> Tuple[bool, bool, str, List[str]]:
    ...
    # Result
    return empty, signoff, linesep.join(context), changes

# Prepare commit title, pylint: disable=too-many-nested-blocks
            ...
            for change in changes:
                if search(type_change, change):
                    for parser in changes_evaluator.parsers:
                        matches = match(parser.match, change)
                        if matches and isinstance(matches.lastindex, int) >= parser.group:
                            return parser.commit_type, matches.group(parser.group)
    ...
    for changes_matcher in CHANGES_MATCHERS:
        for matcher_type in changes_matcher.types:
            for type_change in matcher_type.changes:
                for change in changes:
                    if search(type_change, change):
                        return matcher_type.commit_type, changes_matcher.commit_scope

    # Result
    return COMMITS_DEFAULT_TYPE, COMMITS_DEFAULT_SCOPE
...
def prepare_commit_template(
    ...
    # Result
    return linesep.join(commit_lines)
