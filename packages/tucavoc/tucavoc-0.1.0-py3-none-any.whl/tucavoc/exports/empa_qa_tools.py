...
def export_EmpaQATool(
    ...
    for substance in substances:
        ...
        df_out[f"{export_name}-Value"] = df[(substance, "conc")].copy().astype(str)
        ...
        # Convert to str so we can control the formatting
        df_out[f"{export_name}-Accuracy"] = df[(substance, "u_expanded")].astype(str)
        # Input the missing values as 9. see issue #7 gitlab.empa.ch
        ...
        # Convert to str so we can control the formatting
        df_out[f"{export_name}-Precision"] = df[(substance, "u_precision")].astype(str)
