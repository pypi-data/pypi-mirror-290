...
class LayerNextClient:
    ...
    def retrieve_documents_with_structure(self, document_categories: List[str], document_keywords: List[str], 
                                          user_question: str, page_index=0, page_size=10):
        request_id = uuid.uuid4()
        logger.info(
            f" sdk | retrieve_documents_with_structure | request_id >>>> {request_id}, document_categories:{document_categories}, document_search_keys:{document_keywords}, question:{user_question}, page_index:{page_index}, page_size:{page_size}"
        )
        ...
        for doc in filtered_doc_list:
            element_list = document_element_dict.get(doc["uniqueName"], [])
        ...
        logger.info(
            f" sdk | retrieve_documents_with_structure | request_id <<<< {request_id}, response: {output_list}"
        )
        return output_list
