...
class LayerNextClient:
    ...
    def get_file_details(self, unique_name: str, fields_filter: dict = None):
        _datalake_client = datalake.DatalakeClient(
            self.encoded_key_secret, self.layernext_url
        )
    ...
    def __prepare_doc_elements_list(self, filtered_doc_list: list,  document_elem_dict: dict):
        output_list = []
        # Add chunk_list to each document in the filtered list
        for doc in filtered_doc_list:
            element_list = document_elem_dict.get(doc["uniqueName"], [])
            # Get name and named entities if not exist for this doc
            if "namedEntities" not in doc:
                metadata_obj = self.get_file_details(doc["uniqueName"], {'name': True, 'namedEntities': True})
    ...
    def query_documents_with_structure(self, document_categories: List[str], document_keywords: List[str], 
                                          user_question: str, page_index=0, page_size=10):
        """
        ...
        document_element_dict = self.find_elements(unique_name_list=unique_names, user_question=user_question)

        output_list = self.__prepare_doc_elements_list(filtered_doc_list, document_element_dict)
        logger.info(
            f" sdk | query_documents_with_structure | request_id <<<< {request_id}, response: {output_list}"
        )
        return output_list
    
    def get_documents_with_structure(self, doc_unique_name_list: list, user_question: str):
        """
        ...
        document_element_dict = self.find_elements(unique_name_list=doc_unique_name_list, user_question=user_question, is_target_documents_available=True)
    ...
    def extract_document_elements_keyInfo(self, document_elements_dict: dict, extraction_keys: List[dict], user_question: str, chat_id: str):
        """
        ...
        response = _datalake_client.extract_doc_content(document_elements_dict, extraction_keys, user_question)
    ...
    def find_elements(self, unique_name_list: List[str], user_question: str, is_target_documents_available: bool = False):
        """
        ...
        Parameters:
            ...
            user_question: Question asked by user
