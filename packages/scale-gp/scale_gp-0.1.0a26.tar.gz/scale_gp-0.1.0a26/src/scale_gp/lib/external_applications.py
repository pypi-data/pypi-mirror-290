...
ExternalApplicationCallable = Union[
    Callable[[str], ExternalApplicationOutput],
    Callable[[str, TestCase], ExternalApplicationOutput],
]
...
class ExternalApplication:
    ...
    def initialize(self, *, application_variant_id: str, application: ExternalApplicationCallable) -> Self:
        ...
        def application_wrapper(prompt: str, test_case: TestCase) -> ExternalApplicationOutput:
            args: List[Any] = [prompt]
    ...
    def generate_outputs(self, *, evaluation_dataset_id: str, evaluation_dataset_version: int) -> None:
        ...
        for test_case in test_cases:
            ...
            output = self._format_output(self.application(input_, test_case))
    ...
    def _format_output(self, output: ExternalApplicationOutput) -> ExternalApplicationOutput:
    ...
    def _create_outputs(
        ...
        def get_output_dict(output: ExternalApplicationOutput) -> ResultSchemaGeneration:
            if isinstance(output, ExternalApplicationOutputContextChunks):
                return ResultSchemaGeneration(
                    ...
                    generation_extra_info=ChunkExtraInfoSchema(
                        schema_type="CHUNKS",
                        chunks=[Chunk(metadata=chunk["metadata"], text=chunk["text"]) for chunk in output.chunks],
                    ),
                )
