...
class Balka_Text_Recognize_yolo:
    ...
    def __init__(self, local, model_path, yolo_path):
        ...
        self.balka_prod_sizes = {
            balka_prod.Balka_Prod.altai : Balka_text_recognize_params(
                number_params= Balka_text_cut_recognize_params(5,64,160),
                prod_params= Balka_text_cut_recognize_params(4, 64, 128),
                year_params= Balka_text_cut_recognize_params(2, 64, 64)
            ),
            balka_prod.Balka_Prod.begickaya : Balka_text_recognize_params(
                number_params= Balka_text_cut_recognize_params(5, 64, 160),
                prod_params= Balka_text_cut_recognize_params(2, 64,64),
                year_params= Balka_text_cut_recognize_params(2, 64, 96)
            ),
            balka_prod.Balka_Prod.promlit : Balka_text_recognize_params(
                number_params= Balka_text_cut_recognize_params(5, 64, 160),
                prod_params= Balka_text_cut_recognize_params(2, 64, 64),
                year_params= Balka_text_cut_recognize_params(2, 64, 64)
            ),
            balka_prod.Balka_Prod.ruzhimmash: Balka_text_recognize_params(
                number_params=Balka_text_cut_recognize_params(5, 64, 192),
                prod_params=Balka_text_cut_recognize_params(4, 64, 96),
                year_params=Balka_text_cut_recognize_params(2, 64, 64)
            ),
            balka_prod.Balka_Prod.tihvin: Balka_text_recognize_params(
                number_params=Balka_text_cut_recognize_params(5, 64, 160),
                prod_params=Balka_text_cut_recognize_params(4, 64, 128),
                year_params=Balka_text_cut_recognize_params(2, 64, 64)
            )
    ...
    def work_image_cut(self, _balka_prod, image_text_areas_2_half_balkas, image_balka_cuts):
        ...
        (number, number_conf) = self.work_image_cuts(
            number_image_cuts,
            self.balka_prod_sizes[_balka_prod].number_params.length,
            self.balka_prod_sizes[_balka_prod].number_params.height,
            self.balka_prod_sizes[_balka_prod].number_params.width
        )
        ...
        (prod, prod_conf) = self.work_image_cuts(
            prod_image_cuts,
            self.balka_prod_sizes[_balka_prod].prod_params.length,
            self.balka_prod_sizes[_balka_prod].prod_params.height,
            self.balka_prod_sizes[_balka_prod].prod_params.width
        )
        ...
        (year, year_conf) = self.work_image_cuts(
            year_image_cuts,
            self.balka_prod_sizes[_balka_prod].year_params.length,
            self.balka_prod_sizes[_balka_prod].year_params.height,
            self.balka_prod_sizes[_balka_prod].year_params.width
        )
