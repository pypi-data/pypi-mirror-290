...
def shell(
    ...
    client_fut = asyncio.run_coroutine_threadsafe(
        start_client(api_key, base_api_url, chat_uuid), loop
    )

    input_handler = InputHandler()
    chat = Chat(chat_uuid, settings.base_url, gql_client, model, strategy, autorun)
    shell = Shell(prompt, loop, input_handler, chat)
    shell.run()
    client_fut.cancel()
    print("\rBye!")
...
class Chat:
    ...
    async def process_chat_subscription(  # noqa: PLR0912, PLR0915
        ...
        async for response in self.gql_client.subscribe(CHAT_EVENTS_SUBSCRIPTION, vars):
            ...
            self.parent_uuid = event["eventUuid"] or self.parent_uuid
    ...
    def handle_command_chunk_event(self, event: dict[str, Any]) -> None:
        ...
        if self.block.event_uuid != event_uuid:
            self.close_block()
            self.block = CommandBlock(event_uuid, event["data"])
        else:
            assert isinstance(self.block, CommandBlock)
            self.block.update_data(event["data"])
...
class Shell:
    ...
    def _send_initial_prompt(self) -> None:
        if self.prompt is not None:
            self.stream_fut = self._run_coroutine(self.chat.send_prompt(self.prompt))
...
class CommandBlock(Block):
    def __init__(self, event_uuid: str, data: dict[str, Any]) -> None:
        super().__init__(event_uuid)
        self.status = "streaming"  # valid states: streaming, requires_confirmation, rejected, executing, done
        self.data = data
        self.result_content: str | None = None
    ...
    @property
    def type(self) -> str:
        return str(self.data["type"]).lower()
    ...
    def render(self) -> list[Any]:
        if self.type in ["file_read", "file_open"]:
            return self.render_file_read_or_open()

        return []

    def render_file_read_or_open(self) -> list[Any]:
        header = self.render_header(self.type)
        command = self.render_command(self.data["filePath"], "shell")
        output = self.render_output(self.result_content, self.data["language"])
        footer = self.render_status_footer()

        return [header, command, output, footer, "\n"]

    def render_command(self, content: str | None, lang: str) -> list[Any]:
        if content:
            padding = self.render_padding()
            code = self.highlight_code(content.strip(), lang)

            return [padding, code, padding]

        return []

    def render_output(self, content: str | None, lang: str) -> list[Any]:
        if content:
            header = self.render_header("output")
            code = pad_left(content.strip(), "  ")
            code = self.highlight_code(code, lang, line_numbers=False)
