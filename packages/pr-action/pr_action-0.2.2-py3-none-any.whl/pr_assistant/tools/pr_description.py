...
from pr_assistant.algo.pr_processing import get_pr_diff, retry_with_fallback_models, get_pr_diff_multiple_patchs, \
    OUTPUT_BUFFER_TOKENS_HARD_THRESHOLD
from pr_assistant.algo.token_handler import TokenHandler
from pr_assistant.algo.utils import set_custom_labels
from pr_assistant.algo.utils import load_yaml, get_user_labels, ModelType, show_relevant_configurations, get_max_tokens, \
    clip_tokens
from pr_assistant.config_loader import get_settings
from pr_assistant.git_providers import get_git_provider, GithubProvider, get_git_provider_with_context
    ...
    async def run(self):
        try:
            ...
            if self.git_provider.is_supported("gfm_markdown") and get_settings().pr_description.enable_help_text:
                ...
                pr_body += "\n\n___\n\n> 💡 **PR-Assistant usage**:"
                pr_body += "\n>Comment `/help` on the PR to get a list of all available PR-Assistant tools and their descriptions\n\n"
...
"""
    ...
    def _prepare_pr_answer_with_markers(self) -> Tuple[str, str, str, List[dict]]:
        ...
        if get_settings().pr_description.include_generated_by_header:
            ai_header = f"### 🤖 Generated by PR Assistant at {self.git_provider.last_commit_id.sha}\n\n"
        else:
            ai_header = ""

        ai_type = self.data.get('type')
        if ai_type and not re.search(r'<!--\s*pr_assistant:type\s*-->', body):
            pr_type = f"{ai_header}{ai_type}"
            body = body.replace('pr_assistant:type', pr_type)

        ai_summary = self.data.get('description')
        if ai_summary and not re.search(r'<!--\s*pr_assistant:summary\s*-->', body):
            summary = f"{ai_header}{ai_summary}"
            body = body.replace('pr_assistant:summary', summary)

        ai_walkthrough = self.data.get('pr_files')
        walkthrough_gfm = ""
        pr_file_changes = []
        if ai_walkthrough and not re.search(r'<!--\s*pr_assistant:walkthrough\s*-->', body):
            try:
                walkthrough_gfm, pr_file_changes = self.process_pr_files_prediction(walkthrough_gfm,
                                                                                    self.file_label_dict)
                body = body.replace('pr_assistant:walkthrough', walkthrough_gfm)
            except Exception as e:
                get_logger().error(f"Failing to process walkthrough {self.pr_id}: {e}")
                body = body.replace('pr_assistant:walkthrough', "")
