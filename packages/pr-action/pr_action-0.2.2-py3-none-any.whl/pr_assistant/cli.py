...
def run(inargs=None, args=None):
    ...
    if args.issue_url:
        result = asyncio.run(PRAssistant().handle_request(args.issue_url, [command] + args.rest))
    else:
        result = asyncio.run(PRAssistant().handle_request(args.pr_url, [command] + args.rest))
    if not result:
        parser.print_help()
