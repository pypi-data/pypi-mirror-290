...
class SeleniumAuthentication():
    ...
    def get_webdriver(self, service, intercept=False):
        ...
        if intercept and self.headless:
            ...
            driver = webdriver_wire.Edge(service=service,  options=chrome_options, seleniumwire_options=options)
        elif intercept:
            driver = webdriver_wire.Edge(service=service,  seleniumwire_options=options)
        else:
