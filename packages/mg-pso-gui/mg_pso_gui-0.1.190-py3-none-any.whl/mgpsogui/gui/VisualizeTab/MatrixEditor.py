...
class MatrixEditor(CTkFrame):
    ...
    def render(self):
        ...
        for key_value_pair in self.key_values:
            ...
            if self.edit_mode:
                bb = CTkOptionMenu(self.containerFrame, values=self.columns, width=140, variable=self.key_values[index]["value"], command=self.home_page.update_graph)
                ...
                CTkButton(self.containerFrame, text="Remove", width=140, command=return_func).grid(row=row, column=1, padx=(5, 5), pady=(5, 5), sticky="ew")
            else:
        ...
        if self.edit_mode:
            CTkButton(self.containerFrame, text="Exit", width=140, command=self.toggle_edit_mode).grid(row=row, column=0, padx=(5, 5), pady=(5, 5), sticky="ew")
        else:
            CTkButton(self.containerFrame, text="Edit", width=140, command=self.toggle_edit_mode).grid(row=row, column=0, padx=(5, 5), pady=(5, 5), sticky="ew")
        ...
        CTkButton(self.containerFrame, text="Add Figure", width=140, command=add_key_func).grid(row=row, column=1, padx=(5, 5), pady=(5, 5), sticky="ew")