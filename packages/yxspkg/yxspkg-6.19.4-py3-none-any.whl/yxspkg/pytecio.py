...
def get_dll():
    import teciolib
    p = Path(teciolib.__file__).parent
    if not p.is_dir():
        os.makedirs(p)

    if sys.platform.startswith('win'):
        dll_path = p / 'bin2017r3_tecio_dll.py'

    elif sys.platform.startswith('linux'):
        dll_path = p / 'bin2017r2_tecio_so.py'
...
VarTypeDict = {
    'float64':FieldDataType_Double,
    'float32':FieldDataType_Float,
    'int32':FieldDataType_Int32,
    'int64':FieldDataType_Int32,
    'int':FieldDataType_Int32,
    'int16':FieldDataType_Int16
...
class SzpltData(dict):
    ...
    def __init__(self,filename,isread=False):
        ...
        if not isinstance(filename,str):
            ...
            self.dll = None
            return
        if GLOBAL_DLL[0] is None:
            GLOBAL_DLL[0] = get_dll()   
        
        self.dll = GLOBAL_DLL[0]
    ...
    def _add_variable(self,zone_n,var_name,value):
        ...
        if dtype_name.startswith('int') and value.itemsize==8:
            raise Exception("不支持Int64类型数据")
        info['varTypes'].append(VarTypeDict[dtype_name] )
        ...
        for zone_p, item in enumerate(self.zone_info):
            ...
            if zone_n == zone_p+1:
                ...
                item['varTypes'].append(VarTypeDict[dtype_name])
    ...
    def _retrieve_zone_node_map(self,zone_n):
        info = self.zone_info[zone_n-1]
        if info['zoneType'] is not Structed_Grid and info['shareConnectivityFromZone'] == 0:
            jMax = info['IJK'][1]
    ...
    def close(self):
        ...
        self.is_closed = True
    def __del__(self):
        if self.dll and not self.is_closed:
            self.close()
...
class write_tecio:
    ...
    def __init__(self,filename,dataset=None ,verbose = True):
        ...
        p = Path(filename)
        self.filename = str(p.with_name(p.stem+'_tempx'+p.suffix))
        ...
        self.filehandle = self._get_filehandle()
        empty = np.array([])
        # is_passive = lambda x: if x is None
        for i,zone_name in enumerate(dataset.nameZones):
            ...
            for i_key in dataset.nameVars:
                if i_key in zone_set:
                    _ = zone_set[i_key]
    ...
    def close(self):
        ...
        os.rename(self.filename,self.output_filename)
        self.is_closed = True
    def __del__(self):
