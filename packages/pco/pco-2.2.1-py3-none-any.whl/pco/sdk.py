...
class Sdk:
    ...
    def __init__(self):
        ...
        self.SC2_Cam = shared_library_loader.libs()["sdk"]
        ...
        self.SC2_Cam.PCO_InitializeLib.argtypes = [
        ]

        self.SC2_Cam.PCO_CleanupLib.argtypes = [
        ]
        ...
        self.SC2_Cam.PCO_GetShutterMode.argtypes = [
            C.c_void_p,
            C.POINTER(C.c_uint16),
        ]

        self.SC2_Cam.PCO_SetShutterMode.argtypes = [
            C.c_void_p,
            C.c_uint16,
        ]
        ...
        self.SC2_Cam.PCO_GetSoftFPS.argtypes = [
            C.c_void_p,
            C.POINTER(C.c_uint32),
            C.POINTER(C.c_uint32),
            C.POINTER(C.c_uint16),
        ...
        self.SC2_Cam.PCO_SetSoftFPS.argtypes = [
            C.c_void_p,
            C.POINTER(C.c_uint32),
            C.c_uint32,
            C.c_uint16,
    ...
    def scan_cameras(self, if_type=None):
        """
        ...
        if if_type is not None:
            wIfType = C.c_uint16(interface_str2int[if_type])

        wDeviceCount = C.c_uint16()

        # first get number of avail devices
        null_ptr = C.POINTER(PCO_Device)() 
        error = self.SC2_Cam.PCO_ScanCameras(wIfType, wDeviceCount, null_ptr, 0)
        error_msg = self.get_error_text(error)

        logger.info(error_msg)

        if error:
            raise CameraException(sdk=self, code=error)
        elif wDeviceCount.value == 0:
            raise ValueError("-1: No devices available")

        # Now get devices
        device_arr = (PCO_Device * wDeviceCount.value)()
        device_arr_ptr = C.cast(device_arr, C.POINTER(PCO_Device))
        array_size = C.sizeof(PCO_Device)*wDeviceCount.value

        error = self.SC2_Cam.PCO_ScanCameras(wIfType, wDeviceCount, device_arr_ptr, array_size)
        error_msg = self.get_error_text(error)
        ...
        if error == 0:
            ...
            for device in device_arr:
                ...
                device_dict.update({"interface type": interface_int2str[device.PCO_InterfaceType]})
                device_dict.update({"camera name": device.CameraName.decode("ascii")})
                device_dict.update({"interface name": device.PCO_InterfaceName.decode("ascii")})
    ...
    def open_camera_ex(self, interface, camera_number=0):
        ...
        logger.debug("[{:5.3f} s] [sdk] {}: {}".format(duration, sys._getframe().f_code.co_name, error_msg))

        return {"error": error}


    # -------------------------------------------------------------------------
    # Wrapper for PCO_OpenCameraEx
    # -------------------------------------------------------------------------
    def open_camera_sn(self, interface, serialnumber):
        """
        This function is used to get a connection to a specific camera.
        """
        interface_dict = {
        ...
        dw_flags[0] = interface_dict[interface]
        dw_flags[1] = serialnumber
        strOpenStruct = PCO_OpenStruct()
        strOpenStruct.wSize = C.c_uint16(C.sizeof(PCO_OpenStruct))
        strOpenStruct.wInterfaceType = C.c_uint16(0x8002)  # Hardcoded value of PCO_INTERFACE_BY_SERIAL
        strOpenStruct.wCameraNumber = C.c_uint16(0)  # Unused for open by Serial number
        strOpenStruct.wCameraNumAtInterface = C.c_uint16(0)  # Unused for open by Serial number
        strOpenStruct.wOpenFlags = (C.c_uint16 * 10)(*w_flags)
        strOpenStruct.dwOpenFlags = (C.c_uint32 * 5)(*dw_flags)
        strOpenStruct.wOpenPtr = (C.c_void_p * 6)(*void_ptr)
        strOpenStruct.zzwDummy = (C.c_uint16 * 8)(*w_dummy)
    ...
    def reset_lib(self):
        ...
        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_ResetLib()
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
    ...
    # -------------------------------------------------------------------------
    # 2.1.4 PCO_InitializeLib
    # -------------------------------------------------------------------------
    def initialize_lib(self):
        """
        This function is used to initialize the sc2_cam library.
        This functions is required to call on progam start after loading sc2_cam library.
        """

        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_InitializeLib()
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
    ...
    # -------------------------------------------------------------------------
    # 2.1.4 PCO_CleanupLib
    # -------------------------------------------------------------------------
    def cleanup_lib(self):
        """
        This function is used to clean up the sc2_cam library.
        This functions is required to call on progam end before unloading sc2_cam library.
        """

        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_CleanupLib()
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
    ...
    def get_camera_description_intensified(self):
        ...
        self.SC2_Cam.PCO_GetCameraDescriptionEx.argtypes = [
            C.c_void_p,
            C.POINTER(PCO_Description_Intensified),
            C.c_uint16,
        ]
    ...
    def get_general(self):
        ...
        strGeneral.strCamType.wSize = C.sizeof(PCO_CameraType)
    ...
    def get_camera_type(self):
        ...
        if error == 0:
            ...
            ret.update(
                {
                    "camera type": camera_type.get(
                        strCamType.wCamType, str(hex(int(strCamType.wCamType)))
                    )
                }
            )
            ret.update({"camera type raw": strCamType.wCamType})
            ret.update({"camera subtype": strCamType.wCamSubType})
            ret.update({"serial number": strCamType.dwSerialNumber})
            ret.update({"hw version": strCamType.dwHWVersion})
            ret.update({"fw version": strCamType.dwFWVersion})
            ret.update(
                {
                    "interface type": interface_type.get(
                        strCamType.wInterfaceType, str(hex(int(strCamType.wInterfaceType)))
                    )
                }
            )

            fwCount = strCamType.strFirmwareVersion.DeviceNum
            fwDevices = strCamType.strFirmwareVersion.Device
            fwInfo = []
            for devs in range(fwCount):
                fwInfo.append(
                    {
                        "name": fwDevices[devs].szName.decode("ascii"),
                        "major": fwDevices[devs].bMajorRev,
                        "minor": fwDevices[devs].bMinorRev,
                        "variant": fwDevices[devs].wVariant,
                    }
                )
            ret.update({"firmware version": fwInfo})

            hwCount = strCamType.strHardwareVersion.BoardNum
            hwDevices = strCamType.strHardwareVersion.Board
            hwInfo = []
            for devs in range(hwCount):
                hwInfo.append(
                    {
                        "name": hwDevices[devs].szName.decode("ascii"),
                        "batchno": hwDevices[devs].wBatchNo,
                        "revision": hwDevices[devs].wRevision,
                        "variant": hwDevices[devs].wVariant,
                    }
                )
            ret.update({"hardware version": hwInfo})
    ...
    def arm_camera(self):
        ...
        logger.info("[{:5.3f} s] [sdk] {}: {}".format(duration, sys._getframe().f_code.co_name, self.get_error_text(error)))
    ...
    def get_shutter_mode(self):
        ...
        wShuttermode = C.c_uint16(0)


        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_GetShutterMode(self.camera_handle, wShuttermode)
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
        ...
        ret = {}
        if error == 0:
            ret.update({ "shutter_mode": shutter_mode_types[wShuttermode.value] })
    ...
    def set_shutter_mode(self, shutter_mode):
        ...
        wShuttermode = C.c_uint16(shutter_mode_types[shutter_mode])

        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_SetShutterMode(self.camera_handle, wShuttermode)
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
    ...
    # -------------------------------------------------------------------------
    # 2.4.12 PCO_GetFanControlParameters
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 2.4.13 PCO_SetFanControlParameters
    # -------------------------------------------------------------------------
    def set_fan_control_parameters(self, mode, value=100):
        ...
        error = self.SC2_Cam.PCO_SetFanControlParameters(
            self.camera_handle, wMode, wValue, wReserved
        )
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
    ...
    def get_binning(self):
        """
        Returns the binning values for x and y and the binning mode sum or average (not all cameras support it).
        ...
        >>> get_binning()
        {'binning x': 2, 'binning y': 2, 'binning mode': "sum"}

        """
        wBinHorz = C.c_uint16(0xF000)
        wBinVert = C.c_uint16(0xF000)
        time_start = time.perf_counter()
        ...
        ret = {}
        if error == 0:
            mode = wBinHorz.value & 0xF000
            if mode == 0:
                mode = "sum"
            else:
                mode = "average"
            ret.update({"binning x": (wBinHorz.value & 0x0FFF),
                        "binning y": (wBinVert.value & 0x0FFF),
                        "binning mode": mode})

        logger.info("[{:5.3f} s] [sdk] {}: {}".format(duration, sys._getframe().f_code.co_name, error_msg))

        if error:
            raise CameraException(sdk=self, message=error_msg, code=error)
    ...
    # -------------------------------------------------------------------------
    def set_binning(self, x, y, binningmode="sum"):
        """"""
        if binningmode == "average":
            x |= 0x1000
            y |= 0x1000
        ...
        inp = {}
        inp.update({"binning x": x, "binning y": y, "binning mode": binningmode})
    ...
    def get_coc_runtime(self):
        ...
        :return: {'time second': float
                'time nanosecond': float}
        :rtype: dict
        """
    ...
    def get_hwio_signal_descriptor(self, signal_number):
        ...
        if error == 0:
            signal_names = []
            for n in range(4):
                signal_names.append(bytes(pstrSignal.strSignalName[n]).decode("ascii").strip("\0"))
    ...
    def get_storage_struct(self):
        """
        Gets storage structs for camram cameras
        """
        strStorage = PCO_Storage()
        strStorage.wSize = C.sizeof(PCO_Storage)

        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_GetStorageStruct(
            self.camera_handle, strStorage
        )
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)

        segment_sizes = [
            strStorage.dwRamSegSize[0],
            strStorage.dwRamSegSize[1],
            strStorage.dwRamSegSize[2],
            strStorage.dwRamSegSize[3]
        ]
        ...
        if error == 0:
            ret.update(
                {
                    "ram size": strStorage.dwRamSize,
                    "page size": strStorage.wPageSize,
                    "segment sizes": segment_sizes,
                    "actual segment": strStorage.wActSeg,
                    "compression mode": strStorage.wCompressionMode,
                }
            )
    ...
    # -------------------------------------------------------------------------
    def set_storage_struct(self,ram_size, page_size, segment_sizes, actual_segment, compression_mode):
        """
        Sets storage structs for camram cameras
        """

        strStorage = PCO_Storage()
        strStorage.wSize = C.sizeof(PCO_Storage)

        strStorage.dwRamSize = C.c_uint32(ram_size)
        strStorage.wPageSize = C.c_uint16(page_size)
        strStorage.dwRamSegSize[0] = segment_sizes[0]
        strStorage.dwRamSegSize[1] = segment_sizes[1]
        strStorage.dwRamSegSize[2] = segment_sizes[2]
        strStorage.dwRamSegSize[3] = segment_sizes[3]

        strStorage.wActSeg = actual_segment
        strStorage.wCompressionMode = compression_mode

        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_SetStorageStruct(
            self.camera_handle, strStorage
        )
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
    ...
    def set_camera_ram_segment_size(self, ram_segment_sizes):
        ...
        dwRamSegSize = (C.c_uint32 * 4)(*ram_segment_sizes)
    ...
    # -------------------------------------------------------------------------
    # 2.XX.X PCO_GetSoftFPS
    # -------------------------------------------------------------------------
    def get_soft_fps(self):
        """
        Gets soft fps.

        :return: {'fps': double,
                'exposure': int,
                'exposure timebase': str -> 'ms', 'us' or 'ns'
                }
        :rtype: dict
        ...
        dwFPS = C.c_uint32()
        dwExp = C.c_uint32()
        wTimeBaseExp = C.c_uint16()

        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_GetSoftFPS(
            self.camera_handle,
            dwFPS,
            dwExp,
            wTimeBaseExp)
        ...
        if error == 0:
            timebase = {0: "ns", 1: "us", 2: "ms"}
            ret.update({'fps': dwFPS.value / 1000})
            ret.update({'exposure': dwExp.value})
            ret.update({"exposure timebase": timebase[wTimeBaseExp.value]})
    ...
    # -------------------------------------------------------------------------
    # 2.XX.X PCO_SetSoftFPS
    # -------------------------------------------------------------------------
    def set_soft_fps(self, fps, exposure, exposure_timebase, flags = 0):
        """
        Sets soft fps .

        :param fps: double
        :param exposure: int
        :param exposure_timebase: str -> 'ms', 'us' or 'ns'
        :param flags: int
        ...
        dwFPS = C.c_uint32(fps * 1000)
        wFlags = C.c_uint16(flags)

        time_start = time.perf_counter()
        error = self.SC2_Cam.PCO_SetSoftFPS(
            self.camera_handle,
            dwFPS,
            exposure,
            timebase[exposure_timebase],
