...
class BedrockSummarizer:
    def __init__(self, region_name='us-east-1', default_model_name='sonnet-3.5', 
    ...
    def _get_bedrock_client(self):
        try:
            if self.is_sagemaker:
                logger.info("Running in SageMaker. Using instance role for authentication.")
                ...
                logger.error("AWS credentials are required when not running in SageMaker.")
                raise ValueError("AWS credentials are required when not running in SageMaker. Please provide aws_access_key_id and aws_secret_access_key.")
