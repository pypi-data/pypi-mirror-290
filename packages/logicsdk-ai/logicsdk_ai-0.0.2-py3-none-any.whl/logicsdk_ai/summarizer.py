...
class BedrockSummarizer:
    def __init__(self, region_name='us-west-2', default_model_name='sonnet-3.5', aws_access_key_id=None,
    ...
    def _get_bedrock_client(self):
        try:
            if self.aws_access_key_id and self.aws_secret_access_key:
                ...
                # Attempt to use default credentials (will use IAM role in environments like SageMaker)
                client = boto3.client(service_name="bedrock-runtime", region_name=self.region_name)
                # Test connection or simple API call to confirm credentials work, if required
                # client.describe_model(ModelName='your-model-name')  # This line is just an example
                logger.info("Successfully authenticated using default AWS credentials.")
                return client
        except Exception as e:
            logger.error("Authentication failed. Please check your AWS credentials or IAM roles.")
            raise Exception("Authentication failed. Please check your AWS credentials or IAM roles.") from e
