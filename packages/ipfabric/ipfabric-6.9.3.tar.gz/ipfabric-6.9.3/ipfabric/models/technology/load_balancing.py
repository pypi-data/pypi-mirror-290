...
class LoadBalancing(BaseModel):
    ...
    @computed_field
    @property
    def virtual_servers_f5_partitions(self) -> Table:
        # TODO: Remove 7.0
        warn(
            "LoadBalancing.virtual_servers_f5_partitions will be moved to LoadBalancing.partitions in 7.0",
            DeprecationWarning,
            stacklevel=2,
        )
        return Table(client=self.client, endpoint="tables/load-balancing/f5-partitions", sn=self.sn)
