...
class Policies(BaseModel):
    ...
    def search_policies_for_endpoint(self, api_scope_id: str) -> List[Policy]:
...
class Roles(BaseModel):
    ...
    def search_roles_for_endpoint(self, api_scope_id: str) -> List[Role]:
        policies = self._policies.search_policies_for_endpoint(api_scope_id)
        if not policies:
            logger.warning(f"No policies found for API Scope ID {api_scope_id}.")
            return list()
        roles = set()
        for p in policies:
