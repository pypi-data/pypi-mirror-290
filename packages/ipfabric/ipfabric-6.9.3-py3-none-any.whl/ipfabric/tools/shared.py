...
def validate_ip_network(ip: Union[str, VALID_IP]) -> IPv4Network:
    if isinstance(ip, IPv4Network):
        ...
        try:
            ...
            raise ValueError(f'IP "{ip}" not a valid IP Address')


def validate_ip_network_str(ip: Union[str, IPv4Address, IPv4Network, IPv4Interface]):
    return str((validate_ip_network(ip)))
