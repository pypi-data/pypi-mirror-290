...
class FourierFeatures(nn.Module):
    def __init__(self, in_features, out_features, std=1.0, dtype=None, device=None):
        super().__init__()
        assert out_features % 2 == 0
        self.weight = nn.Parameter(
            torch.empty([out_features // 2, in_features], dtype=dtype, device=device)
        )

    def forward(self, input):
        f = 2 * math.pi * input @ cast_to_input(self.weight.T, input)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        beta = self.beta
        if beta is not None:
            beta = cast_to_input(beta, x)
        return F.layer_norm(
            x,
            x.shape[-1:],
            weight=cast_to_input(self.gamma, x),
            bias=beta,
        )
...
class GLU(nn.Module):
    def __init__(
        ...
        self.act = activation
        self.proj = (
            operations.Linear(dim_in, dim_out * 2, dtype=dtype, device=device)
            if not use_conv
...
class RotaryEmbedding(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "inv_freq", torch.empty((dim // 2,), device=device, dtype=dtype)
        )
    ...
    def forward(self, t):
        ...
        freqs = torch.einsum(
            "i , j -> i j", t, cast_to_input(self.inv_freq, t)
        )
        ...
        scale = cast_to_input(self.scale, t) ** rearrange(
            power, "n -> n 1"
        )
...
def apply_rotary_pos_emb(t, freqs, scale=1):
    ...
    dtype = (
        t.dtype
    )  # reduce(torch.promote_types, (t.dtype, freqs.dtype, torch.float32))
...
class FeedForward(nn.Module):
    def __init__(
        ...
        linear_out = (
            operations.Linear(
                inner_dim, dim_out, bias=not no_bias, dtype=dtype, device=device
            )
            if not use_conv
            else operations.Conv1d(
                inner_dim,
    ...
    def __init__(
        ...
        if dim_context is not None:
            self.to_q = operations.Linear(
                dim, dim, bias=False, dtype=dtype, device=device
            )
            self.to_kv = operations.Linear(
                dim_kv, dim_kv * 2, bias=False, dtype=dtype, device=device
            )
        else:
            self.to_qkv = operations.Linear(
                dim, dim * 3, bias=False, dtype=dtype, device=device
            )
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.pre_norm = (
            LayerNorm(dim, dtype=dtype, device=device, **norm_kwargs)
            if not remove_norms
            else nn.Identity()
        )
...
class ContinuousTransformer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.project_in = (
            operations.Linear(dim_in, dim, bias=False, dtype=dtype, device=device)
            if dim_in is not None
            else nn.Identity()
        )
        self.project_out = (
            operations.Linear(dim, dim_out, bias=False, dtype=dtype, device=device)
        ...
        if rotary_pos_emb:
            self.rotary_pos_emb = RotaryEmbedding(
                max(dim_heads // 2, 32), device=device, dtype=dtype
            )
        else:
...
class AudioDiffusionTransformer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.timestep_features = FourierFeatures(
            1, timestep_features_dim, dtype=dtype, device=device
        )

        self.to_timestep_embed = nn.Sequential(
            operations.Linear(
                timestep_features_dim, embed_dim, bias=True, dtype=dtype, device=device
            ),
            nn.SiLU(),
            operations.Linear(
        ...
        if prepend_cond_dim > 0:
            ...
            self.to_prepend_embed = nn.Sequential(
                operations.Linear(
                    prepend_cond_dim, embed_dim, bias=False, dtype=dtype, device=device
                ),
                nn.SiLU(),
                operations.Linear(
        ...
        self.preprocess_conv = operations.Conv1d(
            dim_in, dim_in, 1, bias=False, dtype=dtype, device=device
        )
        self.postprocess_conv = operations.Conv1d(
            io_channels, io_channels, 1, bias=False, dtype=dtype, device=device
        )
    ...
    def _forward(
        ...
        **kwargs,
        ...
        timestep_embed = self.to_timestep_embed(
            self.timestep_features(t[:, None]).to(x.dtype)
        )  # (b, embed_dim)
