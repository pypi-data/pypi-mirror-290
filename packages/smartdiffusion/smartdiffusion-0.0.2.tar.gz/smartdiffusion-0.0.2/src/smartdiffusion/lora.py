...
def model_lora_keys_clip(model, key_map={}):
    ...
    if k in sdk:
        key_map["lora_te1_text_projection"] = (
            k  # OneTrainer SD3 lora, not necessary but omits warning
        )
    return key_map
...
def model_lora_keys_unet(model, key_map={}):
    ...
    for k in sdk:
        if k.startswith("diffusion_model.") and k.endswith(".weight"):
            ...
            key_map["{}".format(k[: -len(".weight")])] = (
                k  # generic lora format without any weird key names
            )
    ...
    if isinstance(model, SD3):  # Diffusers lora SD3
        diffusers_keys = mmdit_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
                key_lora = "transformer.{}".format(
                    k[: -len(".weight")]
                )  # regular diffusers sd3 lora format
                key_map[key_lora] = to

                key_lora = "base_model.model.{}".format(
                    k[: -len(".weight")]
                )  # format for flash-sd3 lora and others?
                key_map[key_lora] = to

                key_lora = "lora_transformer_{}".format(
                    k[: -len(".weight")].replace(".", "_")
                )  # OneTrainer lora
                key_map[key_lora] = to
    if isinstance(model, AuraFlow):  # Diffusers lora AuraFlow
        diffusers_keys = auraflow_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
                ...
                key_lora = "transformer.{}".format(
                    k[: -len(".weight")]
                )  # simpletrainer and probably regular diffusers lora format
                key_map[key_lora] = to
    if isinstance(model, HunyuanDiT):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
                key_lora = k[len("diffusion_model.") : -len(".weight")]
                key_map["base_model.model.{}".format(key_lora)] = (
                    k  # official hunyuan lora format
                )
    if isinstance(model, Flux):  # Diffusers lora Flux
        ...
        for k in diffusers_keys:
            if k.endswith(".weight"):
                ...
                key_lora = "transformer.{}".format(
                    k[: -len(".weight")]
                )  # simpletrainer and probably regular diffusers flux lora format
                key_map[key_lora] = to
    return key_map
