    ...
    if ckpt.lower().endswith(".safetensors") or ckpt.lower().endswith(".sft"):
        ...
        if safe_load:
            if not "weights_only" in load.__code__.co_varnames:
                logging.warning(
                    "Warning load doesn't support weights_only on this pytorch version, loading unsafely."
                )
                safe_load = False
        if safe_load:
...
def clip_text_transformers_convert(tf, prefix_from, prefix_to):
    ...
    if tp in tf:
        tf["{}text_projection.weight".format(prefix_to)] = (
            tf.pop(tp).transpose(0, 1).contiguous()
        )
    return tf
...
def flux_to_diffusers(mmdit_config, output_prefix=""):
    ...
    for index in range(n_single_layers):
        ...
        for end in ("weight", "bias"):
            ...
            key_map["{}proj_mlp.{}".format(k, end)] = (
                qkv,
                (0, hidden_size * 3, hidden_size),
            )
        block_map = {}  # TODO
...
def repeat_to_batch_size(tensor, batch_size, dim=0):
    if tensor.shape[dim] > batch_size:
        ...
        return tensor.repeat(
            dim * [1]
            + [ceil(batch_size / tensor.shape[dim])]
...
def bislerp(samples, width, height):
    def slerp(b1, b2, r):
        ...
        res = (sin((1.0 - r.squeeze(1)) * omega) / so).unsqueeze(1) * b1_normalized + (
            sin(r.squeeze(1) * omega) / so
    ...
    def generate_bilinear_data(length_old, length_new, device):
        coords_1 = arange(length_old, dtype=float32, device=device).reshape(
            (1, 1, 1, -1)
        )
        ...
        coords_2 = (
            arange(length_old, dtype=float32, device=device).reshape((1, 1, 1, -1)) + 1
        )
        coords_2[:, :, :, -1] -= 1
...
def lanczos(samples, width, height):
    images = [
        Image.fromarray(
            clip(255.0 * image.movedim(0, -1).cpu().numpy(), 0, 255).astype(uint8)
        )
    ...
    images = [
        from_numpy(array(image).astype(float32) / 255.0).movedim(-1, 0)
        for image in images
    ]
    result = stack(images)
