...
def Normalize(in_channels, num_groups=32):
    return disable_weight_init.GroupNorm(
...
class Upsample(nn.Module):
    def __init__(self, in_channels, with_conv):
        ...
        if self.with_conv:
            self.conv = disable_weight_init.Conv2d(
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.conv1 = disable_weight_init.Conv2d(
            in_channels, out_channels, kernel_size=3, stride=1, padding=1
        )
        if temb_channels > 0:
            self.temb_proj = disable_weight_init.Linear(temb_channels, out_channels)
        self.norm2 = Normalize(out_channels)
        self.dropout = torch.nn.Dropout(dropout, inplace=True)
        self.conv2 = disable_weight_init.Conv2d(
            out_channels, out_channels, kernel_size=3, stride=1, padding=1
        )
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                self.conv_shortcut = disable_weight_init.Conv2d(
                    in_channels, out_channels, kernel_size=3, stride=1, padding=1
                )
            else:
                self.nin_shortcut = disable_weight_init.Conv2d(
        ...
        *,
        ...
        if self.use_timestep:
            ...
            self.temb.dense = nn.ModuleList(
                [
                    disable_weight_init.Linear(self.ch, self.temb_ch),
                    disable_weight_init.Linear(self.temb_ch, self.temb_ch),
                ]
            )
        # downsampling

        self.conv_in = disable_weight_init.Conv2d(
        ...
        self.conv_out = disable_weight_init.Conv2d(
