...
class StableZero123_Conditioning:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "width": (
                    "INT",
                    {"default": 256, "min": 16, "max": MAX_RESOLUTION, "step": 8},
                ),
                "height": (
                    "INT",
                    {"default": 256, "min": 16, "max": MAX_RESOLUTION, "step": 8},
    ...
    def encode(
        ...
        cond = torch.cat(
            [pooled, cam_embeds.to(pooled.device).repeat((pooled.shape[0], 1, 1))],
            dim=-1,
        )

        positive = [[cond, {"concat_latent_image": t}]]
        negative = [
            [torch.zeros_like(pooled), {"concat_latent_image": torch.zeros_like(t)}]
        ]
...
class StableZero123_Conditioning_Batched:
    ...
    def encode(
        ...
        cond = torch.cat(
            [utils.repeat_to_batch_size(pooled, batch_size), cam_embeds], dim=-1
        )
        ...
        return (
            positive,
            negative,
            {"samples": latent, "batch_index": [0] * batch_size},
        )
...
class SV3D_Conditioning:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "width": (
                    "INT",
                    {"default": 576, "min": 16, "max": MAX_RESOLUTION, "step": 8},
                ),
                "height": (
                    "INT",
                    {"default": 576, "min": 16, "max": MAX_RESOLUTION, "step": 8},
    ...
    def encode(
        self, clip_vision, init_image, vae, width, height, video_frames, elevation
    ):
        output = clip_vision.encode_image(init_image)
        pooled = output.image_embeds.unsqueeze(0)
        pixels = utils.common_upscale(
