from torch import Tensor, stack, view_as_complex, view_as_real
...
def reshape_for_broadcast(
    ...
    Args:
        freqs_cis (Union[Tensor, Tuple[Tensor]]): Frequency tensor to be reshaped.
...
def rotate_half(x):
    ...
    return stack([-x_imag, x_real], dim=-1).flatten(3)
...
def apply_rotary_emb(
    ...
    Args:
        xq (Tensor): Query tensor to apply rotary embeddings. [B, S, H, D]
        xk (Tensor): Key tensor to apply rotary embeddings.   [B, S, H, D]
        freqs_cis (Union[Tensor, Tuple[Tensor]]): Precomputed frequency tensor for complex exponentials.
        head_first (bool): head dimension first (except batch dim) or not.
    ...
    if isinstance(freqs_cis, tuple):
        ...
        xq_out = view_as_real(xq_ * freqs_cis).flatten(3).type_as(xq)
        if xk is not None:
            xk_ = view_as_complex(
                xk.float().reshape(*xk.shape[:-1], -1, 2)
            )  # [B, S, H, D//2]
            xk_out = view_as_real(xk_ * freqs_cis).flatten(3).type_as(xk)
