...
class VideoTriangleCFGGuidance:
    ...
    def patch(self, model, min_cfg):
        def linear_cfg(args):
            ...
            scale = (values * (cond_scale - min_cfg) + min_cfg).reshape(
                (cond.shape[0], 1, 1, 1)
            )
...
class ImageOnlyCheckpointSave(config_model_merging.CheckpointSave):
    ...
    def save(
        self, model, clip_vision, vae, filename_prefix, prompt=None, extra_pnginfo=None
    ):
        config_model_merging.save_checkpoint(
            model,
            clip_vision=clip_vision,
            vae=vae,
