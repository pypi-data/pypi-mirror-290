...
def calc_cond_uncond_batch(
    ...
    logging.warning(
        "WARNING: The smartdiffusion.samplers.calc_cond_uncond_batch function is deprecated please use the calc_cond_batch one instead."
    )
...
def resolve_areas_and_cond_masks(conditions, h, w, device):
    logging.warning(
        "WARNING: The smartdiffusion.samplers.resolve_areas_and_cond_masks function is deprecated please use the resolve_areas_and_cond_masks_multidim one instead."
    )
...
class CFGGuider:
    ...
    def inner_set_conds(self, conds):
        for k in conds:
            self.original_conds[k] = smartdiffusion.sampler_helpers.convert_cond(
                conds[k]
            )
    ...
    def sample(
        ...
        self.inner_model, self.conds, self.loaded_models = (
            smartdiffusion.sampler_helpers.prepare_sampling(
        ...
        if denoise_mask is not None:
            denoise_mask = smartdiffusion.sampler_helpers.prepare_mask(
        ...
        smartdiffusion.sampler_helpers.cleanup_models(self.conds, self.loaded_models)
