...
def prepare_mask(noise_mask, shape, device):
    ...
    noise_mask = smartdiffusion.utils.repeat_to_batch_size(noise_mask, shape[0])
...
def convert_cond(cond):
    ...
    for c in cond:
        ...
        if c[0] is not None:
            model_conds["c_crossattn"] = smartdiffusion.conds.CONDCrossAttn(
                c[0]
            )  # TODO: remove
            temp["cross_attn"] = c[0]
...
def prepare_sampling(model, noise_shape, conds):
    ...
    smartdiffusion.model_management.load_models_gpu(
        [model] + models,
