...
class CLIPTextModel_(torch.nn.Module):
    def __init__(self, config_dict, dtype, device, operations):
        ...
        self.embeddings = CLIPEmbeddings(
            embed_dim, dtype=dtype, device=device, operations=operations
        )
...
class CLIPVisionEmbeddings(torch.nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        return torch.cat(
            [
                cast_to_input(self.class_embedding, embeds).expand(
                    pixel_values.shape[0], 1, -1
                ),
                embeds,
            ],
            dim=1,
        ) + cast_to_input(self.position_embedding.weight, embeds)
