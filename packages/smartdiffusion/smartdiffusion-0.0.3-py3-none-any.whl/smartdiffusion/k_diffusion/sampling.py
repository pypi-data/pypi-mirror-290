...
def sample_ipndm(
    model, x, sigmas, extra_args=None, callback=None, disable=None, max_order=4
):
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if order == 1:  # First Euler step.
            ...
            x_next = (
                x_cur
                + (t_next - t_cur)
                * (23 * d_cur - 16 * buffer_model[-1] + 5 * buffer_model[-2])
                / 12
            )
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if order == 1:  # First Euler step.
            ...
            x_next = x_cur + (t_next - t_cur) * (
                coeff1 * d_cur + coeff2 * buffer_model[-1]
            )
        elif order == 3:  # Use two history points.
            h_n = t_next - t_cur
            h_n_1 = t_cur - t_steps[i - 1]
            ...
            x_next = x_cur + (t_next - t_cur) * (
                coeff1 * d_cur + coeff2 * buffer_model[-1] + coeff3 * buffer_model[-2]
            )
        elif order == 4:  # Use three history points.
            h_n = t_next - t_cur
            h_n_1 = t_cur - t_steps[i - 1]
            ...
            coeff2 = (
                ...
                - (
                    1
                    + (h_n_1 / h_n_2)
                    + (h_n_1 * (h_n_1 + h_n_2) / (h_n_2 * (h_n_2 + h_n_3)))
                )
                * temp2
            )
            coeff3 = (
            ...
            x_next = x_cur + (t_next - t_cur) * (
                coeff1 * d_cur
                + coeff2 * buffer_model[-1]
                + coeff3 * buffer_model[-2]
                + coeff4 * buffer_model[-3]
            )
        if len(buffer_model) == max_order - 1:
            for k in range(max_order - 2):
                buffer_model[k] = buffer_model[k + 1]
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if order == 1:  # First Euler step.
            ...
            x_next = (
                x_cur
                + coeff_cur * d_cur
                + coeff_prev1 * buffer_model[-1]
                + coeff_prev2 * buffer_model[-2]
            )
        elif order == 4:  # Use three history points.
