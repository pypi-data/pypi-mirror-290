...
from smartdiffusion.ldm.models.autoencoder import AutoencoderKL, AutoencodingEngine
from smartdiffusion.ldm.cascade.stage_a import StageA
from smartdiffusion.ldm.cascade.stage_c_coder import StageC_coder
from smartdiffusion.ldm.audio.autoencoder import AudioOobleckVAE
import yaml

from smartdiffusion.utils import (
    tiled_scale,
...
from smartdiffusion.taesd.taesd import TAESD
from smartdiffusion.model_patcher import ModelPatcher
from smartdiffusion.lora import model_lora_keys_unet, model_lora_keys_clip
from smartdiffusion.t2i_adapter.adapter import StyleAdapter
...
class CLIP:
    def __init__(
        self, target=None, embedding_directory=None, no_init=False, tokenizer_data={}
    ):
        if no_init:
            return
        ...
        self.layer_idx = None
        logging.debug(
            "CLIP model load device: {}, offload device: {}".format(
                load_device, offload_device
            )
        )
        ...
        if config is None:
            if "decoder.mid.block_1.mix_factor" in sd:
                ...
                self.first_stage_model = AutoencodingEngine(
                    regularizer_config={
                        "target": "smartdiffusion.ldm.models.autoencoder.DiagonalGaussianRegularizer"
                    },
                    encoder_config={
                        "target": "smartdiffusion.ldm.modules.diffusionmodules.model.Encoder",
                        "params": encoder_config,
                    },
                    decoder_config={
                        "target": "smartdiffusion.ldm.modules.temporal_ae.VideoDecoder",
                ...
                if "quant_conv.weight" in sd:
                    ...
                    self.first_stage_model = AutoencodingEngine(
                        ...
                        decoder_config={
                            "target": "smartdiffusion.ldm.modules.diffusionmodules.model.Decoder",
                            "params": ddconfig,
                        },
                    )
    ...
    if len(clip_data) == 1:
        if "text_model.encoder.layers.30.mlp.fc1.weight" in clip_data[0]:
            ...
            if weight.shape[-1] == 4096:
                clip_target.clip = sd3_clip.sd3_clip(
                    clip_l=False, clip_g=False, t5=True, dtype_t5=dtype_t5
                )
    ...
    if "parameterization" in model_config_params:
        if model_config_params["parameterization"] == "v":
            ...
            class ModelSamplingAdvanced(
                smartdiffusion.model_sampling.ModelSamplingDiscrete,
                smartdiffusion.model_sampling.V_PREDICTION,
            ):
                pass
