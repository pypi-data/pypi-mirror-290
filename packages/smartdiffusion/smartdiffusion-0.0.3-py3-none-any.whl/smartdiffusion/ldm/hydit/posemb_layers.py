...
def get_2d_sincos_pos_embed(embed_dim, start, *args, cls_token=False, extra_tokens=0):
    ...
    # grid_h = arange(grid_size, dtype=float32)
    # grid_w = arange(grid_size, dtype=float32)
    # grid = meshgrid(grid_w, grid_h)  # here w goes first
    # grid = stack(grid, axis=0)   # [2, W, H]
    ...
    if cls_token and extra_tokens > 0:
        pos_embed = concatenate([zeros([extra_tokens, embed_dim]), pos_embed], axis=0)
    return pos_embed
...
def get_2d_sincos_pos_embed_from_grid(embed_dim, grid):
    ...
    emb = concatenate([emb_h, emb_w], axis=1)  # (H*W, D)
    return emb
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    """
    assert embed_dim % 2 == 0
    omega = arange(embed_dim // 2, dtype=float64)
    omega /= embed_dim / 2.0
    omega = 1.0 / 10000**omega  # (D/2,)

    pos = pos.reshape(-1)  # (M,)
    out = einsum("m,d->md", pos, omega)  # (M, D/2), outer product

    emb_sin = sin(out)  # (M, D/2)
    emb_cos = cos(out)  # (M, D/2)

    emb = concatenate([emb_sin, emb_cos], axis=1)  # (M, D)
    return emb
...
def get_1d_rotary_pos_embed(
    dim: int, pos: Union[ndarray, int], theta: float = 10000.0, use_real=False
):
    """
    ...
    Args:
        ...
        pos (ndarray, int): Position indices for the frequency tensor. [S] or scalar
