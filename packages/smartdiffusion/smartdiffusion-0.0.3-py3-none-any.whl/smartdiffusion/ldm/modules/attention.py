...
from smartdiffusion.ldm.modules.sub_quadratic_attention import (
...
class GEGLU(nn.Module):
    def __init__(
        self, dim_in, dim_out, dtype=None, device=None, operations=disable_weight_init
    ):
        super().__init__()
...
def attention_sub_quad(
    query, key, value, heads, mask=None, attn_precision=None, skip_reshape=False
):
    attn_precision = get_attn_precision(attn_precision)
    ...
    mem_free_total, mem_free_torch = model_management.get_free_memory(
        query.device, True
    )
