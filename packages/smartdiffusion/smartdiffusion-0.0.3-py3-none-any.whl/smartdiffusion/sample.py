...
from smartdiffusion.samplers import KSampler, sample
...
def prepare_noise(latent_image, seed, noise_inds=None):
    ...
    unique_inds, inverse = unique(noise_inds, return_inverse=True)
    noises = []
...
def prepare_sampling(model, noise_shape, positive, negative, noise_mask):
    logging.warning(
        "Warning: smartdiffusion.sample.prepare_sampling isn't used anymore and can be removed"
    )
...
def cleanup_additional_models(models):
    logging.warning(
        "Warning: smartdiffusion.sample.cleanup_additional_models isn't used anymore and can be removed"
    )
