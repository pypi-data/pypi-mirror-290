...
class OptimizedAttention(Module):
    def __init__(self, c, nhead, dropout=0.0, dtype=None, device=None, operations=None):
        ...
        self.in_proj = operations.Linear(
        ...
        self.mlp = Sequential(
            OrderedDict(
                [
                    (
                        "c_fc",
                        operations.Linear(dim, dim * 4, dtype=dtype, device=device),
                    ),
                    ("gelu", QuickGELU()),
                    (
                        "c_proj",
                        operations.Linear(dim * 4, dim, dtype=dtype, device=device),
                    ),
                ]
            )
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if union_controlnet_num_control_type is not None:
            ...
            self.transformer_layes = Sequential(
                *[
            ...
            self.spatial_ch_projs = operations.Linear(
            ...
            class ControlAddEmbedding(Module):
                def __init__(
                    ...
                    self.linear_2 = operations.Linear(
                        out_dim, out_dim, dtype=dtype, device=device
                    )
                ...
                def forward(self, control_type, dtype, device):
                    ...
                    c_type = (
                        timestep_embedding(
                            c_type.flatten(), self.in_dim, repeat_only=False
                        )
                        .to(dtype)
    ...
    def union_controlnet_merge(self, hint, control_type, emb, context):
        ...
        for idx in range(min(1, len(control_type))):
            ...
            if idx < len(control_type):
                feat_seq += self.task_embedding[control_type[idx]].to(
    ...
    def forward(self, x, hint, timesteps, context, y=None, **kwargs):
        ...
        if self.control_add_embedding is not None:  # Union Controlnet
            ...
            if any([c >= self.num_control_type for c in control_type]):
                ...
                max_type_name = {v: k for k, v in UNION_CONTROLNET_TYPES.items()}[
                    max_type
                ]
                raise ValueError(
            ...
            if len(control_type) > 0:
                ...
                guided_hint = self.union_controlnet_merge(
                    hint, control_type, emb, context
                )
