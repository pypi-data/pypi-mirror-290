    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "b2": (
                    "FLOAT",
                    {"default": 1.2, "min": 0.0, "max": 10.0, "step": 0.01},
                ),
                "s1": (
                    "FLOAT",
                    {"default": 0.9, "min": 0.0, "max": 10.0, "step": 0.01},
                ),
                "s2": (
                    "FLOAT",
                    {"default": 0.2, "min": 0.0, "max": 10.0, "step": 0.01},
                ),
            }
        }
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "b1": (
                    "FLOAT",
                    {"default": 1.3, "min": 0.0, "max": 10.0, "step": 0.01},
                ),
                "b2": (
                    "FLOAT",
            ...
            if scale is not None:
                ...
                if hsp.device not in on_cpu_devices:
                    try:
                        ...
                        logging.warning(
                            "Device {} does not support the torch.fft functions used"
                            + " in the FreeU node, switching to CPU.".format(hsp.device)
                        )
                        on_cpu_devices[hsp.device] = True
                        hsp = Fourier_filter(hsp.cpu(), threshold=1, scale=scale[1]).to(
                            hsp.device
