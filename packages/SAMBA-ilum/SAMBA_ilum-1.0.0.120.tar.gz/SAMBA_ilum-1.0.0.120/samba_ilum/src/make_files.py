...
#-------------------------------------------------------------------
# Fixando a estrutura dos arquivos POSCAR em coordenadas diretas ---
#-------------------------------------------------------------------

os.rename(dir_files + '/Structures', dir_files + '/Structures_initial')
os.mkdir(dir_files + '/Structures')

#---------------------------------------------------------------------------------
correspondence = open(dir_out + '/correspondence.txt', 'w')
#---------------------------------------------------------------------------------
files = list_files(dir_files + '/Structures_initial')
#----------------------------------------------------

for i in range(len(files)):
    #--------------------------------------------
    Lattice0 = dir_files + '/Structures_initial/'
    Lattice1 = dir_files + '/Structures/'
    poscar = open(Lattice0 + files[i], 'r')
    VTemp0 = poscar.readline(); VTemp1 = VTemp0.split()
    ...
    #--------------------------------------------------------------
    # Copiando os arquivos POSCAR para o diretório 'Structures' ---
    #--------------------------------------------------------------
    structure = Poscar.from_file(Lattice0 + files[i]).structure
    supercell = structure.copy()
    supercell.make_supercell([1,1,1])
    Poscar(supercell).write_file(Lattice1 + files[i])

    #-------------------------------------------------------------------
    with open(Lattice1 + files[i], 'r') as file: line = file.readlines()
    line[0] = VTemp0
    with open(Lattice1 + files[i], 'w') as file: file.writelines(line)
    #-----------------------------------------------------------------
    ...
    if (VTemp1[0] != 'SAMBA'):
       ...
       #-------------------------------------
       poscar = open(Lattice1 + files[i], "r")
       for j in range(5): VTemp = poscar.readline()
       VTemp0 = poscar.readline().split()
       VTemp1 = poscar.readline().split()
       poscar.close()
       #---------------------------
       for j in range(len(VTemp0)):
           id_material += str(VTemp0[j])
           if (str(VTemp1[j]) != '1'): id_material += str(VTemp1[j])
       ...
       with open(Lattice1 + files[i], 'w') as file: file.writelines(line)
       #-----------------------------------------------------------------
       os.rename(Lattice1 + files[i], Lattice1 + id_material)
       correspondence.write(f'{files[i]} >> {id_material} \n')
#---------------------
correspondence.close()
if (VTemp1[0] == 'SAMBA'): os.remove(dir_out + '/correspondence.txt')
#--------------------------------------------------------------------
...
#-----------------------------------------------------
command_terminal('rm ' + dir_files + '/Structures/.*')
#-----------------------------------------------------
    ...
    for j in range(len(task)):
        ...
        if task[j] in ['a-scan', 'z-scan', 'xy-scan', 'xyz-scan']:
           ...
           if (task[j] == 'xy-scan'):
              ...
              #===============================================================
              content = content.replace('replace_displacement_xy_A1', displacement_A1)
              content = content.replace('replace_displacement_xy_A2', displacement_A2)
           if (task[j] == 'xyz-scan'):
              #===============================================================
              ...
              content = content.replace('replace_displacement_xyz_A1', displacement_A1)
              content = content.replace('replace_displacement_xyz_A2', displacement_A2)
           if (task[j] == 'a-scan'):
              #=========================================================================
        ...
        if (task[j][:5] == 'bands'):
           ...
           content = content.replace('replace_n_reg', str(n_materials))
           for m in range(len(label_materials)):
...
if (vdW != 0): replace_type_vdW = str(vdW)
...
#---------------------------------------------------------------------------------
# Excluindo processos para sistemas com n_materials = 1 --------------------------
#---------------------------------------------------------------------------------
for i in range(len(files)):
    #-----------------------------------------------------------------------
    poscar = open(dir_out + '/' + files[i] + '/' + task[0] + '/POSCAR', "r")
...
os.rename(dir_files + '/Structures_initial', dir_files + '/Structures')
