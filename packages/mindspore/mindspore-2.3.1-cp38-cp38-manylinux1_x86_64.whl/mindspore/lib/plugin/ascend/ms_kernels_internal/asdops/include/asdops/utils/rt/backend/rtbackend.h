 ...
 */
#ifndef COMMON_RT_BACKEDN_RTBACKEND_H
#define COMMON_RT_BACKEDN_RTBACKEND_H
...
public:
    int StreamCreate(AsdRtStream *stream, int32_t priority) override;
    int StreamDestroy(AsdRtStream stream) override;
    int StreamSynchronize(AsdRtStream stream) override;
    int StreamGetId(AsdRtStream stream, int32_t *streamId) override;

public:
    int MemMallocDevice(void **devPtr, uint64_t size, AsdRtMemType memType) override;
    ...
    int MemCopy(void *dst, uint64_t dstLen, const void *srcPtr, uint64_t srcLen,
                        AsdRtMemCopyType copyType) override;
    int MemCopyAsync(void *dst, uint64_t dstLen, const void *srcPtr, uint64_t srcLen,
                     AsdRtMemCopyType copyType, void *stream) override;
...
public:
    int ModuleCreate(AsdRtModuleInfo *moduleInfo, AsdRtModule *module) override;
