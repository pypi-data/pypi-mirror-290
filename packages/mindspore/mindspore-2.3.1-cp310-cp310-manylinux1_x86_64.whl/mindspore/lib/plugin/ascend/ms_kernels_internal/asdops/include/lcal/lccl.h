...
public:
    ...
    int Broadcast(const void *buff, int64_t count, HcclDataType dataType, int32_t root, aclrtStream stream = nullptr);

private:
    bool CheckDataType(const HcclDataType &dataType) const;
    int LoopBack(void *sendBuff, void *recvBuff, int64_t count, HcclDataType dataType) const;
