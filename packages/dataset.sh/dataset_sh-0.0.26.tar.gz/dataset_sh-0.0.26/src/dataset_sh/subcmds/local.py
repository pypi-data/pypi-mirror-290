...
@click.option('--version', '-v', help='use this option to select version', default=None)
@click.option('--tag', '-t', help='use this option to select version with tag', default='latest')
@click.option('--model-only', 'model_only', '-m', default=False, help='Force remove dataset without confirmation.',
              is_flag=True)
@click.pass_obj
def print_code(ctx, name, collection, version, tag, model_only):
    """print data model code of a dataset collection"""
    dv = _get_dataset_version(ctx, name, version, tag)
    if dv.exists():
        if model_only:
            code = dv.usage_code(collection)
            click.echo(code)
        else:
            code = dv.usage_code(collection)
            model = dv.model_entry_point(collection)
            reader_code = get_read_data_code(name, collection, version=version, tag=tag, model=model)
            click.echo(code + ' \n\n' + reader_code)
