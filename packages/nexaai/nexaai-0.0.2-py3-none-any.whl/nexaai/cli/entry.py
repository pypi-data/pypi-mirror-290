...
def run_onnx_inference(args):
    ...
    if args.command == "server":
        from nexaai.onnx.server.nexa_service import run_nexa_ai_service as NexaServer
        NexaServer(model_path, **kwargs)
        return
    if args.onnx_command == "pull":
        from nexaai.general import pull_model
        pull_model(model_path)
        return
    elif args.onnx_command == "remove":
        from nexaai.general import remove_model
        remove_model(model_path)
        return
    elif args.onnx_command == "gen-text":
        from nexaai.onnx.nexa_inference_text import \
...
def run_ggml_inference(args):
    ...
    if args.command == "server":
        from nexaai.gguf.server.nexa_service import run_nexa_ai_service as NexaServer
        NexaServer(model_path, **kwargs)
        return
    ...
    if args.command == "gen-text":
        from nexaai.gguf.nexa_inference_text import NexaTextInference
        inference = NexaTextInference(model_path, stop_words=stop_words, **kwargs)
    elif args.command == "gen-image":
        from nexaai.gguf.nexa_inference_image import NexaImageInference
        ...
        from nexaai.gguf.nexa_inference_vlm import NexaVLMInference
...
def main():
    ...
    if args.command == "onnx":
        ...
        from nexaai.general import list_models
        list_models()
    elif args.command == "login":
        from nexaai.general import login
        login()
    elif args.command == "logout":
        from nexaai.general import logout
        logout()
    elif args.command == "whoami":
        from nexaai.general import whoami
        whoami()
    else:
        parser.print_help()
