...
def _determine_lib_folder(lib_folder: Optional[str]) -> str:
    ...
    if is_nexa_metal_installed(): # nexaai-gpu contains metal for macOS, so is_nexa_metal_installed() needs to be checked first
        return "metal"
    elif is_nexa_cuda_installed():
        return "cuda"
    return "cpu"
