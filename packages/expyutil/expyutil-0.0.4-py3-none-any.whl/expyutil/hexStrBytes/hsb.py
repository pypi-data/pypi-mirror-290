...
class hsb:
    ...
    n_bits: int
    """configurable, used to indicate num of bits of output in binString/binBytes convert"""
    n_bytes: int
    """configurable, used to indicate num of bytes of output in Byte convert"""
    ...
    def trans2target(self, bytes_val: bytes, target: hsbValueType):
        ...
        if target == hsbValueType.num:
            ...
            raise Exception("target branch fail")
        return res

    @property
    def value(self):
        """do transfer accroding to cur_type and target_type"""
        res_a = self.trans2b(self._v)
    ...
    def inplace_bitfilp(self, mask:Mask=None):
        """
        进行bit翻转
        """
        # 使用num类型
        self.to_num()
        if mask is None:
            mask = Mask(self.bitlen_8n)
        self._v = self._v ^ mask.val
        return self

    @classmethod
    def split2(cls, string: str|bytes) -> str|bytes:
        """
        输入平均分成两份, 禁止odd len
        """
        l = len(string)
        if l % 2 != 0:
            raise Exception("odd length error")
        return string[:l//2], string[l//2:]
    
    @classmethod
    def split_per4B(cls, string: str|bytes) -> str|bytes:
        """
        输入每4字节进行一次切割, 需要为4字节对齐
        """
        n = 0
        if type(string) == str:
            n = 8
        ...
        if len(string) % n != 0:
            raise Exception("输入不是4字节对齐")
        else:
            import re
            if n == 4:
                res = re.findall(b'....', string)
            else:
                res = re.findall('........', string)
            return res
    
    @classmethod
    def s2hb(cls, string: str):
        return binascii.b2a_hex(bytes(string, encoding='utf-8')).decode()

class hsbm(hsb):
    @classmethod
    def obj_num(cls, *vs):
        return cls().fm_num(*vs)
    ...
    def trans2b_list(self):
        res = []
        for v in self._v:
            res.append(self.trans2b(v))
        return res

    @property
    def value(self):
        """do transfer accroding to cur_type and target_type"""
        res_a = self.trans2b_list()
        res_b = self.trans2target_list(res_a, self.target)
        if len(res_b) == 1:
            return res_b[0]
        return res_b
    
    def trans2target_list(self, val_list, target):
        res = []
        for val in val_list:
            res.append(self.trans2target(val, target))
        return res
