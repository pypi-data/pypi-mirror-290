...
use sz3_codec as _;
use ::{
    bit_round_codec as _, identity_codec as _, linear_quantize_codec as _, log_codec as _,
    reinterpret_codec as _, round_codec as _, uniform_noise_codec as _, zfp_codec as _,
    zlib_codec as _, zstd_codec as _,
};
...
fn configure_cargo_cmd(
    ...
    cmd.arg(format!(
        "CFLAGS=--target=wasm32-wasi -nodefaultlibs -resource-dir {resource_dir} \
         --sysroot={wasi_sysroot} -isystem {clang_include} -isystem {wasi32_wasi_include} \
         -isystem {include} -B {lld} -D_WASI_EMULATED_PROCESS_CLOCKS -O3 --target=wasm32-wasi",
        ...
        wasi32_wasi_include = wasi_sysroot.join("include").join("wasm32-wasi").display(),
        include = wasi_sysroot.join("include").display(),
        lld = lld.display(),
    ));
    cmd.arg(format!(
        "CXXFLAGS=--target=wasm32-wasi -nodefaultlibs -resource-dir {resource_dir} \
         ...
         -D_WASI_EMULATED_PROCESS_CLOCKS -include {cpp_include_path} -O3 --target=wasm32-wasi",
        resource_dir = libclang.join("clang").join("18").display(),
        wasi_sysroot = wasi_sysroot.display(),
    ...
    cmd.arg(format!(
        "BINDGEN_EXTRA_CLANG_ARGS=--target=wasm32-wasi -nodefaultlibs -resource-dir \
    ...
    cmd.arg(format!(
        "RUSTFLAGS=-C strip=symbols -C link-arg=-L{wasm32_wasi_lib}",
        wasm32_wasi_lib = wasi_sysroot.join("lib").join("wasm32-wasi").display(),
    ));
    cmd.arg(format!(
