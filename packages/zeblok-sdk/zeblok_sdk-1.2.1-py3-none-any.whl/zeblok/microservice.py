...
class MicroService(ServiceSpawnBase):
    ...
    def __get(
        ...
        if resource_id is None:
            ...
            self.__basic_id_validation(resource_id=resource_id.strip())
            response = requests.get(
    ...
    def validate_id(self, microservice_id: str, raise_exception: bool = False):
        ...
        return self.__get(
            ...
            error_message=NO_MICROSERVICES_ID.format(microservice_id=microservice_id.strip())
        )
    ...
    def get_by_id(self, microservice_id: str, print_stdout: bool = True) -> Dict:
        ...
        err_msg = NO_MICROSERVICES_ID.format(microservice_id=microservice_id.strip())
    ...
    def spawn(
        ...
        if response.status_code == 201 and response.json()['success']:
            ...
            if response.json().get('data', None) is not None:
                microservice_url = response.json()['data']['virtualServiceUrl'] if response.json()['data'].get(
                    'virtualServiceUrl', None) is not None else None

                service_name = response.json()['data']['service'] if response.json()['data'].get(
                    'service', None) is not None else None

            if microservice_url is None:
                api_error(status_code=500, message="Unable to fetch the Microservice URL")
            ...
            microservice_details = f"MicroService URL: {microservice_url}, Service Name: {service_name}"

            print(f"{msg} {microservice_details}")

            return microservice_url, service_name
        else:
