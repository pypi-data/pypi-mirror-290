...
class HostnameParser(BaseParser):
    ...
    def parse(self) -> List[IpInfo]:
        ip_list = []
        if self.is_valid:
            ip_list.extend(IpInfo(ip, self.port) for ip in get_resolve_ips(self.source, self.port, socket.AF_INET))
            ip_list.extend(IpInfo(ip, self.port) for ip in get_resolve_ips(self.source, self.port, socket.AF_INET6))
        return ip_list
