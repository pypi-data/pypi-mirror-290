...
SAMPLE_NOTEBOOK = open(
    os.path.join(os.path.dirname(__file__), "fixtures/sample_notebook.ipynb")
).read()
SAMPLE_NEW_NOTEBOOK = open(
    os.path.join(os.path.dirname(__file__), "fixtures/sample_new_notebook.ipynb")
).read()


class NotebookWithoutNewFlag(nbformat.NotebookNode):
    """Helper that tests if a NotebookNode has
    the metadata.civis.new_notebook flag set to True"""
...
class PlatformPersistenceTest(unittest.TestCase):
    ...
    def test_initialize_notebook_will_get_nb_from_platform(
        ...
        rg.return_value = MagicMock(
            spec=requests.Response, status_code=200, content=SAMPLE_NOTEBOOK
        )
    ...
    @patch("civis.APIClient")
    @patch("civis_jupyter_notebooks.platform_persistence.requests.get")
    def test_initialize_notebook_will_pull_nb_from_url(
        self, rg, _client, requirements, _op, _makedirs
    ):
        url = "http://whatever"
        rg.return_value = MagicMock(
            spec=requests.Response, status_code=200, content=SAMPLE_NOTEBOOK
        ...
        platform_persistence.get_client().notebooks.get.return_value.requirements_url = (  # noqa: E501
            None
        )
        platform_persistence.initialize_notebook_from_platform(TEST_NOTEBOOK_PATH)
    ...
    def test_initialize_notebook_will_throw_error_on_nb_pull(
        ...
        rg.return_value = MagicMock(
            spec=requests.Response, status_code=500, response={}
        )
        self.assertRaises(
    ...
    def test_initialize_notebook_will_set_new_notebook_flag_to_false(
        ...
        rg.return_value = MagicMock(
            spec=requests.Response, status_code=200, content=SAMPLE_NEW_NOTEBOOK
        )
        platform_persistence.get_client().notebooks.get.return_value.notebooks_url = (
            "something"
        )
        platform_persistence.get_client().notebooks.get.return_value.requirements_url = (  # noqa: E501
            None
    ...
    @patch("civis.APIClient")
    @patch("civis_jupyter_notebooks.platform_persistence.requests.get")
    def test_initialize_notebook_will_use_s3_notebook_if_not_new_and_git_notebook_exists(  # noqa: E501
        self, rg, _client, _op, _makedirs, nbwrite, isfile
    ):
        rg.return_value = MagicMock(
            spec=requests.Response, status_code=200, content=SAMPLE_NOTEBOOK
        )
    ...
    @patch("civis.APIClient")
    @patch("civis_jupyter_notebooks.platform_persistence.requests.get")
    def test_initialize_notebook_will_discard_s3_notebook_if_new_and_git_notebook_exists(  # noqa: E501
        self, rg, _client, _op, _makedirs, nbwrite, isfile
    ):
        rg.return_value = MagicMock(
            spec=requests.Response, status_code=200, content=SAMPLE_NEW_NOTEBOOK
    ...
    def test_post_save_fetches_urls_from_api(self, _rput, client, _ccc, _op):
        ...
        platform_persistence.get_client().notebooks.list_update_links.assert_called_with(  # noqa: E501
            TEST_PLATFORM_OBJECT_ID
        )
    ...
    def test_post_save_performs_two_put_operations(
    ...
    def test_post_save_skipped_for_non_notebook_types(
        self, guu, save, _rput, _client, _ccc, _op
    ):
    ...
    @patch("civis.APIClient")
    @patch("requests.put")
    def test_generate_preview_throws_error_on_convert(
        self, _rput, _client, check_call, _op
    ):
        check_call.side_effect = subprocess.CalledProcessError("foo", 255)
        self.assertRaises(
            NotebookManagementError,
            lambda: platform_persistence.generate_and_save_preview(
                "http://notebook_url_in_s3", "os/path"
            ),
        )
        check_call.assert_called_with(
            ["jupyter", "nbconvert", "--to", "html", "path"], cwd="os"
        )
    ...
    def test_find_and_install_requirements_calls_pip_install(
        self, pip_install, isdir, isfile
    ):
    ...
    @patch("os.path.isdir")
    @patch("civis_jupyter_notebooks.platform_persistence.pip_install")
    def test_find_and_install_requirements_searches_tree(
        self, pip_install, isdir, isfile
    ):
        os.path.isdir.return_value = True
    ...
    @patch("os.path.isdir")
    @patch("civis_jupyter_notebooks.platform_persistence.pip_install")
    def test_find_and_install_requirements_excludes_root(
        self, pip_install, isdir, isfile
    ):
        os.path.isdir.return_value = True
        os.path.isfile.return_value = True
    ...
    @patch("subprocess.check_output")
    @patch("sys.executable")
    def test_pip_install_failure_raises_notebookmanagementerror(
        self, executable, check_output
    ):
        check_output.side_effect = subprocess.CalledProcessError(
            returncode=1, cmd="cmd", output=b"installation error"
        )
