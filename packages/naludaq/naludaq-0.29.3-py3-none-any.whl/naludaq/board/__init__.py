...
class Board:
    ...
    def __init__(
        ...
        triggers_available = self.params.get("trigger", {}).get(
            "triggers_available", channels
        )
        self.trigger_offsets = [0 for _ in range(triggers_available)]
        self._trigger_values = [0 for _ in range(triggers_available)]

        # Connection
        self._using_new_backend = False
        self._last_server_address: tuple[str, int] = None
        self.connection_info: dict = None
        self.connection = None
        self.serialmode = False
        self.fCRC = False
        self.loopback = True
    ...
    def trigger_values(self):
        ...
        The expected length when setting the trigger values must be
        equal to the `triggers_available` field found in the YAML, or
        the number of channels if said field is unavailable.

        Raises:
            TypeError if trigger values aren't a list or if values
                aren't all integers.
            ValueError if length of the input doesn't match the
                expected length for the board model.
        """
        return self._trigger_values

    @trigger_values.setter
    def trigger_values(self, value):
        trigger_params = self.params.get("trigger", {})
        triggers_available = trigger_params.get("triggers_available", self.channels)

        if not isinstance(value, list):
            raise TypeError("Value not a list")
        if len(value) != triggers_available:
            raise ValueError("Invalid length of trigger values")
        if not all([isinstance(x, int) for x in value]):
            raise TypeError("Not all values are an integer")
        self._trigger_values = value
...
def startup_board(board: Board, init_start=True, stat: list = []) -> bool:
    ...
    if (
        board.params["model"] in ["upac32", "upaci", "zdigitizer"] and is_uart
    ):  # , "hdsocv1", "hdsocv1_evalr1", "hdsocv1_evalr2"]
        # init_board
        if init_hardware(board, init_start):
    ...
    cc = get_connection_controller(board)
    ...
    if not init_hardware(board, init_start):
        status = "Hardware NOT initialized."
        stat.append((0, status))
        LOGGER.debug(status)
        board.connection = None
