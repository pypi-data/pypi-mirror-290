...
class MySqlSingleton1Wrapper(CommonSingleton1Wrapper, MySqlWrapper):
    """
    ...
    def _build_result_handler(self, context: WrapperContext) -> None:
        ...
        if context.pystratum_metadata['pydoc']['return'] == ['bool']:
            statement = f"return self.execute_sp_singleton1({sql}) not in [None, '']"
        else:
            statement = f'return self.execute_sp_singleton1({sql})'
