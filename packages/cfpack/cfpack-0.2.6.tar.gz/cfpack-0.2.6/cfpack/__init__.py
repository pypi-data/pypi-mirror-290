...
def print(*args, error=False, warn=False, highlight=False, color="", input=False, no_prefix=False, **kwargs):
    ...
    builtin_print("", flush=True) # print final line ending, and flush output
...
def plot(y=None, x=None, yerr=None, xerr=None, type=None, xlabel='x', ylabel='y', label=None,
         ...
         xlog=False, ylog=False, xlim=None, ylim=None, legend_loc='upper left',
         axes_format=[None,None], axes_pos=None, show=False, pause=None, save=None, *args, **kwargs):
        ...
        if text is not None:
            ...
            if type == 'pdf' or type == 'histogram' or type == 'hist' or type == 'bars':
                ...
                if type == 'bars':
                    ...
                    plt.bar(x, height=y, width=width, label=label, *args, **kwargs)
                else:
                    plt.stairs(y, edges=edges, label=label, *args, **kwargs)
            if type == 'scatter':
                linestyle = "None"
            ...
            if shaded_err is not None: # yerr must have shape (2,N), containing the lower and upper errors for each of the N data points
                if isinstance(shaded_err, list): # user provided a list with [color, alpha]
                    ...
                    alpha_err = 0.5
                plt.fill_between(x, y-np.abs(yerr[0]), y+np.abs(yerr[1]), color=color_err, alpha=alpha_err, linewidth=0.0, label=label)
    if show or save:
    ...
    ret = show_or_save_plot(plt, ax, show=show, pause=pause, save=save) # now show or save the finished figure, and destroy it afterwards
    return ret
# === END plot ===

# === START plot_map ===
# function to plot a map (of a 2D numpy array)
def plot_map(image=None, xedges=None, yedges=None, dims=None, vmin=None, vmax=None, log=False, colorbar=True,
             cmap='magma', cmap_label=None, xlabel=None, ylabel=None, xlog=False, ylog=False, xlim=None, ylim=None,
             axes_format=[None,None], axes_pos=None, aspect_data='auto', aspect_box=None, dpi=200,
             show=False, pause=None, save=None, *args, **kwargs):
    import matplotlib.pyplot as plt
...
def plot_colorbar(cmap=None, label=None, norm=None, vmin=None, vmax=None, log=False, format=None, aspect=28,
                  extend="neither", panels=1, orientation="vertical", swap=False, show=False, pause=None, save=None):
    import matplotlib.pyplot as plt
    ...
    if norm is None:
        if not log: # linear color scale
            norm = colors.Normalize(vmin=vmin, vmax=vmax)
        else: # log color scale
    ...
    if orientation == "horizontal": # flip width and height for horizontal colorbar
        ...
        xpos = 0.05
        ypos = 0.50
    cbax = plt.gcf().add_axes([xpos, ypos, width, height]) # colorbar axis
    cb = mpl_cbar.ColorbarBase(cbax, cmap=cmap, norm=norm, orientation=orientation, extend=extend, label=label) # draw colorbar
    ...
    ret = show_or_save_plot(plt, cbax, show=show, pause=pause, save=save) # now show or save the finished figure, and destroy it afterwards
    return ret
#=== END plot_colorbar ===
...
def plot_multi_panel(figure_axes, nrows=1, ncols=1, width_panel=0.8, height_panel=0.8, col_sep=0.15, row_sep=0.15,
    ...
    ret = show_or_save_plot(plt, plt.gca(), show=show, pause=pause, save=save) # now show or save the finished figure, and destroy it afterwards
    return ret
#=== END plot_multi_panel ===

# === START show_or_save_plot ===
# internal plot helper function
def show_or_save_plot(plt, figax=None, show=None, pause=None, save=None):
...
def get_moments_from_time_series(time, func, ts=None, te=None):
    ...
    tl = time[ind]
    ret = {"mean": np.nan, "std": np.nan, "skew": 0.0, "kurt": 0.0} # init return dict
    fl = func[ind]
