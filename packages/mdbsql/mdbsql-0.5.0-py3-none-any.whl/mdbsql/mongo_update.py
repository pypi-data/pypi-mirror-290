...
def upload_one_dir():
    ...
    p.db_name = '京东数据1'
    p.collection_name = f'京东_流量来源_日数据'
...
def main():
    username, password, host, port = get_myconf.select_config_values(target_service='home_lx', database='mongodb')
    m = OptimizeDatas(username=username, password=password, host=host, port=port)
    m.db_name_lists = [
        '京东数据1',
        '天猫数据1',
        '市场数据1',
...
if __name__ == '__main__':
    ...
    # username, password, host, port = get_myconf.select_config_values(target_service='home_lx', database='mongodb')
    # m = UploadMongo(username=username, password=password, host=host, port=port)
    #
    # path = '/Users/xigua/数据中心/原始文件2/指数转换/指数转换_非转化_xigua20223.csv'
    # df = pd.read_csv(path, encoding='utf-8_sig', header=0, na_filter=False)
    # print(df)
    # # m.df_to_mongo(df=df, db_name='属性设置1', collection_name='指数转换_非转化指数')
