...
import platform
import json
from mdbsql import mongo_update
from mdbsql import mysql_update
...
class DatabaseUpdate:
    def __init__(self, path):
        ...
        self.up_to_mogo: bool = True  # 不设置则不上传 mongodb
        self.up_to_mysql: bool = True  # 不设置则不上传 mysql
    ...
    def cleaning(self, is_move=True):
        ...
        for root, dirs, files in os.walk(self.path, topdown=False):
            for name in files:
                if '~$' in name or '.DS' in name or '.localized' in name or '.ini' in name:
                    continue
                # 只针对 csv, xlsx 文件进行处理
                    ...
                    if tg_name in name and '报表汇总' not in name and name.endswith('.csv'):  # 排除达摩盘报表: 人群报表汇总
                        ...
                        if '地域' not in name:  # 除了地域报表, 检查数据的字段是否包含“场景名字”,如果没有,说明没有选“pbix” 模块
                            ...
                            if '场景名字' not in ck:
                                ...
                                continue
                        if len(df) == 0:
                            print(f'1.3.0 {name} 报表是空的, 请重新下载, 此报表已移除')
                            continue
                        cols = df.columns.tolist()
                        if '日期' not in cols:
...
def upload():
    path = 'C:\\同步空间\\BaiduSyncdisk\\原始文件2\\生意经\\宝贝指标'
