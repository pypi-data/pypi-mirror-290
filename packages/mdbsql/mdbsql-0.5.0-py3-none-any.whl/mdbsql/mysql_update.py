...
class MysqlUpload:
    ...
    def df_to_mysql(self, df, tabel_name, db_name='远程数据源'):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                # 转换为 mysql 的数据类型
                dtypes.update({col: self.convert_dtype_to_sql(col=col, dtype=dtypes[col]) for col in cols})
    ...
    def convert_dtype_to_sql(self, col, dtype):
    ...
    def convert_df_cols(self, df):
        ...
        for col in cols:
            ...
            df[col] = df[col].apply(lambda x: float(float((str(x).rstrip("%"))) / 100) if str(x).endswith('%') else x)
            # 尝试转换合适的数据类型
            ...
            if df[col].dtype == 'float':  # 对于小数类型, 保留 6 位小数
                df[col] = df[col].apply(lambda x: round(float(x), 6) if x else x)
...
if __name__ == '__main__':
    ...
    username, password, host, port = get_myconf.select_config_values(target_service='home_lx', database='mysql')
    print(username, password, host, port)
    # o = OptimizeDatas(username=username, password=password, host=host, port=port)
    # o.db_name = '生意经1'
    # o.days = 10
    # o.optimize()
    ...
    #         path = os.path.join('C:\\同步空间\\BaiduSyncdisk\\原始文件2', '推广报表\\旧版报表')
    #     elif platform.system() == 'Darwin':
    #         import getpass
    #
    #         path = os.path.join('/Users', getpass.getuser(), '数据中心/原始文件2', '生意经/店铺指标')
    #     else:
    #         path = ''
    #     username, password, host, port = get_myconf.select_config_values(target_service='home_lx', database='mysql')
    #     now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S: ')
    #     print(f'{now}{host}:{port} starting...')
    #     m = MysqlUpload(username=username, password=password, host=host, port=port)
    #     results = []
    #     for root, dirs, files in os.walk(path, topdown=False):
    #         for name in files:
    #             if '2024-08' not in name:
    #                 continue
    #             if name.endswith('.csv') and 'baidu' not in name:
    #                 df = pd.read_csv(os.path.join(root, name), encoding='utf-8_sig', header=0, na_filter=False)
    #                 results.append(df)
