...
import time
import re
from mdbsql import mysql_update as mysql
from mdbsql import get_myconf
from mdbsql import bdup2
from mdbsql import database_update
...
class TbFiles:
    ...
    def tb_file(self):
        ...
        # mysql  家里服务器
        username, password, host, port = get_myconf.select_config_values(target_service='aliyun', database='mysql')
        m = mysql.MysqlUpload(username=username, password=password, host=host, port=port)
        m.upload_pandas(
            update_path=os.path.join(self.data_path, 'pandas数据源'),
            db_name='pandas数据源',
            days=32,  # 更新近期数据, 注释掉则全部更新
        )
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S ')
        print(f'{now}数据上传阿里云完成')
...
def main():
    ...
    while True:
        ...
        if res:
            ...
            dp = database_update.DatabaseUpdate(path=d_path)
            dp.up_to_mogo = False  # 公司台式机不需要 mongodb
            dp.up_to_mysql = True
