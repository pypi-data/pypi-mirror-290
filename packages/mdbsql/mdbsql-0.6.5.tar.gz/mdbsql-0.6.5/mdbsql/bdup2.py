...
class BaiDu:
    ...
    def __init__(self):
        ...
        self.skip = ['.DS_Store', '.localized', 'desktop.ini']
        self.delete_remote_files = ['.DS_Store', '.localized', 'desktop.ini']
        self.bp = ByPy()
    ...
    def upload_dir(self, local_path, remote_path):
        ...
        if platform.system() == 'Windows':
            self.bp.upload(localpath=self.local_path, remotepath=self.remote_path)  # 上传文件到百度云
        else:
    ...
    def upload_file(self, local_path, remote_path):
        ...
        for file in files:
            if os.path.isfile(os.path.join(self.local_path, file)):
                ...
                rt_path = os.path.join(self.remote_path, file)
                print(f'上传文件: {file} -> {rt_path}')
                ...
                for root, dirs, files in os.walk(os.path.join(self.local_path, file), topdown=False):
                    for name in files:
                        ...
                        rt_path = lc_path.replace(self.local_path, self.remote_path)
                        print(f'上传文件: {name} -> {rt_path}')
                        self.bp.upload(localpath=lc_path, remotepath=rt_path)  # 上传文件到百度云

    def delete_files(self):
        """ 移除云端文件，位于 self.remote_path 文件夹下的子文件 """
        for delete_file in self.delete_remote_files:
            self.bp.remove(remotepath=f'{self.remote_path}/{delete_file}')  # 移除文件
    ...
    def download_dir(self, local_path, remote_path):
        ...
        self.bp.download(localpath=f'{self.local_path}', remotepath=f'{self.remote_path}')
