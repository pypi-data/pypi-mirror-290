...
class GenerationParams(BaseModel, extra=Extra.allow, title=''):
    ...
    reward_max_token_input: Optional[int] = Field(
        default=None,
        title="Maximum reward input tokens",
        description="The maximum tokens which are fed into the reward model.")
    ...
    def validate_currently_supported_format(self):
        ...
        self._validate_reward_max_tokens()
    ...
    def _validate_reward_max_tokens(self):
        max_allowed = 1024
        min_allowed = 128

        is_not_none = self.reward_max_token_input is not None
        is_not_in_correct_range = not (min_allowed <= self.reward_max_token_input <= max_allowed)

        if is_not_none and is_not_in_correct_range:
            msg = f"`reward_max_token_input` must be in [{min_allowed}, {max_allowed}], got {self.reward_max_token_input}"
            self._validation_errors.append(msg)
...
class FrontEndSubmissionRequest(BaseModel):
    ...
    def check_generation_params(cls, generation_params, values):
        cls._set_request_defaults(cls, generation_params)
        if values["validate_params"]:
            generation_params.validate_currently_supported_format()
        return generation_params

    @staticmethod
    def _set_request_defaults(cls, generation_params):
        """
        Only set these defaults for new requests and not for existing parameters in
        order to preserve backwards compatibility when new fields are added.
        """
        if generation_params.reward_max_token_input is None:
            generation_params.reward_max_token_input = config.DEFAULT_REWARD_MAX_TOKENS
...
def _serialize_functions(model_submission):
    for key in ["function", "tagging_function"]:
        function = model_submission.get(key)
        if function:
            model_submission[key] = serialize_function(function)
