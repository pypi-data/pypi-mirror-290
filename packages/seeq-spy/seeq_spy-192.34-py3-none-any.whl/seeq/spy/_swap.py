...
def swap(items: pd.DataFrame, assets: pd.DataFrame, *, partial_swaps_ok: bool = False,
    ...
    assets : {pd.DataFrame}
        ...
        If a calculation depends on multiple assets where the immediate
        ...
        specifying that higher level, and spy.swap() will automatically
        figure out the actual lower-level items that are appropriate to swap.
    ...
    row_index = 0
    status.df = pd.DataFrame({'Original ID': pd.Series(dtype='str'),
    ...
    for swap_group in swap_groups:
        ...
        for item_dependency_output, dependencies_with_relevant_assets in item_list:
            ...
            status.df = pd.concat([status.df, staged_swap], ignore_index=True)
