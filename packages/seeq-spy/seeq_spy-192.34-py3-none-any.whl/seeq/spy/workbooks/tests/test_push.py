    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        try:
            ...
            _assert_explanation(
                explanation,
                """
                {...}CalculatedSignal "A Calculated Signal" ({...}) formula parameter $a={...} not found{...}
                """
            )
...
def test_datasource_map_by_id_scoped_stored_item_to_global():
    ...
    # Map from a scoped StoredSignal in `Example Analysis (D833DC83-9A38-48DE-BF45-EB787E9E8375)`'s items.json
...
def test_invalid_ui_config():
    ...
    assert pulled_signal['UIConfig'] == valid_ui_config
...
def test_round_trip_metrics_with_custom_colors():
    ...
    _assert_metric_thresholds_match_expected()
...
def test_push_local_asset_tree():
    ...
    sorted_inventory_2 = sorted(workbook_2.item_inventory.values(), key=lambda x: x.fqn)
    assert [item['ID'] for item in sorted_inventory_1] != [item['ID'] for item in sorted_inventory_2]
    assert pushed_2_workbook_id in sorted_inventory_2[1]['Data ID']
