        ...
        try:
            ...
            for workbook in workbooks:  # type: Workbook
                ...
                if util.safe_exists(workbook_folder):
                    if overwrite:
                        util.safe_rmtree(workbook_folder)
                    else:
                        raise SPyRuntimeError(
                            f'Folder "{workbook_folder}" already exists. Use overwrite=True to overwrite.')
            ...
            if zip_it:
                if util.safe_exists(folder_or_zipfile):
                    if overwrite:
                        util.safe_remove(folder_or_zipfile)
                    else:
                        raise SPyRuntimeError(
                            f'Zip file "{folder_or_zipfile}" already exists. Use overwrite=True to overwrite.')
