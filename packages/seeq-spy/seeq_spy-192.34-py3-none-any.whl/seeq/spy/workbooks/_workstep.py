...
class ExistingWorkstepWalk:
    ...
    def __init__(self, workbook_id: str, worksheet_output: WorksheetOutputV1):
        ...
        self.map = dict()
        self.previous_workstep_id = worksheet_output.workstep
        self.count = 0

    def lookup(self, session: Session, workstep):
        while True:
...
class Workstep(Item):
    ...
    def push_to_specific_worksheet(self, session: Session, pushed_workbook_id, pushed_worksheet_output, item_map,
                                   include_inventory, *, existing_workstep_walk: Optional[ExistingWorkstepWalk] = None,
        ...
        existing_workstep = existing_workstep_walk.lookup(session, workstep_to_push) if existing_workstep_walk else None
    ...
    def referenced_items(self):
        ...
        for match in matches:
            referenced_items.append(Reference(match.group(0), Reference.DETAILS, self.worksheet))
