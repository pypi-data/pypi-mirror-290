...
def save_to_csv(
    ...
    None
    """
    # Determine if the resample base period should be included
    if (
        include_resample_base_period
        and original_period != download_period
        and original_period > download_period
    ):
        filename = f"{stock_symbol}_historical_{start_date}_{end_date}_{original_period}_from_{download_period}.csv"
    else:
        filename = (
            f"{stock_symbol}_historical_{start_date}_{end_date}_{download_period}.csv"
        )

    # Dynamically determine the path of the sttk library
    sttk_spec = importlib.util.find_spec("sttk")
    if sttk_spec is None or sttk_spec.origin is None:
        raise ModuleNotFoundError("sttk module not found")

    # Get the directory containing the sttk library
    sttk_dir = os.path.dirname(sttk_spec.origin)

    # Define the path for the sttk-data directory at the same level as sttk
    data_dir = os.path.join(os.path.dirname(sttk_dir), "sttk-data")
...
if __name__ == "__main__":
    download_and_resample_yf_data(
        ...
        include_resample_base_period=True,
    )
