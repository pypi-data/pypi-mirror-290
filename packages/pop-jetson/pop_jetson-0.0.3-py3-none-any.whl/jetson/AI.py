...
class Object_Follow():
    ...
    def __init__(self, camera=None, label_list=None):
        ...
        try:
            ...
            logging.warning("PyCUDA doesn't exist !!!")
            logging.info("Trying to install PyCUDA now ...")
            logging.info("This may take a while ...")
            import subprocess
            result = subprocess.run(['ping', '-c', '1', '8.8.8.8'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            if result.returncode == 0:
                result = subprocess.run('pip install pycuda --user', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                if result.returncode == 0:                    
                    logging.info("PyCUDA is successfully installed.")
                    logging.info("Restarting the kernel now.")
                    time.sleep(1)
                    pid = os.getpid()
                    kill_command = f"kill -9 {pid}"
                    os.system(kill_command)
                else:
                    raise logging.error(result.stderr.decode())
            else:
                raise logging.fatal("Internet is not connected. Please check the connection.")
    ...
    def generate_trt(self, model_path=None):
        ...
        from .converter import yolo_to_onnx
        from .converter import onnx_to_trt
        ...
        if model_path is None:
            ...
            async def main():
                urls = [
                    ('https://raw.githubusercontent.com/AlexeyAB/darknet/master/cfg/yolov4-tiny.cfg', 'yolov4-tiny.cfg'),
                    ('https://github.com/AlexeyAB/darknet/releases/download/darknet_yolo_v4_pre/yolov4-tiny.weights', 'yolov4-tiny.weights'), 
                ]
                
                async with aiohttp.ClientSession() as session:
                    tasks = [download_file(session, url, file_name) for url, file_name in urls]
                    await asyncio.gather(*tasks)

            asyncio.run(main())
            yolo_to_onnx.convert('yolov4-tiny')
            onnx_to_trt.convert('yolov4-tiny')
            os.chdir(ori_dir)
        else:
            yolo_to_onnx.convert(model_path)
            onnx_to_trt.convert(model_path)
...
class PoseNet():
    def __init__(self,camera=None):
        ...
        from jetson.torch2trt.parse_objects import ParseObjects
    ...
    def torch2trt(self, path=None):
        ...
        import jetson.torch2trt 
        from jetson.torch2trt import models
        ...
        self.model = jetson.torch2trt.torch2trt(model, [data], fp16_mode=True, max_workspace_size=1<<25)
        
        path = path.replace('.pth', '_trt.pth')
    ...
    def load_model(self, path=None):
        ...
        from jetson.torch2trt import trt_module
