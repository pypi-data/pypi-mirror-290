...
def get_config():
    config = ConfigParser()
    # Read config file from (in order) bundled config, XDG_CONFIG_HOME, HOME, or parent folder.
    ...
    config.read(
        [
            conf,
            Path(os.environ.get("XDG_CONFIG_HOME", "~/.config")).expanduser() / "colorls/colorls.toml",
