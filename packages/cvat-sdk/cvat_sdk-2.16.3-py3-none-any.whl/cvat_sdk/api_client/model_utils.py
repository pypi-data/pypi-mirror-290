...
def to_json(obj, *, read_files: bool = True):
    """
    ...
    if isinstance(obj, (ModelNormal, ModelComposed)):
        return {
            key: to_json(val, read_files=read_files)
            for key, val in model_to_dict(obj, serialize=True).items()
        }
    elif isinstance(obj, io.IOBase):
        if read_files:
            return get_file_data_and_close_file(obj)
        else:
            return obj
        ...
        return to_json(obj.value, read_files=read_files)
    elif isinstance(obj, (list, tuple)):
        return [to_json(item, read_files=read_files) for item in obj]
...
def get_file_data_and_close_file(file_instance: file_type) -> bytes:
