    ...
    def _post(self, *, messages: list = None) -> dict:
        ...
        while attempt != self.max_attempts:
            ...
            # HTTP error handling
                ...
                t_regex = r'Limit (\d*).*?Used (\d*).*?Requested (\d*).*?Please try again in (\d*\.?\d*)(s|ms)?'
                ...
                if (error_type == 'tokens' and error_code == 'rate_limit_exceeded') or (error_type == 'requests' and error_code == 'rate_limit_exceeded'):
                    ...
                    limit_wait = limit_wait_number if limit_wait_unit == 's' else limit_wait_number / 1000
                    ...
                    if limit_requested > limit_max:
                        raise Exception('Request of \'{limit_requested}\' exceeds the maximum limit of \'{limit_max}\'')
