...
class BaseJobExecutor(base.Base):
    ...
    def html_modules(self):
        return web.div("Modules: TODO")

    def page(self, params={}):
        page_name = params.get(getattr(self, 'type', 'page'), 'index')
        page = getattr(self, 'page_' + page_name, self.page_index)
        return page(params)
...
class LazyDispatcher(BaseJobExecutor):
    ...
    def write_to(self, p: base.Page, summary=False):
        ...
        for key in self.instances:
            p('<div class="runner_preview part_box">')
            p.h3("Runner: " + key)
    ...
    def page(self, params={}):
        p = base.Page(owner=self)
        opts = {'theme': params.get('theme', 'light')}
        if 'type' in params:
...
class JobRunner(base.Base):
    ...
    def __init__(self, jobexecutor, args={}):
        super().__init__()
    ...
    def page_registry_show(self, params={}):
        ...
        try:
            ...
            if 'pip' in params:
                p(self.registry_install(params, getattr(runner, 'MODULES', [])))
            p.div("Module: " + ','.join(getattr(runner, 'MODULES', [])) + web.a("pip install", self.link_registry_show(params['p_name'])+'&pip=install'))
            p.div("Constructor")
            ...
            p.div(web.a("Load", install))
        except Exception as e:
            p.div("Error: " + str(e))
        return p.nxui()

    def registry_install(self, params, modules):
...
class SerialExecutor(ThreadedQueueExecutor):
    ...
    def page_ctrl(self, params={}):
        ...
        p.pre("", id='buffer')
        p.hr()
        p.prop("Baudrate", "9600, 14400, 19200, 115200, 250000")
    ...
    def write_to(self, p: base.Page, summary=False):
        c = "False" if self.is_connected() else f"True (Buffer:{len(self.buffer)})"
...
class GCodeExecutor(ThreadedQueueExecutor):
    ...
    def page(self, params={}):
        page = getattr(self, 'page_' + params.get(self.type, 'index'), None)
        if page is not None:
            return page(params)
        else:
            return "404, no page() in Runner"
...
class ApiExecutor(BaseJobExecutor):
    ...
    def execute(self, data={}):
        if 'api_key' in data:
            ...
            return {
                'success': False,
                'message': 'API-Key (api_key) Required. No Key found.'
            }
...
def run(args:util.Args, executor=None):
    ...
    if args.hasFlag('install'):
        ...
        runner = JobRunner(jobrunner, args)
