...
class NWebGroup:
    ...
    def create_doc(self, name, content, kind='markup', **data):
        return self.__client.createDoc(name, content, self.__data['group_id'], kind='markup', **data)
...
class NWebDoc:
    ...
    def as_image(self):
        ...
        return Image.open(io.BytesIO(self.getThumbnail('cs')))
...
class NWebClient:
    ...
    def reqJson(self, path, params = {}):
        return json.loads(self.req(path, params))
    ...
    def createDoc(self, name, content, group_id, kind='markup', **data):
        """ Return: JSON """
...
def metric_val(baseUrl, metricName, val):
    ...
    requests.get(url=baseUrl, params={'metric': metricName, 'val': cdval})
