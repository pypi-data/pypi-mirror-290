...
class GCode:
    ...
    def get_pos(self):
        for instruction in self.instructions:
            if instruction.is_move():
                self.x = instruction.get('X', self.x)
                self.y = instruction.get('Y', self.y)
                self.z = instruction.get('Z', self.z)
    ...
    def get_layer_count(self):
        ...
        layer = 0
        for instruction in self.instructions:
            if instruction.is_comment() and instruction.startswith(';LAYER:'):
                layer = str(instruction)[7:]
        return 0

    def transform(self, transformer):
        for instruction in self.instructions:
            instruction.transform(transformer)

    def max(self, component):
        res = 0
        for instruction in self.instructions:
            v = instruction.get(component)
            if v is not None:
                res = max(res, float(v))
        return res

    def min(self, component):
        res = 0
        for instruction in self.instructions:
            v = instruction.get(component)
            if v is not None:
        ...
        for instruction in self.instructions:
            if instruction.is_move():
                ...
                if x is not None and y is not None:
                    path += ' L ' + str(x) + ' ' + str(y)
        line = '<path d="' + path.strip() + '" style="fill:none;stroke:green;stroke-width:3"/>'
        return line

    def to_svg(self):
        return '<svg width="160" height="160">'+self.to_svg_path()+'</svg>'
...
class RelMap:
    ...
    def __call__(self, *args, **kwargs):
        if args[0][0] == self.component:
            v = args[0][:1]
            return self.component + str(self.value + float(v))
        return args[0]
...
class Instruction:
    """
      ...
      @see https://marlinfw.org/docs/gcode/M205.html
    """
    line: str = ''
    args = []
    ...
    def relTransform(self, component, value):
        self.args = map(RelMap(component, value), self.args)

    def transform(self, component, transformer):
        self.args = map(transformer, self.args)

    def is_comment(self):
        return self.line[0] == ';'

    def is_move(self):
        return self.line.startswith("G0") or self.line.startswith("G1")
    ...
    def remove_component(self, component):
        v = self.get(component)
        self.args.remove(v)

    def update_pos(self, gcode_str: str):
        g = Instruction(gcode_str)
        for c in ['X', 'Y', 'Z']:
            if self.contains_component(c) and g.contains_component(c):
                self.remove_component(c)
                self.args.append(c+str(g.get(c)))
            elif g.contains_component(c):
                self.args.append(c + str(g.get(c)))

    def duration(self):
        acceleration = 3000
        lastf = 0
        f = float(self.get('F', 3000))/60 # Feedrate Vorschubgeschwindigkeit in mm pro min,   /60 um auf mm/s zu kommen
        x = float(self.get('X'))
        y = float(self.get('Y'))
        currenttravel = math.sqrt(x*x+y*y)
...
def split(filename, layer):
    ...
    for line in lines:
        ...
        if line.strip() == split_text:
            c = b
            p = g.get_pos()
            m = "G0 X{} Y{} Z{}".format(str(p[0]), str(p[1]), str(float(p[2])+10))
            print("Split at " + str(i) + " with " + m)
            a.append(m)
        c.append(line)
        i += 1
    with open(filename.replace('.gcode', '_a.gcode'), 'w') as file1:
        file1.writelines(a)
    with open(filename.replace('.gcode', '_b.gcode'), 'w') as file2:
        file2.writelines(b)
    print("Done")
...
def svg(args: util.Args):
    ...
    try:
        ...
        gcode_compiler.compile_to_file(filename+'.gcode', passes=2)
    except ImportError:
...
def image(args: util.Args):
    filename = args.shift()
    reader = GcodeReader(filename)
    fig, ax = reader.plot()
    save_figure(fig, filename+'.png', dpi=100)
    print("Done")

def image_layer(args: util.Args):
    filename = args.shift()
    layer = args.shift()
...
def main():
    print("GCode")
    print("")
    print("  Usage: split  {file.gcode} {layer}")
    print("  Usage: analyse  {file.gcode}")
    print("  Usage: svg  {file.svg}")
    print("  Usage: image {file.gcode}")
    print("  Usage: layer {file.gcode} {layer}")
    print("")
    args = util.Args()
    if args.help_requested():
        print("split - Nach Layern Splitten")
        print("image - Erzeugt eine file.gcode.png")
    else:
        args.shift()
        op = args.shift()
        if op == 'split':
...
# Author: Yaqi Zhang
# URL: https://github.com/zhangyaqi1989/Gcode-Reader/blob/master/src/gcode_reader.py
...
class GcodeReader:
    ...
    def __init__(self, filename, filetype=GcodeType.FDM_REGULAR):
        if isinstance(filename, GCode):
            g = filename
            filename = 'current.gcode'
    ...
    def to_image_dataurl(self):
        buffer = io.BytesIO()
        fig, ax = self.plot()
        fig.savefig(buffer, format='png')
        buffer.seek(0)
        pic_hash = base64.b64encode(buffer.read())
        return 'data:image/png;base64,' + str(pic_hash)
    ...
    def _read_class(self, g):
        ...
        #assert(len(self.seg_index_bars) - self.n_layers == 1) #NX
        self.xyzlimits = self._compute_xyzlimits(self.segs)
