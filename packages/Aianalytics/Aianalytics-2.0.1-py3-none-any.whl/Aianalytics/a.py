...
# Conditional frequency distribution for Inaugural corpus
...
if choice == 1:
    ...
    a = re.sub(r'www\.|\.com|\.in', '', text)
else:
    print("Wrong choice...try again")
    exit()
# Load words from file
with open(r"C:\Users\P2-words.txt", 'r') as f:
...
while a:
    seg_word = segment(a)
    if seg_word:
        answer.append(seg_word)
        a = a[len(seg_word):]
    else:
        break
# Output results
print("After segmentation:", " ".join(answer))
...
# Load the dataset
nltk.download('stopwords')
sms_data = pd.read_csv(r"C:\Users\spam.csv", encoding='latin-1')
sms_data.rename(columns={'v1': 'Category', 'v2': 'Message'}, inplace=True)
# Preprocess the text data
stemming = PorterStemmer()
corpus = sms_data['Message'].apply(lambda x: ' '.join(
    [stemming.stem(word) for word in re.sub('[^a-zA-Z]', ' ', x).lower().split() 
...
classification_2hidden_predictProb()
classification_2hidden_reg_predictProb()
kfold_reg_cross_validation()
kfold_mclass_cross_validation()
regularization()
L2rega0001()
L1withL2()
rnn()
...
x=tf.constant([10, 20, 30, 40, 50, 60], shape=[2,3])
print(x)
y=tf.constant([70, 80, 90, 100, 110, 102], shape=[3,2])
print(y)
z=tf.matmul(x, y)
print("Matrix product: ", z)
e_matrix_A = tf.random.uniform([2,2], minval=3, maxval=10, dtype=tf.float32, name="matrixA")
print("Matrix A:\n {} \n\n".format(e_matrix_A))
eigen_values_A, eigen_vectors_A = tf.linalg.eigh(e_matrix_A)
print("Eigen vectors: \n{}\n\nEigen values:\n{}\n".format(eigen_values_A, eigen_vectors_A ))
"""
	print(code)
...
print("Model evaluation: ", model.evaluate(inputs, outputs))
...
df = pd.read_csv(r"C:\Users\Aditi\Documents\Python Scripts\MSc_Sem4\ML Datasets\pima-indians-diabetes.csv",
                 names=["No. of pregnancies", "Glucose level", "Blood Pressure", "Skin thickness", "Insulin", 
                        "BMI", "Diabetes pedigree", "Age", "Class"]).drop(0)
...
# Prepare data and split into training/testing sets
X = df.iloc[:, :-1].astype('float32')
y = df.iloc[:, -1].astype('float32')
xtrain, xtest, ytrain, ytest = train_test_split(X, y, test_size=0.25, random_state=1)

# Train the model
binaryc.fit(xtrain, ytrain, epochs=200, batch_size=20, verbose=0)

# Predict and evaluate
class_labels = (binaryc.predict(xtest) > 0.5).astype(int)
print('Accuracy Score:', accuracy_score(ytest, class_labels))
...
from keras.layers import Dense
# Read the dataset
df = pd.read_csv(r"C:\Users\Aditi\Documents\Python Scripts\MSc_Sem4\ML Datasets\flowers.csv")
print("First few rows: \n", df.head(5))
# Separate features and target variable
X = df.iloc[:, :-1]  # Features: sepal length, sepal width, petal length, petal width
y = df.iloc[:, -1]   # Target variable: species
...
results_df = pd.DataFrame({'Predicted': predicted_classes, 'Actual': y})
print(results_df)

"""
	print(code)
	
def classification_2hidden_predictProb():
	code=r"""
from keras.models import Sequential
from keras.layers import Dense
from sklearn.datasets import make_blobs
...
for i in range(len(X_new)):
    print("X=%s, Predicted_Probability=%s, Predicted_Class=%s" % (X_new[i], Y_class[i], Y_class[i] > 0.5))

"""
	print(code)
	
def classification_2hidden_reg_predictProb():
	code=r"""
from keras.models import Sequential
from keras.layers import Dense
from sklearn.datasets import make_regression
...
def kfold_reg_cross_validation():
	code=r"""
import warnings
...
from sklearn.preprocessing import StandardScaler
from sklearn.pipeline import Pipeline
dataframe = pd.read_csv(r"C:\Users\Aditi\Documents\Python Scripts\MSc_Sem4\ML Datasets\housing.csv", delim_whitespace=False, header=None)
dataset = dataframe.values
X = dataset[:, 0:13]
Y = dataset[:, 13]
def wider_model():
    ...
    model.compile(loss='mean_squared_error', optimizer='adam')
    return model
# Create a KerasRegressor based on the wider_model function
estimators = []
estimators.append(('standardize', StandardScaler()))
estimators.append(('mlp', KerasRegressor(build_fn=wider_model, epochs=20, batch_size=5,verbose=0)))
pipeline = Pipeline(estimators)
...
def kfold_mclass_cross_validation():
	code=r"""
import pandas as pd
import warnings
from keras.models import Sequential
...
warnings.filterwarnings("ignore")
# Load dataset
dataframe = pd.read_csv(r"C:\Users\Aditi\Documents\Python Scripts\MSc_Sem4\ML Datasets\iris.csv", header=None, skiprows=1)
dataset = dataframe.values
...
def regularization():
	code=r"""
#regularization to avoid overfitting in binary classificarion
import matplotlib.pyplot as pp
#from matplotlib import pyplot
from sklearn.datasets import make_moons
from keras.models import Sequential
from keras.layers import Dense
X,Y=make_moons(n_samples=40,noise=0.2,random_state=1)
n_train=10
trainX,testX=X[:n_train,:],X[n_train:]
trainY,testY=Y[:n_train],Y[n_train:]
print("Training X: \n",trainX)
...
model.add(Dense(1,activation='sigmoid'))
model.compile(loss='binary_crossentropy',optimizer='adam',metrics=['accuracy'])
history=model.fit(trainX,trainY,validation_data=(testX,testY),epochs=60)
pp.plot(history.history['accuracy'],label='train')
pp.plot(history.history['val_accuracy'],label='test')
pp.legend()
pp.show()
...
dataset_train = pd.read_csv(r"C:\Users\Aditi\Documents\Python Scripts\MSc_Sem4\ML Datasets\Google_Stock_Price_Train.csv")
training_set = dataset_train.iloc[:, 1:2].values
sc = MinMaxScaler(feature_range=(0, 1))
training_set_scaled = sc.fit_transform(training_set)

# Create training data structure with 60 timesteps
X_train, Y_train = [], []
for i in range(60, 1258):
    X_train.append(training_set_scaled[i-60:i, 0])
    Y_train.append(training_set_scaled[i, 0])
...
dataset_test = pd.read_csv(r"C:\Users\Aditi\Documents\Python Scripts\MSc_Sem4\ML Datasets\Google_Stock_Price_Test.csv")
real_stock_price = dataset_test.iloc[:, 1:2].values
dataset_total = pd.concat((dataset_train['Open'], dataset_test['Open']), axis=0)
...
# Predict stock prices and visualize results
