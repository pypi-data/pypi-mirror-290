...
class FabricRunItemOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> None:
        ...
        while attempt < self.max_retries and item_run_details is None:
            ...
            self.log.info(f"Item run details response: {item_run_details_response}")

            if item_run_details_response.get("failureReason", dict()) is not None and item_run_details_response.get("failureReason", dict()).get("errorCode") in ["RequestExecutionFailed", "NotFound"]:
