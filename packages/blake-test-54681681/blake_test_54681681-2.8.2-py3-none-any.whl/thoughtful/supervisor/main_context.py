...
from thoughtful.supervisor.event_bus import StartProcessEvent
...
class MainContext:
    ...
    def start_process(self, process_id: int, inputs: dict) -> None:
        """
        Requests the Fabric to start a process with the specified ID and inputs.

        Note: Process start is not guaranteed and may fail under certain
        conditions, such as an invalid process ID or inputs.
    ...
    def __exit__(
        ...
        self._stream_run_status_change(work_report.status, work_report.status_message)
