...
print = partial(pprint, max_length=10, max_string=10)
...
def find_and_sort(query_key, limit=5) -> list:
...
def modify_requirements(package_name, package_version=None, action="install", requirements="requirements.txt") -> None:
...
def modify_pyproject_toml(
    ...
    is_hatch_env = hatch_env and "tool" in pyproject and "hatch" in pyproject.setdefault("tool", {})
    ...
    base_project = (
        pyproject.get("tool", {}).setdefault("hatch", {}).get("envs", {}).get(hatch_env, {})
        if is_hatch_env
        else pyproject.setdefault("project", {})
    )
    optional_base = pyproject.setdefault("project", {}).get("optional-dependencies", {})
