...
import click
import tomlkit
from mdstream import Markdown
from rich import print
from rich.traceback import Traceback

from mpypip.create import create_project
from mpypip.pypip import (
    find_and_sort,
    get_package_info,
...
def install_command(
    ...
    """
    try:
        requirements_given = requirements is not None
        ...
        if requirements_given:
            if upgrade:
                packages = packages.union(get_requirements_packages(requirements))
            else:
                package_install_cmd = [sys.executable, "-m", "pip", "install", "-r", requirements]
                stderr, stdout = subprocess.Popen(package_install_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True, text=True).communicate()
                color_code = "\033[92m"
                click.echo(f"{color_code}{stdout}")
                color_code = "\033[91m"
                click.echo(f"{color_code}{stderr}")
                if "A new release of pip is available" in stdout:
                    click.echo("A new release of pip is available. Please upgrade pip.")
