...
class BaseChartField:
    ...
    buckets_key: str = field(
        default="sum", metadata={"description": "取值规则 sum avg count"}
    )
    # key value 格式表使用
...
class FiledSetting:
    ...
    def __init__(self, **kwargs):
        self.filed_keys: list = kwargs.get("filed_keys", [])
    ...
    def analyze_filed_key_to_source(self) -> dict:
        ...
        for filed_key in self.filed_keys:
            date_source = FIELD_SETTINGS.get(filed_key, {}).get("date_source", "")
            table_name = FIELD_SETTINGS.get(filed_key, {}).get("table_name", "")
            if date_source == "mysql":
                if table_name in MIDDLE_MODEL:
                    data_res["middle_model"].append(filed_key)
                elif table_name in INSIGHT_MODEL:
                    data_res["insight_model"].append(filed_key)
                else:
                    data_res["other_model"].append(filed_key)
            elif date_source == "es":
                data_res["es_model"].append(filed_key)
            else:
                data_res["other_model"].append(filed_key)
        return data_res
...
class BaseChart:
    ...
    def __init__(self, **kwargs):
        ...
        self.date_source: str = kwargs.get("date_source", "mysql")
        self.date_type: str = kwargs.get("date_type", "day")
        self.date_field: str = kwargs.get("date_field", "created_at")
        ...
        self.buckets_key: str = kwargs.get("buckets_key", "sum")
        self.insight_key_name: str = kwargs.get("insight_key_name", "key")
        self.insight_name_name: str = kwargs.get("name", "insight_name_name")
        self.__post_init__()
    ...
    def __post_init__(self):
        ...
        for key, value in self.__dict__.items():
            setattr(self, key, field_settings.get(key, getattr(self, key)))
