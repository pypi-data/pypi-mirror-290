...
from buildgrid._exceptions import InvalidArgumentError, NotFoundError, StorageFullError
...
def controller(request, asset_client: AssetClient):
...
@pytest.fixture
def isolated_controllers(request):
    storage = lru_memory_cache.LRUMemoryCache(1024 * 1024)
    db = tempfile.NamedTemporaryFile().name
    sql_provider = SqlProvider(connection_string=f"sqlite:///{db}", automigrate=True)
    ...
    keys = ["OSFamily", "ISA"]
    cache = LruActionCache(storage, 50)
    scheduler_a = Scheduler(sql_provider, storage, action_cache=cache, property_keys=keys, match_properties=keys)
    scheduler_b = Scheduler(sql_provider, storage, action_cache=cache, property_keys=keys, match_properties=keys)
    with scheduler_b, scheduler_a:
        yield ExecutionController(scheduler_a), ExecutionController(scheduler_b)


def mock_queue_job_action(scheduler, skip_cache_lookup=True, request_metadata=None, do_not_cache=False):
    bot_id = "test-worker"
...
def mock_execution_metadata(controller):
    ...
    with scheduler._sql.session(exceptions_to_not_raise_on=[Exception]) as session:
        session.add(JobEntry(name="job", instance_name="sql", action_digest=digest_to_string(action_digest)))
