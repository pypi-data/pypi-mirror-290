...
class MockScheduler:
    def __init__(self, storage, property_keys, match_properties, metering_client=None):
        self.storage = storage
        self.property_keys = property_keys or set()
        self.match_properties = match_properties or set()
...
def test_execute_platform_matching_simple():
    ...
    exec_instance = ExecutionInstance(MockScheduler(storage, property_keys=keys, match_properties=keys))
...
def test_execute_platform_matching_too_many_os():
    ...
    keys = ["OSFamily", "ISA"]
    exec_instance = ExecutionInstance(MockScheduler(MockStorage(pairs), property_keys=keys, match_properties=keys))
...
def test_execute_platform_matching_config_only():
    ...
    keys = ["OSFamily", "ISA", "ChrootDigest"]
    scheduler = MockScheduler(MockStorage(pairs), property_keys=keys, match_properties=keys[0:2])
...
def test_execute_platform_matching_both_empty():
    ...
    pairs = []
    exec_instance = ExecutionInstance(MockScheduler(MockStorage(pairs), property_keys=[], match_properties=[]))
    assert exec_instance.execute(action_digest="fake", skip_cache_lookup=False) == "queued"
...
def test_execute_priority_set():
    ...
    pairs = [("OSFamily", "linux"), ("ISA", "x86-64")]
    keys = ["OSFamily", "ISA", "ChrootDigest"]
    mock_scheduler = MockScheduler(MockStorage(pairs), keys, keys)
...
def test_execute_not_throttled(client_identity: ClientIdentityEntry, mock_metering_client: SyncMeteringServiceClient):
    pairs = [("OSFamily", "linux"), ("ISA", "x86-64")]
    keys = ["OSFamily", "ISA", "ChrootDigest"]
    mock_scheduler = MockScheduler(MockStorage(pairs), keys, keys, metering_client=mock_metering_client)
    exec_instance = ExecutionInstance(mock_scheduler)
...
def test_execute_not_throttled_if_no_client(client_identity: ClientIdentityEntry):
    pairs = [("OSFamily", "linux"), ("ISA", "x86-64")]
    keys = ["OSFamily", "ISA", "ChrootDigest"]
    mock_scheduler = MockScheduler(MockStorage(pairs), keys, keys, metering_client=None)
    exec_instance = ExecutionInstance(mock_scheduler)
