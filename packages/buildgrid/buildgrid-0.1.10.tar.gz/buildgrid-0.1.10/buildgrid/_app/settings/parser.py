...
from buildgrid._enums import ActionCacheEntryType, ServiceName
...
def object_tag(kind: str) -> Callable[[_Func], _Func]:
    """
...
def load_redis_index(storage: StorageABC, redis: "RedisProvider") -> "RedisIndex":
...
def load_replicated_storage(storages: List[StorageABC]) -> ReplicatedStorage:
...
def load_sharded_storage(shards: List[ShardType], thread_pool_size: Optional[int] = None) -> ShardedStorage:
...
def get_logstream_connection_info(logstream: Any) -> Tuple[Optional[str], Optional[Dict[str, str]], Optional[str]]:
    logstream_url = None
    credentials = None
...
def get_schema(strict: bool = False) -> Any:
    ...
    schema_text = files("buildgrid._app.settings").joinpath("schema.yml").read_text()
...
def validate_config(path: "os.PathLike[str]", strict: bool = False) -> None:
    """
    ...
    with open(path) as f:
        return validate_config_value(f.read(), strict=strict)


def validate_config_value(data: str, strict: bool = False) -> None:
    """
    Validate a buildgrid configuration against its schema.
    In this mode, no real components are loaded, it simply detects invalid argument values.
    If in strict mode, all object definitions will set additionalProperties to false
