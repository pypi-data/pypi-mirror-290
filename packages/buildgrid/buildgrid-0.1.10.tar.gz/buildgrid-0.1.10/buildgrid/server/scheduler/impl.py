...
from sqlalchemy import and_, delete, func, insert, literal_column, or_, select, union, update
...
from buildgrid._enums import BotStatus, LeaseState, MetricCategories, OperationStage
...
class AgedJobHandlerOptions(NamedTuple):
    ...
    def from_config(
        ...
        handling_period_cfg: Optional[Dict[str, float]] = None,
        max_handling_window_cfg: Optional[int] = None,
...
class Scheduler:
    ...
    def __init__(
        ...
        *,
        ...
        logstream_channel: Optional[Channel] = None,
        logstream_instance: Optional[str] = None,
        ...
        pruning_options: Optional[AgedJobHandlerOptions] = None,
        queue_timeout_options: Optional[AgedJobHandlerOptions] = None,
    ...
    def _bot_in_instance(self) -> ClauseElement:
    ...
    def queue_job_action(
        ...
        *,
        ...
        command: Command,
        platform_requirements: Dict[str, Set[str]],
        priority: int,
        skip_cache_lookup: bool,
        request_metadata: Optional[RequestMetadata] = None,
        client_identity: Optional[ClientIdentityEntry] = None,
    ) -> str:
        """
        ...
        execute_response: Optional[ExecuteResponse] = None
        if self.action_cache and not action.do_not_cache and not skip_cache_lookup:
            try:
                ...
                LOGGER.info(f"Job cache hit for action [{action_digest.hash}/{action_digest.size_bytes}]")
                ...
                LOGGER.exception(
                    f"Checking ActionCache for action [{action_digest.hash}/{action_digest.size_bytes}] failed."
                )
    ...
    def create_operation_for_existing_job(
        ...
        *,
        ...
        client_identity: Optional[ClientIdentityEntry],
    ) -> Optional[str]:
        ...
        with self._sql.session(exceptions_to_not_raise_on=[Exception]) as session:
            ...
            if priority < job.priority and job.stage == OperationStage.QUEUED.value:
                LOGGER.info(f"Job {job.name} assigned a new priority {priority}")
    ...
    def create_operation_for_new_job(
        ...
        *,
        ...
        action_digest: Digest,
        command: Command,
        execute_response: Optional[ExecuteResponse],
        platform_requirements: Dict[str, Set[str]],
        priority: int,
        request_metadata: Optional[RequestMetadata] = None,
        client_identity: Optional[ClientIdentityEntry] = None,
        ...
        with self._sql.session(exceptions_to_not_raise_on=[Exception]) as session:
            job = JobEntry(
                ...
                platform_requirements=hash_from_dict(convert_values_to_sorted_lists(platform_requirements)),
    ...
    def _populate_platform_requirements(
        self, session: Session, platform_requirements: Dict[str, Set[str]]
    ) -> List[PlatformEntry]:
        ...
        while missing := required_entries - {(k, v) for [k, v] in session.execute(statement).all()}:
            try:
                session.bulk_insert_mappings(PlatformEntry, [{"key": k, "value": v} for k, v in missing])
                session.commit()
    ...
    def _create_operation(
        ...
        *,
        ...
        request_metadata_id: Optional[int] = None
        if request_metadata:
    ...
    def _load_operation(self, op: OperationEntry) -> Operation:
        ...
        if execute_response:
            if self.action_browser_url:
                url = BrowserURL(self.action_browser_url, job.instance_name)
                if url.for_message("action", string_to_digest(job.action_digest)):
                    execute_response.message = str(url.generate())
            operation.response.Pack(execute_response)

        return operation

    def _get_job(self, job_name: str, session: Session, with_for_update: bool = False) -> Optional[JobEntry]:
        statement = select(JobEntry).where(JobEntry.name == job_name, self._job_in_instance())
        if with_for_update:
            statement = statement.with_for_update()

        job: Optional[JobEntry] = session.execute(statement).scalars().first()
        if job:
            LOGGER.debug(
                f"Loaded job from db: name=[{job_name}], stage=[{job.stage}], "
                f"result=[{job.result}], instance=[{job.instance_name}]"
            )

        return job

    def get_operation_job_name(self, operation_name: str) -> Optional[str]:
        with self._sql_ro.session(exceptions_to_not_raise_on=[Exception]) as session:
            if operation := self._get_operation(operation_name, session):
                return operation.job_name
        return None

    def get_operation_request_metadata_by_name(self, operation_name: str) -> Optional[RequestMetadata]:
        with self._sql_ro.session(exceptions_to_not_raise_on=[Exception]) as session:
            ...
            metadata = RequestMetadata(
                tool_details=ToolDetails(
                    tool_name=operation.tool_name or "",
                    tool_version=operation.tool_version or "",
                ),
                action_id=operation.job.action_digest or "",
                correlated_invocations_id=operation.correlated_invocations_id or "",
                tool_invocation_id=operation.invocation_id or "",
            )

            if operation.request_metadata:
                metadata.action_mnemonic = operation.request_metadata.action_mnemonic or ""
                metadata.configuration_id = operation.request_metadata.configuration_id or ""
                metadata.target_id = operation.request_metadata.target_id or ""

            return metadata

    def get_client_identity_by_operation(self, operation_name: str) -> Optional[ClientIdentity]:
        with self._sql_ro.session(exceptions_to_not_raise_on=[Exception]) as session:
            operation = self._get_operation(operation_name, session)
    ...
    def _notify_job_updated(self, job_names: Union[str, List[str]], session: Session) -> None:
        if self._sql.dialect == "postgresql":
            if isinstance(job_names, str):
                job_names = [job_names]
            for job_name in job_names:
                # Mypy bug? "execute" of "_SessionTypingCommon" has incompatible type "str"; expected "Executable
                session.execute(f"NOTIFY job_updated, '{job_name}';")  # type: ignore[arg-type]

    def _get_operation(self, operation_name: str, session: Session) -> Optional[OperationEntry]:
        statement = (
            select(OperationEntry).join(JobEntry).where(OperationEntry.name == operation_name, self._job_in_instance())
        )
        return session.execute(statement).scalars().first()

    def _batch_timeout_jobs(self, job_select_stmt: Select, status_code: int, message: str) -> int:
    ...
    def cancel_jobs_exceeding_execution_timeout(self, max_execution_timeout: Optional[int] = None) -> None:
        ...
        with self._sql.session(sqlite_lock_immediately=True, exceptions_to_not_raise_on=[Exception]) as session:
            ...
            for job in jobs:
                ...
                LOGGER.warning(
                    f"Job=[{job.name}] has been executing for "
    ...
    def get_metrics(self) -> Optional[SchedulerMetrics]:
        ...
        def _get_query_leases_by_state(category: str) -> Select:
            ...
            return select(
                [
                    ...
                    LeaseEntry.state.label("bucket"),
                    func.count(LeaseEntry.id).label("value"),
                ]
            ).group_by(LeaseEntry.state)

        def _cb_query_leases_by_state(leases_by_state: Dict[Any, Any]) -> Dict[Any, Any]:
        ...
        def _get_query_jobs_by_stage(category: str) -> Select:
            ...
            return select(
                [
                    literal_column(f"'{category}'").label("category"),
                    JobEntry.stage.label("bucket"),
                    func.count(JobEntry.name).label("value"),
                ]
            ).group_by(JobEntry.stage)

        def _cb_query_jobs_by_stage(jobs_by_stage: Dict[Any, Any]) -> Dict[Any, Any]:
            # The database only returns counts > 0, so fill in the gaps
        ...
        try:
            with self._sql_ro.session(exceptions_to_not_raise_on=[Exception]) as session:
                ...
                grouped_results: Dict[str, Any] = {category: {} for category, _, _ in results}
                for category, bucket, value in results:
                    grouped_results[category][bucket] = value

                for category, _, category_cb in metrics_to_gather:
                    metrics[category] = category_cb(  # type: ignore[literal-required]
    ...
    def _delete_jobs_prior_to(self, delete_before_datetime: datetime, limit: int) -> int:
        ...
        with self._sql.session() as session:
            ...
            num_rows_deleted: int = session.execute(delete_stmt, execution_options=options).rowcount  # type: ignore
    ...
    def get_or_create_client_identity_in_store(
        ...
        """
        client_id_stored: Optional[ClientIdentityEntry] = (
            session.query(ClientIdentityEntry)
        ...
        if client_id_stored is not None:
            return client_id_stored
    ...
    def _insert_on_conflict_do_nothing(self, model: Type[OrmBase]) -> Insert:
        ...
        if self._sql.dialect == "sqlite":
            ...
            return sqlite_insert.on_conflict_do_nothing()  # type: ignore

        elif self._sql.dialect == "postgresql":
            insertion: postgresql.Insert = postgresql.insert(model)
            return insertion.on_conflict_do_nothing()  # type: ignore
    ...
    def add_bot_entry(self, *, bot_session_id: str, bot_session_status: int) -> str:
        with self._sql.session() as session:
    ...
    def _close_bot_sessions(self, session: Session, bots: List[BotEntry]) -> None:
        for bot in bots:
            log_tags = f" instance_name=[{try_current_instance()}] request.bot_name=[{bot.name}]"
    ...
    def session_expiry_timer_loop(self, shutdown_requested: threading.Event) -> None:
        LOGGER.info(f"Starting BotSession reaper. keepalive_timeout=[{self.bot_session_keepalive_timeout}]")
        while not shutdown_requested.is_set():
            try:
    ...
    def reap_expired_sessions(self) -> bool:
        ...
        with self._sql.session() as session:
            ...
            if bots := cast(List[BotEntry], session.execute(locate_bot_stmt).scalars().all()):
                ...
                for bot in bots:
                    LOGGER.warning(
                        f"BotSession name=[{bot.name}] for bot_id=[{bot.bot_id}] instance=[{bot.instance_name}]"
                        f"with deadline=[{bot.expiry_time}] has expired."
                    )
                    bots_by_instance[bot.instance_name or ""].append(bot)
    ...
    def synchronize_bot_lease(
        self, bot_name: str, bot_id: str, bot_status: int, session_lease: Optional[Lease]
    ) -> Optional[Lease]:
        log_tags = f"instance_name=[{try_current_instance()}]"
        log_tags += f" request.bot_name=[{bot_name}] request.bot_id=[{bot_id}] request.bot_status=[{bot_status}]"

        if session_lease:
            log_tags += f" request.lease_id=[{session_lease.id}] request.lease_state=[{session_lease.state}]"
        else:
        ...
        with self._sql.session(exceptions_to_not_raise_on=[Exception]) as session:
            ...
            try:
                ...
                log_tags += f" db.lease_state=[{db_lease.state}]"
    ...
    def _complete_lease(
        self, session: Session, job: JobEntry, lease: LeaseEntry, status: Status, result: Optional[ProtoAny] = None
    ) -> None:
        ...
        now = datetime.utcnow()
        action_result.execution_metadata.queued_timestamp.FromDatetime(job.queued_timestamp or now)
        ...
        if self.action_cache and result and not job.do_not_cache:
            ...
            try:
                ...
                LOGGER.debug(
                    f"Stored action result=[{action_result}] for action_digest=[{action_digest}] in ActionCache"
                )
                ...
                LOGGER.exception(
                    ...
                    f"[{action_digest.hash}/{action_digest.size_bytes}] wasn't updated."
                )
            except Exception:
                LOGGER.exception(
                    "Unable to update ActionCache for action "
        ...
        self._publish_execution_stats(session, job.name, action_result.execution_metadata)

    def count_bots_by_status(self) -> Dict[BotStatus, int]:
        ...
        with self._sql.session() as session:
            ...
            result = {status: 0 for status in BotStatus}
            for [bot_status, count] in query.all():
                result[BotStatus(bot_status or 0)] = count
            return result
    ...
    def get_execute_action_metadata(self, job: JobEntry) -> ExecutedActionMetadata:
        ...
        executed_action_metadata = ExecutedActionMetadata(worker=worker_name)
        if job.queued_timestamp is not None:
            executed_action_metadata.queued_timestamp.FromDatetime(job.queued_timestamp)
        if job.worker_start_timestamp is not None:
            executed_action_metadata.worker_start_timestamp.FromDatetime(job.worker_start_timestamp)
        if job.worker_completed_timestamp is not None:
            executed_action_metadata.worker_completed_timestamp.FromDatetime(job.worker_completed_timestamp)

        return executed_action_metadata

    def _fetch_execution_stats(
        self, auxiliary_metadata: RepeatedCompositeFieldContainer[ProtoAny]
    ) -> Optional[ExecutionStatistics]:
        ...
        for aux_metadata_any in auxiliary_metadata:
            ...
            try:
                ...
                LOGGER.exception(
                    "Cannot fetch ExecutionStatistics from storage, auxiliary_metadata=%s",
                    aux_metadata_digest.SerializeToString(),
                    exc_info=exc,
                )
                return None
        return None

    def publish_execution_stats(self, job_name: str, execution_metadata: ExecutedActionMetadata) -> None:
        with self._sql_ro.session(expire_on_commit=False) as session:
            self._publish_execution_stats(session, job_name, execution_metadata)

    def _publish_execution_stats(
        self, session: Session, job_name: str, execution_metadata: ExecutedActionMetadata
    ) -> None:
        ...
        timings = {
            "Total": worker_completed - queued,
            # This calculates the queue time based purely on values set in the ActionResult's
            # ExecutedActionMetadata, which may differ than what the job object's queued_time is.
            "Queued": worker_start - queued,
            "Worker": worker_completed - worker_start,
            "Fetch": fetch_completed - fetch_start,
            "Execution": execution_completed - execution_start,
            "Upload": upload_completed - upload_start,
        }
        for tag, value in timings.items():
            publish_timer_metric(METRIC.JOB.DURATION, value, state=tag)
        ...
        try:
            ...
            LOGGER.exception("Cannot publish resource usage, job_name=%s", job_name, exc_info=exc)

    def _update_action_retention(
        self, action: Action, action_digest: Digest, retention_hours: Optional[float]
    ) -> None:
        if not self.asset_client or not retention_hours:
            return
        digest_str = digest_to_string(action_digest)
        ...
        try:
            ...
            LOGGER.debug(f"Extended the retention of action {digest_str} for {retention_hours} hours")
        except Exception:
            LOGGER.exception(f"Failed to push action as an asset: {digest_str}")
            # Not a fatal path, don't reraise here

    def _update_action_result_retention(self, action_result: ActionResult, retention_hours: Optional[float]) -> None:
        if not self.asset_client or not retention_hours:
            return
        try:
            ...
            digest_str = digest_to_string(digest)
            ...
            LOGGER.debug(f"Extended the retention of action_result {digest_str} for {retention_hours} hours")

        except Exception as e:
            LOGGER.exception(f"Failed to push action_result as an asset: {digest_str}", exc_info=e)
            # Not a fatal path, don't reraise here


class JobAssigner:
    def __init__(self, scheduler: Scheduler, job_assignment_interval: float = 1.0, priority_percentage: int = 100):
    ...
    def listener_count(self, instance_name: Optional[str] = None) -> int:
        with self._lock:
            return len(
                {
                    bot_name
    ...
    def assignment_context(self, bot_session: BotSession) -> Iterator[threading.Event]:
        ...
        worker_hashes = set(get_partial_capabilities_hashes(bot_capabilities(bot_session)))
        instance_name = current_instance()
        try:
            with self._lock:
        ...
        while not shutdown_requested.is_set():
            ...
            for instance_name in instance_names:
                try:
                    ...
                    LOGGER.exception(f"Error in job assignment thread instance='{instance_name}'", exc_info=True)
