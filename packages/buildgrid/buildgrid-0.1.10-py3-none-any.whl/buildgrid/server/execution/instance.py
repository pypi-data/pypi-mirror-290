...
class ExecutionInstance(Instance):
    ...
    def execute(
        ...
        *,
        ...
        platform_requirements: Dict[str, Set[str]] = defaultdict(set)
        for platform_property in platform.properties:
            name = platform_property.name
            if name not in self.scheduler.property_keys:
                raise FailedPreconditionError(
                    f"Unregistered platform property [{name}]. Known properties are: [{self.scheduler.property_keys}]"
                )
            if name in self.scheduler.unique_keys and name in platform_requirements:
                raise FailedPreconditionError(f"Platform property [{name}] can only be set once.")
            if name in self.scheduler.match_properties:
        ...
        if should_throttle:
            ...
            LOGGER.info(
                f"Job priority throttled action_digest=[{action_digest}] "
                f"old_priority=[{priority}] new_priority=[{EXECUTION_DEPRIORITIZATION_LIMIT}]"
            )
    ...
    def get_storage_capabilities(self) -> CacheCapabilities:
        return self.scheduler.storage.get_capabilities()

    def get_action_cache_capabilities(self) -> Tuple[Optional[Any], Optional[ActionCacheUpdateCapabilities]]:
        hash_type = None
        capabilities = None
        if self.scheduler.action_cache is not None:
            capabilities = ActionCacheUpdateCapabilities()
            hash_type = self.scheduler.action_cache.hash_type()
            capabilities.update_enabled = self.scheduler.action_cache.allow_updates
        return hash_type, capabilities

    def _meter_execution(self, client_identity: Optional[ClientIdentityEntry], operation_name: str) -> None:
    ...
    def _should_throttle_execution(self, priority: int, client_identity: Optional[ClientIdentityEntry]) -> bool:
        if (
            priority >= EXECUTION_DEPRIORITIZATION_LIMIT
        ...
        try:
            ...
            if response.throttled:
                LOGGER.info(
                    "Execution request is throttled, client_id: [%s], usage: [%s]",
                    client_identity,
                    response.tracked_usage,
                )
