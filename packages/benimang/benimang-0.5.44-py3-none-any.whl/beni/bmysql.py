...
class BModel(BaseModel):
    ...
    @classmethod
    async def remove(cls, cursor: MysqlDbCursor, where: str | WhereMaker = '', *args: Any) -> int:
    ...
    @classmethod
    async def count(cls, cursor: MysqlDbCursor, where: str | WhereMaker = '', *args: Any, tableName: str = '') -> int:
        if type(where) is str:
            where = f'WHERE {where}' if where else ''
        else:
            where = str(where)
        return await cursor.getValue(
            int,
            f'''
            SELECT
                COUNT(*)
...
class MysqlDbCursor:
    ...
    def _getColumns(self) -> tuple[Any, ...]:
