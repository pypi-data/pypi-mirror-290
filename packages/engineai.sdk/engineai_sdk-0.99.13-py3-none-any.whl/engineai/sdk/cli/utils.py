...
def _df_to_table(
    ...
    if len(df.columns) > max_columns:
        df = df[
            df.columns[: max_columns // 2].tolist()
            + df.columns[-max_columns // 2 :].tolist()
        ]
        # add separator column for large tables
        df.insert(len(df.columns) // 2, "...", " ")
    ...
    normalized_index = df.index.astype(str).tolist()
    row_values = df.values.tolist()
    col_length = len(df.columns) + 1 if show_index else len(df.columns)
