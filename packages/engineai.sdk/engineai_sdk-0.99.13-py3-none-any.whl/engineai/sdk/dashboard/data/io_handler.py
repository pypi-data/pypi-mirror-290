...
class IOWriter:
    ...
    def __init__(self, path: str, write_keys: bool, separator: str = "/") -> None:
        ...
        self.__keys_path = Path(os.path.join(_TEMP_DIR, _KEYS_DIR))
        self.__metadata_path = Path(os.path.join(_TEMP_DIR, _METADATA_DIR))
    ...
    def write_metadata(self, data: pd.DataFrame, metadata: Set[str], path: str) -> None:
        ...
        if len(metadata) > 0:
            try:
                ...
                data[list(metadata)].to_json(
                    os.path.join(self.__metadata_path, f"{path}.json")
                )
...
class IOReader:
    ...
    def read_metadata(path: str) -> pd.DataFrame:
        ...
        try:
            ...
            raise KeyError(f"Metadata not found for {p}") from exc
