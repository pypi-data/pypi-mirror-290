...
def _collect_fields(data: pd.DataFrame) -> List[Dict[str, str]]:
    ...
    for name, ty in zip(data, map(str, data.dtypes)):
        ...
        elif ty.startswith("period") or ty.startswith("datetime64"):
            fields.append({"name": str(name), "type": ty})
...
def _setup_index(schema: Dict[str, Any], df: pd.DataFrame) -> None:
    if "index" not in schema:
        return

    idx = schema["index"]
    data = df.get(INDEX_NAME, [])

    if idx["type"] == "category":
        df[INDEX_NAME] = pd.CategoricalIndex(data=data)

    elif idx["type"].startswith("period"):
        df[INDEX_NAME] = pd.PeriodIndex(
            data=pd.to_datetime(data, unit="ms"), dtype=idx["type"]
        )

    elif idx["type"] == "time":
        df[INDEX_NAME] = pd.to_datetime(data).dt.time

    elif idx["type"] == "date":
        df[INDEX_NAME] = pd.to_datetime(data, unit="ms").dt.date

    elif (dt_match := DATETIME_REGEX.match(idx["type"])) is not None:
        _, tz = cast(Tuple[Optional[str], ...], dt_match.groups())

        df[INDEX_NAME] = pd.DatetimeIndex(
        ...
        if tz is not None:
            df[INDEX_NAME] = (
                df[INDEX_NAME].dt.tz_localize(datetime.timezone.utc).dt.tz_convert(tz)
            )
...
def from_records(records: List[Dict[str, Any]], schema: Dict[str, Any]) -> pd.DataFrame:
    ...
    _convert_columns(schema, df)
    _setup_index(schema, df)
    return df
