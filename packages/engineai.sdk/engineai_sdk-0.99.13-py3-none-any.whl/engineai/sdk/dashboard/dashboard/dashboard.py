...
class Dashboard:
    ...
    def __init__(
        ...
        *,
        ...
        self.__app_slug = self._set_app_slug(app_id, app_slug)
        self.__workspace_slug = self._set_workspace_slug(workspace_slug)
        self.__publish()
    ...
    @staticmethod
    def _set_workspace_slug(workspace_slug: Optional[str]) -> Optional[str]:
        """Set workspace slug."""
        if (internal_workspace_slug := os.environ.get("WORKSPACE_SLUG")) is not None:
            return internal_workspace_slug  # for internal use only
        return workspace_slug

    @staticmethod
    def _set_app_slug(app_id: Optional[str], app_slug: Optional[str]) -> Optional[str]:
        """Set app slug."""
        if (internal_app_slug := os.environ.get("APP_ID")) is not None:
            warnings.warn(
                "APP_ID env var is deprecated and will be removed in a future release."
                "Please use `APP_SLUG` parameter instead.",
                DeprecationWarning,
            )
            return internal_app_slug  # for internal use only

        if (internal_app_slug := os.environ.get("APP_SLUG")) is not None:
            return internal_app_slug  # for internal use only

        if app_id is not None:
            warnings.warn(
                "app_id is deprecated and will be removed in a future release."
                "Please use `app_slug` parameter instead.",
                DeprecationWarning,
            )
            return app_id
