...
def binarize_columns(dataset: pd.DataFrame, columns: list, privileged_classes: dict) :
    ...
    for attribute in columns :
        ...
        form_group = lambda x: f"non_{privileged_group}" if x != privileged_group else privileged_group
        ...
        if protected_attributes :
            ...
            if privileged_groups :
                for protected_attribute in protected_attributes :
                    ...
                    assert privileged_groups[protected_attribute] in dataset[protected_attribute].unique(), f"{privileged_groups[protected_attribute]} is not a class of {protected_attribute}"
