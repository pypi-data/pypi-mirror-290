...
class Mitigator(sanity_checker):
    ...
    def __init__(self, method="RandomOverSampler"):
        """
        /!\ DEPRECATED. USE THE fairbalance.mitigation_strategies MODULE INSTEAD. /!\
    ...
    def mitigate(self, mitigation_strategy: str, dataset: pd.DataFrame, target: str,
        ...
        Args:
            mitigation_strategy (str): the mitigation strategy to use. Possible values are "balane_output",
            "balance_protected_attribute", "balance_output for attribute" and "balance_all".
            ...
            protected_attributes (list, optional): The attribute(s) to balance.
            Only necessary for balance_protected_attribute and balance_output_for_attribute. Defaults to None.
    ...
    def balance_output(self, dataset: pd.DataFrame, target: str, protected_attributes: list = None,
        ...
        if protected_attributes and len(protected_attributes) > 1:
            df, protected_attribute = self._make_super_protected(
        ...
        if self.method in ["SMOTENC"]:
            ...
            processed_df = processor.process(
                df, scale_cols=cont_columns, encode_cols=cat_columns)

            # Then resample
            cat_columns_ids = [processed_df.columns.get_loc(
    ...
    def balance_protected_attribute(self, dataset: pd.DataFrame, target: str, protected_attributes: list,
        ...
        Args:
            ...
            cont_columns (list, optional): The continuous columns in the dataset.
            Only Necessary if the sampling strategy is SMOTENC. Defaults to None.
            cat_columns (list, optional): The categorical columns in the dataset.
            Only Necessary if the sampling strategy is SMOTENC. Defaults to None.
        ...
        if self.method in ["SMOTENC"]:
            ...
            cont_col_buff = [
                feature for feature in cont_columns if feature != protected_attribute]
            cat_col_buff = [
                feature for feature in cat_columns if feature != protected_attribute]

            processor = BaseProcessor()

            processed_df = processor.process(df, scale_cols=cont_col_buff, encode_cols=(
                cat_col_buff + [protected_attribute]))
            ...
            cat_column_ids = [processed_df.columns.get_loc(
                col_name) for col_name in cat_column_buff]

            sampler = self.mitigator_function[self.method](
                categorical_features=cat_column_ids)

            X_resampled, attribute_resampled = sampler.fit_resample(
                processed_df, protected_attribute_column)
    ...
    def balance_output_for_attribute(self, dataset: pd.DataFrame, target: str, protected_attributes: list,
        ...
        r, max_class = self._highest_ratio(
            df, target, classes, protected_attribute)
            ...
            if c != max_class:
                ...
                if len(list(class_target.unique())) != 1:
                    ...
                    if self.method in ["SMOTENC"]:
                        ...
                        # preprocess using the protected attribute as a target
                        processor = BaseProcessor()
                        processed_df = processor.process(class_df, scale_cols=cont_col_buff, encode_cols=(
                            cat_col_buff + [protected_attribute]))

                        cat_columns_ids = [processed_df.columns.get_loc(
                            col_name) for col_name in cat_columns]
                        sampler = SMOTENC(sampling_strategy=r,
                                          categorical_features=cat_columns_ids)
                        X_resampled, class_target_resampled = sampler.fit_resample(
                            processed_df, class_target)
                        ...
                        class_resampled, class_target_resampled = sampler.fit_resample(
                            class_df, class_target)
                        class_resampled.loc[:, target] = class_target_resampled
    ...
    def balance_all(self, dataset: pd.DataFrame, target: str, protected_attributes: list,
        ...
        Returns:
            ([pd.DataFrame, pd.DataFrame]): the balanced dataframes for the dataset and the target
        """
        df, t = self.balance_protected_attribute(
            dataset, target, protected_attributes, cont_columns, cat_columns)
        df.loc[:, target] = t
        df, t = self.balance_output_for_attribute(
            df, target, protected_attributes, cont_columns, cat_columns)
    ...
    def _highest_ratio(self, dataset: pd.DataFrame, target: str, classes: dict, protected_attribute: str):
        ...
        c_max = classes[0]
        for c in classes:
            r = dataset[dataset[protected_attribute] == c][target].value_counts(
            )[1]/dataset[dataset[protected_attribute] == c][target].value_counts()[0]
            if r > 1:
                r = 1/r
            if r > r_max:
                r_max = r
