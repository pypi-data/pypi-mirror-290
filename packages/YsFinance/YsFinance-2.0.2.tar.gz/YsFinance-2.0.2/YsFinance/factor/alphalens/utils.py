...
def infer_trading_calendar(factor_idx, prices_idx,benchmark=None):
    """
    ...
    if benchmark is not None:
        full_idx = full_idx.union(benchmark.index)
...
def compute_forward_returns(factor,
    ...
    freq = infer_trading_calendar(factor_dateindex, prices.index, benchmark)

    factor_dateindex = factor_dateindex.intersection(prices.index)
    if benchmark is not None:
        factor_dateindex = factor_dateindex.intersection(benchmark.index)
    ...
    for period in sorted(periods):
        if cumulative_returns:
            ...
            if benchmark is not None:
                returns_benchmark = benchmark.pct_change(period, fill_method=None)
        else:
            returns = prices.pct_change(fill_method=None)
            if benchmark is not None:
                returns_benchmark = benchmark.pct_change(fill_method=None)

        forward_returns = \
            returns.shift(-period).reindex(factor_dateindex)
        if benchmark is not None:
            forward_returns_benchmark = returns_benchmark.shift(-period).reindex(factor_dateindex)
            forward_returns =  forward_returns.apply(lambda x:x.values-forward_returns_benchmark.values)
        ...
        delta_days = period_len.components.days - mode(days_diffs).mode ##############changed .mode[0]
