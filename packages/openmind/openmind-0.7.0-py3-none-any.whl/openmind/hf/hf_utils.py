...
from ..utils.hub import OMValidationError
...
def get_wrapped_autoclass(
    ...
    ignore_pattens: Optional[Union[List[str], str]] = None,
    **kwargs,
):
            ...
            if not os.path.exists(pretrained_model_name_or_path):
                ...
                try:
                    local_dir = snapshot_download(
                        ...
                        ignore_patterns=kwargs.pop("ignore_patterns", ignore_pattens),
                        cache_dir=kwargs.pop("cache_dir", None),
                        force_download=kwargs.pop("force_download", None),
                    )
                    ...
                    error_msg = (
                        ...
                        f"the path to a local folder or the repo_id of a model on the Hub."
                    )
...
AutoFeatureExtractor = get_wrapped_autoclass(HfAutoFeatureExtractor, ignore_pattens=["*.h5", "*.msgpack"])
AutoImageProcessor = get_wrapped_autoclass(HfAutoImageProcessor, ignore_pattens=["*.h5", "*.msgpack"])
...
AutoModel = get_wrapped_autoclass(HfAutoModel, ignore_pattens=["*.h5", "*.msgpack"])
AutoModelForCausalLM = get_wrapped_autoclass(HfAutoModelForCausalLM, ignore_pattens=["*.h5", "*.msgpack"])
AutoModelForPreTraining = get_wrapped_autoclass(HfAutoModelForPreTraining, ignore_pattens=["*.h5", "*.msgpack"])
AutoModelForQuestionAnswering = get_wrapped_autoclass(
    HfAutoModelForQuestionAnswering, ignore_pattens=["*.h5", "*.msgpack"]
)
AutoModelForSequenceClassification = get_wrapped_autoclass(
    HfAutoModelForSequenceClassification, ignore_pattens=["*.h5", "*.msgpack"]
)
AutoModelForSeq2SeqLM = get_wrapped_autoclass(HfAutoModelForSeq2SeqLM, ignore_pattens=["*.h5", "*.msgpack"])
AutoModelForTokenClassification = get_wrapped_autoclass(
    HfAutoModelForTokenClassification, ignore_pattens=["*.h5", "*.msgpack"]
)
