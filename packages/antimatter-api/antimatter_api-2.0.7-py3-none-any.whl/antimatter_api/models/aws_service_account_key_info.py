...
class AWSServiceAccountKeyInfo(BaseModel):
    """
    ...
    """ # noqa: E501
    access_key_id: StrictStr = Field(description="The AWS access key ID.", alias="accessKeyID")
    secret_access_key: StrictStr = Field(description="The access key ID's secret access key.", alias="secretAccessKey")
    key_arn: StrictStr = Field(description="The key ARN to be used.", alias="keyARN")
    provider_name: Optional[StrictStr] = Field(default=None, alias="providerName")
    __properties: ClassVar[List[str]] = ["accessKeyID", "secretAccessKey", "keyARN", "providerName"]
    ...
    def provider_name_validate_enum(cls, value):
        ...
        if value not in set(['aws_sa']):
            raise ValueError("must be one of enum values ('aws_sa')")
        return value
