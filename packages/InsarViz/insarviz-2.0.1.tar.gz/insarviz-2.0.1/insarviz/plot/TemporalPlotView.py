...
from PyQt5.QtCore import Qt, pyqtSlot, pyqtSignal, QModelIndex, QPointF

from PyQt5.QtWidgets import (
    QCheckBox, QComboBox, QListView, QAbstractItemView, QSplitter, QAction, QToolBar, QLabel,
...
from insarviz.map.Selection import SelectionItem
...
class TemporalPlotWidget(AbstractPlotWidget):
    ...
    def on_data_loaded(self, band_index: int) -> None:
        ...
        # x-axis:
        if isinstance(self.plot_model.loader.dates[0], int):  # TODO ugly
    ...
    def add_points_curves(self, first: int, last: int) -> None:
        ...
        for i in range(first, last+1):
            ...
            color = self.plot_model.points.data(point_index, Qt.DecorationRole)
            ...
            curve.opts["name"] = self.plot_model.points.data(point_index, Qt.DisplayRole)
    ...
    def update_points_curves(self, first: int, last: int, roles: list[int]) -> None:
        for i in range(first, last+1):
            ...
            if Qt.EditRole in roles:
                # remove tooltip for this curve
                self.points_curves[i].sigPointsHovered.emit(self.points_curves[i], [], None)
                name = self.plot_model.points.data(point_index, Qt.DisplayRole)
    ...
    def update_profiles_curves(self, first: int, last: int, roles: list[int]) -> None:
        if first <= self.profile <= last:
            ...
            if Roles.CurveColorRole in roles:
                color = self.plot_model.profiles.data(profile_index, Qt.DecorationRole)
                self.profile_curve.setPen(color, width=2)
                self.profile_curve.setSymbolBrush(color)
            if Qt.EditRole in roles:
                # remove tooltip for this curve
                self.profile_curve.sigPointsHovered.emit(self.profile_curve, [], None)
                name = self.plot_model.profiles.data(profile_index, Qt.DisplayRole)
    ...
    def add_references_curves(self, first: int, last: int) -> None:
        ...
        for i in range(first, last+1):
            ...
            color = self.plot_model.references.data(reference_index, Qt.DecorationRole)
            ...
            curve.opts["name"] = self.plot_model.references.data(reference_index, Qt.DisplayRole)
    ...
    def update_references_curves(self, first: int, last: int, roles: list[int]) -> None:
        for i in range(first, last+1):
            ...
            if Qt.EditRole in roles:
                # remove tooltip for this curve
                self.references_curves[i].sigPointsHovered.emit(self.references_curves[i], [], None)
                name = self.plot_model.references.data(reference_index, Qt.DisplayRole)
...
class TemporalPlotWindow(AbstractPlotWindow):
    ...
    def __init__(self, plot_model: PlotModel):
        ...
        self.points_view.setSelectionMode(QAbstractItemView.NoSelection)
        ...
        self.profile_combobox.setSizePolicy(QSizePolicy.MinimumExpanding,
        ...
        self.profile_slider: QSlider = QSlider(Qt.Horizontal, self)
        ...
        self.references_view.setSelectionMode(QAbstractItemView.NoSelection)
        ...
        # layout
        h_splitter: QSplitter = QSplitter(Qt.Horizontal)
        h_splitter.setChildrenCollapsible(False)
        h_splitter.addWidget(self.plot_widget)
        v_splitter: QSplitter = QSplitter(Qt.Vertical)
    ...
    def profile_combobox_index_changed(self, i: int) -> None:
        if i == -1:
            ...
            if i == 0:
                self.profile_slider.setTickPosition(QSlider.NoTicks)
                ...
                if length <= 40:
                    self.profile_slider.setTickPosition(QSlider.TicksBelow)
                else:
                    self.profile_slider.setTickPosition(QSlider.NoTicks)
