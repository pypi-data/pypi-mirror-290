...
from PyQt5.QtGui import QMouseEvent, QWheelEvent, QPolygon, QPainter, QPen, QColor
...
class MinimapView(AbstractMapView):
    ...
    def initializeGL(self) -> None:
        ...
        self.program = create_program(
            create_shader(GL_VERTEX_SHADER, VIEWPORTRECT_VERT_SHADER),
            create_shader(GL_FRAGMENT_SHADER, VIEWPORTRECT_FRAG_SHADER)
        )
        glUseProgram(self.program)
        # set MapView's viewport rectangle color (RGBA)
        set_uniform(self.program, 'rect_color', (1., 1., 1., 1.))
        glUseProgram(0)
        self.doneCurrent()
    ...
    def paintGL(self) -> None:
        ...
        for layer in self.map_model.layer_model.layers:
            if isinstance(layer, MainLayer):
                layer.show(self.view_matrix, self.projection_matrix, blend=False)
        # white rectangle figuring MapView's viewport
        glUseProgram(self.program)
        set_uniform(self.program, 'model_matrix', self.mapview_viewport_matrix)
