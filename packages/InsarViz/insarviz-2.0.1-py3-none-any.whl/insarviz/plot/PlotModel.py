...
from PyQt5.QtCore import (
    Qt, QObject, pyqtSlot, pyqtSignal, QAbstractProxyModel, QModelIndex, QPersistentModelIndex,
    QAbstractItemModel
)
...
class PlotModel(QObject):
    ...
    closed = pyqtSignal()
    updated_pointer_info = pyqtSignal(tuple, np.ndarray)
    points_added = pyqtSignal(int, int)
    points_removed = pyqtSignal(int, int)
    points_updated = pyqtSignal(int, int, list)
    profiles_added = pyqtSignal(int, int)
    profiles_removed = pyqtSignal(int, int)
    profiles_updated = pyqtSignal(int, int, list)
    references_added = pyqtSignal(int, int)
    references_removed = pyqtSignal(int, int)
    references_updated = pyqtSignal(int, int, list)
    ...
    def update_pointer(self, pos: tuple, date_index: Optional[int]) -> None:
        ...
        date_index correspond to MapModel.i (starting from 1 like in gdal)
        """
        if pos == ():
            ...
            # TODO when using remote data, that means downloading a lot of unnecessary data
...
class ProxySelectionModel(QAbstractProxyModel):
    ...
    def flags(self, index: QModelIndex) -> Qt.ItemFlags:
        # pylint: disable=missing-function-docstring
        return super().flags(index) & (~Qt.ItemIsEditable)  # remove editable flag
    ...
    #  TODO cannot find correct pyqtSlot signature:
    # @pyqtSlot(QModelIndex, QModelIndex, list) not working
