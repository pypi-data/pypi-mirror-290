...
from OpenGL.GL import (
    GL_TEXTURE_1D, glBindTexture,
    glGenBuffers, glBindBuffer, glBufferData,
    GL_TEXTURE0, glActiveTexture,
    GL_RGB, GL_TEXTURE_MAG_FILTER, GL_TEXTURE_MIN_FILTER,
    GL_LINEAR, GL_PIXEL_UNPACK_BUFFER, GL_STREAM_DRAW,
    glGenTextures, glTexParameter,
    glDeleteBuffers, glTexImage1D,
    glPixelStorei, GL_UNPACK_ALIGNMENT, glGetIntegerv, GL_UNSIGNED_BYTE,
...
def create_colormap_texture(colormap: ColorMap, texture_unit: int) -> int:
    """
    ...
    if not all(np.isclose(stopsdiffs, stopsdiffs[0])):
        ...
        colormap_colors = colormap.getLookupTable(nPts=256, alpha=False, mode=ColorMap.BYTE
                                                  ).astype(np.uint8)
    else:
        # else we use the colors associated to stops (without the fourth alpha chanel)
        colormap_colors = colormap.getColors(mode=ColorMap.BYTE)[:, :3].astype(np.uint8)
    glActiveTexture(texture_unit)
    colormap_texid = glGenTextures(1)
    glBindTexture(GL_TEXTURE_1D, colormap_texid)
    ...
    pixel_buffer: int = glGenBuffers(1)
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, pixel_buffer)
    uint8_size = np.dtype(np.uint8).itemsize
    w, d = colormap_colors.shape
    glBufferData(GL_PIXEL_UNPACK_BUFFER, w*d*uint8_size, colormap_colors, GL_STREAM_DRAW)
    # colormaps rows are not 4 bytes aligned but 1 byte aligned
    # but OpenGL expects 4 bytes alignemnt, so we change the GL_UNPACK_ALIGNMENT parameter
    default_unpack_alignement: int = 0
    default_unpack_alignement = glGetIntegerv(GL_UNPACK_ALIGNMENT, default_unpack_alignement)
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1)
    glTexImage1D(GL_TEXTURE_1D, 0, GL_RGB, w, 0, GL_RGB, GL_UNSIGNED_BYTE, None)
    glPixelStorei(GL_UNPACK_ALIGNMENT, default_unpack_alignement)
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0)
    glDeleteBuffers(1, [pixel_buffer])
    glBindTexture(GL_TEXTURE_1D, 0)
    glActiveTexture(GL_TEXTURE0)
    return colormap_texid
