...
def build_vrt(src_files: Union[str, list[str]], bands: Union[None, int, list[int], list[list[int]]],
    ...
    """
    Build a VRT (Virtual Raster Dataset) #TODO
    ...
    bands : Union[None, int, list[int], list[list[int]]]
        A list of list of band numbers, one list of band numbers for each source file, of bands to
    ...
    dates : Optional[list[Optional[int]]], optional
        ...
        For example, for the 30th July 2018 : "20180730"
    ...
    if len(src_files) == 0:
        print("There should be at least 1 file to create a vrt.")
        return False
    if bands is None:
        bands = []
    ...
    if len(src_files) != len(bands):
        print("number of scr_files and number of bands not matching, abort")
        return False
    if dates is None:
        dates = [None for band in bands for _ in band]
    if sum(len(_) for _ in bands) != len(dates):
        print("number of bands and number of dates not matching, abort")
        return False
    ...
    if len(width) != 1:
        print(f"conflicting dataset widths found {width}, abort")
        return False
    rasterXSize = width.pop()
    if len(height) != 1:
        print(f"conflicting dataset heights found {height}, abort")
        return False
    rasterYSize = height.pop()
    if len(crs) != 1:
        print(f"conflicting dataset crs found {crs}, abort")
        return False
    if len(transform) != 1:
        print("conflicting dataset georeferencing transformation matrixes"
        ...
        with rasterio.open(src) as dataset, MemoryFile() as memfile:
            ...
            for target_band in target_bands:
                ...
                if len(found_bands) == 0:
                    print(f"did not find band {target_band} in file {src}, abort")
                    return False
                if len(found_bands) > 1:
                    print(f"found multiple band {target_band} in file {src}, abort")
                    return False
    ...
    if crs.pop() is not None:
        with rasterio.open(src_files[0]) as dataset, MemoryFile() as memfile:
            ...
            print(f"CRS and geotransorm from {src_files[0]} set as CRS and geotransform")
    ...
    if value_unit is not None:
        ...
        print(f"{value_unit} set as unit name")
    ...
    print(f"VRT file {out_filename} of {sum(len(_) for _ in bands)} bands created.")
    return True
