...
from OpenGL.GL import (
    ...
    glTexImage2D, glDeleteBuffers, glDeleteTextures,
)
...
from insarviz.map.Layer import Layer, MainLayer, Raster1BLayer, RasterRGBLayer, GeomapLayer
...
class MapModel(QObject):
    ...
    # (texture_unit, texture_target, tex_id)
    texture_changed = pyqtSignal(IntConstant, IntConstant, int)
    ...
    def __init__(self, loader: Loader, context: QOpenGLContext,
        ...
        self.tex_id: int = 0  # OpenGL texture id for data layer
        self.colormap: Optional[ColorMap] = None
        self.colormap_tex_id: int = 0  # OpenGL texture id for colormap
    ...
    def close(self) -> None:
        ...
        glDeleteTextures(1, [self.colormap_tex_id])
        self.colormap_tex_id = 0
    ...
    def show_band(self, i: int) -> None:
        ...
        try:  # looking up cache
            ...
            glBufferData(GL_PIXEL_UNPACK_BUFFER, w*h*2*float_size, z, GL_STREAM_DRAW)
            ...
            self.context.doneCurrent()
            assert texture_id, f"Map Model : cannot load image texture for band {i} in OpenGl"
            # store band texture param
            self.textures[i] = self.tex_id = texture_id
    ...
    def set_colormap(self, colormap: ColorMap) -> None:
        ...
        self.context.makeCurrent(self.offscreen_surface)
        glDeleteTextures(1, [self.colormap_tex_id])
        self.colormap_tex_id = create_colormap_texture(colormap, GL_TEXTURE0+PALETTE_UNIT)
        self.context.doneCurrent()
        self.texture_changed.emit(IntConstant("GL_TEXTURE0+PALETTE_UNIT", GL_TEXTURE0+PALETTE_UNIT),
                                  GL_TEXTURE_1D, self.colormap_tex_id)
        self.request_paint.emit()
