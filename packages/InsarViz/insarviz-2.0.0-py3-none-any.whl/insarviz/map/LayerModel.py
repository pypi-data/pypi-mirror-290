...
from PyQt5.QtCore import (
    Qt, pyqtSlot, pyqtSignal, QModelIndex, QMimeData, QByteArray
)
...
from insarviz.map.Layer import Layer
...
class LayerModel(TreeModel):
    ...
    def canDropMimeData(self, data: QMimeData, action: Qt.DropAction, row: int, column: int,
        ...
        if not parent.isValid() or (not parent.parent().isValid() and row == -1 and column == -1):
            if data.hasFormat('application/insarviz.layer') and action == Qt.MoveAction:
                return True
            elif data.hasFormat('image/tiff') and action == Qt.CopyAction:
                return True
            else:
                return False
        else:
    ...
    def dropMimeData(self, data: QMimeData, action: Qt.DropAction, row: int, column: int,
        ...
        if data.hasFormat('application/insarviz.layer'):
            ...
            return True
        elif data.hasFormat('image/tiff'):
            # TODO tiff case
...
class SetDataCommand(QUndoCommand):
    ...
    def __init__(self, layer_model: LayerModel, index: QModelIndex, value: Any, role: int,
        ...
        if index.data(Roles.EditorRole) is not None:
            ...
            self.setText(
                f"Change {index.data(Qt.ToolTipRole)} of {index.parent().data(Qt.DisplayRole)}")
        else:
            self.setText(f"Change name of {index.data(Qt.DisplayRole)}")
        self.layer_model: LayerModel = layer_model
        self.lineage: list[int] = layer_model.get_lineage(index)
        self.new_value: Any = value
        self.role: int = role
        self.old_value: Any
        if index.data(Roles.EditorRole) == "integer":
            self.old_value = self.layer_model.data(index, role)[0]
        elif index.data(Roles.EditorRole) == "colormap":
            old_colormap = self.layer_model.data(index, Qt.DisplayRole)
            layer = self.layer_model.data(index, Qt.EditRole)
