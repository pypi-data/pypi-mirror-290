...
class Loader(QObject):
    ...
    def open(self, filepath: str) -> None:
        ...
        try:
            self.dates = [datetime.datetime.strptime(d, "%Y%m%d")
                          for i in self.dataset.indexes
                          for d in self.dataset.tags(i)["DATE"]]
    ...
    def compute_histograms(self, binrange: Optional[tuple[float, float]] = None) -> None:
        """
        _summary_
        ...
        t0 = time.time()
        progress_dialog = QProgressDialog("Computing histograms...", None, 0, 100)
        progress_dialog.setWindowTitle(f"Opening: {os.fspath(os.path.basename(self.filepath))}")
        ...
        if len(set(self.dataset.block_shapes)) == 1:
            ...
            if self.dataset.interleaving == rasterio.enums.Interleaving.pixel:
                ...
                xy = xy[np.lexsort((xy[:, 1] // block_width, xy[:, 0] // block_height, xy[:, 2]))]
            else:
        ...
        for i in range(4):
            ...
            progress_dialog.setValue(10 + (i+1)*2)
        ...
        if binrange is None:
            ...
            # [q25 - outlier_threshold*IQR, q75 + outlier_threshold*IQR]
            binrange = (q25 - self.outlier_threshold*IQR, q75 + self.outlier_threshold*IQR)
        ...
        bins_band = [np.array([bins[0], *bins, bins[-1]]) for _ in range(len(self))]
        ...
        histograms_band = [np.zeros(bins_band[i].shape[0]-1) for i in range(len(self))]
        with warnings.catch_warnings():
            ...
            if self.dataset.interleaving == rasterio.enums.Interleaving.pixel:
                ...
                block = np.empty((len(self), *self.dataset.block_shapes[0]))
                ...
                for k, window in enumerate(self.dataset.block_windows()):
                    ...
                    progress_dialog.setValue(20 + (80 * k) // block_count)
            else:
                ...
                for i, idx in enumerate(self.dataset.indexes):
                    ...
                    progress_dialog.setValue(20 + (80 * i) // band_count)
        ...
        self.total_histogram = (histogram_total, bins_total)
        self.band_histograms = list(zip(histograms_band, bins_band))
