...
from eis.insurance import rest
from eis.insurance.configuration import Configuration
from eis.insurance.exceptions import ApiTypeError, ApiValueError, ApiException, UnauthorizedException
from eis.insurance.model_utils import (
    ModelNormal,
    ModelSimple,
    ModelComposed,
...
class ApiClient(object):
    ...
    def __init__(self, configuration=None, header_name=None, header_value=None,
        ...
        self.user_agent = 'OpenAPI-Generator/1.43.0/python'
    ...
    def request(self, method, url, query_params=None, headers=None,
        ...
        try:
            ...
            return self.process_unauthorized_exception(ue, method, url, query_params, headers,
                                                post_params, body, _preload_content,_request_timeout)

        except Exception as e:
            raise e
    
    def process_unauthorized_exception(self, ue: UnauthorizedException, 
        ...
        if self.status == Status.INVALID:
            print('Failed login... Credentials are INVALID.')
            raise ue

        headers = self.update_headers(headers)

        return self.request(method, url, query_params, headers, 
                    post_params, body, _preload_content, 
                    _request_timeout)
        
    def update_headers(self, headers=None):
        headers['Authorization'] = "Bearer " + self.configuration.access_token
        return headers

    def read_env_variables(self):
        try:
            username = os.environ['EMIL_USERNAME']
            password = os.environ['EMIL_PASSWORD']

            return username, password

        except Exception:
            raise Exception('EMIL_USERNAME is not set. Please set your environment variables before running.')
    
    def login(self):
        username, password = self.read_env_variables()

        url = self.configuration.host + "/authservice/v1/login"
        header_params = {}
        header_params.update(self.default_headers)
        ...
        try:
            r = self.request("POST", url=url, headers=header_params, body=body)
            login_response = json.loads(r.data.decode('utf8'))
            self.configuration.access_token = login_response['accessToken']
...
class Endpoint(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        api_instance = BookingFunnelVersionsApi()
        api_instance.get_booking_funnel_version  # this is an instance of the class Endpoint
        api_instance.get_booking_funnel_version()  # this invokes api_instance.get_booking_funnel_version.__call__()
        which then invokes the callable functions stored in that endpoint at
        api_instance.get_booking_funnel_version.callable or self.callable in this class
