...
impl DirectoryManager {
    ...
    fn find_in_file(&self, sub_string: &str, fuzzy_search: bool, extension: Option<Vec<String>>) -> PyResult<Vec<File>> {
        let mut matched_files = Vec::new();

        for file in &self.files {
            let should_process_file = match &extension {
                Some(exts) => exts.contains(&file.extension),
                None => true, // Process all files if no extension is specified
            };
            ...
            if should_process_file {
                ...
                if fuzzy_search {
                    ...
                    if matcher.fuzzy_match(&content, sub_string).is_some() {
                        matched_files.push(file.clone());
                    }
                } else {
