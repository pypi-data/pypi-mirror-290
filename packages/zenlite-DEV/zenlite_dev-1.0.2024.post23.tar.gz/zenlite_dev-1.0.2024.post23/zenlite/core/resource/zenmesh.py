...
class zenmeshbank(zenbank):
    ...
    def rem_mesh(self, mesh_id:int) -> None:
        if self.count-1 >= 0 and mesh_id <= self.max and mesh_id != -1:
            self.vbo.pop(mesh_id)
            self.vao.pop(mesh_id)
            self.shader.pop(mesh_id)
            self.texture.pop(mesh_id)
            self.m_model.pop(mesh_id)
            self.vformat.pop(mesh_id)
            self.vertices.pop(mesh_id)
            self.vattribs.pop(mesh_id)
