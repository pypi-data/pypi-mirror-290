    ...
    with (
        ...
        try:
            ...
            resolver = resolver_type(
                ...
                parse=None,
                resource_hint=None,
                total=parsed.comment_dict.total,
...
class GeneratedSQLFileResolverRegistry:
    def __init__(self):
        super().__init__()
        self.filepath_to_sql_string: Dict[str, Tuple[str, CommentDict]] = {}

    def add_sql_file_resolver(self, filepath: str, sql_string: str, comment_dict: CommentDict):
        if filepath in self.filepath_to_sql_string:
            raise ValueError(f"A SQL file resolver already exists at {filepath}")
        self.filepath_to_sql_string[filepath] = (sql_string, comment_dict)

    def get_generated_sql_file_resolvers(self) -> Iterable[SQLStringResult]:
        for filepath, sql_string_and_comment_dict in self.filepath_to_sql_string.items():
            sql_string, comment_dict = sql_string_and_comment_dict
            yield SQLStringResult(
                path=filepath, sql_string=sql_string, error=None, override_comment_dict=comment_dict, autogenerated=True
            )
...
def make_sql_file_resolver(
    ...
    See https://docs.chalk.ai/docs/sql#sql-file-resolvers for more information on SQL file resolvers.
    ...
    source
        ...
        If a string is provided, it will be used to infer the source by first scanning for a source with the same name,
        then inferring the source if it is a type, e.g. `snowflake` if there is only one database of that type.
        Optional if `source` is specified in `sql`.
    resolves
    ...
    timeout
        ...
        Read more at https://docs.chalk.ai/docs/timeout and https://docs.chalk.ai/docs/duration
    cron
    ...
    fields
        ...
        `txn_id: id`.
    """
    filename = _get_generated_sql_resolver_filepath(name)
    comment_dict = CommentDict(
    ...
    _GENERATED_SQL_FILE_RESOLVER_REGISTRY.add_sql_file_resolver(filename, sql, comment_dict)


def _get_generated_sql_resolver_filepath(filename: str):
    if filename.endswith(".chalk.sql"):
        return filename
    elif filename.endswith(".sql"):
        return f"{filename[:-4]}.chalk.sql"
    return f"{filename}.chalk.sql"
...
def _convert_incremental_settings(settings: IncrementalSettings) -> IncrementalSettingsSQLFileResolver:
    return IncrementalSettingsSQLFileResolver(
        ...
        lookback_period=timedelta_to_duration(settings.lookback_period)
        if settings.lookback_period is not None
        else None,
        mode=settings.mode,
