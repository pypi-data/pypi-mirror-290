...
class ResolverProtocol(Protocol[P, T_co]):
    ...
    resource_hint: Literal["cpu", "io"] | None
...
class Resolver(ResolverProtocol[P, T], abc.ABC):
    def __init__(
        ...
        default_args: list[ResolverArgErrorHandler | None] | None,
        owner: str | None,
    ...
    @property
    def function_definition(self) -> str:
        if self._function_definition is None:
            self._do_parse()
        assert self._function_definition is not None
    ...
    @property
    def default_args(self) -> list[ResolverArgErrorHandler | None]:
...
def parse_function(
    ...
    def f():
        ...
        if (inspect.isgeneratorfunction(fn) or inspect.isasyncgenfunction(fn)) and not (
            ...
            and issubclass(return_annotation, DataFrame)  # pyright: ignore[reportUnnecessaryIsInstance]
        ):
        ...
        if ret_val is None and not ignore_return:
            ...
            elif issubclass(return_annotation, DataFrame):  # pyright: ignore[reportUnnecessaryIsInstance]
...
def online(
    *,
    ...
    name: str | None = None,
    resource_hint: Literal["cpu", "io"] | None = None,
    static: bool = False,
...
def online(
    ...
    *,
    ...
    static
        Whether this resolver should be invoked once during planning time to build a static computation graph.
        If True, all inputs will either be StaticOperators (for has-many and dataframe relationships) or StaticExpressions
        (for individual features). The resolver must return a StaticOperator as output.
    Other Parameters
    ----------------
    fn
...
def offline(
    ...
    *,
    ...
    def decorator(fn: Callable[P, T]):
        ...
        error_builder = get_resolver_error_builder(fn)
        parse_fn = parse_function(
            fn=fn, glbs=caller_globals, lcls=caller_locals, error_builder=error_builder, validate_output=True
        )
