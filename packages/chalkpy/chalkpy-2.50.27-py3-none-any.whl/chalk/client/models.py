...
if TYPE_CHECKING:
    ...
    try:
        from pydantic.v1 import BaseModel, Field, fields, root_validator, validator
    except ImportError:
        from pydantic import BaseModel, Field, fields, root_validator, validator
...
class ResourceRequests(BaseModel):
    ...
    @validator(
        "memory", "ephemeral_volume_size", "ephemeral_storage"
...
class CreateOfflineQueryJobRequest(BaseModel):
    ...
    def _validate_multiple_computers(cls, values: Dict[str, Any]):
        if values["input"] is None or isinstance(values["input"], UploadedParquetShardedOfflineQueryInput):
            return values
