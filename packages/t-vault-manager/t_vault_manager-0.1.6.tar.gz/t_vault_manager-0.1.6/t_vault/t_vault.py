...
class Vault(ABC):
    ...
    @abstractmethod
    def get_attachment(self, item_name: str, attachment: Attachment, file_path: str | None) -> str:
...
class Bitwarden(Vault):
    ...
    def login_from_env(self):
        ...
        if not self.vault_items or not self.is_vault_unlocked:
            raise exceptions.VaultError("Failed to initialize the vault.")

    def login(self, client_id: str, client_secret: str, password: str, username: str = ""):
