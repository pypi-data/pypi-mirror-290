...
class tShift_LLM:
    def __init__(self, clients: List[LiteLLMClient], context_window: int):
    ...
    def log_request(self, client: LiteLLMClient, messages: List[Dict[str, str]], start_time: float,
        ...
        log_entry = {
            ...
            "client_model": str(client.model),  # Convert to string to avoid JSON serialization issues
            "messages": messages,
            "duration": end_time - start_time,
            "status": status
        }
        if response:
            log_entry["response"] = response.dict() if hasattr(response, 'dict') else str(response)
        if error:
    ...
    def completion(self, messages: List[Dict[str, str]], **kwargs) -> Any:
        for _ in range(len(self.clients)):
            ...
            try:
                ...
                print(f"Error with {client.model}: {str(e)}. Shifting to next model.")
