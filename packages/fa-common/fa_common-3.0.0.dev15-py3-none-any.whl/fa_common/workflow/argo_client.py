...
from .enums import ArgoWorkflowStoreType, JobStatus, JobSubmitMode
from .models import (
    ArgoWorkflowRun,
    GetWorkflowRes,
    JobTemplate,
...
class ArgoClient(WorkflowBaseClient):
    ...
    async def submit(self, argo_workflow, namespace: str, verify_ssl=True) -> WorkflowRun:
        """
        ...
        try:
            ...
            workflow = WorkflowRun(name=detail.metadata.name, mode=JobSubmitMode.ISOLATED, detail=detail)

            return workflow.on_submit_res()

        except ApiException as err:
            LOG.warning(f"Workflow submission Error caught: {err}")  # , retrying in 5 secs: {err}")
            raise err

    async def run_job(self, job_base: JobTemplate) -> WorkflowRun:
