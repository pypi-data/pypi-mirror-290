...
class EvaluationSet:
    def __init__(self, id: str = None, name: str = None, description: str = None, system_prompt: str = None, scheduler: str = None, testset_id: str = None, create_testset: bool = False, num_questions: int = 10, knowledge_base: pd.DataFrame = None, testset_name: str = None):
        if id is None:
            if name is None or description is None or system_prompt is None:
                raise ValueError("Name, description, and system_prompt are required when creating a new EvaluationSet")
    ...
    def _update(self):
        ...
        try:
            NeuralTrustApiService.update_evaluation_set(self.id, evalset_data)
        except Exception as e:
            raise
    ...
    def _load_existing_evaluation_set(self):
        ...
        self.testset_id = evalset_data.get("testset_id", None)
        ...
        self.num_questions = evalset_data.get("num_questions", 5)
    ...
    def run(self, max_parallel_evals: int = 5) -> Dict:
        if self.create_testset:
            ...
            dataset = Loader().load_json(f".data/{self.testset_id}.json")
        else:
    ...
    def _generate_testset(self, knowledge_base: pd.DataFrame, num_questions: int, agent_description: str) -> str:
        ...
        testset.save(path=f".data/{self.testset_id}.json")
    ...
    def _run_target(self, data: DataPoint) -> DataPoint:
        ...
        if data['conversation_history'] is not None:
            conversation_history = [ChatMessage(role=msg['role'], content=msg['content']) for msg in data['conversation_history']]
