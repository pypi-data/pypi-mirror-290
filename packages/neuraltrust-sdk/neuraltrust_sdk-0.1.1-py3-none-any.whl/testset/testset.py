...
class Testset:
    ...
    @staticmethod
    def add_testset_rows(testset_id: str, rows: List[TestsetRow]):
        """
        Adds rows to a testset in batches of 100.

        Parameters:
        - testset_id (str): The ID of the testset to add rows to.
        - rows (List[TestsetRow]): The rows to add to the testset.
        ...
        for i in range(0, len(rows), batch_size):
            ...
            try:
                NeuralTrustApiService.add_testset_rows(testset_id, batch)
            except Exception as e:
                raise
