...
Context is a singly-linked list of context variable dictionaries. Each scope points to a parent scope. Scopes
are pushed and popped by the ``context_scope`` decorator. Scopes can shadow each other's keys.
...
Cell = tuple[dict, Union[None, "Cell"]]

# Context needs to remain private so the data type held by it can change.
_CONTEXT: ContextVar[Optional[Cell]] = ContextVar("_CONTEXT", default=None)
