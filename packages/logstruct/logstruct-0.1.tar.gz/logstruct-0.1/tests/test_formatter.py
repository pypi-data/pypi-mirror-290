...
@pytest.fixture(params=[logging.getLogger, logstruct.getLogger])
...
def send_logs(logger: logging.Logger | logstruct.StructuredLogger) -> ExpectedLogContents:
    ...
    logger.error("An info message with data", extra={"struct": "log", "unrepresentable": {1, 2, 3}})
    try:
        print(1 / 0)
...
An info message with data {{"struct": "log", "unrepresentable": "{{1, 2, 3}}"}}
