...
class Response(str):
    ...
    def json(self):
        ...
        try:
            ...
            if self:
                raise errors.RequestError(f'Malformed JSON: {str(self)!r}: {e}')
            else:
                raise errors.RequestError('Malformed JSON: Empty string')
    ...
    async with client:
        ...
        async with request_lock:
            ...
            try:
                ...
                try:
                    ...
                    if response.status_code not in (301, 302, 303, 307, 308):
                        raise errors.RequestError(
                            f'{url}: {utils.html.as_text(response.text)}',
                            url=url,
                            text=response.text,
                ...
                raise errors.RequestError(
                    f'{url}: Timeout after {timeout} second'
                    + ('s' if timeout != 1 else '')
                )

            except ssl.SSLError as e:
                if hasattr(e, 'verify_message'):
                    # E.g. "certificate has expired"
                    msg = e.verify_message.capitalize()
                elif hasattr(e, 'reason'):
                    # E.g. "CERTIFICATE_VERIFY_FAILED"
                    msg = e.reason
                elif e.strerror:
                    # `strerror` is inherited from OSError and should be None if
                    # not provided
                    msg = e.strerror
                else:
                    msg = str(e)
                raise errors.RequestError(f'{url}: TLS: {msg}')

            except httpx.HTTPError as e:
                _log.debug(f'Unexpected HTTP error: {e!r}')
                msg = re.sub(r'^\[Errno -?\d+\]\s*', r'', str(e))
                raise errors.RequestError(f'{url}: {msg}')
...
def _save_permanent_cookies(client, filepath, domain):
    ...
    for cookie in client.cookies.jar:
        if domain.endswith(cookie.domain):
            ...
            _log.debug('Ignoring cookie because %r does not end with %r: %r', domain, cookie.domain, cookie)
