...
class RadioList:
    ...
    :param question: Text that is displayed above the `options` or any falsy
        value
    :param focused: Focused index or option `None` to focus the first option
    ...
    def __init__(self, options=(), question=None, focused=None, autodetected=None, on_accepted=None):
        ...
        if autodetected:
            self.autodetected_index = options.index(autodetected)
        else:
            self.autodetected_index = None
        ...
        def _(event):
            if self.on_accepted is not None:
                try:
                    ...
                    raise RuntimeError(f'No option at index {self.focused_index}: {self.options}')
