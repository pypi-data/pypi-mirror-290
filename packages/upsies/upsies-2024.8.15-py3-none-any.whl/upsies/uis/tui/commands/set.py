...
def _make_pretty_option_list():
    ...
    for option in defaults.option_paths():
        ...
        if option_type_name == 'int':
            ...
            text.append(f'  Options: {", ".join(default_value.options)}')
...
class set(CommandBase):
    ...
    argument_definitions = {
        ...
        ('--dump', '-d'): {
            'type': utils.argtypes.one_of(tuple(defaults.defaults)),
            'nargs': '*',
            'metavar': 'FILE',
            'help': (
                ...
                f'FILE may be ' + ", ".join(sorted(f'"{x}"' for x in defaults.defaults)) +
                ' or omitted to write all configuration files.\n'
