...
class PtpTracker(TrackerBase):
    ...
    def _get_anti_csrf_token(self, response):
        ...
        try:
            ...
            raise errors.RequestError(
                'Looks like you have 2FA enabled? 2FA is not supported.\n'
                'Feel free to make a PR or report this as a bug if you see this with 2FA disabled.'
            )
    ...
    async def get_movie_metadata(self, imdb_id):
        ...
        if imdb_id:
            ...
            # We expect a list of JSON objects that contains exactly one JSON
            # object.
            ...
            # If `imdb_id` is invalid/unknown, "title" is `null` in the JSON
            # response, which is translated to `None`.
        ...
        try:
            response = await self._request(
                ...
                data={
                    ...
                    'AntiCsrfToken': self._anti_csrf_token,
                },
                cache=True,
            )
        except errors.RequestError as e:
            if 'not found' in str(e):
                raise errors.RequestedNotFoundError(artist)
            else:
                raise e
        else:
    ...
    def _get_artist_dict(self, response):
        ...
        try:
            ...
            if artist.get('Message'):
                raise errors.RequestError(artist['Message'])
            else:
                raise errors.RequestError(f'Unexpected response: {artist}')
