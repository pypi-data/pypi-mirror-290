...
class MtvTracker(TrackerBase):
    ...
    async def _get_auth(self):
        ...
        if not hasattr(self, '_auth'):
            ...
            # There is a limit for unchecked torrents. Of course, it is not
            # presented like any other error message and there is no
            # straightforward way to find in the HTML soup.
    ...
    def _raise_error(self, doc, msg_prefix, tracker_jobs=None):
        ...
        # TODO: Remove this if the "zero bytes response / GroupID cannot be
        # null" bug is fixed. Also remember to remove the then unnecessary
        # `tracker_jobs` argument.
        if not doc.string:
            bug_report = ' '.join((
