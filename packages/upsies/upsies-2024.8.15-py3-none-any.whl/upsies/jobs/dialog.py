...
class ChoiceJob(JobBase):
    ...
    def options(self, options):
        ...
        if prev_focused:
            ...
            for index, (label, value) in enumerate(valid_options):
                if value == prev_value:
    ...
    def autodetected_index(self):
        """Index of autodetected option in :attr:`options` or `None`"""
        return getattr(self, '_autodetected_index', None)

    @property
    def autodetected(self):
        """
        Autodetected option (2-tuple) or `None`
        ...
        if autodetected_index is not None:
            return self.options[autodetected_index]
    ...
    def _set_choice(self, choice):
        ...
        if hasattr(self, '_choice'):
            ...
            label, value = self.get_option(choice)
            self._choice = value
    ...
    def make_choice(self, thing):
        ...
        """
        label, value = self.get_option(thing)
        self.send(label)
        self.finalize()
    ...
    def initialize(self, *, name, label, options, question=None,
                   focused=None, autodetected=None,
                   autodetect=None, autofinish=False):
        """
        Set internal state
    ...
    async def run(self):
        ...
        if self._autofinish and self.autodetected is not None:
...
class TextFieldJob(JobBase):
    ...
    def initialize(self, *, name, label,
        ...
        self._run_arguments = (text, default, finish_on_success, nonfatal_exceptions)

    async def run(self):
        text, default, finish_on_success, nonfatal_exceptions = self._run_arguments
    ...
    def send(self, output):
        ...
        try:
            ...
            # Send normalized and validated output.
            super().send(output)
            self.finalize()
