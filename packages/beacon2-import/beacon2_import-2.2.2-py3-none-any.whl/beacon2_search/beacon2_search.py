...
def common_arguments(parser):
    ...
    advance_connection_group.add_argument("-A", "--db-auth-config", type=str, default="", dest="database_auth_config", help="JSON file containing credentials/config e.g.{'db_auth_source':'admin','db_user':'root','db_password':'example'}")
...
def connect_to_mongodb(args):
    if args.advance:
        advanced_required_args = ['database_auth_config']
        if any(getattr(args, arg) == "" for arg in advanced_required_args):
            ...
            parser.print_help()
            sys.exit(1)
        credentials =  Credentials.from_file(args.database_auth_config)
        client = MongoClient(f"mongodb://{credentials.db_user}:{credentials.db_password}@{args.database_host}:{args.database_port}/{args.database}?authSource={credentials.db_auth_source}")
    else:
...
def beacon_query():
    ...
    optional_query_group.add_argument("-g", "--gene", type=str, default="", dest="gene", help="Gene name e.g. 'AL645728.1' .")
    optional_query_group.add_argument("-vt", "--variantType", type=str, default="", dest="variantType", help="Variant Type 'DEL' or 'DUP'")
    optional_query_group.add_argument("-p", "--primarySite", type=str, default="", dest="primarySite", help="Primary Site, e.g. 'brain'")
    optional_query_group.add_argument("-dt", "--diseaseType", type=str, default="", dest="diseaseType", help="Disease Type, e.g. 'adnexal and skin appendage neoplasms'")
    ...
    # If no optional arguments were provided for tools with no positional arguments
    if args.command in ["analyses", "cnv", "runs", "individuals", "datasets", "cohorts", "biosamples" ]:
        if not any(vars(args).get(arg) for arg in vars(args) if arg not in ["command", "database", "collection", "database_host", "database_port", "advance", "database_auth_config"]):
            print(f"No optional arguments provided for the {args.command} command. Please provide at least one optional argument. Use -h or --help for usage details.")
    ...
    if args.command == "sequence":
        ...
        query = {
            ...
            "variation.location.interval.start.value": {"$gte": args.start} if args.start is not None else None,
        ...
        query = {
            "variation.location.sequence_id": args.referenceName,
            "variation.location.interval.start.value": {"$gte": args.start} if args.start is not None else None,
            "variation.location.interval.end.value": {"$lte": args.end} if args.end is not None else None,
    ...
    # query individuals collection
        ...
        query = {
            ...
            "individualId": args.individualId,
            "definitions.Location.start": {"$gte": args.start} if args.start is not None else None,
            "definitions.Location.end": {"$lte": args.end} if args.end is not None else None,
    ...
    if results:
        ...
        print("No records found that match the provided criteria. Please try different parameters.")
