...
class KakaoWebtoonScraper(Scraper[int]):
    ...
    def from_url(cls, url: str, cookie: str | None = None) -> Self:
        try:
            webtoon_id, seo_id = cls._extract_webtoon_id(furl(url))
        except Exception as e:
            raise InvalidURLError.from_url(url, cls) from e

        if webtoon_id is None or seo_id is None:
    ...
    @classmethod
    def _extract_webtoon_id(cls, url) -> tuple[int | None, str | None]:
        if url.host != "webtoon.kakao.com":
            return None, None
        matched = re.match(r"/content/(?P<seo_id>[^/]+)/(?P<webtoon_id>\d+)", str(url.path))
        if not matched:
            return None, None
        return int(matched["webtoon_id"]), matched["seo_id"]
