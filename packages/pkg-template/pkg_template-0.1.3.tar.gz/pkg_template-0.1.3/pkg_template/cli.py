...
def django_create_template(template_name: str, app_name="main"):
    ...
    os.chdir(project_path)
            #fix the app.py 
    with open(os.path.join(project_path, project_path.split("\\")[-1], "urls.py"), "r+") as f:
        file_text = f.read()
        file_text = file_text.replace("MainConfig", f"{app_name[0:1].upper()}{app_name[1::]}Config")
...
def create_app(app_name: str, project_path=os.getcwd()):
    ...
    if os.path.exists(data_path):
        ...
        new_path_pattern = f'"", include("{app_name}.urls")'
        ...
        with open(settings_path, "r+") as f:
            ...
            app_list.append(f"'{app_name}.apps.{app_name[0:1].upper()}{app_name[1::]}Config'")

            print(app_list) # -> ["'django.contrib.admin'", "'django.contrib.auth'", "'django.contrib.contenttypes'", "'django.contrib.sessions'", "'django.contrib.messages'", "'django.contrib.staticfiles'", "'iio.apps.IioConfig'"]
            ...
            f.write(file_text)
            f.truncate() 
        # config the app in urls.py (not tested)

        with open(urls_path, "r+") as f:
            # Read the entire file content
            file_text = f.read()

            # Find the position of the urlpatterns list
            start_index = file_text.find("urlpatterns = [") + len("urlpatterns = [")
            end_index = file_text.find("]", start_index)


            # Extract the current list of URL patterns
            urlpatterns_str = file_text[start_index:end_index].strip()

            # Convert the URL patterns to a list
            urlpatterns_list = [url.strip() for url in urlpatterns_str.split(",") if url.strip()]

            # Add the new URL pattern
            urlpatterns_list.append(f"path({new_path_pattern})")

            # Rebuild the urlpatterns string
            new_urlpatterns = ",\n    ".join(urlpatterns_list)

            # Replace the old urlpatterns list with the new one
            file_text = file_text[:start_index] + "\n    " + new_urlpatterns + "\n" + file_text[end_index:]
