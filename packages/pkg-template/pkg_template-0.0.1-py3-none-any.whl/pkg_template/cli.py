...
def main():    
    parser = argparse.ArgumentParser(description="A description of what your package does.")
    
    #add arguments
    parser.add_argument("-c", "-create-template", type=str, help="create the template with name")
    parser.add_argument("-v", "--version", action="version", version="pkg_template 0.0.1")
    
    args = parser.parse_args()

    if args.c:
        final_directory = f"{os.getcwd()}/{args.c}"
        if not os.path.exists(final_directory):
            print(final_directory)
            
            os.makedirs(final_directory)
            os.makedirs(f"{final_directory}/{args.c}")
...
"""
        ...
        with open(f"{final_directory}/setup.py", "w") as f:
                f.write((the_setup.replace("pkg_template", args.c)))
        with open(f"{final_directory}/README.md", "w") as f:
            f.write(" ")
        with open(f"{final_directory}/{args.c}/__init__.py", "w") as f:
            f.write(" ")
        with open(f"{final_directory}/{args.c}/cli.py", "w") as f:
            f.write(" ")
