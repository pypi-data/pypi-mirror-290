...
def main():    
    ...
    parser.add_argument("-v", "--version", action="version", version="pkg_template 0.1.1")
    
    args = parser.parse_args()

    if args.create_template:
        final_directory = f"{os.getcwd()}/{args.create_template}"
        final_data_path = f"{final_directory}/{args.create_template}/data"
...
setup(
    ...
    package_data={
        'mypkg': ['data/*'],  # Include all files in the 'data' directory (if you don't have any data you can delete it)
    },
    entry_points={
