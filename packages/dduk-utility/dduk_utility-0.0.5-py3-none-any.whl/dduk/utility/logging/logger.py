...
class Logger:
	...
	def __init__(self, name : str) -> None:
		...
		self.__name = name
		self.__internalLogger = logging.getLogger(self.__name)
	...
	#--------------------------------------------------------------------------------
	def Start(self, logType : LogType = LogType.DEBUG, logPath : str = EMPTY):

		useLogFile : bool = logPath is not EMPTY
		timestamp : str = GetTimestampString(EMPTY, EMPTY, EMPTY, True, EMPTY)
		logFilePath : str = f"{logPath}/{self.__name}-{timestamp}.log"
		...
		if useLogFile:
			...
			internalName = logType.GetInternalName()
