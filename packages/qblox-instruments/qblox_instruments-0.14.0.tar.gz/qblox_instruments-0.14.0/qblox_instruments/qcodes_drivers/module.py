...
def _add_rtp_qcodes_params(parent: Union[Instrument, Module]):
    if not parent.is_qcm_type and not parent.is_qrm_type:
        raise TypeError("RTP parameters can only be declared for QRM and QCM modules.")
                ...
                parent.add_parameter(
                    ...
                    set_cmd=partial(
                        lambda output, val: parent.parent._set_pre_distortion_config(
                            parent.slot_idx,
                            {
                                f"out{2 * output}": {"state": {f"stage{i + 1}": val}},
                                f"out{2 * output + 1}": {
                                    "state": {f"stage{i + 1}": val}
                                },
                            },
                        ),
                        output,
