...
class BaseWfsReader(ABC):
    ...
    """ Read wave functions or density matrices from GPAW wave functions dump file
    ...
    filter_times
        A list of times to read in au. The closest times in the wave function dump file
        will be read. Applied after skipping the stridet steps to tmax.
    log
        Logger
    """
    ...
    def time_t(self) -> NDArray[np.float64]:
        """ List of times in au that will be read """
        return self._time_t[self._flt_t]

    @property
    def nt(self) -> int:
        """ Number of times there are to read """
    ...
    def iread(self, *args, **kwargs) -> Generator[DensityMatrixBuffer, None, None]:
        """ Read the density matrices or wave functions time by time
        """
...
class KohnShamRhoWfsReader(BaseWfsReader):

    """ Read density matrices from GPAW wave functions dump file
    ...
    def C0S_sknM(self) -> NDArray[np.float64]:
        if self._C0S_sknM is None:
            self.log(f'(on {self.comm.size} ranks) Constructing C0_sknM', rank=0)
            self._C0S_sknM = read_C0S_parallel(self.ksd.reader, comm=self.comm)
            self.log(f'(on {self.comm.size} ranks) Constructed C0_sknM', rank=0)
...
class LCAORhoWfsReader(BaseWfsReader):
    ...
    """ Read density matrices in the LCAO basis from GPAW wave functions dump file
    ...
    def rho0_skMM(self) -> NDArray[np.float64]:
        if self._rho0_skMM is None:
            self.log(f'(on {self.comm.size} ranks) Constructing rho0_skMM', rank=0)
            self._rho0_skMM = calculate_rho0_parallel(self.f_skn, self.C0_sknM, comm=self.comm)
            self.log(f'(on {self.comm.size} ranks) Constructed rho0_skMM', rank=0)
...
class WfsReader(BaseWfsReader):
    ...
    """ Read wave function LCAO coefficients from GPAW wave functions dump file
...
def prepare_wave_function_readers(mainreader,
    ...
    for roott in readrange:
        ...
        if t < readerlen:
            if t % 100 == 0:
                log(f'Opened item #{t}', flush=True)
    ...
    if comm.rank == 0:
        ...
        for t in range(readerlen):
            ...
            if curtime <= lasttime:
                log(f'Negative time difference at #{t} ({curtime:.1f} <= {lasttime:.1f}). Skipping', flush=True)
                continue
