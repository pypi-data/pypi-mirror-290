...
from ..density_matrices.time import (ConvolutionDensityMatrices, ConvolutionDensityMatrixMetadata,
                                     TimeDensityMatrices, TimeDensityMatrixMetadata)
...
class TimeResultsCollector(ResultsCollector):
    ...
    def __init__(self,
                 ...
                 calc_kwargs: dict[str, Any],
                 exclude: list[str] = []):
        if isinstance(calc.density_matrices, ConvolutionDensityMatrices):
            assert len(calc.density_matrices.pulses) == 1
        else:
            assert isinstance(calc.density_matrices, TimeDensityMatrices)
        Nt = len(calc.times)
    ...
    def accumulate_results(self,
                           work: TimeDensityMatrixMetadata | ConvolutionDensityMatrixMetadata,
                           result: Result):
        assert isinstance(work, (TimeDensityMatrixMetadata, ConvolutionDensityMatrixMetadata))
        assert world.rank == 0
...
class Writer(Generic[WorkMetadataT, ResultsCollectorT]):
    ...
    def common_arrays(self) -> dict[str, NDArray[np.float64] | int | float]:
        ...
        arrays: dict[str, NDArray[np.float64] | int | float] = dict()
    ...
    def calculate_and_save_npz(self,
        ...
        np.savez(out_fname, **self.common_arrays, **result._data,
