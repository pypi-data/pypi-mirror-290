...
try:
    ...
    from .utility_functions import allow_top_navigation, stringify_javascript_function, generate_js_function_from_json
except Exception:
    from date_formatter import python_to_luxon_format
    from utility_functions import allow_top_navigation, stringify_javascript_function, generate_js_function_from_json
...
def st_datatable(data: pd.DataFrame | list[dict] = None, boolean_to_string: bool = False, column_order: List[str] = None, options: Dict = None, dt_config: str = "default", class_name: str = None, common_js_functions: Dict = None, context_menu: Dict = None,  js_scripts: List[str] = None, stylesheets: List[str] = None, enable_top_navigation: bool = False, date_format: str = None, is_datetime: bool = False, export_file_name: str = None, on_select: Optional[Union[Literal["ignore"], Literal["rerun"], Callable]] = "rerun", on_change: Optional[Callable] = None, on_click: Optional[Callable] = None, ajax_reload: bool = False, ajax_setup: Dict = None, enable_diagnostics: bool = False, args: Optional[Tuple[Any, ...]] = None,
    ...
    on_change:
        An optional callback invoked when the datatable is clicked.  Note, it does does support args and kwargs.
    on_click:
    ...
    ajax_reload:
        An optional Boolean when set to True will reload the datatable from it's ajax source.  Default value is False.
    ...
    if options is None:
        options = {}
    if class_name is None and js_scripts is None and stylesheets is None:
        class_name, js_scripts, stylesheets, default_options = get_datatables_net_config(
            dt_config)
    ...
    component_value = _component_func(
        options=options, boolean_to_string=boolean_to_string, class_name=class_name, context_menu=context_menu, common_js_functions=common_js_functions,  js_scripts=js_scripts, stylesheets=stylesheets, key=key, table_id=table_id, on_change=on_change, is_ignored=is_ignored, date_format=luxon_date_format, export_file_name=export_file_name, ajax_reload=ajax_reload, ajax_setup=ajax_setup, enable_diagnostics=enable_diagnostics, default=None)
    ...
    # if on_click is not None and component_value:
    #     if args is None:
    #         args = ()
    #     if kwargs is None:
    #         kwargs = {}
    ...
    #     if key is None:
    #         st.error(
    #             "You must pass a key if you want to use the on_click callback for st_datatable")
    #     else:
    #         args = args if args is not None else []
    #         kwargs = kwargs if kwargs is not None else {}
    #         register_callback(key, on_click, *args, **kwargs)
...
if not _RELEASE:
    ...
    def my_on_click_function(key=None):
        print("You clicked me!", st.session_state["example_1b"])
    ...
    with content_column:
        with st.container(border=True):
            ...
            options["columns"] = [
                ...
                {"data": "price", "title": "price", "className": "secondmenu"},
                {"data": "price", "title": "price + tax", "render": price_plus_tax},
                {"data": None, "defaultContent": "",
                    "title": "", "orderable": False, "render": show_ellipsis},
            ]
            ...
            dt_click = st_datatable(None, options=options, context_menu=context_menu, common_js_functions=common_js_functions, ajax_setup=ajax_setup, ajax_reload=ajax_reload,
                                    key="ajax_table", on_select=on_select, export_file_name=filename, enable_diagnostics=True)
