...
def analyze_module(
    ...
    qc_config = get_qc_config(
        qc_criteria_path or str(data_path / "analysis_cuts.json"), TEST_TYPE
    )
    ...
    assert (
        df["BACKSIDE_FLATNESS"]["Unit"] == "um"
    ), "Must report flatness in um (micrometers)"
