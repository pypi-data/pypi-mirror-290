...
def source_market_single_ticker_bulk_from_api( api_token, symbol_at_data_provider, from_date, records_origin_short_ref):
...
def get_attribute_in_market_records_single_ticker(records_formatting_short_ref:str, attribute_type:AttributeType, records: List[dict]):

    if records_formatting_short_ref=="eodhd_eod":
        date_col_name = "date"
        ...
        if attribute_type==AttributeType.RECENT_DATE:
            ...
            raise ValueError(f"Attribute Type {attribute_type} not supported for records_origin_short_ref {records_formatting_short_ref}.")

    elif records_formatting_short_ref=="sourcing_schema_checked":
        ...
        raise ValueError(f"Data Origin Reference {records_formatting_short_ref} not supported.")
