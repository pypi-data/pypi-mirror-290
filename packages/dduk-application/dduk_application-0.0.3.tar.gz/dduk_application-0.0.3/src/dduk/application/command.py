...
def Help() -> None:
	...
	builtins.print("ddukapp project: 현재 경로를 루트로 삼아 프로젝트 템플릿 생성.")
	builtins.print("ddukapp bat: 현재 경로를 루트로 삼아 프로젝트 배치파일 생성. (Windows)")
	builtins.print("ddukapp sh: 현재 경로를 루트로 삼아 프로젝트 쉘스크립트 생성. (Linux/MacOS)")
...
def Main() -> None:
	...
	if sys.argv:
		# 실행파일 추출.
		executeName = sys.argv[0]
		builtins.print(f"executeName: {executeName}")
		sys.argv = sys.argv[1:]	
		if sys.argv:
			command = sys.argv[0]
