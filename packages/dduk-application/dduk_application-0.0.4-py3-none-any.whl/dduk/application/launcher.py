	...
	def Launch(self) -> None:
		...
		try:
			if isBuild:
				...
				if self.__embeddedData:					
					Application.Log("__EMBEDDED_DATA__")
			...
			self.__application.Log.Run("dduk-application", 0, useLog, logPath)
		...
		try:
			...
			if self.__application.isBuild():
				...
				Application.Log("dduk.application.debug.start()")
				if self.__embeddedData:
				...
				Application.Log("dduk.application.debug.wait()")
				debugpy.wait_for_client()
				Application.Log("dduk.application.debug.started()")

			module = importlib.import_module(self.__embeddedData.START_MODULE_NAME)
