...
class Source:
    def __init__(
        ...
        librarian_client: HmdLibrarianClient or str,
        hmd_home: Path,
        hmd_repo_home: Path,
    ):
    ...
    def get_content_item_type(self, path: Path):
        ...
        return content_item_type, priority
        ...
        for change_type, change_path in changes:
            if change_type == 3:
                ...
                try:
                    ...
                    cit, priority = self.get_content_item_type(path)
                    self.process_file(path, cit, priority)
    ...
    def sync(self):
        ...
        for content_glob in self.content_globs:
            ...
            for res in self.gather_globs_results(content_globs):
                ...
                files_dict[res] = (content_item_type, priority)
        for path, value in files_dict.items():
            self.process_file(Path(path), value[0], priority=value[1])
    ...
    def process_file(self, file_path: Path, content_item_type: str, priority: int = 0):
        ...
        if content_path:
            self.upsert_file(file_path, content_item_type, content_path, priority)
    ...
    def upsert_file(
        ...
        if file:
            ...
            if (
                ...
                logger.info(f"{self.name}: file already up-to-date {str(source_path)}")
                return file
        else:
            ...
            return self._librarian_sync_client.upsert_file_hmd_lang_librarian_sync(file)
