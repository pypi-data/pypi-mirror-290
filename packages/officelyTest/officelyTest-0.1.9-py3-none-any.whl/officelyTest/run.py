...
from officelyTest.team_builder.graph import Edge, Graph
from officelyTest.team_builder.llms.tokenizer import Tokenizer, get_team_tokenizer_callback
import re

from officelyTest.team_builder.nodes.factory import factory_node
from officelyTest.team_builder.nodes.interface import IHistory, InputItem
import json
...
def arun(team:Dict, verbose:bool, chat_history:List, query: str):
    try:
        g = ThreadedGenerator()
        threading.Thread(target=invokeTeam, args=(team, verbose, chat_history, query)).start()
        return g
