...
# Regular expression to match data URIs for images
IMAGE_DATA_URI_PATTERN = re.compile(
    r'^data:image/(png|jpg|jpeg|gif|webp|bmp|svg\+xml);base64,'
)
...
def is_image_uri(uri: str) -> bool:
    """
    Check if the URI is a Data URI for an image.
    """
    return IMAGE_DATA_URI_PATTERN.match(uri) is not None


def data_uri_to_image_stream(data_uri: str) -> Optional[BytesIO]:
    """
    Convert a Data URI to a temporary image file.
    """

    # Check if the Data URI matches the expected format
    data_uri = data_uri.strip()
    if not is_image_uri(data_uri):
        return None
    ...
    try:
        ...
        print(f"Failed to convert Data URI to image stream: {e}")
        return None
