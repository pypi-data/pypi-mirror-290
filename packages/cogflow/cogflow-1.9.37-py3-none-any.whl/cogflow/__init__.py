...
def evaluate(
    ...
    *,
    ...
    try:
        make_post_request(url_metrics, data=result.metrics, params=model_name)
    except Exception as exp:
        print(f"Failed to post metrics: {exp}")

    try:
        make_post_request(url_artifacts, data=result.artifacts, params=model_name)
    except Exception as exp:
...
def log_model(
    ...
    try:
        ...
        if active_run.info.artifact_uri and registered_model_name:
            # Construct the model URI
            artifact_subpath = "*"
            model_uri = f"{active_run.info.artifact_uri}/{registered_model_name}/{artifact_subpath}"
            print("model_uri", model_uri)
            NotebookPlugin().save_model_uri_to_db(model_id, model_uri)
    except Exception as exp:
