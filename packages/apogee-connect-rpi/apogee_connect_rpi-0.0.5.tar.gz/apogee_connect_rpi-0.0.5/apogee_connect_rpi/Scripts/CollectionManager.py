...
class CollectionManager:
    ...
    async def connect(self):
        # TODO: Add in some functionality to retry this at least once
    ...
    async def collect_live_data(self, interval: int, start_time, end_time):
        ...
        if self.sensor.type != "Guardian":
            await self.turn_on_gateway_mode(interval, start_time, end_time)
    ...
    async def align_data_collection(self):
        # Data logs won't be available until one sampling interval after minute. So pause a little until they are ready
        current_time_epoch = int(datetime.datetime.now().timestamp())
