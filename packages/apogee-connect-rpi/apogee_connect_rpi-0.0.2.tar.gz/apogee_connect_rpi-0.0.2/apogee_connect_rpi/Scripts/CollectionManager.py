...
class CollectionManager:
    ...
    def setup_crontab_command(self, interval: int, start_time, end_time):
        ...
        command = f"{home_dir}/.local/bin/apogee run_data_collection {self.address} --file {self.filename} --id {self.sensor.sensorID} --start {start_time} --end {end_time} >> {log_dir}/logs/cron.log 2>&1"
