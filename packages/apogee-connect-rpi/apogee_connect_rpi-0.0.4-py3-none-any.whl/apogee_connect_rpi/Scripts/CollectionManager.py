...
class CollectionManager:
    ...
    async def collect_guardian_data(self, interval: int, start_time, end_time):
        try:            
            ...
            await self.setup_crontab_command(interval, start_time, end_time)
    ...
    # CRON MANAGEMENT
    #
    async def setup_crontab_command(self, interval: int, start_time, end_time):
        if not start_time:
            start_time = 0
        if not end_time:
            end_time = 4294967295
        else:
            end_time += self.sampling_interval_const # Add the sampling interval since logs aren't available until one sampling interval after they are recorded

        cron = CronTab(user=True)
        self.check_for_existing_cron_jobs(cron)
        await self.align_collection_with_crontab_job()

        command = self.create_cron_job_command(start_time, end_time)
        job = cron.new(command=command)
        job.minute.every(interval)
        cron.write()

    def check_for_existing_cron_jobs(self, cron):
        for job in cron:
    ...
    def create_cron_job_command(self, start_time, end_time):
    ...
    async def align_collection_with_crontab_job(self):
        # Since logs aren't available until one sampling interval after they are recorded, make sure whenever I try to collect data is at least one sampling interval after the current minute
        current_time_epoch = int(datetime.datetime.now().timestamp())
        seconds_since_minute_start = current_time_epoch % 60
        if seconds_since_minute_start <= (self.sampling_interval_const + 1):
            print(f"Aligning time with sensor data collection")
            await asyncio.sleep(self.sampling_interval_const - (seconds_since_minute_start + 1))
