...
class CollectionManager:
    ...
    async def collect_live_data(self, interval: int, start_time, end_time):
        ...
        try:            
            ...
            if self.sensor.type == "Guardian":
                ...
                await self.setup_crontab_command(1, start_time, end_time)
    ...
    def create_cron_job_command(self, start_time, end_time):
        ...
        command = f"{executable_path} run_data_collection {self.address} --file {self.filename} --id {self.sensor.sensorID} --start {start_time} --end {end_time} >> {log_dir}/cron.log 2>&1"
        return command
    
    def get_executable(self, command_name, home_dir) -> str:
        # Default and most likely location
        ...
        if not os.path.isfile(executable_path):
            ...
            if result.returncode == 0:
                ...
                print(f"Could not find executable location. Please make sure 'apogee' executable is located at: {home_dir}/.local/bin/apogee")
                exit(1)
    ...
    async def align_data_collection(self):
        ...
        if seconds_since_minute_start <= (self.sampling_interval_const * 2):
            print(f"waiting: {seconds_since_minute_start}")
            await asyncio.sleep((self.sampling_interval_const * 30) - seconds_since_minute_start)
            print(f"finished waiting: {seconds_since_minute_start}")
