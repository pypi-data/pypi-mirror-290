...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.unusualwhales_api_key: Union[str, None] = os.getenv("UNUSUALWHALES_API_KEY", "")
        self.polygon_api_key: Union[str, None] = os.getenv("POLYGON_API_KEY", "")
        self.fmp_api_key: Union[str, None] = os.getenv("FMP_API_KEY", "")
        self.databento_api_key: Union[str, None] = os.getenv("DATABENTO_API_KEY", "")
        self.mongo_uri: Union[str, None] = os.getenv("MONGO_URI", "")
        self.mongo_db_name: Union[str, None] = os.getenv("MONGO_DB_NAME", "tradestream_db")
        ...
        # # Trade Sizing
        # self.config["whale_trades"] or 50_000
        # self.xl_trades:int = self.config["xl_trades"] or 15_000
        # self.l_trades:int = self.config["l_trades"] or 10_000
        # self.m_trades:int = self.config["m_trades"] or 5_000
        # self.s_trades:int = self.config["s_trades"] or 1_000
        # self.xs_trades:int = self.config["xs_trades"] or 0
        # Option Trade Sizing
        # self.whale_orders:int = self.config["whale_orders"] or 500_000
        # self.xl_orders:int = self.config["xl_orders"] or 250_000
        # self.l_premium:int = self.config["l_premium"] or 100_000
        # self.m_orders:int = self.config["m_orders"] or 50_000
        # self.s_orders:int = self.config["s_orders"] or 25_000
        # self.xs_orders:int = self.config["xs_orders"] or 0
...
class OptionTrade(TradeMixin):
    ...
    def from_dict(obj: Dict[str, Any]) -> "OptionTrade":
        ...
        size_class = OptionTrade.determine_trades_class(int(obj.get("size", 0)))
    ...
    @staticmethod
    def determine_trades_class(size: int,) -> str:
...
class StockTrade(TradeMixin):
    ...
    size_category: str = field(default="")
    sentiment: str = field(default="unknown")
    @staticmethod
