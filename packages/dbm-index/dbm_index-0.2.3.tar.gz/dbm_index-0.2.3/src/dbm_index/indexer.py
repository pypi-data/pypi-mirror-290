        ...
        if sort_key:
            ...
            while (offset > 0 or limit > 0) and value_encoded:
                ...
                while (offset > 0 or limit > 0) and key_value_id_encoded:
                    ...
                    encoded_resource_id = self.db.get(
                        self.key_delim.join(
                            [sort_key_hash, value_hash, key_value_id, "value"]
                        )
                    )

                    if encoded_resource_id is None:
                        raise Exception(f'encoded resource id is None. sort key: {sort_key}, value: {value}, key value id: {key_value_id}')
                    resource_id = encoded_resource_id.decode()
            ...
            if retreived_resource_id == resource_id:
                ...
                if lagging_resource_id:
                    if next_retreived_resource_id_encoded:
                        self.db[self.key_delim.join([lagging_resource_id, "next"])] = (
