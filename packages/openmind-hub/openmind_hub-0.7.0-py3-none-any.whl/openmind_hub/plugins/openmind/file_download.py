...
from .utils._validators import validate_om_hub_args, is_sensitive_path, validate_revision
...
def check_repo_exists(repo_id: str, repo_type: str, token: str):
...
def _chmod_and_replace(src: str, dst: str) -> None:
    ...
    logger.info(replace_invalid_characters(f"move {src} to {dst}"))
    shutil.move(src, dst)
...
def om_hub_download(
    ...
    **kwargs,
    ...
    if repo_type not in REPO_TYPES:
        error_msg = f"Invalid repo type: {repo_type}. Accepted repo types are: {str(REPO_TYPES)}"
        raise ValueError(replace_invalid_characters(error_msg))
    check_repo_exists(repo_id, repo_type, token)
    cache_dir = cache_dir or OM_HUB_CACHE
    ...
    with FileLock(lock_path):
        ...
        with temp_file_manager() as temp_file:
            ...
            if "HUB_HTTP_PROXY" in os.environ and "HUB_HTTPS_PROXY" in os.environ:
                proxies = {
