...
def validate_download_url(url: str) -> None:
    if isinstance(url, str):
        parsed_url = urlparse(url)
        if parsed_url.scheme != "https":
            raise OMValidationError("The download url is not HTTPS.")
        for host in DOWNLOAD_CDN_HOSTNAME_WHITE_LIST:
            if parsed_url.hostname.endswith(host):
                return
        raise ValueError("The download url is not within the white list.")
    else:
        if url is None:
            return
        raise OMValidationError("The download url type is not string.")
