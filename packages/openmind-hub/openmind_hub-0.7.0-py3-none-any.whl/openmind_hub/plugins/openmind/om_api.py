...
class DatasetInfo:
    ...
    def __init__(self, **kwargs):
        ...
        self.sha = kwargs.pop("sha", None)  # last_commit
        self.created_at = kwargs.pop("create_at", None)
...
class OmApi:
        ...
        **kwargs,
        ...
        Args:
            ...
            space_image (`str`, *optional*):
                Choice of base image to use if repo_type is "space". Can be "python3.8-pytorch2.1" or
        ...
        try:
            ...
            is_expect_status_code = r.status_code == 400 and r.json().get("code") == DUPLICATE_CREATING_MSG
    ...
    def delete_repo(
        ...
        *,
        ...
        Args:
            ...
            token (`str`, *optional*):
                An authentication token (See https://telecom.openmind.cn/my/tokens).
            repo_type (`str`, *optional*):
    ...
    def dataset_info(
        ...
        **kwargs,
        ...
        repo_tree = list(self.list_repo_tree(repo_id=repo_id, token=token, recursive=True))
        data["sha"] = self.get_repo_last_commit(repo_id=repo_id, token=token).oid
        siblings = []
        for file in repo_tree:
            if isinstance(file, RepoFile):
                siblings.append({"rfilename": file.path, "size": file.size, "blob_id": file.blob_id})
        ...
        **kwargs,
        ...
        if isinstance(filter, ModelFilter):
            ...
            if filter.model_name:
                if not isinstance(filter.model_name, str):
                    raise ValueError("The input ModelFilter is invalid.")
            ...
            if filter.tags:
                if not isinstance(filter.tags, str) and not isinstance(filter.tags, list):
                    raise ValueError("The input ModelFilter is invalid.")
            ...
            if filter.license:
                if not isinstance(filter.license, str):
                    raise ValueError("The input ModelFilter is invalid.")
                params["license"] = filter.license
            if filter.library:
                if not isinstance(filter.library, str) and not isinstance(filter.library, list):
                    raise ValueError("The input ModelFilter is invalid.")
            ...
            if filter.sort_by:
                if not isinstance(filter.sort_by, str):
                    raise ValueError("The input ModelFilter is invalid.")
                params["sort_by"] = filter.sort_by
            if filter.limit:
                if not isinstance(filter.limit, int):
                    raise ValueError("The input ModelFilter is invalid.")
    ...
    def get_full_repo_name(
        ...
        *,
        ...
        Args:
            ...
            token (`str`, *optional*):
                A valid authentication token (see https://telecom.openmind.cn/my/tokens).
