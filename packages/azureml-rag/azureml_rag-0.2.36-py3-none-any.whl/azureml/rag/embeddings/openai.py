...
try:
    ...
    from azureml.rag.langchain.vendor.embeddings.base import Embeddings
...
class OpenAIEmbedder(Embeddings):
    ...
    def __init__(
        ...
        self.api_key = api_key or os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("AZURE_OPENAI_KEY") or ""
    ...
    def _embed(self, texts: List[str]) -> List[List[float]]:
        ...
        # Serverless embedding models don't take tokenized inputs.
        if self.api_type == "serverless":
            class _PassthroughEncoder(object):
                def encode(self, text: str, **kwargs) -> str:
                    return text

            encoding = _PassthroughEncoder()
        else:
            try:
