...
def search_client_from_config(acs_config: dict, credential):
    ...
    return SearchClient(
        ...
        api_version=acs_config.get("api_version", ACS_API_VERSION),
    ...
    if acs_config["index_name"] not in index_client.list_index_names():
        ...
        if (
            ...
            if current_version >= pkg_version.parse("11.4.0"):
                ...
                raise RuntimeError(
                    f"azure-search-documents version {azure_documents_search_version} is not supported when using embeddings. Please upgrade to 11.4.0 or later."
                )
        ...
        if len(new_fields) > existing_fields_length:
            try:
                ...
                logger.error(f"Cannot update search index {acs_config['index_name']}: {e}")
                raise


def create_index_from_raw_embeddings(
    emb: EmbeddingsContainer,
    acs_config: Optional[Dict[str, Any]] = None,
...
def main(args, logger, activity_logger):
    ...
    try:
        ...
        if args.connection_id is not None:
            ...
            acs_config["api_version"] = get_metadata_from_connection(connection).get("apiVersion", ACS_API_VERSION)
