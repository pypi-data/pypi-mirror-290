...
class SingleSorting(object):
    ...
    def single(self, vw=False):
        ...
        # wrap up results
        notes = [f"Single sorting: {sortby1}", "The above table reports excess return rather than raw return"]
...
class SortingResults(object):
    ...
    def _make_table(self, rf_df, rf, alpha_models, layout, strategies, **kwargs):
        ...
        strategy1 = StrategyTable(self.res, self.qret, self.date,
                                  self.sortbys, alpha_models,
                                  layout, strategies, **kwargs)
    ...
    def summary(self, rf_df, rf, alpha_models, layout="default",
                strategies="HML", output_path=None, **kwargs):
        '''
...
class MainTable(object):
    def __init__(self, qret_df, qret, rf_df, rf, date, sortbys, layout="default", raw_ret=False, **kwargs):
        self.excess = "excess"
        self.maxlags = 5
        ...
        self.means = pd.DataFrame(self.mean_table(test_res, **kwargs)).rename(columns={0: " "})
        self.tstats = pd.DataFrame(self.tstats_table(test_res, **kwargs)).rename(columns={0: " "})
...
class StrategyTable(object):
    def __init__(self, qret_df, qret, date,
        ...
        self.get_alphas(date, strategy_df, alpha_models, **kwargs)
    ...
    def test_mean(self, date, strategy_df, model=None):
    ...
    def mean_table(self, test_res, show_stars_strategy=True,
        ...
        means = decimal(test_res["mean"] * 100, mean_decimal) + "%"
    ...
    def get_alphas(self, date, strategy_df, alpha_models, **kwargs):
        ...
        for n, model in enumerate(alpha_models):
            test_res, model_cols = self.test_mean(date, strategy_df,
                                                  model=model)
