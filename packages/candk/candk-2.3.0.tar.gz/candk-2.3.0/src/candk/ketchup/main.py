...
HELP = '''python ketchup.py <filepath> -hwmN -p <page_name> -n <output_name>
...
-p	Give the page a name. If the name has periods, the page title will be only
	the right-most portion, but the full page_name will display on the HTML page
	itself.
...
def main():
	...
	optlist = []
	try:
		optlist, args = getopt.gnu_getopt(sys.argv[1:], 'hwmNp:n:o:c:')
	...
	for o, arg in optlist:
		...
		# if o == '-b':
		# 	ketchup.preserves_linebreaks = True
		if o == '-N':
