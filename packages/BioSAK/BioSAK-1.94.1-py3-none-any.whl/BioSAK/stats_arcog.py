...
def stats_arcog(args):
    ...
    for cog_cate in sorted(list(cog_cate_stats_dict.keys())):
        ...
        for cog_id in sorted(list(cog_id_set)):
            ...
            op_txt_handle.write('%s\t%s\t%s\t%s\n' % (cog_cate, cog_cate_desc.strip(), cog_id, cog_id_desc.strip()))
    op_txt_handle.close()
...
if __name__ == "__main__":
    ...
    stats_arcog_parser.add_argument('-skip1row',    required=False, action='store_true',    help='skip the 1st row of the -cog file')
