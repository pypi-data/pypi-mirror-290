...
class ConfigurableObjectGenerator:
    ...
    def __init__(self, dynamic_importer: DynamicImporter = DynamicImporter(), allowed_modules: List[str] = None,
                 disallowed_modules: List[str] = None, cache_size: int = 128):
        """
        ...
        Args:
            ...
            allowed_modules (List[str], optional): List of allowed module names. If None, all modules are allowed.
            disallowed_modules (List[str], optional): List of disallowed module names.
            cache_size (int, optional): Size of the LRU cache for module imports. Defaults to 128.
        """
        self.importer = dynamic_importer
        self.allowed_modules = set(allowed_modules) if allowed_modules else None
        self.disallowed_modules = set(disallowed_modules) if disallowed_modules else set()
    ...
    def _setup_cache(self):
        ...
        self._cached_import = cached_import
    ...
    def create(self, config: Dict[str, Any]) -> Any:
        ...
        Raises:
            ValueError: If the configuration is invalid or if a module is explicitly disallowed.
        ...
        if any(disallowed in module_name for disallowed in self.disallowed_modules):
            raise ValueError(f"Module '{module_name}' contains a disallowed module name.")

        try:
            module = self._cached_import(module_name)

            if self.allowed_modules is not None and not any(allowed in module_name for allowed in self.allowed_modules):
                raise ValueError(f"Module '{module_name}' is not in the list of allowed modules.")
    ...
    def set_cache_size(self, size: int) -> None:
        """Set the size of the LRU cache for module imports."""
        self._cached_import.cache_clear()
        self._cached_import = lru_cache(maxsize=size)(self._cached_import.__wrapped__)
...
def main():
    ...
    generator = ConfigurableObjectGenerator(importer, allowed_modules=['datetime', 'collections', 'urllib'])
