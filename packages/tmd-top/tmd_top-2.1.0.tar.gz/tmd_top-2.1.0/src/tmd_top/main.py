...
geoip更新时间: 2024-08-13
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Input(placeholder="请输入PID按Enter键",id="input_command")
        yield Footer()
    ...
    def on_mount(self) -> None:
        ...
        listen_table.add_columns(*("PID", "名称", "监听地址","监听端口","IP数","连接数","上传","下载","CPU","内存","IO读","IO写"))
        ...
        details_table.add_columns(*("客户端IP","客户端PORT","上传","下载",'地区'))
        ...
        outside_table.add_columns(*("PID", "名称","IP数","连接数","上传","下载","CPU","内存","IO读","IO写"))
    ...
    def on_key(self,event: events.Key):
        ...
        if event.key == 'enter':
            ...
            instruction_display.update('当前PID: ' + input_value.value)
    ...
    async def update_tables(self) -> None:
        ...
        while True:
            ...
            pidstat_command = self.localExecuteCommand("sudo pidstat -d | grep -v 'kB_rd/s' | grep -v 'CPU)' | grep  -v '^$'")
    ...
    def selectTotalListen(self,conn):
        ...
        /*获取内部流量*/
             select 
                ...
                ps.cpu as cpu,
                ps.men as men,
                ROUND(sum(pidstat.read),2) as read,
                ROUND(sum(pidstat.write),2) as write
            ...
            LEFT JOIN pidstat on pidstat.pid == net_listen.pid 
            GROUP BY 
                net_listen.pid,
    ...
    def selectTotalOut(self,conn):
        select_out_sql = '''
            /*获取访问外部的流量*/
                ...
                LEFT JOIN pidstat on pidstat.pid == netstat.pid
                GROUP BY 
                    netstat.pid,
...
def main():
    ...
    if not command_exists('pidstat'):
        print("pidstat命令不存在，请确保系统上已安装该命令！")
        sys.exit(0)
