...
import numpy as np
from .models.option_models.universal_snapshot import UniversalOptionSnapshot, UniversalSnapshot, OptionData, UniversalOptionSnapshot2
...
class PolygonOptions:
    def __init__(self,user:str='chuck', database:str='markets', host:str='localhost', port:int=5432, password:str='fud'):
        self.conn = None
        self.pool = None
        self.host=host
        self.port=port
...
# Asynchronous method to create an aiohttp session
    ...
    async def connect(self):
        ...
        self.pool = await asyncpg.create_pool(host=self.host, user=self.user,port=self.port,database=self.database,password=self.password)
...
# Example usage
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if data.status_code == 200:
                    ...
                    timestamp = [convert_ns_to_est(i.get('sip_timestamp')) for i in results]
                    price = [i.get('price') for i in results]
                    size = [i.get('size') for i in results]
                    components = get_human_readable_string(ticker)
                    sym = components.get('underlying_symbol')
                    strike = components.get('strike_price')
                    exp = components.get('expiry_date')
                    ...
                    await self.batch_insert_dataframe(df, table_name='option_trades', unique_columns='ticker, strike, expiry, call_put, size, price, timestamp')
    ...
    async def batch_insert_dataframes(self, dfs, table_name, unique_columns, batch_size=250):
        try:
            async with lock:
                for df in dfs:
                    if not await self.table_exists(table_name):
    ...
    async def get_option_chain_all(self, underlying_asset, strike_price=None, strike_price_lte=None, strike_price_gte=None, expiration_date=None, expiration_date_gte=None, expiration_date_lite=None, contract_type=None, order=None, limit=250, sort=None, pool=None, insert:bool=False):
        """
    ...
    async def gpt_filter(self, ticker:str=None, strike:float=None, strike_min:float=None, strike_max:float=None, cp:str=None, expiry:str=None, expiry_min:str=None, expiry_max:str=None, dte_max:int=None, dte_min:int=None, 
        ...
        return chunks
    async def multi_snapshot_httpx(self, symbol_string: str):
        async with httpx.AsyncClient() as client:
            url = f"https://api.polygon.io/v3/snapshot?ticker.any_of={symbol_string}&apiKey={os.environ.get('YOUR_POLYGON_KEY')}&limit=250"
            resp = await client.get(url)
            data = resp.json()
            results = data['results'] if 'results' in data else None
            if results is not None:
                data = OptionData(results)
                return data
