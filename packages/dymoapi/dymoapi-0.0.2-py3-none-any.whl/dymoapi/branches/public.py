...
def get_prayer_times(data):
    ...
    try:
        response = requests.get("https://api.tpeoficial.com/v1/public/islam/prayertimes", params=params)
...
def satinizer(data):
    try:
        ...
        response = requests.get("https://api.tpeoficial.com/v1/public/inputSatinizer", params={"input":quote(input_value)})
        response.raise_for_status()
        return response.json()
    except requests.RequestException as e: raise APIError(str(e))
...
def is_valid_pwd(data):
    try:
        ...
        response = requests.get("https://api.tpeoficial.com/v1/public/validPwd", params=params)
        response.raise_for_status()
        return response.json()
    except requests.RequestException as e: raise APIError(str(e))
