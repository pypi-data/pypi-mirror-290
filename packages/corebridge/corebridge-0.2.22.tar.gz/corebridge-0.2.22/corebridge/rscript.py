...
__all__ = ['syslog', 'read_chunk_size', 'RScriptProcess', 'get_asset_path', 'get_rscript_libpath', 'get_save_path',
           'install_R_package', 'calc_hash_from_flowobject', 'calc_hash_from_files', 'calc_hash_from_input_files',
...
def get_rscript_libpath(save_dir:str):
    return os.path.join(save_dir, 'rscript', 'libs')
def get_save_path(datafile_name:str, save_dir:str): 
    return os.path.join(save_dir, datafile_name)


# %% ../nbs/02_rscriptbridge.ipynb 39
def install_R_package(pkg:str|list, libfolder:str, repo='https://cloud.r-project.org'):
    """
    ...
    if isinstance(pkg, str):
        return install_R_package([pkg], libfolder, repo)

    os.makedirs(get_rscript_libpath(save_dir), exist_ok=True)
    env = dict(os.environ)
    env['R_LIBS_USER'] = libfolder
    
    for pkg_i in pkg: # ['generics', 'timechange', 'rlang', 'stringi'] + 
        print(f"Installing package {pkg_i}, testing attach ...")
        run_script_result = subprocess.run(['Rscript','-e', f"library({pkg_i})"], capture_output=True, env=env)
        ...
        if run_script_result.returncode != 0:
            ...
            if True or run_script_install.returncode != 0:
            ...
            run_script_post = subprocess.run(['Rscript','-e', f"library({pkg_i})"], capture_output=True, env=env)
            if True or run_script_post.returncode != 0:
                print(f"Attach after installing for {pkg_i} returned {run_script_post.returncode} ... ")
                print('STDOUT--------------\n', run_script_post.stdout.decode('UTF-8'))
                print('STDERR--------------\n', run_script_post.stderr.decode('UTF-8'))
