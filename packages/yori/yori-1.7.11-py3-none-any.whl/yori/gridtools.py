...
class Grid():
    ...
    def __init__(self, lat, lon, gridsize, proj, fillvalue):
        ...
        if (proj == 'conformal') or (proj == 'equal_angle'):
            ...
            raise ValueError(f'Unkonwn projection "{proj}". Possible options are ' +
...
# WE CAN POSSIBLY START FROM HERE
        ...
        latgrid = np.arange(-self.y1+gridsize/2, self.y1+gridsize-gridsize/2, gridsize)
        longrid = np.arange(-self.x1+gridsize/2, self.x1+gridsize-gridsize/2, gridsize)

        if self.proj == 'equal_area':
            eqArea = pyproj.Proj(proj='cea')
            eqAngle = pyproj.Proj(proj='longlat')
            _, latgrid = pyproj.transform(eqArea, eqAngle,
                                          np.ones((len(latgrid), 1))*longrid[0], latgrid)
            longrid, _ = pyproj.transform(eqArea, eqAngle,
                                          longrid, np.ones((len(longrid), 1))*latgrid[0])
...
#         self.vargrid = {}
    ...
    def compute_stats(self, var, varname=''):
        ...
        if np.shape(self.lat) != np.shape(self.lon) or np.shape(self.lat) != np.shape(var):
        ...
        for i in range(len(self.box_idx)):
            ...
            if (len(xx) > 0):
                ...
                warnings.warn('The code should not be entering this condition', UserWarning)
                continue
        ...
        vargrid[varname + 'Standard_Deviation'] = np.reshape(varstd, self.grid['matrixsize'])
    ...
    def compute_median(self, var, varname, varbins, resolution):
        ...
        vargrid[varname + '/Median_Distribution'] = np.reshape(median_hist,
    ...
    def compute_wgt_stats(self, var, varname='', weight=''):
        ...
        """
        if weight == '':
            raise ValueError('Weight not specified when requesting the weighted ' +
        ...
        vargrid[varname + 'Weighted_Sum'] = np.reshape(var_wgt_sum,
                                                       self.grid['matrixsize'])
        vargrid[varname + 'Weighted_Sum_Squares'] = np.reshape(var_wgt_ssum,
                                                               self.grid['matrixsize'])
        vargrid[varname + 'Weighted_Mean'] = np.reshape(var_wgt_mean,
                                                        self.grid['matrixsize'])
        vargrid[varname + 'Weighted_Standard_Deviation'] = np.reshape(var_wgt_std,
                                                                      self.grid['matrixsize'])
    ...
    def compute_histogram(self, var, binvec, varname):
        ...
        vargrid = {}
        vargrid[varname + '/Histogram_Counts'] = np.reshape(varstat,
                                                            (self.grid['matrixsize'][0],
                                                             self.grid['matrixsize'][1],
                                                             np.size(binvec)-1))
        return vargrid
    ...
    def compute_2D_histogram(self, var1, var2, binvec1, binvec2, varname, var_name_out):
        ...
        # check if the sizes of the coordinates passed to the function are consistent
        if (np.shape(self.lat) != np.shape(self.lon) or
           np.shape(self.lat) != np.shape(var1) or
           np.shape(self.lat) != np.shape(var2)):
            raise ValueError('Input coordinates have different size')

    # initialize output variables
        var2Dstat = np.zeros((self.init_size, np.size(binvec1)-1, np.size(binvec2)-1))  # *np.nan
    ...
    # loop for the computation of the output variables
        ...
        vargrid = {}
        vargrid[varname + '/' + var_name_out] = np.reshape(var2Dstat,
                                                           (self.grid['matrixsize'][0],
                                                            self.grid['matrixsize'][1],
                                                            np.size(binvec1)-1,
                                                            np.size(binvec2)-1))
...
def estimate_median(hist, edges, resolution):
    msg1 = ('Median Error: something is wrong with i_neg or i_pos that I did not '
            ...
            'issue. Thanks.')
    msg2 = ('Median Error: corr_factor should never be negative. If you see this error '
            'send an email to paolo.veglio@ssec.wisc.edu providing a test case able to '
            'reproduce the issue. Thanks.')

    if np.sum(hist) == 0:
        return {'est_median': np.nan,
                'hist': hist}
    ...
    median_out = {'est_median': est_median,
                  'hist': hist}
