...
class SetupYori():
    ...
    def __init__(self, config_file, input_file, output_file):
        ...
        if os.path.isfile(output_file):
            raise IOError(f'The file {self.outfn} already exists in ' +
        ...
        self.grid_size = np.array(self.ymlSet.grid_settings['gridsize'],
                                  dtype='f8')
        if self.grid_proj == 'equal_area':
            self.grid_size = 1000*self.grid_size

        if ('lat_in' not in self.ymlSet.grid_settings or
           'lon_in' not in self.ymlSet.grid_settings):

            warnings.warn('Coordinate names are not defined in the configuration file. '
        ...
        if (self.fv - np.trunc(self.fv)) != 0:
            warnings.warn('Yori does not support the definition of non-integer fill values. ' +
...
#                                         self.grid_size)
    ...
    def setVariable(self, var_settings, lat, data_in, joint=False):
        ...
        current_mask = create_mask(data_in, np.shape(lat), mask_list=mask_list,
                                   inv_mask_list=inv_mask_list)
        current_mask = create_mask(data_in, np.shape(lat),
    ...
    def runYori(self, debug=False, compression=0):
        ...
        myGrid = grid.ComputeVariables(lat, lon, self.grid_size,
                                       self.grid_proj, self.fv)
        ...
        for i in range(len(self.ymlSet.var_settings)):
            ...
            masked_data_in, var_name_out = self.setVariable(var_settings,
                                                            lat, data_in)
            ...
            if 'median' in var_settings:
                ...
                tmp_gridvar = myGrid.compute_median(masked_data_in, var_name_out,
                                                    varbins,
            ...
            if 'weights' in var_settings:
                ...
                tmp_gridvar = myGrid.compute_wgt_stats(masked_data_in, var_name_out,
                                                       weight=wgt)
                gridvar.update(tmp_gridvar)

            if 'histograms' in var_settings:
                varbins = self.ymlSet.readHist(var_settings['histograms'])
                tmp_gridvar = myGrid.compute_histogram(masked_data_in, varbins,
                                                       var_name_out)
                gridvar.update(tmp_gridvar)
            ...
            if '2D_histograms' in var_settings:
                for k in range(len(var_settings['2D_histograms'])):
                    ...
                    joint_masked_data_in, vno = self.setVariable(hist_2D_settings, lat,
                                                                 data_in, joint=True)

                    joint_var_name_out = hist_2D_settings['name_out']
                    tmp_gridvar = myGrid.compute_2D_histogram(masked_data_in,
                                                              joint_masked_data_in,
                                                              varbins, joint_varbins,
                                                              var_name_out,
                                                              joint_var_name_out)
                    gridvar.update(tmp_gridvar)

            IO.write_output(data_out, gridvar, compression, self.grid_size,
                            self.ymlSet, self.fv,
                            edges=myGrid.edges, write_edges=True)
...
def create_mask(in_data, init_size, mask_list=[], inv_mask_list=[],
                existing_mask=None):
