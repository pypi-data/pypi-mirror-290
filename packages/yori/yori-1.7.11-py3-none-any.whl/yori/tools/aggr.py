...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Command to aggregate gridded files ' +
                                     'produced with yori-grid')
    parser.add_argument('--daily', metavar='YYYY-mm-dd', default='',
    ...
    parser.add_argument('-c', '--compression', default=5,
                        help='set the compression level for the output file (valid ' +
                        'range 1-9)')
    parser.add_argument('--min-pixel-counts', help='set the minimum number of pixels ' +
    ...
    # if the file exists, assume it is a file and try to fetch inputs
        ...
        inputs = [l.strip() for l in open(args.filelist).read().split('\n') if l.strip()]
