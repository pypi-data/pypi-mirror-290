...
def aggregate(flist, fname_out, satellite='', daily='',
    ...
    if grid_vers != str(VERSION):
        message = ("Current yori-aggr version (v" + str(grid_vers) +
    ...
    for f in flist:
        ...
        if hasattr(fin, 'daily_defn_of_day_adjustment'):
            if fin.getncattr('daily_defn_of_day_adjustment') == 'True':
                message = ('The input files have been created with the ' +
    ...
    for batch_idx in range(0, len(group_list), batching):
        ...
        for fin in flist:
            ...
            if not arrays_initialized:
                var_in = tools.initialize_arrays(group_batch, file_in, latkey, lonkey, fv)
            ...
            if daily != '':
                try:
                    ...
                    raise AttributeError('Global attribute "time_coverage_start"' +
                                         'not found in input file ' + fin)
                if satellite == 'aqua':
                    dmask = np.asarray(tools.aqua_daily_aggr(timeattr, daily, gridsize),
                                       bool)
                if satellite == 'terra':
                    dmask = np.asarray(tools.terra_daily_aggr(timeattr, daily, gridsize),
                                       bool)
            else:
            ...
            if use_min_pixel_counts is True:
                try:
                    ...
                    message = (('Global attribute "time_coverage_start" not found in ' +
        ...
        if use_min_pixel_counts is True:
            if len(set(days)) != 1:
                message = ('Warning: not all files belong to the same day. Please ' +
                           'verify your inputs. If this is intended please disregard ' +
                           'this message')
                warnings.warn(message)
        if use_min_valid_days is True:
            if len(set(months)) != 1:
                message = ('Warning: not all files belong to the same month. Please ' +
                           'verify your inputs. If this is intended please disregard ' +
                           'this message')
                warnings.warn(message)
            ...
            for grp in var_in.values():
                ...
                if 'min_pixel_counts' in grp:
                    ...
                    for k in grp.keys():
                        ...
                        if k not in ['Pixel_Counts', 'min_valid_days', 'edges', 'min_pixel_counts', 'Num_Days']:
