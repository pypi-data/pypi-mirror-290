...
def write_output(fout, gridvar, compression, grid_size, ymlSet, fv,
                 edges=[], write_edges=False):
    ...
    for k in list(gridvar):
        ...
        fout.createVariable(k, vartype, dims,
                            complevel=int(compression), zlib=True,
                            fill_value=fv)
        if edges == []:
            fout[k][:] = gridvar[k]
        else:
            fout[k][edges[0]:edges[1]+1, edges[2]:edges[3]+1] = (
                gridvar[k][edges[0]:edges[1]+1, edges[2]:edges[3]+1])
...
def create_dims(fout, variable, k):
    ...
    if variable[k].ndim == 3:
        histo_dim_name = 'Histo_{0}_{1}'.format(k.split('/')[0],
                                                variable[k].shape[2]).lower()
    if variable[k].ndim == 4:
        primary_var = k.split('/')[0]
        joint_var = k.split('/')[-1].replace('JHisto_vs_', '')
        jhisto_primary_name = 'jhisto_{0}_{1}'.format(primary_var,
                                                      variable[k].shape[2]).lower()
        jhisto_joint_name = 'jhisto_{0}_{1}'.format(joint_var,
...
def compute_hist_bins(hist_data):
    hist_bins = np.arange(hist_data['start'], hist_data['stop'], hist_data['interval'],
                          dtype='f8')
    return hist_bins
