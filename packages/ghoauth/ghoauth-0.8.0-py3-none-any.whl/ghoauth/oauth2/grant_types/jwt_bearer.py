...
def decode_jwt_assertion(
    jwt_message,
    decode_key,
    request,
    validator,
    current_ts,
    is_authorization_grant,
):
    try:
        options = {
    ...
    if not isinstance(iss, str) or not validator.validate_iss(iss, request):
        return None
    ...
    if not isinstance(aud, str) or not validator.validate_aud(aud, request):
        return None
    ...
    if "iat" in jwt_claims and not isinstance(jwt_claims["iat"], int):
        return None
    if "jti" in jwt_claims and not validator.validate_jti(
        jwt_claims["jti"], request
    ):
        return None
...
class JWTBearerGrant(AbstractBaseGrantTypeHandler):
    ...
    def __init__(
        self, token_factory=bearer_token_factory, datetime_factory=datetime.now
    ):
    ...
    def validate_token_request(self, request, repository, validator):
        if not request.o2_validate(
            requireds={"assertion"}, optionals={"scope"}
        ):
        ...
        try:
            ...
            request._o2_jwt_claims = decode_jwt_assertion(
                request.o2_assertion,
                decode_key,
                request,
                validator,
                self.datetime_factory().timestamp(),
                True,
            )
    ...
    def handle_token_request(self, request, repository):
        try:
            ...
            token = self.token_factory(
                subject, request.o2_scope, False, request
            )
