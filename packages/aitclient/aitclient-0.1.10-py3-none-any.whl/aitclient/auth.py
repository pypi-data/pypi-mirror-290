...
def aitAUTH(client_id, client_secret, tenant_id, AIT_Development=False, Proxy=None):
    ...
    AIT_Development (bool): Flag to indicate if the development environment should be used.
    ...
    try:
        if Proxy:
            response = requests.head(authentication, proxies=Proxy)
        else:
            response = requests.head(authentication)  # Using HEAD for a lightweight check
    ...
    if Proxy:
        access_token_response = requests.post(authentication, headers=headers, proxies=Proxy)
    else:
        access_token_response = requests.post(authentication, headers=headers)
