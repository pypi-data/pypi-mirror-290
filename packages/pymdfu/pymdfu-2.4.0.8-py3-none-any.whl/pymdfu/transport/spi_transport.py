...
class SpiTransportClient(Transport):
    ...
    def write(self, data):
        ...
        checksum = self.calculate_checksum(length).to_bytes(self.CHECKSUM_SIZE, byteorder="little")
        ...
        if len(data) % 2:
            check_sequence = self.calculate_checksum(data + bytes(1))
        else:
            check_sequence = self.calculate_checksum(data)
        check_sequence = check_sequence.to_bytes(self.CHECKSUM_SIZE, byteorder="little")
