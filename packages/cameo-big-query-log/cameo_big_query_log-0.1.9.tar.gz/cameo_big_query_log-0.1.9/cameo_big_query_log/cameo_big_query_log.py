...
class BigQueryLogger:
    def __init__(self, key_path=None, dataset_id=None, table_name=None):
        self.key_path = key_path or os.getenv('BIG_QUREY_GOOGLE_APPLICATION_CREDENTIALS')
        if not self.key_path:
            raise ValueError("Google application credentials key path must be provided either as an argument or through the 'BIG_QUREY_GOOGLE_APPLICATION_CREDENTIALS' environment variable.")
        
        self.client = bigquery.Client.from_service_account_json(self.key_path)
        self.cipher_suite = Fernet(key)
        
        self.dataset_id = dataset_id or os.getenv('BIG_QUREY_DATASET_ID')
        if not self.dataset_id:
            raise ValueError("Dataset ID must be provided either as an argument or through the 'BIG_QUREY_DATASET_ID' environment variable.")
        
        self.table_name = table_name or os.getenv('BIG_QUREY_TABLE_NAME')
        if not self.table_name:
            raise ValueError("Table name must be provided either as an argument or through the 'BIG_QUREY_TABLE_NAME' environment variable.")
    ...
    def insert_data_to_table(self, user_data: dict):
        ...
        try:
            ...
            data = [{
                ...
                'new_field_1': user_data.new_field_1,
                'new_field_2': user_data.new_field_2
            }]
    ...
    def create_table(self, table_id):
        ...
        schema = [
            ...
            bigquery.SchemaField("new_field_1", "STRING", description="新欄位1"),
            bigquery.SchemaField("new_field_2", "STRING", description="新欄位2")
        ]
