...
class Vault:
    ...
    def delete_items(self, item_ids: List[int], trees: int = 10) -> None:
        '''
        Deletes multiple items from item_ids, a list of integers, and rebuilds the vector database only once.
        '''
        def rebuild_vectors():
            ...
            # Prepare the new index with updated item vectors
            for i in range(num_existing_items):
                if i in item_ids:
                    adjustment += 1
                else:
                    vector = self.vectors.get_item_vector(i)
                    new_index.add_item(i - adjustment, vector)
                    updated_metadata[i - adjustment] = self.old_map[str(i)]

            # Update metadata in parallel
            with ThreadPoolExecutor() as executor:
                for new_i, old_i in updated_metadata.items():
                    executor.submit(download_and_upload_metadata, new_i, old_i)

            self.vectors = new_index
            self.vectors.build(trees)
            self.upload_vectors()
        
        def download_and_upload_metadata(i, old_i):
            metadata = json.loads(self.cloud_manager.download_text_from_cloud(
                cloud_name(self.vault, self.old_map[str(old_i)], self.user, self.api, meta=True)))
            metadata['item_id'] = i
            self.cloud_manager.upload_to_cloud(
                cloud_name(self.vault, self.old_map[str(old_i)], self.user, self.api, meta=True), json.dumps(metadata))
        ...
        self.clear_cache()
        if self.verbose:
            print(f'Item {item_ids} deleted and database rebuilt')
    ...
    def override_cloud_database_with_json_upload(self, json_data: Union[str, dict]):
        ...
        for item_number, item_data in items_dict.items():
            ...
            try:
                ...
                print(f"Error adding item {item_number}: {str(e)}")

        print(f"Successfully added {items_added} items to the vault.")
