    ...
    def delete_items(self, item_ids: List[int], trees: int = 10) -> None:
        ...
        def download_and_upload(i, plus_one):
            if plus_one != 0:
                ...
                print('before:', metadata['item_id'])
                metadata['item_id'] = i - plus_one
                print('after:', metadata['item_id'])
                self.cloud_manager.upload_to_cloud(cloud_name(self.vault, self.old_map[str(i)], self.user, self.api, meta=True), json.dumps(metadata))
                self.map[str(i - plus_one)] = self.old_map[str(i)]
                ...
                with ThreadPoolExecutor() as executor:
                    ...
                    for i in range(num_existing_items):
                        if i == item_id:  # the item being deleted
                            ...
                            futures.append(executor.submit(download_and_upload, i, plus_one))
