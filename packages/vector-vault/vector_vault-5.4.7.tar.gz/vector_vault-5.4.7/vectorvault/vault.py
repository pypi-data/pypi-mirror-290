...
class Vault:
    ...
    def delete_items(self, item_ids: List[int], trees: int = 10) -> None:
        ...
        '''
        lock = threading.Lock() 
        def download_and_upload(i, off_set, lock):
            with lock:  
                if off_set != 0:
                    ...
                    self.map[str(i - off_set)] = self.old_map[str(i)]
        ...
        def rebuild_vectors(item_ids):
            ...
            with ThreadPoolExecutor() as executor:
                ...
                for i in range(num_existing_items):
                    if i in item_ids:  # the item being deleted
                        off_set += 1
                    else:
                        vector = self.vectors.get_item_vector(i)
                        new_index.add_item(i - off_set, vector)
                        futures.append(executor.submit(download_and_upload, i, off_set, lock))
