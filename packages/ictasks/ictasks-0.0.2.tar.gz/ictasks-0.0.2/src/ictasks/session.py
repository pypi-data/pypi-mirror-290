...
class Session:
    ...
    def _log_launch_info(self):
        ...
        logger.info(
            f"""Session started with {num_workers}
                      workers ({ppn} per node)."""
        )
        if self.tasks.has_grouped_tasks():
            grouped_tasks = self.tasks.num_grouped_tasks
            logger.info(
                f"""Session has {num_tasks} tasks,
                grouped as {grouped_tasks} metatasks."""
            )
        else:
            logger.info(f"Session has {num_tasks} tasks.")

        if num_tasks % num_workers != 0:
            logger.warning(
                f"""Session input should ideally provide a multiple of
                  {num_workers} tasks for {num_workers} workers."""
            )
    ...
    def _wait_on_task(self):
        ...
        while True:
            ...
            if len(finished) > 0:
                ...
                exit = signal = finished[first_pid]["status"]
                break
            else:
        ...
        if exit != 0:
            ...
            logger.error(f"'{task_cmd}' killed by sig {signal}")
