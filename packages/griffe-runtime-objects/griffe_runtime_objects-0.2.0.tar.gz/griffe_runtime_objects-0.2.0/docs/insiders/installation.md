...
*griffe-runtime-objects Insiders* is a compatible drop-in replacement for *griffe-runtime-objects*,
...
*griffe-runtime-objects Insiders* can be installed with `pip` [using SSH][using ssh]:

```bash
pip install git+ssh://git@github.com/pawamoy-insiders/griffe-runtime-objects.git
```
...
```bash
pip install git+https://${GH_TOKEN}@github.com/pawamoy-insiders/griffe-runtime-objects.git
```
...
Of course, you can use *griffe-runtime-objects Insiders* directly using Git:

```
git clone git@github.com:pawamoy-insiders/griffe-runtime-objects
```

When cloning with Git, the package must be installed:

```
pip install -e griffe-runtime-objects
```

## Upgrading

When upgrading Insiders, you should always check the version of *griffe-runtime-objects*
