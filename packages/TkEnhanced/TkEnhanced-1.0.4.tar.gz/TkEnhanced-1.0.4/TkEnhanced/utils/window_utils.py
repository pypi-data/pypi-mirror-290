...
from tkinter import Event, Misc, Wm
...
class WindowResizeUtils:
    ...
    def get_cursor_index(event: Event, window: "WindowUtils") -> int:
        ...
        if not is_window:
            return -1
        is_maximized: bool = window.wm_state() == WindowStates.MAXIMIZED
        if is_maximized:
            return -1
...
class WindowUtils(WindowInterface, Misc, Wm):
    ...
    def hide_titlebar(self) -> None:
        ...
        if is_window_hidden:
            return None
        self._user32_api.ShowWindow(window_handle, WindowStates.SHOWN)
    ...
    def minimize_window(self) -> None:
        ...
        window_handle: int = self.retrieve_window_handle()
        self._user32_api.ShowWindow(window_handle, WindowStates.MINIMIZED)
    minimize = minimize_window
