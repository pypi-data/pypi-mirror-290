...
def _broadcast_static_shape(shape_x, shape_y):
  ...
  try:
    ...
    return tensor_shape.TensorShape(
        onp.broadcast(np.zeros(shape_x), np.zeros(shape_y)).shape)
...
def _default_convert_to_tensor_with_dtype(value, dtype,
  ...
  if is_arraylike:
    # Duck-typed for `onp.array`/`oonp.generic`
    arr = np.array(value)
    if dtype is not None:
    ...
    # int.
    dtype_compatible = (dtype == onp.bool_ or np.issubdtype(dtype, np.integer)
