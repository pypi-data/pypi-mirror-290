...
class SetType(type):
    _consts = {'ADD', 'ALL_TYPES', 'BEAM', 'BOX', 'DISCRETE', 'GENERAL', 'GENERATE', 'INTERSECT', 'MM_GROUP', 'MODE', 'NODE', 'PART', 'PART_TREE', 'PERI_LAMINATE', 'SEGMENT', 'SEGMENT_2D', 'SHELL', 'SOLID', 'TSHELL'}
...
class Set(Oasys.gRPC.OasysItem, metaclass=SetType):
    ...
    _rprops = {'advanced', 'collect_children', 'column', 'exists', 'general_lines', 'increment', 'smooth', 'total', 'type'}
...
# Constructor
    def __init__(self, model, sid, type, title=Oasys.gRPC.defaultArg, option=Oasys.gRPC.defaultArg):
        handle = Oasys.PRIMER._connection.constructor(self.__class__.__name__, model, sid, type, title, option)
        ...
        type : constant
            ...
            Set.TSHELL
        title : string
            Optional. Title for the set
        option : constant
            Optional. Set type. Can be
            Set.ADD,
            Set.INTERSECT,
            Set.GENERAL or
            Set.GENERATE
...
# Static methods
    ...
    def GetAll(model, type):
        ...
        list
            List of Set objects
        """
        return Oasys.PRIMER._connection.classMethod(__class__.__name__, "GetAll", model, type)
    ...
    def GetFlagged(model, flag, type):
        ...
        list
            List of Set objects
        """
        return Oasys.PRIMER._connection.classMethod(__class__.__name__, "GetFlagged", model, flag, type)
