...
class SlipringType(type):

    def __getattr__(cls, name):

        raise AttributeError("Slipring class attribute '{}' does not exist".format(name))


class Slipring(Oasys.gRPC.OasysItem, metaclass=SlipringType):
    _props = {'colour', 'dc', 'direct', 'fc', 'fcs', 'funcid', 'include', 'k', 'label', 'lcnffd', 'lcnffs', 'ltime', 'onid', 'sbid1', 'sbid2', 'sbrnid', 'sbsrid', 'transparency'}
...
# If one of the properties we define then get it
        if name in Slipring._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in Slipring._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("Slipring instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in Slipring._rprops:
            raise AttributeError("Cannot set read-only Slipring instance attribute '{}'".format(name))
...
# Constructor
    def __init__(self, model, sbsrid, sbid1, sbid2, sbrnid):
        handle = Oasys.PRIMER._connection.constructor(self.__class__.__name__, model, sbsrid, sbid1, sbid2, sbrnid)
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free slipring label in the model.
        Also see Slipring.LastFreeLabel(),
        Slipring.NextFreeLabel() and
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free slipring label in the model.
        Also see Slipring.FirstFreeLabel(),
        Slipring.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) slipring label in the model.
        Also see Slipring.FirstFreeLabel(),
        Slipring.LastFreeLabel() and
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only sliprings from that model can be picked.
            If the argument is a Flag then only sliprings that
            are flagged with limit can be selected.
            If omitted, or None, any sliprings from any model can be selected.
            from any model
        modal : boolean
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            Optional. If the argument is a Model then only sliprings from that model can be selected.
            If the argument is a Flag then only sliprings that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any sliprings can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches all of the flagged sliprings in the model. The sliprings will be sketched until you either call
        Slipring.Unsketch(),
        Slipring.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the sliprings are sketched.
            If omitted redraw is true. If you want to sketch flagged sliprings several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing sliprings should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        model : Model
            Model that all sliprings will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not after the sliprings are unsketched.
...
# Instance methods
    ...
    def ExtractColour(self):
        ...
        PRIMER cycles through 13 default colours based on the label of the entity. In this case the slipring colour
        property will return the value Colour.PART instead of the actual colour.
        This method will return the actual colour which is used for drawing the slipring
    ...
    def GetParameter(self, prop):
        """
        Checks if a Slipring property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the Slipring.ViewParameters()
    ...
    def Keyword(self):
        ...
        See also Slipring.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the slipring.
        Note that a carriage return is not added.
        See also Slipring.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches the slipring. The slipring will be sketched until you either call
        Slipring.Unsketch(),
        Slipring.UnsketchAll(),
        Model.UnsketchAll(),
        or delete the model
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the slipring is sketched.
            If omitted redraw is true. If you want to sketch several sliprings and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the slipring is unsketched.
            If omitted redraw is true. If you want to unsketch several sliprings and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
