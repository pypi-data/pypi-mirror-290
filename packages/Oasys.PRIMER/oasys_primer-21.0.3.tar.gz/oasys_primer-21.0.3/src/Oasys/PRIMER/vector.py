...
class VectorType(type):
    ...
    def __getattr__(cls, name):
        ...
        raise AttributeError("Vector class attribute '{}' does not exist".format(name))


class Vector(Oasys.gRPC.OasysItem, metaclass=VectorType):
    _props = {'cid', 'heading', 'include', 'label', 'nodeh', 'nodes', 'nodet', 'vid', 'xh', 'xt', 'yh', 'yt', 'zh', 'zt'}
...
# If one of the properties we define then get it
        if name in Vector._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in Vector._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("Vector instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in Vector._rprops:
            raise AttributeError("Cannot set read-only Vector instance attribute '{}'".format(name))
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free vector label in the model.
        Also see Vector.LastFreeLabel(),
        Vector.NextFreeLabel() and
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free vector label in the model.
        Also see Vector.FirstFreeLabel(),
        Vector.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) vector label in the model.
        Also see Vector.FirstFreeLabel(),
        Vector.LastFreeLabel() and
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only vectors from that model can be picked.
            If the argument is a Flag then only vectors that
            are flagged with limit can be selected.
            If omitted, or None, any vectors from any model can be selected.
            from any model
        modal : boolean
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            Optional. If the argument is a Model then only vectors from that model can be selected.
            If the argument is a Flag then only vectors that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any vectors can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the vectors are sketched.
            If omitted redraw is true. If you want to sketch flagged vectors several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing vectors should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        model : Model
            Model that all vectors will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not after the vectors are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a Vector property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the Vector.ViewParameters()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the vector.
        Note that a carriage return is not added.
        See also Vector.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the vector is sketched.
            If omitted redraw is true. If you want to sketch several vectors and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the vector is unsketched.
            If omitted redraw is true. If you want to unsketch several vectors and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
