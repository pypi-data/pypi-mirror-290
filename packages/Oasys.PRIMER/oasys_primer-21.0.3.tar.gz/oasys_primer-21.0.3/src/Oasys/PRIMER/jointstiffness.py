...
class JointStiffnessType(type):
    _consts = {'CYLINDRICAL', 'FLEXION_TORSION', 'GENERALIZED', 'TRANSLATIONAL'}

    def __getattr__(cls, name):
        if name in JointStiffnessType._consts:
            return Oasys.PRIMER._connection.classGetter(cls.__name__, name)

        raise AttributeError("JointStiffness class attribute '{}' does not exist".format(name))


    def __setattr__(cls, name, value):
# If one of the constants we define then error
        if name in JointStiffnessType._consts:
            raise AttributeError("Cannot set JointStiffness class constant '{}'".format(name))

# Set the property locally
        cls.__dict__[name] = value


class JointStiffness(Oasys.gRPC.OasysItem, metaclass=JointStiffnessType):
    _props = {'cida', 'cidb', 'dlcidal', 'dlcidbt', 'dlcidg', 'dlcidp', 'dlcidph', 'dlcidps', 'dlcidr', 'dlcidt', 'dlcidx', 'dlcidy', 'dlcidz', 'esal', 'esbt', 'esph', 'esps', 'esr', 'est', 'esx', 'esy', 'esz', 'fd', 'ffr', 'ffx', 'ffy', 'ffz', 'fmal', 'fmbt', 'fmph', 'fmps', 'fmt', 'fs', 'include', 'jid', 'jsid', 'label', 'lcidal', 'lcidbt', 'lcidg', 'lcidph', 'lcidps', 'lcidr', 'lcidt', 'lcidx', 'lcidy', 'lcidz', 'nsabt', 'nsaph', 'nsaps', 'nsat', 'nsdx', 'nsdy', 'nsdz', 'option', 'pida', 'pidb', 'psabt', 'psaph', 'psaps', 'psat', 'psdr', 'psdx', 'psdy', 'psdz', 'rad1', 'rad2', 'rps', 'saal', 'title'}
...
# If one of the properties we define then get it
        if name in JointStiffness._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in JointStiffness._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("JointStiffness instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in JointStiffness._rprops:
            raise AttributeError("Cannot set read-only JointStiffness instance attribute '{}'".format(name))
...
# Constructor
    def __init__(self, model, options):
        ...
        """
        Create a new JointStiffness object. The fields on card 1 of the joint stiffness can be set in the constructor
        using the option argument. To set any other values use properties
...
# Static methods
    def BlankAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that all joint stiffnesss will be blanked in
        redraw : boolean
    ...
    def BlankFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that all the flagged joint stiffnesss will be blanked in
        flag : Flag
            Flag set on the joint stiffnesss that you want to blank
        redraw : boolean
            Optional. If model should be redrawn or not.
            If omitted redraw is false. If you want to do several (un)blanks and only
            redraw after the last one then use false for all redraws apart from the last one.
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free joint stiffness label in the model.
        Also see JointStiffness.LastFreeLabel(),
        JointStiffness.NextFreeLabel() and
    ...
    def FlagAll(model, flag):
        ...
        ----------
        model : Model
            Model that all joint stiffnesss will be flagged in
        flag : Flag
            Flag to set on the joint stiffnesss
    ...
    def GetAll(model):
        ...
        ----------
        model : Model
            Model to get joint stiffnesss from
    ...
    def GetFlagged(model, flag):
        ...
        model : Model
            Model to get joint stiffnesss from
        flag : Flag
            Flag set on the joint stiffnesss that you want to retrieve
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free joint stiffness label in the model.
        Also see JointStiffness.FirstFreeLabel(),
        JointStiffness.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) joint stiffness label in the model.
        Also see JointStiffness.FirstFreeLabel(),
        JointStiffness.LastFreeLabel() and
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only joint stiffnesss from that model can be picked.
            If the argument is a Flag then only joint stiffnesss that
            are flagged with limit can be selected.
            If omitted, or None, any joint stiffnesss from any model can be selected.
            from any model
        modal : boolean
    ...
    def RenumberAll(model, start):
        ...
        ----------
        model : Model
            Model that all joint stiffnesss will be renumbered in
        start : integer
    ...
    def RenumberFlagged(model, flag, start):
        ...
        ----------
        model : Model
            Model that all the flagged joint stiffnesss will be renumbered in
        flag : Flag
            Flag set on the joint stiffnesss that you want to renumber
        start : integer
            Start point for renumbering
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        ----------
        flag : Flag
            Flag to use when selecting joint stiffnesss
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            Optional. If the argument is a Model then only joint stiffnesss from that model can be selected.
            If the argument is a Flag then only joint stiffnesss that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any joint stiffnesss can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that all the flagged joint stiffnesss will be sketched in
        flag : Flag
            Flag set on the joint stiffnesss that you want to sketch
        redraw : boolean
            Optional. If model should be redrawn or not after the joint stiffnesss are sketched.
            If omitted redraw is true. If you want to sketch flagged joint stiffnesss several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing joint stiffnesss should be counted. If false or omitted
            referenced but undefined joint stiffnesss will also be included in the total
    ...
    def UnblankAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that all joint stiffnesss will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not.
            If omitted redraw is false. If you want to do several (un)blanks and only
            redraw after the last one then use false for all redraws apart from the last one.
    ...
    def UnblankFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that the flagged joint stiffnesss will be unblanked in
        flag : Flag
            Flag set on the joint stiffnesss that you want to unblank
        redraw : boolean
            Optional. If model should be redrawn or not.
            If omitted redraw is false. If you want to do several (un)blanks and only
            redraw after the last one then use false for all redraws apart from the last one.
    ...
    def UnflagAll(model, flag):
        ...
        ----------
        model : Model
            Model that the defined flag for all joint stiffnesss will be unset in
        flag : Flag
            Flag to unset on the joint stiffnesss
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        model : Model
            Model that all joint stiffnesss will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not after the joint stiffnesss are unsketched.
    ...
    def UnsketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that all joint stiffnesss will be unsketched in
        flag : Flag
            Flag set on the joint stiffnesss that you want to unsketch
        redraw : boolean
            Optional. If model should be redrawn or not after the joint stiffnesss are unsketched.
            If omitted redraw is true. If you want to unsketch several things and only
            redraw after the last one then use false for redraw and call
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a JointStiffness property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the JointStiffness.ViewParameters()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the jstf.
        Note that a carriage return is not added.
        See also JointStiffness.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the joint stiffness is sketched.
            If omitted redraw is true. If you want to sketch several joint stiffnesss and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the joint stiffness is unsketched.
            If omitted redraw is true. If you want to unsketch several joint stiffnesss and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
