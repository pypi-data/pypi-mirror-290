...
class SensorType(type):

    def __getattr__(cls, name):

        raise AttributeError("Sensor class attribute '{}' does not exist".format(name))


class Sensor(Oasys.gRPC.OasysItem, metaclass=SensorType):
    _props = {'acc', 'atime', 'colour', 'dmn', 'dmx', 'dof', 'include', 'label', 'nid', 'nid1', 'nid2', 'pulmn', 'pulmx', 'pulrat', 'pultim', 'sbrid', 'sbsfl', 'sbsid', 'sbstyp', 'time', 'transparency'}
...
# If one of the properties we define then get it
        if name in Sensor._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in Sensor._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("Sensor instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in Sensor._rprops:
            raise AttributeError("Cannot set read-only Sensor instance attribute '{}'".format(name))
...
# Constructor
    def __init__(self, model, sbsid, sbstyp, sbsfl=Oasys.gRPC.defaultArg, nid=Oasys.gRPC.defaultArg, nid2=Oasys.gRPC.defaultArg):
        handle = Oasys.PRIMER._connection.constructor(self.__class__.__name__, model, sbsid, sbstyp, sbsfl, nid, nid2)
        ...
        sbstyp : integer
            Sensor type
        sbsfl : integer
            Optional. Sensor flag. Default 0
        nid : integer
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free sensor label in the model.
        Also see Sensor.LastFreeLabel(),
        Sensor.NextFreeLabel() and
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free sensor label in the model.
        Also see Sensor.FirstFreeLabel(),
        Sensor.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) sensor label in the model.
        Also see Sensor.FirstFreeLabel(),
        Sensor.LastFreeLabel() and
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only sensors from that model can be picked.
            If the argument is a Flag then only sensors that
            are flagged with limit can be selected.
            If omitted, or None, any sensors from any model can be selected.
            from any model
        modal : boolean
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            Optional. If the argument is a Model then only sensors from that model can be selected.
            If the argument is a Flag then only sensors that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any sensors can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the sensors are sketched.
            If omitted redraw is true. If you want to sketch flagged sensors several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing sensors should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        model : Model
            Model that all sensors will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not after the sensors are unsketched.
...
# Instance methods
    ...
    def ExtractColour(self):
        ...
        PRIMER cycles through 13 default colours based on the label of the entity. In this case the sensor colour
        property will return the value Colour.PART instead of the actual colour.
        This method will return the actual colour which is used for drawing the sensor
    ...
    def GetParameter(self, prop):
        """
        Checks if a Sensor property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the Sensor.ViewParameters()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the sensor.
        Note that a carriage return is not added.
        See also Sensor.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the sensor is sketched.
            If omitted redraw is true. If you want to sketch several sensors and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the sensor is unsketched.
            If omitted redraw is true. If you want to unsketch several sensors and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
