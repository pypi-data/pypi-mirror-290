...
class SphType(type):
    ...
    def __getattr__(cls, name):
        ...
        raise AttributeError("Sph class attribute '{}' does not exist".format(name))


class Sph(Oasys.gRPC.OasysItem, metaclass=SphType):
    _props = {'colour', 'include', 'mass', 'nid', 'pid', 'transparency'}
...
# If one of the properties we define then get it
        if name in Sph._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in Sph._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("Sph instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in Sph._rprops:
            raise AttributeError("Cannot set read-only Sph instance attribute '{}'".format(name))
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free sph label in the model.
        Also see Sph.LastFreeLabel(),
        Sph.NextFreeLabel() and
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free sph label in the model.
        Also see Sph.FirstFreeLabel(),
        Sph.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) sph label in the model.
        Also see Sph.FirstFreeLabel(),
        Sph.LastFreeLabel() and
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only sphs from that model can be picked.
            If the argument is a Flag then only sphs that
            are flagged with limit can be selected.
            If omitted, or None, any sphs from any model can be selected.
            from any model
        modal : boolean
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            Optional. If the argument is a Model then only sphs from that model can be selected.
            If the argument is a Flag then only sphs that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any sphs can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the sphs are sketched.
            If omitted redraw is true. If you want to sketch flagged sphs several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing sphs should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        model : Model
            Model that all sphs will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not after the sphs are unsketched.
...
# Instance methods
    ...
    def ExtractColour(self):
        ...
        PRIMER cycles through 13 default colours based on the label of the entity. In this case the sph colour
        property will return the value Colour.PART instead of the actual colour.
        This method will return the actual colour which is used for drawing the sph
    ...
    def GetParameter(self, prop):
        """
        Checks if a Sph property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the Sph.ViewParameters()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the sph.
        Note that a carriage return is not added.
        See also Sph.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the sph is sketched.
            If omitted redraw is true. If you want to sketch several sphs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the sph is unsketched.
            If omitted redraw is true. If you want to unsketch several sphs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
