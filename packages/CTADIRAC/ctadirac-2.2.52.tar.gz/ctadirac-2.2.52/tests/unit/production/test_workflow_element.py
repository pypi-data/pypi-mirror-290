...
def test_we_set_constrained_job_attribute() -> None:
    ...
    # Processing
    workflow_elmt_proc = WorkflowElement(workflow_elmt_sim.prod_step, "ctapipeprocessing")
    assert getattr(workflow_elmt_proc, "moon", None) is None
    workflow_elmt_proc.set_constrained_job_attribute("moon", "half")
    assert workflow_elmt_proc.job.output_file_metadata["nsb"] == 5
...
def test_build_input_data() -> None:
    ...
    workflow_elmt_simtel = WorkflowElement(workflow_elmt_sim.prod_step, "simtelprocessing")
...
def test_build_job_attributes() -> None:
    ...
    workflow_elmt_simtel.build_job_attributes(WORKFLOW_CONFIG, SIM_TELARRAY_PROCESSING_CONFIG)
...
def test_build_element_config() -> None:
    ...
    descr_short: list[str] = [
        ...
        if "descr_short" in line and line.replace("<descr_short>", "").replace("</descr_short>", "")
    ]
    assert descr_short == [
        "Setup software",
        "Run ctapipe merge",
...
def test_get_merging_level() -> None:
    ...
    assert res_merged == 0
    worflow_elmt_merge.prod_step.ParentStep.Outputquery = deepcopy(CTAPIPE_PROCESS_OUTPUT_METADATA)
