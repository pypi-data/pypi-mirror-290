...
class SPScienceCalibration(ScienceCalibrationBase):
    ...
    def combine_beams_into_fits_access(
        ...
        if self.constants.correct_for_polarization:
            avg_array = self._combine_polarimetric_beams(array_dict)
        else:
            avg_array = self._combine_spectrographic_beams(array_dict)
    ...
    def _combine_polarimetric_beams(self, array_dict: dict[str, np.ndarray]) -> np.ndarray:
        """
    ...
    def _combine_spectrographic_beams(self, array_dict: dict[str, np.ndarray]) -> np.ndarray:
