...
class SPBeamBoundariesCalibration(BeamBoundariesCalibrationBase, ShiftMeasurementsMixin):
    ...
    def _split_beams(self, array: np.ndarray) -> list[np.ndarray]:
        ...
        """
        self.split_boundary = self._compute_split_boundary(array)
    ...
    def _compute_final_beam_boundaries(
        ...
        # Step 8:
        spectral_shift, spatial_shift = self._compute_offset(beam_1_array, beam_2_array)

        # Step 9:
        new_beam_boundaries = self._compute_shared_boundaries_from_overlapping_beams(
            boundaries, spectral_shift, spatial_shift
        )
        return new_beam_boundaries

    def _compute_split_boundary(self, average_solar_gain_array: np.ndarray) -> list[int]:
        """
    ...
    def _compute_offset(self, beam_1: np.ndarray, beam_2: np.ndarray) -> list[int]:
        """
    ...
    def _compute_shared_boundaries_from_overlapping_beams(
        self,
