...
class SPSolarGainCalibration(CryonirspTaskBase):
                ...
                for beam in range(1, self.constants.num_beams + 1):
                    ...
                    if self.parameters.fringe_correction_on:
                        with self.apm_processing_step(
                            ...
                            final_gain = self._compute_fringe_corrected_gain(
    ...
    def _do_initial_corrections(
        ...
        basic_corrected_solar_array = self._do_dark_and_bad_pixel_corrections(
            beam, exposure_conditions
        )
    ...
    def _do_dark_and_bad_pixel_corrections(
        self, beam: int, exposure_conditions: ExposureConditions
    ) -> np.ndarray:
        """
    ...
    def _compute_char_spectrum(self, array: np.ndarray, beam: int) -> np.ndarray:
        """
    ...
    def _geo_corrected_data(self, beam: int, exposure_conditions: ExposureConditions) -> np.ndarray:
        """
    ...
    def _remove_solar_signal(
        ...
        input_gain = self._geo_corrected_data(beam=beam, exposure_conditions=exposure_conditions)
        array_with_solar_signal_removed = input_gain / char_solar_spectra
        return array_with_solar_signal_removed

    def _write_solar_gain_calibration(self, gain_array: np.ndarray, beam: int) -> None:
        """
    ...
    def _compute_fringe_corrected_gain(self, beam: int, exposure_conditions: float) -> np.ndarray:
        """
        ...
        with self.apm_processing_step(f"Perform initial corrections for {apm_str}"):
            corrected_solar_array = self._do_dark_and_bad_pixel_corrections(
                beam=beam, exposure_conditions=exposure_conditions
            )

        with self.apm_processing_step(f"Compute the flux-scaled lamp gain for {apm_str}"):
            scaled_lamp_array = self._compute_flux_scaled_lamp_gain(corrected_solar_array, beam)

        with self.apm_processing_step(f"Apply spectral filtering for {apm_str}"):
            filtered_lamp_array = self._apply_spectral_and_spatial_filtering(scaled_lamp_array)

        with self.apm_processing_step(f"Isolate and remove fringes for {apm_str}"):
            final_gain_array = self._isolate_and_remove_fringes(
                filtered_lamp_array, scaled_lamp_array
            )

        return final_gain_array

    def _compute_flux_scaled_lamp_gain(
    ...
    def _apply_spectral_and_spatial_filtering(self, scaled_lamp_array: np.ndarray) -> np.ndarray:
        """
    ...
    def _isolate_and_remove_fringes(
