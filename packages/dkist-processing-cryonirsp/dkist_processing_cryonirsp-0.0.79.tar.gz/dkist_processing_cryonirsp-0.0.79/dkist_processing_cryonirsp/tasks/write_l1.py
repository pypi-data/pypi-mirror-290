...
class CryonirspWriteL1Frame(WriteL1Frame, ABC, InputDatasetMixin):
    ...
    def add_dataset_headers(
        ...
        boresight_coordinates = self._get_boresight_coords(header)
        self._correct_spatial_wcs_info(header, boresight_coordinates)
        self._update_spectral_headers(header)
    ...
    @abstractmethod
    def _get_boresight_coords(self, header: fits.Header) -> tuple[float, float]:
        """Get boresight coordinates to use in spatial correction."""
        pass

    def _correct_spatial_wcs_info(
        ...
        x, y, wci_updated_eq, crval_eq = self._get_arm_specific_coords(
    ...
    def _update_spectral_headers(self, header: fits.Header):
...
class CIWriteL1Frame(CryonirspWriteL1Frame):
    ...
    def correct_helioprojective_coords(
        ...
        # correct pcij values
        ##################################################################################
        crval = np.array([header["CRVAL1"], header["CRVAL1"]])
        pcij_updated = (
