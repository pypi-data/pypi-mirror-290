"""APXServeController: the central controller of APXServe.
...
from apx.serve import autoscalers
...
class APXServeController:
    """APXServeController: control everything about replica.

    This class is responsible for:
        - Starting and terminating the replica monitor and autoscaler.
        - Providing the HTTP Server API for APXServe to communicate with.
    """

    def __init__(self, service_name: str, service_spec: serve.ApxServiceSpec,
                 task_yaml: str, host: str, port: int) -> None:
        self._service_name = service_name
        self._replica_manager: replica_managers.ReplicaManager = (
            replica_managers.APXReplicaManager(service_name=service_name,
                                                    spec=service_spec,
    ...
    def run(self) -> None:
        ...
        async def update_service(request: fastapi.Request):
            ...
            try:
                ...
                # See apx/serve/core.py::update
                latest_task_yaml = serve_utils.generate_task_yaml_file_name(
                    self._service_name, version)
                service = serve.ApxServiceSpec.from_yaml(latest_task_yaml)
                logger.info(
        ...
        def configure_logger():
            ...
            for handler in uvicorn_access_logger.handlers:
                handler.setFormatter(apx_logging.FORMATTER)

        threading.Thread(target=self._run_autoscaler).start()

        logger.info('APXServe Controller started on '
...
def run_controller(service_name: str, service_spec: serve.ApxServiceSpec,
                   task_yaml: str, controller_host: str, controller_port: int):
    controller = APXServeController(service_name, service_spec, task_yaml,
