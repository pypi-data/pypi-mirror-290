...
from apx.serve import load_balancing_policies as lb_policies
...
class APXServeLoadBalancer:
    """APXServeLoadBalancer: distribute incoming traffic with proxy.
    ...
    async def _proxy_with_retries(
        ...
        # APXServe supports serving on Spot Instances. To avoid preemptions
        ...
        while True:
            ...
            if ready_replica_url is None:
                response_or_exception = fastapi.HTTPException(
                    ...
                    'Use "apx serve status [SERVICE_NAME]" '
            ...
            if retry_cnt == constants.LB_MAX_RETRY:
                ...
                raise fastapi.HTTPException(
                    ...
                    '"apx serve logs [SERVICE_NAME] --load-balancer" '
    ...
    def run(self):
        ...
        logger.info('APXServe Load Balancer started on '
...
def run_load_balancer(controller_addr: str, load_balancer_port: int):
    load_balancer = APXServeLoadBalancer(controller_url=controller_addr,
