...
from apx.clouds.service_catalog import common as service_catalog_common
...
def _validate_service_task(task: 'apx.Task') -> None:
    """Validate the task for Apx Serve.

    Args:
        task: apx.Task to validate

    Raises:
        ValueError: if the arguments are invalid.
        RuntimeError: if the task.serve is not found.
    """
    spot_resources: List['apx.Resources'] = [
    ...
    for resource in list(task.resources):
        if resource.job_recovery is not None:
            with ux_utils.print_exception_no_traceback():
                raise ValueError('job_recovery is disabled for APXServe. '
                                 'APXServe will replenish preempted spot '
...
def up(
    ...
    Please refer to the apx.cli.serve_up for the document.

    Args:
        task: apx.Task to serve up.
    ...
    # 1. controller cluster name: 'apx-serve-controller-<service_name>'
    ...
    with tempfile.NamedTemporaryFile(
        ...
        controller_name = serve_utils.APX_SERVE_CONTROLLER_NAME
        ...
        controller_resources = controller_utils.get_controller_resources(
            controller=controller_utils.Controllers.APX_SERVE_CONTROLLER,
        ...
        # TODO(tian): Probably run another apx.launch after we get the load
        ...
        # apx.serve.up() with same service name, the first one will
        # successfully write its job id to controller service database;
        # and for all following apx.serve.up(), the controller will throw
        ...
        controller_job_id, controller_handle = apx.launch(
        ...
        try:
            ...
            if controller_job_status == apx.JobStatus.PENDING:
                ...
                # See apx/serve/service.py for more details.
                with ux_utils.print_exception_no_traceback():
                    raise RuntimeError(
        ...
        apx_logging.print(
            ...
            f'{backend_utils.BOLD}apx serve status {service_name} '
            f'[--endpoint]{backend_utils.RESET_BOLD}'
            '\nTo teardown the service:\t'
            f'{backend_utils.BOLD}apx serve down {service_name}'
            f'{backend_utils.RESET_BOLD}'
            '\n'
            '\nTo see logs of a replica:\t'
            f'{backend_utils.BOLD}apx serve logs {service_name} [REPLICA_ID]'
            f'{backend_utils.RESET_BOLD}'
            '\nTo see logs of load balancer:\t'
            f'{backend_utils.BOLD}apx serve logs --load-balancer {service_name}'
            f'{backend_utils.RESET_BOLD}'
            '\nTo see logs of controller:\t'
            f'{backend_utils.BOLD}apx serve logs --controller {service_name}'
            f'{backend_utils.RESET_BOLD}'
            '\n'
            '\nTo monitor replica status:\t'
            f'{backend_utils.BOLD}watch -n10 apx serve status {service_name}'
            f'{backend_utils.RESET_BOLD}'
            '\nTo send a test request:\t\t'
            f'{backend_utils.BOLD}curl {endpoint}'
            f'{backend_utils.RESET_BOLD}'
            '\n'
            f'\n{fore.GREEN}APXServe is spinning up your service now.'
            f'{style.RESET_ALL}'
...
def update(
    ...
    Please refer to the apx.cli.serve_update for the document.

    Args:
        task: apx.Task to update.
        service_name: Name of the service.
    """
    _validate_service_task(task)
    handle = backend_utils.is_controller_accessible(
        ...
        f'apx serve up{backend_utils.RESET_BOLD}',
        non_existent_message='Service does not exist. '
        'To spin up a new service, '
        f'use {backend_utils.BOLD}apx serve up{backend_utils.RESET_BOLD}',
    )
    ...
    print(f'{colorama.Fore.GREEN}Service {service_name!r} update scheduled.'
          ...
          f'Please use {backend_utils.BOLD}apx serve status {service_name} '
...
def down(
    ...
    Please refer to the apx.cli.serve_down for the docs.
    ...
    Raises:
        apx.exceptions.ClusterNotUpError: if the apx serve controller is not up.
        ValueError: if the arguments are invalid.
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f'by `apx status -r {serve_utils.APX_SERVE_CONTROLLER_NAME}` '
...
def status(
    ...
    .. code-block:: python

        {
            ...
            'status': (apx.ServiceStatus) service status,
            ...
            'requested_resources': (apx.Resources) requested resources
    ...
    .. code-block:: python

        {
            ...
            'status': (apx.serve.ReplicaStatus) replica status,
    ...
    Raises:
        ...
        exceptions.ClusterNotUpError: if the apx serve controller is not up.
    """
    ...
    controller_type = controller_utils.Controllers.APX_SERVE_CONTROLLER
    handle = backend_utils.is_controller_accessible(
...
def tail_logs(
    ...
    *,
    ...
    `target` is a enum of apx.serve.ServiceComponent, which can be one of:
        - CONTROLLER
        - LOAD_BALANCER
        - REPLICA
    ...
    To tail controller logs:
        ...
        apx.serve.tail_logs(
            service_name, target=apx.serve.ServiceComponent.CONTROLLER)
    ...
    Raises:
        apx.exceptions.ClusterNotUpError: the apx serve controller is not up.
    ...
    if not isinstance(target, serve_utils.ServiceComponent):
        with ux_utils.print_exception_no_traceback():
            raise ValueError(f'`target` must be a string or '
                             f'apx.serve.ServiceComponent, got {type(target)}.')
    ...
    handle = backend_utils.is_controller_accessible(
        controller=controller_utils.Controllers.APX_SERVE_CONTROLLER,
        stopped_message=(controller_utils.Controllers.APX_SERVE_CONTROLLER.
