...
from apx import authentication
...
from apx.serve import controller
from apx.serve import load_balancer
...
# `apx.serve.service` namespace when executed directly, so as
# to inherit the setup from the `apx` logger.
logger = apx_logging.init_logger('apx.serve.service')
...
def _start(service_name: str, tmp_task_yaml: str, job_id: int):
    """Starts the service."""
    # Generate ssh key pair to avoid race condition when multiple apx.launch
    ...
    # Directly throw an error here. See apx/serve/api.py::up
    # for more details.
    if not success:
        cleanup_storage(tmp_task_yaml)
        with ux_utils.print_exception_no_traceback():
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Apx Serve Service')
