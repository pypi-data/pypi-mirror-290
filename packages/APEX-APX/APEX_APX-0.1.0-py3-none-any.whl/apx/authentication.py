"""Module to enable a single APX key for all VMs in each cloud.
...
1. Replace the placeholders in the ray yaml file `apxdeploy:ssh_user` and
   `apxdeploy:ssh_public_key_content` with the actual username and public key
...
`~/.ssh/apx-key.pub` on the remote VM, because it will cause private/public
key pair mismatch when the user tries to launch new VM from that remote VM
using APX, e.g., the node is used as a jobs controller. (Lambda cloud
...
from apx.adaptors import runpod
from apx.clouds.utils import lambda_utils
from apx.provision.fluidstack import fluidstack_utils
...
PRIVATE_SSH_KEY_PATH = '~/.ssh/apx-key'
PUBLIC_SSH_KEY_PATH = '~/.ssh/apx-key.pub'
_SSH_KEY_GENERATION_LOCK = '~/.apx/generated/ssh/.__internal-apx-key.lock'
...
def get_or_generate_keys() -> Tuple[str, str]:
    ...
    # We should have the folder ~/.apx/generated/ssh to have 0o700 permission,
...
def configure_ssh_info(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    config_str = config_str.replace('apxdeploy:ssh_user',
                                    config['auth']['ssh_user'])
    config_str = config_str.replace('apxdeploy:ssh_public_key_content',
                                    public_key)
...
def setup_lambda_authentication(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with open(public_key_path, 'r', encoding='utf-8') as f:
        public_key = f.read().strip()
    prefix = f'apx-key-{common_utils.get_user_hash()}'
...
def setup_ibm_authentication(config):
    """ registers keys if they do not exist in apx folder
    and updates config file.
    keys default location: '~/.ssh/apx-key' and '~/.ssh/apx-key.pub'
    """

    def _get_unique_key_name():
        suffix_len = 10
        return f'apxdeploy-key-{str(uuid.uuid4())[:suffix_len]}'
...
def setup_kubernetes_authentication(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    network_mode_str = apxdeploy_config.get_nested(('kubernetes', 'networking'),
                                                  port_forward_mode.value)
    try:
        network_mode = kubernetes_enums.KubernetesNetworkingMode.from_str(
            network_mode_str)
    except ValueError as e:
        # Add message saying "Please check: ~/.apx/config.yaml" to the error
        # message.
        with ux_utils.print_exception_no_traceback():
            raise ValueError(str(e) + ' Please check: ~/.apx/config.yaml.') \
                from None
    get_or_generate_keys()

    # Add the user's public key to the APX cluster.
    public_key_path = os.path.expanduser(PUBLIC_SSH_KEY_PATH)
    secret_name = clouds.Kubernetes.APX_SSH_KEY_SECRET_NAME
    ...
    if network_mode == nodeport_mode:
        ssh_jump_name = clouds.Kubernetes.APX_SSH_JUMP_NAME
