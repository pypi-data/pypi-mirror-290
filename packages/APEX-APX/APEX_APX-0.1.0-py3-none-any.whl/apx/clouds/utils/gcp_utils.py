...
def _list_reservations_for_instance_type(
    ...
    in us-central1-c. `apx launch --gpus V100` will fail.
    """
    prioritize_reservations = apxdeploy_config.get_nested(
        ('gcp', 'prioritize_reservations'), False)
    specific_reservations = apxdeploy_config.get_nested(
...
def get_minimal_permissions() -> List[str]:
    ...
    if apxdeploy_config.get_nested(('gcp', 'vpc_name'), None) is None:
        # If custom VPC is not specified, permissions to modify network are
        # required to ensure APX to be able to setup the network, and
        # allow opening ports (e.g., via `resources.ports`).
        permissions += constants.FIREWALL_PERMISSIONS

    if (apxdeploy_config.get_nested(('gcp', 'prioritize_reservations'), False) or
            apxdeploy_config.get_nested(('gcp', 'specific_reservations'), [])):
