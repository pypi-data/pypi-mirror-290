...
class OCIConfig:
    ...
    COMPARTMENT = 'apxdeploy_compartment'
    VCN_NAME = 'apxdeploy_vcn'
    VCN_DNS_LABEL = 'apxdeployvcn'
    VCN_INTERNET_GATEWAY_NAME = 'apxdeploy_vcn_internet_gateway'
    VCN_SUBNET_NAME = 'apxdeploy_subnet'
    ...
    # Map APX disk_tier to OCI VPU (Volume Performance Units) for Boot
    # Volume.
    ...
    def get_compartment(cls, region):
        ...
        default_compartment_ocid = apxdeploy_config.get_nested(
            ('oci', 'default', 'compartment_ocid'), None)
        compartment = apxdeploy_config.get_nested(
            ('oci', region, 'compartment_ocid'), default_compartment_ocid)
        return compartment

    @classmethod
    def get_vcn_subnet(cls, region):
        vcn = apxdeploy_config.get_nested(('oci', region, 'vcn_subnet'), None)
        return vcn
    ...
    def get_default_gpu_image_tag(cls) -> str:
        ...
        # the apx's user-config file (if not specified, use the hardcode one at
        # last)
        return apxdeploy_config.get_nested(('oci', 'default', 'image_tag_gpu'),
                                          'apxdeploy:gpu-ubuntu-2004')

    @classmethod
    def get_default_image_tag(cls) -> str:
        # Get the default image tag. Instead of hardcoding, we give a choice to
        # set the default image tag in the apx's user-config file. (if not
        # specified, use the hardcode one at last)
        return apxdeploy_config.get_nested(
            ('oci', 'default', 'image_tag_general'), 'apxdeploy:cpu-ubuntu-2004')

    @classmethod
    def get_apx_user_config_file(cls) -> str:
        config_path_via_env_var = os.environ.get(
            apxdeploy_config.ENV_VAR_APXDEPLOY_CONFIG)
        if config_path_via_env_var is not None:
            config_path = config_path_via_env_var
        else:
            config_path = apxdeploy_config.CONFIG_PATH
        return config_path
