...
clouds.Cloud is lightweight stateless objects. APX may create multiple such
...
cloud provider and cached, create a module in apx/clouds/utils/ and probably add
caches for the return value (e.g., apx/clouds/utils/gcp_utils), so they can be
...
class ProvisionerVersion(enum.Enum):
    ...
    2: [Deprecated] ray node provider for provisioning and APX provisioner
    for stopping and termination
    3: APX provisioner for both provisioning and stopping
    """
    RAY_AUTOSCALER = 1
    RAY_PROVISIONER_APXDEPLOY_TERMINATOR = 2
    APXDEPLOY = 3
...
class StatusVersion(enum.Enum):
    ...
    2: APX provisioner based implementation
    """
    CLOUD_CLI = 1
    APXDEPLOY = 2
...
class Cloud:
    ...
    # NOTE: new clouds being added should use the latest version, i.e. APXDEPLOY.
    ...
    def make_deploy_resources_variables(
        ...
        """Converts planned apx.Resources to cloud-specific resource variables.
    ...
    def get_current_user_identity(cls) -> Optional[List[str]]:
        ...
        The user "identity" is associated with each APX cluster they
    ...
    def check_quota_available(cls,
        ...
        APX would attempt to provision resources in all of these regions.
    ...
    # are currently only used in `apx launch --clone-disk-from` to clone a
