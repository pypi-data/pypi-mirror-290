...
class IBMVPCProvider:
    ...
    def create_vpc(self):
        ...
        vpc_data = self.vpc_client.create_vpc(
            ...
            name=f"apx-vpc-{self.cluster_name}-{str(uuid.uuid4())[:5]}",
    ...
    def create_subnet(self, vpc_id, zone_name):
        ...
        subnet_name = f"apx-subnet-{self.cluster_name}-{str(uuid.uuid4())[:5]}"
    ...
    def remote_cluster_removal(self, vpc_id, region):
        ...
        remotely, e.g. by `apx autostop --down`"""
...
class ClusterCleaner:
    ...
    namespace_name = "apxdeploy-namespace"
    # default name for the cloud function action.
    action_name = "apxdeploy-vpc-cleaner-action"
