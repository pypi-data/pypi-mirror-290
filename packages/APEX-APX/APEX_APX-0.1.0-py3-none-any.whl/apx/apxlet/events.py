...
# Seconds of sleep between the processing of apxlet events.
    ...
    def run(self):
        ...
        if self._n % self._event_interval == 0:
            ...
            try:
                ...
                # Keep the apxlet running even if an event fails.
...
class JobSchedulerEvent(ApxletEvent):
    """Apxlet event for scheduling jobs"""
...
class ManagedJobUpdateEvent(ApxletEvent):
    """Apxlet event for updating managed job status."""
    EVENT_INTERVAL_SECONDS = 300
...
class ServiceUpdateEvent(ApxletEvent):
    """Apxlet event for updating apx serve service status.
...
class AutostopEvent(ApxletEvent):
    """Apxlet event for autostop.
    ...
    def _stop_cluster(self, autostop_config):
        if (autostop_config.backend ==
            ...
            if (cloud.PROVISIONER_VERSION >= clouds.ProvisionerVersion.
                    RAY_PROVISIONER_APXDEPLOY_TERMINATOR):
            ...
            if is_cluster_multinode:
                ...
                subprocess.run(f'{constants.APX_PYTHON_CMD} {script}',
                               check=True,
                               shell=True,
                               env=env)
                ...
                subprocess.run(
                    f'{constants.APX_RAY_CMD} down -y --workers-only '
                    f'{config_path}',
                    check=True,
                    shell=True,
            ...
            subprocess.run(f'{constants.APX_RAY_CMD} stop',
                           shell=True,
                           check=True)

            logger.info('Running final ray down.')
            subprocess.run(
                f'{constants.APX_RAY_CMD} down -y {config_path}',
                check=True,
                shell=True,
                # We pass env inherited from os.environ due to calling `ray
                # <cmd>`.
    ...
    def _stop_cluster_with_new_provisioner(self, autostop_config,
        ...
        logger.info('Stopping the ray cluster.')
        subprocess.run(f'{constants.APX_RAY_CMD} stop', shell=True, check=True)
    ...
    def _replace_yaml_for_stopping(self, yaml_path: str, down: bool):
        ...
        # 'auth:', `ray up ~/.apx/apx_ray.yaml` on the head node will fail (in
