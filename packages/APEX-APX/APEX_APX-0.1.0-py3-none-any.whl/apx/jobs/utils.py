...
from apx.jobs import state as managed_job_state
...
if typing.TYPE_CHECKING:
    import apx
    from apx import dag as dag_lib
...
JOB_CONTROLLER_NAME: str = (
    f'apx-jobs-controller-{common_utils.get_user_hash()}')
LEGACY_JOB_CONTROLLER_NAME: str = (
    f'apx-spot-controller-{common_utils.get_user_hash()}')
SIGNAL_FILE_PREFIX = '/tmp/apx_jobs_controller_signal_{}'
LEGACY_SIGNAL_FILE_PREFIX = '/tmp/apx_spot_controller_signal_{}'
...
# the managed job status fails to be updated and keep the `apx jobs logs`
    ...
    for job_id_ in job_ids:
        ...
        if controller_status is None or controller_status.is_terminal():
            ...
            managed_job_state.set_failed(
                ...
                f' run: apx jobs logs --controller {job_id_}')
...
def event_callback_func(job_id: int, task_id: int, task: 'apx.Task'):
    ...
    def callback_func(status: str):
        ...
        log_path = os.path.join(constants.APX_LOGS_DIRECTORY,
                                'managed_job_event',
        ...
        result = log_lib.run_bash_command_with_log(
            ...
            log_path=log_path,
            env_vars=dict(
                APXDEPLOY_TASK_ID=str(
                    task.envs.get(constants.TASK_ID_ENV_VAR, 'N.A.')),
                APXDEPLOY_TASK_IDS=str(
...
def generate_managed_job_cluster_name(task_name: str, job_id: int) -> str:
    ...
    # the underlying apx.launch, hiding the `job_id` in the cluster name.
...
class ManagedJobCodeGen:
    ...
    _PREFIX = textwrap.dedent("""\
        ...
        try:
            from apx.jobs import utils
            from apx.jobs import constants as managed_job_constants
            from apx.jobs import state as managed_job_state

            managed_job_version = managed_job_constants.MANAGED_JOBS_VERSION
        except ImportError:
            from apx.spot import spot_state as managed_job_state
            from apx.spot import spot_utils as utils
        """)
    ...
    def stream_logs(cls,
        ...
        from apx.apxlet import job_lib, log_lib
        from apx.apxlet import constants
        try:
            from apx.jobs.utils import stream_logs_by_id
        except ImportError:
            from apx.spot.spot_utils import stream_logs_by_id
    ...
    def _build(cls, code: str) -> str:
        ...
        return f'{constants.APX_PYTHON_CMD} -u -c {shlex.quote(generated_code)}'
