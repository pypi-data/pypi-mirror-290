...
from apx.benchmark import benchmark_state
...
_APX_LOCAL_BENCHMARK_DIR = os.path.expanduser('~/.apx/benchmarks')
_APX_REMOTE_BENCHMARK_DIR = '~/.apx/apx_benchmark_dir'
# NOTE: This must be the same as _APX_REMOTE_BENCHMARK_DIR
# in apx/callbacks/apx_callback/base.py.
_APX_REMOTE_BENCHMARK_DIR_SYMLINK = '~/apx_benchmark_dir'
...
def _generate_cluster_names(benchmark: str, num_clusters: int) -> List[str]:
    if num_clusters == 1:
        names = [f'apx-bench-{benchmark}']
    else:
        names = [f'apx-bench-{benchmark}-{i}' for i in range(num_clusters)]
    for name in names:
...
def _get_optimized_resources(
    ...
    for config in candidate_configs:
        with apx.Dag() as dag:
            ...
            resources = apx.Resources.from_yaml_config(resources)
            task = apx.Task()
            task.set_resources(resources)

        dag = apx.optimize(dag, quiet=True)
        task = dag.tasks[0]
...
def _create_benchmark_bucket() -> Tuple[str, str]:
    ...
    bucket_name = f'apx-bench-{uuid.uuid4().hex[:4]}-{getpass.getuser()}'

    # Select the bucket type.
    enabled_clouds = storage_lib.get_cached_enabled_storage_clouds_or_refresh(
        raise_if_no_cloud_access=True)
    # Apx Benchmark only supports S3 (see _download_remote_dir and
    # _delete_remote_dir).
    enabled_clouds = [
        cloud for cloud in enabled_clouds if cloud in [str(clouds.AWS())]
    ]
    assert enabled_clouds, ('No enabled cloud storage found. Apx Benchmark '
...
def _parallel_run_with_interrupt_handling(func: Callable,
    ...
    with pool.ThreadPool(processes=len(args)) as p:
        try:
            ...
            subprocess_utils.run('apx status')
            sys.exit(1)
...
def _launch_with_log_suppress_exception(
    ...
    """Executes `apx launch` in a subprocess and returns normally.
...
def _update_benchmark_result(benchmark_result: Dict[str, Any]) -> Optional[str]:
    ...
    local_dir = os.path.join(_APX_LOCAL_BENCHMARK_DIR, benchmark, cluster)
    ...
    callback_log_dirs = glob.glob(os.path.join(local_dir, 'apx-callback-*'))
    ...
    if summary_path is not None and os.path.exists(summary_path):
        # (1) ApxCallback has saved the summary.
        ...
        # without ApxCallback logs.
        record = benchmark_state.BenchmarkRecord(start_time=start_time,
                                                 last_time=end_time)
    elif job_status == job_lib.JobStatus.RUNNING:
        # (3) ApxCallback is not initialized yet or not used.
        message = ('ApxCallback is not initialized yet '
...
def generate_benchmark_configs(
    ...
    for cluster, candidate in zip(clusters, candidates):
        ...
        # Mount the benchmark bucket to APX_BENCHMARK_DIR.
        if 'file_mounts' not in candidate_config:
            candidate_config['file_mounts'] = {}
        # The bucket name and type are specified at launch time.
        candidate_config['file_mounts'][_APX_REMOTE_BENCHMARK_DIR] = {
            'name': None,
            'mode': 'MOUNT',
            'store': None,
        }

        benchmark_dir = os.path.join(_APX_REMOTE_BENCHMARK_DIR, benchmark,
                                     cluster)
        ...
        candidate_config['setup'] = textwrap.dedent(f"""\
            ...
            ln -s {benchmark_dir} {_APX_REMOTE_BENCHMARK_DIR_SYMLINK}
            {candidate_config['setup']}""")
...
def launch_benchmark_clusters(benchmark: str, clusters: List[str],
    ...
    # Use a Apx storage to save the benchmark logs.
    ...
    # The benchmark bucket is mounted to _APX_REMOTE_BENCHMARK_DIR.
    for candidate_config in candidate_configs:
        bucket_config = candidate_config['file_mounts'][
            _APX_REMOTE_BENCHMARK_DIR]
    ...
    launch_cmds = [['apx', 'launch', yaml_fd.name, '-c', cluster] + cmd
    ...
    log_dir = os.path.join(constants.APX_LOGS_DIRECTORY, run_timestamp)
    ...
    # add the benchmark to the state so that `apx bench down` can
...
def update_benchmark_state(benchmark: str) -> None:
    ...
    handle = global_user_state.get_handle_from_storage_name(bucket_name)
    bucket_type = list(handle.apx_stores.keys())[0]
    ...
    local_dir = os.path.join(_APX_LOCAL_BENCHMARK_DIR, benchmark)
