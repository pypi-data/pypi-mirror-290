...
def get_port_mode(
    ...
    if running_kind:
        # If running in kind (`apx local up`), use ingress mode
        return kubernetes_enums.KubernetesPortMode.INGRESS

    mode_str = mode_str or apxdeploy_config.get_nested(
        ('kubernetes', 'ports'),
...
def get_networking_mode(
    ...
    try:
        ...
        with ux_utils.print_exception_no_traceback():
            raise ValueError(str(e) +
                             ' Please check: ~/.apx/config.yaml.') from None
...
def fill_loadbalancer_template(namespace: str, service_name: str,
    ...
    with open(template_path, 'r', encoding='utf-8') as fin:
        template = fin.read()
    annotations = apxdeploy_config.get_nested(
        ('kubernetes', 'custom_metadata', 'annotations'), {})
    labels = apxdeploy_config.get_nested(
...
def get_ingress_external_ip_and_ports(
    ...
    if ingress_service.status.load_balancer.ingress is None:
        ...
        # 2. Use the apxdeploy.co/external-ip annotation in the service
