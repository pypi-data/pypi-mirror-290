...
from apx.utils.resources_utils import port_ranges_to_set

logger = apx_logging.init_logger(__name__)

_PATH_PREFIX = '/apxdeploy/{namespace}/{cluster_name_on_cloud}/{port}'
_LOADBALANCER_SERVICE_NAME = '{cluster_name_on_cloud}--apxdeploy-lb'
...
def _open_ports_using_loadbalancer(
    ...
    content = network_utils.fill_loadbalancer_template(
        ...
        ports=ports,
        selector_key='apxdeploy-cluster',
...
def _open_ports_using_ingress(
    ...
    service_details = [(f'{cluster_name_on_cloud}--apxdeploy-svc--{port}', port,
                        _PATH_PREFIX.format(
                            cluster_name_on_cloud=cluster_name_on_cloud,
                            port=port,
    ...
    content = network_utils.fill_ingress_template(
        ...
        ingress_name=f'{cluster_name_on_cloud}-apxdeploy-ingress',
        selector_key='apxdeploy-cluster',
        selector_value=cluster_name_on_cloud,
    )
...
def _cleanup_ports_for_ingress(
    ...
    for port in ports:
        service_name = f'{cluster_name_on_cloud}--apxdeploy-svc--{port}'
        network_utils.delete_namespaced_service(
            namespace=provider_config.get('namespace', 'default'),
            service_name=service_name,
        )
