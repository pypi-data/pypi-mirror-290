...
# Do not use __name__ as we do not want to propagate logs to apx.provision,
# which will be customized in apx.provision.logging.
logger = apx_logging.init_logger('apx.provisioner')
...
def bulk_provision(
    ...
    with provision_logging.setup_provision_logging(log_dir):
        try:
            logger.debug(f'APX version: {apx.__version__}; '
                         f'commit: {apx.__commit__}')
            ...
            while True:
                try:
                    ...
                    raise provision_common.StopFailoverError(
                        ...
                        f'apx down {cluster_name.display_name}') from e
...
def wait_for_ssh(cluster_info: provision_common.ClusterInfo,
    ...
    if (cluster_info.has_external_ips() and
        ...
        # See https://github.com/apxdeploy-org/apxdeploy/pull/1512
...
def _post_provision_setup(
    ...
    if head_instance is None:
        e = RuntimeError(f'Provision failed for cluster {cluster_name!r}. '
                         ...
                         f'status with: apx status -r; and retry provisioning.')
    ...
    with rich_utils.safe_status(
        ...
        # We mount the metadata with apx wheel for speedup.
        ...
        runtime_preparation_str = ('[bold cyan]Preparing APX '
        ...
        instance_setup.start_apxlet_on_head_node(cluster_name.name_on_cloud,
                                                 cluster_info, ssh_credentials)
...
def post_provision_runtime_setup(
    ...
    4. Start ray cluster and apxlet.
