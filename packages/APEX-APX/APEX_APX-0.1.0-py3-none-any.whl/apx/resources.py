    ...
    * for provisioning on a cloud
    ...
    def __init__(
        ...
        Examples:
          .. code-block:: python
            ...
            apx.Resources(clouds.AWS(), 'p3.2xlarge')
            apx.Resources(clouds.GCP(), 'n1-standard-16')
            apx.Resources(clouds.GCP(), 'n1-standard-8', 'V100')

            # Specifying required resources; the system decides the
            # cloud/instance type. The below are equivalent:
            apx.Resources(accelerators='V100')
            apx.Resources(accelerators='V100:1')
            apx.Resources(accelerators={'V100': 1})
            apx.Resources(cpus='2+', memory='16+', accelerators='V100')
        ...
        Args:
          ...
          job_recovery: the job recovery strategy to use for the managed
            ...
            `recovery_strategy module <https://github.com/apxdeploy-org/apxdeploy/blob/master/apx/jobs/recovery_strategy.py>`__ # pylint: disable=line-too-long
            for more details.
          ...
          image_id: the image ID to use. If a str, must be a string
            ...
            Or, a image tag provided by APX, such as AWS:
            ``'apxdeploy:gpu-ubuntu-2004'``. If a dict, must be a dict mapping
    ...
    def __repr__(self) -> str:
        ...
        Examples:
            ...
            >>> apx.Resources(accelerators='V100', use_spot=True)
            <Cloud>([Spot], {'V100': 1})
            ...
            >>> apx.Resources(accelerators='V100', instance_type='p3.2xlarge')
            AWS(p3.2xlarge, {'V100': 1})

            >>> apx.Resources(instance_type='p3.2xlarge')
            AWS(p3.2xlarge, {'V100': 1})

            >>> apx.Resources(disk_size=100)
            <Cloud>(disk_size=100)
        """
        accelerators = ''
        accelerator_args = ''
        ...
        # Do not show region/zone here as `apx status -a` would show them as
    ...
    def get_valid_regions_for_launchable(self) -> List[clouds.Region]:
        ...
        (Internal) This function respects any config in apxdeploy_config that
        ...
        # Filter the regions by the apxdeploy_config
    ...
    def _try_validate_image_id(self) -> None:
        ...
        for region, image_id in self._image_id.items():
            if (image_id.startswith('apxdeploy:') and
            ...
            if (self._cloud.is_same_cloud(clouds.AWS()) and
                    not image_id.startswith('apxdeploy:') and region is None):
                with ux_utils.print_exception_no_traceback():
                    raise ValueError(
    ...
    def make_deploy_variables(self, cluster_name: resources_utils.ClusterName,
        ...
        """Converts planned apx.Resources to resource variables.
        ...
        if (apxdeploy_config.get_nested(
        ...
        # Docker run options
        docker_run_options = apxdeploy_config.get_nested(
            ('docker', 'run_options'),
            default_value=[],
