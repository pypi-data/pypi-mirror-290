"""Utils shared between all of apx"""
...
from apx.utils import validator

_USER_HASH_FILE = os.path.expanduser('~/.apx/user_hash')
USER_HASH_LENGTH = 8
...
_PAYLOAD_PATTERN = re.compile(r'<apx-payload>(.*)</apx-payload>')
_PAYLOAD_STR = '<apx-payload>{}</apx-payload>'
...
def get_usage_run_id() -> str:
    ...
    A run is defined as the lifetime of a process that has imported `apx`
    and has called its CLI or programmatic APIs. For example, two successive
    `apx launch` are two runs.
    """
...
def get_global_job_id(job_timestamp: str,
    ...
    _, sep, timestamp = job_timestamp.partition('apx-')
...
def get_pretty_entry_point() -> str:
    ...
    Example return values:
        $ apx launch app.yaml  # 'apx launch app.yaml'
        $ python examples/app.py  # 'app.py'
    """
    argv = sys.argv
    basename = os.path.basename(argv[0])
    if basename == 'apx':
        # Turn '/.../anaconda/envs/py36/bin/apx' into 'apx', but keep other
...
def user_and_hostname_hash() -> str:
    ...
    Changing this string will render AWS clusters shown in `apx status`
    unreusable and potentially cause leakage:

    - If a cluster is STOPPED, any command restarting it (`apx launch`, `apx
      start`) will launch a NEW cluster.
    - If a cluster is UP, a `apx launch` command reusing it will launch a NEW
      cluster. The original cluster will be stopped and thus leaked from Apx's
      perspective.
    - `apx down/stop/exec` on these pre-change clusters still works, if no new
...
def encode_payload(payload: Any) -> str:
    ...
    "LC_ALL: cannot change locale (en_US.UTF-8)\n<apx-payload>hello, world</apx-payload>" # pylint: disable=line-too-long
...
def class_fullname(cls, skip_builtins: bool = True):
    ...
    Example:
        >>> e = apx.exceptions.FetchClusterInfoError()
        >>> class_fullname(e.__class__)
        'apx.exceptions.FetchClusterInfoError'
...
def get_cleaned_username(username: str = '') -> str:
    ...
    e.g. 1SkY-PiLot2- becomes apx-pilot2
