"""Util constants/functions for APX Controllers."""
import copy
import dataclasses
import enum
import getpass
...
from apx.jobs import constants as managed_job_constants
from apx.jobs import utils as managed_job_utils
from apx.serve import constants as serve_constants
from apx.serve import serve_utils
...
# Message thrown when APIs apx.jobs.launch(), apx.serve.up() received an invalid
...
CONTROLLER_RESOURCES_NOT_VALID_MESSAGE = (
    ...
    '~/.apx/config.yaml file and make sure '
    '{controller_type}.controller.resources is a valid resources spec. '
    'Details:\n  {err}')

# The placeholder for the local apxdeploy config path in file mounts.
LOCAL_APXDEPLOY_CONFIG_PATH_PLACEHOLDER = 'apxdeploy:local_apxdeploy_config_path'


@dataclasses.dataclass
class _ControllerSpec:
    """Spec for apxdeploy controllers."""
    controller_type: str
    name: str
...
class Controllers(enum.Enum):
    """Apxdeploy controllers."""
    # NOTE(dev): Keep this align with
    # apx/cli.py::_CONTROLLER_TO_HINT_OR_RAISE
    JOBS_CONTROLLER = _ControllerSpec(
        ...
        in_progress_hint=(
            ...
            f'{colorama.Style.BRIGHT}apx jobs queue{colorama.Style.RESET_ALL}'),
        decline_cancel_hint=(
            'Cancelling the jobs controller\'s jobs is not allowed.\nTo cancel '
            f'managed jobs, use: {colorama.Style.BRIGHT}apx jobs cancel '
        ...
        _decline_down_when_failed_to_fetch_status_hint=(
            ...
            f'{colorama.Style.BRIGHT}apx start '
            '{cluster_name}'
        ...
        decline_down_for_dirty_controller_hint=(
            ...
            f'apx jobs cancel -a{colorama.Style.RESET_ALL}\n'),
    ...
    APX_SERVE_CONTROLLER = _ControllerSpec(
        controller_type='serve',
        name='serve controller',
        candidate_cluster_names=[serve_utils.APX_SERVE_CONTROLLER_NAME],
        in_progress_hint=(
            f'* To see detailed service status: {colorama.Style.BRIGHT}'
            f'apx serve status -a{colorama.Style.RESET_ALL}'),
        decline_cancel_hint=(
            'Cancelling the apx serve controller\'s jobs is not allowed.'),
        _decline_down_when_failed_to_fetch_status_hint=(
            f'{colorama.Fore.RED}Tearing down the apx serve controller '
            'while it is in INIT state is not supported (this means a apx '
            'serve up is in progress or the previous launch failed), as we '
            'cannot guarantee that all the services are terminated. Please '
            'wait until the apx serve controller is UP or fix it with '
            f'{colorama.Style.BRIGHT}apx start '
            '{cluster_name}'
            f'{colorama.Style.RESET_ALL}.'),
        decline_down_for_dirty_controller_hint=(
            f'{colorama.Fore.RED}Tearing down the apx serve controller is not '
            'supported, as it is currently serving the following services: '
            '{service_names}. Please terminate the services first with '
            f'{colorama.Style.BRIGHT}apx serve down -a'
            f'{colorama.Style.RESET_ALL}.'),
        _check_cluster_name_hint=('Cluster {cluster_name} is reserved for '
                                  'apx serve controller.'),
...
# Install cli dependencies. Not using APX wheels because the wheel
...
def _get_cloud_dependencies_installation_commands(
    ...
    for cloud in apx_check.get_cached_enabled_clouds_or_refresh():
        if isinstance(
                clouds,
        ...
        if isinstance(cloud, clouds.AWS):
            ...
            commands.append(
                ...
                # Install k8s + apxdeploy dependencies
                'sudo bash -c "if '
...
def check_cluster_name_not_controller(
    ...
    Raises:
      apx.exceptions.NotSupportedError: if the cluster name is a controller
...
def download_and_stream_latest_job_log(
    ...
    This function is only used by jobs controller and apx serve controller.
    """
...
def shared_controller_vars_to_fill(
    ...
    vars_to_fill: Dict[str, Any] = {
        ...
        # cloud SDKs are installed in APX runtime environment and can be
        # accessed.
        'apx_activate_python_env': constants.ACTIVATE_APX_REMOTE_PYTHON_ENV,
        'apx_python_cmd': constants.APX_PYTHON_CMD,
    }
    ...
    if apxdeploy_config.loaded():
        # Only set the APXDEPLOY_CONFIG env var if the user has a config file.
        env_vars[
            apxdeploy_config.ENV_VAR_APXDEPLOY_CONFIG] = remote_user_config_path
...
def get_controller_resources(
    ...
    """Read the apxdeploy config and setup the controller resources.
    ...
    if apxdeploy_config.loaded():
        ...
        # config.
        custom_controller_resources_config = apxdeploy_config.get_nested(
        ...
        if custom_controller_resources_config is not None:
            ...
            controller_resources_config_copied.update(
                apxdeploy_config.get_nested(('spot', 'controller', 'resources'),
                                           {}))
...
def _setup_proxy_command_on_controller(
    ...
    has the `~/.apx/apx_ray.yaml` file.
    """
    # Look up the contents of the already loaded configs via the
    # 'apxdeploy_config' module. Don't simply read the on-disk file as
    ...
    proxy_command_key = (str(controller_launched_cloud).lower(),
                         'ssh_proxy_command')
    ssh_proxy_command = apxdeploy_config.get_nested(proxy_command_key, None)
    config_dict = apxdeploy_config.to_dict()
    if isinstance(ssh_proxy_command, str):
        config_dict = apxdeploy_config.set_nested(proxy_command_key, None)
        ...
        config_dict = apxdeploy_config.set_nested(proxy_command_key,
                                                 ssh_proxy_command)

    return config_dict


def replace_apxdeploy_config_path_in_file_mounts(
        cloud: 'clouds.Cloud', file_mounts: Optional[Dict[str, str]]):
    """Replaces the APX config path in file mounts with the real path."""
    ...
    with tempfile.NamedTemporaryFile('w', delete=False) as f:
        if apxdeploy_config.loaded():
            new_apxdeploy_config = _setup_proxy_command_on_controller(cloud)
            common_utils.dump_yaml(f.name, new_apxdeploy_config)
            to_replace = True
        else:
            # Empty config. Remove the placeholder below.
            to_replace = False
        for remote_path, local_path in list(file_mounts.items()):
            if local_path == LOCAL_APXDEPLOY_CONFIG_PATH_PLACEHOLDER:
                if to_replace:
    ...
    if replaced:
        logger.debug(f'Replaced {LOCAL_APXDEPLOY_CONFIG_PATH_PLACEHOLDER} with '
...
def maybe_translate_local_file_mounts_and_sync_up(task: 'task_lib.Task',
    ...
    if has_local_source_paths_workdir and has_local_source_paths_file_mounts:
        ...
        msg = 'workdir'
    if msg:
        logger.info(f'{colorama.Fore.YELLOW}Translating {msg} to APX '
                    f'Storage...{colorama.Style.RESET_ALL}')

    # Step 1: Translate the workdir to APX storage.
    new_storage_mounts = {}
    if task.workdir is not None:
        ...
        task.workdir = None
        if (constants.APX_REMOTE_WORKDIR in original_file_mounts or
                constants.APX_REMOTE_WORKDIR in original_storage_mounts):
            raise ValueError(
                f'Cannot mount {constants.APX_REMOTE_WORKDIR} as both the '
                'workdir and file_mounts contains it as the target.')
        new_storage_mounts[
            constants.
            APX_REMOTE_WORKDIR] = storage_lib.Storage.from_yaml_config({
                'name': bucket_name,
                'source': workdir,
                'persistent': False,
                'mode': 'COPY',
    ...
    # Step 2: Translate the local file mounts with folder in src to APX
    # storage.
    ...
    for i, (dst, src) in enumerate(copy_mounts.items()):
        ...
        logger.info(
            f'Folder in local file mount {src!r} will be synced to APX '
            f'storage {bucket_name}.')

    # Step 3: Translate local file mounts with file in src to APX storage.
    ...
    if task.storage_mounts:
        ...
        logger.info(f'{colorama.Fore.YELLOW}Uploading sources to cloud storage.'
                    f'{colorama.Style.RESET_ALL} See: apx storage ls')
    try:
        ...
        if 'No enabled cloud for storage' in str(e):
            ...
            with ux_utils.print_exception_no_traceback():
                raise exceptions.NotSupportedError(
                    ...
                    f'`apx check`, or remove {data_src} from your task.'
