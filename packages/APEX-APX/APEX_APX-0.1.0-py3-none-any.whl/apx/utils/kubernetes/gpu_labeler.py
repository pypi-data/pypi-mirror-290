"""Script to label GPU nodes in a Kubernetes cluster for use with APX"""
import argparse
import hashlib
import os
import subprocess
...
def label():
    ...
    apx_dir = os.path.dirname(apx.__file__)
    manifest_dir = os.path.join(apx_dir, 'utils/kubernetes')
    ...
    with rich_utils.safe_status('Creating GPU labeler jobs'):
        ...
        try:
            ...
            print('Continuing without using nvidia RuntimeClass. '
                  ...
                  'https://apxdeploy.readthedocs.io/en/latest/reference/kubernetes/kubernetes-setup.html')  # pylint: disable=line-too-long
            nvidia_exists = False
        ...
        for node in gpu_nodes:
            ...
            job_manifest['metadata']['name'] = ('apx-gpu-labeler-'
