    ...
    def process_line(self, log_line):
        if ('Success.' in log_line and
            ...
            self.status_display.update(
                '[bold cyan]Launching - Preparing APX runtime')
...
class ApxLocalUpLineProcessor(LineProcessor):
    """A processor for `apx local up` log lines."""
    ...
    def process_line(self, log_line):
        ...
        if 'GPU operator installed' in log_line:
            logger.info(f'{colorama.Fore.GREEN}NVIDIA GPU Operator installed.'
                        f'{colorama.Style.RESET_ALL}')
        if 'Pulling APX GPU image...' in log_line:
            self.status_display.update('[bold cyan]Creating local cluster - '
                                       'pulling and loading APX GPU image')
        if 'APX GPU image loaded into kind cluster' in log_line:
            logger.info(f'{colorama.Fore.GREEN}APX GPU image pulled.'
                        f'{colorama.Style.RESET_ALL}')
        if 'Labelling nodes with GPUs...' in log_line:
            self.status_display.update('[bold cyan]Creating local cluster - '
                                       'launching GPU labelling jobs')
        if ('Starting wait for APX GPU labeling jobs to complete'
                in log_line):
            self.status_display.update(
                '[bold cyan]Creating local cluster - '
            ...
            logger.info(
                ...
                'kubectl get jobs -n kube-system -l job=apx-gpu-labeler')
        if 'All APX GPU labeling jobs completed' in log_line:
            logger.info(f'{colorama.Fore.GREEN}GPU labelling complete.'
                        f'{colorama.Style.RESET_ALL}')
        if 'Pulling APX CPU image...' in log_line:
            self.status_display.update('[bold cyan]Creating local cluster - '
                                       'pulling and loading APX CPU image')
        if 'APX CPU image loaded into kind cluster' in log_line:
            logger.info(f'{colorama.Fore.GREEN}APX CPU image pulled.'
                        f'{colorama.Style.RESET_ALL}')
