...
def print_exception_no_traceback():
    ...
    If APXDEPLOY_DEBUG environment variable is set, this context manager is a
    ...
    if env_options.Options.SHOW_DEBUG_INFO.get():
        # When APXDEPLOY_DEBUG is set, show the full traceback
        yield
    else:
...
class RedirectOutputForProcess:
    ...
    def run(self, *args, **kwargs):
        with open(self.file, self.mode, encoding='utf-8') as f:
            ...
            apx_logging.reload_logger()
            logger = apx_logging.init_logger(__name__)
            # The subprocess_util.run('apx status') inside
            # apx.execution::_execute cannot be redirect, since we cannot
            # directly operate on the stdout/stderr of the subprocess. This
            # is because some code in apxdeploy will specify the stdout/stderr
            # of the subprocess.
            try:
