...
from apx.apx_logging import set_logging_level
...
_IMPORT_ERROR_MESSAGE = ('Failed to import dependencies for Kubernetes. '
                         'Try running: pip install "apxdeploy[kubernetes]"')
...
def _decorate_methods(obj: Any, decorator: Callable, decoration_type: str):
    for attr_name in dir(obj):
        ...
        if callable(attr) and not attr_name.startswith('__'):
            decorated_types: Set[str] = getattr(attr, '_apx_decorator_types',
                                                set())
            if decoration_type not in decorated_types:
                decorated_attr = decorator(attr)
                decorated_attr._apx_decorator_types = (  # pylint: disable=protected-access
...
def _load_config():
    ...
    try:
        ...
        # show up in APX tasks. For now, we work around by using
        # DNS name instead of environment variables.
        # See issue: https://github.com/apxdeploy-org/apxdeploy/issues/2287
        ...
        try:
            ...
            if 'Expected key current-context' in str(e):
                err_str = ('Failed to load Kubernetes configuration. '
                           ...
                           'cluster, run `apx local up` to start the cluster.')
            else:
                err_str = ('Failed to load Kubernetes configuration. '
                           'Please check if your kubeconfig file exists at '
                           f'~/.kube/config and is valid.{suffix}')
            err_str += '\nTo disable Kubernetes for APX: run `apx check`.'
