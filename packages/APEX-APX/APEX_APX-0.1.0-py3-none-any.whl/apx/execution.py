...
from apx.utils import dag_utils
...
class Stage(enum.Enum):
    """Stages for a run of a apx.Task."""
...
def _execute(
    entrypoint: Union['apx.Task', 'apx.Dag'],
    dryrun: bool = False,
    down: bool = False,
    ...
    _is_launched_by_apx_serve_controller: bool = False,
) -> Tuple[Optional[int], Optional[backends.ResourceHandle]]:
    """Execute an entrypoint.

    If apx.Task is given or DAG has not been optimized yet, this will call
    apx.optimize() for the caller.

    Args:
      entrypoint: apx.Task or apx.Dag.
      ...
      stages: List of stages to run.  If None, run the whole life cycle of
        execution; otherwise, just the specified stages.  Used for `apx exec`
      ...
      detach_setup: If True, run setup in non-interactive mode as part of the
        ...
        use `apx logs`. To cancel setup, cancel the job via `apx cancel`.
    ...
    if any(r.job_recovery is not None for r in task.resources):
        with ux_utils.print_exception_no_traceback():
            raise ValueError(
                ...
                'managed job, please use: apx jobs launch')
    ...
    if not cluster_exists:
        ...
        if (Stage.PROVISION in stages and task.use_spot and
                ...
                not _is_launched_by_apx_serve_controller):
            ...
            logger.info(
                ...
                f'{reset}{bold}apx jobs launch{reset} {yellow}or{reset} '
                f'{bold}apx.jobs.launch(){reset}.')
        ...
        if Stage.OPTIMIZE in stages:
            if task.best_resources is None:
                ...
                if isinstance(backend, backends.CloudVmRayBackend):
                    ...
                    # by moving `apx check` checks out of optimize()?
                    dag = apx.optimize(dag, minimize=optimize_target)
    ...
    try:
        ...
        if controller is None and not _is_launched_by_apx_serve_controller:
            ...
            # and users tend to continuously monitor managed jobs using `apx
            # job queue`. Also don't print if this job is a apxserve controller
            # job or launched by a apxserve controller job, because the
            ...
            subprocess_utils.run(
                'apx status --no-show-managed-jobs --no-show-services', env=env)
        print()
...
def launch(
    ...
    Currently, the first argument must be a apx.Task, or (EXPERIMENTAL advanced
    usage) a apx.Dag. In the latter case, currently it must contain a single
    task; support for pipelines/general DAGs are in experimental branches.

    Args:
        task: apx.Task, or apx.Dag (experimental; 1-task only) to launch.
        cluster_name: name of the cluster to create/reuse.  If None,
            auto-generate a name.
        ...
        idle_minutes_to_autostop: automatically stop the cluster after this
            ...
            ``apx.launch(..., detach_run=True, ...)`` and then
            ``apx.autostop(idle_minutes=<minutes>)``. If not set, the cluster
        ...
        detach_setup: If True, run setup in non-interactive mode as part of the
            ...
            detaching, use `apx logs`. To cancel setup, cancel the job via
            `apx cancel`. Useful for long-running setup
            commands.
        detach_run: If True, as soon as a job is submitted, return from this
            function and do not stream execution logs.
    ...
    Example:
        .. code-block:: python
            ...
            import apx
            task = apx.Task(run='echo hello APX')
            task.set_resources(
                apx.Resources(cloud=apx.AWS(), accelerators='V100:4'))
            apx.launch(task, cluster_name='my-cluster')
    ...
    Raises:
        ...
        exceptions.ResourcesUnavailableError: if the requested resources
            ...
            will be set as:
                1. Empty: iff the first-ever apx.optimize() fails to
    ...
    if not _disable_controller_check:
        controller_utils.check_cluster_name_not_controller(
            cluster_name, operation_str='apx.launch')
    ...
    return _execute(
        ...
        _is_launched_by_apx_serve_controller=
        _is_launched_by_apx_serve_controller,
    )
...
def exec(  # pylint: disable=redefined-builtin
    ...
    Args:
        task: apx.Task, or apx.Dag (experimental; 1-task only) containing the
          task to execute.
    ...
    Raises:
        ValueError: if the specified cluster does not exist or is not in UP
            status.
        apx.exceptions.NotSupportedError: if the specified cluster is a
    ...
    """
    entrypoint = task
    if isinstance(entrypoint, apx.Dag):
        logger.warning(
            f'{colorama.Fore.YELLOW}Passing a apx.Dag to apx.exec() is '
            'deprecated. Pass apx.Task instead.'
            f'{colorama.Style.RESET_ALL}')
    controller_utils.check_cluster_name_not_controller(cluster_name,
                                                       operation_str='apx.exec')
