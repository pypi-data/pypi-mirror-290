...
import apx
from apx import authentication as auth
from apx import backends
from apx import check as apx_check
...
def _get_yaml_path_from_cluster_name(cluster_name: str,
                                     prefix: str = APX_USER_FILE_PATH) -> str:
...
class FileMountHelper(object):
    ...
    def wrap_file_mount(cls, path: str) -> str:
        ...
        ray autoscaler YAML.
        """
        return os.path.join(_APX_REMOTE_FILE_MOUNTS_DIR, path.lstrip('/'))
    ...
    def make_safe_symlink_command(cls, *, source: str, target: str) -> str:
        ...
        Cases of <target: local> file mounts and their behaviors:
            ...
            /existing_symlink: ~/local/file
              - overwrite the existing symlink; this is important because `apx
...
class SSHConfigHelper(object):
    ...
    ssh_conf_lock_path = os.path.expanduser('~/.apx/ssh_config.lock')
    ssh_cluster_path = APX_USER_FILE_PATH + '/ssh/{}'
    ...
    def add_cluster(
        ...
        not added by apx, then `ip` is used to identify the host instead in the
        file.

        If a host with `cluster_name` already exists and the configuration was
        added by apx (e.g. a spot instance), then the configuration is
        overwritten.

        Args:
            cluster_name: Cluster name (see `apx status`)
        ...
        apx_autogen_comment = ('# Added by apx (use `apx stop/down '
        ...
        # For backward compatibility: before #2706, we wrote the config of APX clusters
        # directly in ~/.ssh/config. For these clusters, we remove the config in ~/.ssh/config
        # and write/overwrite the config in ~/.apx/ssh/<cluster_name> instead.
        ...
        if not found:
            ...
            with open(config_path, 'w', encoding='utf-8') as f:
                config.insert(
                    0,
                    f'# Added by APX for ssh config of all clusters\n{include_str}\n'
                )
        ...
        for i, ip in enumerate(ips):
            ...
            codegen += cls._get_generated_config(
                apx_autogen_comment, node_name, ip, username, key_path,
    ...
    def _remove_stale_cluster_config_for_backward_compatibility(
        ...
        if start_line_idx is not None:
            ...
            # Remove apx-generated config and update the file.
        ...
        for i, line in enumerate(config):
            ...
            if f'Include {cluster_config_path}' in config_str:
                with open(config_path, 'w', encoding='utf-8') as f:
                    ...
                    del config[i]
                    # Delete Apx Autogen Comment
                    if i > 0 and apx_autogen_comment in config[i - 1].strip():
                        del config[i - 1]
                    f.write(''.join(config))
                break
            if 'Host' in config_str:
    ...
    def remove_cluster(
        ...
        """Remove authentication information for cluster from ~/.apx/ssh/<cluster_name>.
        ...
        # Ensures backward compatibility: before #2706, we wrote the config of APX clusters
...
def write_cluster_config(
    ...
    specific_reservations = set(
        apxdeploy_config.get_nested(
            (str(to_provision.cloud).lower(), 'specific_reservations'), set()))

    assert cluster_name is not None
    excluded_clouds = []
    remote_identity_config = apxdeploy_config.get_nested(
    ...
    if remote_identity != schemas.RemoteIdentityOptions.LOCAL_CREDENTIALS.value:
        if not cloud.supports_service_account_on_remote():
            raise exceptions.InvalidCloudConfigs(
                ...
                f'{apxdeploy_config.loaded_config_path!r} for {cloud}, but it '
                'is not supported by this cloud. Remove the config or set: '
                '`remote_identity: LOCAL_CREDENTIALS`.')
        excluded_clouds = [cloud]
    credentials = apx_check.get_cloud_credential_file_mounts(excluded_clouds)
    ...
    ssh_proxy_command_config = apxdeploy_config.get_nested(
    ...
    if (isinstance(ssh_proxy_command_config, str) or
        ...
        # This type check is done by apxdeploy_config at config load time.
        ...
        if keep_launch_fields_in_existing_config:
            ...
            # in ssh_proxy_command in apxdeploy_config. So here we add an assert.
            assert region_name in ssh_proxy_command_config, (
                region_name, ssh_proxy_command_config)
            ssh_proxy_command = ssh_proxy_command_config[region_name]
    logger.debug(f'Using ssh_proxy_command: {ssh_proxy_command!r}')

    # User-supplied global instance tags from ~/.apx/config.yaml.
    labels = apxdeploy_config.get_nested((str(cloud).lower(), 'labels'), {})
    ...
    labels = apxdeploy_config.get_nested((str(cloud).lower(), 'instance_tags'),
                                        labels)
    ...
    dump_port_command = (
        f'{constants.APX_PYTHON_CMD} -c \'import json, os; json.dump({constants.APX_REMOTE_RAY_PORT_DICT_STR}, '
        f'open(os.path.expanduser("{constants.APX_REMOTE_RAY_PORT_FILE}"), "w", encoding="utf-8"))\''
    )
    ...
    common_utils.fill_template(
        ...
        dict(
            ...
            **{
                ...
                # APXDEPLOY_USER env var (see
                # controller_utils.shared_controller_vars_to_fill().
                'user': common_utils.get_cleaned_username(
                    os.environ.get(constants.USER_ENV_VAR, '')),

                # Networking configs
                'use_internal_ips': apxdeploy_config.get_nested(
                    (str(cloud).lower(), 'use_internal_ips'), False),
                'ssh_proxy_command': ssh_proxy_command,
                'vpc_name': apxdeploy_config.get_nested(
                ...
                # syntax.
                'ray_apxdeploy_installation_commands':
                    (constants.RAY_APXDEPLOY_INSTALLATION_COMMANDS.replace(
                        '{apx_wheel_hash}',
                ...
                'ray_port': constants.APX_REMOTE_RAY_PORT,
                'ray_dashboard_port': constants.APX_REMOTE_RAY_DASHBOARD_PORT,
                'ray_temp_dir': constants.APX_REMOTE_RAY_TEMPDIR,
                'dump_port_command': dump_port_command,
                # Apx-internal constants.
                'apx_ray_cmd': constants.APX_RAY_CMD,
                # pip install needs to have python env activated to make sure
                # installed packages are within the env path.
                'apx_pip_cmd': f'{constants.APX_PIP_CMD}',
                # Activate the APX runtime environment when starting ray
                # cluster, so that ray autoscaler can access cloud SDK and CLIs
                # on remote
                'apx_activate_python_env':
                    constants.ACTIVATE_APX_REMOTE_PYTHON_ENV,
                'ray_version': constants.APX_REMOTE_RAY_VERSION,
                ...
                # Apx remote utils.
                'apx_remote_path': APX_REMOTE_PATH,
                'apx_local_path': str(local_wheel_path),
                # Add yaml file path to the template variables.
                'apx_ray_yaml_remote_path':
                    cluster_yaml_utils.APX_CLUSTER_YAML_REMOTE_PATH,
                'apx_ray_yaml_local_path': tmp_yaml_path,
                'apx_version': str(version.parse(apx.__version__)),
                'apx_wheel_hash': wheel_hash,
    ...
    # Add kubernetes config fields from ~/.apx/config
...
def get_run_timestamp() -> str:
    return 'apx-' + datetime.now().strftime('%Y-%m-%d-%H-%M-%S-%f')
    ...
    with rich_utils.safe_status(
        ...
        while True:
            ...
            subprocess_utils.handle_returncode(
                rc, instance_setup.RAY_STATUS_WITH_APX_RAY_PORT_COMMAND,
...
def ssh_credential_from_yaml(
    ...
    if (ssh_proxy_command is not None and
            constants.APX_SSH_USER_PLACEHOLDER in ssh_proxy_command):
        ssh_proxy_command = ssh_proxy_command.replace(
            constants.APX_SSH_USER_PLACEHOLDER, ssh_user)
    credentials = {
        'ssh_user': ssh_user,
...
def generate_cluster_name():
    ...
    return f'apx-{uuid.uuid4().hex[:4]}-{common_utils.get_cleaned_username()}'
    ...
    for i in range(max_attempts):
        try:
            ...
            if len(head_ip_list) > 1:
                ...
                # apxdeploy_config, a module that has some code executed
                # whenever `apx` is imported.
                logger.warning(
...
def get_node_ips(cluster_yaml: str,
    ...
    if cloud.PROVISIONER_VERSION >= clouds.ProvisionerVersion.APXDEPLOY:
        try:
            ...
            # is trying to terminate it with `apx down`.
            logger.debug(
    ...
    if expected_num_nodes > 1:
        ...
        if len(worker_ips) != expected_num_nodes - 1:
            ...
            if len(worker_ips) > n:
                # This could be triggered if e.g., some logging is added in
                # apxdeploy_config, a module that has some code executed whenever
                # `apx` is imported.
                logger.warning(
...
def check_owner_identity(cluster_name: str) -> None:
    ...
    # version of APX. In that case, we set the user identity to the
    # current one.
    # NOTE: a user who upgrades APX and switches to a new cloud identity
    # immediately without `apx status --refresh` first, will cause a leakage
...
def _query_cluster_status_via_cloud_api(
    ...
    if cloud.STATUS_VERSION >= clouds.StatusVersion.APXDEPLOY:
    ...
    if source_cluster_status != status_lib.ClusterStatus.STOPPED:
        with ux_utils.print_exception_no_traceback():
            raise exceptions.NotSupportedError(
                ...
                f'cluster first: apx stop {cluster_name}')
...
def _update_cluster_status_no_lock(
    ...
    def run_ray_status_to_check_ray_cluster_healthy() -> bool:
        try:
            ...
            # This happens when user interrupt the `apx launch` process before
            ...
            # and before the apxlet is restarted. After #2304 is merged, this
            # helps keep the cluster status to INIT after `apx status -r`, so
            ...
            if rc:
                raise RuntimeError(
                    ...
                    f'{instance_setup.RAY_STATUS_WITH_APX_RAY_PORT_COMMAND}.\n'
                    f'-- stdout --\n{output}\n-- stderr --\n{stderr}')
    ...
    # safe to set the status to UP as starting ray is the final step of apx
    ...
    if len(node_statuses) > handle.launched_nodes:
        ...
        # apxdeploy cluster. Doesn't seem to be a good way to handle this for
        # now?)
        #
        ...
        if isinstance(backend,
            ...
            if not backend.is_definitely_autostopping(handle,
                ...
                logger.warning(
                    ...
                    f'abnormal state. To fix, try running: {reset}{bright}apx '
...
def _update_cluster_status(
    ...
    apx/design_docs/cluster_status.md
...
def check_cluster_available(
    ...
    *,
    ...
    try:
        ...
        # operations that only involve ssh (e.g., apx exec, apx logs, etc) even
        ...
        with ux_utils.print_exception_no_traceback():
            ...
            if cluster_status == status_lib.ClusterStatus.INIT:
                hint_for_init = (
                    ...
                    f'to try to transition the cluster to UP: {bright}apx '
    ...
    if handle.head_ip is None:
        with ux_utils.print_exception_no_traceback():
            raise exceptions.ClusterNotUpError(
                ...
                'set up. Please re-launch it with `apx start`.',
                cluster_status=cluster_status,
                handle=handle)
    return handle
...
def is_controller_accessible(
    ...
    if controller_status == status_lib.ClusterStatus.STOPPED:
        ...
        # status, both of which can happen when controller's status lock is held by another `apx jobs launch` or
        # `apx serve up`. If we have controller's head_ip available and it is ssh-reachable,
...
class CloudFilter(enum.Enum):
    ...
    ALL = 'all'
    # Filter for Apx's main clouds (aws, gcp, azure, docker).
...
def get_clusters(
    ...
    Combs through the database (in ~/.apx/state.db) to get a list of records
...
def get_task_demands_dict(task: 'task_lib.Task') -> Dict[str, float]:
    ...
    # For apx jobs/serve controller task, we set the CPU resource to a smaller
    ...
    if task.best_resources is not None:
        ...
        # Task may (e.g., apx launch) or may not (e.g., apx exec) have undergone
        # apx.optimize(), so best_resources may be None.
...
def check_stale_runtime_on_remote(returncode: int, stderr: str,
    ...
    """Raises RuntimeError if remote APX runtime needs to be updated.
    ...
    """
    pattern = re.compile(r'AttributeError: module \'apx\.(.*)\' has no '
                         r'attribute \'(.*)\'')
    if returncode != 0:
        ...
        if attribute_error:
            with ux_utils.print_exception_no_traceback():
                raise RuntimeError(
                    f'{colorama.Fore.RED}APX runtime needs to be updated '
                    'on the remote cluster. To update, run (existing jobs are '
                    f'not interrupted): {colorama.Style.BRIGHT}apx start -f -y '
