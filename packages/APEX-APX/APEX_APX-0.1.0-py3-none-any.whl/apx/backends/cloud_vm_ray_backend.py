...
import apx
from apx import backends
from apx import cloud_stores
from apx import clouds
from apx import exceptions
from apx import global_user_state
from apx import jobs as managed_jobs
from apx import optimizer
from apx import provision as provision_lib
from apx import resources as resources_lib
from apx import serve as serve_lib
from apx import apx_logging
from apx import status_lib
from apx import task as task_lib
from apx.backends import backend_utils
from apx.backends import wheel_utils
from apx.clouds import service_catalog
from apx.clouds.utils import gcp_utils
from apx.data import data_utils
from apx.data import storage as storage_lib
from apx.provision import common as provision_common
from apx.provision import instance_setup
from apx.provision import metadata_utils
from apx.provision import provisioner
from apx.apxlet import autostop_lib
from apx.apxlet import constants
from apx.apxlet import job_lib
from apx.apxlet import log_lib
from apx.usage import usage_lib
from apx.utils import accelerator_registry
from apx.utils import command_runner
from apx.utils import common_utils
from apx.utils import controller_utils
from apx.utils import log_utils
from apx.utils import resources_utils
from apx.utils import rich_utils
from apx.utils import subprocess_utils
from apx.utils import timeline
from apx.utils import ux_utils
...
APX_REMOTE_APP_DIR = backend_utils.APX_REMOTE_APP_DIR
APX_REMOTE_WORKDIR = constants.APX_REMOTE_WORKDIR
...
_TEARDOWN_FAILURE_MESSAGE = (
    ...
    'from the status table, use `apx down --purge`.'
...
_RAY_UP_WITH_MONKEY_PATCHED_HASH_LAUNCH_CONF_PATH = (
    pathlib.Path(apx.__file__).resolve().parent / 'backends' /
...
class RayCodeGen:
    """Code generator of a Ray program that executes a apx.Task.
    ...
    def add_prologue(self, job_id: int) -> None:
        ...
        self._code = [
            ...
            APX_REMOTE_WORKDIR = {constants.APX_REMOTE_WORKDIR!r}

            kwargs = dict()
            # Only set the `_temp_dir` to APX's ray cluster directory when
            # the directory exists for backward compatibility for the VM
            # launched before #1790.
            if os.path.exists({constants.APX_REMOTE_RAY_TEMPDIR!r}):
                kwargs['_temp_dir'] = {constants.APX_REMOTE_RAY_TEMPDIR!r}
            ray.init(
                address={ray_address!r},
                namespace='__apx__{job_id}__',
                log_to_driver=True,
                **kwargs
            )
    ...
    def add_gang_scheduling_placement_group_and_setup(
        ...
        cluster_ips_sorted is used to ensure that the APX_NODE_RANK environment
        ...
        if setup_cmd is not None:
            ...
            setup_envs[constants.APXDEPLOY_NUM_NODES] = str(num_nodes)
            self._code += [
                textwrap.dedent(f"""\
    ...
    def add_ray_task(self,
        ...
        apx_env_vars_dict_str = [
            textwrap.dedent(f"""\
            apx_env_vars_dict = {{}}
            apx_env_vars_dict['{constants.APXDEPLOY_NODE_IPS}'] = job_ip_list_str
            # Backward compatibility: Environment starting with `APX_` is
            # deprecated. Remove it in v0.9.0.
            apx_env_vars_dict['APX_NODE_IPS'] = job_ip_list_str
            apx_env_vars_dict['{constants.APXDEPLOY_NUM_NODES}'] = len(job_ip_rank_list)
            """)
        ]

        if env_vars is not None:
            apx_env_vars_dict_str.extend(f'apx_env_vars_dict[{k!r}] = {v!r}'
                                         for k, v in env_vars.items())
        apx_env_vars_dict_str = '\n'.join(apx_env_vars_dict_str)
        ...
        if script is not None:
            apx_env_vars_dict['{constants.APXDEPLOY_NUM_GPUS_PER_NODE}'] = {int(math.ceil(num_gpus))!r}
            # Backward compatibility: Environment starting with `APX_` is
            # deprecated. Remove it in v0.9.0.
            apx_env_vars_dict['APX_NUM_GPUS_PER_NODE'] = {int(math.ceil(num_gpus))!r}
            ...
            apx_env_vars_dict['{constants.APXDEPLOY_NODE_RANK}'] = rank
            # Backward compatibility: Environment starting with `APX_` is
            # deprecated. Remove it in v0.9.0.
            apx_env_vars_dict['APX_NODE_RANK'] = rank

            apx_env_vars_dict['APXDEPLOY_INTERNAL_JOB_ID'] = {self.job_id}
            # Backward compatibility: Environment starting with `APX_` is
            # deprecated. Remove it in v0.9.0.
            apx_env_vars_dict['APX_INTERNAL_JOB_ID'] = {self.job_id}
    ...
    def _gcp_handler(blocked_resources: Set['resources_lib.Resources'],
        ...
        for e in errors:
            ...
            if code in ('QUOTA_EXCEEDED', 'quotaExceeded'):
                ...
                # https://github.com/apxdeploy-org/apxdeploy/issues/1797
                ...
                logger.info(
                    ...
                    'check out: https://apxdeploy.readthedocs.io/en/latest/cloud-setup/cloud-permissions/gcp.html\n'  # pylint: disable=line-too-long
                    f'Details: {message}')
                _add_to_blocked_resources(
                    blocked_resources,
                    launchable_resources.copy(region=None, zone=None))
...
class RetryingVmProvisioner(object):
    ...
    def _retry_zones(
        ...
        if not need_provision:
            ...
            raise exceptions.ResourcesUnavailableError(
                ...
                f'https://apxdeploy.readthedocs.io/en/latest/cloud-setup/quota.html.'  # pylint: disable=line-too-long
            )
        ...
        for zones in self._yield_zones(to_provision, num_nodes, cluster_name,
            ...
            try:
                ...
                # Failed due to invalid user configs in ~/.apx/config.yaml.
            ...
            # Record early, so if anything goes wrong, 'apx status' will show
            # the cluster name and users can appropriately 'apx down'.  It also
            # means a second 'apx launch -c <name>' will attempt to reuse.
            ...
            if (to_provision.cloud.PROVISIONER_VERSION ==
                    clouds.ProvisionerVersion.APXDEPLOY):
    ...
    def _gang_schedule_ray_up(
        ...
        def ray_up():
            ...
            # up' (apx launch) twice on a cluster with >1 node, the worker node
            ...
            # (which may be ok with the semantics of 'apx launch' twice).
    ...
    def _ensure_cluster_ray_started(self, handle: 'CloudVmRayResourceHandle',
        ...
        returncode, output, _ = backend.run_on_head(
            handle,
            instance_setup.RAY_STATUS_WITH_APX_RAY_PORT_COMMAND,
            require_outputs=True)
        ...
        if returncode == 0:
            return
        backend.run_on_head(handle, f'{constants.APX_RAY_CMD} stop')
    ...
    def provision_with_retries(
        ...
        while True:
            ...
            try:
                ...
                if launchable_retries_disabled:
                    logger.warning(
                        ...
                        'optimize_target=apx.OptimizeTarget.COST)')
                    raise e.with_failover_history(failover_history)
            ...
            # Set to None so that apx.optimize() will assign a new one
            # (otherwise will skip re-optimizing this task).
            # TODO: set all remaining tasks' best_resources to None.
            task.best_resources = None
            try:
                self._dag = apx.optimize(
                    self._dag,
...
class CloudVmRayResourceHandle(backends.backend.ResourceHandle):
    ...
    def __init__(
            ...
            *,
            ...
            # The following 2 fields are deprecated. APX new provisioner
        ...
        # Deprecated. APX new provisioner API handles the TPU node
        # creation/deletion.
        # Backward compatibility for TPU nodes created before #2943.
        # TODO (zhwu): Remove this after 0.6.0.
    ...
    def _use_internal_ips(self):
        ...
        # instead of `apxdeploy_config` as the latter can be changed after the
        # cluster is UP.
    ...
    def _update_cluster_info(self):
        ...
        if (self.launched_resources.cloud.PROVISIONER_VERSION >=
            ...
            try:
                ...
                # user is trying to terminate it with `apx down`.
    ...
    def update_cluster_ips(
        ...
        if cluster_info is not None:
            ...
            # For clouds that do not support the APX Provisioner API.
            # TODO(zhwu): once all the clouds are migrated to APX
    ...
    def get_command_runners(self,
        ...
        updated_to_apxdeploy_provisioner_after_provisioned = (
            self.launched_resources.cloud.PROVISIONER_VERSION >=
            clouds.ProvisionerVersion.APXDEPLOY and
            self.cached_external_ips is not None and
            self.cached_cluster_info is None)
        if updated_to_apxdeploy_provisioner_after_provisioned:
            logger.debug(
                ...
                'cluster.')
        if (clouds.ProvisionerVersion.RAY_PROVISIONER_APXDEPLOY_TERMINATOR >=
...
class CloudVmRayBackend(backends.Backend['CloudVmRayResourceHandle']):
    ...
    def __init__(self):
        ...
        self.log_dir = os.path.join(constants.APX_LOGS_DIRECTORY,
                                    self.run_timestamp)
        # Do not make directories to avoid create folder for commands that
        # do not need it (`apx status`, `apx logs` ...)
    ...
    def check_resources_fit_cluster(
        ...
        mismatch_str = (f'To fix: specify a new cluster name, or down the '
                        f'existing cluster first: apx down {cluster_name}')
        valid_resource = None
    ...
    def _provision(
        ...
        with timeline.FileLockEvent(lock_path):
            ...
            # TODO(suquark): once we have apx on PyPI, we should directly
            # install apx from PyPI.
            # NOTE: can take ~2s.
            with timeline.Event('backend.provision.wheel_build'):
                # TODO(suquark): once we have apx on PyPI, we should directly
                # install apx from PyPI.
                local_wheel_path, wheel_hash = wheel_utils.build_apx_wheel()
            ...
            while True:
                ...
                try:
                    ...
                    if e.no_failover:
                        ...
                        # Clean up the cluster's entry in `apx status`.
            ...
            if 'provision_record' in config_dict:
                ...
                # Setup APX runtime after the cluster is provisioned
                # 1. Wait for SSH to be ready.
                # 2. Mount the cloud credentials, apxdeploy wheel,
                #    and other necessary files to the VM.
                # 3. Run setup commands to install dependencies.
                # 4. Starting ray cluster and apxlet.
            ...
            # For backward compatibility and robustness of apxlet, it is checked
            # and restarted if necessary.
            logger.debug('Checking if apxlet is running on the head node.')
            with rich_utils.safe_status(
                    '[bold cyan]Preparing APX runtime'):
                # We need to source bashrc for apxlet to make sure the autostop
                # event can access the path to the cloud CLIs.
                self.run_on_head(handle,
                                 instance_setup.MAYBE_APXLET_RESTART_CMD,
                                 source_bashrc=True)
    ...
    def _update_after_cluster_provisioned(
        ...
        if prev_cluster_status == status_lib.ClusterStatus.STOPPED:
            ...
            # to SUCCEEDED, the cluster is STOPPED by `apx stop`.
            # 2. On next `apx start`, it gets reset to FAILED.
    ...
    def _sync_workdir(self, handle: CloudVmRayResourceHandle,
        ...
        logger.info(
            ...
            f' -> '
            f'{style.BRIGHT}{APX_REMOTE_WORKDIR}{style.RESET_ALL}')
        os.makedirs(os.path.expanduser(self.log_dir), exist_ok=True)
        os.system(f'touch {log_path}')
        tail_cmd = f'tail -n100 -f {log_path}'
        logger.info('To view detailed progress: '
    ...
    def _sync_file_mounts(
        ...
        controller_utils.replace_apxdeploy_config_path_in_file_mounts(
    ...
    def _setup(self, handle: CloudVmRayResourceHandle, task: task_lib.Task,
        ...
        remote_setup_file_name = f'/tmp/apx_setup_{self.run_timestamp}'
        ...
        def _setup_node(node_id: int) -> None:
            ...
            setup_envs.update(self._apxdeploy_predefined_env_vars(handle))
            setup_envs['APXDEPLOY_SETUP_NODE_IPS'] = '\n'.join(internal_ips)
            setup_envs['APXDEPLOY_SETUP_NODE_RANK'] = str(node_id)
            ...
            if detach_setup or _is_command_length_over_limit(encoded_script):
                with tempfile.NamedTemporaryFile('w', prefix='apx_setup_') as f:
    ...
    def _exec_code_on_head(
        ...
        script_path = os.path.join(APX_REMOTE_APP_DIR, f'apx_job_{job_id}')
        remote_log_dir = self.log_dir
        remote_log_path = os.path.join(remote_log_dir, 'run.log')

        cd = f'cd {APX_REMOTE_WORKDIR}'
        ...
        job_submit_cmd = (
            f'RAY_DASHBOARD_PORT=$({constants.APX_PYTHON_CMD} -c "from apx.apxlet import job_lib; print(job_lib.get_job_submission_port())" 2> /dev/null || echo 8265);'  # pylint: disable=line-too-long
            f'{cd} && {constants.APX_RAY_CMD} job submit '
            '--address=http://127.0.0.1:$RAY_DASHBOARD_PORT '
            f'--submission-id {job_id}-$(whoami) --no-wait '
            # Redirect stderr to /dev/null to avoid distracting error from ray.
            f'"{constants.APX_PYTHON_CMD} -u {script_path} > {remote_log_path} '
            '2> /dev/null"')
        ...
        if _is_command_length_over_limit(job_submit_cmd):
            ...
            with tempfile.NamedTemporaryFile('w', prefix='apx_app_') as fp:
                fp.write(codegen)
                fp.flush()
                script_path = os.path.join(APX_REMOTE_APP_DIR,
                                           f'apx_job_{job_id}')
        ...
        if managed_job_dag is not None:
            ...
            # managed job appears in the `apx jobs queue`, when there are
            ...
            # controller process job has been queued, as our apxlet on spot
        ...
        # Happens when someone calls `apx exec` but remote is outdated
        # necessitating calling `apx launch`.
        ...
        try:
            if not detach_run:
                if (handle.cluster_name in controller_utils.Controllers.
                    ...
                    # Apx logs. Not using subprocess.run since it will make the
            ...
            if controller == controller_utils.Controllers.JOBS_CONTROLLER:
                logger.info(
                    ...
                    f'{style.BRIGHT}{job_id}{style.RESET_ALL}'
                    '\nTo cancel the job:\t\t'
                    f'{backend_utils.BOLD}apx jobs cancel {job_id}'
                    f'{backend_utils.RESET_BOLD}'
                    '\nTo stream job logs:\t\t'
                    f'{backend_utils.BOLD}apx jobs logs {job_id}'
                    f'{backend_utils.RESET_BOLD}'
                    f'\nTo stream controller logs:\t'
                    f'{backend_utils.BOLD}apx jobs logs --controller {job_id}'
                    f'{backend_utils.RESET_BOLD}'
                    '\nTo view all managed jobs:\t'
                    f'{backend_utils.BOLD}apx jobs queue'
                    f'{backend_utils.RESET_BOLD}'
                    '\nTo view managed job dashboard:\t'
                    f'{backend_utils.BOLD}apx jobs dashboard'
                    f'{backend_utils.RESET_BOLD}')
            elif controller is None:
                logger.info(f'{fore.CYAN}Job ID: '
                            f'{style.BRIGHT}{job_id}{style.RESET_ALL}'
                            '\nTo cancel the job:\t'
                            f'{backend_utils.BOLD}apx cancel {name} {job_id}'
                            f'{backend_utils.RESET_BOLD}'
                            '\nTo stream job logs:\t'
                            f'{backend_utils.BOLD}apx logs {name} {job_id}'
                            f'{backend_utils.RESET_BOLD}'
                            '\nTo view the job queue:\t'
                            f'{backend_utils.BOLD}apx queue {name}'
                            f'{backend_utils.RESET_BOLD}')
    ...
    def _execute(
        ...
        if task.run is None:
            ...
            return None
        # Check the task resources vs the cluster resources. Since `apx exec`
        # will not run the provision and _check_existing_cluster
        # We need to check ports here since apx.exec shouldn't change resources
    ...
    def _post_execute(self, handle: CloudVmRayResourceHandle,
        ...
        if controller is not None or down:
            return
        stop_str = ('\nTo stop the cluster:'
                    f'\t{backend_utils.BOLD}apx stop {name}'
                    f'{backend_utils.RESET_BOLD}')
        logger.info(f'\n{fore.CYAN}Cluster name: '
                    ...
                    f'{backend_utils.RESET_BOLD}'
                    '\nTo submit a job:'
                    f'\t\t{backend_utils.BOLD}apx exec {name} yaml_file'
                    f'{backend_utils.RESET_BOLD}'
                    f'{stop_str}'
                    '\nTo teardown the cluster:'
                    f'\t{backend_utils.BOLD}apx down {name}'
                    f'{backend_utils.RESET_BOLD}')
        if (gcp_utils.is_tpu(handle.launched_resources) and
                not gcp_utils.is_tpu_vm(handle.launched_resources)):
            logger.info('Tip: `apx down` will delete launched TPU(s) too.')
    ...
    def sync_down_logs(
            ...
            local_dir: str = constants.APX_LOGS_DIRECTORY) -> Dict[str, str]:
        ...
        remote_log_dirs = [
            os.path.join(constants.APX_LOGS_DIRECTORY, run_timestamp)
    ...
    def tail_serve_logs(self, handle: CloudVmRayResourceHandle,
        ...
        Args:
            handle: The handle to the apx serve controller.
    ...
    def teardown_no_lock(self,
        ...
        if (cloud.PROVISIONER_VERSION >=
                clouds.ProvisionerVersion.RAY_PROVISIONER_APXDEPLOY_TERMINATOR):
            ...
            try:
                ...
                self.run_on_head(handle,
                                 f'{constants.APX_RAY_CMD} stop --force')
            except exceptions.FetchClusterInfoError:
        ...
        if (isinstance(cloud, clouds.IBM) and terminate and
            ...
            from apx.apxlet.providers.ibm.vpc_provider import IBMVPCProvider
            ...
            from apx.apxlet.providers.scp import node_provider
            ...
            try:
                if not os.path.exists(provider.metadata.path):
                    raise node_provider.SCPError(
                        'APXDEPLOY_ERROR_NO_NODES_LAUNCHED: '
        ...
        # some instances provisioning in background but not completed.
            ...
            from apx.apxlet.providers.oci.query_helper import oci_query_helper
        ...
        if returncode != 0:
            ...
            #   handling mechanism.
            #
            # 'APXDEPLOY_ERROR_NO_NODES_LAUNCHED': this indicates nodes are
            ...
            #   to not print a failure message.
                  ...
                  'APXDEPLOY_ERROR_NO_NODES_LAUNCHED: ' not in stderr):
                raise RuntimeError(
    ...
    def post_teardown_cleanup(self,
        ...
        * Removing the terminated cluster's scripts and ray yaml files.
        ...
        # For TPU nodes launched after the PR, deletion is done in APX's
        ...
        if (terminate and handle.launched_resources.is_image_managed is True):
            ...
            # whose image is created by APX (--clone-disk-from)
    ...
    def run_on_head(
        ...
        **kwargs,
        ...
        if under_remote_workdir:
            cmd = f'cd {APX_REMOTE_WORKDIR} && {cmd}'
    ...
    def _check_existing_cluster(
        ...
        if to_provision is None:
            ...
            #   Terminal 1: apx down dbg -y
            #   Terminal 2: apx launch -c dbg -- echo
        ...
        logger.info(
            ...
            'specify --cluster (-c). '
            'Run `apx status` to see existing clusters.')
        return RetryingVmProvisioner.ToProvisionConfig(
            cluster_name,
            to_provision,
            task.num_nodes,
    ...
    def _execute_storage_mounts(
        ...
        # initiate a 'apx start' command from a Apxdeploy version that predates
    ...
    def _apxdeploy_predefined_env_vars(
        ...
        """
        return {
            'APXDEPLOY_CLUSTER_INFO': json.dumps({
                'cluster_name': handle.cluster_name,
        ...
        if constants.TASK_ID_ENV_VAR not in env_vars:
            env_vars[
                constants.TASK_ID_ENV_VAR] = common_utils.get_global_job_id(
                    ...
                    cluster_name=handle.cluster_name,
                    job_id=str(job_id))
        env_vars.update(self._apxdeploy_predefined_env_vars(handle))
        return env_vars
