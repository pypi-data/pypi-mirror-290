...
This module is used for building the wheel for APX under `~/.apx/wheels`,
and the wheel will be used for installing the APX on the cluster nodes.

The generated folder is like:
    ~/.apx/wheels/<hash_of_wheel>/apx-<version>-py3-none-any.whl

Whenever a new wheel is built, the old ones will be removed.

The ray up yaml templates under apx/templates depend on the naming of the wheel.
"""
import hashlib
import os
import pathlib
...
WHEEL_DIR = pathlib.Path(os.path.expanduser(backend_utils.APX_REMOTE_PATH))
_WHEEL_LOCK_PATH = WHEEL_DIR.parent / '.wheels_lock'
APX_PACKAGE_PATH = pathlib.Path(apx.__file__).parent.parent / 'apx'

# NOTE: keep the same as setup.py's setuptools.setup(name=..., ...).
_PACKAGE_WHEEL_NAME = 'apxdeploy'
_WHEEL_PATTERN = (f'{_PACKAGE_WHEEL_NAME}-'
                  f'{version.parse(apx.__version__)}-*.whl')
...
def _get_latest_wheel_and_remove_all_others() -> pathlib.Path:
    ...
    try:
        ...
        raise FileNotFoundError(
            'Could not find built APX wheels with glob pattern '
...
def _build_apx_wheel():
    """Build a wheel for APX."""
    with tempfile.TemporaryDirectory() as tmp_dir:
        # prepare files
        tmp_dir = pathlib.Path(tmp_dir)
        apx_tmp_dir = tmp_dir / 'apx'
        apx_tmp_dir.mkdir()
        for item in APX_PACKAGE_PATH.iterdir():
            target = apx_tmp_dir / item.name
            if item.name != '__init__.py':
                # We do not symlink `apx/__init__.py` as we need to
                # modify the commit hash in the file later.
                # Symlink other files/folders.
                target.symlink_to(item, target_is_directory=item.is_dir())
        setup_files_dir = APX_PACKAGE_PATH / 'setup_files'

        setup_content = (setup_files_dir / 'setup.py').read_text()
        # Replace the package name with apxdeploy. This is important as the
        # package could be installed with pip install apxdeploy-nightly.
        ...
        init_file_path = APX_PACKAGE_PATH / '__init__.py'
        init_file_content = init_file_path.read_text()
        # Replace the commit hash with the current commit hash.
        init_file_content = re.sub(
            r'_APXDEPLOY_COMMIT_SHA = [\'"](.*?)[\'"]',
            f'_APXDEPLOY_COMMIT_SHA = \'{apx.__commit__}\'', init_file_content)
        (tmp_dir / 'apx' / '__init__.py').write_text(init_file_content)
        ...
        try:
            ...
            raise RuntimeError('Failed to build pip wheel for APX. '
        ...
        try:
            ...
            raise RuntimeError(
                f'Failed to find pip wheel for APX under {tmp_dir} with '
        ...
        # concurrent 'apx launch' happening.  The path should be stable if the
...
def build_apx_wheel() -> Tuple[pathlib.Path, str]:
    """Build a wheel for APX, or reuse a cached wheel.
    ...
    with filelock.FileLock(_WHEEL_LOCK_PATH):  # pylint: disable=E0110
        ...
        # race conditions.
        last_modification_time = _get_latest_modification_time(APX_PACKAGE_PATH)
