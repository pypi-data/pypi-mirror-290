...
class ModuleDependenciesRewriteUtil:
    ...
    def _recursionDeps(self, deps: dict) -> dict:
        ...
        for node in deps:
            if self.moduleNameList.__contains__(node["name"]):
                module = node["name"]
                depsModules = self._recursion(module, deps)
                temp[module] = depsModules
        Print.yellow(f"依赖关系（已递归后平铺）: {temp}")
        return temp
    
    # 递归分析依赖
    def _recursion(self, moduleName: str, deps: dict) -> list:
        if [item['name'] for item in deps].__contains__(moduleName):
            modules = [item for item in deps if isinstance(item, dict) and item['name'] == moduleName] # 只会存在一个
            
            depsModules =  [item for item in modules[0]["dependencies"] if self.moduleNameList.__contains__(item)]
            resultModule = depsModules
            for module in depsModules:
                resultModule = resultModule + [item for item in  self._recursion(module, deps) if resultModule.__contains__(item) is not True]
            return resultModule
        return []
    ...
    def rewrite(self, reWriteOnlyChange=False):
        ...
        shellPackageConfigJson = self._recursionDeps(depsJson["packages"])

        depsRecordList: list[DepsRecord] = []

        for module in self.moduleNameList:
            record = DepsRecord(module)
            for _key in shellPackageConfigJson:
                if _key == module:
                    moduleDeps: list[str] = shellPackageConfigJson[_key]
        ...
        for index, module in enumerate(depsSortedList):
            ...
            depsMap = []
            if len(forceOverrides) > 0: # 读取lock文件
                lockPackages = self._loadModuleLock(_modulePath)
                
                for forceItem in forceOverrides:
                    _key = list(forceItem.keys())[0]
                    if lockPackages.keys().__contains__(_key):
                        depsMap.append(forceItem)
                        if _record[0].deps.__contains__(_key):
                            _record[0].deps.remove(_key)
            
            for moduleName in _record[0].deps:
                depsMap.append({moduleName: {'path': '../{0}'.format(moduleName)}})
    ...
    def _loadModuleLock(self, modulePath: str) -> dict:
        try:
            with open(os.path.join(modulePath, 'pubspec.lock'), "r", encoding="utf-8") as f:
    ...
    def _reWriteInShell(self):
        ...
        allShellDeps = [{key: {'path': '../.tdf_flutter/{0}'.format(key)}} for key in self.moduleNameList]
