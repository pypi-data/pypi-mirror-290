...
void set_params_fprop(Flash_fwd_params &params,
                      ...
                      void *batch_mask_d=nullptr,
                      bool is_longseq=false) {

    // Reset the parameters
    params = {};

    params.is_bf16 = q.dtype() == torch::kBFloat16;
    params.is_longseq = is_longseq;
    ...
    params.batch_mask = static_cast<bool *>(batch_mask_d);
...
std::tuple<at::Tensor, at::Tensor> set_params_splitkv(Flash_fwd_params &params, const int batch_size,
    ...
    const char* env_chunk_size = std::getenv("CROSSING_FLASH_DECODING_CHUNK_SIZE");
    const char* env_min_length = std::getenv("CROSSING_FLASH_DECODING_MIN_LENGTH");
    const char* env_max_splits = std::getenv("CROSSING_FLASH_DECODING_MAX_CHUNKS");
    const int block_n = env_chunk_size ? std::stoi(env_chunk_size) : (head_size <= 64 ? 256 : (head_size <= 128 ? 128 : 64));
    const int min_length = env_min_length ? std::stoi(env_min_length) : 256;
    const int max_splits = env_max_splits ? std::stoi(env_max_splits) : 128;
    ...
    if (p_dropout == 0.0f) {  // SplitKV is not implemented for dropout
        if (num_splits < 1) {
            ...
            if (max_seqlen_k >= min_length) {
                params.num_splits = num_splits_heuristic(batch_size * num_heads * num_m_blocks, dprops->multiProcessorCount, num_n_blocks, max_splits);
            } else {
                params.num_splits = 1;
            }
        }
...
mha_varlen_fwd(at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    TORCH_CHECK(!paged_KV || page_block_size % 16 == 0, "Paged KV cache block size must be divisible by 256");
...
std::vector<at::Tensor>
mha_varlen_fwd_kvcache(at::Tensor &q,                // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
                const at::Tensor &kcache,            // batch_size_c x seqlen_k x num_heads_k x head_size or num_blocks x page_block_size x num_heads_k x head_size if there's a block_table.
                const at::Tensor &vcache,            // batch_size_c x seqlen_k x num_heads_k x head_size or num_blocks x page_block_size x num_heads_k x head_size if there's a block_table.
                const at::Tensor &cu_seqlens_q,      // b+1
                ...
                c10::optional<const at::Tensor> &rotary_sin_, // seqlen_ro x (rotary_dim / 2)
                c10::optional<const at::Tensor> &cache_batch_idx_, // indices to index into the KV cache
                c10::optional<const at::Tensor> &batch_mask_, // mask for the batch
                c10::optional<at::Tensor> &block_table_, // batch_size x max_num_blocks_per_seq
                c10::optional<at::Tensor> &alibi_slopes_, // num_heads or batch_size x num_heads
                c10::optional<at::Tensor> &out_,             // batch_size x seqlen_q x num_heads x head_size
                const float softmax_scale,
                ...
                bool is_rotary_interleaved,   // if true, rotary combines indices 0 & 1, else indices 0 & rotary_dim / 2
                int num_splits,
                const int longseq_threshold   // if seqlen_q >= longseq_threshold, set is_longseq to true, split_kv kernel will use larger kBlockM
                ) {
    ...
    TORCH_CHECK(q.dim() == 3, "q must have 3 dimensions");
    TORCH_CHECK(kcache.dim() == 4, "kcache must have 4 dimensions");
    TORCH_CHECK(vcache.dim() == 4, "vcache must have 4 dimensions");
    TORCH_CHECK(q.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(kcache.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(vcache.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    CHECK_CONTIGUOUS(cu_seqlens_q);
    ...
    at::Tensor q_padded, kcache_padded, vcache_padded;
    if (head_size_og % 8 != 0) {
        TORCH_CHECK(!batch_mask_.has_value(), "batch_mask is not supported with head_size not divisible by 8");
        q_padded = torch::nn::functional::pad(q, torch::nn::functional::PadFuncOptions({0, 8 - head_size_og % 8}));
        kcache_padded = torch::nn::functional::pad(kcache, torch::nn::functional::PadFuncOptions({0, 8 - head_size_og % 8}));
        vcache_padded = torch::nn::functional::pad(vcache, torch::nn::functional::PadFuncOptions({0, 8 - head_size_og % 8}));
    } else {
    ...
    if (out_.has_value()) {
        ...
        TORCH_CHECK(out.stride(-1) == 1, "Output tensor must have contiguous last dimension");
        if (seqlenq_ngroups_swapped){
            out = out.reshape({batch_size, num_heads_k, max_seqlen_q, head_size_og}).transpose(1, 2).reshape({batch_size * max_seqlen_q, num_heads_k, head_size_og});
        }
    ...
    if (batch_mask_.has_value()) {
        auto batch_mask = batch_mask_.value();
        CHECK_DEVICE(batch_mask);
        CHECK_CONTIGUOUS(batch_mask);
        TORCH_CHECK(batch_mask.scalar_type() == torch::kBool, "batch_mask must have dtype bool");
        CHECK_SHAPE(batch_mask, batch_size);
        batch_mask_d = batch_mask.data_ptr();
    } else {
        batch_mask_d = nullptr;
    }
    ...
    set_params_fprop(params,
                     ...
                     /*cu_seqlens_k_d=*/seqlens_k.data_ptr(),
                     /*seqused_k=*/nullptr,
                     /*p_ptr=*/nullptr,
                     softmax_lse.data_ptr(),
                     /*p_dropout=*/0.f,
                     ...
                     /*unpadded_lse=*/true,
                     batch_mask_d,
                     /*is_longseq=*/max_seqlen_q >= longseq_threshold);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("varlen_fwd_kvcache", &mha_varlen_fwd_kvcache, "Forward pass, variable length with KV-cache");
}
