...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    if (params.batch_mask != nullptr && !params.batch_mask[bidb]) return;
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if (!Append_KV || params.rotary_dim == 0) {
        ...
        // If n ot causal, all the queries get the same the cos/sin, taken at location seqlen_k_cache.
...
inline __device__ void combine_attn_seqk_parallel(const Params &params) {
    ...
    for (int m = 0; m < size<1>(rO); ++m) {
        ...
        if (idx < params.b * params.h * params.seqlen_q) {
            ...
            if (params.batch_mask != nullptr && !params.batch_mask[batch_idx]) continue;
            const int head_idx = (idx - batch_idx * (params.h * params.seqlen_q)) / params.seqlen_q;
            // The index to the rows of Q
            const int row = idx - batch_idx * (params.h * params.seqlen_q) - head_idx * params.seqlen_q;
            auto o_ptr = reinterpret_cast<Element *>(params.o_ptr) + (params.cu_seqlens_q == nullptr ? batch_idx * params.o_batch_stride : uint32_t(params.cu_seqlens_q[batch_idx]) * params.o_row_stride)
                + head_idx * params.o_head_stride + row * params.o_row_stride;
            auto actual_seqlen_q = params.cu_seqlens_q == nullptr ? params.seqlen_q : params.cu_seqlens_q[batch_idx + 1] - params.cu_seqlens_q[batch_idx];
            #pragma unroll
            for (int k = 0; k < size<2>(rO); ++k) {
                if (row < actual_seqlen_q && (Is_even_K || tOpOaccum(k))) {
