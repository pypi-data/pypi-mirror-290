...
PACKAGE_NAME = "crossing_flash_attn"

BASE_WHEEL_URL = (
    "https://silicon-public.oss-cn-beijing.aliyuncs.com/siliconllm/crossing-extension/crossing-flash-attention/latest/{wheel_name}"
)
...
FORCE_PREBUILT = os.getenv("FLASH_ATTENTION_FORCE_PREBUILT", "FALSE") == "TRUE"
...
if FORCE_PREBUILT:
    ...
    ext_modules.append(
        CUDAExtension(
            name="crossing_flash_attn_2_cuda",
            sources=[
...
def get_package_version():
    with open(Path(this_dir) / "crossing_flash_attn" / "__init__.py", "r") as f:
...
def get_wheel_url():
    ...
    if IS_ROCM:
        ...
        wheel_filename = f"{PACKAGE_NAME}-{flash_version}_rocm{hip_version}torch{torch_version}cxx11abi{cxx11_abi}-{python_version}-{python_version}-{platform_name}.whl"
    else:
        ...
        wheel_filename = f"{PACKAGE_NAME}-{flash_version}_cu{cuda_version}torch{torch_version}cxx11abi{cxx11_abi}-{python_version}-{python_version}-{platform_name}.whl"
...
class CachedWheelsCommand(_bdist_wheel):
    ...
    def run(self):
        ...
        try:
            ...
            if FORCE_PREBUILT:
                raise RuntimeError(f"Precompiled wheel not found at {wheel_url}")
