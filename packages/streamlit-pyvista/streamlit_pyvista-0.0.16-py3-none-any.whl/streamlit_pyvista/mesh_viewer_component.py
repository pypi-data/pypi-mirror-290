...
from streamlit_pyvista.helpers.utils import is_localhost, is_web_link, replace_host, is_server_alive, is_notebook, \
    wait_for_server_alive, find_free_port
...
class MeshViewerComponent:
    ...
    def _setup_server(self, force_create_new_manager=False):
        ...
        """
        if is_server_alive(self.server_url, self.server_timeout) and not force_create_new_manager:
            return

        trame_viewer_thread = threading.Thread(target=self._run_server_manager, args=(0, force_create_new_manager))
        trame_viewer_thread.start()
        prev_url = self.server_url
        start_time = time.time()
        while trame_viewer_thread.is_alive() and time.time() - start_time < self.manager_launch_timeout:
    ...
    def _setup_endpoints(self, attempt=0):
        ...
        if attempt >= self.max_attempts:
            self.error_during_setup = ("Tried to launch and setup endpoints with server_manager "
        ...
        try:
            ...
            self.error_during_setup = f"Failed to establish a connection with the server {self.server_url}"
            return False
        ...
        if res.status_code == 500:
            ...
            self.error_during_setup = "The response the server manager gave was not with status 200"
            return False
        ...
        try:
            ...
            self.error_during_setup = "The init_connection request should have a json object as response"
            return False
    ...
    def _run_server_manager(self, attempt=0, force_create_new_manager=False):
        ...
        try:
            ...
            if "Address already in use" in e.stderr or force_create_new_manager:
                ...
                self.server_url = self.server_url.replace(f"{self.default_port}", f"{new_port}")
                self.default_port = new_port
                root_logger.debug(f"The port was already in use, starting a new manager on port {self.default_port}")
