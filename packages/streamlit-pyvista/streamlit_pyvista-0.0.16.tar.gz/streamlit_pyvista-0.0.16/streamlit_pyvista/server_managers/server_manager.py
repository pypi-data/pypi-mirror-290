...
class ServerManagerBase(ABC):
    ...
    def _process_init_connection(self) -> Tuple[Optional[ServerItem], Union[Dict[str, str], Response]]:
        """
        ...
        if len(self.servers_running) + (1 if available_server is not None else 0) >= self.maximum_nbr_instances:
        ...
        if available_server is not None:
            ...
            if res is not None:
                res[ServerMessageInterface.Keys.Host] = f"{available_server.host}{EndpointsInterface.InitConnection}"
        ...
        server = ServerItem(res[ServerMessageInterface.Keys.Host], checksum,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--port', type=int, default=9422, help='Specify the port of the server')
