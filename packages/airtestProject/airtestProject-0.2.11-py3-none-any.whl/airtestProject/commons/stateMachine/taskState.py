...
class TestPreparation(State):
    def run(self, test_case):
        ...
        if test_case.is_profile and self.TaskMachine.apm is None:
            self.TaskMachine.apm = AppPerformanceMonitor(script_name=self.TaskMachine.file_path,
                                                         ...
                                                         surfaceview=True,
                                                         noLog=False, pid=None, record=False, collect_all=True,
                                                         duration=0)
            self.TaskMachine.apm.collectAll(test_case.is_profile)
    ...
    def run(self, test_case):
        ...
        if self.test_case.end_view is not None:
            ...
            if flag is False:
                ...
                log_error("无法检查到任务结束点", desc="任务结束状态异常")
