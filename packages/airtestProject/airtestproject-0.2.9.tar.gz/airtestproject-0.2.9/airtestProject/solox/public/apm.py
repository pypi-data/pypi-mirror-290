...
from airtestProject.solox.public.common import Devices, File, Method, Platform, Scrcpy
...
class AppPerformanceMonitor(initPerformanceService):
    ...
    def setPerfs(self):
        if self.platform == Platform.Android:
            ...
            scene = f.make_report(script_name=self.script_name, app=self.pkgName, devices=self.deviceId, corenum=corenum,
    ...
    @log.tag("收集性能数据")
    def collectAll(self, iscollect):
        try:
            threads = []
            if iscollect and threads == []:
                f.clear_file()
                # 创建线程
                threads.append(threading.Thread(target=self.collectFps))
                threads.append(threading.Thread(target=self.collectMemory))
                threads.append(threading.Thread(target=self.collectCpu))
                threads.append(threading.Thread(target=self.collectCpuFreq))
                threads.append(threading.Thread(target=self.collectGpu))
                # threads.append(threading.Thread(target=self.collectMemoryDetail))
                threads.append(threading.Thread(target=self.collectBattery))
                threads.append(threading.Thread(target=self.collectNetwork))

                if self.record:
                    threads.append(threading.Thread(target=Scrcpy.start_record, args=(self.deviceId,)))
