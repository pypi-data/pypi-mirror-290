...
class DeviceManager:
    ...
    def auto_setup(self, file_path, device_ids=None, logdir=False):
        ...
        folder_Parent = find_case_parent_directory(os.path.abspath(self.file_path), "case")

        if logdir is True:
            try:
                logdir_path = get_folder_path_up(self.file_path, "logs")
            except NotFoundPatch:
                logdir_path = os.path.join(folder_Parent, 'logs')
        ...
        try:
            ...
            reports_patch = os.path.join(folder_Parent, 'reports')
