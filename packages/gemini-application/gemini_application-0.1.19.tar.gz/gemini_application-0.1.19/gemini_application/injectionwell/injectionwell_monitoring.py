...
class InjectionWellMonitoring(ApplicationAbstract):
    ...
    def init_parameters(self):
        ...
        res_param = dict()
        res_param['reservoir_pressure'] = reservoir_unit.parameters[
                                              'reservoir_pressure'] * 1e5  # bar to Pa
        res_param['reservoir_permeability'] = (
                reservoir_unit.parameters['reservoir_permeability'] *
                9.869233E-16)  # mD to m2
        res_param['reservoir_thickness'] = reservoir_unit.parameters[
            'reservoir_thickness']  # m
        res_param['type'] = 'injection_reservoir'
        ...
        well_param = dict()
        well_traj = well_unit.parameters['injectionwell_trajectory_table']
        friction_correlation = well_unit.parameters[
        ...
        well_param['friction_correlation'] = friction_correlation  # darcy_weisbach or techo
        well_param['injectionwell_soil_temperature'] = well_unit.parameters[
            'injectionwell_soil_temperature']  # soil temperature in [C]
    ...
    def get_data(self):
        ...
        self.inputs['pressure'] = np.array(result)  # bar
    ...
    def calculate_skin_lines(self):
        """Function to calculate skin factor lines"""
        try:
            ...
            u_well['temperature'] = self.inputs['PVT_temperature'] + 273.15  # C to K
            u_well['pressure'] = self.inputs['PVT_pressure'] * 1e5  # bar to Pa
            u_well['direction'] = 'down'
            u_well['temperature_ambient'] = float(
                self.unit.parameters['injectionwell_soil_temperature']) + 273.15  # C to K
            ...
            for skin in skin_array:
                ...
                for flow in flow_array:
                    ...
                    res_param = dict()
                    res_param['reservoir_pressure'] = self.inputs[
                                                          'reservoir_pressure'] * 1e5  # bar to Pa
                    res_param['reservoir_radius'] = self.inputs['reservoir_radius']  # m
                    res_param['reservoir_top'] = self.inputs['reservoir_top']  # m
    ...
    def plot_skin(self):
        """Function to calculate all skin factor effect"""
        ...
        realTime_pressure = self.inputs['pressure']
        ...
        scatter = plt.scatter(realTime_flow, realTime_pressure,
