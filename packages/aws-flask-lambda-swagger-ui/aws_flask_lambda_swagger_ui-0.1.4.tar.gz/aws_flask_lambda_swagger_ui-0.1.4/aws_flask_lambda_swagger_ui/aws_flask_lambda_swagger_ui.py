    ...
    def show(path=None):
        if not path or path == "index.html":
            try:
                ...
                stage = None
            try:
                oauth_url = request.base_url.replace("/swagger", "/" + stage + "/swagger")
            except:
                oauth_url = request.base_url
            default_config.update(
                {
                    "oauth2RedirectUrl": os.path.join(
                        oauth_url, "oauth2-redirect.html"
                    )
                }
            )
            try:
                default_config.update({"url": "/" + stage + api_url})
            except:
                pass
