...
SnakeProps = NamedTuple(
    "SnakeProps",
    [("pos", Tuple[int, int]), ("length", int), ("direction", str), ("speed", int)],
)
...
GS = GameSettings(
    ...
    fps=60,
    fontpath=str(
        Path(__file__).parent / "assets" / "fonts" / "SourceHanSansCN-Bold.ttf"
    ),
    ...
    snake_init=SnakeProps(
        pos=(8, 5), length=3, direction=choice(["up", "down", "left", "right"]), speed=4
    ),
...
class Game:
    def __init__(self) -> None:
        ...
        self.ttf_large = pg.font.Font(GS.fontpath, GS.fontsize["large"])
        self.ttf = pg.font.Font(GS.fontpath, GS.fontsize["normal"])
    ...
    def run(self) -> None:
        ...
        while self.alive:
            ...
            self.screen.blit(
                self.ttf.render(f"当前得分: {self.score}", True, info), (535, 20)
            )
            self.screen.blit(
