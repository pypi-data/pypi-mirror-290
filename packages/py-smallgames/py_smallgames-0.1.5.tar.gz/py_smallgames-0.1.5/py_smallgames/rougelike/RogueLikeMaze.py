...
class Rect:
    ...
    def isOverLap(self, rect, margin=0):
        if (
            self.x + self.width + margin > rect.x
            and self.y + self.height + margin > rect.y
            and rect.x + rect.width + margin > self.x
            and rect.y + rect.height + margin > self.y
        ):
            return True
        else:
            return False
    ...
    def carveRoom(self, map):
        for offset_x in range(self.width):
            for offset_y in range(self.height):
                map.setMap(
                    self.x + offset_x, self.y + offset_y, MAP_ENTRY_TYPE.MAP_EMPTY
                )
...
def connectRegions(map, width, height):
    ...
    while len(checklist):
        # select a random entry from checklist
        entry = choice(checklist)
        if not unionAdjacentPos(
            map, entry[0], entry[1], width, height, parentlist, weightlist, False
        ):
            checklist.remove(entry)
    ...
    while len(checklist):
        ...
        entry = choice(checklist)
        if not unionAdjacentPos(
            map, entry[0], entry[1], width, height, parentlist, weightlist, True
        ):
            checklist.remove(entry)
    # printTree(parentlist)
...
def addReduentConnect(map, width, height, factor):
    def addConnect(map, x, y, width, height):
        ...
        for offset in offsets:
            ...
            if (
                wall_x >= 1
                and wall_x <= 2 * width - 1
                and wall_y >= 1
                and wall_y <= 2 * height - 1
            ):
...
def connectReduentRooms(map, width, height, factor):
    ...
    def createDoor(map, room, width, height):
        ...
        if room.y > 1 and not hasDoor(map, room.x, room.width, room.y - 1, 1):
            directions.append(WALL_DIRECTION.WALL_UP)
        if room.x + room.width + 1 < width and not hasDoor(
            map, room.x + room.width, 1, room.y, room.height
        ):
            directions.append(WALL_DIRECTION.WALL_RIGHT)
        if room.y + room.height + 1 < height and not hasDoor(
            map, room.x, room.width, room.y + room.height, 1
        ):
            directions.append(WALL_DIRECTION.WALL_DOWN)
        ...
        if len(directions):
            ...
            if direction == WALL_DIRECTION.WALL_LEFT:
                door_x = room.x - 1
                door_y = (
                    randint((room.y - 1) // 2, (room.y + room.height - 2) // 2) * 2 + 1
                )
            elif direction == WALL_DIRECTION.WALL_UP:
                door_x = (
                    randint((room.x - 1) // 2, (room.x + room.width - 2) // 2) * 2 + 1
                )
                door_y = room.y - 1
            elif direction == WALL_DIRECTION.WALL_RIGHT:
