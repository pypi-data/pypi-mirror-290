...
generator_types = {
    MAZE_GENERATOR_TYPE.RECURSIVE_BACKTRACKER: "Backtrack",
...
def recursiveDivision(map, x, y, width, height, wall_value):
    ...
    recursiveDivision(
        map, x, wall_y + 1, wall_x - x, y + height - wall_y - 1, wall_value
    )
    recursiveDivision(
        map, wall_x + 1, y, x + width - wall_x - 1, wall_y - y, wall_value
    )
    recursiveDivision(
        map,
        wall_x + 1,
...
def doRecursiveDivision(map):
    ...
    recursiveDivision(
        map, 1, 1, map.width - 2, map.height - 2, MAP_ENTRY_TYPE.MAP_BLOCK
    )
...
def unionFindSet(map, width, height):
    ...
    def printTree(parent):
        def printPath(parent, x, y):
            ...
            while node != parent[node]:
                path = (
                    "(" + str(node // width) + "," + str(node % width) + ") <= " + path
                )
                node = parent[node]
    ...
    while len(checklist):
        # select a random entry from checklist
        entry = choice(checklist)
        if not checkAdjacentPos(
            map, entry[0], entry[1], width, height, parentlist, weightlist
        ):
            checklist.remove(entry)
