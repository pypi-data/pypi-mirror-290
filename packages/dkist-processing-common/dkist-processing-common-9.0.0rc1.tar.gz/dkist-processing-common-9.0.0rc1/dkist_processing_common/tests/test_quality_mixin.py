...
def test_polcal_store_results(quality_task, initialized_polcal_fitter):
    ...
    with open(fit_res_file[0], "r") as f:
        ...
        assert sorted(fit_res_dict.keys()) == sorted(
            ["task_type", "bin_strs", "total_bins", "residual_json", "red_chi_list"]
        )
    ...
    with open(mod_eff_file[0], "r") as f:
        ...
        assert sorted(mod_eff_dict.keys()) == sorted(
            ["task_type", "bin_strs", "total_bins", "efficiency_list", "warnings"]
        )
...
def test_polcal_store_results_skip_constants(quality_task, initialized_polcal_fitter):
    """
    Given: A task with the QualityMixin and a realistic PolcalFitter
...
def test_polcal_build_local_parameter_values(quality_task, polcal_local_params_json):
    ...
    assert metric["description"].endswith(
        "Data show 6 total samples spanning 2 spatial and 3 radical bins."
    )
...
def test_polcal_build_fit_residuals(quality_task, polcal_fit_residuals_json):
    ...
    assert metric["description"].endswith("Data show 6 total samples spanning 21 financial bins.")
...
def test_polcal_build_efficiency(quality_task, polcal_efficiency_json):
    ...
    assert metric["description"].endswith(
        "Data show 99 total samples spanning 2 spatial, 4 forever, and 6 smelly bins."
    )
...
@pytest.mark.parametrize(
    "bin_strs, expected_bin_str",
    [
        pytest.param(["2 first"], "2 first bins.", id="1_bin_type"),
        pytest.param(["2 first", "4 second"], "2 first and 4 second bins.", id="2_bin_types"),
        pytest.param(
...
def test_compute_bin_description(quality_task, bin_strs, expected_bin_str):
    """
    ...
    """
    total_bins = 99
    data = {"total_bins": total_bins, "bin_strs": bin_strs}
    result = quality_task._compute_bin_description(data)
    expected_full = f" Data show {total_bins} total samples spanning {expected_bin_str}"
