...
class LogicGenerator(BaseGenerator):
    ...
    def _gen_client_wrappers(self):
        for tag in self.api_descriptor.apis:
            ...
            rendered_code = self.env.get_template("api_helper.jinja2").render(
                ...
                models=models,
                handlers=handlers if self.api_descriptor.target else [],
                api_name=tag,
                service_name=service_name,
            )
            file_name = f"{to_snake_case(self.api_descriptor.service_name)}/{to_snake_case(tag)}_helper.py"
            file_path = self.BASE_PATH / file_name
            if file_path.exists() and self.api_descriptor.target:
                self._append_http_method(async_mode, file_path, handlers)
            elif file_path.exists():
                LOGGER.info(f"File: {file_path} exists but it's not target service, skip generate!")
            else:
    ...
    def append_http_method(self, path):
        for tag in self.api_descriptor.apis:
            LOGGER.info(f"Generate client logic for: {tag}")
            ...
            file_name = f"{to_snake_case(service_name)}/{to_snake_case(tag)}_helper.py"
            file_path = self.BASE_PATH / file_name

            with (open(file_path, "r+") as file):
                text = file.read()
                for handler in handlers:
                    full_name = handler.method + handler.path
                    if to_snake_case(full_name) != to_snake_case(path):
                        continue
                    search = to_snake_case(full_name)
                    if not re.search(rf"{search}\(", text, re.MULTILINE):
                        template = self.env.get_template("http_helper_method.jinja2")
                        method_code = template.render(handler, async_mode=async_mode)
                        file.write(method_code)
                        LOGGER.info(
                            f"File: {file_path} is up to date with new method {handler.method + handler.path}!"
                        )
    ...
    def _gen_logic_facade(self):
        ...
        file_name = f"{service_name}_helper.py"
