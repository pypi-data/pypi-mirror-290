...
static int calc_r(struct Point2D* r_ii, struct Geometry* geo, struct Point2D* new_beam_center, struct Shape* new_image_shape) {
    ...
    for (size_t cc = 0; cc < (size_t)geo->columns; ++cc) {
        x[cc] = geo->poni_x - (double)cc;
    }
    for (size_t rr = 0; rr < (size_t)geo->rows; ++rr) {
        y[rr] = geo->poni_y - (double)rr;
    }

    double tilt_cos, tilt_sin, x_pos, y_pos, hori_travel_sq, alpha_scattered, phi_scattered, cos_alpha;
    ...
    for (size_t rr = 0; rr < (size_t)geo->rows; ++rr) {
        for (size_t cc = 0; cc < (size_t)geo->columns; ++cc) {
            ...
            phi_scattered = asin(x_pos / sqrt(hori_travel_sq)) - geo->yaw_angle;
            alpha_scattered = asin(y_pos / sqrt(hori_travel_sq + y_pos * y_pos)) - geo->incident_angle - geo->pitch_angle;
            cos_alpha = cos(alpha_scattered);
            q_xy_sq = cos_alpha * cos_alpha + cos_incident * cos_incident - 2.0 * cos_incident * cos_alpha * cos(phi_scattered);
    ...
    new_image_shape->rows = (npy_intp)ceil(new_beam_center->y - min_y) + 1;
    free(x);
    free(y);
    return 1;
}
...
static PyObject* transform(PyObject* self, PyObject* args) {
    ...
    double incident_angle, pixel_size, poni_y, poni_x, det_dist, pitch_angle, yaw_angle, tilt_angle, to_pixel;
    
    if (!PyArg_ParseTuple(args, "Odddddddd", &input_data_obj, &incident_angle, &pixel_size,
        &poni_y, &poni_x, &det_dist, &pitch_angle, &yaw_angle, &tilt_angle)) {
    ...
    if (PyTuple_Check(input_data_obj) || PyList_Check(input_data_obj)) {
        ...
        for (Py_ssize_t ii = 1; ii < im_num; ++ii) {
            ...
            if (PyArray_TYPE(data_array_obj_ptr[ii]) != NPY_DOUBLE) {
                PyErr_SetString(PyExc_ValueError, "An element was not dtype=np.float64 and should be.");
                free(data_array_obj_ptr);
                return NULL;
            }
            if (ndim != 2) {
                PyErr_SetString(PyExc_ValueError, "An element of the data input list/tuple is not a 2D NumPy Array and should be.");
                free(data_array_obj_ptr);
                return NULL;
            }
            ...
            if (!PyArray_IS_C_CONTIGUOUS(data_array_obj_ptr[ii])) {
                PyErr_SetString(PyExc_ValueError, "An element is not C-contiguous.");
                free(data_array_obj_ptr);
                return NULL;
            }
        }
    ...
    for (size_t ii = 0; ii < (size_t)im_num; ++ii) {
    ...
    struct Geometry geometry = {
        .poni_x = poni_x * to_pixel,
        .poni_y = (double)rows - poni_y * to_pixel,
    ...
    for (size_t ii = 0; ii < (size_t)im_num; ++ii) {
        ...
        *transformed_array_obj_ptr = (PyArrayObject*)PyArray_ZEROS(2, dim, NPY_DOUBLE, 0);
        if (*transformed_array_obj_ptr == NULL) {
            PyErr_SetString(PyExc_ValueError, "Failed to create internal arrays. Likely due to data input being incorrect shape.");
