    ...
    if image_file_name in dataset.keys():
        ...
        if "mask" in data.keys():
            ...
            for mask_name in mask_names:
                ...
                mask_sz = ResizeWithAspectRatio(mask, width=1024)
        ...
        if "objects" in data.keys():
            ...
            for obj in data["objects"]:
                ...
                texts.append(str(x) + ',' + str(y) + ' cid=' + str(obj["class_id"]))

                cv2.line(vis, [x-10, y], [x+10, y], color, 2)
                cv2.line(vis, [x, y-10], [x, y+10], color, 2)
            ...
            if len(texts) == len(texts_positions):
                for text, pos in zip(texts, texts_positions):
                    cv2.putText(vis, text, (pos[0]+15, pos[1]), font,
                                1, color, 2)
...
if __name__ == "__main__":
    ...
    for image_name, image_data in dataset.items():
        ...
        displayed_image = ResizeWithAspectRatio(img, width=1024)
        (h, w) = img.shape[:2]
