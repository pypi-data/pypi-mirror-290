...
from spartaqube_app.secrets import sparta_46e854c995
from project.sparta_cb7699d1a4.sparta_88ebfcbd43.qube_c949be3307 import sparta_f8b6cdb75e
import pytz
UTC=pytz.utc
class dotdict(dict):__getattr__=dict.get;__setattr__=dict.__setitem__;__delattr__=dict.__delitem__
def sparta_3b391b7acf(appViewsModels):
	A=appViewsModels
	...
	return A
def sparta_477438fc9d(thisText):A=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));A=A+str('/log/log.txt');B=open(A,'a');B.write(thisText);B.writelines('\n');B.close()
def sparta_f43a064780(request):A=request;return{'appName':'Project','user':A.user,'ip_address':A.META['REMOTE_ADDR']}
def sparta_6b8ce67fcf():return conf_settings.PLATFORM
def sparta_4ddedd7098():
...
def sparta_eacc3c863b(request):
	...
	try:
		...
		if F.count()==0:AppVersioning.objects.create(last_check_date=E);A=_A
			...
			D=F[0];G=D.last_check_date;H=E-G;I=sparta_f8b6cdb75e();J=D.last_available_version_pip
			if not I==J:A=_A
...
def sparta_3f89e05aa0(captcha):
	D='errorMsg';B='res';A=captcha
	try:
		if A is not _C:
			if len(A)>0:
				E=sparta_46e854c995()['CAPTCHA_SECRET_KEY'];F=f"https://www.google.com/recaptcha/api/siteverify?secret={E}&response={A}";C=requests.get(F)
				if int(C.status_code)==200:
					G=json.loads(C.text)
					if G['success']:return{B:1}
	except Exception as H:return{B:-1,D:str(H)}
	return{B:-1,D:'Invalid captcha'}
def sparta_7a48e2ac6d(password):
...
def sparta_981707199d(user):
	...
	A[_D]=sparta_4ddedd7098();return A
def sparta_a89dbf4053(user):A=dict();A[_D]=sparta_4ddedd7098();return A
def sparta_e3ff9dcca0():
	try:socket.create_connection(('1.1.1.1',53));return _A
	except OSError:pass
	return _B
def sparta_afcedc3765():A=socket.gethostname();B=socket.gethostbyname(A);return A,B