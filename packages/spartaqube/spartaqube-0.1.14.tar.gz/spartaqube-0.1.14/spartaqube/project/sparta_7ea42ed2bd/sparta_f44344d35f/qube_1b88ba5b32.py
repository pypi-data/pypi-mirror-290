...
class db_connection_postgre(db_connection_sql):
	...
	def create_connection(A):
		if A.bPrint:print('create_connection for POSTGRESQL now');print('self.hostname => '+str(A.hostname));print('self.user => '+str(A.user));print('self.password => '+str(A.password));print('self.port => '+str(A.port));print('self.schemaName => '+str(A.schemaName));print('self.database => '+str(A.db))
