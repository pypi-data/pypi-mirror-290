...
from project.sparta_cb7699d1a4.sparta_6fab622ea8 import qube_616d89410b as qube_616d89410b
def sparta_5a95164dbd():return str(uuid.uuid4())
def sparta_26d67bddfd(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_9fd69d8c2d(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_8be17fefd3(path):
	with open(path,'a'):os.utime(path,None)
def sparta_e51ff06034(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_9fd69d8c2d(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_8be17fefd3(B)
	...
	return{_A:1}
def sparta_55e932f206(json_data,userObj):
	...
	return{_A:1}
def sparta_3baf64a99e(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_9fd69d8c2d(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_7f7d195fff(json_data,userObj):A=json_data[_B];B=qube_616d89410b.sparta_e7706de71a(A);C={_A:1,'folderStructure':B};return C
def sparta_c645f12831(json_data,userObj):
	A=json_data
	try:
...
def sparta_02dc241237(json_data,userObj):
	...
	return{_A:1}
def sparta_0e8ad9a87b(json_data,userObj):
	...
	return{_A:1}
def sparta_7fc9396d78(json_data,userObj):
...
def sparta_06cdeae40d(json_data,userObj):
	...
	return{_A:-1}
def sparta_e95565406c(json_data,userObj):
