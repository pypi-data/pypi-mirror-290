...
def sendEmailFunc(emailObj,USERNAME_SMTP=_A,PASSWORD_SMTP=_A,EMAIL_SMTP=_A,HOST=_A,PORT=25,SENDERNAME=_A):
	...
	if F is _A:D=sparta_46e854c995();HOST=D['EMAIL_HOST_SMTP'];F=D['EMAIL_USERNAME_SMTP'];H=D['EMAIL_RECIPIENT'];K=D['EMAIL_PASSWORD_SMTP'];PORT=D['EMAIL_PORT_SMTP'];G=D['EMAIL_SENDERNAME']
	if F is _A:return{O:-1,P:'You need to configure an email sender service in your profile view'}
	X=C.getEmailTitle();Y=C.getHTML();B=V('related');B['Subject']=X;B['From']=email.utils.formataddr((G,H));B['To']=I;B['Message-ID']=make_msgid();print('RECIPIENT');print(I);Z=C.getEmailB64ImgList();a=C.getEmailImgNameArr()
	...
	for(J,f)in enumerate(d):A=L('application','octet-stream');A.set_payload(f);A.add_header(Q,R);M=e[J];print('fileName > '+str(M));A[S]=T%M;B.attach(A)
