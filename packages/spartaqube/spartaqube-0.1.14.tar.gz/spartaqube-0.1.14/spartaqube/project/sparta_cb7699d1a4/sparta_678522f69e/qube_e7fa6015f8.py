...
from project.sparta_7ea42ed2bd.sparta_4b6986daa6.qube_8d44c27597 import sparta_3f89e05aa0,sparta_ebd326bc35,sparta_7a48e2ac6d
from project.sparta_cb7699d1a4.sparta_67553e9f2a.qube_7480dd05a3 import sparta_f1b9c99a56
def sparta_20564b22a2(json_data,user_obj):
...
def sparta_7c4e4ed336(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_02146abdca(json_data,userObj):
	...
	return B
def sparta_8ef626b216(json_data,userObj):
	...
	return A
def sparta_c6e31146a7(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_f1b9c99a56(F):return{_A:-1,_B:'Invalid current code'}
	...
	return{_A:1}
def sparta_c9290ebfa9(json_data,userObj):A=userObj;C=json_data['base64image'];K=hashlib.sha256((str(A.id)+'_'+A.email+str(datetime.now())).encode(_D)).hexdigest();D,E=C.split(';base64,');F,L=D.split('/');G=F.split(':')[-1];B=UserProfile.objects.get(user=A);H=datetime.now();I=Avatar.objects.create(avatar=G,image64=E,date_created=H);B.avatar=I;B.save();J={_A:1};return J
def sparta_b6fb7fd68f(json_data,userObj):B=json_data['bDarkTheme'];A=UserProfile.objects.get(user=userObj);A.is_dark_theme=B;A.save();C={_A:1};return C
def sparta_ee636968a5(json_data,userObj):B=json_data['theme'];A=UserProfile.objects.get(user=userObj);A.editor_theme=B;A.save();C={_A:1};return C
def sparta_fdf6dc481a():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_2603c5933c(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_7a48e2ac6d(E):return{_A:-1,_B:_I}
...
def sparta_614b9ef11e(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_3f89e05aa0(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_ebd326bc35(F):return{_A:-1,_B:_I}
...
def sparta_00ff3bcba2(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_3f89e05aa0(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_ebd326bc35(G):return{_A:-1,_B:_I}
