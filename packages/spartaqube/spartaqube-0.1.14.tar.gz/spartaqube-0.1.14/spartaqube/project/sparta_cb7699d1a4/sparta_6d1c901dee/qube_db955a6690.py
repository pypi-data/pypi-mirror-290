...
from project.sparta_cb7699d1a4.sparta_6d1c901dee import qube_758d26fd35 as qube_758d26fd35
...
def sparta_99f7e83369(user_obj):
	...
	return B
def sparta_8c240afc6d(json_data,user_obj):
	D=user_obj;E=sparta_99f7e83369(D)
...
def sparta_0027046b45():return{_A:1,'available_engines':qube_dc1adaef53.sparta_0027046b45()}
def sparta_54bcb37169(json_data,user_obj):
	...
	return{_A:1}
def sparta_27ec2b3396(json_data):
...
def sparta_f7157d8b8e(json_data):
...
def sparta_9c68f4d945(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_dc1adaef53.sparta_f8c88202a6(A[_j]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_k],organization=A[_l],lib_dir=A[_m],driver=A[_n],trusted_connection=A[_o],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_2a6225f065(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_99f7e83369(C)
...
def sparta_69959643b2(json_data,user_obj):
	C=json_data[_H];A=sparta_bf96eec88b(C,user_obj)
...
def sparta_9c181e95b1(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_bf96eec88b(H,user_obj)
...
def sparta_ccc5ff8e17(json_data,db_connector_obj):
...
def sparta_949bb4e75c(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_bf96eec88b(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_ccc5ff8e17(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_59d061ad78(E)}
def sparta_73036bd2c8(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_bf96eec88b(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_ccc5ff8e17(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_400295145a(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_54828d7d38(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_bf96eec88b(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_ccc5ff8e17(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_59d061ad78(E)}
def sparta_65654ca2ba(json_data,user_obj):
	A=json_data;print('SAVE json_data');print(A);J=A[_L];D=_B
	if J:D=A[_s];D=qube_6b4f96c9f7.sparta_c77468c31f(D)
...
def sparta_b94efd081e(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_s];C=qube_6b4f96c9f7.sparta_c77468c31f(C)
	...
	return{_A:1}
def sparta_b7e0bb7778(json_data,user_obj):0
def sparta_d00c8c20ac(json_data,user_obj):
	D=user_obj;F=sparta_99f7e83369(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_cfd732dd0d(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_bf96eec88b(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_59d061ad78(sparta_51c1448d0f(i,variable_name=W))
...
def sparta_6e36cd36c3(json_data,user_obj):
	...
	return{_A:1}
def sparta_ab609df21f(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_59d061ad78(sparta_51c1448d0f(D,variable_name=A))
	return B
def sparta_17482e3ef4(session_id):A=sparta_ab609df21f(session_id);B=sparta_64e560e361(b_return_type_id=_C);C=json.loads(A[_AP])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AQ];return{_A:1,_AR:A,_AS:A.get(_R,dict()),_Q:D}
def sparta_56b2ead101(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_ab609df21f(session_id);I=sparta_64e560e361(b_return_type_id=_C);J=json.loads(F[_AP])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AQ];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AR:F,_AS:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_102383c5ff(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_99f7e83369(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_6b4f96c9f7.sparta_79c39310fc(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_dcc80e34a9(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_6b4f96c9f7.sparta_c77468c31f(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_99f7e83369(C)
	...
	return{_A:-1}
def sparta_80ecee9724(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_99f7e83369(C)
	...
	return{_A:1,_AD:_D}
def sparta_3f96f52071(plot_db_chart_obj):
	...
	for A in B.keys():
		if A in INPUTS_KEYS:
			if A==_x:C[A]=1
				...
				if D>0:C[A]=D
	return C
def sparta_57ee9f41e9(json_data,user_obj):
	B=user_obj;D=sparta_99f7e83369(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_s];C=qube_6b4f96c9f7.sparta_c77468c31f(C)
...
def sparta_095cc40ded(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_s];C=qube_6b4f96c9f7.sparta_c77468c31f(C)
	...
	return{_A:1}
def sparta_a47a281bf2(json_data,user_obj):
	...
	return{_A:1}
def sparta_97cdf90505(user_obj):
	B=user_obj;C=sparta_99f7e83369(B)
	...
	return E
def sparta_8a3753806a(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_80ecee9724(A['widget_id'],B);D=C[_AD]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_cfd732dd0d(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_6b9c3ea675(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_758d26fd35.sparta_19b45ab7e7(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_da06b88dc3(json_data,user_obj):
...
def sparta_1d30ff77b1(json_data,user_obj):
