...
from project.sparta_cb7699d1a4.sparta_ab3246baf1.qube_000080ec94 import EngineBuilder
from project.sparta_cb7699d1a4.sparta_054a28acad.qube_2ba6f147b4 import sparta_51c1448d0f
class QuestDBConnector(EngineBuilder):
	...
	def get_data_table(A,table_name):E=f"{A.host}:{A.port}/exec";F=f"SELECT * FROM {table_name}";B=requests.get(E,params={_A:F},auth=HTTPBasicAuth(A.user,A.password));B.raise_for_status();C=B.json();G=[A['name']for A in C[_C]];D=sparta_51c1448d0f(C[_B]);D.columns=G;return D
	def get_data_table_query(A,sql,table_name=None):E=f"{A.host}:{A.port}/exec";F=sql;B=requests.get(E,params={_A:F},auth=HTTPBasicAuth(A.user,A.password));B.raise_for_status();C=B.json();G=[A['name']for A in C[_C]];D=sparta_51c1448d0f(C[_B]);D.columns=G;return D