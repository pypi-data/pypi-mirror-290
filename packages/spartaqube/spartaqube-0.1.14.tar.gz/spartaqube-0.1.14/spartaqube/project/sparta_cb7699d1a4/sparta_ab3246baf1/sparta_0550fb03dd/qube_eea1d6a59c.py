...
class DuckDBConnector(EngineBuilder):
	...
	def get_available_tables(A):
		...
		except Exception as D:A.connector.close();print(f"Failed to list tables: {D}");return[]
	def get_table_columns(A,table_name):
		...
		except Exception as F:A.connector.close();print(f"Failed to list columns for table '{B}': {F}");return[]
