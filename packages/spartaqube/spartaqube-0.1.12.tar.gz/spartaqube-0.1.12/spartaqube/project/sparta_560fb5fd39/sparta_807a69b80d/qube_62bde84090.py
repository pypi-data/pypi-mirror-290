...
def sparta_c6d6af1d81(json_data,user_obj):
	...
	for A in K:
		B=sparta_dfb820915b(model_to_dict(A));Q=int(A.typeObject)
		if not A.is_seen:print('Add 1');E+=1
		for R in P:B.pop(R,None)
		C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.date_created.astimezone(UTC);B[N]=G;B[O]=sparta_5779032dd2(H)
		if Q==0:0
	...
	for A in L:
		...
		B=sparta_dfb820915b(model_to_dict(A));C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.dateCreated.astimezone(UTC);B[N]=G;B['type_object']=-2;B[O]=sparta_5779032dd2(H);F.append(B)
...
def sparta_5779032dd2(dateCreated):
	...
	return str(A)
def sparta_ecd65e0514(json_data,user_obj):
