...
from project.sparta_560fb5fd39.sparta_940f55046f import qube_f9c9dc80ac as qube_f9c9dc80ac
def sparta_e9131142c0(json_data,userObj):
...
def sparta_d1abcd8e95(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_578150723d.sparta_c46725fb9e(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_578150723d.sparta_c46725fb9e(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_ab3d09524e(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_578150723d.sparta_c46725fb9e(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_578150723d.sparta_c46725fb9e(A);C=[A.userGroup for A in H]
...
def sparta_1d19526d0f(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';J=json_data;B=user_obj;print('json_data SHARE OBJECTS');print(J);L=datetime.now().astimezone(UTC);P=qube_578150723d.sparta_c46725fb9e(B);M=[A.userGroup for A in P];U=J['member2ShareArr'];I=int(J[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_578150723d.sparta_c46725fb9e(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_578150723d.sparta_c46725fb9e(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_6f1345de10(json_data,user_obj):
