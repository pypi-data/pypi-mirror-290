...
from project.sparta_560fb5fd39.sparta_b63ad33d08 import qube_b807b86190 as qube_b807b86190
...
def sparta_5b68c4c43c(user_obj):
	A=qube_578150723d.sparta_c46725fb9e(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_c2d8b9ac68(json_data,user_obj):
	D=user_obj;E=sparta_5b68c4c43c(D)
...
def sparta_05cf9541af():return{_A:1,'available_engines':qube_4efb9f6ee6.sparta_05cf9541af()}
def sparta_cbd5e582d8(json_data,user_obj):
	...
	return{_A:1}
def sparta_e63643c4ab(json_data):
...
def sparta_d7bbfc28e7(json_data):
...
def sparta_05e2c3e29c(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_4efb9f6ee6.sparta_067d8c87ea(A[_j]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_k],organization=A[_l],lib_dir=A[_m],driver=A[_n],trusted_connection=A[_o],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_453f78f92a(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_5b68c4c43c(C)
...
def sparta_fe339b5aca(json_data,user_obj):
	C=json_data[_H];A=sparta_e9cf5d547c(C,user_obj)
...
def sparta_27e12d0296(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_e9cf5d547c(H,user_obj)
...
def sparta_dc415fcc6c(json_data,db_connector_obj):
...
def sparta_5253148b63(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_e9cf5d547c(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_dc415fcc6c(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_b07bd8c600(E)}
def sparta_4b4923a938(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_e9cf5d547c(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_dc415fcc6c(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_e16d746adc(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_735b4183f6(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_e9cf5d547c(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_dc415fcc6c(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_b07bd8c600(E)}
def sparta_3fb41e1c73(json_data,user_obj):
	A=json_data;print('SAVE json_data');print(A);J=A[_L];D=_B
	if J:D=A[_s];D=qube_cdd031e311.sparta_909d2945d2(D)
...
def sparta_bc75ca0f55(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_s];C=qube_cdd031e311.sparta_909d2945d2(C)
	...
	return{_A:1}
def sparta_7d34ad1d30(json_data,user_obj):0
def sparta_84f9ef5266(json_data,user_obj):
	D=user_obj;F=sparta_5b68c4c43c(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_2b9a318db0(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_e9cf5d547c(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_b07bd8c600(sparta_dc18dcc477(i,variable_name=W))
...
def sparta_8a839249c3(json_data,user_obj):
	...
	return{_A:1}
def sparta_5a05183e71(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_b07bd8c600(sparta_dc18dcc477(D,variable_name=A))
	return B
def sparta_6fcbf11e32(session_id):A=sparta_5a05183e71(session_id);B=sparta_0ab07f545c(b_return_type_id=_C);C=json.loads(A[_AP])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AQ];return{_A:1,_AR:A,_AS:A.get(_R,dict()),_Q:D}
def sparta_c0fc59ec6d(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_5a05183e71(session_id);I=sparta_0ab07f545c(b_return_type_id=_C);J=json.loads(F[_AP])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AQ];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AR:F,_AS:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_560db30322(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_5b68c4c43c(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_cdd031e311.sparta_7d78a25dc0(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_4e84715a99(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_cdd031e311.sparta_909d2945d2(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_5b68c4c43c(C)
	...
	return{_A:-1}
def sparta_82e7ee952c(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_5b68c4c43c(C)
	...
	return{_A:1,_AD:_D}
def sparta_b97785b7ff(plot_db_chart_obj):
	...
	for A in B.keys():
		if A in INPUTS_KEYS:
			if A==_x:C[A]=1
				...
				if D>0:C[A]=D
	return C
def sparta_614aa49104(json_data,user_obj):
	B=user_obj;D=sparta_5b68c4c43c(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_s];C=qube_cdd031e311.sparta_909d2945d2(C)
...
def sparta_d28d2fd563(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_s];C=qube_cdd031e311.sparta_909d2945d2(C)
	...
	return{_A:1}
def sparta_b6f974dbaa(json_data,user_obj):
	...
	return{_A:1}
def sparta_0c2d3611fe(user_obj):
	B=user_obj;C=sparta_5b68c4c43c(B)
	...
	return E
def sparta_bd91bcf24b(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_82e7ee952c(A['widget_id'],B);D=C[_AD]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_2b9a318db0(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_90655c3fef(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_b807b86190.sparta_2cb95cb503(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_681037f6e3(json_data,user_obj):
...
def sparta_e469921e95(json_data,user_obj):
