...
from project.sparta_560fb5fd39.sparta_2aa5cd3b20 import qube_6958cdc79f as qube_6958cdc79f
def sparta_2268868c41():return str(uuid.uuid4())
def sparta_32b84ecc9f(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_b2d7b99982(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_a57b13a38a(path):
	with open(path,'a'):os.utime(path,None)
def sparta_6b2ae25455(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_b2d7b99982(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_a57b13a38a(B)
	...
	return{_A:1}
def sparta_eabb20d474(json_data,userObj):
	...
	return{_A:1}
def sparta_6e84a3fcfe(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_b2d7b99982(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_98f730f705(json_data,userObj):A=json_data[_B];B=qube_6958cdc79f.sparta_e008cf3585(A);C={_A:1,'folderStructure':B};return C
def sparta_b89fd1fbac(json_data,userObj):
	A=json_data
	try:
...
def sparta_1dbd6a417e(json_data,userObj):
	...
	return{_A:1}
def sparta_25ca8b097b(json_data,userObj):
	...
	return{_A:1}
def sparta_882253a7ce(json_data,userObj):
...
def sparta_ee0bc81dfb(json_data,userObj):
	...
	return{_A:-1}
def sparta_7f054fd86f(json_data,userObj):
