...
def sparta_3b7fda7ed5(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_106deea7a7(G,F,A,B);H={_A:1,_C:C};return H
def sparta_106deea7a7(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_7f78e2d2b5(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_c46725fb9e(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_c158624b7f(json_data,user_obj):A=sparta_7f78e2d2b5(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_8a906e6671(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_94631dde19(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_106deea7a7(G,A,I,C)
	J={_A:1};return J
def sparta_03b0115b9b(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_1f860dd7f6(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_f08833eb68(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_d863c6b34e(json_data,user_obj):
