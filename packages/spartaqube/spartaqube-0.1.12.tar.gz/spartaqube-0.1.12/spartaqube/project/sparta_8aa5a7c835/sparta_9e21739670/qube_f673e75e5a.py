...
from spartaqube_app.secrets import sparta_016c61e21a
from project.sparta_560fb5fd39.sparta_ae003213a1.qube_6c05ecbe46 import sparta_805880f1a1
import pytz
UTC=pytz.utc
class dotdict(dict):__getattr__=dict.get;__setattr__=dict.__setitem__;__delattr__=dict.__delitem__
def sparta_dfb820915b(appViewsModels):
	A=appViewsModels
	...
	return A
def sparta_3575b15cb9(thisText):A=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));A=A+str('/log/log.txt');B=open(A,'a');B.write(thisText);B.writelines('\n');B.close()
def sparta_b024cde646(request):A=request;return{'appName':'Project','user':A.user,'ip_address':A.META['REMOTE_ADDR']}
def sparta_7a10034405():return conf_settings.PLATFORM
def sparta_eef3e44917():
...
def sparta_f06548cd94(request):
	...
	try:
		...
		if F.count()==0:AppVersioning.objects.create(last_check_date=E);A=_A
			...
			D=F[0];G=D.last_check_date;H=E-G;I=sparta_805880f1a1();J=D.last_available_version_pip
			if not I==J:A=_A
...
def sparta_1d487ed1b4(captcha):
	D='errorMsg';B='res';A=captcha
	try:
		if A is not _C:
			if len(A)>0:
				E=sparta_016c61e21a()['CAPTCHA_SECRET_KEY'];F=f"https://www.google.com/recaptcha/api/siteverify?secret={E}&response={A}";C=requests.get(F)
				if int(C.status_code)==200:
					G=json.loads(C.text)
					if G['success']:return{B:1}
	except Exception as H:return{B:-1,D:str(H)}
	return{B:-1,D:'Invalid captcha'}
def sparta_7ddebadc9c(password):
...
def sparta_48f5515226(user):
	...
	A[_D]=sparta_eef3e44917();return A
def sparta_18cb673724(user):A=dict();A[_D]=sparta_eef3e44917();return A
def sparta_68254c7dfc():
	try:socket.create_connection(('1.1.1.1',53));return _A
	except OSError:pass
	return _B
def sparta_d11e279f79():A=socket.gethostname();B=socket.gethostbyname(A);return A,B