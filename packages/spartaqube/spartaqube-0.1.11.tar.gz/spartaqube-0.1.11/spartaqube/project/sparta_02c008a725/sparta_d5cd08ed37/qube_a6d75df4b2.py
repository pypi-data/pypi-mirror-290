...
from project.sparta_a1c64beb30.sparta_bf5619a7b3.qube_f9dc643d78 import IPythonKernel as IPythonKernel
from project.sparta_a1c64beb30.sparta_bf5619a7b3.qube_a6ee4ade7b import sparta_4e0f4f4486
	...
	def receive(A,text_data):
		...
		if len(M)>0:
			...
			if B=='init-socket'or B=='reconnect-kernel':E={G:1,F:B};A.start_kernel();D=json.dumps(E);A.send(text_data=D);return
				...
				O=[];P=[]
				for I in C[S]:
					X=A.kernel_obj.get_workspace_variable(kernel_variable=I);J=sparta_572804153f(X,variable_name=I);print('workspace_variable_df');print(J);print(type(J))
					try:O.append(sparta_989c399d8e(J));P.append(I)
					except:pass
