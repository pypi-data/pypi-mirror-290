...
from project.sparta_a1c64beb30.sparta_e2ccc257c8 import qube_b68b73ca7a as qube_b68b73ca7a
def sparta_20a442ef2a():return str(uuid.uuid4())
def sparta_6a2e3b5148(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_8ab825c2eb(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_89b3e51ba3(path):
	with open(path,'a'):os.utime(path,None)
def sparta_9ba75b06ab(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_8ab825c2eb(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_89b3e51ba3(B)
	...
	return{_A:1}
def sparta_5afba87b38(json_data,userObj):
	...
	return{_A:1}
def sparta_d0f1d007ba(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_8ab825c2eb(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_2a5f0dbcd4(json_data,userObj):A=json_data[_B];B=qube_b68b73ca7a.sparta_bf6693e157(A);C={_A:1,'folderStructure':B};return C
def sparta_805ac17435(json_data,userObj):
	A=json_data
	try:
...
def sparta_2093a666cc(json_data,userObj):
	...
	return{_A:1}
def sparta_2576bff371(json_data,userObj):
	...
	return{_A:1}
def sparta_ee53bd9e16(json_data,userObj):
...
def sparta_a645d2095c(json_data,userObj):
	...
	return{_A:-1}
def sparta_9d36c88c4f(json_data,userObj):
