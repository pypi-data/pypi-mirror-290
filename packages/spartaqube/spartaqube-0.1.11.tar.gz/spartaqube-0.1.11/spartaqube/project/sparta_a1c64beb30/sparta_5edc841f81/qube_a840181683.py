...
from project.sparta_02c008a725.sparta_27262642be.qube_ddd37178e0 import sparta_5d58fce052,sparta_1bb7372f09,sparta_e39f2dce3b
from project.sparta_a1c64beb30.sparta_d5a4a4b037.qube_53339f257e import sparta_644b48ef5e
def sparta_12dd40d3d1(json_data,user_obj):
...
def sparta_4fc53e5ae5(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_6adc290f96(json_data,userObj):
	...
	return B
def sparta_40096586e2(json_data,userObj):
	...
	return A
def sparta_f44ce15e78(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_644b48ef5e(F):return{_A:-1,_B:'Invalid current code'}
	...
	return{_A:1}
def sparta_914c58b08b(json_data,userObj):A=userObj;C=json_data['base64image'];K=hashlib.sha256((str(A.id)+'_'+A.email+str(datetime.now())).encode(_D)).hexdigest();D,E=C.split(';base64,');F,L=D.split('/');G=F.split(':')[-1];B=UserProfile.objects.get(user=A);H=datetime.now();I=Avatar.objects.create(avatar=G,image64=E,date_created=H);B.avatar=I;B.save();J={_A:1};return J
def sparta_3b7fa09516(json_data,userObj):B=json_data['bDarkTheme'];A=UserProfile.objects.get(user=userObj);A.is_dark_theme=B;A.save();C={_A:1};return C
def sparta_3de530ac1c(json_data,userObj):B=json_data['theme'];A=UserProfile.objects.get(user=userObj);A.editor_theme=B;A.save();C={_A:1};return C
def sparta_c5a41b1e71():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_c522eb03a6(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_e39f2dce3b(E):return{_A:-1,_B:_I}
...
def sparta_aa30edf516(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_5d58fce052(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_1bb7372f09(F):return{_A:-1,_B:_I}
...
def sparta_694e50c875(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_5d58fce052(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_1bb7372f09(G):return{_A:-1,_B:_I}
