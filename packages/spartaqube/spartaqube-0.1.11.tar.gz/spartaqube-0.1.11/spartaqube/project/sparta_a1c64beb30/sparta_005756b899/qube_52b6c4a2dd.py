...
from project.sparta_02c008a725.sparta_27262642be.qube_ddd37178e0 import sparta_95f1a84224
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_9f201df891(json_data,user_obj):
...
def sparta_38aaf2661b(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_d00b9802a8(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_95f1a84224(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_95f1a84224(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_7687f542a5(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_95f1a84224(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_36234817ba(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_38aaf2661b(F,A.type_ticket,None)
...
def sparta_bab01a8143(json_data,user_obj):
	...
	return{_A:1}
def sparta_6cd88af124(json_data):
	...
	return{_A:1}
def sparta_9a990e68c7(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}