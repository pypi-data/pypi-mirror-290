...
from project.sparta_a1c64beb30.sparta_21e3337ca7 import qube_02e4902b30 as qube_02e4902b30
...
def sparta_cf49c2b161(user_obj):
	A=qube_53baa17931.sparta_a4e2f461ac(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_6e317e662f(json_data,user_obj):
	D=user_obj;E=sparta_cf49c2b161(D)
...
def sparta_c4cc4010b3():return{_A:1,'available_engines':qube_d5ae4ab5b8.sparta_c4cc4010b3()}
def sparta_9ab4ad53b0(json_data,user_obj):
	...
	return{_A:1}
def sparta_1577f1c5e8(json_data):
...
def sparta_9a1ea3b55c(json_data):
...
def sparta_555798cf4f(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_d5ae4ab5b8.sparta_00ed7546d4(A[_j]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_k],organization=A[_l],lib_dir=A[_m],driver=A[_n],trusted_connection=A[_o],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_b592c2dde0(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_cf49c2b161(C)
...
def sparta_4b3d4396a1(json_data,user_obj):
	C=json_data[_H];A=sparta_3c8472bfc8(C,user_obj)
...
def sparta_b0ffcbc70f(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_3c8472bfc8(H,user_obj)
...
def sparta_99840fc515(json_data,db_connector_obj):
...
def sparta_038bec0a6d(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_3c8472bfc8(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_99840fc515(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_989c399d8e(E)}
def sparta_69d7f79b4a(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_3c8472bfc8(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_99840fc515(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_aee914beed(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_6970e2f6a5(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_3c8472bfc8(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_99840fc515(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_989c399d8e(E)}
def sparta_a4d514d330(json_data,user_obj):
	A=json_data;print('SAVE json_data');print(A);J=A[_L];D=_B
	if J:D=A[_s];D=qube_10eb74ee26.sparta_c4abd8977b(D)
...
def sparta_12324fe37c(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_s];C=qube_10eb74ee26.sparta_c4abd8977b(C)
	...
	return{_A:1}
def sparta_8043c94940(json_data,user_obj):0
def sparta_e82acc6c2d(json_data,user_obj):
	D=user_obj;F=sparta_cf49c2b161(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_7ecff44a28(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_3c8472bfc8(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_989c399d8e(sparta_572804153f(i,variable_name=W))
...
def sparta_f43c16ee46(json_data,user_obj):
	...
	return{_A:1}
def sparta_de5a7c2c30(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_989c399d8e(sparta_572804153f(D,variable_name=A))
	return B
def sparta_6670279906(session_id):A=sparta_de5a7c2c30(session_id);B=sparta_5a5c45f0d3(b_return_type_id=_C);C=json.loads(A[_AP])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AQ];return{_A:1,_AR:A,_AS:A.get(_R,dict()),_Q:D}
def sparta_51cadbf0d8(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_de5a7c2c30(session_id);I=sparta_5a5c45f0d3(b_return_type_id=_C);J=json.loads(F[_AP])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AQ];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AR:F,_AS:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_73ea6c3549(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_cf49c2b161(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_10eb74ee26.sparta_e82e406e32(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_27d0b279ae(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_10eb74ee26.sparta_c4abd8977b(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_cf49c2b161(C)
	...
	return{_A:-1}
def sparta_80c856ab56(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_cf49c2b161(C)
	...
	return{_A:1,_AD:_D}
def sparta_1bc6603a6c(plot_db_chart_obj):
	...
	for A in B.keys():
		if A in INPUTS_KEYS:
			if A==_x:C[A]=1
				...
				if D>0:C[A]=D
	return C
def sparta_bdc83ad1ba(json_data,user_obj):
	B=user_obj;D=sparta_cf49c2b161(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_s];C=qube_10eb74ee26.sparta_c4abd8977b(C)
...
def sparta_07ce79b0c3(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_s];C=qube_10eb74ee26.sparta_c4abd8977b(C)
	...
	return{_A:1}
def sparta_ed31660ffa(json_data,user_obj):
	...
	return{_A:1}
def sparta_e22d40b05b(user_obj):
	B=user_obj;C=sparta_cf49c2b161(B)
	...
	return E
def sparta_b520095f70(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_80c856ab56(A['widget_id'],B);D=C[_AD]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_7ecff44a28(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_23388675bd(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_02e4902b30.sparta_3993ccadb7(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_b3912762c2(json_data,user_obj):
...
def sparta_b7c9007d5f(json_data,user_obj):
