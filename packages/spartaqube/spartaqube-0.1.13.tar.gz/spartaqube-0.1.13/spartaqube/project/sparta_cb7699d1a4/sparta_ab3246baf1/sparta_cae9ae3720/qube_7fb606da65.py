...
class CassandraConnector(EngineBuilder):
	def __init__(A,host,port,user,password,keyspace):C=password;B=keyspace;A.keyspace=B;super().__init__(host=host,port=port,user=user,password=C,engine_name='cassandra');A.set_url_engine(f"cassandra://{user}:{C}@{host}:{port}/{B}");A.cluster=A.build_cassandra(A.keyspace);A.connector=A.cluster.connect(B);A.connector.set_keyspace(B)
	...
	def get_data_table(A,table_name):
		try:B=A.connector;C=f"SELECT * FROM {table_name}";D=B.execute(C);E=[dict(A._asdict())for A in D];return sparta_51c1448d0f(E)
		except Exception as F:A.cluster.shutdown();raise Exception(F)
	def get_data_table_query(A,sql,table_name=None):
		try:B=A.connector;C=sql;D=B.execute(C);E=[dict(A._asdict())for A in D];return sparta_51c1448d0f(E)
		except Exception as F:A.cluster.shutdown();raise Exception(F)