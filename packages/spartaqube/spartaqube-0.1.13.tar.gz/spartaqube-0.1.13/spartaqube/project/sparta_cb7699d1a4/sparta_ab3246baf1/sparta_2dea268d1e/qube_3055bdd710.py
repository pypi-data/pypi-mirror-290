...
_B='res'
_A=None
import sys,json,requests,asyncio,websocket,threading,pandas as pd
...
class WssConnector(EngineBuilder):
	...
	def start_stream(self,gui_websocket,b_get_print_buffer=True):
		...
		def thread_streaming_processing_func():
			while not self.stop_event.is_set():
				while queue_connector:msg=queue_connector.pop();msg_df=sparta_51c1448d0f(msg);msg_json=sparta_59d061ad78(msg_df);res={_B:1,'msg':msg,'msg_json':msg_json,'maxlen':queue_connector.maxlen};resJson=json.dumps(res);gui_websocket.send(resJson)
	...
	def stop_stream(self,gui_websocket):print('### stop stream event ###');self.stop_threads();res={_B:-100};resJson=json.dumps(res);gui_websocket.send(resJson)
	def send_error_event_and_stop(self,gui_websocket,error_msg):
		print(f"Error detected, stop stream and kill threads: {error_msg}")
