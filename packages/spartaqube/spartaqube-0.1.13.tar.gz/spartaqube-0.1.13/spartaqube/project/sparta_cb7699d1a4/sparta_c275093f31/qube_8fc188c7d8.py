...
from project.sparta_7ea42ed2bd.sparta_4b6986daa6.qube_8d44c27597 import sparta_3b391b7acf
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_cb14aa2d5e(json_data,user_obj):
...
def sparta_d428fdd980(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_64fe564782(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_3b391b7acf(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_3b391b7acf(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_435f4dc944(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_3b391b7acf(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_d39859a585(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_d428fdd980(F,A.type_ticket,None)
...
def sparta_a188c8a091(json_data,user_obj):
	...
	return{_A:1}
def sparta_3f7da0b643(json_data):
	...
	return{_A:1}
def sparta_bf484c712a(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}