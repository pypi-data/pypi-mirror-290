...
class IndraClient:
    ...
    async def _get_history_block(
        username=None, password=None, domain=None, start_time=None, end_time=None
    ):
        ic = IndraClient(profile_name="default", verbose=False)
        await ic.init_connection()
        ret = await ic.login_wait(username, password)
        if ret is not None:
            data = await ic.get_wait_history(domain, start_time, end_time)
    ...
    @staticmethod
    def get_history_sync(username, password, domain, start_time, end_time=None):
        return asyncio.run(
            IndraClient._get_history_block(
                username, password, domain, start_time, end_time
            )
        )
