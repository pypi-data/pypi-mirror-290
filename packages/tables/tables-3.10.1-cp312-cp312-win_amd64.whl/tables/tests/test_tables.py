...
class OldRecord(tb.IsDescription):
    ...
    if hasattr(tb, 'Complex192Col'):
        var14 = tb.ComplexCol(itemsize=24, shape=(), dflt=(1.-0.j))
    if hasattr(tb, 'Complex256Col'):
        var15 = tb.ComplexCol(itemsize=32, shape=(), dflt=(1.-0.j))
...
class DefaultValues(common.TempFileMixin, common.PyTablesTestCase):
    ...
    def test00(self):
        ...
        # Create a table
        table = self.h5file.create_table(self.h5file.root, 'table',
                                         self.record)
...
class PointSelectionTestCase(common.TempFileMixin, common.PyTablesTestCase):
    def setUp(self):
        ...
        recarr["f0"][:] = data
        recarr["f1"][:] = data
        self.table = self.h5file.create_table(self.h5file.root, 'table',
                                              recarr)
...
class ExhaustedIter(common.TempFileMixin, common.PyTablesTestCase):
    def setUp(self):
        ...
        table = self.h5file.create_table('/', 'observations', Observations,
                                         chunkshape=32)
...
class AccessClosedTestCase(common.TempFileMixin, common.PyTablesTestCase):
    ...
    def test_read(self):
        self.h5file.close()
        self.assertRaises(
            tb.ClosedNodeError, self.table.read)

    def test_getitem(self):
        self.h5file.close()
        self.assertRaises(
            tb.ClosedNodeError, self.table.__getitem__, 0)

    def test_setitem(self):
        data = self.table[0]
        self.h5file.close()
        self.assertRaises(
            tb.ClosedNodeError, self.table.__setitem__, 0, data)

    def test_append(self):
        data = self.table[0]
        self.h5file.close()
        self.assertRaises(
            tb.ClosedNodeError, self.table.append, data)
    ...
    def test_readSorted(self):
        self.h5file.close()
        self.assertRaises(
            tb.ClosedNodeError, self.table.read_sorted, 'var2')
...
def suite():
    ...
    for n in range(niter):
        ...
        theSuite.addTest(
            common.make_suite(OldRecordBasicWriteTestCase))
        ...
        theSuite.addTest(
            common.make_suite(RecArrayAlignedWriteTestCase))
        theSuite.addTest(
            common.make_suite(CompressBloscTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBlosc2TablesTestCase))
        ...
        theSuite.addTest(
            common.make_suite(CompressBloscLZ4TablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBlosc2LZ4TablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBloscLZ4HCTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBlosc2LZ4HCTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBloscSnappyTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBloscZlibTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBlosc2ZlibTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBloscZstdTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressBlosc2ZstdTablesTestCase))
        theSuite.addTest(common.make_suite(CompressLZOTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressLZOShuffleTablesTestCase))
        theSuite.addTest(common.make_suite(CompressZLIBTablesTestCase))
        theSuite.addTest(
            common.make_suite(CompressZLIBShuffleTablesTestCase))
        ...
        theSuite.addTest(
            common.make_suite(AlignedNoPaddingOpenCopyTestCase))
    ...
    if common.heavy:
        theSuite.addTest(
            common.make_suite(CompressBzip2TablesTestCase))
        theSuite.addTest(common.make_suite(
