...
from . import xsd
from .models import Header, Merchant, Buyer, Item, Disbursement, SmartBillResponse, EmailDetails
...
class SmartBillClient:
    # https://ws.smartbill.ro/SBORO/api/payment/text?cif={cif}&id={id}
    ...
    def __init__(self,
        ...
        """
        :param username: SmartBill.ro user
        :param password: SmartBill.ro’s user password
        :param token: SmartBill.ro / Token
        ...
        # username:token
        # text = f'{self.username}:{self.token}'
        # self.base64 = base64.b64encode()
        # self.base64 = 'Z3JhbmVjYUBjZW50cnVtLnJvOjAwM3xkMTY3MzI2YmI4Y2UwMWNlYzFjZDRiYzEyOTk0YTYzYQ=='
    ...
    def generate_invoice(self,
        ...
        if header.proforma_invoice:
            raise ValueError("Proforma invoice cant be sent as invoice")

        r = self.request_maker(
            template=templates.generate_invoice,
            template_data=payload_xml,
            xsd_xml=xsd.generate_invoice,
            url=f'{self.url}invoice',
        )
        response = SmartBillResponse(r, xml_namespace="{http://www.smartbill.ro/SBORO/api/invoice}")
        logger.info(f"success = {response.http_request_success}")
        if response.http_request_success == 'true':
    ...
    def generate_proforma_invoice(self,
        ...
        if not header.proforma_invoice:
            raise ValueError("Invoice cant be sent as proforma invoice")

        r = self.request_maker(
            template=templates.generate_proforma_invoice,
            template_data=payload_xml,
            xsd_xml=xsd.generate_proforma_invoice,
            url=f'{self.url}estimate',
        )
        response = SmartBillResponse(r, xml_namespace="{http://www.smartbill.ro/SBORO/api/estimate}")
        logger.info(f"success = {response.http_request_success}")
        if response.http_request_success == 'true':
            logger.info(f"invoice_number = {response.invoice_number}")
            logger.info(f"buyer_account_url = {response.buyer_account_url}")
    ...
    def reverse_invoice(self,
        ...
        r = self.request_maker(
            ...
            url=f'{self.url}invoice/reverse',
        )
        response = SmartBillResponse(r, xml_namespace="")
        logger.info(f"success = {response.http_request_success}")
        if response.http_request_success == 'true':
            logger.info(f"invoice_number = {response.invoice_number}")
            logger.info(f"buyer_account_url = {response.buyer_account_url}")
    ...
    def query_invoice_pdf(self,
        ...
        response = SmartBillResponse(r, xml_namespace="{http://www.szamlazz.hu/xmlszamlavalasz}")
        logger.info(f"success = {response.http_request_success}")
        logger.info(f"invoice_number = {response.invoice_number}")
        return response
    ...
    def query_invoice_xml(self,
        ...
        response = SmartBillResponse(r, xml_namespace="{http://www.szamlazz.hu/szamla}")
        logger.info(f"success = {response.http_request_success}")
        logger.info(f"invoice_number = {response.invoice_number}")
        return response
    ...
    def delete_pro_forma_invoice(self,
        ...
        response = SmartBillResponse(r, xml_namespace="{http://www.szamlazz.hu/xmlszamladbkdelvalasz}")
        logger.info(f"success = {response.http_request_success}")
        logger.info(f"invoice_number = {response.invoice_number}")
        return response
...
# pass payload to generate_receipt(payload=payload)
    ...
    def reverse_receipt(self,
        ...
        response = SmartBillResponse(r, xml_namespace="{http://www.szamlazz.hu/xmlnyugtavalasz}")
        logger.info(f"success = {response.http_request_success}")
        logger.info(f"invoice_number = {response.invoice_number}")
        logger.info(f"buyer_account_url = {response.buyer_account_url}")
        return response
    ...
    def send_receipt(self,
        ...
        response = SmartBillResponse(r, xml_namespace="{http://www.szamlazz.hu/xmlnyugtasendvalasz}")
        logger.info(f"success = {response.http_request_success}")
        logger.info(f"invoice_number = {response.invoice_number}")
        logger.info(f"buyer_account_url = {response.buyer_account_url}")
        return response
    ...
    def request_maker(self, template: str, template_data: dict, xsd_xml: str = "", payload_extra_attachments: dict = None, url = None, method:str = "POST") -> Response:
        """
        ...
        logger.info(f"request_maker / Payload xml: {output}")
        ...
        if method == 'POST':
            return requests.post(url, auth=authentication, headers=headers, data=payload.encode('utf-8'))
        elif method == 'PUT':
            return requests.put(url, auth=authentication, headers=headers, files=payload)
        elif method == 'GET':
            return requests.get(url, auth=authentication, headers=headers, files=payload)
        else:
            raise AssertionError(f"reqiest method not supported: {method}")
