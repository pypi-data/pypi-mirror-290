...
class MetaCli(type):
    ...
    def with_client(f, client_cls):
        ...
        def wrapper(self: "OpenAPICli", *args, **kwargs):
            try:
                return f(*args, **kwargs, client=self.get_api_client(client_cls))
    ...
    @staticmethod
    def with_transform_body(func, body_cls):
    ...
    def add_to_click(cls, config: CliConfig, namespace, func: T, value, name) -> T:
        ...
        func = cls.print_result(cls.with_self(func))
    ...
    def add_self_to_static_commands(cls, namespace: dict[str, Any]) -> None:
        ...
        for k, v in namespace.items():
            if isinstance(v, Command):
                v.callback = cls.with_self(v.callback)
