...
from jjsystem.subsystem.capability_functionality.resource \
    import CapabilityFunctionality
from jjsystem.subsystem.capability_module.resource import CapabilityModule
from jjsystem.subsystem.domain import tasks
from jjsystem.common.subsystem import operation, manager
from jjsystem.subsystem.domain.resource import Domain
from jjsystem.subsystem.functionality.resource import FunctionalityRoute
from jjsystem.subsystem.image.resource import QualityImage
from jjsystem.subsystem.domain.files import utils
from jjsystem.common.utils import round_abnt
from jjsystem.subsystem.module.resource import ModuleFunctionality
...
class SendEmailActivateAccount(operation.Create):
    ...
    def do(self, session, **kwargs):
        ...
        if not domains:
            raise exception.NotFound('Domain not found')
        ...
        if not users:
            raise exception.NotFound('Invalid information')
...
class GetUsageInfoByDomain(operation.List):
    ...
    def _get_disk_space_totals_by_db(self, session):
        from jjsystem.subsystem.domain.sql.disk_space_totals_by_db \
            import QUERY
    ...
    def _valid_weights(self):
        ...
        if total > 1:
            raise exception.BadRequest(
                'the sum of the general weights must total 100%')
    ...
    def pre(self, **kwargs):
        ...
        if precision is not None:
            try:
                ...
                raise exception.BadRequest(
                    'the precision parameter must be an integer')
...
class List(operation.List):
    ...
    def do(self, session, **kwargs):
        ...
        query = self.driver.apply_order_by(query, **kwargs)
