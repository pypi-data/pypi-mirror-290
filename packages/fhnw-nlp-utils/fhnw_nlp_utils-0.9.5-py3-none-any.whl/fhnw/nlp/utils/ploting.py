...
def plot_confusion_matrix(y_true, y_pred, label2index=None, title='', percentage=True, filename=None):
    ...
    if label2index is None:
        classes = list(set(get_unique_elements(y_true) + get_unique_elements(y_pred)))
        classes.sort()
    ...
    if is_list_like(y_true[0]):
        ...
        clazzes = [["others", clazz] for clazz in classes]
    else:
...
def report_classification_results(params, y_true, y_pred):
    ...
    if path is not None:
        ...
        plot_confusion_matrix(y_true, y_pred, label2index=label2index, filename=path_confusion_matrix)
    else:
        plot_confusion_matrix(y_true, y_pred, label2index=label2index)
    ...
    if path is not None:
        ...
        report = classification_report(y_true, y_pred, labels=list(label2index.values()), target_names=list(label2index.keys()))
        print(report)
