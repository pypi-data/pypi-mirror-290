...
class FilePath(Path):
    ...
    patterns = {
        ...
        FileType.IMAGE_TILE: {
            "regex": r"VOI_(\d+).X(\d+?).Y(\d+?).Z(\d+?).C(\d+)-T(\d+)",
            "groups": ["voi", "X", "Y", "Z","channel","time"],
    ...
    registered_metadata = ["provider", "channel", "frame", "time", "filetype"]
    ...
    def __init__(self, *args):
        ...
        self.metadata = self.extract_meta_data(self)
    ...
    def is_nirvana_filetype(cls, filepath):
        ...
        """
        metadata = cls.extract_meta_data(filepath)
    ...
    @classmethod
    def extract_meta_data(cls, filepath):
        """
        ...
        for filetype, matching_info in cls.patterns.items():
    ...
    def delete_xattr(self, prefix="bliq.nirvana"):
        ...
        for key, _ in self.metadata.items():
            if key != "filepath":
                job = subprocess.run(["xattr", "-d", f"{prefix}.{key}", str(self)], check=True)
                if job.returncode != 0:
                    raise RuntimeError(
...
class TestClasses(unittest.TestCase):
    ...
    def test_check_metadata(self):
        ...
        metadata = nirvana_file.extract_meta_data(self.filepath)
