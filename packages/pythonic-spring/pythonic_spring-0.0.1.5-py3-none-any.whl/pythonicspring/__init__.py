...
class SpringApplication(FastAPI):
    __instance__ = None
    ...
    def __init__(self, scan_regex: Optional[List[str]] = None, *args, **kwargs):
        if not hasattr(self, '__bean_factory__'):
    ...
    def get_bean_by_type(self, cls: str):
        result = self.get_beans_by_type(cls)
        if len(result) == 0:
            return None
        if len(result) == 1:
            return result[0]
        else:
            raise Exception("There are more than one beans of type {}".format(cls))
...
def Service(cls=None, *, bean_id=None):
    if global_injector.is_spring_application_mode():
        if cls is None:
