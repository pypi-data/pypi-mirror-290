...
class OnesPlugin(NotificationPlugin):
  ...
  def create_ones_issue(self, group, event, **kwargs):
     ...
     json_group_string = json.dumps(group, indent=4, ensure_ascii=False,default=self.custom_serializer)
     json_event_string = json.dumps(event, indent=4, ensure_ascii=False,default=self.custom_serializer)
     print('json_group_string',json_group_string)
     print('json_event_string',json_event_string)
     config = {
