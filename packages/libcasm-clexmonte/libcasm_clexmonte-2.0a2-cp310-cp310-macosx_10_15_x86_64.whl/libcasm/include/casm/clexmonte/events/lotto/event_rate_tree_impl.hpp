...
EventRateNodeData<EventIDType>::EventRateNodeData(const EventIDType &event_id,
                                                  double rate)
...
template <typename EventIDType>
EventRateNodeData<EventIDType> EventRateNodeData<EventIDType>::operator+(
    const EventRateNodeData<EventIDType> &rhs_node) const {
...
template <typename EventIDType>
EventRateNodeData<EventIDType>::EventRateNodeData()
    : event_id{std::nullopt}, rate(0.0){};
...
template <typename EventIDType>
EventRateTree<EventIDType>::EventRateTree(
    const std::vector<EventIDType> &all_event_ids,
...
template <typename EventIDType>
const EventIDType &EventRateTree<EventIDType>::query_tree(
...
template <typename EventIDType>
void EventRateTree<EventIDType>::update_rate(const EventIDType &event_id,
                                             double new_rate) {
...
EventRateTree<EventIDType>::event_to_leaf_index_map() const {
  ...
  for (Index leaf_ix = 0; leaf_ix < tree_leaves.size(); ++leaf_ix) {
    const EventIDType &leaf_event_id =
        tree_leaves[leaf_ix].get()->data.get_event_id();
...
template <typename EventIDType>
const Node<EventIDType> *EventRateTree<EventIDType>::bifurcate(
    const Node *current_node_ptr, double &running_rate) const {
  if (current_node_ptr->left_child == nullptr &&
      current_node_ptr->right_child == nullptr) {
    return nullptr;
  }
...
template <typename EventIDType>
std::vector<NodeData<EventIDType> > EventRateTree<EventIDType>::leaf_data()
    const {
  std::vector<NodeData> leaf_data;
  const auto &tree_leaves = this->event_rate_tree.leaves();
  for (auto leaf_it = tree_leaves.begin(); leaf_it != tree_leaves.end();
       ++leaf_it) {
