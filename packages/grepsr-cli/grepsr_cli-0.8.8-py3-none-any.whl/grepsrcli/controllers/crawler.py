...
class Crawler(Controller):
    ...
    def create(self):
        ...
        if self.app.pargs.init:
            ...
            site_link = ask_user_input('Enter Website Link: ')
            scrape_category = ask_user_input('Enter Scrape Category Type if any: ', allow_empty=True)
            pid = ask_user_input('Enter Project ID: ', allow_empty=True)
            pid = first_found_numbers(pid)
            col_headers_input: str = ask_user_input('Enter column headers if any (comma/newline seperated): ', allow_empty=True, raw_input=True)
        ...
        data = {
            ...
            'host_name': parsed_url.hostname if parsed_url else '',
        ...
        if template == 'vc':
            ...
            create_boilerplate(plugin_path, "node_boilerplate.jinja2", data, 'js')
            render_boilerplate("node_package_json.jinja2", data, path.join(plugin_path, 'package.json'))
        elif template == 'brp':
