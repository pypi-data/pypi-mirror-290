...
def listen_commands(book: CrewRoster, notes: SeaNotes, on_exit: callable):
    try:
        print("Ahoy, matey! Welcome aboard the Pirate Ledger!")
        ...
        while True:
            ...
            if command in (Commands.EXIT, Commands.CLOSE):
                on_exit(book, notes)
                print("Fair winds and following seas! Farewell!")
                break

            elif command == Commands.HELLO:
                print("What be yer orders, captain?")

            elif command == Commands.HELP:
                print("\n- ".join(["Here be the commands ye can give:", *commands]))
                ...
                print("Arrr, that be an invalid command, matey! Try again.")
