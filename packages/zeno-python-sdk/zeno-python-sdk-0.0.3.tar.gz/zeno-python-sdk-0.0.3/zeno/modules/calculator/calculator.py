from zeno.constants.common import BIE8, BPS, DAYS
import math
...
class Calculator:
  ...
  @staticmethod
  def get_borrowing_rate(asset_class_config, asset_class, zlp_tvl):
    return Calculator.get_next_borrowing_rate_without_interval(
      asset_class_config["base_borrowing_rate"], asset_class["reserve_value_e30"], zlp_tvl)

  @staticmethod
  def get_next_borrowing_rate(base_borrowing_rate: int, reserve_value_e30: int, zlp_tvl: int, block_timestamp: int, last_borrowing_time: int, funding_interval: int):
    if last_borrowing_time + funding_interval > block_timestamp or zlp_tvl == 0:
      return 0
    borrowing_rate = Calculator.get_next_borrowing_rate_without_interval(
      base_borrowing_rate, reserve_value_e30, zlp_tvl)
    intervals = (block_timestamp - last_borrowing_time) - funding_interval
    return borrowing_rate * intervals

  @staticmethod
  def get_next_borrowing_rate_without_interval(base_borrowing_rate: int, reserve_value_e30: int, zlp_tvl: int):
    if zlp_tvl == 0:
      return 0
    return base_borrowing_rate * reserve_value_e30 // zlp_tvl
  ...
  def get_trading_fee(size: int, base_fee_rate_bps: int, skew: int, adaptive_fee_info, maker_taker_fee_info):
    if adaptive_fee_info == None:
      return base_fee_rate_bps

    if maker_taker_fee_info == None:
      return Calculator.get_adaptive_fee_e8(size, base_fee_rate_bps, adaptive_fee_info)
