...
class Algorithm(Generic[InputType, OutputType], metaclass=ABCMeta):
    def __init__(self, inputs: InputType):
        self.logger = structlog.getLogger(f"{self.name}@{self.version}")
    ...
    @final
    def execute(self):
        structlog.contextvars.bind_contextvars(sample_id=self.inputs.sample_id)
        ...
        try:
            ...
            if self.inputs.handle_exceptions:
        ...
        if self.inputs.publish:
            self.outputs.publish_files(self.inputs.publish_dir, logger=self.logger)

        self.logger.info("Execution of algorithm finished")
        execution_report.to_logger(logger=self.logger)
        self.qc_report.to_logger(logger=self.logger)

        self.inputs.unstage_files()

        structlog.contextvars.unbind_contextvars("sample_id")
