...
class BookkeepingWatchAgent(AgentModule, TransformationAgentsUtilities):
        ...
        while True:  # not self.bkQueriesToBeChecked.empty():
            ...
            try:
                ...
                for lfnChunk in breakListIntoChunks(files, self.chunkSize):
                    ...
                    self._logVerbose(f"Got metadata from BK for {len(lfnChunk)} files", transID=transID, reftime=start)
                    if not res["OK"]:
                        self._logError(
                            f"Failed to get BK metadata for {len(lfnChunk)} files", res["Message"], transID=transID
                        )
                        ...
                        if invalid:
                            self._logError(
                                "Invalid LFNs returned by getFileMetadata", ",".join(sorted(invalid)), transID=transID
                            )
                            for inv in invalid:
                                success.pop(inv)
    ...
    def __getFiles(self, transID, bkQuery, now):
        ...
        if not result["OK"]:
            ...
            if result["Value"]:
                self._logInfo(f"Obtained {len(result['Value'])} files from BK", transID=transID)
            return result["Value"]
    ...
    def __addRunsMetadata(self, transID, runsList):
        ...
        newRuns = list(set(runsList) - set(runsInCache["Value"]))
        if newRuns:
            self._logVerbose(f"Associating run duration to {len(newRuns)} runs", transID=transID)
