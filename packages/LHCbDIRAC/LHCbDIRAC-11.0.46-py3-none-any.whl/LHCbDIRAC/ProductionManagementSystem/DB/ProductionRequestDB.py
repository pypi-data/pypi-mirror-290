...
class ProductionRequestDB(DB):
    ...
    def __prefixComments(update, old, user):
        ...
        if not update.startswith(old) and not update.endswith(old):
            return update
        prefix = f"Comment by {user} on {time.strftime('%b %d, %Y')}: "
    ...
    def __getRequestInfo(self, iD, connection):
        ...
        result = self._query(
            f"SELECT {','.join(inFields)} " + "FROM ProductionRequests " + f"WHERE RequestID={iD};", conn=connection
        )
        if not result["OK"]:
            self.lock.release()
            return result
    ...
    def createProductionRequest(self, requestDict, creds):
        ...
        if rec["RequestState"]:
            if not rec["RequestState"] in ["New", "BK Check", "Submitted"]:
                return S_ERROR(f"The request can't be created in '{rec['RequestState']}' state")
        ...
        if rec["RequestState"]:
            result = self._update(
                ...
                + f") VALUES ( {requestID},'{str(rec['RequestState'])}','{str(rec['RequestAuthor'])}')",
                conn=connection,
            )
            if not result["OK"]:
    ...
    def getProductionRequest(
        self, requestIDList, subrequestsFor=0, sortBy="", sortOrder="ASC", offset=0, limit=0, filterIn=None
    ):
        ...
        req = f"SELECT {fields} ,COUNT(sr.RequestID) AS HasSubrequest "
        ...
        if requestIDList:
            ...
            where = f"t.RequestID IN ({idlist})"
        else:
            if subrequestsFor:
                where = f"t.ParentID={subrequestsFor}"
            else:
                where = sfilter
                if not idFilter:
                    if where:
        ...
        if sortBy:
            ...
            order = f" ORDER BY {sortBy} {sortOrder}"
            req += order
        if limit and not subrequestsFor:
            req += f" LIMIT {offset},{limit}"
        ...
        total = len(rows)
        if limit:
            result = self._query(f"SELECT COUNT(*) FROM ProductionRequests AS t WHERE {where}")
            if not result["OK"]:
                return result
    ...
    def __checkUpdate(self, update, old, creds, connection):
        ...
        inform = {
            "rec": rec,
            "state": "",
            "author": str(requestAuthor),
            "oldstate": requestState,
            "inform": requestInform,
        }

        hasSubreq = False
        if not old["MasterID"]:
            result = self._query(
                "SELECT RequestID " + "FROM ProductionRequests " + f"WHERE MasterID={requestID}", conn=connection
            )
            if not result["OK"]:
                self.lock.release()
                return result
        ...
        if requestState in ["New", "BK OK", "Rejected"]:
            ...
            self.lock.release()
            return S_ERROR(f"The request is in unknown state '{requestState}'")
            ...
            if requestState == "New":
                for x in update:
                    if x not in ["EventType", "NumberOfEvents", "RealNumberOfEvents", "Comments"]:
                        self.lock.release()
                        return S_ERROR(f"{x} is not allowed in subrequests")
        ...
        if requestState == "New":
            ...
            for x in update:
                if x not in [
                    ...
                    self.lock.release()
                    return S_ERROR(f"{x} can't be modified during BK check")
            if "RequestState" not in update:
                return S_OK(inform)
            ...
            for x in update:
                if x not in ["RequestState", "Comments", "Inform", "Extra"]:
                    self.lock.release()
                    return S_ERROR(f"{x} can't be modified after BK check")
            if "RequestState" not in update:
                return S_OK(inform)
            ...
            if creds["Group"] == "lhcb_ppg":
                for x in update:
                    if x not in (
                        ...
                        self.lock.release()
                        return S_ERROR(f"{x} can't be modified during PPG signing")
                if "RequestState" not in update:
                    return S_OK(inform)
            ...
            if creds["Group"] == "lhcb_tech":
                for x in update:
                    if x not in ["RequestState", "Comments", "Inform", "ProPath", "ProID", "ProDetail", "Extra"]:
                        self.lock.release()
                        return S_ERROR(f"{x} can't be modified during Tech signing")
                if "RequestState" not in update:
                    return S_OK(inform)
                if update["RequestState"] == "Accepted":
        ...
        #        return S_ERROR("Registered processing pass is required to sign for Tech OK")
            ...
            for x in update:
                if x not in ["RequestState", "Comments", "Inform", "ProPath", "ProID", "ProDetail", "Extra"]:
                    self.lock.release()
                    return S_ERROR(f"{x} can't be modified during the progress")
            if "RequestState" not in update:
                return S_OK(inform)
    ...
    def updateProductionRequest(self, requestID, requestDict, creds):
        ...
        req = f"SELECT {fields} "
        req += "FROM ProductionRequests as t "
        req += f"WHERE t.RequestID={requestID}"
        result = self._query(req, conn=connection)
        if not result["OK"]:
            self.lock.release()
            return result
        ...
        for x in rec:
            if x in ("ProDetail", "SimCondDetail", "Extra"):
                ...
                try:
                    ...
                    if x == "SimCondDetail":
                        # Parameters without values are discarded by Web part,
                        # extra checks are for backward compatibility
                        if "inSMOG2State" in oldx and oldx["inSMOG2State"] is None and "inSMOG2State" not in recx:
                            recx["inSMOG2State"] = None
                        if "inExtendedDQOK" in oldx and oldx["inExtendedDQOK"] is None and "inExtendedDQOK" not in recx:
                            recx["inExtendedDQOK"] = None
                    if recx == oldx:
                        continue
                except TypeError:
        ...
        if "NumberOfEvents" in update:  # Update RealNumberOfEvents if specified
            ...
            try:
                ...
                gLogger.warn(f"NumberOfEvents not defined for {requestID}")
            update["RealNumberOfEvents"] = str(num)

        if "Comments" in update:
            update["Comments"] = self.__prefixComments(update["Comments"], old["Comments"], creds["User"])
        ...
        for dateValue in self.dateColumns:
            ...
            if dateValue in self.dateColumns and requestDict.get(dateValue):
                ...
                # my MySQL does not like time part in strict mode...
                dateOnly = str(requestDict.get(dateValue)).split(" ")[0]
                updateValues.append(f"STR_TO_DATE('{dateOnly}','{self.dateFormat}')")
        updates = ",".join([x + "=" + y for x, y in zip(recl_fields, updateValues)])

        req = "UPDATE ProductionRequests "
        req += f"SET {updates} "
        req += f"WHERE RequestID={requestID}"
        result = self._update(req, conn=connection)
        if not result["OK"]:
            self.lock.release()
            return result
        ...
        if "RequestState" in update:
            result = self._update(
                "INSERT INTO RequestHistory ("
                + ",".join(self.historyFields[:-1])
                + f") VALUES ( {requestID},'{str(update['RequestState'])}','{str(creds['User'])}')",
                conn=connection,
            )
            if not result["OK"]:
                gLogger.error(result["Message"])
    ...
    def __getSubrequestsList(self, iD, master, connection):
        ...
        """
        result = self._query(
            "SELECT RequestID " + "FROM ProductionRequests " + f"WHERE ParentID={iD} and MasterID={master}",
            conn=connection,
        )
        if not result["OK"]:
            self.lock.release()
    ...
    def deleteProductionRequest(self, requestID, creds):
        ...
        if creds["Group"] not in ["diracAdmin", "lhcb_admin"]:
            if requestAuthor != creds["User"] and not (isModel and creds["Group"] == "lhcb_tech"):
                self.lock.release()
                gLogger.error(f"{creds['User']} can't remove {requestAuthor} request")
        ...
        if not masterID:  # this request is a master
            # delete history
            result = self._update("DELETE FROM RequestHistory " + f"WHERE RequestID={requestID}")
            if not result["OK"]:
                self.lock.release()
                return result
            # delete tracking
            result = self._update(
                ...
                + f"WHERE RequestID={requestID} OR MasterID={requestID}"
                + ")",
                conn=connection,
            )
            if not result["OK"]:
            ...
            req += f"WHERE MasterID={requestID}"
            ...
            result = self._update(
                "DELETE FROM ProductionProgress "
                + f"WHERE RequestID IN ({','.join([str(x) for x in rlist] + [str(requestID)])})"
            )
            if not result["OK"]:
                self.lock.release()
                return result
            if len(rlist):
                req = "DELETE FROM ProductionRequests "
                req += f"WHERE RequestID in ({','.join([str(x) for x in rlist])})"
        if req:
            result = self._update(req, conn=connection)
            if not result["OK"]:
                self.lock.release()
        ...
        # move substructure
        req = f"UPDATE ProductionRequests SET ParentID={upperID} "
        req += f"WHERE ParentID={requestID}"
        result = self._update(req, conn=connection)
        if not result["OK"]:
            self.lock.release()
            return result
    ...
    def __duplicateDeep(self, requestID, masterID, parentID, creds, connection, clearpp):
        ...
        try:
            ...
            gLogger.warn(f"NumberOfEvents is not defined for {requestID}")
        ...
        if rec["RequestState"]:
            result = self._update(
                "INSERT INTO RequestHistory ("
                + ",".join(self.historyFields[:-1])
                + f") VALUES ( {newRequestID},'{str(rec['RequestState'])}','{str(rec['RequestAuthor'])}')",
                conn=connection,
            )
            if not result["OK"]:
                gLogger.error(result["Message"])
        ...
        req += "FROM ProductionRequests as t "
        req += f"WHERE t.ParentID={requestID}"
        result = self._query(req, conn=connection)
        if not result["OK"]:
            self.lock.release()
            return result
    ...
    def duplicateProductionRequest(self, requestID, creds, clearpp):
        ...
        if masterID:
            ...
            if not result["OK"]:
                return result
            requestState, requestAuthor, _requestInform, isModel = result["Value"]
            if requestState != "New" or (
                requestAuthor != creds["User"] and not (isModel and creds["Group"] == "lhcb_tech")
            ):
                self.lock.release()
    ...
    def __moveChildDeep(self, requestID, masterID, setParent, connection):
        ...
        if not setParent:
            raise NotImplementedError("This is not supported and probably never be")
        req = "UPDATE ProductionRequests "
        req += f"SET ParentID={str(masterID)},MasterID={str(masterID)} "
        req += f"WHERE RequestID=ParentID={str(masterID)},MasterID={str(masterID)}"
        result = self._update(req, conn=connection)
        if not result["OK"]:
            return result
        ...
        for ch in result["Value"]:
            if ch[1]:
                ...
                if not ret["OK"]:
                    gLogger.error(f"_moveChildDeep: can not move to {str(masterID)}: {ret['Message']}")
    ...
    def splitProductionRequest(self, requestID, splitlist, creds):
        ...
        if not rsplitlist:
            req = "DELETE FROM ProductionRequests "
            req += f"WHERE RequestID={str(newRequestID)}"
            result = self._update(req, conn=connection)
            self.lock.release()
        ...
        req = "SELECT " + ",".join(self.historyFields) + f" FROM RequestHistory WHERE RequestID={requestID} "
        req += "ORDER BY TimeStamp"
        result = self._query(req, conn=connection)
        if not result["OK"]:
            gLogger.error(f"SplitProductionRequest: can not get history for {str(requestID)}: {result['Message']}")
        else:
            for x in result["Value"]:
                x = list(x)
                x[0] = newRequestID
    ...
    def getProductionProgress(self, requestID):
        ...
        already checked."""
        req = f"SELECT * FROM ProductionProgress WHERE RequestID={requestID}"
        result = self._query(req)
        if not result["OK"]:
            return result
    ...
    def removeProductionFromRequest(self, productionID):
        ...
        req += f"WHERE ProductionID={str(productionID)}"
    ...
    def getRequestHistory(self, requestID):
        ...
        req += f" FROM RequestHistory WHERE RequestID={requestID} "
        req += "ORDER BY TimeStamp"
        result = self._query(req)
        if not result["OK"]:
            return result
    ...
    def updateTrackedProductions(self, update):
        ...
        for x in update:
            result = self._update(
                ...
                + f"SET BkEvents={x['BkEvents']} "
                + f"WHERE ProductionID={x['ProductionID']}"
            )
            if not result["OK"]:
                gLogger.info(f"Problem in updating progress. Not fatal: {result['Message']}")
        return S_OK("")
    ...
    def updateTrackedInput(self, update):
        ...
        for x in update:
            if not x["RequestID"] in allowed:
                ...
                continue
            req = "UPDATE ProductionRequests "
            req += f"SET RealNumberOfEvents={str(x['RealNumberOfEvents'])} "
            req += f"WHERE RequestID={str(x['RequestID'])}"
            result = self._update(req)
            if not result["OK"]:
                skiped.append(x["RequestID"])
        if skiped:
            return S_ERROR(f"updateTrackedInput has skiped requests {','.join(skiped)}")
        return S_OK("")

    def getProductionList(self, requestID):
        """return a list of all productions associated with the request or any its
        subrequest."""
        req1 = f"SELECT RequestID FROM ProductionRequests WHERE MasterID={requestID}"
        req = "SELECT ProductionID FROM ProductionProgress WHERE RequestID "
        req += f"in ({req1}) OR RequestID={requestID}"
    ...
    def getFilterOptions(self):
        ...
        for key, value in [
            ...
            req = f"SELECT DISTINCT {value} FROM ProductionRequests "
            req += f"WHERE {value} IS NOT NULL "
            req += f"ORDER BY {value}"
            res = self._query(req)
            if not res["OK"]:
                return res
