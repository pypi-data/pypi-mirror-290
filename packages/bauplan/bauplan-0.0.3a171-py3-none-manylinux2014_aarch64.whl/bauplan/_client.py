...
from .schema import APIBranch, APIResponse, Ref, Table, TableField, TableWithMetadata
...
class Client:
    ...
    **Handling Exceptions**
    ...
    Run/Query/Scan operations raise a subclass of ``bauplan.exceptions.BauplanError`` that represents, and also return a ``RunState`` object containing details and logs:

        * ``RunError`` e.g. something went wrong in a run/query/scan; includes error details
    ...
    def query(
        ...
        try:
            ...
            if hasattr(e, 'details'):
                raise exceptions.RunError(e.details()) from e
            raise exceptions.RunError(e) from e
    ...
    def get_branches(
        self,
        itersize: Optional[str] | None = None,
    ...
    def plan_import(
        self, table_name: str, branch_name: str, search_string: str, args: Optional[Dict] | None = None
    ) -> PlanImportState:
        """
        ...
        try:
            return self._import.plan(
                search_string=search_string, table=table_name, branch=branch_name, args=args
            )
        except grpc._channel._InactiveRpcError as e:
            if hasattr(e, 'details'):
                raise exceptions.RunError(e.details()) from e
    ...
    def apply_import(
        self, plan_yaml: str, branch_name: str, table_name: str, args: Optional[Dict] | None = None
    ) -> ApplyPlanState:
        """
        ...
        .. code-block:: python
            ...
            s3_path = 's3://path/to/my/files/*.parquet'
            plan_state = client.plan_import(s3_path)
            if plan_state.error:
                plan_error_action(...)
        ...
        :param branch_name: name of the branch on which to apply the plan
        :param table_name: name of the table on which to apply the plan
        :param args: dict of arbitrary args to pass to the backend
        """
        try:
