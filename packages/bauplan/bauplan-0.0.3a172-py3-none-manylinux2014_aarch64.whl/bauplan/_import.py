...
class _Import(_OperationContainer):
    ...
    def apply(
        ...
        The user needs to specify one between a path to a file, or pass directly a yml string
        as the plan in case there is no file.
        """
        # make sure the inviariants are met
        assert plan_file or plan_yaml, 'plan_file or plan_yaml is required'

        if not isinstance(onto_branch, str) or not onto_branch.strip():
            raise ValueError("branch is required, e.g. 'myname.mybranch'")

        # if we have a file, not a yml string
        if not plan_yaml:
            if not os.path.exists(plan_file):
                raise ValueError(f"Plan file '{plan_file}' does not exist")
            plan_yaml = Path(plan_file).read_text()

        # now we have a yml string, either way, passed by the user
        # or read from a file
