...
class GDPSyncroniserHelper2(object):
    ...
    def DoSync(self, definition, workspace=None, startEditing=False, startOperation=False, with_undo=False,
               multiuser=False):
        ...
        self.AddMessage(u'>>>>Start the synchronization of the tables {0} and {1}'.format(self.definition.inTable,
                                                                                          self.definition.outTable))
        ...
        if inputAsTableObject:
            ...
            self.AddMessage(
                u'>>>>>>>>Join {0}.{1}={2}.{3}'.format(self.definition.inTable, self.definition.inTableJoinField,
                                                       self.definition.outTable, self.definition.outTableJoinField))
    ...
    def __SyncSourceToDestination_TableObject(self):
        ...
        for inRow in self.definition.inTableObject:
            ...
            with self.gp.da.UpdateCursor(self.definition.outTable, self.definition.outTableFields,
                                         u'{0} = {2}{1}{2}'.format(self.definition.outTableJoinField, syncItem.id,
                                                                   self.definition.idvalueseparator)) as outCur:
                recordsFoundCount = 0
                for outRow in outCur:
                    ...
                    if recordsFoundCount == 0 or self.definition.allowMultiple:
                        ...
                        syncItem.id = inRow[0]
                        errCount = errCount + 1
                        syncItem.error = u"Error: row in the results table is not unique " + self.definition.messageDefinition.format(
                            *syncItem.inSyncRow)
            ...
            if recordsFoundCount == 0 and not self.definition.createNew:
                errCount = errCount + 1
                syncItem.error = u"Error: no related records found in the results table " + self.definition.messageDefinition.format(
                    *syncItem.inSyncRow)
                self.AddMessage(u'        ...{0}'.format(syncItem.error))
                syncItem.id = inRow[0]
                syncItem.ClerRowInfo()
            ...
            if processedCountOperation >= 1000:
                if self.isTool:
                    self.AddMessage(
                        u'>>>> ...Processed [{0}] from [{1}], stored [{2}], faulty [{3}] ({4})'.format(processedCount,
                                                                                                       recordCount,
                                                                                                       updatedCount,
                                                                                                       errCount,
                                                                                                       T.GetTimeReset()))
        ...
        self.AddMessage(
            u'>>>> ...Processed [{0}], stored [{1}], faulty [{2}]'.format(processedCount, updatedCount, errCount))
        return rezultList
    ...
    def __SyncSourceToDestination(self):
        ...
        # get summary record count
        recordCount = 0
        with self.gp.da.SearchCursor(self.definition.inTable, self.definition.inTableFields,
                                     self.definition.inTableQuery) as cur:
            for row in cur:
        ...
        with self.gp.da.SearchCursor(self.definition.inTable, self.definition.inTableFields,
                                     self.definition.inTableQuery) as inCur:
            T = TimerHelper.TimerHelper()
            for inRow in inCur:
                syncItem = SyncItem2()
                syncItem.inSyncRow = inRow
                ...
                if processedCountOperation >= 1000:
                    if self.isTool:
                        self.AddMessage(u'>>>> ...Processed [{0}] from [{1}], stored [{2}], faulty [{3}] ({4})'.format(
                            processedCount, recordCount, updatedCount, errCount, T.GetTimeReset()))
                    processedCountOperation = 0
                    if self.startOperation:
                        self.edit.stopOperation()
                    if self.startOperation:
    ...
    def __SyncDestinationToSource(self):
        ...
        # get summary record count
        recordCount = 0
        with self.gp.da.SearchCursor(self.definition.outTable, self.definition.outTableFields,
                                     self.definition.outTableQuery) as cur:
            for row in cur:
                recordCount = recordCount + 1
        self.AddMessage(u'>>>> ...Records to process {0}'.format(recordCount))
        ...
        with self.gp.da.UpdateCursor(self.definition.outTable, self.definition.outTableFields,
                                     self.definition.outTableQuery) as outCur:
            T = TimerHelper.TimerHelper()
            for outRow in outCur:
                syncItem = SyncItem2()
                syncItem.outSyncRow = outRow
                ...
                with self.gp.da.SearchCursor(self.definition.inTable, self.definition.inTableFields,
                                             u'{0} = {2}{1}{2}'.format(self.definition.inTableJoinField, syncItem.id,
                                                                       self.definition.idvalueseparator)) as inCur:
                    recordsFoundCount = 0
                    for inRow in inCur:
                        # print "rec {0} - {1}".format(u'{0} = {2}{1}{2}'.format(self.definition.outTableJoinField, syncItem.id, self.definition.idvalueseparator), (recordsFoundCount == 0 or self.definition.allowMultiple))
                        if recordsFoundCount == 0:
                            ...
                            syncItem.id = outRow[0]
                            errCount = errCount + 1
                            syncItem.error = u"Error: row in the source table is not unique " + self.definition.messageDefinition.format(
                                *syncItem.outSyncRow)
                            self.AddMessage(u'        ...{0}'.format(syncItem.error))
                            syncItem.ClerRowInfo()
                            syncItem.syncResult = 'error'
                ...
                if recordsFoundCount == 0:
                    errCount = errCount + 1
                    syncItem.error = u"Error: no related records found in the source table " + self.definition.messageDefinition.format(
                        *syncItem.outSyncRow)
                    self.AddMessage(u'        ...{0}'.format(syncItem.error))
                    syncItem.ClerRowInfo()
                    syncItem.syncResult = 'error'
