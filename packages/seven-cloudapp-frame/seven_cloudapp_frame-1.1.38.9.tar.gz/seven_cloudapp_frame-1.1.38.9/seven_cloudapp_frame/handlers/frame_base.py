...
@LastEditTime: 2024-05-17 10:18:34
...
class FrameBaseHandler(FrameBaseApiHandler):
    ...
    def get_app_id(self):
        ...
        if plat_type == PlatType.tb.value:
            ...
            if not app_id:
                if request_source_type == 2: # 2-server(服务端)
                    ...
                    if user_nick:
                        ...
                        app_info_dict = AppInfoModel(context=self).get_cache_dict(where=condition_where.to_string(), limit="1", field="app_id", params=params)
            ...
            if request_source_type == 1 and (app_id == share_config.get_value("client_template_id") or app_id == ""):
                test_config = share_config.get_value("test_config", {})
    ...
    def prepare(self):
        ...
        try:
            ...
            if is_api_encrypt == True:
                ...
                if self.request_params.__contains__("timestamp"):
                    if (now_time - int(self.request_params["timestamp"]) > encrypt_limit_time) or (int(self.request_params["timestamp"]) - now_time > encrypt_limit_time):
