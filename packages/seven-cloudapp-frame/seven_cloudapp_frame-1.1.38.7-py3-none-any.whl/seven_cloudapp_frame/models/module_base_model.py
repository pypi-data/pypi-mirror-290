...
@LastEditTime: 2024-02-02 09:22:17
...
class ModuleBaseModel():
    ...
    def get_act_module_list(self,app_id,act_id,module_name,start_date,end_date,is_del,page_size,page_index,order_by="sort_index desc,id asc",condition="",params=[],is_cache=True):
        ...
        if is_cache:
            page_list, total = ActModuleModel(context=self.context).get_cache_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition_where.to_string(), group_by="", order_by=order_by, params=params_list,dependency_key=DependencyKey.act_module_list(act_id))
        else:
