...
@LastEditTime: 2024-07-29 10:15:21
...
class CacheModel(FrameDbModel):
    ...
    def __get_redis_config(self, config_dict=None):
        """
    ...
    def __redis_init(self, config_dict=None, decode_responses=True):
        ...
        :last_editors: HuangJianYi
        """
        redis_cli = RedisExHelper.init(config_dict=self.__get_redis_config(config_dict), decode_responses=decode_responses)
        return redis_cli

    def check_is_cluster(self, config_dict=None):
        """
        :Description: 判断是否是集群
        :return: 是否是集群
        :last_editors: HuangJianYi
        """
        check_config_dict = self.__get_redis_config(config_dict)
    ...
    def get_dependency_key(self, dependency_key=''):
        ...
        if self.is_cluster == True:
            return key + ":" + config.get_value("project_name", "default")
        return key
    ...
    def get_dependency_cache(self, dependency_key, cache_key, redis_config_dict=None):
        ...
        if share_config.get_value("is_redis_cache",True) == False:
            return None
        self.check_is_cluster(redis_config_dict)
        ...
        if not cache_data:
            ...
            limit_size = share_config.get_value("cache_limit_size", 1024) # 单位KB
    ...
    def set_dependency_cache(self, dependency_key, cache_key, value, cache_expire, redis_config_dict=None):
        ...
        if self.is_cluster == True:
            ...
            if not dependency_date_value:
                pipeline.set(dependency_key,str(TimeHelper.get_now_format_time()),ex=share_config.get_value("dependency_cache_expire",5*60*60))
