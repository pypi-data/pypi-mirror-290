...
@LastEditTime: 2024-03-20 11:26:33
...
class AppBaseModel():
    ...
    def get_app_id(self, store_name, template_id=""):
        ...
        app_info_dict = AppInfoModel(context=self.context).get_cache_dict(condition_where.to_string(), limit="1", field="app_id", params=params)
        if app_info_dict:
    ...
    def get_app_info_result(self, user_nick, open_id, access_token, app_key, app_secret, main_user_open_id=""):
        ...
        base_info = BaseInfoModel(context=self.context).get_dict(field="client_ver,project_ver")
        if not base_info:
            invoke_result_data.success = False
            invoke_result_data.error_code = "error"
        ...
        if app_info:
            ...
            app_info_model.update_entity(app_info,"expiration_date,project_code,access_token,main_user_open_id,store_name")
