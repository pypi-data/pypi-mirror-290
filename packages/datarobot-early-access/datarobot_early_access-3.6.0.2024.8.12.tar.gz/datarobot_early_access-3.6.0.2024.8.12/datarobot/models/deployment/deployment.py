...
class Deployment(APIObject, MonitoringDataQueryBuilderMixin, BrowserMixin):
    ...
    def get_capabilities(self) -> List[Capability]:
        ...
        """
        path = f"{self._path}{self.id}/capabilities/"
        response = self._client.get(path)
        data = response.json()["data"]
        return cast(List["Capability"], from_api(list(data), keep_null_keys=True))
    ...
    def get_moderation_events(
        ...
        List[MLOpsEvent]
        """
        path = f"{self._path}{self.id}/events/"
        params = dict(limit=limit, offset=offset, event_category="moderation")
        ...
        return [MLOpsEvent.from_server_data(event) for event in data]
