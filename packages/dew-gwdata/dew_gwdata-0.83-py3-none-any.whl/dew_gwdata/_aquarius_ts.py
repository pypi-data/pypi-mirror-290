...
def convert_aq_timestamp(ts):
    ...
    if ts_sub[:4] == "0001":
        ...
        return None
    return pd.Timestamp(datetime.datetime.strptime(ts_sub, "%Y-%m-%d%H:%M:%S.%f"))
...
class DEWAquarius:
    ...
    def _find_locations_from_api(self, kwargs):
        ...
        df = pd.io.json.json_normalize(resp.json()["LocationDescriptions"])
        if len(df) == 0:
            df = pd.DataFrame(
                columns=[
                    "Name",
    ...
    def fetch_timeseries_data(
        ...
        **kwargs,
        ...
        if ts_uid is None:
            ...
            if len(loc_ts) == 0:
                # return [pd.DataFrame(
                #     columns=["timestamp", param, "grade", "approval", "qualifier"]
                # )]
                return []
            elif len(loc_ts) == 1:
...
def get_swims_metadata_connection():
    conn = pyodbc.connect(
        "Driver={SQL Server};"
        "Server=sql2016-prod.env.sa.gov.au;"
