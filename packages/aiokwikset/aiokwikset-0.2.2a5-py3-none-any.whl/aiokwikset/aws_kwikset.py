...
def pad_hex(long_int):
    """
    Converts a Long integer (or hex string) to hex format padded with zeroes
    for hashing
...
class AWSKWIKSET(object):

    PASSWORD_VERIFIER_CHALLENGE = 'PASSWORD_VERIFIER'
    CUSTOM_VERIFIER_CHALLENGE = 'CUSTOM_CHALLENGE'

    def __init__(self, username, password, code_type, pool_id, client_id,
    ...
    def get_password_authentication_key(self, username, password,
                                        server_b_value, salt):
        """
        Calculates the final hkdf based on computed S value, and computed U
            value and the key
        ...
        username_password = '%s%s:%s' % (self.pool_id.split('_')[1],
                                         username, password)
        ...
        s_value = pow(int_value2, self.small_a_value + u_value * x_value,
                      self.big_n)
    ...
    def get_auth_params(self):
        auth_params = {'PASSWORD': self.password, 'USERNAME': self.username,
    ...
    def get_secret_hash(username, client_id, client_secret):
        ...
        hmac_obj = hmac.new(bytearray(client_secret, 'utf-8'), message,
                            hashlib.sha256)
    ...
    def process_custom_challenge(self):
        response = {'ANSWER': 'answerType:generateCode,medium:{},codeType:login'.format(self.code_type),"USERNAME": self.username}
