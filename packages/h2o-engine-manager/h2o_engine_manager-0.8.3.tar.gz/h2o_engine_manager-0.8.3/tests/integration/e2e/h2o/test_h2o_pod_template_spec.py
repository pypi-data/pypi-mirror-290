...
def test_pod_template_spec_gpu_toleration_applied(h2o_engine_client):
    """
    ...
    # When created H2OEngine with customized H2OSetup.
    create_h2o_engine(
    ...
    try:
        ...
        h2o_engine_client.client_info.api_instance.h2_o_engine_service_delete_h2_o_engine(
            name_2=f"workspaces/{workspace_id}/h2oEngines/{engine_id}"
        )


def create_h2o_engine(
    h2o_engine_client: H2OEngineClient, workspace_id: str, engine_id: str, gpu: int = 0
) -> H2OEngine:
...
def get_h2o_pods(namespace: str, workspace_id: str, engine_id: str) -> List[V1Pod]:
    ...
    for i in range(node_count):
        pod = wait_until_get_h2o_pod(
            pod_namespace=namespace, pod_name=f"engine-{h2o['metadata']['uid']}-{i}"
        )
        h2o_pods.append(pod)
