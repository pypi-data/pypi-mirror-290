...
@pytest.fixture(scope="session")
def dai_setup_client_standard_user(clients) -> DAISetupClient:
    return clients.dai_setup_client


@pytest.fixture(scope="session")
def dai_setup_client_super_admin(super_admin_clients) -> DAISetupClient:
    return super_admin_clients.dai_setup_client


@pytest.fixture(scope="session")
def h2o_setup_client_standard_user(clients) -> H2OSetupClient:
    return clients.h2o_setup_client


@pytest.fixture(scope="session")
def h2o_setup_client_super_admin(super_admin_clients) -> H2OSetupClient:
    return super_admin_clients.h2o_setup_client
...
@pytest.fixture(scope="function")
def internal_dai_version(internal_dai_version_client):
    v = internal_dai_version_client.create_version(
        internal_dai_version_id="1.10.6",
        image="dai-1.10.6",
    )
    id = v.internal_dai_version_id

    yield v

    internal_dai_version_client.delete_version(internal_dai_version_id=id)


@pytest.fixture(scope="function")
def internal_h2o_version(internal_h2o_version_client):
    v = internal_h2o_version_client.create_version(
        internal_h2o_version_id="3.40.0.5",
        image="h2o-3.40.0.5",
    )
    id = v.internal_h2o_version_id

    yield v

    internal_h2o_version_client.delete_version(internal_h2o_version_id=id)
