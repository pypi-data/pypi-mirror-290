...
def serve_file(directory: str, filename: str):
    """
    Serves a file from a specified directory and filename.

    Checks for the file in both the provided directory and its parent directory.
    ...
    Returns:
        FileResponse: A file response object if the file is found.

    Raises:
        HTTPException: If the file is not found, raises a 404 HTTPException.
    """
    directory = directory.rstrip("/")
    parent_dir = directory.rsplit("/", 1)[0]

    # If the directory is the same as one folder up, it means the directory is the root directory
    if parent_dir == directory:
        parent_dir = "."

    filenames = [f"{directory}/{filename}", f"{parent_dir}/{filename}"]
