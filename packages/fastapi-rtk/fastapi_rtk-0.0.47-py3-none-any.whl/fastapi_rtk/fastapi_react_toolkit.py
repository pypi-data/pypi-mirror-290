...
from .const import BASE_APIS, DEFAULT_STATIC_FOLDER, logger
...
from .utils import safe_call, serve_file, smart_run
...
class FastAPIReactToolkit:
    ...
    Args:
        ...
        `instrumentator` (Instrumentator | None, optional): The instrumentator to use for monitoring the FastAPI application. Defaults to Instrumentator(**g.config.get("INSTRUMENTATOR_CONFIG", {})).
    ...
    def initialize(self, app: FastAPI) -> None:
        ...
        if not self.instrumentator:
            self.instrumentator = Instrumentator(
                **g.config.get("INSTRUMENTATOR_CONFIG", {})
            )
        self.instrumentator.instrument(
            app=self.app, **g.config.get("INSTRUMENTATOR_INSTRUMENT_CONFIG", {})
        )
    ...
    def _mount_static_folder(self):
        ...
        def favicon():
            return serve_file(static_folder, "favicon.ico")

        @self.app.get("/{full_path:path}", include_in_schema=False)
        def index():
            return serve_file(static_folder, "index.html")
    ...
    def _init_lifespan(self):
        ...
        async def lifespan(app: FastAPI):
            ...
            self.instrumentator.expose(
                app, **g.config.get("INSTRUMENTATOR_EXPOSE_CONFIG", {})
            )
