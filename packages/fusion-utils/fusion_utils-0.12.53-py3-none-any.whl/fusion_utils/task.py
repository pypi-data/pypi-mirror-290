...
class Task:
    ...
    def get_condition(self):
        ...
        try:
            ...
            # Test the lambda function with a dummy DataFrame to ensure it returns a boolean
            test_df = pd.DataFrame()  # Empty DataFrame just for testing
            result = condition_func(test_df)
            
            # Ensure the result is a boolean
            if not isinstance(result, bool):
                raise ValueError(f"The condition does not evaluate to a boolean: {self.condition_str}")
