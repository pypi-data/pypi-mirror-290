...
def read_candidate_by_name(name: str):
   ...
   candidates = Candidate.find(Candidate.name.lower()==name.lower()).all()
   if candidates:
...
def read_candidate_by_pi(name: str, email:str):
   ...
   candidates = Candidate.find(Candidate.name.lower()==name.lower()).all()
   if candidates:
      samecandidates = [c for c in candidates if c.email.lower()==email.lower()]
      if samecandidates:
...
def read_cv_evaluation_by_candidate(candidate_pk: str, company: str=None, jobdesc: str=None):
      ...
      if evaluations:
         if company and jobdesc: 
            evaluations = [e for e in evaluations if e.company==company and e.jobdesc==jobdesc]
         elif company:
            evaluations = [e for e in evaluations if e.company==company]
         elif jobdesc:
