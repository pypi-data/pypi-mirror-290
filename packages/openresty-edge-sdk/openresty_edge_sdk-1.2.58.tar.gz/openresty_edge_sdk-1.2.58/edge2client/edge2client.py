...
class Edge2Client(object):
    ...
    def put_app_config(self, app_id=None, limiter=None, enable_websocket=None, access_log=dict()):
        http_verb = 'PUT'
        if not app_id:
            raise Exception('no active application selected')
    ...
    def new_release(self, app_type="http", app_id=None):
        ...
        return self.do_api('POST', url, {'comment': 'a new release'})
    ...
    def put_rule(self, rule_id=None, http_verb='PUT',
                 ...
                 top=0, last=None, order=0, reorder=False,
    ...
    def get_app_dymetrics_data(self, app_id, id, chart_type='line', start_time=None, end_time=None, node_id=None):
        if not app_id:
            raise Exception('no active application selected')
        ...
        url = "{}?type=app&id={}&chart_type={}&start_time={}&end_time={}{}".format(
            DymetricsDataUrl, metric_id, chart_type, start_time, end_time, node_id_arg)
    ...
    def get_global_ip_list(self, rule_id):
        if not rule_id:
            raise Exception('no active IP list selected')

        return self.do_api(
            'GET', '{}/{}?detail=1'.format(GlobalIPListUrl, rule_id))

    def get_all_global_ip_lists(self):
        url = GlobalIPListUrl + '?detail=1'

        ip_lists = self.do_api('GET', url)
        if isinstance(ip_lists, list):
            return ip_lists

        if isinstance(ip_lists, dict):
            ip_lists = self.get_all(url, True)
    ...
    def get_ip_list(self, rule_id=None):
        if not self.app_id:
            raise Exception('no active application selected')
        if not rule_id:
            raise Exception('no active IP List selected')

        return self.do_api('GET', AppIPListUrl.format(self.app_id) + '/' +
                           str(rule_id) + '?detail=1')

    def get_all_ip_lists(self):
        if not self.app_id:
            raise Exception('no active application selected')

        url = AppIPListUrl.format(self.app_id) + '?detail=1'
