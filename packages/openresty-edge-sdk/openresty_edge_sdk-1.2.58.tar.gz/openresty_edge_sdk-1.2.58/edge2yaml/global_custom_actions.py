...
def check_global_custom_actions(filename, actions):
    for action in actions:
        ...
        if 'conditions' not in action or not isinstance(action['conditions'], list):
            error(f"Missing or invalid 'conditions' in global custom action {action['name']}, filename: {filename}, line: {line(action)}")
...
def is_changed(ctx, new_rule, old_rule):
    ...
    if new_rule['name'] != old_rule['name']:
        return True
    if new_rule['conditions'] != old_conditions:
        return True
...
def export_global_custom_actions(ctx):
    ...
    for action in actions:
        ...
        formatted_action = {
            'name': action['name'],
            'conditions': export_conditions(ctx, action['conditions']),
            'actions': new_actions
        }
