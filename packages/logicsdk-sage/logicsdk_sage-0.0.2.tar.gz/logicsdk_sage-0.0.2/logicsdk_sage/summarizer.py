...
class BedrockSummarizer:
    def __init__(self, region_name='us-east-1', default_model_name='sonnet-3.5', aws_access_key_id=None,
        ...
        self.is_sagemaker = self._is_running_on_sagemaker()
        self.bedrock = self._get_bedrock_client()

    def _is_running_on_sagemaker(self):
        return 'SAGEMAKER_CHANNEL_TRAINING' in os.environ

    def _get_bedrock_client(self):
        try:
            if self.is_sagemaker:
                logger.info("Running on SageMaker. Using instance role for authentication.")
                return boto3.client(service_name="bedrock-runtime", region_name=self.region_name)
            elif self.aws_access_key_id and self.aws_secret_access_key:
                logger.info("Using provided or environment AWS credentials for authentication.")
                session = boto3.Session(
                    aws_access_key_id=self.aws_access_key_id,
                    aws_secret_access_key=self.aws_secret_access_key,
                    region_name=self.region_name
                )
                logger.info("Successfully created AWS session with credentials.")
                return session.client(service_name="bedrock-runtime")
            else:
                # Try to use default credentials (AWS CLI configuration, EC2 instance role, etc.)
                logger.info("Attempting to use default AWS credentials.")
                return boto3.client(service_name="bedrock-runtime", region_name=self.region_name)
        except botocore.exceptions.NoCredentialsError:
            logger.error("No AWS credentials found. Please set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables or provide them explicitly.")
            raise ValueError("AWS credentials are required. Please set environment variables or provide them explicitly.")
        except botocore.exceptions.PartialCredentialsError as e:
            logger.error(f"Incomplete AWS credentials: {str(e)}")
            raise ValueError(f"Incomplete AWS credentials: {str(e)}")
    ...
    def _get_user_friendly_error(self, error):
        if isinstance(error, ValueError):
            ...
            return "No AWS credentials found. Please set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables or provide them explicitly."
        elif isinstance(error, botocore.exceptions.PartialCredentialsError):
            return f"Incomplete AWS credentials: {str(error)}"
        else:
