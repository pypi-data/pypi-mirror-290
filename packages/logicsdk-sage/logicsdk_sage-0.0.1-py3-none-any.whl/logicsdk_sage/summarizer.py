...
class BedrockSummarizer:
    ...
    def _get_bedrock_client(self):
        try:
            if self.is_sagemaker:
                ...
                logger.error("AWS credentials are not set. Please provide AWS credentials when running locally.")
                raise ValueError("AWS credentials are required when running locally.")
    ...
    def _get_user_friendly_error(self, error):
        if isinstance(error, ValueError):
            ...
            return "No AWS credentials found. Please provide valid credentials when running locally."
        else:
