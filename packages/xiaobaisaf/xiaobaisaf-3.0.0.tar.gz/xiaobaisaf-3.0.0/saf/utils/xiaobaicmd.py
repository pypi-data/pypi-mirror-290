...
@click.option(
    "--rewrite", "-r", default=1, type=int, nargs=1, help="自动生成PageObject是否重写"
)
@click.option(
    "--monitor",
    "-m",
...
def main(template, dirname, domains, url, rewrite, monitor, device):
    if template:
        if "web" == template.lower():
            ...
            copytree(
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), r"../example/api"
                ),
            ...
            copytree(
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), r"../example/app"
                ),
