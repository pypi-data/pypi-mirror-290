...
class SettingsUI(object):
    def __init__(self):
        ...
        self._download_urls = ["https://archive.apache.org/dist/jmeter/binaries/"]
...
class SoftwareManagerUI(tk.Tk):
    ...
    def remove_thread(self):
        ...
        if version in ["未选择...", ""]:
            ...
            try:
                remove_jmeter_path = self.setting.installed_paths[
    ...
    def SET_JMETER_INSTALLED_VERSION(self) -> None:
        """在指定安装路径下获取JMeter的列表"""
        _t = threading.Thread(
            target=self.finder_thread, args=(self.setting.install_path,)
        )
        # _t.setDaemon(True)
        _t.start()
        _t.join()
