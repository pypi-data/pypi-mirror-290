...
class RequestCodeTemplate:
    ...
    def getCode(self):
        if self.__type__() == 'fetch(nodejs)2requests(base)':
            return self.FetchToRequestsOfBaseHeader() + self.FetchToRequestsOfBaseRequest() if self.first \
                else self.FetchToRequestsOfBaseRequest()
        elif self.__type__() == 'fetch(nodejs)2requests(pytest)':
            return self.FetchToRequestsOfPytestHeader() + self.FetchToRequestsOfPytestRequest() if self.first \
                else self.FetchToRequestsOfPytestRequest()
        elif self.__type__() == 'fetch(nodejs)2requests(pytest_allure)':
            return self.FetchToRequestsOfPytestAllureHeader() + self.FetchToRequestsOfPytestAllureRequest() if self.first \
                else self.FetchToRequestsOfPytestAllureRequest()

    def FetchToRequestsOfBaseHeader(self):
...
'''
    ...
    def FetchToRequestsOfBaseRequest(self):
...
'''
    ...
    def FetchToRequestsOfPytestHeader(self):
        return f'''#! /usr/bin/env python
##################################
#       requests(PyTest)         #
#       author: xiaobaiTser      #
...
'''
    ...
    def FetchToRequestsOfPytestRequest(self):
        _path = urlparse(self.url).path
        func_name_fix = _path.split('/')[-1] if _path not in ['', '/'] else 'index'
        func_name_fix = re.sub(r'\W', '_', func_name_fix)
        return f'''
...
'''
    ...
    def FetchToRequestsOfPytestAllureHeader(self):
        return f'''#! /usr/bin/env python
##################################
...
'''
    ...
    def FetchToRequestsOfPytestAllureRequest(self):
        _path = urlparse(self.url).path
        func_name_fix = _path.split('/')[-1] if _path not in ['', '/'] else 'index'
        func_name_fix = re.sub(r'\W', '_', func_name_fix)
        return f'''
